...
/**
 * Caches frequently used names to facilitate reuse. (example: byte[] representation of the file
 * name in {@link INode}).
 *
 * <p>This class is used by initially adding all the file names. Cache tracks the number of times a
 * name is used in a transient map. It promotes a name used more than {@code useThreshold} to the
 * cache.
 *
 * <p>One all the names are added, {@link #initialized()} should be called to finish initialization.
 * The transient map where use count is tracked is discarded and cache is ready for use.
 *
 * <p>This class must be synchronized externally.
 *
...
class NameCache<K> {
   ...
   * Constructor
   *
   * @param useThreshold names occurring more than this is promoted to the cache
   */
  ...
  /**
   * Add a given name to the cache or track use count. exist. If the name already exists, then the
   * internal value is returned.
   *
  ...
  /**
   * Mark the name cache as initialized. The use count is no longer tracked and the transient map
   * used for initializing the cache is discarded to save heap space.
   */
  void initialized() {
