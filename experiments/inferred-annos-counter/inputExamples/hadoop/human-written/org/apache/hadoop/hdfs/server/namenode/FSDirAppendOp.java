...
final class FSDirAppendOp {
  ...
  /** Private constructor for preventing FSDirAppendOp object creation. Static-only class. */
  private FSDirAppendOp() {}

  /**
   * Append to an existing file.
   *
   * <p>The method returns the last block of the file if this is a partial block, which can still be
   * used for writing more data. The client uses the returned block locations to form the data
   * pipeline for this block.<br>
   * The {@link LocatedBlock} will be null if the last block is full. The client then allocates a
   * new block with the next call using {@link
   * org.apache.hadoop.hdfs.protocol.ClientProtocol#addBlock}.
   *
   * <p>For description of parameters and exceptions thrown see {@link
   ...
   */
  static LastBlockWithStatus appendFile(
      ...
      if (file.isStriped() && !newBlock) {
        throw new UnsupportedOperationException(
            "Append on EC file without new block is not supported. Use "
                ...
                + " create flag while appending file.");
      }
    ...
    if (lb != null) {
      NameNode.stateChangeLog.debug(
          "DIR* NameSystem.appendFile: file {} for {} at {} block {} block" + " size {}",
          srcArg,
          holder,
          clientMachine,
          lb.getBlock(),
  ...
  static LocatedBlock prepareFileForAppend(
    ...
    if (writeToEditLog) {
      ...
      if (NameNodeLayoutVersion.supports(
          Feature.APPEND_NEW_BLOCK, fsn.getEffectiveLayoutVersion())) {
  ...
  /**
   * Verify quota when using the preferred block size for UC block. This is usually used by append
   * and truncate.
   *
   * @throws QuotaExceededException when violating the storage quota
   * @return expected quota usage update. null means no change or no need to update quota usage
   *     later
   */
  private static QuotaCounts verifyQuotaForUCBlock(
      FSNamesystem fsn, INodeFile file, INodesInPath iip) throws QuotaExceededException {
    FSDirectory fsd = fsn.getFSDirectory();
  ...
  private static QuotaCounts computeQuotaDeltaForUCBlock(FSNamesystem fsn, INodeFile file) {
    ...
    if (lastBlock != null) {
      ...
      final BlockStoragePolicy policy =
          fsn.getFSDirectory().getBlockStoragePolicySuite().getPolicy(file.getStoragePolicyID());
