...
class SnapshotDiffInfo {
            ...
            if (right == null) {
              ...
              return cmp == 0
                  ? SignedBytes.lexicographicalComparator()
                      .compare(left.getLocalNameBytes(), right.getLocalNameBytes())
                  : cmp;
            }
          }
        }
  ...
  /**
   * A map recording modified INodeFile and INodeDirectory and their relative path corresponding to
   * the snapshot root. Sorted based on their names.
   */
  private final SortedMap<INode, byte[][]> diffMap = new TreeMap<INode, byte[][]>(INODE_COMPARATOR);
  /**
   * A map capturing the detailed difference about file creation/deletion. Each key indicates a
   * directory whose children have been changed between the two snapshots, while its associated
   * value is a {@link ChildrenDiff} storing the changes (creation/deletion) happened to the
   * children (files).
   */
    ...
    for (Map.Entry<INode, byte[][]> drEntry : diffMap.entrySet()) {
      ...
      if (node.isDirectory()) {
        List<DiffReportEntry> subList =
            generateReport(dirDiffMap.get(node), path, isFromEarlier(), renameMap);
        diffReportList.addAll(subList);
      }
    }

    SnapshotDiffReport.DiffStats dStats =
        new SnapshotDiffReport.DiffStats(
            this.totalDirsCompared,
            this.totalDirsProcessed,
            this.totalFilesCompared,
            this.totalFilesProcessed,
   ...
   * @param fromEarlier True indicates {@code diff=later-earlier}, False indicates {@code
   *     diff=earlier-later}
  ...
  private List<DiffReportEntry> generateReport(
      ChildrenDiff dirDiff,
      byte[][] parentPath,
      boolean fromEarlier,
      Map<Long, RenameEntry> renameMap) {
