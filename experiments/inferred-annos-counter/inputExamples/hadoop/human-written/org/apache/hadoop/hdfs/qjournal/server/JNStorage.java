 ...
 *
 * <p>The JN has a storage directory for each namespace for which it stores metadata. There is only
 * a single directory per JN in the current design.
 */
class JNStorage extends Storage {
   ...
   */
  protected JNStorage(
      Configuration conf, File logDir, StartupOption startOpt, StorageErrorReporter errorReporter)
      throws IOException {
  ...
  /**
   * Find an edits file spanning the given transaction ID range. If no such file exists, an
   * exception is thrown.
   */
  File findFinalizedEditsFile(long startTxId, long endTxId) throws IOException {
    File ret =
        new File(sd.getCurrentDir(), NNStorage.getFinalizedEditsFileName(startTxId, endTxId));
    if (!ret.exists()) {
  ...
  /**
   * @return the path for an in-progress edits file starting at the given transaction ID. This does
   *     not verify existence of the file.
   */
   ...
   * @param epoch the epoch number of the writer which is coordinating recovery
   * @return the temporary path in which an edits log should be stored while it is being downloaded
   *     from a remote JournalNode
   */
  ...
  /**
   * Directory {@code edits.sync} temporarily holds the log segments downloaded through {@link
   * JournalNodeSyncer} before they are moved to {@code current} directory.
   *
  ...
  File getTemporaryEditsFile(long startTxId, long endTxId) {
    return new File(
        getEditsSyncDir(),
        String.format(
            "%s_%019d-%019d", NNStorage.NameNodeFile.EDITS.getName(), startTxId, endTxId));
  }
  ...
  /**
   * @return the path for the file which contains persisted data for the paxos-like recovery process
   *     for the given log segment.
   */
  ...
  void purgeDataOlderThan(long minTxIdToKeep) throws IOException {
    ...
    purgeMatching(getOrCreatePaxosDir(), PAXOS_DIR_PURGE_REGEXES, minTxIdToKeep);
  }

  /**
   * Purge files in the given directory which match any of the set of patterns. The patterns must
   * have a single numeric capture group which determines the associated transaction ID of the file.
   * Only those files for which the transaction ID is less than the <code>minTxIdToKeep</code>
   * parameter are removed.
   */
  private static void purgeMatching(File dir, List<Pattern> patterns, long minTxIdToKeep)
      throws IOException {
  ...
  void checkConsistentNamespace(NamespaceInfo nsInfo) throws IOException {
    if (nsInfo.getNamespaceID() != getNamespaceID()) {
      throw new IOException(
          "Incompatible namespaceID for journal "
              + this.sd
              + ": NameNode has nsId "
              + nsInfo.getNamespaceID()
              + " but storage has nsId "
              + getNamespaceID());
    }

    if (!nsInfo.getClusterID().equals(getClusterID())) {
      throw new IOException(
          "Incompatible clusterID for journal "
              + this.sd
              + ": NameNode has clusterId '"
              + nsInfo.getClusterID()
              + "' but storage has clusterId '"
              + getClusterID()
              + "'");
    }
  }
