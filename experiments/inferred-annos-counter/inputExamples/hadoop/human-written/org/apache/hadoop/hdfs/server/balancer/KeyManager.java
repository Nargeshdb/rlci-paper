...
public class KeyManager implements Closeable, DataEncryptionKeyFactory {
  ...
  public KeyManager(
    ...
    if (isBlockTokenEnabled) {
      ...
      LOG.info(
          "Block token params received from NN: update interval="
              + StringUtils.formatTime(updateInterval)
              + ", token lifetime="
              + StringUtils.formatTime(tokenLifetime));
      ...
      this.blockTokenSecretManager =
          new BlockTokenSecretManager(
              updateInterval, tokenLifetime, blockpoolID, encryptionAlgorithm, enableProtobuf);
  ...
  public Token<BlockTokenIdentifier> getAccessToken(
      ExtendedBlock eb, StorageType[] storageTypes, String[] storageIds) throws IOException {
    if (!isBlockTokenEnabled) {
      ...
      return blockTokenSecretManager.generateToken(
          null,
          eb,
    ...
    if (encryptDataTransfer) {
      synchronized (this) {
        if (encryptionKey == null || encryptionKey.expiryDate < timer.now()) {
          ...
          LOG.debug(
              "Generating new data encryption key because current key "
                  + (encryptionKey == null
                      ? "is null."
