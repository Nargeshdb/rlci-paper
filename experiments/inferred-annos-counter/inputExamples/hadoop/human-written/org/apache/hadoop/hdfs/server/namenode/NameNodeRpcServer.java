...
/**
 * This class is responsible for handling all of the RPC calls to the NameNode. It is created,
 * started, and stopped by {@link NameNode}.
 */
...
public class NameNodeRpcServer implements NamenodeProtocols {
  ...
  public NameNodeRpcServer(Configuration conf, NameNode nn) throws IOException {
    ...
    int maxDataLength = conf.getInt(IPC_MAXIMUM_DATA_LENGTH, IPC_MAXIMUM_DATA_LENGTH_DEFAULT);
    ...
    BlockingService lifelineProtoPbService =
        DatanodeLifelineProtocolService.newReflectiveBlockingService(lifelineProtoPbTranslator);
    ...
    BlockingService reconfigurationPbService =
    ...
    if (serviceRpcAddr != null) {
      ...
      DFSUtil.addPBProtocol(
          conf, ReconfigurationProtocolPB.class, reconfigurationPbService, serviceRpcServer);
      ...
      DFSUtil.addPBProtocol(
          conf, RefreshCallQueueProtocolPB.class, refreshCallQueueService, serviceRpcServer);
      DFSUtil.addPBProtocol(
          conf, GenericRefreshProtocolPB.class, genericRefreshService, serviceRpcServer);
      DFSUtil.addPBProtocol(
    ...
    if (lifelineRpcAddr != null) {
      ...
      int lifelineHandlerCount = conf.getInt(DFS_NAMENODE_LIFELINE_HANDLER_COUNT_KEY, 0);
      if (lifelineHandlerCount <= 0) {
        float lifelineHandlerRatio =
            conf.getFloat(
                DFS_NAMENODE_LIFELINE_HANDLER_RATIO_KEY,
                DFS_NAMENODE_LIFELINE_HANDLER_RATIO_DEFAULT);
        lifelineHandlerCount = Math.max((int) (handlerCount * lifelineHandlerRatio), 1);
      }
      lifelineRpcServer =
          new RPC.Builder(conf)
      ...
      DFSUtil.addPBProtocol(
          conf, DatanodeLifelineProtocolPB.class, lifelineProtoPbService, lifelineRpcServer);

      // Update the address with the correct port
      InetSocketAddress listenAddr = lifelineRpcServer.getListenerAddress();
      lifelineRPCAddress =
          new InetSocketAddress(lifelineRpcAddr.getHostName(), listenAddr.getPort());
    ...
    boolean enableStateContext =
        conf.getBoolean(
            DFS_NAMENODE_STATE_CONTEXT_ENABLED_KEY, DFS_NAMENODE_STATE_CONTEXT_ENABLED_DEFAULT);
    ...
    DFSUtil.addPBProtocol(
        conf, ReconfigurationProtocolPB.class, reconfigurationPbService, clientRpcServer);
    ...
    DFSUtil.addPBProtocol(
        conf, RefreshCallQueueProtocolPB.class, refreshCallQueueService, clientRpcServer);
    DFSUtil.addPBProtocol(
        conf, GenericRefreshProtocolPB.class, genericRefreshService, clientRpcServer);
    DFSUtil.addPBProtocol(
    ...
    clientRpcServer.addTerseExceptions(
    ...
    clientRpcServer.addSuppressedLoggingExceptions(
    ...
    int[] auxiliaryPorts = conf.getInts(DFS_NAMENODE_RPC_ADDRESS_AUXILIARY_KEY);
  ...
  @Override // NamenodeProtocol
  public void errorReport(NamenodeRegistration registration, int errorCode, String msg)
      throws IOException {
    checkNNStartup();
  ...
  @Override // NamenodeProtocol
  public NamenodeRegistration registerSubordinateNamenode(NamenodeRegistration registration)
      throws IOException {
    checkNNStartup();
    namesystem.checkSuperuserPrivilege();
  ...
  public HdfsFileStatus create(
    ...
    if (stateChangeLog.isDebugEnabled()) {
      stateChangeLog.debug(
          "*DIR* NameNode.create: file " + src + " for " + clientName + " at " + clientMachine);
    }
    if (!checkPathLength(src)) {
      throw new IOException(
          "create: Pathname too long.  Limit "
              + MAX_PATH_LENGTH
              + " characters, "
              + MAX_PATH_DEPTH
              + " levels.");
  ...
  public LastBlockWithStatus append(String src, String clientName, EnumSetWritable<CreateFlag> flag)
    ...
    if (stateChangeLog.isDebugEnabled()) {
      stateChangeLog.debug(
          "*DIR* NameNode.append: file " + src + " for " + clientName + " at " + clientMachine);
    }
    namesystem.checkOperation(OperationCategory.WRITE);
    CacheEntryWithPayload cacheEntry = RetryCache.waitForCompletion(retryCache, null);
    if (cacheEntry != null && cacheEntry.isSuccess()) {
  ...
  public LocatedBlock addBlock(
    ...
    LocatedBlock locatedBlock =
        namesystem.getAdditionalBlock(
            src, fileId, clientName, previous, excludedNodes, favoredNodes, addBlockFlags);
  ...
  public LocatedBlock getAdditionalDatanode(
    ...
    LocatedBlock locatedBlock =
        namesystem.getAdditionalDatanode(
            src,
            fileId,
            blk,
            existings,
  ...
  /**
   * The client has detected an error on the specified located blocks and is reporting them to the
   * server. For now, the namenode will mark the block as corrupt. In the future we might check the
   * blocks are actually corrupt.
   */
  ...
  public void updatePipeline(
    ...
    boolean success = false;
    try {
      namesystem.updatePipeline(
          clientName, oldBlock, newBlock, newNodes, newStorageIDs, cacheEntry != null);
      success = true;
    } finally {
      RetryCache.setState(cacheEntry, success);
    }
  ...
  public void commitBlockSynchronization(
      ...
      throws IOException {
    checkNNStartup();
    namesystem.commitBlockSynchronization(
        block,
        newgenerationstamp,
        newlength,
        closeFile,
  ...
  public boolean rename(String src, String dst) throws IOException {
    ...
    if (!checkPathLength(dst)) {
      throw new IOException(
          "rename: Pathname too long.  Limit "
              + MAX_PATH_LENGTH
              + " characters, "
              + MAX_PATH_DEPTH
              + " levels.");
  ...
  public boolean mkdirs(String src, FsPermission masked, boolean createParent) throws IOException {
    ...
    if (!checkPathLength(src)) {
      throw new IOException(
          "mkdirs: Pathname too long.  Limit "
              + MAX_PATH_LENGTH
              + " characters, "
              + MAX_PATH_DEPTH
              + " levels.");
    ...
    return namesystem.mkdirs(
        src, new PermissionStatus(getRemoteUser().getShortUserName(), null, masked), createParent);
  }
  ...
  public BatchedDirectoryListing getBatchedListing(
      String[] srcs, byte[] startAfter, boolean needLocation) throws IOException {
    checkNNStartup();
  ...
  public HdfsLocatedFileStatus getLocatedFileInfo(String src, boolean needBlockToken)
    ...
    return (HdfsLocatedFileStatus) namesystem.getFileInfo(src, true, true, needBlockToken);
  }
  ...
  public BatchedEntries<OpenFileEntry> listOpenFiles(long prevId) throws IOException {
    return listOpenFiles(
        prevId, EnumSet.of(OpenFilesType.ALL_OPEN_FILES), OpenFilesIterator.FILTER_PATH_DEFAULT);
  }
  ...
  public DatanodeCommand blockReport(
      ...
      final BlockReportContext context)
      throws IOException {
    checkNNStartup();
    verifyRequest(nodeReg);
    ...
    try {
      if (bm.checkBlockReportLease(context, nodeReg)) {
        for (int r = 0; r < reports.length; r++) {
          ...
          noStaleStorages =
              bm.runBlockOp(
                  () -> bm.processReport(nodeReg, reports[index].getStorage(), blocks, context));
        }
      }
    } catch (UnregisteredNodeException une) {
      LOG.debug("Datanode {} is attempting to report but not register yet.", nodeReg);
  ...
  public DatanodeCommand cacheReport(
      DatanodeRegistration nodeReg, String poolId, List<Long> blockIds) throws IOException {
    checkNNStartup();
    verifyRequest(nodeReg);
    if (blockStateChangeLog.isDebugEnabled()) {
      blockStateChangeLog.debug(
          "*BLOCK* NameNode.cacheReport: " + "from " + nodeReg + " " + blockIds.size() + " blocks");
    }
    ...
    if (blockStateChangeLog.isDebugEnabled()) {
      blockStateChangeLog.debug(
          "*BLOCK* NameNode.blockReceivedAndDeleted: "
              ...
              + nodeReg
              + " "
              + receivedAndDeletedBlocks.length
              + " blocks.");
    }
    final BlockManager bm = namesystem.getBlockManager();
  ...
  private void verifyRequest(NodeRegistration nodeReg) throws IOException {
    ...
    if (!expectedID.equals(id)) {
      LOG.warn(
          "Registration IDs mismatched: the "
              + nodeReg.getClass().getSimpleName()
              + " ID is "
              + id
              + " but the expected ID is "
              + expectedID);
  ...
  public synchronized void monitorHealth()
      throws HealthCheckFailedException, AccessControlException, IOException {
    checkNNStartup();
    nn.monitorHealth();
  }
  ...
  public String createSnapshot(String snapshotRoot, String snapshotName) throws IOException {
    ...
    if (!checkPathLength(snapshotRoot)) {
      throw new IOException(
          "createSnapshot: Pathname too long.  Limit "
              + MAX_PATH_LENGTH
              + " characters, "
              + MAX_PATH_DEPTH
              + " levels.");
  ...
  public SnapshotDiffReport getSnapshotDiffReport(
    ...
    SnapshotDiffReport report =
        namesystem.getSnapshotDiffReport(snapshotRoot, earlierSnapshotName, laterSnapshotName);
  ...
  public SnapshotDiffReportListing getSnapshotDiffReportListing(
    ...
    SnapshotDiffReportListing report =
        namesystem.getSnapshotDiffReportListing(
  ...
  @Override // ClientProtocol
  public long addCacheDirective(CacheDirectiveInfo path, EnumSet<CacheFlag> flags)
      throws IOException {
    checkNNStartup();
    namesystem.checkOperation(OperationCategory.WRITE);
    CacheEntryWithPayload cacheEntry = RetryCache.waitForCompletion(retryCache, null);
  ...
  @Override // ClientProtocol
  public void reencryptEncryptionZone(final String zone, final ReencryptAction action)
      throws IOException {
    checkNNStartup();
    namesystem.checkOperation(OperationCategory.WRITE);
    final CacheEntry cacheEntry = RetryCache.waitForCompletion(retryCache);
  ...
  @Override // ClientProtocol
  public BatchedEntries<ZoneReencryptionStatus> listReencryptionStatus(final long prevId)
      throws IOException {
    checkNNStartup();
  ...
  public void setErasureCodingPolicy(String src, String ecPolicyName) throws IOException {
    ...
    try {
      if (ecPolicyName == null) {
        ...
        LOG.trace("No policy name is specified, " + "set the default policy name instead");
      }
  ...
  public long getCurrentEditLogTxid() throws IOException {
    ...
    return namesystem.getEditLog().isOpenForWrite()
        ? namesystem.getEditLog().getLastWrittenTxId()
        : -1;
  }
        ...
        SecurityUtil.doAsLoginUser(
            new PrivilegedExceptionAction<EventBatchList>() {
              ...
              public EventBatchList run() throws IOException {
                return getEventBatchList(syncTxid, txid, log, readInProgress, maxEventsPerRPC);
              }
            });
    return ret;
  }

  private EventBatchList getEventBatchList(
      long syncTxid, long txid, FSEditLog log, boolean readInProgress, int maxEventsPerRPC)
      throws IOException {
    ...
    try {
      ...
      // will result
      LOG.info(
          "NN is transitioning from active to standby and FSEditLog "
    ...
    for (EditLogInputStream elis : streams) {
      ...
      try {
        ...
        while ((op = readOp(elis)) != null) {
          ...
          if (totalEvents >= maxEventsPerRPC
              || (syncTxid > 0 && op.getTransactionId() == syncTxid)) {
            // we're done
            breakOuter = true;
            break;
          }
  ...
  public AddErasureCodingPolicyResponse[] addErasureCodingPolicies(ErasureCodingPolicy[] policies)
    ...
    checkNNStartup();
    namesystem.checkSuperuserPrivilege();
    final CacheEntryWithPayload cacheEntry = RetryCache.waitForCompletion(retryCache, null);
    if (cacheEntry != null && cacheEntry.isSuccess()) {
      return (AddErasureCodingPolicyResponse[]) cacheEntry.getPayload();
    }
    boolean success = false;
    AddErasureCodingPolicyResponse[] responses = new AddErasureCodingPolicyResponse[0];
    try {
      responses = namesystem.addErasureCodingPolicies(policies, cacheEntry != null);
      success = true;
    } finally {
  ...
  public void enableErasureCodingPolicy(String ecPolicyName) throws IOException {
    ...
    boolean success = false;
    try {
      success = namesystem.enableErasureCodingPolicy(ecPolicyName, cacheEntry != null);
    } finally {
      RetryCache.setState(cacheEntry, success);
    }
  }
  ...
  public void disableErasureCodingPolicy(String ecPolicyName) throws IOException {
    ...
    boolean success = false;
    try {
      success = namesystem.disableErasureCodingPolicy(ecPolicyName, cacheEntry != null);
    } finally {
      RetryCache.setState(cacheEntry, success);
    }
  }
  ...
  public Long getNextSPSPath() throws IOException {
    ...
    StoragePolicySatisfyManager spsMgr = namesystem.getBlockManager().getSPSManager();
    StoragePolicySatisfierMode spsMode =
        (spsMgr != null ? spsMgr.getMode() : StoragePolicySatisfierMode.NONE);
    if (spsMode != StoragePolicySatisfierMode.EXTERNAL) {
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            "SPS service mode is {}, so external SPS service is "
                + "not allowed to fetch the path Ids",
            spsMode);
      }
      throw new IOException(
          "SPS service mode is "
