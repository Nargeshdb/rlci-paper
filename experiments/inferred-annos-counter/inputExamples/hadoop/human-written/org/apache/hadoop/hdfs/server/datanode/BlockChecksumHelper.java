...
/** Utilities for Block checksum computing, for both replicated and striped blocks. */
...
final class BlockChecksumHelper {
  ...
  abstract static class AbstractBlockChecksumComputer {
    ...
    AbstractBlockChecksumComputer(DataNode datanode, BlockChecksumOptions blockChecksumOptions)
        throws IOException {
  ...
  abstract static class BlockChecksumComputer extends AbstractBlockChecksumComputer {
    ...
    BlockChecksumComputer(
        DataNode datanode, ExtendedBlock block, BlockChecksumOptions blockChecksumOptions)
        throws IOException {
  ...
  static class ReplicatedBlockChecksumComputer extends BlockChecksumComputer {
    ...
    @Override
    @SuppressWarnings(
        "builder:contracts.postcondition") // FP depends on inaccessible variable: getChecksumIn()
                                           // and getMetadataIn() return this.checksumIn and
                                           // this.metadataIn (validated)
    @EnsuresCalledMethods(
        value = {"this.checksumIn", "this.metadataIn"},
        methods = {"close"})
      ...
      try {
        ...
        switch (type) {
          ...
          case COMPOSITE_CRC:
            ...
            break;
          default:
            throw new IOException(String.format("Unrecognized BlockChecksumType: %s", type));
        }
      } finally {
    ...
    private void computeMd5Crc() throws IOException {
      ...
      LOG.debug(
          "block={}, bytesPerCRC={}, crcPerBlock={}, md5out={}",
          getBlock(),
          getBytesPerCRC(),
          getCrcPerBlock(),
          md5out);
    ...
    private void computeCompositeCrc(long stripeLength) throws IOException {
      long checksumDataLength = Math.min(getVisibleLength(), getRequestLength());
      if (stripeLength <= 0 || stripeLength > checksumDataLength) {
        stripeLength = checksumDataLength;
      }

      CrcComposer crcComposer =
          CrcComposer.newStripedCrcComposer(getCrcType(), getBytesPerCRC(), stripeLength);
      ...
      if (partialChunkSize > 0) {
        if (isPartialBlk()) {
          ...
          crcComposer.update(
              partialChunkCrcBytes, 0, partialChunkCrcBytes.length, partialChunkSize);
        } else {
  ...
  static class BlockGroupNonStripedChecksumComputer extends AbstractBlockChecksumComputer {
    ...
    void compute() throws IOException {
      ...
      for (int idx = 0; idx < blkIndxLen; idx++) {
        liveDns.put(blockIndices[idx], new LiveBlockInfo(datanodes[idx], blockTokens[idx]));
      }
      long checksumLen = 0;
      for (int idx = 0; idx < numDataUnits && idx < blkIndxLen; idx++) {
        ...
        try {
          ...
          if (liveBlkInfo == null) {
            ...
            try {
              checksumBlock(block, idx, liveBlkInfo.getToken(), liveBlkInfo.getDn());
    ...
    /**
     * @param checksumLen The sum of bytes associated with the block checksum data being digested
     *     into a block-group level checksum.
     */
    private byte[] reassembleNonStripedCompositeCrc(long checksumLen) throws IOException {
      int numDataUnits = ecPolicy.getNumDataUnits();
      CrcComposer crcComposer = CrcComposer.newCrcComposer(getCrcType(), ecPolicy.getCellSize());
      ...
      for (long cellIndex = 0; cellIndex < numFullCells; ++cellIndex) {
        ...
        int cellCrc = CrcUtil.readInt(flatBlockChecksumData, checksumCursor);
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            "flatBlockChecksumData.length={}, numDataUnits={}, " + "checksumLen={}, digest={}",
    ...
    private ExtendedBlock getInternalBlock(int numDataUnits, int idx) {
      ...
      ExtendedBlock block =
          StripedBlockUtil.constructInternalBlock(
    ...
    private void checksumBlock(
        ...
        throws IOException {
      int timeout = 3000;
      try (IOStreamPair pair =
          getDatanode().connectToDN(targetDatanode, timeout, block, blockToken)) {
        ...
        BlockChecksumType groupChecksumType = getBlockChecksumOptions().getBlockChecksumType();
        switch (groupChecksumType) {
          ...
          case COMPOSITE_CRC:
            childOptions =
                new BlockChecksumOptions(BlockChecksumType.COMPOSITE_CRC, ecPolicy.getCellSize());
            break;
          default:
            throw new IOException("Unknown BlockChecksumType: " + groupChecksumType);
        }
        createSender(pair).blockChecksum(block, blockToken, childOptions);

        final DataTransferProtos.BlockOpResponseProto reply =
            DataTransferProtos.BlockOpResponseProto.parseFrom(PBHelperClient.vintPrefixed(pair.in));
        ...
        if (checksumData.hasCrcType()) {
          ...
          LOG.debug(
              "Retrieving checksum from an earlier-version DataNode: "
                  + "inferring checksum by reading first byte");
          ct = DataChecksum.Type.DEFAULT;
        }

        setOrVerifyChecksumProperties(
            blockIdx, checksumData.getBytesPerCrc(), checksumData.getCrcPerBlock(), ct);

        switch (groupChecksumType) {
          case MD5CRC:
            // read md5
            final MD5Hash md5 = new MD5Hash(checksumData.getBlockChecksum().toByteArray());
            md5.write(blockChecksumBuf);
            LOG.debug("got reply from datanode:{}, md5={}", targetDatanode, md5);
            break;
          case COMPOSITE_CRC:
            BlockChecksumType returnedType =
            ...
            if (returnedType != BlockChecksumType.COMPOSITE_CRC) {
              throw new IOException(
                  String.format(
                      "Unexpected blockChecksumType '%s', expecting COMPOSITE_CRC", returnedType));
            }
            byte[] checksumBytes = checksumData.getBlockChecksum().toByteArray();
            blockChecksumBuf.write(checksumBytes, 0, checksumBytes.length);
            if (LOG.isDebugEnabled()) {
              LOG.debug(
                  "got reply from datanode:{} for blockIdx:{}, checksum:{}",
                  targetDatanode,
                  blockIdx,
     ...
     *
     * @param errBlkIndex error index to be reconstructed and recalculate checksum.
     * @param blockLength number of bytes in the block to compute checksum.
     * @throws IOException
     */
    private void recalculateChecksum(int errBlkIndex, long blockLength) throws IOException {
      ...
      StripedReconstructionInfo stripedReconInfo =
          new StripedReconstructionInfo(blockGroup, ecPolicy, blockIndices, datanodes, errIndices);
      BlockChecksumType groupChecksumType = getBlockChecksumOptions().getBlockChecksumType();
      ...
      setOrVerifyChecksumProperties(
          errBlkIndex, checksum.getBytesPerChecksum(), crcPerBlock, checksum.getChecksumType());
      LOG.debug(
          "Recalculated checksum for the block index:{}, checksum={}",
          errBlkIndex,
          checksumRecon.getDigestObject());
    }

    private void setOrVerifyChecksumProperties(
        int blockIdx, int bpc, final long cpb, DataChecksum.Type ct) throws IOException {
      // read byte-per-checksum
      if (blockIdx == 0) { // first block
        setBytesPerCRC(bpc);
      } else if (bpc != getBytesPerCRC()) {
        throw new IOException(
            "Byte-per-checksum not matched: bpc=" + bpc + " but bytesPerCRC=" + getBytesPerCRC());
      }
      ...
      if (blockIdx == 0) {
        LOG.debug("set bytesPerCRC={}, crcPerBlock={}", getBytesPerCRC(), getCrcPerBlock());
      }
    }
  }
}
