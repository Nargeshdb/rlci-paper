...
class FSDirStatAndListingOp {
  ...
  static boolean isFileClosed(FSDirectory fsd, FSPermissionChecker pc, String src)
      throws IOException {
    final INodesInPath iip = fsd.resolvePath(pc, src, DirOp.READ);
    return !INodeFile.valueOf(iip.getLastINode(), src).isUnderConstruction();
  }

  static ContentSummary getContentSummary(FSDirectory fsd, FSPermissionChecker pc, String src)
      throws IOException {
  ...
  static GetBlockLocationsResult getBlockLocations(
    ...
    try {
      ...
      final ErasureCodingPolicy ecPolicy =
          FSDirErasureCodingOp.unprotectedGetErasureCodingPolicy(fsd.getFSNamesystem(), iip);

      final LocatedBlocks blocks =
          bm.createLocatedBlocks(
   ...
   *
   * <p>We will stop when any of the following conditions is met: 1) this.lsLimit files have been
   * added 2) needLocation is true AND enough files have been added such that at least this.lsLimit
   * block locations are in the response
   *
   * @param fsd FSDirectory
   * @param iip the INodesInPath instance containing all the INodes along the path
   * @param startAfter the name to start listing after
   * @param needLocation if block locations are returned
   * @return a partial listing starting after startAfter
   */
  private static DirectoryListing getListing(
      FSDirectory fsd, INodesInPath iip, byte[] startAfter, boolean needLocation)
      throws IOException {
    ...
    try {
      ...
      if (!targetNode.isDirectory()) {
        ...
        return new DirectoryListing(
            new HdfsFileStatus[] {
              createFileStatus(fsd, iip, null, parentStoragePolicy, needLocation, false)
            },
            0);
      }
      ...
      for (int i = 0; i < numOfListing && locationBudget > 0; i++) {
        ...
        byte childStoragePolicy =
            !child.isSymlink()
                ? getStoragePolicyID(child.getLocalStoragePolicyID(), parentStoragePolicy)
                : parentStoragePolicy;
        listing[i] = createFileStatus(fsd, iip, child, childStoragePolicy, needLocation, false);
        listingCnt++;
        if (listing[i] instanceof HdfsLocatedFileStatus) {
          ...
          locationBudget -=
              (blks == null) ? 0 : blks.locatedBlockCount() * listing[i].getReplication();
        }
      }
  ...
  private static DirectoryListing getSnapshotsListing(
      FSDirectory fsd, INodesInPath iip, byte[] startAfter) throws IOException {
    ...
    for (int i = 0; i < numOfListing; i++) {
      ...
      listing[i] =
          createFileStatus(
              fsd, iip, sRoot, HdfsConstants.BLOCK_STORAGE_POLICY_ID_UNSPECIFIED, false, false);
    }
  ...
  static HdfsFileStatus getFileInfo(
    ...
    try {
      ...
      byte policy =
          (includeStoragePolicy && !node.isSymlink())
              ? node.getStoragePolicyID()
              : HdfsConstants.BLOCK_STORAGE_POLICY_ID_UNSPECIFIED;
      return createFileStatus(fsd, iip, null, policy, needLocation, needBlockToken);
    } finally {
      fsd.readUnlock();
    }
  }
  ...
  static HdfsFileStatus getFileInfo(
      FSDirectory fsd, INodesInPath iip, boolean needLocation, boolean needBlockToken)
      throws IOException {
    fsd.readLock();
    try {
   ...
   */
  static HdfsFileStatus createFileStatusForEditLog(FSDirectory fsd, INodesInPath iip)
      throws IOException {
    return createFileStatus(
        fsd, iip, null, HdfsConstants.BLOCK_STORAGE_POLICY_ID_UNSPECIFIED, false, false);
  }
  ...
  private static HdfsFileStatus createFileStatus(
    ...
    if (node.isFile()) {
      ...
      if (needLocation) {
        ...
        final long fileSize =
            !inSnapshot && isUc ? fileNode.computeFileSizeNotIncludingLastUcBlock() : size;
        loc =
            fsd.getBlockManager()
                .createLocatedBlocks(
                    fileNode.getBlocks(snapshot),
                    fileSize,
                    isUc,
                    0L,
                    size,
  ...
  private static ContentSummary getContentSummaryInt(
      FSDirectory fsd, FSPermissionChecker pc, INodesInPath iip) throws IOException {
    fsd.readLock();
    try {
      ...
      if (targetNode == null) {
        ...
        ContentSummaryComputationContext cscc =
            new ContentSummaryComputationContext(
                ...
                pc);
        ContentSummary cs =
            targetNode.computeAndConvertContentSummary(iip.getPathSnapshotId(), cscc);
  ...
  static QuotaUsage getQuotaUsage(FSDirectory fsd, FSPermissionChecker pc, String src)
      throws IOException {
    final INodesInPath iip;
    fsd.readLock();
    try {
        ...
        if (feature != null) {
          ...
          usage =
              new QuotaUsage.Builder()
                  ...
                  .typeQuota(quotas.getTypeSpaces().asArray())
                  .build();
        }
      }
      return usage;
