...
public class DFSHAAdmin extends HAAdmin {
  ...
  private static final Map<String, UsageInfo> USAGE_DFS_ONLY =
      ImmutableMap.<String, UsageInfo>builder()
          .put(
              "-transitionToObserver",
              new UsageInfo("<serviceId>", "Transitions the service into Observer state"))
          .put(
              "-failover",
              new UsageInfo(
                  "[--" + FORCEFENCE + "] [--" + FORCEACTIVE + "] " + "<serviceId> <serviceId>",
                  "Failover from the first service to the second.\n"
                      + "Unconditionally fence services if the --"
                      + FORCEFENCE
                      + " option is used.\n"
  ...
  private static final Map<String, UsageInfo> USAGE_DFS_MERGED =
  ...
  private boolean checkSupportObserver(HAServiceTarget target) {
    if (target.supportObserver()) {
      ...
      errOut.println("The target " + target + " doesn't support Observer state.");
      return false;
    }
  }
  ...
  private int failover(CommandLine cmd) throws IOException, ServiceFailedException {
    ...
    if (fromNode.isAutoFailoverEnabled()) {
      ...
      try {
        ...
        errOut.println(
            "Failover command is not supported with "
    ...
    FailoverController fc = new FailoverController(getConf(), getRequestSource());
