 ...
 *
 * <ul>
 *   <li>The QuorumJournalManager, when reading edits, fetches the edit streams from the journal
 *       nodes.
 *   <li>During edits synchronization, one journal node will fetch edits from another journal node.
 * </ul>
 */
...
public class GetJournalEditServlet extends HttpServlet {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(GetJournalEditServlet.class);
  ...
  private boolean checkRequestorOrSendError(
      Configuration conf, HttpServletRequest request, HttpServletResponse response)
      throws IOException {
  ...
  private boolean checkStorageInfoOrSendError(
      JNStorage storage, HttpServletRequest request, HttpServletResponse response)
      throws IOException {
    int myNsId = storage.getNamespaceID();
    String myClusterId = storage.getClusterID();

    String theirStorageInfoString =
        StringEscapeUtils.escapeHtml4(request.getParameter(STORAGEINFO_PARAM));

    if (theirStorageInfoString != null) {
      int theirNsId = StorageInfo.getNsIdFromColonSeparatedString(theirStorageInfoString);
      String theirClusterId =
          StorageInfo.getClusterIdFromColonSeparatedString(theirStorageInfoString);
      if (myNsId != theirNsId || !myClusterId.equals(theirClusterId)) {
        String msg =
            "This node has namespaceId '"
                + myNsId
                + " and clusterId '"
                + myClusterId
  ...
  public void doGet(final HttpServletRequest request, final HttpServletResponse response)
    ...
    try {
      ...
      if (inProgressOkStr != null && inProgressOkStr.equalsIgnoreCase("false")) {
        inProgressOk = false;
      } else {
        inProgressOk = true;
      }
      QuorumJournalManager.checkJournalId(journalId);
      final JNStorage storage =
          JournalNodeHttpServer.getJournalFromContext(context, journalId).getStorage();
  ...
  public static String buildPath(
      String journalId, long segmentTxId, NamespaceInfo nsInfo, boolean inProgressOk) {
    ...
    try {
      ...
      path.append("&" + STORAGEINFO_PARAM)
          .append("=")
          .append(URLEncoder.encode(nsInfo.toColonSeparatedString(), "UTF-8"));
      path.append("&" + IN_PROGRESS_OK).append("=").append(inProgressOk);
