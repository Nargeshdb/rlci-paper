...
/**
 * Manages the BPOfferService objects for the data node. Creation, removal, starting, stopping,
 * shutdown on BPOfferService objects must be done via APIs in this class.
 */
...
class BlockPoolManager {
  ...
  /**
   * Returns a list of BPOfferService objects. The underlying list implementation is a
   * CopyOnWriteArrayList so it can be safely iterated while BPOfferServices are being added or
   * removed.
   *
   * <p>Caution: The BPOfferService returned could be shutdown any time.
   */
  ...
  void refreshNamenodes(Configuration conf) throws IOException {
    LOG.info(
        "Refresh request received for nameservices: " + conf.get(DFSConfigKeys.DFS_NAMESERVICES));
  ...
  private void doRefreshNamenodes(
    ...
    synchronized (this) {
      ...
      assert toRefresh.size() + toAdd.size() == addrMap.size()
          : "toAdd: "
    ...
    if (!toRemove.isEmpty()) {
      LOG.info(
          "Stopping BPOfferServices for nameservices: "
              + Joiner.on(",").useForNull("<default>").join(toRemove));
    ...
    if (!toRefresh.isEmpty()) {
      LOG.info(
          "Refreshing list of NNs for nameservices: "
              + Joiner.on(",").useForNull("<default>").join(toRefresh));
