...
public class CancelCommand extends Command {
  ...
  public CancelCommand(Configuration conf) {
    super(conf);
    addValidCommandParameters(DiskBalancerCLI.CANCEL, "Cancels a running plan.");
    addValidCommandParameters(
        DiskBalancerCLI.NODE, "Node to run the command against in node:port format.");
  }
  ...
  private void cancelPlan(String planData) throws IOException {
    ...
    try {
      ...
      LOG.error(
          "Cancelling plan on  {} failed. Result: {}, Message: {}",
          plan.getNodeName(),
   ...
   * @param hash - Sha512 hash of the plan, which can be read from datanode using query status
   *     command.
   * @throws IOException
   */
