...
/**
 * This class is to be used as a builder for {@link ReplicaInfo} objects. The state of the replica
 * is used to determine which object is instantiated.
 */
public class ReplicaBuilder {
  ...
  /**
   * Set the suffix of the {@link Path} associated with the replica. Intended to be use only for
   * {@link ProvidedReplica}s.
   *
  ...
  /**
   * Set the prefix of the {@link Path} associated with the replica. Intended to be use only for
   * {@link ProvidedReplica}s.
   *
  ...
  private LocalReplicaInPipeline buildRBW() throws IllegalArgumentException {
    if (null != fromReplica && fromReplica.getState() == ReplicaState.RBW) {
      ...
      throw new IllegalArgumentException(
          "Incompatible fromReplica " + "state: " + fromReplica.getState());
    } else {
      if (null != block) {
        if (null == writer) {
          throw new IllegalArgumentException(
        ...
        if (length != -1) {
          return new ReplicaBeingWritten(
              blockId, length, genStamp, volume, directoryUsed, writer, bytesToReserve);
        } else {
  ...
  private LocalReplicaInPipeline buildTemporaryReplica() throws IllegalArgumentException {
    if (null != fromReplica && fromReplica.getState() == ReplicaState.TEMPORARY) {
      ...
      if (null != block) {
        ...
        if (length != -1) {
          return new LocalReplicaInPipeline(
              blockId, length, genStamp, volume, directoryUsed, writer, bytesToReserve);
        } else {
          return new LocalReplicaInPipeline(
              blockId, genStamp, volume, directoryUsed, bytesToReserve);
        }
      }
    }
  }
  ...
  private LocalReplica buildFinalizedReplica() throws IllegalArgumentException {
    if (null != fromReplica && fromReplica.getState() == ReplicaState.FINALIZED) {
      ...
      if (null != block) {
        return new FinalizedReplica(block, volume, directoryUsed, lastPartialChunkChecksum);
      } else {
        return new FinalizedReplica(
            blockId, length, genStamp, volume, directoryUsed, lastPartialChunkChecksum);
      }
    }
  }
  ...
  private LocalReplica buildRUR() throws IllegalArgumentException {
    ...
    if (null != writer || null != block) {
      throw new IllegalArgumentException(
          "Invalid state for " + "recovering from replica with blk id " + fromReplica.getBlockId());
    }
  ...
  private ProvidedReplica buildProvidedFinalizedReplica() throws IllegalArgumentException {
    ...
    if (fileRegion == null && uri == null && (pathPrefix == null || pathSuffix == null)) {
      throw new IllegalArgumentException(
          "Trying to construct a provided replica on " + volume + " without enough information");
    }
    if (fileRegion == null) {
      if (uri != null) {
        info =
            new FinalizedProvidedReplica(
                blockId, uri, offset, length, genStamp, pathHandle, volume, conf, remoteFS);
      } else {
        info =
            new FinalizedProvidedReplica(
                blockId,
  ...
  private ProvidedReplica buildProvidedReplica() throws IllegalArgumentException {
    ...
    switch (this.state) {
      ...
      default:
        throw new IllegalArgumentException(
            "Unknown replica state " + state + " for PROVIDED replica");
    }
    return info;
  }
