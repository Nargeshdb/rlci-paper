...
class IPCLoggerChannelMetrics {
  ...
  /**
   * In the case of the NN transitioning between states, edit logs are closed and reopened. Thus,
   * the IPCLoggerChannel instance that writes to a given JournalNode may change over the lifetime
   * of the process. However, metrics2 doesn't have a function to unregister a set of metrics and
   * fails if a new metrics class is registered with the same name as the existing one. Hence, we
   * have to maintain our own registry ("multiton") here, so that we have exactly one metrics
   * instance per JournalNode, and switch out the pointer to the underlying IPCLoggerChannel
   * instance.
   */
  ...
  private IPCLoggerChannelMetrics(IPCLoggerChannel ch) {
    ...
    if (intervals != null) {
      ...
      for (int i = 0; i < writeEndToEndLatencyQuantiles.length; i++) {
        ...
        writeEndToEndLatencyQuantiles[i] =
            registry.newQuantiles(
                "writesE2E" + interval + "s",
                "End-to-end time for write operations",
                "ops",
                "LatencyMicros",
                interval);
        writeRpcLatencyQuantiles[i] =
            registry.newQuantiles(
                "writesRpc" + interval + "s",
                "RPC RTT for write operations",
                "ops",
                "LatencyMicros",
                interval);
      }
