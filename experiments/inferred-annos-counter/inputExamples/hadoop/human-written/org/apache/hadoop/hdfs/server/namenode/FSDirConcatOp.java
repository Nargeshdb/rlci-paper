...
class FSDirConcatOp {
  ...
  static FileStatus concat(
      FSDirectory fsd, FSPermissionChecker pc, String target, String[] srcs, boolean logRetryCache)
      throws IOException {
    ...
    if (NameNode.stateChangeLog.isDebugEnabled()) {
      NameNode.stateChangeLog.debug(
          "DIR* NameSystem.concat: " + Arrays.toString(srcs) + " to " + target);
    }
  ...
  private static void validatePath(String target, String[] srcs) throws IOException {
    ...
    if (FSDirectory.isReservedRawName(target) || FSDirectory.isReservedInodesName(target)) {
      throw new IOException(
          "Concat operation doesn't support "
  ...
  private static void verifyTargetFile(
      FSDirectory fsd, final String target, final INodesInPath targetIIP) throws IOException {
    // check the target
  ...
  private static INodeFile[] verifySrcFiles(
      FSDirectory fsd, String[] srcs, INodesInPath targetIIP, FSPermissionChecker pc)
      throws IOException {
    ...
    for (String src : srcs) {
      ...
      if (srcINode.isReference()
          && ((INodeReference.WithCount) srcINode.asReference().getReferredINode())
                  .getReferenceCount()
              > 1) {
        throw new SnapshotException(
            "Concat: the source file "
                + src
      ...
      if (srcINode.equals(targetINode)) {
        throw new HadoopIllegalArgumentException(
            "concat: the src file "
                + src
                + " is the same with the target file "
                + targetIIP.getPath());
      }
      ...
      if (srcINodeFile.getPreferredBlockSize() > targetINode.getPreferredBlockSize()) {
        throw new HadoopIllegalArgumentException(
            "concat: source file "
                + src
                + " has preferred block size "
                + srcINodeFile.getPreferredBlockSize()
  ...
  private static QuotaCounts computeQuotaDeltas(
      FSDirectory fsd, INodeFile target, INodeFile[] srcList) {
  ...
  private static void verifyQuota(FSDirectory fsd, INodesInPath targetIIP, QuotaCounts deltas)
  ...
  static void unprotectedConcat(
      FSDirectory fsd, INodesInPath targetIIP, INodeFile[] srcList, long timestamp)
      throws IOException {
