 ...
 *
 * <p>These files control which DataNodes the NameNode expects to see in the cluster. Loosely
 * speaking, the include file, if it exists and is not empty, is a list of everything we expect to
 * see. The exclude file is a list of everything we want to ignore if we do see it.
 *
...
public class HostFileManager extends HostConfigManager {
  ...
  public void refresh() throws IOException {
    refresh(conf.get(DFSConfigKeys.DFS_HOSTS, ""), conf.get(DFSConfigKeys.DFS_HOSTS_EXCLUDE, ""));
  }
  ...
  static InetSocketAddress parseEntry(String type, String fn, String line) {
    try {
      ...
      LOG.warn(
          String.format(
              "Failed to parse `%s` in `%s`. " + "Ignoring in " + "the %s list.", line, fn, type));
    }
    return null;
  }
  ...
  /**
   * Read the includes and excludes lists from the named files. Any previous includes and excludes
   * lists are discarded.
   *
  ...
  /**
   * Set the includes and excludes lists by the new HostSet instances. The old instances are
   * discarded.
   *
