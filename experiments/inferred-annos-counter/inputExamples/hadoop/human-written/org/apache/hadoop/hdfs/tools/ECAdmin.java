...
public class ECAdmin extends Configured implements Tool {
  ...
  private static class ListECPoliciesCommand implements AdminHelper.Command {
    ...
    public int run(Configuration conf, List<String> args) throws IOException {
      ...
      try {
        final Collection<ErasureCodingPolicyInfo> policies = dfs.getAllErasureCodingPolicies();
        if (policies.isEmpty()) {
  ...
  private static class AddECPoliciesCommand implements AdminHelper.Command {
    ...
    public String getLongUsage() {
      ...
      listing.addRow("<file>", "The path of the xml file which defines the EC policies to add");
      return getShortUsage()
          + "\n"
    ...
    public int run(Configuration conf, List<String> args) throws IOException {
      ...
      final DistributedFileSystem dfs = AdminHelper.getDFS(conf);
      try {
        List<ErasureCodingPolicy> policies = new ECPolicyLoader().loadPolicy(filePath);
        if (policies.size() > 0) {
  ...
  private static class RemoveECPolicyCommand implements AdminHelper.Command {
    ...
    @Override
    public int run(Configuration conf, List<String> args) throws IOException {
      final String ecPolicyName = StringUtils.popOptionWithArgument("-policy", args);
  ...
  private static class SetECPolicyCommand implements AdminHelper.Command {
    ...
    public String getLongUsage() {
      ...
      listing.addRow("<policy>", "The name of the erasure coding policy");
      listing.addRow("-replicate", "force 3x replication scheme on the directory");
      return getShortUsage()
          + "\n"
    ...
    public int run(Configuration conf, List<String> args) throws IOException {
      ...
      String ecPolicyName = StringUtils.popOptionWithArgument("-policy", args);
      ...
      if (replicate) {
        if (ecPolicyName != null) {
          System.err.println(
              getName() + ": -replicate and -policy cannot been used at the same time");
          return 2;
        }
  ...
  private static class UnsetECPolicyCommand implements AdminHelper.Command {
    ...
    public String getLongUsage() {
      ...
      listing.addRow(
          "<path>",
          "The path of the directory " + "from which the erasure coding policy will be unset.");
      return getShortUsage()
          + "\n"
    ...
    public int run(Configuration conf, List<String> args) throws IOException {
      ...
      try {
        ...
        System.err.println(AdminHelper.prettifyException(e));
        System.err.println(
            "Use '-setPolicy -path <PATH> -replicate' to enforce"
  ...
  private static class ListECCodecsCommand implements AdminHelper.Command {
    ...
    public int run(Configuration conf, List<String> args) throws IOException {
      ...
      final DistributedFileSystem dfs = AdminHelper.getDFS(conf);
      try {
        Map<String, String> codecs = dfs.getAllErasureCodingCodecs();
        if (codecs.isEmpty()) {
  ...
  private static class EnableECPolicyCommand implements AdminHelper.Command {
    ...
    public int run(Configuration conf, List<String> args) throws IOException {
      ...
      try {
        ...
        ECTopologyVerifierResult result = dfs.getECTopologyResultForPolicies(ecPolicyName);
        if (!result.isSupported()) {
          System.err.println(
              "Warning: The cluster setup does not support "
                  + "EC policy "
                  + ecPolicyName
                  + ". Reason: "
