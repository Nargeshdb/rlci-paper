...
/**
 * This manages erasure coding policies predefined and activated in the system. It loads customized
 * policies and syncs with persisted ones in NameNode image.
 *
 * <p>This class is instantiated by the FSNamesystem.
 */
...
public final class ErasureCodingPolicyManager {
  ...
  /**
   * All policies sorted by name for fast querying, include built-in policy, user defined policy,
   * removed policy.
   */
  private Map<String, ErasureCodingPolicyInfo> policiesByName;

  /**
   * All policies sorted by ID for fast querying, including built-in policy, user defined policy,
   * removed policy.
   */
   ...
   *
   * <p>The difference between persisted policies and all policies is that if a default policy is
   * only enabled at startup, it will appear as disabled in the persisted policy list and in the
   * fsimage.
   */
  private Map<Byte, ErasureCodingPolicyInfo> allPersistedPolicies;

  /**
   * All enabled policies sorted by name for fast querying, including built-in policy, user defined
   * policy.
   */
  ...
  public void init(Configuration conf) throws IOException {
    ...
    /**
     * TODO: load user defined EC policy from fsImage HDFS-7859 load persistent policies from image
     * and editlog, which is done only once during NameNode startup. This can be done here or in a
     * separate method.
     */
    ...
    for (ErasureCodingPolicy policy : SystemErasureCodingPolicies.getPolicies()) {
      ...
      allPersistedPolicies.put(policy.getId(), new ErasureCodingPolicyInfo(policy));
    }
  ...
  /**
   * @return if the specified storage policy ID is suitable for striped EC files.
   */
  ...
  public ErasureCodingPolicyInfo[] getPersistedPolicies() {
    return allPersistedPolicies.values().toArray(new ErasureCodingPolicyInfo[0]);
  }
  ...
  /**
   * Get a {@link ErasureCodingPolicy} by policy ID, including system policy and user defined
   * policy.
   *
  ...
  /**
   * Get a {@link ErasureCodingPolicyInfo} by policy ID, including system policy and user defined
   * policy.
   */
  ...
  /**
   * Get a {@link ErasureCodingPolicy} by policy name, including system policy and user defined
   * policy.
   *
   * @return ecPolicy, or null if not found
   */
  ...
  /**
   * Get a {@link ErasureCodingPolicy} by policy name, including system policy, user defined policy
  ...
  /**
   * Get a {@link ErasureCodingPolicyInfo} by policy name, including system policy and user defined
   * policy.
   *
   * @return ecPolicy, or null if not found
   */
  ...
  public synchronized ErasureCodingPolicy addPolicy(ErasureCodingPolicy policy) {
    ...
    String assignedNewName =
        ErasureCodingPolicy.composePolicyName(policy.getSchema(), policy.getCellSize());
    for (ErasureCodingPolicyInfo info : getPolicies()) {
      ...
      if (p.getSchema().equals(policy.getSchema()) && p.getCellSize() == policy.getCellSize()) {
        LOG.info(
            "A policy with same schema "
                + policy.getSchema().toString()
                + " and cell size "
                + p.getCellSize()
                + " already exists");
        return p;
    ...
    policy =
        new ErasureCodingPolicy(
            assignedNewName, policy.getSchema(), policy.getCellSize(), getNextAvailablePolicyID());
    ...
    allPolicies = policiesByName.values().toArray(new ErasureCodingPolicyInfo[0]);
    allPersistedPolicies.put(policy.getId(), new ErasureCodingPolicyInfo(policy));
    return policy;
  }
  ...
  public synchronized void removePolicy(String name) {
    ...
    if (ecPolicy.isSystemPolicy()) {
      throw new HadoopIllegalArgumentException(
          "System erasure coding policy " + name + " cannot be removed");
    }

    if (enabledPoliciesByName.containsKey(name)) {
      enabledPoliciesByName.remove(name);
      enabledPolicies = enabledPoliciesByName.values().toArray(new ErasureCodingPolicy[0]);
    }
    info.setState(ErasureCodingPolicyState.REMOVED);
    LOG.info("Remove erasure coding policy " + name);
    allPersistedPolicies.put(
        ecPolicy.getId(), createPolicyInfo(ecPolicy, ErasureCodingPolicyState.REMOVED));
    /*
  ...
  public synchronized boolean disablePolicy(String name) {
    ...
    if (enabledPoliciesByName.containsKey(name)) {
      ...
      allPersistedPolicies.put(
          ...
          createPolicyInfo(info.getPolicy(), ErasureCodingPolicyState.DISABLED));
      return true;
    }
    return false;
  }
  ...
  public synchronized boolean enablePolicy(String name) {
    ...
    enabledPolicies = enabledPoliciesByName.values().toArray(new ErasureCodingPolicy[0]);
    allPersistedPolicies.put(
        ecPolicy.getId(), createPolicyInfo(info.getPolicy(), ErasureCodingPolicyState.ENABLED));
  ...
  private void loadPolicy(ErasureCodingPolicyInfo info) {
    ...
    allPersistedPolicies.put(policy.getId(), createPolicyInfo(policy, info.getState()));
  }
  ...
  public synchronized void loadPolicies(
      List<ErasureCodingPolicyInfo> ecPolicies, Configuration conf) throws IOException {
  ...
  private void enableDefaultPolicy(Configuration conf) throws IOException {
    ...
    if (!defaultPolicyName.isEmpty()) {
      ...
      if (info == null) {
        String names =
            policiesByName.values().stream()
                .map((pi) -> pi.getPolicy().getName())
                .collect(Collectors.joining(", "));
        String msg =
            String.format(
                "EC policy '%s' specified at %s is not a "
                    + "valid policy. Please choose from list of available "
                    + "policies: [%s]",
                defaultPolicyName, DFSConfigKeys.DFS_NAMENODE_EC_SYSTEM_DEFAULT_POLICY, names);
  ...
  public String getEnabledPoliciesMetric() {
    return StringUtils.join(", ", enabledPoliciesByName.keySet());
  }

  private ErasureCodingPolicyInfo createPolicyInfo(
      ErasureCodingPolicy p, ErasureCodingPolicyState s) {
