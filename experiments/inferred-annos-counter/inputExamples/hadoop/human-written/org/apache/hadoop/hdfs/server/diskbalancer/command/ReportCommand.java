 ...
 *
 * <p>This command will report volume information for a specific DataNode or top X DataNode(s)
 * benefiting from running DiskBalancer.
 *
 * <p>This is done by reading the cluster info, sorting the DiskbalancerNodes by their
 * NodeDataDensity and printing out the info.
 */
public class ReportCommand extends Command {
  ...
  public ReportCommand(Configuration conf, final PrintStream ps) {
    ...
    String desc =
        String.format("Top number of nodes to be processed. Default: %d", getDefaultTop());
  ...
  private void handleTopReport(
      final CommandLine cmd, final TextStringBuilder result, final String nodeFormat)
    ...
    final String outputLine =
        String.format(
            "Reporting top %d DataNode(s) benefiting from running DiskBalancer.", getTopNodes());
    recordOutput(result, outputLine);
  ...
  private void handleNodeReport(
      final CommandLine cmd,
      TextStringBuilder result,
  ...
  private void recordNodeReport(
      TextStringBuilder result,
      DiskBalancerDataNode dbdn,
      final String nodeFormat,
      final String volumeFormat)
      throws Exception {
  ...
  public void printHelp() {
    String header =
        "Report command reports the volume information of given"
            + " datanode(s), or prints out the list of nodes that will benefit "
            + "from running disk balancer. Top defaults to "
            + getDefaultTop();
    String footer =
        ". E.g.:\n"
    ...
    helpFormatter.printHelp(
        "hdfs diskbalancer -fs http://namenode.uri " + "-report [options]",
        header,
        DiskBalancerCLI.getReportOptions(),
        footer);
  }
}
