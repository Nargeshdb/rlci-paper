...
/**
 * InMemoryLevelDBAliasMapClient is the client for the InMemoryAliasMapServer. This is used by the
 * Datanode and fs2img to store and retrieve FileRegions based on the given Block.
 */
...
public class InMemoryLevelDBAliasMapClient extends BlockAliasMap<FileRegion>
  ...
  class LevelDbReader extends BlockAliasMap.Reader<FileRegion> {
    ...
    private class LevelDbIterator extends BlockAliasMap<FileRegion>.ImmutableIterator {
      ...
      private void batch(Optional<Block> newNextMarker) {
        try {
          InMemoryAliasMap.IterationResult iterationResult = aliasMap.list(newNextMarker);
  ...
  class LevelDbWriter extends BlockAliasMap.Writer<FileRegion> {
    ...
    public void store(FileRegion fileRegion) throws IOException {
      aliasMap.write(fileRegion.getBlock(), fileRegion.getProvidedStorageLocation());
    }
  ...
  private InMemoryAliasMapProtocol getAliasMap(String blockPoolID) throws IOException {
    ...
    for (InMemoryAliasMapProtocol aliasMap : aliasMaps) {
      try {
        ...
        if (aliasMapBlockPoolId != null && aliasMapBlockPoolId.equals(blockPoolID)) {
          return aliasMap;
        }
      } catch (IOException e) {
  ...
  public Reader<FileRegion> getReader(Reader.Options opts, String blockPoolID) throws IOException {
    ...
    LOG.info("Loading InMemoryAliasMapReader for block pool id {}", blockPoolID);
  ...
  public Writer<FileRegion> getWriter(Writer.Options opts, String blockPoolID) throws IOException {
    InMemoryAliasMapProtocol aliasMap = getAliasMap(blockPoolID);
    LOG.info("Loading InMemoryAliasMapWriter for block pool id {}", blockPoolID);
