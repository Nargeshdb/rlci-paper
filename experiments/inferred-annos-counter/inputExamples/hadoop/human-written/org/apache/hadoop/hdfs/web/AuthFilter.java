...
/** Subclass of {@link AuthenticationFilter} that obtains Hadoop-Auth configuration for webhdfs. */
public class AuthFilter extends ProxyUserAuthenticationFilter {
  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain filterChain)
    ...
    final HttpServletRequest httpRequest =
        ProxyUserAuthenticationFilter.toLowerCase((HttpServletRequest) request);
    final String tokenString = httpRequest.getParameter(DelegationParam.NAME);
    if (tokenString != null
        && httpRequest.getServletPath().startsWith(WebHdfsFileSystem.PATH_PREFIX)) {
