...
/**
 * DiskBalancerDataNode represents a DataNode that exists in the cluster. It also contains a metric
 * called nodeDataDensity which allows us to compare between a set of Nodes.
 */
public class DiskBalancerDataNode implements Comparable<DiskBalancerDataNode> {
  ...
  public int compareTo(DiskBalancerDataNode that) {
    ...
    if (Double.compare(this.nodeDataDensity - that.getNodeDataDensity(), 0) < 0) {
      return -1;
    }
   ...
   *
   * <p>This metric allows us to compare different nodes and how well the data is spread across a
   * set of volumes inside the node.
   */
   ...
   *
   * <p>It is assumed that we have one thread per node hence this call is not synchronised neither
   * is the map is protected.
   *
   * @param volume - volume
   */
