...
/**
 * A class describing the difference between snapshots of a snapshottable directory where the
 * difference is limited by dfs.snapshotDiff-report.limit.
 */
class SnapshotDiffListingInfo {
   ...
   */
  private final List<DiffReportListingEntry> modifiedList = new ChunkedArrayList<>();

  private final List<DiffReportListingEntry> createdList = new ChunkedArrayList<>();

  private final List<DiffReportListingEntry> deletedList = new ChunkedArrayList<>();

  SnapshotDiffListingInfo(
      INodeDirectory snapshotRootDir,
      INodeDirectory snapshotDiffScopeDir,
      Snapshot start,
      Snapshot end,
      int snapshotDiffReportLimit) {
    Preconditions.checkArgument(
        snapshotRootDir.isSnapshottable()
  ...
  boolean addDirDiff(long dirId, byte[][] parent, ChildrenDiff diff) {
    ...
    if (lastIndex == -1) {
      if (getTotalEntries() < maxEntries) {
        modifiedList.add(new DiffReportListingEntry(dirId, dirId, parent, true, null));
      } else {
        setLastPath(parent);
        setLastIndex(-1);
        return false;
    ...
    if (lastIndex == -1 || lastIndex < clist.size()) {
      ListIterator<INode> iterator =
          lastIndex != -1 ? clist.listIterator(lastIndex) : clist.listIterator();
      while (iterator.hasNext()) {
        if (getTotalEntries() < maxEntries) {
          INode created = iterator.next();
          byte[][] path = newPath(parent, created.getLocalNameBytes());
          createdList.add(
              new DiffReportListingEntry(
  ...
  boolean addFileDiff(INodeFile file, byte[][] relativePath) {
    if (getTotalEntries() < maxEntries) {
      modifiedList.add(
          new DiffReportListingEntry(file.getId(), file.getId(), relativePath, false, null));
    } else {
  ...
  /**
   * Generate a {@link SnapshotDiffReportListing} based on detailed diff information.
   *
  ...
  public SnapshotDiffReportListing generateReport() {
    return new SnapshotDiffReportListing(
        lastPath, modifiedList, createdList, deletedList, lastIndex, isFromEarlier());
  }
}
