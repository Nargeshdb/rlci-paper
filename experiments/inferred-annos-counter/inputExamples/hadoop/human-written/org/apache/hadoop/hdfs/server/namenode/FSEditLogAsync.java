...
class FSEditLogAsync extends FSEditLog implements Runnable {
  ...
  private Semaphore overflowMutex =
      new Semaphore(8) {
        ...
        private void tryRelease(int prmits) {
          pendingReleases.getAndAdd(prmits);
          if (!draining.get()) {
        ...
        @Override
        public void release(int prmits) {
          draining.set(false);
          tryRelease(prmits);
        }
      };
