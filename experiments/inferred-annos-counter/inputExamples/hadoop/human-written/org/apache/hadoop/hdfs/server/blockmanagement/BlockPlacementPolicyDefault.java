 ...
 * The replica placement strategy is that if the writer is on a datanode, the 1st replica is placed
 * on the local machine by default (By passing the {@link
 * org.apache.hadoop.fs.CreateFlag#NO_LOCAL_WRITE} flag the client can request not to put a block
 * replica on the local datanode. Subsequent replicas will still follow default block placement
 * policy.). If the writer is not on a datanode, the 1st replica is placed on a random node. The 2nd
 * replica is placed on a datanode that is on a different rack. The 3rd replica is placed on a
 * datanode which is on a different node of the rack as the second replica.
 */
...
public class BlockPlacementPolicyDefault extends BlockPlacementPolicy {
  ...
  protected void chooseFavouredNodes(
      String src,
      int numOfReplicas,
      List<DatanodeDescriptor> favoredNodes,
      Set<Node> favoriteAndExcludedNodes,
  ...
  private DatanodeStorageInfo[] chooseTarget(
    ...
    boolean avoidLocalRack =
        (addBlockFlags != null
            && addBlockFlags.contains(AddBlockFlag.NO_LOCAL_RACK)
            && writer != null
  ...
  /**
   * Calculate the maximum number of replicas to allocate per rack. It also limits the total number
   * of replicas to the total number of nodes in the cluster. Caller should adjust the replica count
   * to the return value.
   *
   * @param numOfChosen The number of already chosen nodes.
   * @param numOfReplicas The number of additional nodes to allocate.
   * @return integer array. Index 0: The number of nodes allowed to allocate in addition to already
   *     chosen nodes. Index 1: The maximum allowed number of nodes per rack. This is independent of
   *     the number of chosen nodes, as it is calculated using the target number of replicas.
   */
  ...
  private Node chooseTarget(
    ...
    final List<StorageType> requiredStorageTypes =
        storagePolicy.chooseStorageTypes(
    ...
    try {
      ...
      final String message =
          "Failed to place enough replicas, still in need of "
              + (totalReplicasExpected - results.size())
              + " to reach "
  ...
  protected DatanodeStorageInfo chooseLocalStorage(
      ...
      EnumMap<StorageType, Integer> storageTypes)
      throws NotEnoughReplicasException {
    return chooseLocalOrFavoredStorage(
        localMachine,
        false,
        excludedNodes,
        blocksize,
  ...
  /**
   * Choose <i>localMachine</i> as the target. if <i>localMachine</i> is not available, choose a
  ...
  /**
   * Add <i>localMachine</i> and related nodes to <i>excludedNodes</i> for next replica choosing. In
   * sub class, we can add more nodes within the same failure domain of localMachine
   *
  ...
  /**
   * Choose one node from the rack that <i>localMachine</i> is on. if no such node is available,
   * choose one node from the rack where a second replica is on. if still no such node is available,
   * choose a random node in the cluster.
   *
  ...
  protected DatanodeStorageInfo chooseLocalRack(
    ...
    try {
      ...
      for (DatanodeStorageInfo resultStorage : results) {
        ...
        if (nextNode != localMachine) {
          if (LOG.isDebugEnabled()) {
            LOG.debug(
                "Failed to choose from local rack (location = "
                    + localRack
      ...
      LOG.debug(
          "Failed to choose from local rack (location = {}); the second"
              + " replica is not found, retry choosing randomly",
          localRack,
          e);
  ...
  private DatanodeStorageInfo chooseFromNextRack(
    ...
    try {
      ...
      LOG.debug(
          "Failed to choose from the next rack (location = {}), " + "retry choosing randomly",
          nextRack,
          e);
  ...
  /**
   * Choose <i>numOfReplicas</i> nodes from the racks that <i>localMachine</i> is NOT on. If not
   * enough nodes are available, choose the remaining ones from the local rack
   */
  protected void chooseRemoteRack(
    ...
    try {
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            "Failed to choose remote rack (location = ~"
                + localMachine.getNetworkLocation()
  ...
  /**
   * Choose a datanode from the given <i>scope</i> with specified storage type.
   *
   * @return the chosen node, if there is any.
   */
  protected DatanodeDescriptor chooseDataNode(
      final String scope, final Collection<Node> excludedNodes, StorageType type) {
    return (DatanodeDescriptor)
        ((DFSNetworkTopology) clusterMap)
            .chooseRandomWithStorageTypeTwoTrial(scope, excludedNodes, type);
  }

  /**
   * Choose a good storage of given storage type from datanode, and add it to the result list.
   *
  ...
  DatanodeStorageInfo chooseStorage4Block(
    ...
    if (storage != null) {
      ...
      logNodeIsNotChosen(
          dnd, NodeNotChosenReason.NOT_ENOUGH_STORAGE_SPACE, " for storage type " + storageType);
    }
    return storage;
  }
  ...
  private static void logNodeIsNotChosen(
      DatanodeDescriptor node, NodeNotChosenReason reason, String reasonDetails) {
    assert reason != null;
    if (LOG.isDebugEnabled()) {
  ...
  boolean excludeNodeByLoad(DatanodeDescriptor node) {
    ...
    if ((nodeLoad > maxLoad) && (maxLoad > 0)) {
      logNodeIsNotChosen(
          node, NodeNotChosenReason.NODE_TOO_BUSY, "(load: " + nodeLoad + " > " + maxLoad + ")");
      return true;
    }
    return false;
  }
   ...
   *
   * @param node The target datanode
   * @param maxTargetPerRack Maximum number of targets per rack. The value of this parameter depends
   *     on the number of racks in the cluster and total number of replicas for a block
   * @param considerLoad whether or not to consider load of the target node
   * @param results A list containing currently chosen nodes. Used to check if too many nodes has
   *     been chosen in the target rack.
   * @param avoidStaleNodes Whether or not to avoid choosing stale nodes
   * @return Return true if the datanode is good candidate, otherwise false
   */
  boolean isGoodDatanode(
  ...
  /**
   * Return a pipeline of nodes. The pipeline is formed finding a shortest path that starts from the
   * writer and traverses all <i>nodes</i> This is basically a traveling salesman problem.
   */
  ...
  public BlockPlacementStatus verifyBlockPlacement(DatanodeInfo[] locs, int numberOfReplicas) {
    ...
    return new BlockPlacementStatusDefault(
        Math.toIntExact(rackCount), minRacks, clusterMap.getNumOfRacks());
  }

  /**
   * Decide whether deleting the specified replica of the block still makes the block conform to the
   * configured block placement policy.
   *
   * @param moreThanOne The replica locations of this block that are present on more than one unique
   *     racks.
   * @param exactlyOne Replica locations of this block that are present on exactly one unique racks.
   * @param excessTypes The excess {@link StorageType}s according to the {@link BlockStoragePolicy}.
  ...
  public List<DatanodeStorageInfo> chooseReplicasToDelete(
    ...
    while (delCandidates.size() - expectedNumOfReplicas > excessReplicas.size()) {
      ...
      if (firstOne
          && useDelHint(
              delNodeHintStorage, addedNodeStorage, moreThanOne, exactlyOne, excessTypes)) {
  ...
  @VisibleForTesting
  boolean useDelHint(
      DatanodeStorageInfo delHint,
      DatanodeStorageInfo added,
      List<DatanodeStorageInfo> moreThanOne,
      Collection<DatanodeStorageInfo> exactlyOne,
      List<StorageType> excessTypes) {
    if (delHint == null) {
  ...
  public boolean isMovable(
      Collection<DatanodeInfo> locs, DatanodeInfo source, DatanodeInfo target) {
  ...
  /**
   * Pick up replica node set for deleting replica as over-replicated. First set contains replica
   * nodes on rack with more than one replica while second set contains remaining replica nodes. If
   * only 1 rack, pick all. If 2 racks, pick all that have more than 1 replicas on the same rack; if
   * no such replicas, pick all. If 3 or more racks, pick all.
   */
