...
/**
 * A BlockStorageMovementCommand is an instruction to a DataNode to move the given set of blocks to
 * specified target DataNodes to fulfill the block storage policy.
 *
 * <p>Upon receiving this command, this DataNode pass the array of block movement details to {@link
 * org.apache.hadoop.hdfs.server.sps.ExternalSPSBlockMoveTaskHandler} service. Later,
 * ExternalSPSBlockMoveTaskHandler will schedule block movement tasks for these blocks and monitors
 * the completion of each task. After the block movement attempt is finished(with success or
 * failure) this DataNode will send response back to NameNode about the block movement attempt
 * finished details.
 */
public class BlockStorageMovementCommand extends DatanodeCommand {
   ...
   * @param blockMovingInfos block to storage info that will be used for movement
   */
  public BlockStorageMovementCommand(
      int action, String blockPoolId, Collection<BlockMovingInfo> blockMovingInfos) {
    super(action);
  ...
  public static class BlockMovingInfo {
     ...
     * @param sourceDnInfo node that can be the source of a block move
     * @param srcStorageType type of source storage media
     */
    public BlockMovingInfo(
        Block block,
        DatanodeInfo sourceDnInfo,
