...
/**
 * The class is responsible for choosing the desired number of targets for placing block replicas on
 * environment with node-group layer. The replica placement strategy is adjusted to: If the writer
 * is on a datanode, the 1st replica is placed on the local node(or local node-group or on local
 * rack), otherwise a random datanode. The 2nd replica is placed on a datanode that is on a
 * different rack with 1st replica node. The 3rd replica is placed on a datanode which is on a
 * different node-group but the same rack as the second replica node.
 */
public class BlockPlacementPolicyWithNodeGroup extends BlockPlacementPolicyDefault {
  ...
  /**
   * choose all good favored nodes as target. If no enough targets, then choose one replica from
  ...
  protected void chooseFavouredNodes(
    ...
    if (results.size() < numOfReplicas) {
      ...
      for (int i = 0; i < favoredNodes.size() && results.size() < numOfReplicas; i++) {
        ...
        if (target == null) {
          LOG.warn(
              "Could not find a target for file "
                  + src
                  + " within nodegroup of favored node "
                  + favoredNode);
          continue;
        }
  ...
  /**
   * choose local node of <i>localMachine</i> as the target. If localMachine is not available, will
   * fallback to nodegroup/rack when flag <i>fallbackToNodeGroupAndLocalRack</i> is set.
   *
   ...
   */
  private static DatanodeDescriptor secondNode(
      Node localMachine, List<DatanodeStorageInfo> results) {
  ...
  protected DatanodeStorageInfo chooseLocalRack(
    ...
    try {
      ...
      return chooseRandom(
          scope, excludedNodes, blocksize, maxNodesPerRack, results, avoidStaleNodes, storageTypes);
      ...
      if (newLocal != null) {
        try {
          return chooseRandom(
              clusterMap.getRack(newLocal.getNetworkLocation()),
              excludedNodes,
              blocksize,
              maxNodesPerRack,
              results,
  ...
  /**
   * Find other nodes in the same nodegroup of <i>localMachine</i> and add them into
   * <i>excludeNodes</i> as replica should not be duplicated for nodes within the same nodegroup
   *
   ...
   * @return number of new excluded nodes
   */
  private int addDependentNodesToExcludedNodes(
      DatanodeDescriptor chosenNode, Set<Node> excludedNodes) {
   ...
   * first is not empty, divide first set into two subsets: moreThanOne contains nodes on nodegroup
   * with more than one replica exactlyOne contains the remaining nodes in first set then pickup
   * priSet if not empty. If first is empty, then pick second.
   */
  @Override
  public Collection<DatanodeStorageInfo> pickupReplicaSet(
    ...
    for (DatanodeStorageInfo storage : first) {
      final String nodeGroupName =
          NetworkTopology.getLastHalf(storage.getDatanodeDescriptor().getNetworkLocation());
  ...
  /**
   * Check if there are any replica (other than source) on the same node group with target. If true,
   * then target is not a good candidate for placing specific replica as we don't want 2 replicas
   * under the same nodegroup.
   *
   * @return true if there are any replica (other than source) on the same node group with target
   */
  @Override
