...
/**
 * Encapsulates the URI and storage medium that together describe a storage directory. The default
 * storage medium is assumed to be DISK, if none is specified.
 */
...
public class StorageLocation
  ...
  /**
   * Regular expression that describes a storage uri with a storage type. e.g.
   * [Disk]/storages/storage1/
   */
  ...
  public boolean matchesStorageDirectory(StorageDirectory sd) throws IOException {
    return this.equals(sd.getStorageLocation());
  }

  public boolean matchesStorageDirectory(StorageDirectory sd, String bpid) throws IOException {
    ...
    return this.getBpURI(bpid, Storage.STORAGE_DIR_CURRENT)
        .normalize()
        .equals(sd.getRoot().toURI().normalize());
  }

  /**
   * Attempt to parse a storage uri with storage class and URI. The storage class component of the
   * uri is case-insensitive.
   *
   * @param rawLocation Location string of the format [type]uri, where [type] is optional.
   * @return A StorageLocation object if successfully parsed, null otherwise. Does not throw any
   *     exceptions.
   */
  ...
  public int compareTo(StorageLocation obj) {
    ...
    } else if (this.getNormalizedUri() == null && otherStorage.getNormalizedUri() == null) {
   ...
   *
   * @param blockPoolID the block pool id
  ...
  public void makeBlockPoolDir(String blockPoolID, Configuration conf) throws IOException {
    ...
    if (storageType == StorageType.PROVIDED) {
      ...
      Storage.LOG.info(
          "Skipping creating directory for block pool "
              + blockPoolID
              + " for PROVIDED storage location "
              + this);
      return;
    }
    ...
    try {
      ...
      DataStorage.LOG.warn(
          "Invalid directory in: " + data.getCanonicalPath() + ": " + e.getMessage());
    }
  }
  ...
  public VolumeCheckResult check(CheckContext context) throws IOException {
    ...
    if (storageType != StorageType.PROVIDED) {
      DiskChecker.checkDir(context.localFileSystem, new Path(baseURI), context.expectedPermission);
    }
