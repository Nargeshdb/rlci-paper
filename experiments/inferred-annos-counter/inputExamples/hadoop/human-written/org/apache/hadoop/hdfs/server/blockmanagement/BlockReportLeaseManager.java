...
/**
 * The BlockReportLeaseManager manages block report leases.
 *
 * <p>DataNodes request BR leases from the NameNode by sending a heartbeat with the
 * requestBlockReportLease field set. The NameNode may choose to respond with a non-zero lease ID.
 * If so, that DataNode can send a block report with the given lease ID for the next few minutes.
 * The NameNode will accept these full block reports.
 *
 * <p>BR leases limit the number of incoming full block reports to the NameNode at any given time.
 * For compatibility reasons, the NN will always accept block reports sent with a lease ID of 0 and
 * queue them for processing immediately. Full block reports which were manually triggered will also
 * have a lease ID of 0, bypassing the rate-limiting.
 *
 * <p>Block report leases expire after a certain amount of time. This mechanism is in place so that
 * a DN which dies while holding a lease does not permanently decrease the number of concurrent
 * block reports which the NN is willing to accept.
 *
 * <p>When considering which DNs to grant a BR lease, the NameNode gives priority to the DNs which
 * have gone the longest without sending a full block report.
 *
 * <p>
 */
class BlockReportLeaseManager {
  ...
  BlockReportLeaseManager(int maxPending, long leaseExpiryMs) {
    Preconditions.checkArgument(
        maxPending >= 1,
        "Cannot set the maximum number of block report leases to a " + "value less than 1.");
  ...
  public synchronized long requestLease(DatanodeDescriptor dn) {
    ...
    if (node == null) {
      LOG.warn(
          "DN {} ({}) requested a lease even though it wasn't yet "
    ...
    if (node.leaseId != 0) {
      ...
      LOG.debug(
          "Removing existing BR lease 0x{} for DN {} in order to " + "issue a new one.",
    ...
    if (numPending >= maxPending) {
      if (LOG.isDebugEnabled()) {
        ...
        LOG.debug(
            "Can't create a new BR lease for DN {}, because "
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          "Created a new BR lease 0x{} for DN {}.  numPending = {}",
          Long.toHexString(node.leaseId),
          dn.getDatanodeUuid(),
          numPending);
    }
  ...
  private synchronized boolean pruneIfExpired(long monotonicNowMs, NodeData node) {
    ...
    LOG.info(
        "Removing expired block report lease 0x{} for DN {}.",
        Long.toHexString(node.leaseId),
        node.datanodeUuid);
  ...
  public synchronized boolean checkLease(DatanodeDescriptor dn, long monotonicNowMs, long id) {
    if (id == 0) {
      LOG.debug(
          "Datanode {} is using BR lease id 0x0 to bypass " + "rate-limiting.",
          dn.getDatanodeUuid());
      return true;
    }
    NodeData node = nodes.get(dn.getDatanodeUuid());
    if (node == null) {
      LOG.info(
          "BR lease 0x{} is not valid for unknown datanode {}",
          Long.toHexString(id),
          dn.getDatanodeUuid());
      return false;
    }
    if (node.leaseId == 0) {
      LOG.warn(
          "BR lease 0x{} is not valid for DN {}, because the DN " + "is not in the pending set.",
          Long.toHexString(id),
          dn.getDatanodeUuid());
      return false;
    }
    if (pruneIfExpired(monotonicNowMs, node)) {
      LOG.warn(
          "BR lease 0x{} is not valid for DN {}, because the lease " + "has expired.",
          Long.toHexString(id),
          dn.getDatanodeUuid());
      return false;
    }
    if (id != node.leaseId) {
      LOG.warn(
          "BR lease 0x{} is not valid for DN {}.  Expected BR lease 0x{}.",
          Long.toHexString(id),
          dn.getDatanodeUuid(),
          Long.toHexString(node.leaseId));
      return false;
  ...
  public synchronized long removeLease(DatanodeDescriptor dn) {
    ...
    if (LOG.isTraceEnabled()) {
      LOG.trace(
          "Removed BR lease 0x{} for DN {}.  numPending = {}",
          Long.toHexString(id),
          dn.getDatanodeUuid(),
          numPending);
    }
