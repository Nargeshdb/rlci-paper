...
/**
 * An implementation of the abstract class {@link EditLogOutputStream}, which streams edits to a
 * backup node.
 *
 * @see org.apache.hadoop.hdfs.server.protocol.NamenodeProtocol#journal
 *     (org.apache.hadoop.hdfs.server.protocol.NamenodeRegistration, int, int, byte[])
 */
class EditLogBackupOutputStream extends EditLogOutputStream {
  ...
  EditLogBackupOutputStream(
      ...
      int logVersion) // active name-node
      throws IOException {
    super();
  ...
  public void close() throws IOException {
    ...
    if (size != 0) {
      throw new IOException(
          "BackupEditStream has " + size + " records still to be flushed and cannot be closed.");
    }
