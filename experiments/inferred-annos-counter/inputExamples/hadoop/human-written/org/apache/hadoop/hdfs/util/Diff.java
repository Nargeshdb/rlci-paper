 ...
 *
 * <p>Given a previous state of a set and a sequence of create, delete and modify operations such
 * that the current state of the set can be obtained by applying the operations on the previous
 * state, the following algorithm construct the difference between the current state and the
 * previous state of the set.
 *
 * <pre>
...
public class Diff<K, E extends Diff.Element<K>> {
  ...
  /**
   * Undo information for some operations such as delete(E) and {@link Diff#modify(Element,
   * Element)}.
   */
  public static class UndoInfo<E> {
    ...
    private UndoInfo(
        final int createdInsertionPoint, final E trashed, final Integer deletedInsertionPoint) {
   ...
   *
   * @return -1 if the list is null; otherwise, return the insertion point defined in {@link
   *     Collections#binarySearch(List, Object)}. Note that, when the list is null, -1 is the
  ...
  private static <E> void remove(final List<E> elements, final int i, final E expected) {
    ...
    Preconditions.checkState(
        removed == expected, "removed != expected=%s, removed=%s.", expected, removed);
  }
  ...
  public List<E> getCreatedUnmodifiable() {
    return created != null ? Collections.unmodifiableList(created) : Collections.emptyList();
  }

  public E setCreated(int index, E element) {
    final E old = created.set(index, element);
    if (old.compareTo(element.getKey()) != 0) {
      throw new AssertionError("Element mismatched: element=" + element + " but old=" + old);
    }
    return old;
  }
  ...
  public List<E> getDeletedUnmodifiable() {
    return deleted != null ? Collections.unmodifiableList(deleted) : Collections.emptyList();
  }

  @SuppressWarnings(
      "mustcall:argument") // FP Java 8 type argument inference:
                           // https://github.com/typetools/checker-framework/issues/979 (validated)
  ...
  /**
   * @return null if the element is not found; otherwise, return the element in the deleted list.
   */
  @SuppressWarnings(
      "mustcall:argument") // FP Java 8 type argument inference:
                           // https://github.com/typetools/checker-framework/issues/979 (validated)
  ...
  @SuppressWarnings(
      "mustcall:type.argument") // FP Java 8 type argument inference:
  ...
  /**
   * Add the given element to the created list, provided the element does not exist, i.e. i < 0.
   *
   * @param i the insertion point defined in {@link Collections#binarySearch(List, Object)}
   * @throws AssertionError if i >= 0.
   */
  private void addCreated(final E element, final int i) {
    if (i >= 0) {
      throw new AssertionError(
          "Element already exists: element=" + element + ", created=" + created);
    }
    if (created == null) {
  ...
  private void addDeleted(final E element, final int i) {
    if (i >= 0) {
      throw new AssertionError(
          "Element already exists: element=" + element + ", deleted=" + deleted);
    }
    if (deleted == null) {
  ...
  /**
   * Undo the previous create(E) operation. Note that the behavior is undefined if the previous
   * operation is not create(E).
   */
  ...
  /**
   * Undo the previous delete(E) operation. Note that the behavior is undefined if the previous
   * operation is not delete(E).
   */
  ...
  public UndoInfo<E> modify(final E oldElement, final E newElement) {
    Preconditions.checkArgument(
        ...
        newElement);
    Preconditions.checkArgument(
        oldElement.compareTo(newElement.getKey()) == 0,
  ...
  /**
   * Undo the previous modify(E, E) operation. Note that the behavior is undefined if the previous
   * operation is not modify(E, E).
   */
   ...
   *
   * @return null if the element cannot be determined in the previous state since no change is
   *     recorded and it should be determined in the current state; otherwise, return a {@link
   *     Container} containing the element in the previous state. Note that the element can possibly
   *     be null which means that the element is not found in the previous state.
   */
  @SuppressWarnings({
    "mustcall:argument"
  }) // FP Java 8 type argument inference: https://github.com/typetools/checker-framework/issues/979
     // (validated)
   ...
   *
   * @return null if the element cannot be determined in the current state since no change is
   *     recorded and it should be determined in the previous state; otherwise, return a {@link
   *     Container} containing the element in the current state. Note that the element can possibly
   *     be null which means that the element is not found in the current state.
   */
  @SuppressWarnings(
      "mustcall:argument") // FP Java 8 type argument inference:
                           // https://github.com/typetools/checker-framework/issues/979 (validated)
  ...
  public List<E> apply2Previous(final List<E> previous) {
    return apply2Previous(previous, getCreatedUnmodifiable(), getDeletedUnmodifiable());
  }
  ...
  public List<E> apply2Current(final List<E> current) {
    return apply2Previous(current, getDeletedUnmodifiable(), getCreatedUnmodifiable());
  }
  ...
  public void combinePosterior(final Diff<K, E> posterior, final Processor<E> deletedProcesser) {
    final Iterator<E> createdIterator = posterior.getCreatedUnmodifiable().iterator();
    final Iterator<E> deletedIterator = posterior.getDeletedUnmodifiable().iterator();
