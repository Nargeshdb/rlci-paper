...
public class CryptoAdmin extends Configured implements Tool {
  ...
  /**
   * NN exceptions contain the stack trace as part of the exception message. When it's a known
   * error, pretty-print the error and squish the stack trace.
   */
  ...
  private static class CreateZoneCommand implements AdminHelper.Command {
    ...
    public String getLongUsage() {
      ...
      listing.addRow(
          "<path>",
          "The path of the encryption zone to create. "
  ...
  private static class GetFileEncryptionInfoCommand implements AdminHelper.Command {
    ...
    public int run(Configuration conf, List<String> args) throws IOException {
      ...
      try {
        final FileEncryptionInfo fei = admin.getFileEncryptionInfo(p);
        if (fei == null) {
  ...
  private static class ReencryptZoneCommand implements AdminHelper.Command {
    ...
    public String getLongUsage() {
      final TableListing listing = AdminHelper.getOptionDescriptionListing();
      listing.addRow("<action>", "The re-encrypt action to perform. Must be -start or -cancel.");
    ...
    public int run(Configuration conf, List<String> args) throws IOException {
      ...
      try {
        ...
        System.out.println(
            "re-encrypt command successfully submitted for "
                + "zone: "
                + path
                + " action: "
                + action);
      ...
      try {
        final TableListing listing =
            new TableListing.Builder()
                .addField("Zone Name")
                .addField("Status")
                .addField("EZKey Version Name")
                .addField("Submission Time")
                .addField("Is Canceled?")
                .addField("Completion Time")
                .addField("Number of files re-encrypted")
                .addField("Number of failures")
                .addField("Last File Checkpointed")
                .showHeaders()
                .build();
        final RemoteIterator<ZoneReencryptionStatus> it = admin.listReencryptionStatus();
        ...
        while (it.hasNext()) {
          ...
          listing.addRow(
              zs.getZoneName(),
              zs.getState().toString(),
              zs.getEzKeyVersionName(),
              Time.formatTime(zs.getSubmissionTime()),
        ...
        if (failuresMet) {
          System.out.println(
              "There are re-encryption failures. Files that are"
