...
/**
 * A JournalNode can manage journals for several clusters at once. Each such journal is entirely
 * independent despite being hosted by the same JVM.
 */
public class Journal implements Closeable {
  ...
  /**
   * When a new writer comes along, it asks each node to promise to ignore requests from any
   * previous writer, as identified by epoch number. In order to make such a promise, the epoch
   * number of that writer is stored persistently on disk.
   */
  private PersistentLongFile lastPromisedEpoch;

  /**
   * Each IPC that comes from a given client contains a serial number which only increases from the
   * client's perspective. Whenever we switch epochs, we reset this back to -1. Whenever an IPC
   * comes from a client, we ensure that it is strictly higher than any previous IPC. This guards
   * against any bugs in the IPC layer that would re-order IPCs or cause a stale retry from an old
   * request to resurface and confuse things.
   */
  private long currentEpochIpcSerial = -1;

  /**
   * The epoch number of the last writer to actually write a transaction. This is used to
   * differentiate log segments after a crash at the very beginning of a segment. See the the
   * 'testNewerVersionOfSegmentWins' test case.
   */
  private PersistentLongFile lastWriterEpoch;

  /**
   * Lower-bound on the last committed transaction ID. This is not depended upon for correctness,
   * but acts as a sanity check during the recovery procedures, and as a visibility mark for clients
   * reading in-progress logs.
   */
  ...
  /**
   * Reload any data that may have been cached. This is necessary when we first load the Journal,
   * but also after any formatting operation, since the cached data is no longer relevant.
   */
  ...
  private synchronized void refreshCachedData() {
    ...
    this.lastPromisedEpoch =
        new PersistentLongFile(new File(currentDir, LAST_PROMISED_FILENAME), 0);
    this.lastWriterEpoch = new PersistentLongFile(new File(currentDir, LAST_WRITER_EPOCH), 0);
    this.committedTxnId =
        new BestEffortLongFile(
            new File(currentDir, COMMITTED_TXID_FILENAME), HdfsServerConstants.INVALID_TXID);
  }

  /**
   * Scan the local storage directory, and return the segment containing the highest transaction.
   *
   * @return the EditLogFile with the highest transactions, or null if no files exist.
   */
  ...
  void format(NamespaceInfo nsInfo, boolean force) throws IOException {
    Preconditions.checkState(
        nsInfo.getNamespaceID() != 0, "can't format with uninitialized namespace info: %s", nsInfo);
    LOG.info(
        "Formatting journal id : "
            + journalId
            + " with namespace info: "
            + nsInfo
            + " and force: "
...
// for the purpose of using counting scripts
  ...
  /**
   * @return the last epoch which this node has promised not to accept any lower epoch, or 0 if no
  ...
  /**
   * Update the highest Tx ID that has been written to the journal. Also update the {@link
   * FileJournalManager#lastReadableTxId} of the underlying fjm.
   *
   ...
   *
   * @param nsInfo the namespace, which is verified for consistency or used to format, if the
   *     Journal has not yet been written to.
   * @param epoch the epoch to start
   * @return the status information necessary to begin recovery
   * @throws IOException if the node has already made a promise to another writer with a higher
   *     epoch number, if the namespace is inconsistent, or if a disk error occurs.
   */
  ...
  /**
   * Write a batch of edits to the journal. {@see QJournalProtocol#journal(RequestInfo, long, long,
   * int, byte[])}
   */
  synchronized void journal(
      RequestInfo reqInfo, long segmentTxId, long firstTxnId, int numTxns, byte[] records)
      throws IOException {
    checkFormatted();
    ...
    checkSync(curSegment != null, "Can't write, no segment open" + " ; journal id: " + journalId);
    ...
    if (curSegmentTxId != segmentTxId) {
      ...
      JournalOutOfSyncException e =
          new JournalOutOfSyncException(
    ...
    if (LOG.isTraceEnabled()) {
      LOG.trace("Writing txid " + firstTxnId + "-" + lastTxnId + " ; journal id: " + journalId);
    }
    if (cache != null) {
  ...
  private void checkFormatted() throws JournalNotFormattedException {
    if (!isFormatted()) {
      throw new JournalNotFormattedException(
          "Journal "
  ...
  /**
   * @throws JournalOutOfSyncException if the given expression is not true. The message of the
   *     exception is formatted using the 'msg' and 'formatArgs' parameters.
   */
  ...
  /**
   * @throws AssertionError if the given expression is not true. The message of the exception is
   *     formatted using the 'msg' and 'formatArgs' parameters.
   *     <p>This should be used in preference to Java's built-in assert in non-performance-critical
   *     paths, where a failure of this invariant might cause the protocol to lose data.
   */
   ...
   */
  public synchronized void startLogSegment(RequestInfo reqInfo, long txid, int layoutVersion)
      throws IOException {
    assert fjm != null;
    checkFormatted();
    checkRequest(reqInfo);
    ...
    if (existing != null) {
      if (!existing.isInProgress()) {
        throw new IllegalStateException(
            "Already have a finalized segment "
                + existing
                + " beginning at "
                + txid
                + " ; journal id: "
  ...
  public synchronized void finalizeLogSegment(RequestInfo reqInfo, long startTxId, long endTxId)
    ...
    if (elf.isInProgress()) {
      if (needsValidation) {
        LOG.info(
            "Validating log segment "
                + elf.getFile()
                + " about to be "
                + "finalized ; journal id: "
                + journalId);
        elf.scanLog(Long.MAX_VALUE, false);

        checkSync(
            elf.getLastTxId() == endTxId,
            "Trying to finalize in-progress log segment %s to end at "
      ...
      Preconditions.checkArgument(
          endTxId == elf.getLastTxId(),
          "Trying to re-finalize already finalized log "
              + elf
              + " with different endTxId "
              + endTxId
              + " ; journal id: "
              + journalId);
    }
   ...
   */
  public synchronized void purgeLogsOlderThan(RequestInfo reqInfo, long minTxIdToKeep)
      throws IOException {
    checkFormatted();
    checkRequest(reqInfo);
  ...
  /**
   * Remove the previously-recorded 'accepted recovery' information for a given log segment, once it
  ...
  private void purgePaxosDecision(long segmentTxId) throws IOException {
    ...
    if (paxosFile.exists()) {
      if (!paxosFile.delete()) {
        throw new IOException(
            "Unable to delete paxos file " + paxosFile + " ; journal id: " + journalId);
      }
    }
  }
   ...
   */
  public RemoteEditLogManifest getEditLogManifest(long sinceTxId, boolean inProgressOk)
      throws IOException {
   ...
   */
  public GetJournaledEditsResponseProto getJournaledEdits(long sinceTxId, int maxTxns)
      throws IOException {
    if (cache == null) {
      throw new IOException(
          "The journal edits cache is not enabled, which "
  ...
  /**
   * @return the current state of the given segment, or null if the segment does not exist.
   */
  @VisibleForTesting
  SegmentStateProto getSegmentInfo(long segmentTxId) throws IOException {
    ...
    SegmentStateProto ret =
        SegmentStateProto.newBuilder()
  ...
  public synchronized PrepareRecoveryResponseProto prepareRecovery(
    ...
    if (previouslyAccepted != null && !hasFinalizedSegment) {
      ...
      builder
          .setAcceptedInEpoch(previouslyAccepted.getAcceptedInEpoch())
  ...
  /**
   * @see QJournalProtocol#acceptRecovery(RequestInfo, QJournalProtocolProtos.SegmentStateProto,
   *     URL)
   */
  public synchronized void acceptRecovery(
      RequestInfo reqInfo, SegmentStateProto segment, URL fromUrl) throws IOException {
    checkFormatted();
    checkRequest(reqInfo);
    ...
    Preconditions.checkArgument(
        segment.getEndTxId() > 0 && segment.getEndTxId() >= segmentTxId,
    ...
    PersistedRecoveryPaxosData newData =
        PersistedRecoveryPaxosData.newBuilder()
    ...
    if (oldData != null) {
      alwaysAssert(
          oldData.getAcceptedInEpoch() <= reqInfo.getEpoch(),
    ...
    if (currentSegment == null || currentSegment.getEndTxId() != segment.getEndTxId()) {
      if (currentSegment == null) {
        LOG.info(
            "Synchronizing log "
                + TextFormat.shortDebugString(segment)
                + ": no current segment in place ; journal id: "
                + journalId);
        ...
        LOG.info(
            "Synchronizing log "
                ...
                + " is not the right length ; journal id: "
                + journalId);
        ...
        alwaysAssert(
            ...
            "Should never be asked to synchronize a different log on top of "
                + "an already-finalized segment ; journal id: "
                + journalId);
      ...
      LOG.info(
          "Skipping download of log "
              + TextFormat.shortDebugString(segment)
              + ": already have up-to-date logs ; journal id: "
              + journalId);
    }
  ...
  private Range<Long> txnRange(SegmentStateProto seg) {
    Preconditions.checkArgument(
        seg.hasEndTxId(), "invalid segment: %s ; journal id: %s", seg, journalId);
    return Range.between(seg.getStartTxId(), seg.getEndTxId());
  }

  /**
   * Synchronize a log segment from another JournalNode. The log is downloaded from the provided URL
   * into a temporary location on disk, which is named based on the current request's epoch.
   *
   * @return the temporary location of the downloaded file
   */
  private File syncLog(RequestInfo reqInfo, final SegmentStateProto segment, final URL url)
      throws IOException {
    final File tmpFile =
        storage.getSyncLogTemporaryFile(segment.getStartTxId(), reqInfo.getEpoch());
  ...
  /**
   * In the case the node crashes in between downloading a log segment and persisting the associated
   * paxos recovery data, the log segment will be left in its temporary location on disk. Given the
   * paxos data, we can check if this was indeed the case, and &quot;roll forward&quot; the atomic
   * operation.
   *
   * <p>See the inline comments in {@link #acceptRecovery(RequestInfo, SegmentStateProto, URL)} for
   * more details.
   *
   * @throws IOException if the temporary file is unable to be renamed into place
   */
  private void completeHalfDoneAcceptRecovery(PersistedRecoveryPaxosData paxosData)
      throws IOException {
    if (paxosData == null) {
      return;
    }
  ...
  private PersistedRecoveryPaxosData getPersistedPaxosData(long segmentTxId) throws IOException {
    ...
    try {
      ...
      Preconditions.checkState(
          ret != null && ret.getSegmentState().getStartTxId() == segmentTxId,
  ...
  private void persistPaxosData(long segmentTxId, PersistedRecoveryPaxosData newData)
      throws IOException {
    File f = storage.getPaxosFile(segmentTxId);
    boolean success = false;
    ...
    try {
      ...
      try (OutputStreamWriter writer = new OutputStreamWriter(fos, Charsets.UTF_8)) {
  ...
  public synchronized void doUpgrade(StorageInfo sInfo) throws IOException {
    ...
    PersistentLongFile prevLastPromisedEpoch =
        new PersistentLongFile(new File(previousDir, LAST_PROMISED_FILENAME), 0);
    PersistentLongFile prevLastWriterEpoch =
        new PersistentLongFile(new File(previousDir, LAST_WRITER_EPOCH), 0);
    BestEffortLongFile prevCommittedTxnId =
        new BestEffortLongFile(
  ...
  public synchronized void doFinalize() throws IOException {
    LOG.info(
        "Finalizing upgrade for journal "
            + storage.getRoot()
            + "."
  ...
  synchronized boolean moveTmpSegmentToCurrent(File tmpFile, File finalFile, long endTxId)
      throws IOException {
    ...
    if (endTxId <= committedTxnId.get()) {
      if (!finalFile.getParentFile().exists()) {
        LOG.error(
            finalFile.getParentFile()
                + " doesn't exist. Aborting tmp "
                + "segment move to current directory ; journal id: "
                + journalId);
        return false;
      }
      ...
      if (finalFile.exists() && FileUtil.canRead(finalFile)) {
        ...
        success = false;
        LOG.warn(
            "Unable to move edits file from "
                + tmpFile
                + " to "
                + finalFile
                + " ; journal id: "
      ...
      success = false;
      LOG.error(
          "The endTxId of the temporary file is not less than the "
