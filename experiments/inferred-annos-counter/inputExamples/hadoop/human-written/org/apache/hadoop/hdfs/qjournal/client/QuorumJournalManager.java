...
/**
 * A JournalManager that writes to a set of remote JournalNodes, requiring a quorum of nodes to ack
 * each write.
 */
...
public class QuorumJournalManager implements JournalManager {
  ...
  public static final String QJM_RPC_MAX_TXNS_KEY = "dfs.ha.tail-edits.qjm.rpc.max-txns";
  ...
  public QuorumJournalManager(
      Configuration conf, URI uri, NamespaceInfo nsInfo, String nameServiceId) throws IOException {
    this(conf, uri, nsInfo, nameServiceId, IPCLoggerChannel.FACTORY);
  }

  @VisibleForTesting
  QuorumJournalManager(
      Configuration conf, URI uri, NamespaceInfo nsInfo, AsyncLogger.Factory loggerFactory)
      throws IOException {
  ...
  QuorumJournalManager(
    ...
    this.timeoutMs =
        (int)
            conf.getTimeDuration(
                DFSConfigKeys.DFS_QJM_OPERATIONS_TIMEOUT,
                DFSConfigKeys.DFS_QJM_OPERATIONS_TIMEOUT_DEFAULT,
    ...
    this.connectionFactory =
        URLConnectionFactory.newDefaultURLConnectionFactory(connectTimeoutMs, readTimeoutMs, conf);
  ...
  public static void checkJournalId(String jid) {
    Preconditions.checkArgument(
        jid != null && !jid.isEmpty() && !jid.contains("/") && !jid.startsWith("."),
        "bad journal id: " + jid);
  }

  /**
   * Fence any previous writers, and obtain a unique epoch number for write-access to the journal
   * nodes.
   *
  ...
  Map<AsyncLogger, NewEpochResponseProto> createNewUniqueEpoch() throws IOException {
    ...
    Map<AsyncLogger, GetJournalStateResponseProto> lastPromises =
        loggers.waitForWriteQuorum(
            loggers.getJournalState(), getJournalStateTimeoutMs, "getJournalState()");
    ...
    Map<AsyncLogger, NewEpochResponseProto> resps =
        loggers.waitForWriteQuorum(
            loggers.newEpoch(nsInfo, myEpoch), newEpochTimeoutMs, "newEpoch(" + myEpoch + ")");
  ...
  public void format(NamespaceInfo nsInfo, boolean force) throws IOException {
    ...
    try {
      call.waitFor(loggers.size(), loggers.size(), 0, timeoutMs, "format");
   ...
   *
   * <ul>
   *   <li>This segment will be finalized on a majority of nodes.
   *   <li>All nodes which contain the finalized segment will agree on the length.
   * </ul>
   *
  ...
  private void recoverUnclosedSegment(long segmentTxId) throws IOException {
    ...
    Map<AsyncLogger, PrepareRecoveryResponseProto> prepareResponses =
        loggers.waitForWriteQuorum(
            prepare, prepareRecoveryTimeoutMs, "prepareRecovery(" + segmentTxId + ")");
    LOG.info(
    ...
    Entry<AsyncLogger, PrepareRecoveryResponseProto> bestEntry =
        Collections.max(prepareResponses.entrySet(), SegmentRecoveryComparator.INSTANCE);
    ...
    if (bestResponse.hasAcceptedInEpoch()) {
      ...
      for (PrepareRecoveryResponseProto resp : prepareResponses.values()) {
        assert !resp.hasSegmentState()
            : "One of the loggers had a response, but no best logger " + "was found.";
      }
    ...
    loggers.waitForWriteQuorum(
        finalize,
        finalizeSegmentTimeoutMs,
        String.format(
            "finalizeLogSegment(%s-%s)", logToSync.getStartTxId(), logToSync.getEndTxId()));
  }

  static List<AsyncLogger> createLoggers(
      Configuration conf,
      URI uri,
      NamespaceInfo nsInfo,
  ...
  public EditLogOutputStream startLogSegment(long txId, int layoutVersion) throws IOException {
    ...
    return new QuorumOutputStream(
        loggers, txId, outputBufferCapacity, writeTxnsTimeoutMs, layoutVersion);
  }
  ...
  public void setOutputBufferCapacity(int size) {
    int ipcMaxDataLength =
        conf.getInt(
            CommonConfigurationKeys.IPC_MAXIMUM_DATA_LENGTH,
            CommonConfigurationKeys.IPC_MAXIMUM_DATA_LENGTH_DEFAULT);
    if (size >= ipcMaxDataLength) {
      throw new IllegalArgumentException(
          "Attempted to use QJM output buffer "
              + "capacity ("
              + size
              + ") greater than the IPC max data length ("
              + CommonConfigurationKeys.IPC_MAXIMUM_DATA_LENGTH
              + " = "
              + ipcMaxDataLength
              + "). This will cause journals to reject edits.");
    }
  ...
  public void recoverUnfinalizedSegments() throws IOException {
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          "newEpoch(" + loggers.getEpoch() + ") responses:\n" + QuorumCall.mapToString(resps));
    }
  ...
  public void selectInputStreams(
    ...
    if (inProgressOk && inProgressTailingEnabled) {
      ...
      try {
        ...
        LOG.warn(
            "Encountered exception while tailing edits >= "
                + fromTxnId
                + " via RPC; falling back to streaming.",
            ioe);
      }
    }
  ...
  /**
   * Select input streams from the journals, specifically using the RPC mechanism optimized for low
   * latency.
   *
   ...
   * @param onlyDurableTxns Iff true, only include transactions which have been committed to a
   *     quorum of the journals.
   * @throws IOException Upon issues, including cache misses on the journals.
   */
  private void selectRpcInputStreams(
      Collection<EditLogInputStream> streams, long fromTxnId, boolean onlyDurableTxns)
      throws IOException {
    ...
    Map<AsyncLogger, GetJournaledEditsResponseProto> responseMap =
        loggers.waitForWriteQuorum(q, selectInputStreamsTimeoutMs, "selectRpcInputStreams");
    ...
    if (LOG.isDebugEnabled() || highestTxnCount < 0) {
      ...
      msg.append(fromTxnId).append("; got ").append(responseMap.size()).append(" responses: <");
      for (Map.Entry<AsyncLogger, GetJournaledEditsResponseProto> ent : responseMap.entrySet()) {
        msg.append("[")
            .append(ent.getKey())
            .append(", ")
            .append(ent.getValue().getTxnCount())
            .append("],");
      }
      msg.append(">");
      if (highestTxnCount < 0) {
        throw new IOException("Did not get any valid JournaledEdits " + "responses: " + msg);
      } else {
    ...
    if (logAction.shouldLog()) {
      LOG.info(
          "Selected loggers with >= "
              + maxAllowedTxns
              + " transactions "
              + "starting from lowest txn ID "
              + logAction.getStats(0).getMin()
              + LogThrottlingHelper.getLogSupressionMessage(logAction));
    }
    PriorityQueue<EditLogInputStream> allStreams =
        new PriorityQueue<>(JournalSet.EDIT_LOG_INPUT_STREAM_COMPARATOR);
    for (GetJournaledEditsResponseProto resp : responseMap.values()) {
      long endTxnId = fromTxnId - 1 + Math.min(maxAllowedTxns, resp.getTxnCount());
      allStreams.add(
          EditLogFileInputStream.fromByteString(resp.getEditLog(), fromTxnId, endTxnId, true));
    }
    JournalSet.chainAndMakeRedundantStreams(streams, allStreams, fromTxnId);
  }

  /**
   * Select input streams from the journals, specifically using the streaming mechanism optimized
   * for resiliency / bulk load.
   */
  private void selectStreamingInputStreams(
    ...
    Map<AsyncLogger, RemoteEditLogManifest> resps =
        loggers.waitForWriteQuorum(q, selectInputStreamsTimeoutMs, "selectStreamingInputStreams");

    LOG.debug(
        "selectStreamingInputStream manifests:\n"
            + Joiner.on("\n").withKeyValueSeparator(": ").join(resps));

    final PriorityQueue<EditLogInputStream> allStreams =
        new PriorityQueue<EditLogInputStream>(64, JournalSet.EDIT_LOG_INPUT_STREAM_COMPARATOR);
  ...
  public void doPreUpgrade() throws IOException {
    ...
    try {
      call.waitFor(loggers.size(), loggers.size(), 0, timeoutMs, "doPreUpgrade");
  ...
  public void doUpgrade(Storage storage) throws IOException {
    ...
    try {
      call.waitFor(loggers.size(), loggers.size(), 0, timeoutMs, "doUpgrade");
  ...
  public void doFinalize() throws IOException {
    ...
    try {
      call.waitFor(loggers.size(), loggers.size(), 0, timeoutMs, "doFinalize");
  ...
  public boolean canRollBack(StorageInfo storage, StorageInfo prevStorage, int targetLayoutVersion)
    ...
    QuorumCall<AsyncLogger, Boolean> call =
        loggers.canRollBack(storage, prevStorage, targetLayoutVersion);
    try {
      call.waitFor(loggers.size(), loggers.size(), 0, timeoutMs, "lockSharedStorage");

      if (call.countExceptions() > 0) {
        call.rethrowException(
            "Could not check if roll back possible for" + " one or more JournalNodes");
      }
  ...
  public void doRollback() throws IOException {
    ...
    try {
      call.waitFor(loggers.size(), loggers.size(), 0, timeoutMs, "doRollback");
  ...
  public void discardSegments(long startTxId) throws IOException {
    ...
    try {
      call.waitFor(loggers.size(), loggers.size(), 0, timeoutMs, "discardSegments");
      if (call.countExceptions() > 0) {
  ...
  public long getJournalCTime() throws IOException {
    ...
    try {
      call.waitFor(loggers.size(), loggers.size(), 0, timeoutMs, "getJournalCTime");
