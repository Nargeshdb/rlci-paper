...
class FSDirAclOp {
  static FileStatus modifyAclEntries(
      FSDirectory fsd, FSPermissionChecker pc, final String srcArg, List<AclEntry> aclSpec)
      throws IOException {
    String src = srcArg;
  ...
  static FileStatus removeDefaultAcl(FSDirectory fsd, FSPermissionChecker pc, final String srcArg)
      throws IOException {
    String src = srcArg;
    checkAclsConfigFlag(fsd);
    INodesInPath iip;
  ...
  static FileStatus removeAcl(FSDirectory fsd, FSPermissionChecker pc, final String srcArg)
      throws IOException {
    String src = srcArg;
    checkAclsConfigFlag(fsd);
    INodesInPath iip;
  ...
  static AclStatus getAclStatus(FSDirectory fsd, FSPermissionChecker pc, String src)
      throws IOException {
    checkAclsConfigFlag(fsd);
    fsd.readLock();
    try {
      ...
      return new AclStatus.Builder()
          .owner(inodeAttrs.getUserName())
          .group(inodeAttrs.getGroupName())
  ...
  static List<AclEntry> unprotectedSetAcl(
      FSDirectory fsd, INodesInPath iip, List<AclEntry> aclSpec, boolean fromEdits)
      throws IOException {
  ...
  private static void unprotectedRemoveAcl(FSDirectory fsd, INodesInPath iip) throws IOException {
    ...
    if (featureEntries.get(0).getScope() == AclEntryScope.ACCESS) {
      ...
      AclEntry groupEntryKey =
          new AclEntry.Builder().setScope(AclEntryScope.ACCESS).setType(AclEntryType.GROUP).build();
      int groupEntryIndex =
          Collections.binarySearch(
              featureEntries, groupEntryKey, AclTransformation.ACL_ENTRY_COMPARATOR);
      ...
      FsPermission newPerm =
          new FsPermission(
              perm.getUserAction(), groupPerm, perm.getOtherAction(), perm.getStickyBit());
