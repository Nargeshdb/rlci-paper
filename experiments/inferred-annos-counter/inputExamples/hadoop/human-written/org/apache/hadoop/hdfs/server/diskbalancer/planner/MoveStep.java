...
/**
 * Ignore fields with default values. In most cases Throughtput, diskErrors tolerancePercent and
 * bandwidth will be the system defaults. So we will avoid serializing them into JSON.
 */
@JsonInclude(JsonInclude.Include.NON_DEFAULT)
/** Move step is a step that planner can execute that will move data from one volume to another. */
public class MoveStep implements Step {
   ...
   */
  public MoveStep(
      DiskBalancerVolume sourceVolume,
      double idealStorage,
  ...
  /**
   * Tolerance Percentage indicates when a move operation is considered good enough. This is a
   * percentage of deviation from ideal that is considered fine.
   *
   * <p>For example : if the ideal amount on each disk was 1 TB and the tolerance was 10%, then
   * getting to 900 GB on the destination disk is considerd good enough.
   *
  ...
  /**
   * Gets the disk Bandwidth. That is the MB/Sec to copied. We will max out on this amount of
   * throughput. This is useful to prevent too much I/O on datanode while data node is in use.
   *
   * @return long.
   */
  @Override
   ...
   *
   * @param bandwidth - Long, MB / Sec of data to be moved between source and destinatin volume.
   */
  @Override
