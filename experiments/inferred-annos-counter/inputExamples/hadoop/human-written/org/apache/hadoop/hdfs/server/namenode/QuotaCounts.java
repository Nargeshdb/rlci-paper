...
public class QuotaCounts {
  ...
  /**
   * We pre-define 4 most common used EnumCounters objects. When the nsSsCounts and tsCounts are set
   * to the 4 most common used value, we just point them to the pre-defined const EnumCounters
   * objects instead of constructing many objects with the same value. See HDFS-14547.
   */
  static final EnumCounters<Quota> QUOTA_RESET =
      new ConstEnumCounters<>(Quota.class, HdfsConstants.QUOTA_RESET);

  static final EnumCounters<Quota> QUOTA_DEFAULT = new ConstEnumCounters<>(Quota.class, 0);
  ...
  /**
   * Modify counter with action. If the counter is ConstEnumCounters, copy all the values of it to a
   * new EnumCounters object, and modify the new obj.
   *
   * @param counter the EnumCounters to be modified.
   * @param action the modifying action on counter.
   * @return the modified counter.
   */
  static <T extends Enum<T>> EnumCounters<T> modify(
      EnumCounters<T> counter, Consumer<EnumCounters<T>> action) {
    try {
  ...
  @VisibleForTesting EnumCounters<Quota> nsSsCounts;
  // Storage type space counts
  @VisibleForTesting EnumCounters<StorageType> tsCounts;
  ...
  public static class Builder {
    ...
    public Builder storageSpace(long val) {
      nsSsCounts = setQuotaCounter(nsSsCounts, Quota.STORAGESPACE, Quota.NAMESPACE, val);
      return this;
    }
    ...
    public Builder quotaCount(QuotaCounts that) {
      ...
      if (that.tsCounts == STORAGE_TYPE_DEFAULT || that.tsCounts == STORAGE_TYPE_RESET) {
  ...
  /**
   * Set inputCounts' value of Quota type quotaToSet to val. inputCounts should be the left side
  ...
  private static EnumCounters<Quota> setQuotaCounter(
      EnumCounters<Quota> inputCounts, Quota quotaToSet, Quota otherQuota, long val) {
    if (val == HdfsConstants.QUOTA_RESET
