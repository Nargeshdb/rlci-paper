...
/**
 * The JournalNode is a daemon which allows namenodes using the QuorumJournalManager to log and
 * retrieve edits stored remotely. It is a thin wrapper around a local edit log directory with the
 * addition of facilities to participate in the quorum protocol.
 */
...
public class JournalNode implements Tool, Configurable, JournalNodeMXBean {
  ...
  synchronized Journal getOrCreateJournal(String jid, String nameServiceId, StartupOption startOpt)
      throws IOException {
  ...
  public void setConf(Configuration conf) {
    ...
    nameserviceIds = conf.getTrimmedStringCollection(DFSConfigKeys.DFS_INTERNAL_NAMESERVICES_KEY);

    if (nameserviceIds.size() == 0) {
      nameserviceIds = conf.getTrimmedStringCollection(DFSConfigKeys.DFS_NAMESERVICES);
    }
    ...
    if (nameserviceIds.size() < 2) {
      ...
      for (String nameService : nameserviceIds) {
        journalNodeDir = conf.get(DFSConfigKeys.DFS_JOURNALNODE_EDITS_DIR_KEY + "." + nameService);
      }
      if (journalNodeDir == null) {
        journalNodeDir =
            conf.get(
                DFSConfigKeys.DFS_JOURNALNODE_EDITS_DIR_KEY,
    ...
    if (this.tracer == null) {
      this.tracer =
          new Tracer.Builder("JournalNode")
   ...
   *
   * @param rc the status code with which to exit (non-zero should indicate an error)
   */
  ...
  private File getLogDir(String jid, String nameServiceId) throws IOException {
    ...
    if (nameServiceId != null) {
      dir = conf.get(DFSConfigKeys.DFS_JOURNALNODE_EDITS_DIR_KEY + "." + nameServiceId);
    }
    if (dir == null) {
      dir =
          conf.get(
  ...
  private class ErrorReporter implements StorageErrorReporter {
    ...
    public void reportErrorOnFile(File f) {
      LOG.error("Error reported on file " + f + "... exiting", new Exception());
      stop(1);
    }
  }
  ...
  @SuppressWarnings(
      "objectconstruction:reset.not.owning") // FP CreatesMustCallFor should permit the call
                                             ...
                                             // doUpgrade() below with or without
                                             // @CreatesMustCallFor on this method(validated)
  ...
  public Boolean canRollBack(
      ...
      throws IOException {
    return getOrCreateJournal(journalId, nameServiceId, StartupOption.ROLLBACK)
  ...
  public void doRollback(String journalId, String nameServiceId) throws IOException {
    getOrCreateJournal(journalId, nameServiceId, StartupOption.ROLLBACK).doRollback();
  }

  public void discardSegments(String journalId, long startTxId, String nameServiceId)
      throws IOException {
  ...
  protected InetSocketAddress getHttpServerBindAddress(Configuration configuration) {
    ...
    final String bindHost = configuration.getTrimmed(DFS_JOURNALNODE_HTTP_BIND_HOST_KEY);
