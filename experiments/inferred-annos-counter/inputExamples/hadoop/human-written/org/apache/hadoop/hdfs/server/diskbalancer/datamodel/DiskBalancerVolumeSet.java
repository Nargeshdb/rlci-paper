...
/**
 * DiskBalancerVolumeSet is a collection of storage devices on the data node which are of similar
 * StorageType.
 */
...
public class DiskBalancerVolumeSet {
  ...
  @JsonIgnore private TreeSet<DiskBalancerVolume> sortedQueue;
  ...
  /**
   * Computes Volume Data Density. Adding a new volume changes the volumeDataDensity for all
   * volumes. So we throw away our priority queue and recompute everything.
   *
   * <p>we discard failed volumes from this computation.
   *
   * <p>totalCapacity = totalCapacity of this volumeSet totalUsed = totalDfsUsed for this volumeSet
   * idealUsed = totalUsed / totalCapacity dfsUsedRatio = dfsUsedOnAVolume / Capacity On that Volume
  ...
  public void computeVolumeDataDensity() {
    ...
    for (DiskBalancerVolume volume : volumes) {
      if (!volume.isFailed() && !volume.isSkip()) {
        double dfsUsedRatio =
            truncateDecimals(volume.getUsed() / (double) volume.computeEffectiveCapacity());
  ...
  /**
   * Truncate to 4 digits since uncontrolled precision is some times counter intitive to what users
   * expect.
   *
   * @param value - double.
   * @return double.
  ...
  public void addVolume(DiskBalancerVolume volume) throws Exception {
    ...
    Preconditions.checkState(
        isTransient() == volume.isTransient(),
        "Mismatch in volumeSet and volume's transient " + "properties.");

    if (this.storageType == null) {
      Preconditions.checkState(
          volumes.size() == 0L, "Storage Type is Null but" + " volume size is " + volumes.size());
      this.storageType = volume.getStorageType();
    } else {
      Preconditions.checkState(
          this.storageType.equals(volume.getStorageType()),
  ...
  /**
   * Computes whether we need to do any balancing on this volume Set at all. It checks if any disks
   * are out of threshold value
   *
   ...
   *
   * <p>This call does not recompute the volumeDataDensity. It has to be done manually after this
   * call.
   *
