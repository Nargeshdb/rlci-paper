...
final class FSDirSatisfyStoragePolicyOp {
  ...
  /**
   * Private constructor for preventing FSDirSatisfyStoragePolicyOp object creation. Static-only
   * class.
   */
  private FSDirSatisfyStoragePolicyOp() {}

  /**
   * Satisfy storage policy function which will add the entry to SPS call queue and will perform
   * satisfaction async way.
   *
   ...
   * @throws IOException
   */
  static FileStatus satisfyStoragePolicy(
      FSDirectory fsd, BlockManager bm, String src, boolean logRetryCache) throws IOException {
  ...
  private static boolean inodeHasSatisfyXAttr(INode inode) {
    ...
    if (inode.isFile() && f != null && f.getXAttr(XATTR_SATISFY_STORAGE_POLICY) != null) {
      return true;
    }
    return false;
  }
  ...
  static void removeSPSXattr(FSDirectory fsd, INode inode, XAttr spsXAttr) throws IOException {
    try {
      ...
      XAttrStorage.updateINodeXAttrs(
          inode, existingXAttrs, INodesInPath.fromINode(inode).getLatestSnapshotId());
