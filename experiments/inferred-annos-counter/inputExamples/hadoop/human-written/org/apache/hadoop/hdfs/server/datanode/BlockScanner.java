...
public class BlockScanner {
  ...
  static class Conf {
    ...
    @VisibleForTesting
    static final long INTERNAL_DFS_BLOCK_SCANNER_CURSOR_SAVE_INTERVAL_MS_DEFAULT =
     ...
     *
     * <p>For compatibility with prior releases of HDFS, if the configured value is zero then the
     * scan period is set to 3 weeks.
     *
     * <p>If the configured value is less than zero then the scanner is disabled.
     *
     * @param conf Configuration object.
     * @return block scan period in milliseconds.
     */
    private static long getConfiguredScanPeriodMs(Configuration conf) {
      long tempScanPeriodMs =
          getUnitTestLong(
              conf,
              INTERNAL_DFS_DATANODE_SCAN_PERIOD_MS,
              TimeUnit.MILLISECONDS.convert(
                  conf.getLong(
                      DFS_DATANODE_SCAN_PERIOD_HOURS_KEY, DFS_DATANODE_SCAN_PERIOD_HOURS_DEFAULT),
                  TimeUnit.HOURS));

      if (tempScanPeriodMs == 0) {
        tempScanPeriodMs =
            TimeUnit.MILLISECONDS.convert(DFS_DATANODE_SCAN_PERIOD_HOURS_DEFAULT, TimeUnit.HOURS);
      }
    ...
    Conf(Configuration conf) {
      ...
      if (allowUnitTestSettings) {
        this.resultHandler =
            (Class<? extends ScanResultHandler>)
                conf.getClass(INTERNAL_VOLUME_SCANNER_SCAN_RESULT_HANDLER, ScanResultHandler.class);
      } else {
   ...
   *
   * <p>If the block scanner is disabled, no volume scanners will be created, and no threads will
   * start.
   */
   ...
   *
   * <p>This function will block until the volume scanner has stopped.
   *
  ...
  public synchronized void removeVolumeScanner(FsVolumeSpi volume) {
    ...
    LOG.info("Removing scanner for volume {} (StorageID {})", volume, volume.getStorageID());
    scanner.shutdown();
   ...
   *
   * <p>This function will block until all the volume scanners have stopped.
   */
   ...
   *
   * <p>This means that we should try to rescan it soon. Note that the VolumeScanner keeps a list of
   * recently suspicious blocks, which it uses to avoid rescanning the same block over and over in a
   * short time frame.
   *
   * @param storageId The ID of the storage where the block replica is being stored.
   * @param block The block's ID and block pool id.
   */
  synchronized void markSuspectBlock(String storageId, ExtendedBlock block) {
    if (!isEnabled()) {
      LOG.debug(
          "Not scanning suspicious block {} on {}, because the block " + "scanner is disabled.",
          block,
          storageId);
      return;
    }
    ...
    if (scanner == null) {
      ...
      LOG.info(
          "Not scanning suspicious block {} on {}, because there is no "
              + "volume scanner for that storageId.",
          block,
          storageId);
      return;
    }
