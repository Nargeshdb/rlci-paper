...
/**
 * The HDFS-specific representation of a network topology inner node. The difference is this class
 * includes the information about the storage type info of this subtree. This info will be used when
 * selecting subtrees in block placement.
 */
public class DFSTopologyNodeImpl extends InnerNodeImpl {
  ...
  static final InnerNodeImpl.Factory FACTORY = new DFSTopologyNodeImpl.Factory();
  ...
  /**
   * The core data structure of this class. The information about what storage types this subtree
   * has. Basically, a map whose key is a child id, value is a enum map including the counts of each
   * storage type. e.g. DISK type has count 5 means there are 5 leaf datanodes with DISK type
   * available. This value is set/updated upon datanode joining and leaving.
   *
   * <p>NOTE : It might be sufficient to keep only a map from storage type to count, omitting the
   * child node id. But this might make it hard to keep consistency when there are updates from
   * children.
   *
   * <p>For example, if currently R has two children A and B with storage X, Y, and A : X=1 Y=1 B :
   * X=2 Y=2 so we store X=3 Y=3 as total on R.
   *
   * <p>Now say A has a new X plugged in and becomes X=2 Y=1.
   *
   * <p>If we know that "A adds one X", it is easy to update R by +1 on X. However, if we don't know
   * "A adds one X", but instead got "A now has X=2 Y=1", (which seems to be the case in current
   * heartbeat) we will not know how to update R. While if we store on R "A has X=1 and Y=1" then we
   * can simply update R by completely replacing the A entry and all will be good.
   */
  private final HashMap<String, EnumMap<StorageType, Integer>> childrenStorageInfo;

  /**
   * This map stores storage type counts of the subtree. We can always get this info by iterate over
   * the childrenStorageInfo variable. But for optimization purpose, we store this info directly to
   * avoid the iteration.
   */
   ...
   *
   * <p>In normal execution, nodes are added only once and this should not happen. However if node
   * restarts, we may run into the case where the same node tries to add itself again with
   * potentially different storage type info. In this case this method will update the meta data
   * according to the new storage info.
   *
   * <p>Note that it is important to also update all the ancestors if we do have updated the local
   * node storage info.
   *
   * @param dnDescriptor the node that is added another time, with potentially different storage
   *     types.
   */
  private void updateExistingDatanode(DatanodeDescriptor dnDescriptor) {
    if (childrenStorageInfo.containsKey(dnDescriptor.getName())) {
      ...
      boolean same =
          dnDescriptor.getStorageTypes().size()
              == childrenStorageInfo.get(dnDescriptor.getName()).keySet().size();
      for (StorageType type : childrenStorageInfo.get(dnDescriptor.getName()).keySet()) {
      ...
      for (StorageType type : dnDescriptor.getStorageTypes()) {
        if (!childrenStorageInfo.get(dnDescriptor.getName()).containsKey(type)) {
  ...
  private DFSTopologyNodeImpl createParentNode(String parentName) {
    return new DFSTopologyNodeImpl(parentName, getPath(this), this, this.getLevel() + 1);
  }
   ...
   *
   * <p>lock is needed as different datanodes may call recursively to modify the same parent. TODO :
   * this may not happen at all, depending on how heartheat is processed
   *
  ...
  public synchronized void childAddStorage(String childName, StorageType type) {
    ...
    EnumMap<StorageType, Integer> typeCount = childrenStorageInfo.get(childName);
