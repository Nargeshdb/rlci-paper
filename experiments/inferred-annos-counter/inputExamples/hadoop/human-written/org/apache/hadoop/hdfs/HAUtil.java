...
public class HAUtil {
  ...
  public static final org.slf4j.Logger LOG = LoggerFactory.getLogger(HAUtil.class.getName());
  ...
  /**
   * Get the namenode Id by matching the {@code addressKey} with the the address of the local node.
   *
   * <p>If {@link DFSConfigKeys#DFS_HA_NAMENODE_ID_KEY} is not specifically configured, this method
   * determines the namenode Id by matching the local node's address with the configured addresses.
   * When a match is found, it returns the namenode Id from the corresponding configuration key.
   *
   * @param conf Configuration
  ...
  public static String getNameNodeId(Configuration conf, String nsId) {
    ...
    String suffixes[] =
        DFSUtil.getSuffixIDs(
            conf, DFS_NAMENODE_RPC_ADDRESS_KEY, nsId, null, DFSUtil.LOCAL_ADDRESS_MATCHER);
    if (suffixes == null) {
      String msg =
          "Configuration "
              + DFS_NAMENODE_RPC_ADDRESS_KEY
  ...
  /**
   * Similar to {@link DFSUtil#getNameServiceIdFromAddress(Configuration, InetSocketAddress,
   * String...)}
   */
  public static String getNameNodeIdFromAddress(
  ...
  /**
   * Given the configuration for this node, return a list of configurations for the other nodes in
   * an HA setup.
   *
  ...
  /**
   * Check whether logical URI is needed for the namenode and the corresponding failover proxy
  ...
  public static boolean useLogicalUri(Configuration conf, URI nameNodeUri) throws IOException {
    ...
    AbstractNNFailoverProxyProvider<ClientProtocol> provider =
        NameNodeProxiesClient.createFailoverProxyProvider(
            conf, nameNodeUri, ClientProtocol.class, false, null);
  ...
  /**
   * Get the internet address of the currently-active NN. This should rarely be used, since callers
   * of this method who connect directly to the NN using the resulting InetSocketAddress will not be
   * able to connect to the active NN if a failover were to occur after this method has been called.
   *
  ...
  public static InetSocketAddress getAddressOfActive(FileSystem fs) throws IOException {
    ...
    if (isHAEnabled(dfsConf, nsId)) {
      List<ClientProtocol> namenodes = getProxiesForAllNameNodesInNameservice(dfsConf, nsId);
  ...
  /**
   * Get an RPC proxy for each NN in an HA nameservice. Used when a given RPC call should be made on
   * every NN in an HA nameservice, not just the active.
   *
   * @param conf configuration
  ...
  public static <T> List<ProxyAndInfo<T>> getProxiesForAllNameNodesInNameservice(
    ...
    for (InetSocketAddress nnAddress : nnAddresses.values()) {
      ProxyAndInfo<T> proxyInfo =
          NameNodeProxies.createNonHAProxy(
              conf, nnAddress, xface, UserGroupInformation.getCurrentUser(), false);
