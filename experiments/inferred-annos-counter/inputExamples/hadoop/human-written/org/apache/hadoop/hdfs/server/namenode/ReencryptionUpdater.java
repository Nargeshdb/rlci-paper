...
/**
 * Class for finalizing re-encrypt EDEK operations, by updating file xattrs with edeks returned from
 * reencryption.
 *
 * <p>The tasks are submitted by ReencryptionHandler.
 *
 * <p>It is assumed only 1 Updater will be running, since updating file xattrs requires namespace
 * write lock, and performance gain from multi-threading is limited.
 */
...
public final class ReencryptionUpdater implements Runnable {
  ...
  /**
   * Class to track re-encryption submissions of a single zone. It contains all the submitted
   * futures, and statistics about how far the futures are processed.
   */
  ...
  /**
   * Class representing the task for one batch of a re-encryption command. It also contains
   * statistics about how far this single batch has been executed.
   */
  static final class ReencryptionTask {
    ...
    ReencryptionTask(final long id, final int failures, final ReencryptionBatch theBatch) {
      zoneId = id;
  ...
  /**
   * Class that encapsulates re-encryption details of a file. It contains the file inode, stores the
   * initial edek of the file, and the new edek after re-encryption.
   *
   * <p>Assumptions are the object initialization happens when dir lock is held, and inode is valid
   * and is encrypted during initialization.
   *
   * <p>Namespace changes may happen during re-encryption, and if inode is changed the re-encryption
   * is skipped.
   */
  static final class FileEdekInfo {
    ...
    FileEdekInfo(FSDirectory dir, INodeFile inode) throws IOException {
      ...
      Preconditions.checkNotNull(fei, "FileEncryptionInfo is null for " + inodeId);
      existingEdek =
          EncryptedKeyVersion.createForDecryption(
              fei.getKeyName(),
              fei.getEzKeyVersionName(),
              fei.getIV(),
              fei.getEncryptedDataEncryptionKey());
    }
  ...
  ReencryptionUpdater(
    ...
    Preconditions.checkArgument(
        throttleLimitRatio > 0.0f,
        DFS_NAMENODE_REENCRYPT_THROTTLE_LIMIT_UPDATER_RATIO_KEY + " is not positive.");
  }

  /**
   * Called by the submission thread to indicate all tasks have been submitted. If this is called
   * but no tasks has been submitted, the re-encryption is considered complete.
   *
   * @param zoneId Id of the zone inode.
   * @throws IOException
   * @throws InterruptedException
   */
  void markZoneSubmissionDone(final long zoneId) throws IOException, InterruptedException {
  ...
  /**
   * Process a completed ReencryptionTask. Each inode id is resolved to an INode object, skip if the
   * inode is deleted.
   *
   * <p>Only file xattr is updated by this method. Re-encryption progress is not updated.
   *
   ...
   * @throws InterruptedException
   */
  private void processTaskEntries(final String zoneNodePath, final ReencryptionTask task)
    ...
    if (!task.batch.isEmpty() && task.numFailures == 0) {
      LOG.debug(
          ...
          zoneNodePath,
          task.batch.getFirstFilePath());
      final int batchSize = task.batch.size();
      for (Iterator<FileEdekInfo> it = task.batch.getBatch().iterator(); it.hasNext(); ) {
        ...
        if (inode == null) {
          LOG.debug("INode {} doesn't exist, skipping re-encrypt.", entry.getInodeId());
        ...
        if (!fei.getKeyName().equals(entry.edek.getEncryptionKeyName())) {
          LOG.debug("Inode {} EZ key changed, skipping re-encryption.", entry.getInodeId());
          it.remove();
          continue;
        }
        if (fei.getEzKeyVersionName().equals(entry.edek.getEncryptionKeyVersionName())) {
          LOG.debug(
              "Inode {} EZ key version unchanged, skipping re-encryption.", entry.getInodeId());
          it.remove();
          continue;
        }
        if (!Arrays.equals(
          ...
          LOG.debug("Inode {} existing edek changed, skipping re-encryption", entry.getInodeId());
          it.remove();
          continue;
        }
        FileEncryptionInfo newFei =
            new FileEncryptionInfo(
                ...
                entry.edek.getEncryptedKeyIv(),
                fei.getKeyName(),
                entry.edek.getEncryptionKeyVersionName());
        final INodesInPath iip = INodesInPath.fromINode(inode);
        FSDirEncryptionZoneOp.setFileEncryptionInfo(dir, iip, newFei, XAttrSetFlag.REPLACE);
        task.lastFile = iip.getPath();
        ++task.numFilesUpdated;
      }

      LOG.info(
          "Updated xattrs on {}({}) files in zone {} for re-encryption," + " starting:{}.",
          task.numFilesUpdated,
          batchSize,
          zoneNodePath,
          task.batch.getFirstFilePath());
  ...
  /**
   * Iterate tasks for the given zone, and update progress accordingly. The checkpoint indicates all
   * files before it are done re-encryption, so it will be updated to the position where all tasks
   * before are completed.
   *
   * @param zoneNode the EZ inode.
   * @param tracker the zone submission tracker.
   * @return the list containing the last checkpointed xattr. Empty if no checkpoint happened.
   * @throws ExecutionException
   * @throws IOException
   * @throws InterruptedException
   */
  private List<XAttr> processCheckpoints(final INode zoneNode, final ZoneSubmissionTracker tracker)
      throws ExecutionException, IOException, InterruptedException {
    assert dir.hasWriteLock();
    final long zoneId = zoneNode.getId();
    final String zonePath = zoneNode.getFullPathName();
    final ZoneReencryptionStatus status = handler.getReencryptionStatus().getZoneStatus(zoneId);
    assert status != null;
    ...
    synchronized (handler) {
      while (iter.hasNext()) {
        ...
        LOG.debug(
            "Updating re-encryption checkpoint with completed task." + " last: {} size:{}.",
            task.lastFile,
            task.batch.size());
        assert zoneId == task.zoneId;
        try {
          final XAttr xattr =
              FSDirEncryptionZoneOp.updateReencryptionProgress(
                  dir, zoneNode, status, task.lastFile, task.numFilesUpdated, task.numFailures);
          xAttrs.clear();
          xAttrs.add(xattr);
        } catch (IOException ie) {
          LOG.warn(
              "Failed to update re-encrypted progress to xattr" + " for zone {}", zonePath, ie);
          ++task.numFailures;
        }
    ...
    if (tracker.isCompleted()) {
      LOG.debug(
          "Removed re-encryption tracker for zone {} because it completed" + " with {} tasks.",
          zonePath,
          tracker.numCheckpointed);
  ...
  private void takeAndProcessTasks() throws Exception {
    ...
    do {
      ...
      try {
        ...
        LOG.info(
            "Exception when processing re-encryption task for zone {}, " + "retrying...",
            task.zoneId,
            re);
        shouldRetry = true;
  ...
  private void processTask(ReencryptionTask task)
    ...
    try {
      ...
      LOG.info(
          "Processing returned re-encryption task for zone {}({}), " + "batch size {}, start:{}",
          zonePath,
          task.zoneId,
          task.batch.size(),
          task.batch.getFirstFilePath());
      final ZoneSubmissionTracker tracker = handler.getTracker(zoneNode.getId());
      if (tracker == null) {
  ...
  private synchronized void checkPauseForTesting() throws InterruptedException {
    ...
    if (pauseAfterNthCheckpoint != 0) {
      ZoneSubmissionTracker tracker = handler.unprotectedGetTracker(pauseZoneId);
      if (tracker != null) {
  ...
  /**
   * Throttles the ReencryptionUpdater to prevent from contending FSN/FSD write locks. This is done
  ...
  private void throttle() throws InterruptedException {
    ...
    final long expect = (long) (throttleTimerAll.now(TimeUnit.MILLISECONDS) * throttleLimitRatio);
    final long actual = throttleTimerLocked.now(TimeUnit.MILLISECONDS);
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          "Re-encryption updater throttling expect: {}, actual: {}," + " throttleTimerAll:{}",
          expect,
          actual,
    ...
    if (expect - actual < 0) {
      ...
      final long sleepMs =
          (long) (actual / throttleLimitRatio) - throttleTimerAll.now(TimeUnit.MILLISECONDS);
