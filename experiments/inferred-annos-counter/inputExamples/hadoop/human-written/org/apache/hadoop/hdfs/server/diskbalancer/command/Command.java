...
public abstract class Command extends Configured implements Closeable {
   ...
   *
   * <p>The main goal is to delete id file created in {@link org.apache.hadoop.hdfs.server.balancer
   * .NameNodeConnector#checkAndMarkRunning} , otherwise, it's not allowed to run multiple commands
   * in a row.
   */
  @Override
  ...
  protected void setOutputPath(String path) throws IOException {
        ...
        diskBalancerLogs =
            new Path(
                System.getProperty("user.dir")
                    + DEFAULT_LOG_DIR.toString()
                    + Path.SEPARATOR
                    + format.format(now));
      } else {
        diskBalancerLogs =
            new Path(DEFAULT_LOG_DIR.toString() + Path.SEPARATOR + format.format(now));
      }
    } else {
    ...
    if (fs.exists(diskBalancerLogs)) {
      LOG.debug(
          "Another Diskbalancer instance is running ? - Target " + "Directory already exists. {}",
          diskBalancerLogs);
      throw new IOException(
          "Another DiskBalancer files already exist at the "
              + "target location. "
              + diskBalancerLogs.toString());
    }
  ...
  protected void setNodesToProcess(List<DiskBalancerDataNode> nodes) {
    if (cluster == null) {
      throw new IllegalStateException(
          "Set nodes to process invoked before " + "initializing cluster. Illegal usage.");
    }
  ...
  protected Set<String> getNodeList(String listArg) throws IOException {
    ...
    if (listArg.startsWith("file://")) {
      ...
      try {
        HostsFileReader.readFileToSet(
            "include", Paths.get(listURL.getPath()).toString(), resultSet);
      } catch (NoSuchFileException e) {
        String warnMsg =
            String.format(
                "The input host file path '%s' is not a valid path. "
                    + "Please make sure the host file exists.",
                listArg);
        throw new DiskBalancerException(
            warnMsg, DiskBalancerException.Result.INVALID_HOST_FILE_PATH);
      }
    } else {
      nodeData = listArg;
  ...
  protected List<DiskBalancerDataNode> getNodes(String listArg) throws IOException {
    ...
    if (!invalidNodeList.isEmpty()) {
      ...
      String warnMsg =
          String.format(
              "The node(s) '%s' not found. " + "Please make sure that '%s' exists in the cluster.",
  ...
  protected void verifyCommandOptions(String commandName, CommandLine cmd) {
    ...
    while (iter.hasNext()) {
      ...
      if (!validArgs.containsKey(opt.getLongOpt())) {
        String errMessage =
            String.format(
                "%nInvalid argument found for command %s : %s%n", commandName, opt.getLongOpt());
  ...
  public ClientDatanodeProtocol getDataNodeProxy(String datanode) throws IOException {
    ...
    ClientDatanodeProtocol dnProtocol =
        DFSUtilClient.createClientDatanodeProtocolProxy(
            ...
            getUGI(),
            getConf(),
            NetUtils.getSocketFactory(getConf(), ClientDatanodeProtocol.class));
    return dnProtocol;
  }
  ...
  protected int parseTopNodes(final CommandLine cmd, final TextStringBuilder result)
    ...
    if (StringUtils.isBlank(topVal)) {
      outputLine =
          String.format("No top limit specified, using default top value %d.", getDefaultTop());
      LOG.info(outputLine);
      result.appendln(outputLine);
      nodes = getDefaultTop();
    } else {
      try {
        ...
        outputLine =
            String.format(
                "Top limit input is not numeric, using default top value %d.", getDefaultTop());
        LOG.info(outputLine);
        result.appendln(outputLine);
        nodes = getDefaultTop();
      }
  ...
  /**
   * Reads the Physical path of the disks we are balancing. This is needed to make the disk balancer
   * human friendly and not used in balancing.
   *
  ...
  protected void populatePathNames(DiskBalancerDataNode node) throws IOException {
    ...
    String volumeNameJson =
        dnClient.getDiskBalancerSetting(DiskBalancerConstants.DISKBALANCER_VOLUME_NAME);

    @SuppressWarnings("unchecked")
    Map<String, String> volumeMap = READER.readValue(volumeNameJson);
