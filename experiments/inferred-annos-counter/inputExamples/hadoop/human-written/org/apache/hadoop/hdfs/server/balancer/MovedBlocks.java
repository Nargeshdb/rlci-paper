...
/**
 * This window makes sure to keep blocks that have been moved within a fixed time interval (default
 * is 1.5 hour). Old window has blocks that are older; Current window has blocks that are more
 * recent; Cleanup method triggers the check if blocks in the old window are more than the fixed
 * time interval. If yes, purge the old window and then move blocks in current window to old window.
 *
