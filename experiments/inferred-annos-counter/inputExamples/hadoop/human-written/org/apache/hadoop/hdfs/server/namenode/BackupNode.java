 ...
 * BackupNode.
 *
 * <p>Backup node can play two roles.
 *
 * <ol>
 *   <li>{@link NamenodeRole#CHECKPOINT} node periodically creates checkpoints, that is downloads
 *       image and edits from the active node, merges them, and uploads the new image back to the
 *       active.
 *   <li>{@link NamenodeRole#BACKUP} node keeps its namespace in sync with the active node, and
 *       periodically creates checkpoints by simply saving the namespace image to local disk(s).
 * </ol>
 */
...
public class BackupNode extends NameNode {
  ...
  protected void loadNamesystem(Configuration conf) throws IOException {
    conf.setFloat(
        DFSConfigKeys.DFS_NAMENODE_SAFEMODE_THRESHOLD_PCT_KEY, BN_SAFEMODE_THRESHOLD_PCT_DEFAULT);
  ...
  static class BackupNodeRpcServer extends NameNodeRpcServer implements JournalProtocol {
    private BackupNodeRpcServer(Configuration conf, BackupNode nn) throws IOException {
      ...
      BlockingService service =
          JournalProtocolService.newReflectiveBlockingService(journalProtocolTranslator);
    ...
    private void verifyJournalRequest(JournalInfo journalInfo) throws IOException {
      ...
      if (journalInfo.getNamespaceId() != expectedNamespaceID) {
        errorMsg =
            "Invalid namespaceID in journal request - expected "
                + expectedNamespaceID
                + " actual "
                + journalInfo.getNamespaceId();
        LOG.warn(errorMsg);
        throw new UnregisteredNodeException(journalInfo);
      }
      if (!journalInfo.getClusterId().equals(namesystem.getClusterId())) {
        errorMsg =
            "Invalid clusterId in journal request - expected "
                + journalInfo.getClusterId()
                + " actual "
    ...
    @Override
    public void journal(
        JournalInfo journalInfo, long epoch, long firstTxId, int numTxns, byte[] records)
        throws IOException {
      namesystem.checkOperation(OperationCategory.JOURNAL);
      verifyJournalRequest(journalInfo);
    ...
    @Override
    public FenceResponse fence(JournalInfo journalInfo, long epoch, String fencerInfo)
        throws IOException {
  ...
  private NamespaceInfo handshake(Configuration conf) throws IOException {
    ...
    this.nnHttpAddress =
        DFSUtil.getInfoServer(nnAddress, conf, DFSUtil.getHttpClientScheme(conf)).toURL();
  ...
  private static NamespaceInfo handshake(NamenodeProtocol namenode)
    ...
    if (!nsInfo.getBuildVersion().equals(Storage.getBuildVersion())) {
      errorMsg =
          "Incompatible build versions: active name-node BV = "
              + nsInfo.getBuildVersion()
              + "; backup node BV = "
  ...
  private class BNHAContext extends NameNodeHAContext {
     ...
     *
     * <p>The following services should be muted (not run or not pass any control commands to
     * DataNodes) on BackupNode: {@link LeaseManager.Monitor} protected by SafeMode. {@link
     * BlockManager.RedundancyMonitor} protected by SafeMode. {@link HeartbeatManager.Monitor}
     * protected by SafeMode. {@link DatanodeAdminManager.Monitor} need to prohibit refreshNodes().
     * {@link PendingReconstructionBlocks.PendingReconstructionMonitor} harmless, because
     * RedundancyMonitor is muted.
     */
    @Override
