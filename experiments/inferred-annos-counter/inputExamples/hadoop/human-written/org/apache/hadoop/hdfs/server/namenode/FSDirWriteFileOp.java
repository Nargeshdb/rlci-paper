...
class FSDirWriteFileOp {
  ...
  static boolean unprotectedRemoveBlock(
      FSDirectory fsd, String path, INodesInPath iip, INodeFile fileNode, Block block)
      throws IOException {
    ...
    if (uc.getUnderConstructionFeature() != null) {
      DatanodeStorageInfo.decrementBlocksScheduled(
          uc.getUnderConstructionFeature().getExpectedStorageLocations());
    }
    fsd.getBlockManager().removeBlockFromMap(uc);

    if (NameNode.stateChangeLog.isDebugEnabled()) {
      NameNode.stateChangeLog.debug(
          "DIR* FSDirectory.removeBlock: "
              + path
              + " with "
              + block
    ...
    fsd.updateCount(
        iip, 0, -fileNode.getPreferredBlockSize(), fileNode.getPreferredBlockReplication(), true);
    return true;
  }
  ...
  static void checkBlock(FSNamesystem fsn, ExtendedBlock block) throws IOException {
    ...
    if (block != null && !bpId.equals(block.getBlockPoolId())) {
      throw new IOException(
          "Unexpected BlockPoolId " + block.getBlockPoolId() + " - expected " + bpId);
    }
  }

  /**
   * Part I of getAdditionalBlock(). Analyze the state of the file under read lock to determine if
   * the client can add a new block, detect potential retries, lease mismatches, and minimal
   * replication of the penultimate block.
   *
   * <p>Generate target DataNode locations for the new block, but do not create the new block yet.
   */
  static ValidateAddBlockResult validateAddBlock(
    ...
    if (blockType == BlockType.STRIPED) {
      ...
      numTargets =
          (short)
              (ecPolicy.getSchema().getNumDataUnits() + ecPolicy.getSchema().getNumParityUnits());
    } else {
      numTargets = pendingFile.getFileReplication();
    }
    storagePolicyID = pendingFile.getStoragePolicyID();
    return new ValidateAddBlockResult(
        blockSize, numTargets, storagePolicyID, clientMachine, blockType, ecPolicy);
  }

  static LocatedBlock makeLocatedBlock(
      FSNamesystem fsn, BlockInfo blk, DatanodeStorageInfo[] locs, long offset) throws IOException {
    LocatedBlock lBlk =
        BlockManager.newLocatedBlock(fsn.getExtendedBlock(new Block(blk)), blk, locs, offset);
    fsn.getBlockManager().setBlockToken(lBlk, BlockTokenIdentifier.AccessMode.WRITE);
    return lBlk;
  }

  /**
   * Part II of getAdditionalBlock(). Should repeat the same analysis of the file state as in Part
   * 1, but under the write lock. If the conditions still hold, then allocate a new block with the
   * new targets, add it to the INode and to the BlocksMap.
   */
  static LocatedBlock storeAllocatedBlock(
      FSNamesystem fsn,
      String src,
      long fileId,
      String clientName,
    ...
    if (onRetryBlock[0] != null) {
      if (onRetryBlock[0].getLocations().length > 0) {
        ...
        lastBlockInFile
            ...
            .setExpectedLocations(lastBlockInFile, targets, pendingFile.getBlockType());
    ...
    fsn.commitOrCompleteLastBlock(
        pendingFile, fileState.iip, ExtendedBlock.getLocalBlock(previous));
  ...
  static DatanodeStorageInfo[] chooseTargetForNewBlock(
      ...
      ValidateAddBlockResult r)
      throws IOException {
    Node clientNode = null;

    boolean ignoreClientLocality =
        (flags != null && flags.contains(AddBlockFlag.IGNORE_CLIENT_LOCALITY));
    ...
    Set<Node> excludedNodesSet =
        (excludedNodes == null) ? new HashSet<>() : new HashSet<>(Arrays.asList(excludedNodes));

    List<String> favoredNodesList =
        (favoredNodes == null) ? Collections.emptyList() : Arrays.asList(favoredNodes);

    // choose targets for the new block to be allocated.
    return bm.chooseTarget4NewBlock(
        src,
        r.numTargets,
        clientNode,
        excludedNodesSet,
  ...
  static INodesInPath resolvePathForStartFile(
      FSDirectory dir,
      FSPermissionChecker pc,
      String src,
   ...
   * Once the file is create the client then allocates a new block with the next call using {@link
  ...
  static HdfsFileStatus startFile(
      ...
      INode.BlocksMapUpdateInfo toRemoveBlocks,
    ...
    if (iip.getLastINode() != null) {
      if (overwrite) {
        ...
        long ret =
            FSDirDeleteOp.delete(fsd, iip, toRemoveBlocks, toRemoveINodes, toRemoveUCFiles, now());
        if (ret >= 0) {
        ...
        fsn.recoverLeaseInternal(
            FSNamesystem.RecoverLeaseOp.CREATE_FILE, iip, src, holder, clientMachine, false);
    ...
    if (feInfo != null) {
      FSDirEncryptionZoneOp.setFileEncryptionInfo(fsd, iip, feInfo, XAttrSetFlag.CREATE);
    }
    setNewINodeStoragePolicy(fsd.getBlockManager(), iip, isLazyPersist);
    fsd.getEditLog().logOpenFile(src, newNode, overwrite, logRetryEntry);
    if (NameNode.stateChangeLog.isDebugEnabled()) {
      NameNode.stateChangeLog.debug(
          "DIR* NameSystem.startFile: added " + src + " inode " + newNode.getId() + " " + holder);
    }
  ...
  static INodeFile addFileForEditLog(
    ...
    try {
      ...
      ErasureCodingPolicy ecPolicy = null;
      if (isStriped) {
        ecPolicy = fsd.getFSNamesystem().getErasureCodingPolicyManager().getByID(ecPolicyID);
        if (ecPolicy == null) {
          throw new IOException(
              String.format(
                  "Cannot find erasure coding policy for new file %s/%s, " + "ecPolicyID=%d",
    ...
    try {
      ...
      if (blockType == BlockType.STRIPED) {
        ...
        fsd.updateCount(
            inodesInPath,
            0,
            fileINode.getPreferredBlockSize(),
  ...
  private static INodesInPath addFile(
    ...
    try {
      ...
      if (!shouldReplicate) {
        ecPolicy =
            FSDirErasureCodingOp.getErasureCodingPolicy(
    ...
    if (newiip == null) {
      NameNode.stateChangeLog.info(
          "DIR* addFile: failed to add "
              + existing.getPath()
              + "/"
              + DFSUtil.bytes2String(localName));
      return null;
    }

    if (NameNode.stateChangeLog.isDebugEnabled()) {
      NameNode.stateChangeLog.debug(
          "DIR* addFile: " + DFSUtil.bytes2String(localName) + " is added");
    }
    return newiip;
  }
  ...
  private static FileState analyzeFileState(
    ...
    if (!Block.matchingIdAndGenStamp(previousBlock, lastBlockInFile)) {
      ...
      if (previous == null
        ...
        NameNode.stateChangeLog.info(
            "BLOCK* allocateBlock: caught retry for "
                + "allocation of a new block in "
                + src
        ...
        onRetryBlock[0] =
            makeLocatedBlock(fsn, lastBlockInFile, uc.getExpectedStorageLocations(), offset);
        return new FileState(file, src, iip);
      } else {
        // Case 3
        throw new IOException(
            "Cannot allocate block in "
                + src
                + ": "
                + "passed 'previous' block "
                + previous
  ...
  static boolean completeFile(
    ...
    checkBlock(fsn, last);
    INodesInPath iip = fsn.dir.resolvePath(pc, src, fileId);
    return completeFileInternal(fsn, iip, holder, ExtendedBlock.getLocalBlock(last), fileId);
  }

  private static boolean completeFileInternal(
      FSNamesystem fsn, INodesInPath iip, String holder, Block last, long fileId)
      throws IOException {
    assert fsn.hasWriteLock();
    final String src = iip.getPath();
        ...
        if (Block.matchingIdAndGenStamp(last, realLastBlock)) {
          NameNode.stateChangeLog.info(
              "DIR* completeFile: "
                  ...
                  + "("
                  + src
                  + ") which is already closed. But, it appears to be "
    ...
    if (NameNode.stateChangeLog.isDebugEnabled()) {
      NameNode.stateChangeLog.debug(
          "persistNewBlock: "
              + path
              + " with new block "
              + file.getLastBlock().toString()
              + ", current total block count is "
              + file.getBlocks().length);
    }
  }
   ...
   * @param inodesInPath representing each of the components of src. The last INode is the INode for
   *     {@code src} file.
  ...
  private static void setNewINodeStoragePolicy(
      BlockManager bm, INodesInPath iip, boolean isLazyPersist) throws IOException {
    ...
    ValidateAddBlockResult(
      ...
      if (blockType == BlockType.STRIPED) {
        Preconditions.checkArgument(
            ecPolicy != null, "ecPolicy is not specified for striped block");
      }
    }
  }
}
