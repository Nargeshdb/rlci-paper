...
/**
 * This class detects and maintains DataNode disk outliers and their latencies for different ops
 * (metadata, read, write).
 */
...
public class DataNodeDiskMetrics {
  ...
  private volatile Map<String, Map<DiskOp, Double>> diskOutliersStats = Maps.newHashMap();
  ...
  public DataNodeDiskMetrics(DataNode dn, long diskOutlierDetectionIntervalMs) {
    ...
    slowDiskDetector = new OutlierDetector(MIN_OUTLIER_DETECTION_DISKS, SLOW_DISK_LOW_THRESHOLD_MS);
    shouldRun = true;
    startDiskOutlierDetectionThread();
  }

  private void startDiskOutlierDetectionThread() {
    slowDiskDetectionDaemon =
        new Daemon(
            new Runnable() {
              @Override
              public void run() {
  ...
  private void detectAndUpdateDiskOutliers(
      Map<String, Double> metadataOpStats,
      Map<String, Double> readIoStats,
      Map<String, Double> writeIoStats) {
    Map<String, Map<DiskOp, Double>> diskStats = Maps.newHashMap();

    // Get MetadataOp Outliers
    Map<String, Double> metadataOpOutliers = slowDiskDetector.getOutliers(metadataOpStats);
    ...
    // Get ReadIo Outliers
    Map<String, Double> readIoOutliers = slowDiskDetector.getOutliers(readIoStats);
    ...
    Map<String, Double> writeIoOutliers = slowDiskDetector.getOutliers(writeIoStats);
  ...
  private void addDiskStat(
      Map<String, Map<DiskOp, Double>> diskStats, String disk, DiskOp diskOp, double latency) {
  ...
  @VisibleForTesting
  public void addSlowDiskForTesting(String slowDiskPath, Map<DiskOp, Double> latencies) {
