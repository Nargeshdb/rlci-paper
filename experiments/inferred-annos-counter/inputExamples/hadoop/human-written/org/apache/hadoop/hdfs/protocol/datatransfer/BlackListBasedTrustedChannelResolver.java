...
/** Implements {@link TrustedChannelResolver} to trust ips/host/subnets based on a blackList. */
public class BlackListBasedTrustedChannelResolver extends TrustedChannelResolver {
  ...
  /**
   * Path to the file containing subnets and ip addresses to form fixed BlackList. Server side
   * config.
   */
  ...
  /**
   * Path to the file containing subnets and ip addresses to form variable BlackList. Server side
   * config.
   */
  ...
  /**
   * Time in seconds after which the variable BlackList file is checked for updates. Server side
   * config.
   */
  ...
  /**
   * Path to the file containing subnets and ip addresses to form fixed BlackList. This key is for
   * client.
   */
  ...
  /**
   * Path to the file to containing subnets and ip addresses to form variable BlackList. This key is
   * for client.
   */
  ...
  /**
   * Time in seconds after which the variable BlackList file is checked for updates. This key is for
   * client.
   */
  ...
  public void setConf(Configuration conf) {
    super.setConf(conf);
    String fixedFile =
        conf.get(DFS_DATATRANSFER_SERVER_FIXED_BLACK_LIST_FILE, FIXED_BLACK_LIST_DEFAULT_LOCATION);
    ...
    if (conf.getBoolean(DFS_DATATRANSFER_SERVER_VARIABLE_BLACK_LIST_ENABLE, false)) {
      ...
      expiryTime =
          conf.getLong(DFS_DATATRANSFER_SERVER_VARIABLE_BLACK_LIST_CACHE_SECS, 3600) * 1000;
    }
    ...
    if (conf.getBoolean(DFS_DATATRANSFER_CLIENT_VARIABLE_BLACK_LIST_ENABLE, false)) {
      ...
      expiryTime =
          conf.getLong(DFS_DATATRANSFER_CLIENT_VARIABLE_BLACK_LIST_CACHE_SECS, 3600) * 1000;
    }
  ...
  public boolean isTrusted() {
    try {
      return !blackListForClient.isIn(InetAddress.getLocalHost().getHostAddress());
