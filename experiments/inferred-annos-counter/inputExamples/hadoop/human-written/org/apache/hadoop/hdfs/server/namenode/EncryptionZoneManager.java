 ...
 *
 * <p>The EncryptionZoneManager has its own lock, but relies on the FSDirectory lock being held for
 * many operations. The FSDirectory lock should not be taken if the manager lock is already held.
 */
public class EncryptionZoneManager {
  ...
  /**
   * EncryptionZoneInt is the internal representation of an encryption zone. The external
   * representation of an EZ is embodied in an EncryptionZone and contains the EZ's pathname.
   */
  private static class EncryptionZoneInt {
    ...
    EncryptionZoneInt(
        long inodeId, CipherSuite suite, CryptoProtocolVersion version, String keyName) {
  ...
  @VisibleForTesting
  public void pauseForTestingAfterNthCheckpoint(final String zone, final int count)
      throws IOException {
    INodesInPath iip;
    ...
    reencryptionHandler.pauseForTestingAfterNthCheckpoint(iip.getLastINode().getId(), count);
  }
  ...
  @VisibleForTesting
  public ZoneReencryptionStatus getZoneStatus(final String zone) throws IOException {
    final FSPermissionChecker pc = dir.getPermissionChecker();
    final INode inode;
    dir.getFSNamesystem().readLock();
    dir.readLock();
  ...
  public EncryptionZoneManager(FSDirectory dir, Configuration conf) {
    this.dir = dir;
    maxListEncryptionZonesResponses =
        conf.getInt(
    ...
    Preconditions.checkArgument(
        maxListEncryptionZonesResponses >= 0,
        DFSConfigKeys.DFS_NAMENODE_LIST_ENCRYPTION_ZONES_NUM_RESPONSES
            + " "
            + "must be a positive integer.");
    if (getProvider() != null) {
      reencryptionHandler = new ReencryptionHandler(this, conf);
      reencryptionThreadFactory =
          new ThreadFactoryBuilder()
              .setDaemon(true)
              .setNameFormat("reencryptionHandlerThread #%d")
              .build();
    }
    ...
    Preconditions.checkArgument(
        maxListRecncryptionStatusResponses >= 0,
        DFS_NAMENODE_LIST_REENCRYPTION_STATUS_NUM_RESPONSES_KEY + " must be a positive integer.");
  ...
  void startReencryptThreads() {
    ...
    reencryptHandlerExecutor = Executors.newSingleThreadExecutor(reencryptionThreadFactory);
   ...
   * Add a new encryption zone.
   *
   * <p>Does not assume that the FSDirectory lock is held.
   *
   * @param inodeId of the encryption zone
   * @param keyName encryption zone key name
   */
  void unprotectedAddEncryptionZone(
      Long inodeId, CipherSuite suite, CryptoProtocolVersion version, String keyName) {
  ...
  /**
   * Get the key name for an encryption zone. Returns null if <tt>iip</tt> is not within an
   * encryption zone.
   *
   * <p>Called while holding the FSDirectory lock.
   */
  ...
  /**
   * Looks up the EncryptionZoneInt for a path within an encryption zone. Returns null if path is
   * not within an EZ.
   *
   * <p>Called while holding the FSDirectory lock.
   */
  ...
  /**
   * Looks up the nearest ancestor EncryptionZoneInt that contains the given path (excluding
   * itself). Returns null if path is not within an EZ, or the path is the root dir '/'
   *
   * <p>Called while holding the FSDirectory lock.
   */
  ...
  /**
   * Returns an EncryptionZone representing the ez for a given path. Returns an empty marker
   * EncryptionZone if path is not in an ez.
   *
  ...
  /**
   * Throws an exception if the provided path cannot be renamed into the destination because of
   * differing parent encryption zones.
   *
   * <p>Called while holding the FSDirectory lock.
   *
  ...
  void checkMoveValidity(INodesInPath srcIIP, INodesInPath dstIIP) throws IOException {
    ...
    final EncryptionZoneInt dstParentEZI = getParentEncryptionZoneForPath(dstIIP);
    ...
    if (srcInEZ) {
      if (!srcParentEZI.equals(dstParentEZI)) {
        ...
        sb.append(" can't be moved from encryption zone ")
            .append(srcEZPath)
            .append(" to encryption zone ")
            .append(dstEZPath)
            .append(".");
        throw new IOException(sb.toString());
      }
      checkMoveValidityForReencryption(srcIIP.getPath(), srcParentEZI.getINodeId());
    } else if (dstInEZ) {
      checkMoveValidityForReencryption(dstIIP.getPath(), dstParentEZI.getINodeId());
    }
  }

  private void checkMoveValidityForReencryption(final String pathName, final long zoneId)
      throws IOException {
    assert dir.hasReadLock();
   ...
   * <p>Called while holding the FSDirectory lock.
   */
  XAttr createEncryptionZone(
      INodesInPath srcIIP, CipherSuite suite, CryptoProtocolVersion version, String keyName)
      throws IOException {
    assert dir.hasWriteLock();
  ...
  BatchedListEntries<EncryptionZone> listEncryptionZones(long prevId) throws IOException {
    ...
    for (EncryptionZoneInt ezi : tailMap.values()) {
      ...
      */
      ...
      zones.add(
          new EncryptionZone(
              ezi.getINodeId(), pathName, ezi.getSuite(), ezi.getVersion(), ezi.getKeyName()));
      count++;
  ...
  /**
   * Resolves the path to inode id, then check if it's the same as the inode id passed in. This is
   * necessary to filter out zones in snapshots.
   *
   * @param zoneId
   * @param zonePath
  ...
  /**
   * Re-encrypts the given encryption zone path. If the given path is not the root of an encryption
   * zone, an exception is thrown.
   *
   * @param zoneIIP
   * @param keyVersionName
   * @throws IOException
   */
  List<XAttr> reencryptEncryptionZone(final INodesInPath zoneIIP, final String keyVersionName)
      throws IOException {
    assert dir.hasWriteLock();
    if (reencryptionHandler == null) {
    ...
    LOG.info("Zone {}({}) is submitted for re-encryption.", zoneName, inode.getId());
    final XAttr xattr =
        FSDirEncryptionZoneOp.updateReencryptionSubmitted(dir, zoneIIP, keyVersionName);
    xAttrs.add(xattr);
  ...
  /**
   * Cancels the currently-running re-encryption of the given encryption zone. If the given path is
   * not the root of an encryption zone, an exception is thrown.
   *
   * @param zoneIIP
   * @throws IOException
   */
  List<XAttr> cancelReencryptEncryptionZone(final INodesInPath zoneIIP) throws IOException {
    assert dir.hasWriteLock();
    if (reencryptionHandler == null) {
      throw new IOException("No key provider configured, re-encryption " + "operation is rejected");
    }
    ...
    return FSDirEncryptionZoneOp.updateReencryptionFinish(
        dir, zoneIIP, reencryptionStatus.getZoneStatus(zoneId));
  }
  ...
  BatchedListEntries<ZoneReencryptionStatus> listReencryptionStatus(final long prevId)
    ...
    NavigableMap<Long, ZoneReencryptionStatus> stats = reencryptionStatus.getZoneStatuses();

    if (stats.isEmpty()) {
      return EMPTY_LIST;
    }

    NavigableMap<Long, ZoneReencryptionStatus> tailMap = stats.tailMap(prevId, false);
    final int numResp = Math.min(maxListRecncryptionStatusResponses, tailMap.size());
    final List<ZoneReencryptionStatus> ret = Lists.newArrayListWithExpectedSize(numResp);
    int count = 0;
   ...
   */
  boolean isEncryptionZoneRoot(final INode inode, final String name) throws FileNotFoundException {
    assert dir.hasReadLock();
    if (inode == null) {
    ...
    if (!hasCreatedEncryptionZone() || !encryptionZones.containsKey(inode.getId())) {
      return false;
    }
    return true;
  }
   ...
   * @param name
   * @throws IOException if the inode is not a directory, or is a directory but not the root of an
   *     EZ.
   */
  void checkEncryptionZoneRoot(final INode inode, final String name) throws IOException {
  ...
  public int getNumEncryptionZones() {
    return hasCreatedEncryptionZone() ? encryptionZones.size() : 0;
  }

  /**
   * @return Whether there has been any attempt to create an encryption zone in the cluster at all.
   *     If not, it is safe to quickly return null when checking the encryption information of any
   *     file or directory in the cluster.
   */
