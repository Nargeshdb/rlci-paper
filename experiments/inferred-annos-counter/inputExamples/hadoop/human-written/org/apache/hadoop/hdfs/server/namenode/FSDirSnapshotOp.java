...
class FSDirSnapshotOp {
  ...
  static void verifySnapshotName(FSDirectory fsd, String snapshotName, String path)
  ...
  static void allowSnapshot(FSDirectory fsd, SnapshotManager snapshotManager, String path)
      throws IOException {
    fsd.writeLock();
    try {
  ...
  static void disallowSnapshot(FSDirectory fsd, SnapshotManager snapshotManager, String path)
      throws IOException {
    fsd.writeLock();
    try {
    ...
    try {
      snapshotPath =
          snapshotManager.createSnapshot(
              fsd.getFSNamesystem().getLeaseManager(), iip, snapshotRoot, snapshotName, now);
    } finally {
      fsd.writeUnlock();
    }
    fsd.getEditLog().logCreateSnapshot(snapshotRoot, snapshotName, logRetryCache, now);

    return snapshotPath;
  }

  static void renameSnapshot(
      FSDirectory fsd,
      FSPermissionChecker pc,
      SnapshotManager snapshotManager,
      String path,
    ...
    fsd.writeLock();
    try {
      snapshotManager.renameSnapshot(iip, path, snapshotOldName, snapshotNewName, now);
    } finally {
      fsd.writeUnlock();
    }
    fsd.getEditLog().logRenameSnapshot(path, snapshotOldName, snapshotNewName, logRetryCache, now);
  }

  static SnapshottableDirectoryStatus[] getSnapshottableDirListing(
      FSDirectory fsd, FSPermissionChecker pc, SnapshotManager snapshotManager) throws IOException {
    fsd.readLock();
    try {
    ...
    try {
      ...
      diffs =
          snapshotManager.diff(
              iip, path, fromSnapshot, toSnapshot, startPath, index, snapshotDiffReportLimit);
    } catch (Exception e) {
      throw e;
    } finally {
      fsd.readUnlock();
   ...
   */
  static Collection<String> getSnapshotFiles(
      FSDirectory fsd, List<DirectorySnapshottableFeature> lsf, String file) throws IOException {
    ...
    for (DirectorySnapshottableFeature sf : lsf) {
      ...
      for (Snapshot s : lsnap) {
        ...
        HdfsFileStatus stat = fsd.getFSNamesystem().getFileInfo(snapname, true, false, false);
        if (stat != null) {
          snaps.add(snapname);
        }
      }
  ...
  static INode.BlocksMapUpdateInfo deleteSnapshot(
    ...
    removedINodes.clear();
    fsd.getEditLog().logDeleteSnapshot(snapshotRoot, snapshotName, logRetryCache, now);

    return collectedBlocks;
  }

  private static void checkSubtreeReadPermission(
      FSDirectory fsd, final FSPermissionChecker pc, String snapshottablePath, String snapshot)
      throws IOException {
    final String fromPath =
        snapshot == null
            ? snapshottablePath
            : Snapshot.getSnapshotPath(snapshottablePath, snapshot);
  ...
  /**
   * Check if the given INode (or one of its descendants) is snapshottable and already has
   * snapshots.
   *
   * @param target The given INode
   * @param snapshottableDirs The list of directories that are snapshottable but do not have
   *     snapshots yet
   */
  private static void checkSnapshot(INode target, List<INodeDirectory> snapshottableDirs)
  ...
  static void checkSnapshot(
      FSDirectory fsd, INodesInPath iip, List<INodeDirectory> snapshottableDirs)
      throws SnapshotException {
