...
public class ExecuteCommand extends Command {
  ...
  public ExecuteCommand(Configuration conf) {
    ...
    addValidCommandParameters(
        DiskBalancerCLI.SKIPDATECHECK, "skips the date check and force execute the plan");
  }
  ...
  public void execute(CommandLine cmd) throws Exception {
    ...
    Preconditions.checkArgument(
        planFile != null && !planFile.isEmpty(), "Invalid plan file specified.");
    ...
    if (cmd.hasOption(DiskBalancerCLI.SKIPDATECHECK)) {
      skipDateCheck = true;
      LOG.warn(
          "Skipping date check on this plan. This could mean we are "
   ...
   * @throws IOException
   */
  private void submitPlan(final String planFile, final String planData, boolean skipDateCheck)
      throws IOException {
    ...
    try {
      dataNode.submitDiskBalancerPlan(
          planHash, DiskBalancerCLI.PLAN_VERSION, planFile, planData, skipDateCheck);
    } catch (DiskBalancerException ex) {
      LOG.error(
          "Submitting plan on  {} failed. Result: {}, Message: {}",
          plan.getNodeName(),
          ex.getResult().toString(),
          ex.getMessage());
      throw ex;
    }
  }
  ...
  public void printHelp() {
    String header =
        "Execute command runs a submits a plan for execution on " + "the given data node.\n\n";
