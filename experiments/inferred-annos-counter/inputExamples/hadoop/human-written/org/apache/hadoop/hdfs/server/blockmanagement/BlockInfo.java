...
/**
 * For a given block (or an erasure coding block group), BlockInfo class maintains 1) the {@link
 * BlockCollection} it is part of, and 2) datanodes where the replicas of the block, or blocks
 * belonging to the erasure coding block group, are stored.
 */
...
public abstract class BlockInfo extends Block implements LightWeightGSet.LinkedElement {
   ...
   *
   * @param size the block's replication factor, or the total number of blocks in the block group
   */
  ...
  /**
   * Count the number of data-nodes the block currently belongs to (i.e., NN has received block
   * reports from the DN).
   */
   ...
   * @param reportedBlock The block reported from the datanode. This is only used by erasure coded
   *     blocks, this block's id contains information indicating the index of the block in the
  ...
  /**
   * Commit block's length and generation stamp as reported by the client. Set block state to {@link
