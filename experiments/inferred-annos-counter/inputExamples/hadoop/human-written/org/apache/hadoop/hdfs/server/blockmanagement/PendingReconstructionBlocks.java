...
class PendingReconstructionBlocks {
  ...
  private static final long DEFAULT_RECHECK_INTERVAL = 5 * 60 * 1000;
  ...
  /**
   * One reconstruction request for this block has finished. Decrement the number of pending
   * reconstruction requests for this block.
   *
   ...
   *
   * @param block The given block whose pending reconstruction requests need to be removed
   */
  ...
  /**
   * Returns a list of blocks that have timed out their reconstruction requests. Returns null if no
  ...
  BlockInfo[] getTimedOutBlocks() {
    synchronized (timedOutItems) {
      ...
      BlockInfo[] blockList = timedOutItems.toArray(new BlockInfo[size]);
      timedOutItems.clear();
      timedOutCount += size;
      return blockList;
    }
  ...
  /**
   * An object that contains information about a block that is being reconstructed. It records the
   * timestamp when the system started reconstructing the most recent copy of this block. It also
   * records the list of Datanodes where the reconstruction requests are in progress.
   */
