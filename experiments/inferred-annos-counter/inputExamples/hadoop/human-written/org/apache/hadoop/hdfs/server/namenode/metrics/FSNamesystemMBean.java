...
/**
 * This Interface defines the methods to get the status of a the FSNamesystem of a name node. It is
 * also used for publishing via JMX (hence we follow the JMX naming convention.)
 *
 * <p>Note we have not used the MetricsDynamicMBeanBase to implement this because the interface for
 * the NameNodeStateMBean is stable and should be published as an interface.
 *
 * <p>Name Node runtime activity statistic info is reported in
 *
...
public interface FSNamesystemMBean {
  ...
  /**
   * Returns an estimate of total capacity lost due to volume failures in bytes across all live data
   * nodes.
   *
  ...
  /**
   * Returns a nested JSON object listing the top users for different RPC operations over tracked
   * time windows.
   *
   * @return JSON string
   */
