...
/**
 * A class that receives a block and writes to its own disk, meanwhile may copies it to another
 * site. If a throttler is provided, streaming throttling is also supported.
 */
class BlockReceiver implements Closeable {
  public static final Logger LOG = DataNode.LOG;
  static final Log ClientTraceLog = DataNode.ClientTraceLog;

  @VisibleForTesting static long CACHE_DROP_LAG_BYTES = 8 * 1024 * 1024;
  ...
  /**
   * In the case that the client is writing with a different checksum polynomial than the block is
   * stored with on disk, the DataNode needs to recalculate checksums before writing.
   */
  ...
  BlockReceiver(
    ...
    try {
      ...
      this.datanodeSlowLogThresholdMs = datanode.getDnConf().getSlowIoWarningThresholdMs();
      ...
      if (isDatanode) { // replication or move
        ...
        switch (stage) {
          ...
          case TRANSFER_FINALIZED:
            ...
            replicaHandler =
                datanode.data.createTemporary(storageType, storageId, block, isTransfer);
            break;
          default:
      ...
      this.dropCacheBehindWrites =
          (cachingStrategy.getDropBehind() == null)
      ...
      final boolean isCreate =
          isDatanode || isTransfer || stage == BlockConstructionStage.PIPELINE_SETUP_CREATE;
      ...
      this.checksumOut =
          new DataOutputStream(
              new BufferedOutputStream(
                  streams.getChecksumOut(), DFSUtilClient.getSmallBufferSize(datanode.getConf())));
      ...
      DataNode.LOG.warn(
          "IOException in BlockReceiver constructor :"
              + ioe.getMessage()
              + (cause == null ? "" : ". Cause is "),
          cause);
      if (cause != null) {
  ...
  public void close() throws IOException {
    ...
    try {
      if (checksumOut != null) {
        ...
        checksumOut.close();
        //        checksumOut = null; //
        // https://stackoverflow.com/questions/2495223/java-making-reference-null-after-closing-stream
      }
    } catch (IOException e) {
      ioe = e;
    } finally {
   ...
   *
   * <p>Some example of when this method may return false:
   *
   * <ul>
   *   <li>Upstream did not send packet for a long time
   *   <li>Packet was received but got stuck in local disk I/O
   *   <li>Packet was received but got stuck on send to mirror
   * </ul>
   *
   * @return true if packet was sent within an acceptable period of time; otherwise false.
   */
  boolean packetSentInTime() {
    ...
    if (!allowedIdleTime) {
      LOG.warn(
          "A packet was last sent {}ms ago. Maximum idle time: {}ms.", diff, this.maxSendIdleTime);
    }
  ...
  /**
   * While writing to mirrorOut, failure to write to mirror should not affect this datanode unless
   * it is caused by interruption.
   */
    ...
    try {
      ...
      if (srcDataNode != null && isDatanode) {
        try {
          ...
          LOG.warn(
              "Failed to report bad " + block + " from datanode " + srcDataNode + " to namenode");
        }
      }
      throw new IOException(
  ...
  /**
   * Translate CRC chunks from the client's checksum implementation to the disk checksum
   * implementation.
   *
   * <p>This does not verify the original checksums, under the assumption that they have already
   * been validated.
   */
  ...
  /**
   * Check whether checksum needs to be verified. Skip verifying checksum iff this is not the last
   * one in the pipeline and clientName is non-null. i.e. Checksum is verified on all the datanodes
   * when the data is being written by a datanode rather than a client. Whe client is writing the
   * data, protocol includes acks and only the last datanode needs to verify checksum.
   *
  ...
  /**
   * Receives and processes a packet. It can contain many chunks. returns the number of data bytes
   * that the packet has.
   */
  private int receivePacket() throws IOException {
    ...
    if (header.getOffsetInBlock() > replicaInfo.getNumBytes()) {
      throw new IOException(
          "Received an out-of-sequence packet for "
              + block
              + "from "
              + inAddr
              + " at offset "
              + header.getOffsetInBlock()
              + ". Expecting packet starting at "
              + replicaInfo.getNumBytes());
    }
    if (header.getDataLen() < 0) {
      throw new IOException(
          "Got wrong length during writeBlock("
              + block
              + ") from "
              + inAddr
              + " at offset "
    ...
    if (lastPacketInBlock || len == 0) {
      ...
      if (checksumReceivedLen > 0 && checksumReceivedLen != checksumLen) {
        throw new IOException(
            "Invalid checksum length: received length is "
                + checksumReceivedLen
                + " but expected length is "
                + checksumLen);
      }
      ...
      if (checksumReceivedLen > 0 && shouldVerifyChecksum()) {
        try {
          ...
          if (responder != null) {
            try {
              ((PacketResponder) responder.getRunnable())
                  .enqueue(seqno, lastPacketInBlock, offsetInBlock, Status.ERROR_CHECKSUM);
      ...
      final boolean shouldNotWriteChecksum =
          checksumReceivedLen == 0 && streams.isTransientStorage();
      try {
        ...
        if (onDiskLen < offsetInBlock) {
          ...
          if (!alignedInPacket && len > bytesPerChecksum) {
            throw new IOException(
                "Unexpected packet data length for "
                    + block
                    + " from "
                    + inAddr
                    + ": a partial chunk must be "
                    + " sent in an individual packet (data length = "
                    + len
                    + " > bytesPerChecksum = "
                    + bytesPerChecksum
                    + ")");
          ...
          if (doCrcRecalc) {
            ...
            long offsetInChecksum =
                BlockMetadataHeader.getHeaderSize() + onDiskLen / bytesPerChecksum * checksumSize;
          ...
          int startByteToDisk =
              (int) (onDiskLen - firstByteInBlock) + dataBuf.arrayOffset() + dataBuf.position();
          ...
          if (shouldNotWriteChecksum) {
            ...
            if (skippedDataBytes > 0) {
              skip +=
                  (int) (skippedDataBytes / bytesPerChecksum)
    ...
    if (isReplaceBlock && (Time.monotonicNow() - lastResponseTime > responseInterval)) {
      BlockOpResponseProto.Builder response =
          BlockOpResponseProto.newBuilder().setStatus(Status.IN_PROGRESS);
  ...
  /**
   * Only tracks the latency of sending packet to the last node in pipeline. This is a conscious
   * design choice.
   *
   * <p>In the case of pipeline [dn0, dn1, dn2], 5ms latency from dn0 to dn1, 100ms from dn1 to dn2,
   * NameNode claims dn2 is slow since it sees 100ms latency to dn2. Note that NameNode is not ware
   * of pipeline structure in this context and only sees latency between two DataNodes.
   *
   * <p>In another case of the same pipeline, 100ms latency from dn0 to dn1, 5ms from dn1 to dn2,
   * NameNode will miss detecting dn1 being slow since it's not the last node. However the
   * assumption is that in a busy enough cluster there are many other pipelines where dn1 is the
   * last node, e.g. [dn3, dn4, dn1]. Also our tracking interval is relatively long enough (at least
   * an hour) to improve the chances of the bad DataNodes being the last nodes in multiple
   * pipelines.
   */
      ...
      if (streams.getOutFd() != null
        ...
        if (syncBehindWrites) {
          if (syncBehindWritesInBackground) {
            ...
            streams.syncFileRangeIfPossible(
                lastCacheManagementOffset,
                offsetInBlock - lastCacheManagementOffset,
                SYNC_FILE_RANGE_WRITE);
          }
        ...
        if (dropPos > 0 && dropCacheBehindWrites) {
          streams.dropCacheBehindWrites(block.getBlockName(), 0, dropPos, POSIX_FADV_DONTNEED);
        }
  ...
  void receiveBlock(
    ...
    try {
      if (isClient && !isTransfer) {
        responder =
            new Daemon(datanode.threadGroup, new PacketResponder(replyOut, mirrIn, downstreams));
      ...
      if (!responderClosed) { // Data transfer was not complete.
        if (responder != null) {
          ...
          if (datanode.isRestarting() && isClient && !isTransfer) {
            try (Writer out =
                new OutputStreamWriter(replicaInfo.createRestartMetaStream(), "UTF-8")) {
  ...
  /**
   * If we have downstream DNs and peerMetrics are enabled, then initialize some state for
   * monitoring the performance of downstream DNs.
   *
    ...
    if (downstreams != null && downstreams.length > 0) {
      ...
      if (isPenultimateNode && datanode.getPeerMetrics() != null) {
        mirrorNameForMetrics =
            (downstreams[0].getInfoSecurePort() != 0
                ? downstreams[0].getInfoSecureAddr()
                : downstreams[0].getInfoAddr());
   ...
   *
   * @returns Volume base URI as string if available. Else returns the the string "unavailable".
   */
  ...
  /**
   * Processes responses from downstream datanodes in the pipeline and sends back replies to the
   * originator.
   */
  class PacketResponder implements Runnable, Closeable {
    ...
    PacketResponder(
      ...
      this.type =
          downstreams == null
              ? PacketResponderType.NON_PIPELINE
              : downstreams.length == 0
                  ? PacketResponderType.LAST_IN_PIPELINE
      ...
      if (type == PacketResponderType.HAS_DOWNSTREAM_IN_PIPELINE) {
        b.append(", downstreams=")
            .append(downstreams.length)
            .append(":")
    ...
    void enqueue(
      ...
      final Packet p =
          new Packet(seqno, lastPacketInBlock, offsetInBlock, System.nanoTime(), ackStatus);
    ...
    /**
     * Send an OOB response. If all acks have been sent already for the block and the responder is
     * about to close, the delivery is not guaranteed. This is because the other end can close the
     * connection independently. An OOB coming from downstream will be automatically relayed
     * upstream by the responder. This method is used only by originating datanode.
     *
    ...
    void sendOOBResponse(final Status ackStatus) throws IOException, InterruptedException {
      ...
      try {
        sendAckUpstreamUnprotected(
            null,
            PipelineAck.UNKOWN_SEQNO,
            0L,
            0L,
          ...
          try {
            ...
            if (seqno != PipelineAck.UNKOWN_SEQNO || type == PacketResponderType.LAST_IN_PIPELINE) {
              ...
              if (type == PacketResponderType.HAS_DOWNSTREAM_IN_PIPELINE && seqno != expected) {
                throw new IOException(
                    myString + "seqno: expected=" + expected + ", received=" + seqno);
              }
              if (type == PacketResponderType.HAS_DOWNSTREAM_IN_PIPELINE) {
            ...
            if (Thread.interrupted()) {
              ...
              LOG.warn(
                  "The downstream error might be due to congestion in "
    ...
    private void finalizeBlock(long startTime) throws IOException {
      ...
      datanode.closeBlock(
          block, null, replicaInfo.getStorageUuid(), replicaInfo.isOnTransientStorage());
     ...
     */
    private void sendAckUpstream(
        PipelineAck ack, long seqno, long totalAckTimeNanos, long offsetInBlock, int myHeader)
        throws IOException {
      try {
     ...
     * @param myHeader the local ack header
     */
    private void sendAckUpstreamUnprotected(
        PipelineAck ack, long seqno, long totalAckTimeNanos, long offsetInBlock, int myHeader)
        throws IOException {
      final int[] replies;
      if (ack == null) {
      ...
      upstreamOut.flush();
      long duration = Time.monotonicNow() - begin;
      DataNodeFaultInjector.get().logDelaySendingAckToUpstream(inAddr, duration);
     ...
     *
     * <p>This should be called only when the ack queue is not empty
     */
