...
public class WebHdfsHandler extends SimpleChannelInboundHandler<HttpRequest> {
        ...
        new PrivilegedExceptionAction<Void>() {
          ...
          public Void run() throws Exception {
            try {
              ...
              REQLOG.info(
                  host + " " + req.getMethod() + " " + req.getUri() + " " + getResponseCode());
            }
            return null;
          }
        });
  ...
  int getResponseCode() {
    return (resp == null) ? INTERNAL_SERVER_ERROR.code() : resp.getStatus().code();
  }
  ...
  @SuppressWarnings(
      "builder:required.method.not.called") // TP: out remains open in possible exceptional exit due
                                            // to new URI(HDFS_URI_SCHEME, nnId, path, null, null)
                                            // (validated)
