...
/**
 * Class that helps in checking file system permission. The state of this class need not be
 * synchronized as it has data structures that are read-only.
 *
 * <p>Some of the helper methods are guarded by {@link FSNamesystem#readLock()}.
 */
public class FSPermissionChecker implements AccessControlEnforcer {
  ...
  private String toAccessControlString(
      INodeAttributes inodeAttrib, String path, FsAction access, boolean deniedFromAcl) {
    StringBuilder sb =
        new StringBuilder("Permission denied: ")
            .append("user=")
            .append(getUser())
            .append(", ")
            .append("access=")
            ...
            .append(path)
            .append("\":")
            .append(inodeAttrib.getUserName())
            .append(':')
            .append(inodeAttrib.getGroupName())
            .append(':')
  ...
  private AccessControlEnforcer getAccessControlEnforcer() {
    return (attributeProvider != null)
        ? attributeProvider.getExternalAccessControlEnforcer(this)
        : this;
  }

  /**
   * Verify if the caller has the required permission. This will result into an exception if the
   * caller is not allowed to access the resource.
   */
  public void checkSuperuserPrivilege() throws AccessControlException {
    if (!isSuperUser()) {
      throw new AccessControlException(
          "Access denied for user " + getUser() + ". Superuser privilege is required");
    }
  }
   ...
   *
   * <p>Parent path means the parent directory for the path. Ancestor path means the last (the
   * closest) existing ancestor directory of the path. Note that if the parent path exists, then the
   * parent path and the ancestor path are the same.
   *
   * <p>For example, suppose the path is "/foo/bar/baz". No matter baz is a file or a directory, the
   * parent path is "/foo/bar". If bar exists, then the ancestor path is also "/foo/bar". If bar
   * does not exist and foo exists, then the ancestor path is "/foo". Further, if both foo and bar
   * do not exist, then the ancestor path is "/".
   *
   * @param doCheckOwner Require user to be the owner of the path?
   * @param ancestorAccess The access required by the ancestor of the path.
   * @param parentAccess The access required by the parent of the path.
   * @param access The access required by the path.
   * @param subAccess If path is a directory, it is the access required of the path and all the
   *     sub-directories. If path is not a directory, there is no effect.
   * @param ignoreEmptyDir Ignore permission checking for empty directory?
   * @throws AccessControlException
   *     <p>Guarded by {@link FSNamesystem#readLock()} Caller of this method must hold that lock.
   */
  ...
  /**
   * Check permission only for the given inode (not checking the children's access).
   *
  ...
  void checkPermission(INode inode, int snapshotId, FsAction access) throws AccessControlException {
    ...
    INodeAttributes nodeAttributes =
        getINodeAttrs(pathComponents, pathComponents.length - 1, inode, snapshotId);
    try {
      ...
      if (this.authorizeWithContext && opType != null) {
        ...
        enforcer.checkPermission(
            ...
            snapshotId,
            null,
            -1, // this will skip checkTraverse() because
      ...
      throw new AccessControlException(
          toAccessControlString(nodeAttributes, inode.getFullPathName(), access));
    }
  }
  ...
  public void checkPermission(
    ...
    if (subAccess != null) {
      ...
      checkSubAccess(
          components, inodeAttrs.length - 1, rawLast, snapshotId, subAccess, ignoreEmptyDir);
    }
    if (doCheckOwner) {
  ...
  public void checkPermissionWithContext(INodeAttributeProvider.AuthorizationContext authzContext)
    ...
    this.checkPermission(
        authzContext.getFsOwner(),
        authzContext.getSupergroup(),
        authzContext.getCallerUgi(),
        authzContext.getInodeAttrs(),
        authzContext.getInodes(),
        authzContext.getPathByNameArr(),
        authzContext.getSnapshotId(),
        authzContext.getPath(),
        authzContext.getAncestorIndex(),
        authzContext.isDoCheckOwner(),
        authzContext.getAncestorAccess(),
        authzContext.getParentAccess(),
        authzContext.getAccess(),
        authzContext.getSubAccess(),
        authzContext.isIgnoreEmptyDir());
  }

  private INodeAttributes getINodeAttrs(
      byte[][] pathByNameArr, int pathIdx, INode inode, int snapshotId) {
    INodeAttributes inodeAttrs = inode.getSnapshotINode(snapshotId);
    if (getAttributesProvider() != null) {
      String[] elements = new String[pathIdx + 1];
      /**
       * {@link INode#getPathComponents(String)} returns a null component for the root only path
       * "/". Assign an empty string if so.
       */
  ...
  private void checkTraverse(
      INodeAttributes[] inodeAttrs, INode[] inodes, byte[][] components, int last)
      ...
      if (!(cList.isEmpty() && ignoreEmptyDir)) {
        ...
        if (inodeAttr.getFsPermission().getStickyBit()) {
          for (INode child : cList) {
            INodeAttributes childInodeAttr = getINodeAttrs(components, pathIdx, child, snapshotId);
            if (isStickyBitViolated(inodeAttr, childInodeAttr)) {
              ...
              byte[][] allComponents = allComponentList.toArray(new byte[][] {});
  ...
  /** Guarded by {@link FSNamesystem#readLock()} */
  private void check(INodeAttributes[] inodes, byte[][] components, int i, FsAction access)
      throws AccessControlException {
  ...
  /**
   * Checks requested access against an Access Control List. This method relies on finding the ACL
   * data in the relevant portions of {@link FsPermission} and {@link AclFeature} as implemented in
   * the logic of {@link AclStorage}. This method also relies on receiving the ACL entries in sorted
   * order. This is assumed to be true, because the ACL modification methods in {@link
   * AclTransformation} sort the resulting entries.
   *
   * <p>More specifically, this method depends on these invariants in an ACL: - The list must be
   * sorted. - Each entry in the list must be unique by scope + type + name. - There is exactly one
   * each of the unnamed user/group/other entries. - The mask entry must not have a name. - The
   * other entry must not have a name. - Default entries may be present, but they are ignored during
   * enforcement.
   *
  ...
  private boolean hasAclPermission(
      INodeAttributes inode, FsAction access, FsPermission mode, AclFeature aclFeature) {
  ...
  /** Guarded by {@link FSNamesystem#readLock()} */
  private void checkStickyBit(INodeAttributes[] inodes, byte[][] components, int index)
      throws AccessControlException {
  ...
  private void throwStickyBitException(
      String inodePath, INodeAttributes inode, String parentPath, INodeAttributes parent)
      throws AccessControlException {
    throw new AccessControlException(
        String.format(
            ...
            inode.getGroupName(),
            inode.isDirectory() ? "d" : "-",
            inode.getFsPermission().toString(),
            parentPath,
            parent.getUserName(),
            parent.getGroupName(),
            parent.isDirectory() ? "d" : "-",
  ...
  public void checkPermission(CachePool pool, FsAction access) throws AccessControlException {
    ...
    throw new AccessControlException(
        "Permission denied while accessing pool "
            + pool.getPoolName()
            + ": user "
            + getUser()
            + " does not have "
  ...
  /**
   * Verifies that all existing ancestors are directories. If a permission checker is provided then
   * the user must have exec access. Ancestor symlinks will throw an unresolved exception, and
   * resolveLink determines if the last inode will throw an unresolved exception. This method should
   * always be called after a path is resolved into an IIP.
   *
   ...
   * @throws ParentNotDirectoryException
   */
  static void checkTraverse(FSPermissionChecker pc, INodesInPath iip, boolean resolveLink)
      throws AccessControlException, UnresolvedPathException, ParentNotDirectoryException {
    try {
  ...
  private static void checkIsDirectory(INode inode, byte[][] components, int i)
    ...
    if (inode != null && !inode.isDirectory()) {
      ...
      throw new ParentNotDirectoryException(getPath(components, 0, i) + " (is not a directory)");
    }
  }
  ...
  static class TraverseAccessControlException extends AccessControlException {
    ...
    public void throwCause()
        throws UnresolvedPathException, ParentNotDirectoryException, AccessControlException {
