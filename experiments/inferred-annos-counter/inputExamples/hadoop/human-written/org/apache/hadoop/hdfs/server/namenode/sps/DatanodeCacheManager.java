 ...
 *
 * <p>This class is instantiated by StoragePolicySatisifer. It maintains the array of datanode
 * storage reports. It has a configurable refresh interval and periodically refresh the datanode
 * cache by fetching latest {@link Context#getLiveDatanodeStorageReport()} once it reaches refresh
 * interval.
 */
...
public class DatanodeCacheManager {
  ...
  /**
   * Returns the live datanodes and its storage details, which has available space (&gt; 0) to
   * schedule block moves. This will return array of datanodes from its local cache. It has a
   * configurable refresh interval in millis and periodically refresh the datanode cache by fetching
   * latest {@link Context#getLiveDatanodeStorageReport()} once it elapsed refresh interval.
   *
   * @throws IOException
   */
  public DatanodeMap getLiveDatanodeStorageReport(Context spsContext) throws IOException {
    ...
    if (refreshNeeded) {
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            "elapsedTimeMs > refreshIntervalMs : {} > {}," + " so refreshing cache",
            elapsedTimeMs,
            refreshIntervalMs);
      }
