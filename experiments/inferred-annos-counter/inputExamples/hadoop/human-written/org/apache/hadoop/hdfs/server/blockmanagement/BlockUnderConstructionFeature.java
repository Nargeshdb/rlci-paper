...
/**
 * Represents the under construction feature of a Block. This is usually the last block of a file
...
public class BlockUnderConstructionFeature {
  ...
  /** Index of the primary data node doing the recovery. Useful for log messages. */
  private int primaryNodeIndex = -1;

  /**
   * The new generation stamp, which this block will have after the recovery succeeds. Also used as
   * a recovery id to identify the right recovery if any of the abandoned recoveries re-appear.
   */
  ...
  public BlockUnderConstructionFeature(
      Block blk, BlockUCState state, DatanodeStorageInfo[] targets, BlockType blockType) {
  ...
  public void setExpectedLocations(
      Block block, DatanodeStorageInfo[] targets, BlockType blockType) {
    if (targets == null) {
      return;
    }
    int numLocations = 0;
      ...
      if (targets[i] != null) {
        ...
        Block replicaBlock =
            blockType == BlockType.STRIPED
                ...
                : block;
        replicas[offset++] =
            new ReplicaUnderConstruction(replicaBlock, targets[i], ReplicaState.RBW);
      }
    }
  }
  ...
  /**
   * Note that this iterator doesn't guarantee thread-safe. It depends on external mechanisms such
   * as the FSNamesystem lock for protection.
   */
  ...
  /**
   * @return the index array indicating the block index in each storage. Used only by striped
   *     blocks.
   */
  ...
  /**
   * when committing a striped block whose size is less than a stripe, we need to decrease the
   * scheduled block size of the DataNodes that do not store any internal block.
   */
    ...
    if (realDataBlockNum < dataBlockNum) {
      for (ReplicaUnderConstruction replica : replicas) {
        ...
        if (index >= realDataBlockNum && index < dataBlockNum) {
          final DatanodeStorageInfo storage = replica.getExpectedStorageLocation();
  ...
  /**
   * Initialize lease recovery for this block. Find the first alive data-node starting from the
   * previous primary and make it primary.
   *
  ...
  public void initializeBlockRecovery(BlockInfo blockInfo, long recoveryId, boolean startRecovery) {
    ...
    for (ReplicaUnderConstruction replica : replicas) {
      ...
      if (replica.isAlive()) {
        allLiveReplicasTriedAsPrimary =
            allLiveReplicasTriedAsPrimary && replica.getChosenAsPrimary();
      }
    }
    ...
    for (int i = 0; i < replicas.length; i++) {
      ...
      final long lastUpdate =
          ruc.getExpectedStorageLocation().getDatanodeDescriptor().getLastUpdateMonotonic();
  ...
  void addReplicaIfNotPresent(
      DatanodeStorageInfo storage, Block reportedBlock, ReplicaState rState) {
    if (replicas.length == 0) {
      ...
      for (int i = 0; i < replicas.length; i++) {
        ...
        if (expected == storage) {
          ...
          return;
            ...
            && expected.getDatanodeDescriptor() == storage.getDatanodeDescriptor()) {
      ...
      newReplicas[newReplicas.length - 1] =
          new ReplicaUnderConstruction(reportedBlock, storage, rState);
