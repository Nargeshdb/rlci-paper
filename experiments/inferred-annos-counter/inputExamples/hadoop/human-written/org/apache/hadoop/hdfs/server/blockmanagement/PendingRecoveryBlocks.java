...
/**
 * PendingRecoveryBlocks tracks recovery attempts for each block and their timeouts to ensure we do
 * not have multiple recoveries at the same time and retry only after the timeout for a recovery has
 * expired.
 */
class PendingRecoveryBlocks {
  ...
  /**
   * The timeout for issuing a block recovery again. (it should be larger than the time to recover a
   * block)
   */
  ...
  synchronized boolean add(BlockInfo block) {
    ...
    if (recoveryAttempt == null) {
      BlockRecoveryAttempt newAttempt =
          new BlockRecoveryAttempt(block, curTime + recoveryTimeoutInterval);
      ...
      LOG.info(
          "Block recovery attempt for "
              + block
              + " rejected, as the "
              + "previous attempt times out in "
              + timeoutIn
              + " seconds.");
    }
    return added;
  ...
  synchronized boolean isUnderRecovery(BlockInfo b) {
    BlockRecoveryAttempt recoveryAttempt = recoveryTimeouts.getElement(new BlockRecoveryAttempt(b));
