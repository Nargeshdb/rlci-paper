...
/**
 * The balancer is a tool that balances disk space usage on an HDFS cluster when some datanodes
 * become full or when new empty nodes join the cluster. The tool is deployed as an application
 * program that can be run by the cluster administrator on a live HDFS cluster while applications
 ...
 * <p>DESCRIPTION
 *
 * <p>The threshold parameter is a fraction in the range of (1%, 100%) with a default value of 10%.
 * The threshold sets a target for whether the cluster is balanced. A cluster is balanced if for
 * each datanode, the utilization of the node (ratio of used space at the node to total capacity of
 * the node) differs from the utilization of the (ratio of used space in the cluster to total
 * capacity of the cluster) by no more than the threshold value. The smaller the threshold, the more
 * balanced a cluster will become. It takes more time to run the balancer for small threshold
 * values. Also for a very small threshold the cluster may not be able to reach the balanced state
 * when applications write and delete files concurrently.
 *
 * <p>The tool moves blocks from highly utilized datanodes to poorly utilized datanodes iteratively.
 * In each iteration a datanode moves or receives no more than the lesser of 10G bytes or the
 * threshold fraction of its capacity. Each iteration runs no more than 20 minutes. At the end of
 * each iteration, the balancer obtains updated datanodes information from the namenode.
 *
 * <p>A system property that limits the balancer's use of bandwidth is defined in the default
 * configuration file:
 *
 * <pre>
 * &lt;property&gt;
 ...
 * </pre>
 *
 * <p>This property determines the maximum speed at which a block will be moved from one datanode to
 * another. The default value is 1MB/s. The higher the bandwidth, the faster a cluster can reach the
 * balanced state, but with greater competition with application processes. If an administrator
 * changes the value of this property in the configuration file, the change is observed when HDFS is
 * next restarted.
 *
 * <p>MONITERING BALANCER PROGRESS
 *
 * <p>After the balancer is started, an output file name where the balancer progress will be
 * recorded is printed on the screen. The administrator can monitor the running of the balancer by
 * reading the output file. The output shows the balancer's status iteration by iteration. In each
 * iteration it prints the starting time, the iteration number, the total number of bytes that have
 * been moved in the previous iterations, the total number of bytes that are left to move in order
 * for the cluster to be balanced, and the number of bytes that are being moved in this iteration.
 * Normally "Bytes Already Moved" is increasing while "Bytes Left To Move" is decreasing.
 *
 * <p>Running multiple instances of the balancer in an HDFS cluster is prohibited by the tool.
 *
 * <p>The balancer automatically exits when any of the following five conditions is satisfied:
 *
 * <ol>
 ...
 * </ol>
 *
 * <p>Upon exit, a balancer returns an exit code and prints one of the following messages to the
 * output file in corresponding to the above exit reasons:
 *
 * <ol>
 ...
 * </ol>
 *
 * <p>The administrator can interrupt the execution of the balancer at any time by running the
 * command "stop-balancer.sh" on the machine where the balancer is running.
 */
...
public class Balancer {
  ...
  private static final String USAGE =
      "Usage: hdfs balancer"
          ...
          + BalancingPolicy.Node.INSTANCE.getName()
          + " or "
  ...
  @VisibleForTesting private static volatile boolean serviceRunning = false;
  ...
  private static void checkReplicationPolicyCompatibility(Configuration conf)
    ...
    if (!(placementPolicies.getPolicy(CONTIGUOUS) instanceof BlockPlacementPolicyDefault)) {
  ...
  /**
   * Construct a balancer. Initialize balancer. It sets the value of the threshold, and builds the
   * communication proxies to namenode as a client and a secondary namenode and retry proxies when
   * connection fails.
   */
  Balancer(NameNodeConnector theblockpool, BalancerParameters p, Configuration conf) {
    final long movedWinWidth =
        getLong(
            conf,
    ...
    this.defaultBlockSize =
        getLongBytes(conf, DFSConfigKeys.DFS_BLOCK_SIZE_KEY, DFSConfigKeys.DFS_BLOCK_SIZE_DEFAULT);
  }
  ...
  /**
   * Given a datanode storage set, build a network topology and decide over-utilized storages, above
   * average utilized storages, below average utilized storages, and underutilized storages. The
   * input datanode storage set is shuffled in order to randomize to the storage matching later on.
   *
  ...
  private long init(List<DatanodeStorageReport> reports) {
    ...
    for (DatanodeStorageReport r : reports) {
      ...
      for (StorageType t : StorageType.getMovableTypes()) {
        ...
        final long maxSize2Move =
            computeMaxSize2Move(capacity, getRemaining(r, t), utilizationDiff, maxSizeToMove);
    ...
    Preconditions.checkState(
        dispatcher.getStorageGroupMap().size()
            == overUtilized.size()
                + underUtilized.size()
                + aboveAvgUtilized.size()
  ...
  private static long computeMaxSize2Move(
      final long capacity, final long remaining, final double utilizationDiff, final long max) {
  ...
  private static long percentage2bytes(double percentage, long capacity) {
    Preconditions.checkArgument(percentage >= 0, "percentage = %s < 0", percentage);
  ...
  private static <T extends StorageGroup> void logUtilizationCollection(
      String name, Collection<T> items) {
    LOG.info(items.size() + " " + name + ": " + items);
  }

  /**
   * Decide all <source, target> pairs and the number of bytes to move from a source to a target
   * Maximum bytes to be moved per storage group is min(1 Band worth of bytes, MAX_SIZE_TO_MOVE).
   *
  ...
  /**
   * For each datanode, choose matching nodes from the candidates. Either the datanodes or the
   * candidates are source nodes with (utilization > Avg), and the others are target nodes with
   * (utilization < Avg).
   */
  private <G extends StorageGroup, C extends StorageGroup> void chooseStorageGroups(
      Collection<G> groups, Collection<C> candidates, Matcher matcher) {
   ...
   */
  private <C extends StorageGroup> boolean choose4One(
      StorageGroup g, Collection<C> candidates, Matcher matcher) {
  ...
  private <G extends StorageGroup, C extends StorageGroup> C chooseCandidate(
      G g, Iterator<C> candidates, Matcher matcher) {
    if (g.hasSpaceForScheduling()) {
  ...
  private boolean matchStorageGroups(StorageGroup left, StorageGroup right, Matcher matcher) {
    return left.getStorageType() == right.getStorageType()
        && matcher.match(dispatcher.getCluster(), left.getDatanodeInfo(), right.getDatanodeInfo());
  }
  ...
  static class Result {
    ...
    Result(
        ExitStatus exitStatus, long bytesLeftToMove, long bytesBeingMoved, long bytesAlreadyMoved) {
  ...
  Result runOneIteration() {
    try {
      ...
      if (!runDuringUpgrade && nnc.isUpgrading()) {
        System.err.println(
            "Balancer exiting as upgrade is not finalized, "
  ...
  /**
   * Balance all namenodes. For each iteration, for each namenode, execute a {@link Balancer} to
   * work through all datanodes once.
   */
  private static int doBalance(
      Collection<URI> namenodes, final BalancerParameters p, Configuration conf)
  ...
  static int run(Collection<URI> namenodes, final BalancerParameters p, Configuration conf)
      throws IOException, InterruptedException {
    ...
    int retryOnException =
        conf.getInt(
            DFSConfigKeys.DFS_BALANCER_SERVICE_RETRIES_ON_EXCEPTION,
    ...
    while (serviceRunning) {
      ...
      LOG.info("Finished one round, will wait for {} for next round", time2Str(scheduleInterval));
  ...
  private static void checkKeytabAndInit(Configuration conf) throws IOException {
    if (conf.getBoolean(
      ...
      String addr =
          conf.get(
              DFSConfigKeys.DFS_BALANCER_ADDRESS_KEY, DFSConfigKeys.DFS_BALANCER_ADDRESS_DEFAULT);
      InetSocketAddress socAddr =
          NetUtils.createSocketAddr(addr, 0, DFSConfigKeys.DFS_BALANCER_ADDRESS_KEY);
      SecurityUtil.login(
          conf,
        ...
        try {
          for (int i = 0; i < args.length; i++) {
            if ("-threshold".equalsIgnoreCase(args[i])) {
              checkArgument(
                  ++i < args.length, "Threshold value is missing: args = " + Arrays.toString(args));
              try {
              ...
              checkArgument(
                  ++i < args.length, "Policy value is missing: args = " + Arrays.toString(args));
              try {
              ...
              checkArgument(
                  ++i < args.length,
                  "blockpools value is missing: args = " + Arrays.toString(args));
              ...
              LOG.info(
                  "Will run the balancer even during an ongoing HDFS "
          ...
          checkArgument(
              excludedNodes == null || includedNodes == null,
