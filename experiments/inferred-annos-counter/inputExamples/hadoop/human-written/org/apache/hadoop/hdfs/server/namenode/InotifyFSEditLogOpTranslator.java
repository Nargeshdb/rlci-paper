    ...
    switch (op.opCode) {
          ...
          return new EventBatch(
              ...
              new Event[] {
                new Event.CreateEvent.Builder()
                    ...
                    .erasureCoded(
                        addOp.erasureCodingPolicyId != ErasureCodeConstants.REPLICATION_POLICY_ID)
                    .iNodeType(Event.CreateEvent.INodeType.FILE)
                    .build()
              });
        } else { // append
          return new EventBatch(
              op.txid, new Event[] {new Event.AppendEvent.Builder().path(addOp.path).build()});
        }
      case OP_CLOSE:
        FSEditLogOp.CloseOp cOp = (FSEditLogOp.CloseOp) op;
        return new EventBatch(
            op.txid, new Event[] {new Event.CloseEvent(cOp.path, getSize(cOp), cOp.mtime)});
      case OP_APPEND:
        FSEditLogOp.AppendOp appendOp = (FSEditLogOp.AppendOp) op;
        return new EventBatch(
            op.txid,
            new Event[] {
              new Event.AppendEvent.Builder()
                  .path(appendOp.path)
        ...
        return new EventBatch(
            ...
            new Event[] {
              new Event.MetadataUpdateEvent.Builder()
                  ...
                  .path(setRepOp.path)
                  .replication(setRepOp.replication)
                  .build()
            });
      ...
      case OP_DELETE:
        ...
        return new EventBatch(
            op.txid,
            new Event[] {
              new Event.UnlinkEvent.Builder().path(delOp.path).timestamp(delOp.timestamp).build()
            });
      case OP_MKDIR:
        ...
        return new EventBatch(
            ...
            new Event[] {
              new Event.MetadataUpdateEvent.Builder()
                  ...
                  .path(ownOp.src)
                  .ownerName(ownOp.username)
                  .groupName(ownOp.groupname)
                  .build()
            });
      case OP_TIMES:
        ...
        return new EventBatch(
            ...
            new Event[] {
              new Event.CreateEvent.Builder()
                  ...
                  .iNodeType(Event.CreateEvent.INodeType.SYMLINK)
                  .build()
            });
        ...
        return new EventBatch(
            ...
            new Event[] {
              new Event.MetadataUpdateEvent.Builder()
                  ...
                  .path(saOp.src)
                  .acls(saOp.aclEntries)
                  .build()
            });
      case OP_TRUNCATE:
        FSEditLogOp.TruncateOp tOp = (FSEditLogOp.TruncateOp) op;
        return new EventBatch(
            op.txid, new Event[] {new Event.TruncateEvent(tOp.src, tOp.newLength, tOp.timestamp)});
      default:
        return null;
    }
  }
