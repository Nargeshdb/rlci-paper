...
/**
 * Implementation for protobuf service that forwards requests received on {@link NamenodeProtocolPB}
 * to the {@link NamenodeProtocol} server implementation.
 */
public class NamenodeProtocolServerSideTranslatorPB implements NamenodeProtocolPB {
  private final NamenodeProtocol impl;

  private static final ErrorReportResponseProto VOID_ERROR_REPORT_RESPONSE =
      ErrorReportResponseProto.newBuilder().build();

  private static final EndCheckpointResponseProto VOID_END_CHECKPOINT_RESPONSE =
  ...
  public GetBlocksResponseProto getBlocks(RpcController unused, GetBlocksRequestProto request)
    ...
    DatanodeInfo dnInfo =
        new DatanodeInfoBuilder().setNodeID(PBHelperClient.convert(request.getDatanode())).build();
    BlocksWithLocations blocks;
    try {
      blocks = impl.getBlocks(dnInfo, request.getSize(), request.getMinBlockSize());
    } catch (IOException e) {
  ...
  public GetBlockKeysResponseProto getBlockKeys(
      RpcController unused, GetBlockKeysRequestProto request) throws ServiceException {
  ...
  public GetTransactionIdResponseProto getTransactionId(
      RpcController unused, GetTransactionIdRequestProto request) throws ServiceException {
    long txid;
    try {
  ...
  public ErrorReportResponseProto errorReport(RpcController unused, ErrorReportRequestProto request)
    ...
    try {
      impl.errorReport(
          PBHelper.convert(request.getRegistration()), request.getErrorCode(), request.getMsg());
    } catch (IOException e) {
      throw new ServiceException(e);
    }
  ...
  public StartCheckpointResponseProto startCheckpoint(
      RpcController unused, StartCheckpointRequestProto request) throws ServiceException {
    NamenodeCommand cmd;
    try {
  ...
  public EndCheckpointResponseProto endCheckpoint(
      RpcController unused, EndCheckpointRequestProto request) throws ServiceException {
    try {
      impl.endCheckpoint(
          PBHelper.convert(request.getRegistration()), PBHelper.convert(request.getSignature()));
    } catch (IOException e) {
      throw new ServiceException(e);
    }
  ...
  public IsUpgradeFinalizedResponseProto isUpgradeFinalized(
      RpcController controller, IsUpgradeFinalizedRequestProto request) throws ServiceException {
    ...
    return IsUpgradeFinalizedResponseProto.newBuilder()
        .setIsUpgradeFinalized(isUpgradeFinalized)
        .build();
  }

  @Override
  public IsRollingUpgradeResponseProto isRollingUpgrade(
      RpcController controller, IsRollingUpgradeRequestProto request) throws ServiceException {
    ...
    return IsRollingUpgradeResponseProto.newBuilder().setIsRollingUpgrade(isRollingUpgrade).build();
  }

  @Override
  public GetNextSPSPathResponseProto getNextSPSPath(
      RpcController controller, GetNextSPSPathRequestProto request) throws ServiceException {
    try {
      Long nextSPSPath = impl.getNextSPSPath();
      if (nextSPSPath == null) {
        return GetNextSPSPathResponseProto.newBuilder().build();
      }
      return GetNextSPSPathResponseProto.newBuilder().setSpsPath(nextSPSPath).build();
    } catch (IOException e) {
      throw new ServiceException(e);
    }
  }
