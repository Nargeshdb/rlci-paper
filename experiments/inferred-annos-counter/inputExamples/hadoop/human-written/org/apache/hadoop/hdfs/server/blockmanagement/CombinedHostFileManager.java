...
/**
 * This class manages datanode configuration using a json file. Please refer to {@link
 * CombinedHostsFileReader} for the json format.
 *
 * <p>Entries may or may not specify a port. If they don't, we consider them to apply to every
 * DataNode on that host. The code canonicalizes the entries into IP addresses.
 *
 * <p>The code ignores all entries that the DNS fails to resolve their IP addresses. This is okay
 * because by default the NN rejects the registrations of DNs when it fails to do a forward and
 * reverse lookup. Note that DNS resolutions are only done during the loading time to minimize the
 * latency.
 */
public class CombinedHostFileManager extends HostConfigManager {
  ...
  static class HostProperties {
    ...
    synchronized void add(InetAddress addr, DatanodeAdminProperties properties) {
      ...
      if (properties.getAdminState().equals(AdminStates.NORMAL)) {
    ...
    synchronized boolean isIncluded(final InetSocketAddress address) {
      return emptyInServiceNodeLists
          || Iterables.any(
    ...
    synchronized boolean isExcluded(final InetSocketAddress address) {
      return Iterables.any(
          ...
          new Predicate<DatanodeAdminProperties>() {
            public boolean apply(DatanodeAdminProperties input) {
              return input.getAdminState().equals(AdminStates.DECOMMISSIONED)
                  && (input.getPort() == 0 || input.getPort() == address.getPort());
            }
          });
    }
      ...
      return new Iterable<InetSocketAddress>() {
        ...
        public Iterator<InetSocketAddress> iterator() {
          return new HostIterator(
              Collections2.filter(
                  allDNs.entries(),
                  new Predicate<java.util.Map.Entry<InetAddress, DatanodeAdminProperties>>() {
                    public boolean apply(
                        java.util.Map.Entry<InetAddress, DatanodeAdminProperties> entry) {
    ...
    synchronized long getMaintenanceExpireTimeInMS(final InetSocketAddress address) {
      Iterable<DatanodeAdminProperties> datanode =
          Iterables.filter(
              ...
              new Predicate<DatanodeAdminProperties>() {
                public boolean apply(DatanodeAdminProperties input) {
                  return input.getAdminState().equals(AdminStates.IN_MAINTENANCE)
                      && (input.getPort() == 0 || input.getPort() == address.getPort());
                }
              });
      ...
      return datanode.iterator().hasNext()
          ? datanode.iterator().next().getMaintenanceExpireTimeInMS()
          : 0;
    }
    ...
    static class HostIterator extends UnmodifiableIterator<InetSocketAddress> {
      ...
      public HostIterator(
          Collection<java.util.Map.Entry<InetAddress, DatanodeAdminProperties>> nodes) {
  ...
  private void refresh(final String hostsFile) throws IOException {
    ...
    DatanodeAdminProperties[] all = CombinedHostsFileReader.readFile(hostsFile);
    for (DatanodeAdminProperties properties : all) {
      InetSocketAddress addr =
          parseEntry(hostsFile, properties.getHostName(), properties.getPort());
      if (addr != null) {
  ...
  @VisibleForTesting
  static InetSocketAddress parseEntry(final String fn, final String hostName, final int port) {
