...
/**
 * A directory with this feature is a snapshottable directory, where snapshots can be taken. This
 * feature extends {@link DirectoryWithSnapshotFeature}, and maintains extra information about all
...
public class DirectorySnapshottableFeature extends DirectoryWithSnapshotFeature {
  ...
  /**
   * Snapshots of this directory in ascending order of snapshot names. Note that snapshots in
   * ascending order of snapshot id are stored in {@link DirectoryWithSnapshotFeature}.diffs (a
   * private field).
   */
   ...
   */
  public void renameSnapshot(String path, String oldName, String newName, long mtime)
  ...
  /** Simply add a snapshot into the {@link #snapshotsByNames}. Used when loading fsimage. */
   ...
   * @throws SnapshotException Throw SnapshotException when there is a snapshot with the same name
   *     already exists or snapshot quota exceeds
   */
  public Snapshot addSnapshot(
      ...
      final boolean captureOpenFiles,
    ...
    if (captureOpenFiles) {
      try {
        ...
        throw new SnapshotException(
            "Failed to add snapshot: Unable to "
  ...
  public void computeContentSummary4Snapshot(
      final BlockStoragePolicySuite bsps, final ContentCounts counts)
   ...
   * directory. The diff calculation can be scoped to either the snapshot root or any descendant
   * directory under the snapshot root.
   *
   * @param snapshotRootDir the snapshot root directory
   * @param snapshotDiffScopeDir the descendant directory under snapshot root to scope the diff
   *     calculation to.
   ...
   */
  SnapshotDiffInfo computeDiff(
      final INodeDirectory snapshotRootDir,
      final INodeDirectory snapshotDiffScopeDir,
      final String from,
      final String to)
      throws SnapshotException {
    ...
    SnapshotDiffInfo diffs =
        new SnapshotDiffInfo(snapshotRootDir, snapshotDiffScopeDir, fromSnapshot, toSnapshot);
    ...
    // snapshot scope dir.
    computeDiffRecursively(snapshotDiffScopeDir, snapshotDiffScopeDir, new ArrayList<>(), diffs);
    return diffs;
  }
   ...
   *     snapshotdiffentry limit. -1 indicates, the snapshotdiff computation needs to start right
   *     from the startPath provided.
   * @return The difference between the start/end points.
   * @throws SnapshotException If there is no snapshot matching the starting point, or if
   *     endSnapshotName is not null but cannot be identified as a previous snapshot.
   */
  SnapshotDiffListingInfo computeDiff(
      final INodeDirectory snapshotRootDir,
      final INodeDirectory snapshotDiffScopeDir,
      final String from,
      final String to,
      byte[] startPath,
      int index,
      int snapshotDiffReportEntriesLimit)
      throws SnapshotException {
    Preconditions.checkArgument(snapshotDiffScopeDir.isDescendantOfSnapshotRoot(snapshotRootDir));
    Snapshot fromSnapshot = getSnapshotByName(snapshotRootDir, from);
    Snapshot toSnapshot = getSnapshotByName(snapshotRootDir, to);
    ...
    SnapshotDiffListingInfo diffs =
        new SnapshotDiffListingInfo(
            snapshotRootDir,
            snapshotDiffScopeDir,
            fromSnapshot,
            toSnapshot,
            snapshotDiffReportEntriesLimit);
    diffs.setLastIndex(index);
    computeDiffRecursively(
        snapshotDiffScopeDir,
        snapshotDiffScopeDir,
        new ArrayList<byte[]>(),
        diffs,
   ...
   * @throws SnapshotException If snapshotName is not null or empty, but there is no snapshot
   *     matching the name.
   */
  private Snapshot getSnapshotByName(INodeDirectory snapshotRoot, String snapshotName)
      throws SnapshotException {
    Snapshot s = null;
    if (snapshotName != null && !snapshotName.isEmpty()) {
      ...
      if (index < 0) {
        throw new SnapshotException(
            "Cannot find the snapshot of directory "
                + snapshotRoot.getFullPathName()
                + " with name "
                + snapshotName);
      }
   ...
   *
   * @param snapshotDir The directory where snapshots were taken. Can be a snapshot root directory
   *     or any descendant directory under snapshot root directory.
  ...
  private void computeDiffRecursively(
      final INodeDirectory snapshotDir,
      INode node,
      List<byte[]> parentPath,
      SnapshotDiffInfo diffReport) {
    final Snapshot earlierSnapshot =
        diffReport.isFromEarlier() ? diffReport.getFrom() : diffReport.getTo();
    final Snapshot laterSnapshot =
        diffReport.isFromEarlier() ? diffReport.getTo() : diffReport.getFrom();
        ...
        if (!toProcess && child instanceof INodeReference.WithName) {
          byte[][] renameTargetPath =
              findRenameTargetPath(
                  snapshotDir,
                  (WithName) child,
                  laterSnapshot == null ? Snapshot.CURRENT_STATE_ID : laterSnapshot.getId());
  ...
  /**
   * Recursively compute the difference between snapshots under a given directory/file partially.
   *
   * @param snapshotDir The directory where snapshots were taken. Can be a snapshot root directory
   *     or any descendant directory under snapshot root directory.
   * @param node The directory/file under which the diff is computed.
   * @param parentPath Relative path (corresponding to the snapshot root) of the node's parent.
   * @param diffReport data structure used to store the diff.
   * @param resume path from where to resume the snapshotdiff computation in one rpc call
   * @param level indicates the level of the directory tree rooted at snapshotRoot.
   * @param processFlag indicates that the dir/file where the snapshotdiff computation has to start
   *     is processed or not.
   */
  private boolean computeDiffRecursively(
      final INodeDirectory snapshotDir,
      INode node,
      List<byte[]> parentPath,
      SnapshotDiffListingInfo diffReport,
      final byte[][] resume,
      int level,
      boolean processFlag) {
    ...
    if (node.isDirectory()) {
      ...
      if (processFlag) {
        DirectoryWithSnapshotFeature sf = dir.getDirectoryWithSnapshotFeature();
        if (sf != null) {
          boolean change = sf.computeDiffBetweenSnapshots(earlier, later, diff, dir);
          if (change) {
      ...
      for (INode child : children) {
        ...
        if (!toProcess && child instanceof INodeReference.WithName) {
          byte[][] renameTargetPath =
              findRenameTargetPath(snapshotDir, (WithName) child, Snapshot.getSnapshotId(later));
          if (renameTargetPath != null) {
            toProcess = true;
          }
        }
        if (toProcess) {
          ...
          processFlag =
              computeDiffRecursively(
                  snapshotDir, child, parentPath, diffReport, resume, level, processFlag);
          parentPath.remove(parentPath.size() - 1);
          if (!processFlag) {
            return false;
          }
      ...
      INodeFile file = node.asFile();
      boolean change =
          file.getFileWithSnapshotFeature().changedBetweenSnapshots(file, earlier, later);
      if (change) {
  ...
  /**
   * We just found a deleted WithName node as the source of a rename operation. However, we should
   * include it in our snapshot diff report as rename only if the rename target is also under the
   * same snapshottable directory.
   */
  public byte[][] findRenameTargetPath(
      final INodeDirectory snapshotRoot, INodeReference.WithName wn, final int snapshotId) {
    ...
    while (inode != null) {
      ...
      if (inode instanceof INodeReference.WithCount) {
        ...
        INode parent =
            inode.getParentReference() != null ? inode.getParentReference() : inode.getParent();
  ...
  @VisibleForTesting
  public void dumpTreeRecursively(
      INodeDirectory snapshotRoot, PrintWriter out, StringBuilder prefix, int snapshot) {
    if (snapshot == Snapshot.CURRENT_STATE_ID) {
      ...
      Preconditions.checkState(
          n == snapshotsByNames.size(),
          "#n=" + n + ", snapshotsByNames.size()=" + snapshotsByNames.size());
      out.print(", #snapshot=");
      out.println(n);

      INodeDirectory.dumpTreeRecursively(
          out,
          prefix,
            ...
            public Iterator<SnapshotAndINode> iterator() {
              return new Iterator<SnapshotAndINode>() {
                ...
                public SnapshotAndINode next() {
                  final SnapshotAndINode pair =
                      new SnapshotAndINode(
                          next.getSnapshotId(), getSnapshotById(next.getSnapshotId()).getRoot());
                  next = findNext();
                  return pair;
                }
