...
/**
 * Channel to a remote JournalNode using Hadoop IPC. All of the calls are run on a separate thread,
 * and return {@link ListenableFuture} instances to wait for their result. This allows calls to be
 * bound together using the {@link QuorumCall} class.
 */
...
public class IPCLoggerChannel implements AsyncLogger {
  ...
  /**
   * Executes tasks submitted to it serially, on a single thread, in FIFO order (generally used for
   * write tasks that should not be reordered).
   */
  private final ListeningExecutorService singleThreadExecutor;
  /**
   * Executes tasks submitted to it in parallel with each other and with those submitted to
   * singleThreadExecutor (generally used for read tasks that can be safely reordered and
  ...
  /** Nanotime of the last time we successfully journaled some edits to the remote node. */
  private long lastAckNanos = 0;

  /**
   * Nanotime of the last time that committedTxId was update. Used to calculate the lag in terms of
   * time, rather than just a number of txns.
   */
  private long lastCommitNanos = 0;

  /**
   * The maximum number of bytes that can be pending in the queue. This keeps the writer from
   * hitting OOME if one of the loggers starts responding really slowly. Eventually, the queue
   * overflows and it starts to treat the logger as having errored.
   */
  private final int queueSizeLimitBytes;

  /**
   * If this logger misses some edits, or restarts in the middle of a segment, the writer won't be
   * able to write any more edits until the beginning of the next segment. Upon detecting this
   * situation, the writer sets this flag to true to avoid sending useless RPCs.
   */
  ...
  static final Factory FACTORY =
      new AsyncLogger.Factory() {
        @Override
        public AsyncLogger createLogger(
            Configuration conf,
  ...
  public IPCLoggerChannel(
      Configuration conf, NamespaceInfo nsInfo, String journalId, InetSocketAddress addr) {
  ...
  public synchronized void setCommittedTxId(long txid) {
    Preconditions.checkArgument(
        txid >= committedTxId,
        "Trying to move committed txid backwards in client " + "old: %s new: %s",
        committedTxId,
        txid);
  ...
  protected QJournalProtocol createProxy() throws IOException {
    ...
    RPC.setProtocolEngine(confCopy, QJournalProtocolPB.class, ProtobufRpcEngine2.class);
    return SecurityUtil.doAsLoginUser(
        new PrivilegedExceptionAction<QJournalProtocol>() {
          @Override
          public QJournalProtocol run() throws IOException {
            RPC.setProtocolEngine(confCopy, QJournalProtocolPB.class, ProtobufRpcEngine2.class);
            QJournalProtocolPB pbproxy =
                RPC.getProxy(
  ...
  protected ExecutorService createParallelExecutor() {
    int numThreads =
        conf.getInt(
            DFSConfigKeys.DFS_QJOURNAL_PARALLEL_READ_NUM_THREADS_KEY,
  ...
  public URL buildURLToFetchLogs(long segmentTxId) {
    ...
    try {
      String path = GetJournalEditServlet.buildPath(journalId, segmentTxId, nsInfo, true);
  ...
  private synchronized RequestInfo createReqInfo() {
    ...
    return new RequestInfo(journalId, nameServiceId, epoch, ipcSerial++, committedTxId);
  }
  ...
  /**
   * @return true if the server has gotten out of sync from the client, and thus a log roll is
   *     required for this logger to successfully start logging more edits.
   */
  ...
  public ListenableFuture<GetJournalStateResponseProto> getJournalState() {
    return singleThreadExecutor.submit(
        new Callable<GetJournalStateResponseProto>() {
          @Override
  ...
  public ListenableFuture<NewEpochResponseProto> newEpoch(final long epoch) {
    return singleThreadExecutor.submit(
        new Callable<NewEpochResponseProto>() {
          @Override
  ...
  public ListenableFuture<Void> sendEdits(
      final long segmentTxId, final long firstTxnId, final int numTxns, final byte[] data) {
    try {
              ...
              new Callable<Void>() {
                ...
                public Void call() throws IOException {
                  ...
                  try {
                    ...
                    long rpcTime =
                        TimeUnit.MICROSECONDS.convert(now - rpcSendTimeNanos, TimeUnit.NANOSECONDS);
                    long endToEndTime =
                        TimeUnit.MICROSECONDS.convert(now - submitNanos, TimeUnit.NANOSECONDS);
  ...
  /**
   * When we've entered an out-of-sync state, it's still useful to periodically send an empty RPC to
   * the server, such that it has the up to date committedTxId. This acts as a sanity check during
   * recovery, and also allows that node's metrics to be up-to-date about its lag.
   *
   * <p>In the future, this method may also be used in order to check that the current node is still
   * the current writer, even if no edits are being written.
   */
  private void heartbeatIfNecessary() throws IOException {
    if (lastHeartbeatStopwatch.now(TimeUnit.MILLISECONDS) > HEARTBEAT_INTERVAL_MILLIS
  ...
  private synchronized void reserveQueueSpace(int size) throws LoggerTooFarBehindException {
    ...
    if (queuedEditsSizeBytes + size > queueSizeLimitBytes && queuedEditsSizeBytes > 0) {
      QuorumJournalManager.LOG.warn(
          "Pending edits to "
              + IPCLoggerChannel.this
              + " is going to exceed limit size: "
              + queueSizeLimitBytes
              + ", current queued edits size: "
              + queuedEditsSizeBytes
              + ", will silently drop "
              + size
              + " bytes of edits!");
    ...
    return parallelExecutor.submit(
        new Callable<GetJournaledEditsResponseProto>() {
          ...
          public GetJournaledEditsResponseProto call() throws IOException {
            return getProxy()
                .getJournaledEdits(journalId, nameServiceId, fromTxnId, maxTransactions);
          }
        });
  }
  ...
  public ListenableFuture<RemoteEditLogManifest> getEditLogManifest(
    ...
    return parallelExecutor.submit(
        new Callable<RemoteEditLogManifest>() {
          @Override
          public RemoteEditLogManifest call() throws IOException {
            GetEditLogManifestResponseProto ret =
                getProxy().getEditLogManifest(journalId, nameServiceId, fromTxnId, inProgressOk);
  ...
  public ListenableFuture<PrepareRecoveryResponseProto> prepareRecovery(final long segmentTxId) {
    return singleThreadExecutor.submit(
        new Callable<PrepareRecoveryResponseProto>() {
          @Override
  ...
  @Override
  public ListenableFuture<Boolean> canRollBack(
      final StorageInfo storage, final StorageInfo prevStorage, final int targetLayoutVersion) {
    return singleThreadExecutor.submit(
        new Callable<Boolean>() {
          @Override
          public Boolean call() throws IOException {
            return getProxy()
                .canRollBack(journalId, nameServiceId, storage, prevStorage, targetLayoutVersion);
          }
        });
  }
  ...
  public synchronized long getLagTimeMillis() {
    return TimeUnit.MILLISECONDS.convert(
        Math.max(lastCommitNanos - lastAckNanos, 0), TimeUnit.NANOSECONDS);
  }
   ...
   *
   * <p>The fromURL field in the response specifies the endpoint of the http server. However, the
   * address might not be accurate since the server can bind to multiple interfaces. Here the client
   * plugs in the address specified in the configuration and generates the URI.
   */
