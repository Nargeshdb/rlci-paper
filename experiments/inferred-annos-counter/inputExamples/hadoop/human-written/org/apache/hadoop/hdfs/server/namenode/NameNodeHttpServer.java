...
public class NameNodeHttpServer {
  ...
  public static void initWebHdfs(
      Configuration conf,
      String hostname,
      String httpKeytab,
    ...
    httpServer2.addJerseyResourcePackage(
        jerseyResourcePackage + ";" + Param.class.getPackage().getName(), pathSpec, params);
  }
  ...
  void start() throws IOException {
    ...
    String httpKeytab =
        conf.get(DFSUtil.getSpnegoKeytabKey(conf, DFSConfigKeys.DFS_NAMENODE_KEYTAB_FILE_KEY));
    initWebHdfs(
        conf,
    ...
    if (policy.isHttpEnabled()) {
      ...
      conf.set(
          DFSConfigKeys.DFS_NAMENODE_HTTP_ADDRESS_KEY, NetUtils.getHostPortString(httpAddress));
    }

    if (policy.isHttpsEnabled()) {
      httpsAddress = httpServer.getConnectorAddress(connIdx);
      conf.set(
          DFSConfigKeys.DFS_NAMENODE_HTTPS_ADDRESS_KEY, NetUtils.getHostPortString(httpsAddress));
    }
  }
  ...
  private static void setupServlets(HttpServer2 httpServer, Configuration conf) {
    ...
    httpServer.addInternalServlet(
        IsNameNodeActiveServlet.SERVLET_NAME,
  ...
  public static HAServiceProtocol.HAServiceState getNameNodeStateFromContext(
