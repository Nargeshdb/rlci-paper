 ...
 *
 * <p>We will currently try each edit log stream exactly once. In other words, we don't handle the
 * "ping pong" scenario where different edit logs contain a different subset of the available edits.
 */
class RedundantEditLogInputStream extends EditLogInputStream {
  public static final Logger LOG =
      LoggerFactory.getLogger(RedundantEditLogInputStream.class.getName());
  private int curIdx;
  ...
  private static enum State {
    ...
    /** There are no more opcodes to read from this RedundantEditLogInputStream */
    EOF;
  }
  ...
  RedundantEditLogInputStream(Collection<EditLogInputStream> streams, long startTxId) {
    this.curIdx = 0;
    this.prevTxId =
        (startTxId == HdfsServerConstants.INVALID_TXID)
    ...
    for (EditLogInputStream s : streams) {
      Preconditions.checkArgument(
          s.getFirstTxId() != HdfsServerConstants.INVALID_TXID,
          "invalid first txid in stream: %s",
          s);
      Preconditions.checkArgument(
          s.getLastTxId() != HdfsServerConstants.INVALID_TXID,
          "invalid last txid in stream: %s",
          s);
      if (first == null) {
        first = s;
      } else {
        Preconditions.checkArgument(
            s.getFirstTxId() == first.getFirstTxId(),
  ...
  public String getName() {
    ...
    for (EditLogInputStream elis : streams) {
      bld.append(prefix).append(elis.getName());
      prefix = ", ";
    }
    return bld.toString();
  }
            ...
            if (op == null) {
              ...
              if (streams[curIdx].getLastTxId() == prevTxId) {
                ...
                throw new PrematureEOFException(
                    "got premature end-of-file "
                        + "at txid "
                        + prevTxId
                        + "; expected file to go up to "
        ...
        case STREAM_FAILED:
          ...
          if (newLast < oldLast) {
            throw new IOException(
                "We encountered an error reading "
                    + streams[curIdx].getName()
                    + ".  During automatic edit log "
                    ...
                    + oldLast
                    + ".  If you continue, metadata will be lost forever!");
          }
          LOG.error(
              "Got error reading edit log input stream "
                  + streams[curIdx].getName()
  ...
  private static final class PrematureEOFException extends IOException {
