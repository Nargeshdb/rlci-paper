...
/**
 * Utility class to start a datanode in a secure cluster, first obtaining privileged resources
 * before main startup and handing them to the datanode.
 */
public class SecureDataNodeStarter implements Daemon {
  ...
  /**
   * Acquire privileged resources (i.e., the privileged ports) for the data node. The privileged
   * resources consist of the port of the RPC server and the port of HTTP (not HTTPS) server.
   */
  @VisibleForTesting
  @SuppressWarnings({
    "builder:required.method.not.called", // TP: httpChannel possible exceptional exit due to throw
                                          // new RuntimeException("Unable to bind on specified info
                                          // port in secure context. Needed " +
                                          // infoSocAddr.getPort() + ", got " + ss.getLocalPort())
                                          // (validated)
    "builder:required.method.not.called" // TP: ss also remains open (validated)
  })
  public static SecureResources getSecureResources(Configuration conf) throws Exception {
    ...
    int socketWriteTimeout =
        conf.getInt(
            DFSConfigKeys.DFS_DATANODE_SOCKET_WRITE_TIMEOUT_KEY, HdfsConstants.WRITE_TIMEOUT);
    int backlogLength =
        conf.getInt(
            CommonConfigurationKeysPublic.IPC_SERVER_LISTEN_QUEUE_SIZE_KEY,
            CommonConfigurationKeysPublic.IPC_SERVER_LISTEN_QUEUE_SIZE_DEFAULT);
    ServerSocket ss =
        (socketWriteTimeout > 0) ? ServerSocketChannel.open().socket() : new ServerSocket();
    ...
    if (policy.isHttpEnabled()) {
      ...
      InetSocketAddress localAddr =
          (InetSocketAddress) httpChannel.socket().getLocalSocketAddress();

      if (localAddr.getPort() != infoSocAddr.getPort()) {
        throw new RuntimeException(
            "Unable to bind on specified info port in "
