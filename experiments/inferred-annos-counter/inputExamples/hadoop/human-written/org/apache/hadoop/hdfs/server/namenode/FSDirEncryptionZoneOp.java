...
final class FSDirEncryptionZoneOp {
  ...
  /**
   * Private constructor for preventing FSDirEncryptionZoneOp object creation. Static-only class.
   */
  private FSDirEncryptionZoneOp() {}

  /**
   * Invoke KeyProvider APIs to generate an encrypted data encryption key for an encryption zone.
   * Should not be called with any locks held.
   *
  ...
  static KeyProvider.Metadata ensureKeyIsInitialized(
      final FSDirectory fsd, final String keyName, final String src) throws IOException {
   ...
   *
   * @param fsd fsdirectory
   * @param srcArg the path of a directory which will be the root of the encryption zone. The
   ...
   * @throws IOException
   */
  static FileStatus createEncryptionZone(
      final FSDirectory fsd,
      final String srcArg,
  ...
  static Map.Entry<EncryptionZone, FileStatus> getEZForPath(
      final FSDirectory fsd, final String srcArg, final FSPermissionChecker pc) throws IOException {
    final INodesInPath iip;
  ...
  static EncryptionZone getEZForPath(final FSDirectory fsd, final INodesInPath iip)
      throws IOException {
    fsd.readLock();
    try {
  ...
  static List<XAttr> reencryptEncryptionZone(
      final FSDirectory fsd, final INodesInPath iip, final String keyVersionName)
      throws IOException {
    assert keyVersionName != null;
    return fsd.ezManager.reencryptEncryptionZone(iip, keyVersionName);
  }

  static List<XAttr> cancelReencryptEncryptionZone(final FSDirectory fsd, final INodesInPath iip)
      throws IOException {
  ...
  /**
   * Update re-encryption progress (submitted). Caller should logSync after calling this, outside of
   * the FSN lock.
   *
   * <p>The reencryption status is updated during SetXAttrs.
   */
  static XAttr updateReencryptionSubmitted(
      final FSDirectory fsd, final INodesInPath iip, final String ezKeyVersionName)
      throws IOException {
    ...
    final ReencryptionInfoProto newProto =
        PBHelperClient.convert(ezKeyVersionName, Time.now(), false, 0, 0, null, null);
    final ZoneEncryptionInfoProto newZoneProto =
        PBHelperClient.convert(
            PBHelperClient.convert(zoneProto.getSuite()),
            PBHelperClient.convert(zoneProto.getCryptoProtocolVersion()),
            zoneProto.getKeyName(),
            newProto);

    final XAttr xattr =
        XAttrHelper.buildXAttr(CRYPTO_XATTR_ENCRYPTION_ZONE, newZoneProto.toByteArray());
  ...
  /**
   * Update re-encryption progress (start, checkpoint). Caller should logSync after calling this,
   * outside of the FSN lock.
   *
   * <p>The reencryption status is updated during SetXAttrs. Original reencryption status is passed
   * in to get existing information such as ezkeyVersionName and submissionTime.
   */
  static XAttr updateReencryptionProgress(
      final FSDirectory fsd,
      final INode zoneNode,
      final ZoneReencryptionStatus origStatus,
      final String lastFile,
      final long numReencrypted,
      final long numFailures)
      throws IOException {
    assert fsd.hasWriteLock();
    ...
    final ReencryptionInfoProto newProto =
        PBHelperClient.convert(
            origStatus.getEzKeyVersionName(),
            origStatus.getSubmissionTime(),
            false,
            origStatus.getFilesReencrypted() + numReencrypted,
            origStatus.getNumReencryptionFailures() + numFailures,
            null,
            lastFile);
  ...
  /**
   * Log re-encrypt complete (cancel, or 100% re-encrypt) to edits. Caller should logSync after
   * calling this, outside of the FSN lock.
   *
   * <p>Original reencryption status is passed in to get existing information, this should include
   * whether it is finished due to cancellation. The reencryption status is updated during SetXAttrs
   * for completion time.
   */
  static List<XAttr> updateReencryptionFinish(
      final FSDirectory fsd, final INodesInPath zoneIIP, final ZoneReencryptionStatus origStatus)
      throws IOException {
    assert origStatus != null;
    assert fsd.hasWriteLock();
    fsd.ezManager.getReencryptionStatus().markZoneCompleted(zoneIIP.getLastINode().getId());
    final XAttr xattr = generateNewXAttrForReencryptionFinish(zoneIIP, origStatus);
    final List<XAttr> xattrs = Lists.newArrayListWithCapacity(1);
    xattrs.add(xattr);
  ...
  static XAttr generateNewXAttrForReencryptionFinish(
      final INodesInPath iip, final ZoneReencryptionStatus status) throws IOException {
    final ZoneEncryptionInfoProto zoneProto = getZoneEncryptionInfoProto(iip);
    final ReencryptionInfoProto newRiProto =
        PBHelperClient.convert(
            status.getEzKeyVersionName(),
            status.getSubmissionTime(),
            status.isCanceled(),
            status.getFilesReencrypted(),
            status.getNumReencryptionFailures(),
            Time.now(),
            null);
  ...
  private static ZoneEncryptionInfoProto getZoneEncryptionInfoProto(final INodesInPath iip)
      throws IOException {
    final XAttr fileXAttr =
        FSDirXAttrOp.unprotectedGetXAttrByPrefixedName(
            iip.getLastINode(), iip.getPathSnapshotId(), CRYPTO_XATTR_ENCRYPTION_ZONE);
    if (fileXAttr == null) {
    ...
    try {
      ...
      throw new IOException(
          "Could not parse file encryption info for " + "inode " + iip.getPath(), e);
    }
  }

  /** Save the batch's edeks to file xattrs. */
  static void saveFileXAttrsForBatch(FSDirectory fsd, List<FileEdekInfo> batch) {
    assert fsd.getFSNamesystem().hasWriteLock();
    if (batch != null && !batch.isEmpty()) {
      for (FileEdekInfo entry : batch) {
        ...
        if (inode == null) {
          NameNode.LOG.info(
              "Cannot find inode {}, skip saving xattr for" + " re-encryption", entry.getInodeId());
          continue;
        }
        fsd.getEditLog()
            .logSetXAttrs(inode.getFullPathName(), inode.getXAttrFeature().getXAttrs(), false);
      }
    }
  }
   ...
   * @throws IOException
   */
  static void setFileEncryptionInfo(
      final FSDirectory fsd,
      final INodesInPath iip,
      final FileEncryptionInfo info,
      final XAttrSetFlag flag)
      throws IOException {
  ...
  /**
   * This function combines the per-file encryption info (obtained from the inode's XAttrs), and the
   * encryption info from its zone, and returns a consolidated FileEncryptionInfo instance. Null is
   * returned for non-encrypted or raw files.
   *
   * @param fsd fsdirectory
   * @param iip inodes in the path containing the file, passed in to avoid obtaining the list of
   *     inodes again
   * @return consolidated file encryption info; null for non-encrypted files
   */
  static FileEncryptionInfo getFileEncryptionInfo(final FSDirectory fsd, final INodesInPath iip)
      throws IOException {
    ...
    try {
      ...
      if (encryptionZone == null) {
        ...
        if (NameNode.LOG.isDebugEnabled()) {
          NameNode.LOG.debug(
              "Encryption zone " + encryptionZone.getPath() + " does not have a valid path.");
        }
      }

      XAttr fileXAttr =
          FSDirXAttrOp.unprotectedGetXAttrByPrefixedName(
              iip.getLastINode(), iip.getPathSnapshotId(), CRYPTO_XATTR_FILE_ENCRYPTION_INFO);
      if (fileXAttr == null) {
        NameNode.LOG.warn(
            "Could not find encryption XAttr for file "
                + iip.getPath()
                + " in encryption zone "
      ...
      try {
        HdfsProtos.PerFileEncryptionInfoProto fileProto =
            HdfsProtos.PerFileEncryptionInfoProto.parseFrom(fileXAttr.getValue());
  ...
  /**
   * If the file and encryption key are valid, return the encryption info, else throw a retry
   * exception. The startFile method generates the EDEK outside of the lock so the zone must be
   * reverified.
   *
   ...
   */
  static FileEncryptionInfo getFileEncryptionInfo(
      FSDirectory dir, INodesInPath iip, EncryptionKeyInfo ezInfo)
   ...
   * thread to warm them up.
   */
  static void warmUpEdekCache(
      final ExecutorService executor, final FSDirectory fsd, final int delay, final int interval) {
    fsd.readLock();
    try {
  ...
  /**
   * EDEKCacheLoader is being run in a separate thread to loop through all the EDEKs and warm them
   * up in the KMS cache.
   */
  static class EDEKCacheLoader implements Runnable {
    ...
    public void run() {
      NameNode.LOG.info(
          "Warming up {} EDEKs... (initialDelay={}, " + "retryInterval={})",
          keyNames.length,
          initialDelay,
          retryInterval);
      try {
  ...
  /**
   * If the file is in an encryption zone, we optimistically create an EDEK for the file by calling
   * out to the configured KeyProvider. Since this typically involves doing an RPC, the fsn lock is
   * yielded.
   *
   * <p>Since the path can flip-flop between being in an encryption zone and not in the meantime,
   * the call MUST re-resolve the IIP and re-check preconditions if this method does not return
   * null;
   *
   ...
   */
  static EncryptionKeyInfo getEncryptionKeyInfo(
      FSNamesystem fsn, INodesInPath iip, CryptoProtocolVersion[] supportedVersions)
      throws IOException {
    ...
    Preconditions.checkArgument(
        !suite.equals(CipherSuite.UNKNOWN), "Chose an UNKNOWN CipherSuite!");
    ...
    try {
      ...
      return new EncryptionKeyInfo(
          protocolVersion, suite, ezKeyName, generateEncryptedDataEncryptionKey(fsd, ezKeyName));
    } finally {
      fsn.writeLock();
  ...
  /**
   * Get the current key version name for the given EZ. This will first drain the provider's local
   * cache, then generate a new edek.
   *
   * <p>The encryption key version of the newly generated edek will be used as the target key
   * version of this re-encryption - meaning all edeks' keyVersion are compared with it, and only
   * sent to the KMS for re-encryption when the version is different.
   *
   * <p>Note: KeyProvider has a getCurrentKey interface, but that is under a different ACL. HDFS
   * should not try to operate on additional ACLs, but rather use the generate ACL it already has.
   */
  static String getCurrentKeyVersion(
      final FSDirectory dir, final FSPermissionChecker pc, final String zone) throws IOException {
    assert dir.getProvider() != null;
    assert !dir.hasReadLock();
    final String keyName = FSDirEncryptionZoneOp.getKeyNameForZone(dir, pc, zone);
    if (keyName == null) {
  ...
  /**
   * Resolve the zone to an inode, find the encryption zone info associated with that inode, and
   * return the key name. Does not contact the KMS.
   */
  static String getKeyNameForZone(
      final FSDirectory dir, final FSPermissionChecker pc, final String zone) throws IOException {
    assert dir.getProvider() != null;
    final INodesInPath iip;
