...
public class JournalNodeRpcServer implements QJournalProtocol, InterQJournalProtocol {
  ...
  JournalNodeRpcServer(Configuration conf, JournalNode jn) throws IOException {
    ...
    InterQJournalProtocolServerSideTranslatorPB qJournalProtocolServerSideTranslatorPB =
        new InterQJournalProtocolServerSideTranslatorPB(this);

    BlockingService interQJournalProtocolService =
        InterQJournalProtocolService.newReflectiveBlockingService(
            qJournalProtocolServerSideTranslatorPB);

    DFSUtil.addPBProtocol(
        confCopy, InterQJournalProtocolPB.class, interQJournalProtocolService, server);
  ...
  public GetJournalStateResponseProto getJournalState(String journalId, String nameServiceId)
      throws IOException {
    long epoch = jn.getOrCreateJournal(journalId, nameServiceId).getLastPromisedEpoch();
  ...
  public NewEpochResponseProto newEpoch(
    ...
    return jn.getOrCreateJournal(journalId, nameServiceId).newEpoch(nsInfo, epoch);
  }

  @Override
  @SuppressWarnings(
      "objectconstruction:reset.not.owning") // FP CreatesMustCallFor should permit the call
                                             // (checker bug): this error is issued at the call to
                                             // format() below with or without @CreatesMustCallFor
                                             // on this method(validated)
  ...
  public void heartbeat(RequestInfo reqInfo) throws IOException {
    jn.getOrCreateJournal(reqInfo.getJournalId(), reqInfo.getNameServiceId()).heartbeat(reqInfo);
  }
  ...
  public GetEditLogManifestResponseProto getEditLogManifest(
    ...
    RemoteEditLogManifest manifest =
        jn.getOrCreateJournal(jid, nameServiceId).getEditLogManifest(sinceTxId, inProgressOk);
  ...
  public GetJournaledEditsResponseProto getJournaledEdits(
    ...
    return jn.getOrCreateJournal(jid, nameServiceId).getJournaledEdits(sinceTxId, maxTxns);
  }

  @Override
  public PrepareRecoveryResponseProto prepareRecovery(RequestInfo reqInfo, long segmentTxId)
      throws IOException {
    return jn.getOrCreateJournal(reqInfo.getJournalId(), reqInfo.getNameServiceId())
        .prepareRecovery(reqInfo, segmentTxId);
  }

  @Override
  public void acceptRecovery(RequestInfo reqInfo, SegmentStateProto log, URL fromUrl)
      throws IOException {
    jn.getOrCreateJournal(reqInfo.getJournalId(), reqInfo.getNameServiceId())
  ...
  public Boolean canRollBack(
      ...
      throws IOException {
    return jn.canRollBack(journalId, storage, prevStorage, targetLayoutVersion, nameServiceId);
  }
