...
/**
 * IndentedImageVisitor walks over an FSImage and displays its structure using indenting to organize
 * sections within the image file.
 */
class IndentedImageVisitor extends TextWriterImageVisitor {
  ...
  private final DepthCounter dc = new DepthCounter(); // to track leading spacing
  ...
  /**
   * Print an appropriate number of spaces for the current level. FsImages can potentially be
   * millions of lines long, so caching can significantly speed up output.
   */
  ...
  private void printIndents() throws IOException {
    try {
      ...
      for (int i = 0; i < dc.getLevel(); i++) write(" ");
    }
  }
}
