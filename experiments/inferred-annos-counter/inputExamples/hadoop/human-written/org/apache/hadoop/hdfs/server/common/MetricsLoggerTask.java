...
public class MetricsLoggerTask implements Runnable {
      ...
      try {
        ...
        final AttributeList attributes =
            server.getAttributes(
                mbeanName, attributeNames.toArray(new String[attributeNames.size()]));
        ...
        metricsLog.error(
            "Failed to get " + nodeName + " metrics for mbean " + mbeanName.toString(), e);
      }
    }
  ...
  private String trimLine(String valueStr) {
    ...
    return (valueStr.length() < maxLogLineLength
        ? valueStr
        : valueStr.substring(0, maxLogLineLength) + "...");
  }
  ...
  /** Get the list of attributes for the MBean, filtering out a few attribute types. */
