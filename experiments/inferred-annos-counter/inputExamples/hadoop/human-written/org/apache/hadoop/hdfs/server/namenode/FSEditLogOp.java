...
/**
 * Helper classes for reading the ops from an InputStream. All ops derive from FSEditLogOp and are
 * only instantiated from Reader#readOp()
 */
...
public abstract class FSEditLogOp {
  ...
  public static class OpInstanceCache {
    ...
    static final class OpInstanceCacheMap extends EnumMap<FSEditLogOpCodes, FSEditLogOp> {
      OpInstanceCacheMap() {
  ...
  private static ImmutableMap<String, FsAction> fsActionMap() {
    ...
    for (FsAction v : FsAction.values()) b.put(v.SYMBOL, v);
    return b.build();
  }
  ...
  /**
   * Constructor for an EditLog Op. EditLog ops cannot be constructed directly, but only through
   * Reader#readOp.
   */
  @VisibleForTesting
  ...
  private static void writeRpcIds(final byte[] clientId, final int callId, DataOutputStream out)
      throws IOException {
  ...
  void readRpcIdsFromXml(Stanza st) {
    this.rpcClientId =
        st.hasChildren("RPC_CLIENTID")
            ? ClientId.toBytes(st.getValue("RPC_CLIENTID"))
            : RpcConstants.DUMMY_CLIENT_ID;
    this.rpcCallId =
        st.hasChildren("RPC_CALLID")
            ? Integer.parseInt(st.getValue("RPC_CALLID"))
            : RpcConstants.INVALID_CALL_ID;
  }

  private static void appendRpcIdsToString(
      final StringBuilder builder, final byte[] clientId, final int callId) {
    builder
        .append(", RpcClientId=")
  ...
  private static void appendRpcIdsToXml(
      ContentHandler contentHandler, final byte[] clientId, final int callId) throws SAXException {
  ...
  private static final class AclEditLogUtil {
      ...
      for (int i = 0; i < size; ++i) {
        ...
        aclEntries.add(
            new AclEntry.Builder()
                ...
                .setType(ACL_ENTRY_TYPE_VALUES[t])
                .build());
      }
    ...
    private static void write(List<AclEntry> aclEntries, DataOutputStream out) throws IOException {
      ...
      for (AclEntry e : aclEntries) {
        ...
        int v =
            (e.getScope().ordinal() << ACL_EDITLOG_ENTRY_SCOPE_OFFSET)
  ...
  private static List<XAttr> readXAttrsFromEditLog(DataInputStream in, int logVersion)
      throws IOException {
  ...
  @SuppressWarnings("unchecked")
  abstract static class AddCloseOp extends FSEditLogOp implements BlockListUpdatingOp {
    int length;
    long inodeId;
    String path;
    short replication;
    ...
    void readFields(DataInputStream in, int logVersion) throws IOException {
      ...
      if (this.opCode == OP_ADD) {
        ...
        if (NameNodeLayoutVersion.supports(
          ...
          this.erasureCodingPolicyId = ErasureCodeConstants.REPLICATION_POLICY_ID;
        }
    ...
    public static final int MAX_BLOCKS = 1024 * 1024 * 64;
      ...
      if (numBlocks < 0) {
        ...
        throw new IOException(
            "invalid number of blocks: "
                + numBlocks
                + ".  The maximum number of blocks per file is "
                + MAX_BLOCKS);
      }
    ...
    protected void toXml(ContentHandler contentHandler) throws SAXException {
      ...
      XMLUtils.addSaxString(contentHandler, "REPLICATION", Short.toString(replication));
      ...
      if (this.opCode == OP_ADD) {
        ...
        XMLUtils.addSaxString(
            contentHandler, "ERASURE_CODING_POLICY_ID", Byte.toString(erasureCodingPolicyId));
  ...
  /** {@literal @AtMostOnce} for {@link ClientProtocol#create} and {@link ClientProtocol#append} */
  static class AddOp extends AddCloseOp {
    ...
    public String toString() {
      StringBuilder builder = new StringBuilder();
      builder.append("AddOp ").append(stringifyMembers());
      return builder.toString();
    }
  }

  /**
   * Although {@link ClientProtocol#append} may also log a close op, we do not need to record the
   * rpc ids here since a successful appendFile op will finally log an AddOp.
   */
  static class CloseOp extends AddCloseOp {
    ...
    public String toString() {
      StringBuilder builder = new StringBuilder();
      builder.append("CloseOp ").append(stringifyMembers());
      return builder.toString();
    }
  }
  ...
  /**
   * {@literal @AtMostOnce} for {@link ClientProtocol#updatePipeline}, but {@literal @Idempotent}
   * for some other ops.
   */
  static class UpdateBlocksOp extends FSEditLogOp implements BlockListUpdatingOp {
    ...
    void fromXml(Stanza st) throws InvalidXmlException {
      this.path = st.getValue("PATH");
      List<Stanza> blocks =
          st.hasChildren("BLOCK") ? st.getChildren("BLOCK") : new ArrayList<Stanza>();
      this.blocks = new Block[blocks.size()];
      for (int i = 0; i < blocks.size(); i++) {
        this.blocks[i] = FSEditLogOp.blockFromXml(blocks.get(i));
      }
  ...
  static class ConcatDeleteOp extends FSEditLogOp {
    ...
    String[] srcs;
    long timestamp;
    public static final int MAX_CONCAT_SRC = 1024 * 1024;
    ...
    ConcatDeleteOp setSources(String[] srcs) {
      if (srcs.length > MAX_CONCAT_SRC) {
        throw new RuntimeException(
            "ConcatDeleteOp can only have " + MAX_CONCAT_SRC + " sources at most.");
      }
      this.srcs = srcs;
    ...
    void readFields(DataInputStream in, int logVersion) throws IOException {
      ...
      if (srcSize < 0) {
        ...
        throw new IOException(
            "Incorrect data format. "
                + "ConcatDeleteOp can have at most "
                + MAX_CONCAT_SRC
                + " sources, but we tried to have "
                + (length - 3)
                + " sources.");
      }
    ...
    public String toString() {
      ...
      builder
          ...
          .append(timestamp);
      appendRpcIdsToString(builder, rpcClientId, rpcCallId);
      builder.append(", opCode=").append(opCode).append(", txid=").append(txid).append("]");
      return builder.toString();
    }
    ...
    void fromXml(Stanza st) throws InvalidXmlException {
      ...
      int i = 0;
      while (true) {
        if (!sources.get(0).hasChildren("SOURCE" + (i + 1))) break;
        i++;
      }
      srcs = new String[i];
   ...
   * ClientProtocol#updateBlockForPipeline}, {@link ClientProtocol#recoverLease}, {@link
   * ClientProtocol#addBlock}) or already bound with other editlog op which records rpc ids ( {@link
   * ClientProtocol#create}). Thus no need to record rpc ids here.
   */
  ...
  /**
   * This operation does not actually update gen stamp immediately, the new gen stamp is recorded as
   * impending gen stamp. The global generation stamp on Standby Node is updated when the block with
   * the next generation stamp is actually received. We keep logging this operation for backward
   * compatibility. The impending gen stamp will take effect when the standby transition to become
   * an active.
   */
  static class SetGenstampV2Op extends FSEditLogOp {
    ...
    public String toString() {
      StringBuilder builder = new StringBuilder();
      builder
          .append("SetGenstampV2Op [GenStampV2=")
          .append(genStampV2)
          .append(", opCode=")
          .append(opCode)
          .append(", txid=")
  ...
  static class AllocateBlockIdOp extends FSEditLogOp {
    ...
    public String toString() {
      StringBuilder builder = new StringBuilder();
      builder
          .append("AllocateBlockIdOp [blockId=")
          .append(blockId)
          .append(", opCode=")
          .append(opCode)
          .append(", txid=")
  ...
  static class SetPermissionsOp extends FSEditLogOp {
    ...
    protected void toXml(ContentHandler contentHandler) throws SAXException {
      XMLUtils.addSaxString(contentHandler, "SRC", src);
      XMLUtils.addSaxString(contentHandler, "MODE", Short.toString(permissions.toShort()));
    }
  ...
  static class TruncateOp extends FSEditLogOp {
    ...
    protected void toXml(ContentHandler contentHandler) throws SAXException {
      ...
      XMLUtils.addSaxString(contentHandler, "TIMESTAMP", Long.toString(timestamp));
      if (truncateBlock != null) FSEditLogOp.blockToXml(contentHandler, truncateBlock);
    }
    ...
    void fromXml(Stanza st) throws InvalidXmlException {
      ...
      this.timestamp = Long.parseLong(st.getValue("TIMESTAMP"));
      if (st.hasChildren("BLOCK")) this.truncateBlock = FSEditLogOp.blockFromXml(st);
    }
  ...
  /**
   * {@literal @Idempotent} for {@link ClientProtocol#recoverLease}. In the meanwhile, startFile and
   * appendFile both have their own corresponding editlog op.
   */
  static class ReassignLeaseOp extends FSEditLogOp {
    ...
    public String toString() {
      StringBuilder builder = new StringBuilder();
      builder
          .append("ReassignLeaseOp [leaseHolder=")
          .append(leaseHolder)
          .append(", path=")
          .append(path)
          .append(", newHolder=")
  ...
  static class GetDelegationTokenOp extends FSEditLogOp {
    ...
    public String toString() {
      StringBuilder builder = new StringBuilder();
      builder
          .append("GetDelegationTokenOp [token=")
          .append(token)
          .append(", expiryTime=")
          .append(expiryTime)
          .append(", opCode=")
  ...
  static class RenewDelegationTokenOp extends FSEditLogOp {
    ...
    public String toString() {
      StringBuilder builder = new StringBuilder();
      builder
          .append("RenewDelegationTokenOp [token=")
          .append(token)
          .append(", expiryTime=")
          .append(expiryTime)
          .append(", opCode=")
  ...
  static class CancelDelegationTokenOp extends FSEditLogOp {
    ...
    public String toString() {
      StringBuilder builder = new StringBuilder();
      builder
          .append("CancelDelegationTokenOp [token=")
          .append(token)
          .append(", opCode=")
          .append(opCode)
          .append(", txid=")
  ...
  /**
   * Operation corresponding to creating a snapshot. {@literal @AtMostOnce} for {@link
   * ClientProtocol#createSnapshot}.
   */
  static class CreateSnapshotOp extends FSEditLogOp {
    ...
    public String toString() {
      StringBuilder builder = new StringBuilder();
      builder
          .append("CreateSnapshotOp [snapshotRoot=")
          .append(snapshotRoot)
          .append(", snapshotName=")
          .append(snapshotName)
          .append(", mtime=")
  ...
  /**
   * Operation corresponding to delete a snapshot. {@literal @AtMostOnce} for {@link
   * ClientProtocol#deleteSnapshot}.
   */
  static class DeleteSnapshotOp extends FSEditLogOp {
    ...
    public String toString() {
      StringBuilder builder = new StringBuilder();
      builder
          .append("DeleteSnapshotOp [snapshotRoot=")
          .append(snapshotRoot)
          .append(", snapshotName=")
          .append(snapshotName)
          .append(", mtime=")
  ...
  /**
   * Operation corresponding to rename a snapshot. {@literal @AtMostOnce} for {@link
   * ClientProtocol#renameSnapshot}.
   */
  static class RenameSnapshotOp extends FSEditLogOp {
    ...
    public String toString() {
      StringBuilder builder = new StringBuilder();
      builder
          .append("RenameSnapshotOp [snapshotRoot=")
          .append(snapshotRoot)
  ...
  static class AllowSnapshotOp extends FSEditLogOp { // @Idempotent
    ...
    public String toString() {
      StringBuilder builder = new StringBuilder();
      builder.append("AllowSnapshotOp [snapshotRoot=").append(snapshotRoot).append("]");
      return builder.toString();
    }
  }
  ...
  static class DisallowSnapshotOp extends FSEditLogOp { // @Idempotent
    ...
    public String toString() {
      StringBuilder builder = new StringBuilder();
      builder.append("DisallowSnapshotOp [snapshotRoot=").append(snapshotRoot).append("]");
      return builder.toString();
    }
  }
  ...
  static class ModifyCacheDirectiveInfoOp extends FSEditLogOp {
    ...
    public String toString() {
      StringBuilder builder = new StringBuilder();
      builder.append("ModifyCacheDirectiveInfoOp[").append("id=").append(directive.getId());
  ...
  static class RemoveCacheDirectiveInfoOp extends FSEditLogOp {
    ...
    public String toString() {
      StringBuilder builder = new StringBuilder();
      builder.append("RemoveCacheDirectiveInfo [").append("id=" + Long.toString(id));
      appendRpcIdsToString(builder, rpcClientId, rpcCallId);
      builder.append("]");
      return builder.toString();
    }
  ...
  /**
   * A class to read in blocks stored in the old format. The only two fields in the block were
   * blockid and length.
   */
  ...
  static class AddErasureCodingPolicyOp extends FSEditLogOp {
    ...
    static AddErasureCodingPolicyOp getInstance(OpInstanceCache cache) {
      return (AddErasureCodingPolicyOp) cache.get(OP_ADD_ERASURE_CODING_POLICY);
    }
    ...
    protected void toXml(ContentHandler contentHandler) throws SAXException {
      ...
      XMLUtils.addSaxString(
          contentHandler, "DATAUNITS", Integer.toString(ecPolicy.getNumDataUnits()));
      XMLUtils.addSaxString(
          contentHandler, "PARITYUNITS", Integer.toString(ecPolicy.getNumParityUnits()));
      XMLUtils.addSaxString(contentHandler, "CELLSIZE", Integer.toString(ecPolicy.getCellSize()));
    ...
    public String toString() {
      StringBuilder builder = new StringBuilder();
      builder.append("AddErasureCodingPolicy [").append(ecPolicy.toString());
  ...
  static class EnableErasureCodingPolicyOp extends FSEditLogOp {
    ...
    static EnableErasureCodingPolicyOp getInstance(OpInstanceCache cache) {
      return (EnableErasureCodingPolicyOp) cache.get(OP_ENABLE_ERASURE_CODING_POLICY);
    }
    ...
    public String toString() {
      StringBuilder builder = new StringBuilder();
      builder.append("EnableErasureCodingPolicy [").append(ecPolicyName);
  ...
  static class DisableErasureCodingPolicyOp extends FSEditLogOp {
    ...
    static DisableErasureCodingPolicyOp getInstance(OpInstanceCache cache) {
      return (DisableErasureCodingPolicyOp) cache.get(OP_DISABLE_ERASURE_CODING_POLICY);
    }
    ...
    public String toString() {
      StringBuilder builder = new StringBuilder();
      builder.append("DisableErasureCodingPolicy [").append(ecPolicyName);
  ...
  static class RemoveErasureCodingPolicyOp extends FSEditLogOp {
    ...
    static RemoveErasureCodingPolicyOp getInstance(OpInstanceCache cache) {
      return (RemoveErasureCodingPolicyOp) cache.get(OP_REMOVE_ERASURE_CODING_POLICY);
    }
    ...
    public String toString() {
      StringBuilder builder = new StringBuilder();
      builder.append("RemoveErasureCodingPolicy [").append(ecPolicyName);
  ...
  static class SetStoragePolicyOp extends FSEditLogOp {
    ...
    public String toString() {
      StringBuilder builder = new StringBuilder();
      builder
          .append("SetStoragePolicyOp [path=")
          .append(path)
          .append(", policyId=")
          .append(policyId)
          .append(", opCode=")
    ...
    protected void toXml(ContentHandler contentHandler) throws SAXException {
      XMLUtils.addSaxString(contentHandler, "PATH", path);
      XMLUtils.addSaxString(contentHandler, "POLICYID", Byte.toString(policyId));
    }
  ...
  public abstract static class Reader {
     ...
     *
     * <p>Note that the objects returned from this method may be re-used by future calls to the same
     * method.
     *
     * @param skipBrokenEdits If true, attempt to skip over damaged parts of the input stream,
     *     rather than throwing an IOException
     * @return the operation read from the stream, or null at the end of the file
     * @throws IOException on error. This function should only throw an exception when
     *     skipBrokenEdits is false.
     */
    ...
    void verifyTerminator() throws IOException {
      /**
       * The end of the edit log should contain only 0x00 or 0xff bytes. If it contains other bytes,
       * the log itself may be corrupt. It is important to check this; if we don't, a stray
       * OP_INVALID byte could make us stop reading the edit log halfway through, and we'd never
     ...
     *     <p>If an exception is thrown, the stream's mark will be set to the first problematic
     *     byte. This usually means the beginning of the opcode.
     */
    public abstract FSEditLogOp decodeOp() throws IOException;

    /**
     * Similar to decodeOp(), but we only retrieve the transaction ID of the Op rather than reading
     * it. If the edit log format supports length prefixing, this can be much faster than full
     * decoding.
     *
  ...
  /**
   * Reads edit logs which are prefixed with a length. These edit logs also include a checksum and
   * transaction ID.
   */
  private static class LengthPrefixedReader extends Reader {
     ...
     *
     * <p>The minimum Op has: 1-byte opcode 4-byte length 8-byte txid 0-byte body 4-byte checksum
     */
     ...
     * The op id length.
     *
     * <p>Not included in the stored length.
     */
    ...
    /**
     * Decode the opcode "frame". This includes reading the opcode and transaction ID, and
     * validating the checksum and length. It does not include reading the opcode-specific fields.
     * The input stream will be advanced to the end of the op at the end of this function.
     *
     * @return An op with the txid set, but none of the other fields filled in, or null if we hit
     *     EOF.
     */
    private long decodeOpFrame() throws IOException {
      ...
      if (opLength > maxOpSize) {
        throw new IOException(
            "Op " + (int) opCodeByte + " has size " + opLength + ", but maxOpSize = " + maxOpSize);
  ...
  /** Read edit logs which have a checksum and a transaction ID, but not a length. */
  private static class ChecksummedReader extends Reader {
    private final Checksum checksum;

    ChecksummedReader(
        Checksum checksum, DataInputStream in, StreamLimiter limiter, int logVersion) {
  ...
  /**
   * Read older edit logs which may or may not have transaction IDs and other features. This code is
   * used during upgrades and to allow HDFS INotify to read older edit log files.
   */
  ...
  public static void delegationTokenToXml(
      ContentHandler contentHandler, DelegationTokenIdentifier token) throws SAXException {
    ...
    XMLUtils.addSaxString(
        contentHandler, "SEQUENCE_NUMBER", Integer.toString(token.getSequenceNumber()));
    ...
    XMLUtils.addSaxString(
        contentHandler, "MASTER_KEY_ID", Integer.toString(token.getMasterKeyId()));
  ...
  public static DelegationTokenIdentifier delegationTokenFromXml(Stanza st)
    ...
    DelegationTokenIdentifier token =
        new DelegationTokenIdentifier(new Text(owner), new Text(renewer), new Text(realuser));
  ...
  public static void delegationKeyToXml(ContentHandler contentHandler, DelegationKey key)
      throws SAXException {
  ...
  public static void fsPermissionToXml(ContentHandler contentHandler, FsPermission mode)
      throws SAXException {
  ...
  private static FsAction fsActionFromXml(Stanza st) throws InvalidXmlException {
    ...
    if (v == null) throw new InvalidXmlException("Invalid value for FsAction");
    return v;
  }

  private static void appendAclEntriesToXml(
      ContentHandler contentHandler, List<AclEntry> aclEntries) throws SAXException {
    for (AclEntry e : aclEntries) {
    ...
    for (Stanza s : stanzas) {
      AclEntry e =
          new AclEntry.Builder()
              ...
              .setPermission(fsActionFromXml(s))
              .build();
      aclEntries.add(e);
    }
    return aclEntries;
  ...
  private static void appendXAttrsToXml(ContentHandler contentHandler, List<XAttr> xAttrs)
      throws SAXException {
    for (XAttr xAttr : xAttrs) {
      ...
      if (xAttr.getValue() != null) {
        try {
          XMLUtils.addSaxString(
              contentHandler, "VALUE", XAttrCodec.encodeValue(xAttr.getValue(), XAttrCodec.HEX));
        } catch (IOException e) {
  ...
  private static List<XAttr> readXAttrsFromXml(Stanza st) throws InvalidXmlException {
    ...
    for (Stanza a : stanzas) {
      ...
      builder
          .setNameSpace(XAttr.NameSpace.valueOf(a.getValue("NAMESPACE")))
