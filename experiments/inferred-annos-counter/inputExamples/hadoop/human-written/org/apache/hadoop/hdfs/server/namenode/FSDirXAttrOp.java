...
class FSDirXAttrOp {
  ...
  static List<XAttr> getXAttrs(
      FSDirectory fsd, FSPermissionChecker pc, final String srcArg, List<XAttr> xAttrs)
      throws IOException {
    String src = srcArg;
    checkXAttrsConfigFlag(fsd);
  ...
  static List<XAttr> listXAttrs(FSDirectory fsd, FSPermissionChecker pc, String src)
      throws IOException {
  ...
  static FileStatus removeXAttr(
      FSDirectory fsd, FSPermissionChecker pc, String src, XAttr xAttr, boolean logRetryCache)
      throws IOException {
    FSDirXAttrOp.checkXAttrsConfigFlag(fsd);
    XAttrPermissionFilter.checkPermissionForApi(pc, xAttr, FSDirectory.isReservedRawName(src));
  ...
  static List<XAttr> unprotectedRemoveXAttrs(
      FSDirectory fsd, final INodesInPath iip, final List<XAttr> toRemove) throws IOException {
  ...
  /**
   * Filter XAttrs from a list of existing XAttrs. Removes matched XAttrs from toFilter and puts
   * them into filtered. Upon completion, toFilter contains the filter XAttrs that were not found,
   * while fitleredXAttrs contains the XAttrs that were found.
   *
  ...
  static List<XAttr> filterINodeXAttrs(
      final List<XAttr> existingXAttrs, final List<XAttr> toFilter, final List<XAttr> filtered)
      throws AccessControlException {
    if (existingXAttrs == null
        || existingXAttrs.isEmpty()
        || toFilter == null
        || toFilter.isEmpty()) {
      ...
      for (ListIterator<XAttr> it = toFilter.listIterator(); it.hasNext(); ) {
        ...
        if (UNREADABLE_BY_SUPERUSER_XATTR.equalsIgnoreValue(filter)) {
          throw new AccessControlException(
              "The xattr '" + SECURITY_XATTR_UNREADABLE_BY_SUPERUSER + "' can not be deleted.");
        }
    ...
    for (XAttr xattr : newXAttrs) {
      ...
      if (CRYPTO_XATTR_FILE_ENCRYPTION_INFO.equals(xaName)) {
        HdfsProtos.PerFileEncryptionInfoProto fileProto =
            HdfsProtos.PerFileEncryptionInfoProto.parseFrom(xattr.getValue());
        String keyVersionName = fileProto.getEzKeyVersionName();
        String zoneKeyName = fsd.ezManager.getKeyName(iip);
        if (zoneKeyName == null) {
          throw new IOException(
              "Cannot add raw feInfo XAttr to a file in a " + "non-encryption zone");
        }

        if (!KeyProviderCryptoExtension.getBaseName(keyVersionName).equals(zoneKeyName)) {
          throw new IllegalArgumentException(
              String.format(
                  "KeyVersion '%s' does not belong to the key '%s'", keyVersionName, zoneKeyName));
        }
      }
      ...
      if (CRYPTO_XATTR_ENCRYPTION_ZONE.equals(xaName)) {
        ...
        fsd.ezManager.addEncryptionZone(
            inode.getId(),
      ...
      if (!isFile && SECURITY_XATTR_UNREADABLE_BY_SUPERUSER.equals(xaName)) {
        throw new IOException(
            "Can only set '" + SECURITY_XATTR_UNREADABLE_BY_SUPERUSER + "' on a file.");
      }
    }
  ...
  static List<XAttr> setINodeXAttrs(
      FSDirectory fsd,
      final List<XAttr> existingXAttrs,
    ...
    if (userVisibleXAttrsNum > fsd.getInodeXAttrsLimit()) {
      throw new IOException(
          "Cannot add additional XAttr to inode, "
  ...
  static XAttr getXAttrByPrefixedName(FSDirectory fsd, INodesInPath iip, String prefixedName)
      throws IOException {
    fsd.readLock();
    try {
      return XAttrStorage.readINodeXAttrByPrefixedName(
          iip.getLastINode(), iip.getPathSnapshotId(), prefixedName);
    } finally {
      fsd.readUnlock();
    }
  }

  static XAttr unprotectedGetXAttrByPrefixedName(INode inode, int snapshotId, String prefixedName)
      throws IOException {
    return XAttrStorage.readINodeXAttrByPrefixedName(inode, snapshotId, prefixedName);
  }

  private static void checkXAttrChangeAccess(
      FSDirectory fsd, INodesInPath iip, XAttr xAttr, FSPermissionChecker pc)
  ...
  /**
   * Verifies that the combined size of the name and value of an xattr is within the configured
   * limit. Setting a limit of zero disables this check.
   */
