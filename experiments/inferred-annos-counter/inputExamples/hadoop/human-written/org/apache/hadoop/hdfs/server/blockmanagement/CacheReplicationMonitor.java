 ...
 *
 * <p>The CacheReplicationMonitor does a full scan when the NameNode first starts up, and at
 * configurable intervals afterwards.
 */
...
public class CacheReplicationMonitor extends Thread implements Closeable {
  ...
  /**
   * The CacheReplicationMonitor (CRM) lock. Used to synchronize starting and waiting for rescan
   * operations.
   */
  ...
  /**
   * The number of rescans completed. Used to wait for scans to finish. Protected by the
   * CacheReplicationMonitor lock.
   */
  private long completedScanCount = 0;

  /**
   * The scan we're currently performing, or -1 if no scan is in progress. Protected by the
   * CacheReplicationMonitor lock.
   */
  ...
  public CacheReplicationMonitor(
      FSNamesystem namesystem, CacheManager cacheManager, long intervalMs, ReentrantLock lock) {
  ...
  public void run() {
    ...
    Thread.currentThread()
        .setName("CacheReplicationMonitor(" + System.identityHashCode(this) + ")");
  ...
  /**
   * Waits for a rescan to complete. This doesn't guarantee consistency with pending operations,
   * only relative recency, since it will not force a new rescan if a rescan is already underway.
   *
   * <p>Note that this call will release the FSN lock, so operations before and after are not
   * atomic.
   */
  public void waitForRescanIfNeeded() {
    Preconditions.checkArgument(
        !namesystem.hasWriteLock(), "Must not hold the FSN write lock when waiting for a rescan.");
    Preconditions.checkArgument(
        lock.isHeldByCurrentThread(), "Must hold the CRM lock when waiting for a rescan.");
  ...
  /**
   * Indicates to the CacheReplicationMonitor that there have been CacheManager changes that require
   * a rescan.
   */
  public void setNeedsRescan() {
    Preconditions.checkArgument(
        lock.isHeldByCurrentThread(), "Must hold the CRM lock when setting the needsRescan bit.");
    if (curScanCount >= 0) {
  ...
  /**
   * Scan all CacheDirectives. Use the information to figure out what cache replication factor each
   * block should have.
   */
  private void rescanCacheDirectives() {
    ...
    for (CacheDirective directive : cacheManager.getCacheDirectives()) {
      ...
      if (directive.getExpiryTime() > 0 && directive.getExpiryTime() <= now) {
        LOG.debug(
            "Directive {}: the directive expired at {} (now = {})",
            directive.getId(),
            directive.getExpiryTime(),
            now);
        continue;
      }
      ...
      if (node == null) {
        ...
        LOG.debug(
            "Directive {}: ignoring non-directive, non-file inode {} ", directive.getId(), node);
      }
    }
  }
  ...
  private void rescanFile(CacheDirective directive, INodeFile file) {
    ...
    if (pool.getBytesNeeded() > pool.getLimit()) {
      LOG.debug(
          "Directive {}: not scanning file {} because "
    ...
    for (BlockInfo blockInfo : blockInfos) {
      ...
      if (ocblock == null) {
        ...
        long cachedByBlock =
            Math.min(cachedOn.size(), directive.getReplication()) * blockInfo.getNumBytes();
  ...
  /**
   * Scan through the cached block map. Any blocks which are under-replicated should be assigned new
   * Datanodes. Blocks that are over-replicated should be removed from Datanodes.
   */
    ...
    for (DatanodeDescriptor dn : datanodes) {
      ...
      for (Iterator<CachedBlock> it = dn.getPendingCached().iterator(); it.hasNext(); ) {
        ...
        if (blockInfo == null) {
          ...
          LOG.debug(
              "Block {}: cannot be found in block manager and hence"
                  + " skipped from calculation for node {}.",
              cblock.getBlockId(),
              dn.getDatanodeUuid());
          continue;
        }
        if (blockInfo.getNumBytes() > remaining) {
          LOG.debug(
              "Block {}: removing from PENDING_CACHED for node {} "
    ...
    for (Iterator<CachedBlock> cbIter = cachedBlocks.iterator(); cbIter.hasNext(); ) {
      ...
      for (Iterator<DatanodeDescriptor> iter = pendingUncached.iterator(); iter.hasNext(); ) {
        ...
        if (!cblock.isInList(datanode.getCached())) {
          LOG.trace(
              "Block {}: removing from PENDING_UNCACHED for node {} "
      ...
      if ((neededCached == 0) && pendingUncached.isEmpty() && pendingCached.isEmpty()) {
        ...
        LOG.trace(
            "Block {}: removing from cachedBlocks, since neededCached "
   ...
   * @param pendingUncached A list of DataNodes that will soon uncache the block.
   */
  private void addNewPendingUncached(
    ...
    while (neededUncached > 0) {
      if (possibilities.isEmpty()) {
        LOG.warn(
            "Logic error: we're trying to uncache more replicas than "
                + "actually exist for "
                + cachedBlock);
        return;
      }
   ...
   * @param cached A list of DataNodes currently caching the block.
   * @param pendingCached A list of DataNodes that will soon cache the block.
   */
  private void addNewPendingCached(
      final int neededCached,
      CachedBlock cachedBlock,
      List<DatanodeDescriptor> cached,
    ...
    if (blockInfo == null) {
      LOG.debug(
          "Block {}: can't add new cached replicas,"
    ...
    for (int i = 0; i < numReplicas; i++) {
      ...
      if (pendingCapacity < blockInfo.getNumBytes()) {
        LOG.trace(
            "Block {}: DataNode {} is not a valid possibility "
    ...
    for (DatanodeDescriptor datanode : chosen) {
      LOG.trace(
          "Block {}: added to PENDING_CACHED on DataNode {}",
          blockInfo.getBlockId(),
          datanode.getDatanodeUuid());
    ...
    if (neededCached > chosen.size()) {
      LOG.debug(
          "Block {}: we only have {} of {} cached replicas."
              + " {} DataNodes have insufficient cache capacity.",
          blockInfo.getBlockId(),
          (cachedBlock.getReplication() - neededCached + chosen.size()),
          cachedBlock.getReplication(),
          outOfCapacity);
    }
  }

  /**
   * Chooses datanode locations for caching from a list of valid possibilities. Non-stale nodes are
   * chosen before stale nodes.
   *
  ...
  private static List<DatanodeDescriptor> chooseDatanodesForCaching(
      final List<DatanodeDescriptor> possibilities,
      final int neededCached,
  ...
  /**
   * Choose a single datanode from the provided list of possible targets, weighted by the percentage
   * of free space remaining on the node.
   *
