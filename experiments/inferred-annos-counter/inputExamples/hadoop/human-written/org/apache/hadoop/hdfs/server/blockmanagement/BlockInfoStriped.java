 ...
 *
 * <p>We still use a storage array to store DatanodeStorageInfo for each block in the block group.
 * For a (m+k) block group, the first (m+k) storage units are sorted and strictly mapped to the
 * corresponding block.
 *
 * <p>Normally each block belonging to group is stored in only one DataNode. However, it is possible
 * that some block is over-replicated. Thus the storage array's size can be larger than (m+k). Thus
 * currently we use an extra byte array to record the block index for each entry.
 */
@InterfaceAudience.Private
public class BlockInfoStriped extends BlockInfo {
  private final ErasureCodingPolicy ecPolicy;
  /**
   * Always the same size with storage. Record the block index for each entry TODO: actually this is
   * only necessary for over-replicated block. Thus can be further optimized to save memory usage.
   */
  ...
  /**
   * If the block is committed/completed and its length is less than a full stripe, it returns the
   * the number of actual data blocks. Otherwise it returns the number of data units specified by
  ...
  boolean addStorage(DatanodeStorageInfo storage, Block reportedBlock) {
    Preconditions.checkArgument(
        BlockIdManager.isStripedBlockID(reportedBlock.getBlockId()),
        "reportedBlock is not striped");
    Preconditions.checkArgument(
        BlockIdManager.convertToStripedID(reportedBlock.getBlockId()) == this.getBlockId(),
  ...
  /**
   * Identify the block stored in the given datanode storage. Note that the returned block has the
   * same block Id with the one seen/reported by the DataNode.
   */
  ...
  public long spaceConsumed() {
    ...
    return StripedBlockUtil.spaceConsumedByStripedBlock(
        getNumBytes(),
  ...
  /**
   * Striped blocks on Provided Storage is not supported. All blocks on Provided storage are assumed
   * to be "contiguous".
   */
  @Override
  boolean isProvided() {
