...
/**
 * A Class to track the block collection IDs (Inode's ID) for which physical storage movement needed
 * as per the Namespace and StorageReports from DN. It scan the pending directories for which
 * storage movement is required and schedule the block collection IDs for movement. It track the
 * info of scheduled items and remove the SPS xAttr from the file/Directory once movement is
 * success.
 */
...
public class BlockStorageMovementNeeded {
  ...
  /**
   * Map of startPath and number of child's. Number of child's indicate the number of files pending
   ...
   *
   * @param trackInfo - track info for satisfy the policy
   */
   ...
   * @param itemInfoList - List of child in the directory
   * @param scanCompleted -Indicates whether the start id directory has no more elements to scan.
   */
  @VisibleForTesting
   ...
   *
   * @param itemInfo - child in the directory
   * @param scanCompleted -Indicates whether the ItemInfo start id directory has no more elements to
   *     scan.
   */
  @VisibleForTesting
  ...
  private void updatePendingDirScanStats(
      long startPath, int numScannedFiles, boolean scanCompleted) {
  ...
  /**
   * Gets the satisfier files for which block storage movements check necessary and make the
  ...
  /**
   * Decrease the pending child count for directory once one file blocks moved successfully. Remove
   * the SPS xAttr if pending child count is zero.
   */
  public synchronized void removeItemTrackInfo(ItemInfo trackInfo, boolean isSuccess)
      throws IOException {
    if (trackInfo.isDir()) {
  ...
  /**
   * Clean all the movements in spsDirsToBeTraveresed/storageMovementNeeded and notify to clean up
   * required resources.
   */
  ...
  /**
   * Take dir tack ID from the spsDirsToBeTraveresed queue and collect child ID's to process for
   * satisfy the policy.
   */
  ...
  public static class DirPendingWorkInfo {
    ...
    /** Decrement the pending work count for directory one track info is completed. */
    public synchronized void decrementPendingWorkCount() {
      this.pendingWorkCount--;
    }

    /** Return true if all the pending work is done and directory fully scanned, otherwise false. */
