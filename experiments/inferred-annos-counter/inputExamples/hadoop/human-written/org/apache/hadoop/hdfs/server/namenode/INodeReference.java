 ...
 *
 * <p>This class and its subclasses are used to support multiple access paths. A file/directory may
 * have multiple access paths when it is stored in some snapshots and it is renamed/moved to other
 * locations.
 *
 * <p>For example, (1) Suppose we have /abc/foo, say the inode of foo is inode(id=1000,name=foo) (2)
 * create snapshot s0 for /abc (3) mv /abc/foo /xyz/bar, i.e. inode(id=1000,name=...) is renamed
 * from "foo" to "bar" and its parent becomes /xyz.
 *
 * <p>Then, /xyz/bar and /abc/.snapshot/s0/foo are two different access paths to the same inode,
 * inode(id=1000,name=bar).
 *
 * <p>With references, we have the following - /abc has a child ref(id=1001,name=foo). - /xyz has a
 * child ref(id=1002) - Both ref(id=1001,name=foo) and ref(id=1002) point to another reference,
 * ref(id=1003,count=2). - Finally, ref(id=1003,count=2) points to inode(id=1000,name=bar).
 *
 * <p>Note 1: For a reference without name, e.g. ref(id=1002), it uses the name of the referred
 * inode. Note 2: getParent() always returns the parent in the current state, e.g.
 * inode(id=1000,name=bar).getParent() returns /xyz but not /abc.
 */
public abstract class INodeReference extends INode {
  /**
   * Try to remove the given reference and then return the reference count. If the given inode is
   * not a reference, return -1;
   */
  ...
  /**
   * Remove the given reference and then return the reference count. If the referred inode is not a
   * WithCount, return -1;
   */
  ...
  /**
   * When destroying a reference node (WithName or DstReference), we call this method to identify
   * the snapshot which is the latest snapshot before the reference node's creation.
   */
  ...
  public void cleanSubtree(ReclaimContext reclaimContext, int snapshot, int prior) {
  ...
  public QuotaCounts computeQuotaUsage(
      ...
      boolean useCache,
      int lastSnapshotId) {
    return referred.computeQuotaUsage(bsps, blockStoragePolicyId, useCache, lastSnapshotId);
  }
  ...
  public static class WithCount extends INodeReference {
    ...
    /**
     * Compare snapshot with IDs, where null indicates the current status thus is greater than any
     * non-null snapshot.
     */
  ...
  public static class WithName extends INodeReference {
    ...
    /**
     * The id of the last snapshot in the src tree when this WithName node was generated. When
     * calculating the quota usage of the referred node, only the files/dirs existing when this
     * snapshot was taken will be counted for this WithName node and propagated along its ancestor
     * path.
     */
    ...
    public final ContentSummaryComputationContext computeContentSummary(
      ...
      Preconditions.checkState(
          snapshotId == Snapshot.CURRENT_STATE_ID || this.lastSnapshotId >= snapshotId);
      final INode referred = this.getReferredINode().asReference().getReferredINode();
      int id = snapshotId != Snapshot.CURRENT_STATE_ID ? snapshotId : this.lastSnapshotId;
    ...
    public final QuotaCounts computeQuotaUsage(
      ...
      // time of the rename.
      Preconditions.checkState(
          lastSnapshotId == Snapshot.CURRENT_STATE_ID || this.lastSnapshotId >= lastSnapshotId);
      final INode referred = this.getReferredINode().asReference().getReferredINode();
  ...
  public static class DstReference extends INodeReference {
    /**
     * Record the latest snapshot of the dst subtree before the rename. For later operations on the
     * moved/renamed files/directories, if the latest snapshot is after this dstSnapshot, changes
     * will be recorded to the latest snapshot. Otherwise changes will be recorded to the snapshot
     * belonging to the src of the rename.
     *
     * <p>{@link Snapshot#NO_SNAPSHOT_ID} means no dstSnapshot (e.g., src of the first-time rename).
     */
    ...
    /**
     * {@inheritDoc} <br>
     * To destroy a DstReference node, we first remove its link with the referred node. If the
     * reference number of the referred node is &lt;= 0, we destroy the subtree of the referred
     * node. Otherwise, we clean the referred node's subtree and delete everything created after the
     * last rename operation, i.e., everything outside of the scope of the prior WithName nodes.
     *
