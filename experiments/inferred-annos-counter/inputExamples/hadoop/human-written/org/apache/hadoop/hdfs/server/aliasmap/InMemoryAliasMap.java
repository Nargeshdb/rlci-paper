...
public class InMemoryAliasMap implements InMemoryAliasMapProtocol, Configurable {
  ...
  public static @Nonnull InMemoryAliasMap init(Configuration conf, String blockPoolID)
      throws IOException {
    ...
    if (!levelDBpath.exists()) {
      LOG.warn("InMemoryAliasMap location {} is missing. Creating it.", levelDBpath);
  ...
  public IterationResult list(Optional<Block> marker) throws IOException {
    try (DBIterator iterator = levelDb.iterator()) {
      Integer batchSize =
          conf.getInt(
              DFSConfigKeys.DFS_PROVIDED_ALIASMAP_INMEMORY_BATCH_SIZE,
  ...
  public @Nonnull Optional<ProvidedStorageLocation> read(@Nonnull Block block) throws IOException {
  ...
  public void write(@Nonnull Block block, @Nonnull ProvidedStorageLocation providedStorageLocation)
      throws IOException {
    byte[] extendedBlockDbFormat = toProtoBufBytes(block);
    byte[] providedStorageLocationDbFormat = toProtoBufBytes(providedStorageLocation);
  ...
  public static ProvidedStorageLocation fromProvidedStorageLocationBytes(
      @Nonnull byte[] providedStorageLocationDbFormat) throws InvalidProtocolBufferException {
    ProvidedStorageLocationProto providedStorageLocationProto =
        ProvidedStorageLocationProto.parseFrom(providedStorageLocationDbFormat);
  ...
  public static byte[] toProtoBufBytes(@Nonnull ProvidedStorageLocation providedStorageLocation)
      throws IOException {
    ProvidedStorageLocationProto providedStorageLocationProto =
  ...
  /**
   * Transfer this aliasmap for bootstrapping standby Namenodes. The map is transferred as a tar.gz
   * archive. This archive needs to be extracted on the standby Namenode.
   *
   ...
   * @throws IOException
   */
  public static void transferForBootstrap(
      HttpServletResponse response, Configuration conf, InMemoryAliasMap aliasMap)
      throws IOException {
    ...
    try {
      ...
      try (FileInputStream fis = new FileInputStream(compressedAliasMap)) {
        ...
        TransferFsImage.copyFileToStream(
            response.getOutputStream(), compressedAliasMap, fis, throttler);
      }
    } finally {
      ...
      if (aliasMapSnapshot != null && !FileUtil.fullyDelete(aliasMapSnapshot)) {
        errMessage
            .append("Failed to fully delete the aliasmap snapshot ")
            .append(aliasMapSnapshot.getAbsolutePath())
            .append("\n");
      }
  ...
  /**
   * Create a new LevelDB store which is a snapshot copy of the original aliasmap.
   *
  ...
  static File createSnapshot(InMemoryAliasMap aliasMap) throws IOException {
    ...
    try (Snapshot snapshot = originalDB.getSnapshot()) {
      ...
      try (DB snapshotDB = JniDBFactory.factory.open(newLevelDBDir, options)) {
        try (DBIterator iterator = originalDB.iterator(new ReadOptions().snapshot(snapshot))) {
          iterator.seekToFirst();
   ...
   */
  @SuppressWarnings(
      "builder:required.method.not.called") // TP: finish() throws IOException (validated)
  private static File getCompressedAliasMap(File aliasMapDir) throws IOException {
    ...
    try {
      bOut = new BufferedOutputStream(Files.newOutputStream(outCompressedFile.toPath()));
   ...
   * @throws IOException
   */
  private static void addFileToTarGzRecursively(
      TarArchiveOutputStream tOut, File file, String prefix, Configuration conf)
      throws IOException {
  ...
  /**
   * Extract the aliasmap archive to complete the bootstrap process. This method has to be called
   * after the aliasmap archive is transfered from the primary Namenode.
   *
  ...
  /**
   * CheckedFunction is akin to {@link java.util.function.Function} but specifies an IOException.
   *
