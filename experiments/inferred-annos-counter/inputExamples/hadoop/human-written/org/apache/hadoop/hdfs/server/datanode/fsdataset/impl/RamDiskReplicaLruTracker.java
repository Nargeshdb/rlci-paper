...
public class RamDiskReplicaLruTracker extends RamDiskReplicaTracker {
  ...
  private class RamDiskReplicaLru extends RamDiskReplica {
    ...
    private RamDiskReplicaLru(
        String bpid, long blockId, FsVolumeImpl ramDiskVolume, long lockedBytesReserved) {
  ...
  /**
   * Queue of replicas that need to be written to disk. Stale entries are GC'd by
  ...
  synchronized void addReplica(
    ...
    RamDiskReplicaLru ramDiskReplicaLru =
        new RamDiskReplicaLru(bpid, blockId, transientVolume, lockedBytesReserved);
  ...
  /**
   * Discard any state we are tracking for the given replica. This could mean the block is either
   * deleted from the block space or the replica is no longer on transient storage.
   *
   * @param deleteSavedCopies true if we should delete the saved copies on persistent storage. This
   *     should be set by the caller when the block is no longer needed.
   */
  @Override
  synchronized void discardReplica(
      final String bpid, final long blockId, boolean deleteSavedCopies) {
    Map<Long, RamDiskReplicaLru> map = replicaMaps.get(bpid);
