 ...
 *
 * <p>It uses the {@link FsDatasetImpl} object for synchronization.
 */
...
public class FsVolumeImpl implements FsVolumeSpi {
  ...
  /**
   * Per-volume worker pool that processes new blocks to cache. The maximum number of workers per
   * volume is bounded (configurable via dfs.datanode.fsdatasetcache.max.threads.per.volume) to
  ...
  FsVolumeImpl(
    ...
    if (this.usage != null) {
      reserved =
          new ReservedSpaceCalculator.Builder(conf)
              .setUsage(this.usage)
              .setStorageType(storageType)
              .build();
      boolean fixedSizeVolume =
          conf.getBoolean(
  ...
  private void printReferenceTraceInfo(String op) {
    ...
    FsDatasetImpl.LOG.trace(
        "Reference count: " + op + " " + this + ": " + this.reference.getReferenceCount());
    FsDatasetImpl.LOG.trace(Joiner.on("\n").join(Thread.currentThread().getStackTrace()));
  }

  /**
   * Increase the reference count. The caller must increase the reference count before issuing IOs.
   *
  ...
  boolean checkClosed() {
    if (this.reference.getReferenceCount() > 0) {
      FsDatasetImpl.LOG.debug(
          "The reference count for {} is {}, wait to be 0.", this, reference.getReferenceCount());
      return false;
    }
    return true;
  }
  ...
  /**
   * Return either the configured capacity of the file system if configured; or the capacity of the
   * file system excluding space reserved for non-HDFS.
   *
  ...
  /**
   * Calculate the available space of the filesystem, excluding space reserved for non-HDFS and
   * space reserved for RBW.
   *
   ...
   *
   * @return Disk usage excluding space used by HDFS and excluding space reserved for blocks open
   *     for write.
   * @throws IOException
   */
  ...
  private static class BlockIteratorState {
    ...
    @JsonProperty private long lastSavedMs;

    // The wall-clock ms since the epoch at which this iterator was created.
    @JsonProperty private long iterStartMs;

    @JsonProperty private String curFinalizedDir;

    @JsonProperty private String curFinalizedSubDir;

    @JsonProperty private String curEntry;

    @JsonProperty private boolean atEnd;
  }
  ...
  private class BlockIteratorImpl implements FsVolumeSpi.BlockIterator {
     ...
     *
     * @return The next subdirectory within the block pool slice, or null if there are no more.
     */
    private String getNextSubDir(String prev, File dir) throws IOException {
      List<String> children =
          fileIoProvider.listDirectory(FsVolumeImpl.this, dir, SubdirFilter.INSTANCE);
      cache = null;
      cacheMs = 0;
      if (children.isEmpty()) {
        LOG.trace(
    ...
    private String getNextFinalizedSubDir() throws IOException {
      ...
      File dir =
          Paths.get(bpidDir.getAbsolutePath(), "current", "finalized", state.curFinalizedDir)
              .toFile();
    ...
    private List<String> getSubdirEntries() throws IOException {
      ...
      File dir =
          Paths.get(
                  bpidDir.getAbsolutePath(),
                  "current",
                  "finalized",
                  state.curFinalizedDir,
                  state.curFinalizedSubDir)
              .toFile();
      List<String> entries =
          fileIoProvider.listDirectory(FsVolumeImpl.this, dir, BlockFileFilter.INSTANCE);
      if (entries.isEmpty()) {
        entries = null;
        LOG.trace(
     ...
     * Get the next block.
     *
     * <p>Each volume has a hierarchical structure.
     *
     * <p><code>
     * BPID B0
     *   finalized/
     ...
     *         ...
     *   rbw/
     * </code> When we run out of entries at one level of the structure, we search progressively
     * higher levels. For example, when we run out of blk_ entries in a subdirectory, we search for
     * the next subdirectory. And so on.
     */
    @Override
        ...
        while (true) {
          ...
          if (entries != null) {
            ...
            if (state.curEntry == null) {
              ...
              File actualBlockDir =
                  Paths.get(".", state.curFinalizedDir, state.curFinalizedSubDir).toFile();
              if (!expectedBlockDir.equals(actualBlockDir)) {
                LOG.error(
                    "nextBlock({}, {}): block id {} found in invalid "
                        + "directory.  Expected directory: {}.  "
                        + "Actual directory: {}",
                    storageID,
                    bpid,
                    block.getBlockId(),
                    expectedBlockDir.getPath(),
    ...
    private File getBlockFile(String bpid, ExtendedBlock blk) throws IOException {
      return new File(
          DatanodeUtil.idToBlockDir(getFinalizedDir(bpid), blk.getBlockId()).toString()
              + "/"
              + blk.getBlockName());
    }
    ...
    public void save() throws IOException {
      ...
      try (BufferedWriter writer =
          new BufferedWriter(
              new OutputStreamWriter(
                  fileIoProvider.getFileOutputStream(FsVolumeImpl.this, getTempSaveFile()),
                  "UTF-8"))) {
      ...
      fileIoProvider.move(
          FsVolumeImpl.this,
          getTempSaveFile().toPath(),
          getSaveFile().toPath(),
          StandardCopyOption.ATOMIC_MOVE);
      if (LOG.isTraceEnabled()) {
        LOG.trace("save({}, {}): saved {}", storageID, bpid, WRITER.writeValueAsString(state));
      }
    }
  ...
  /**
   * @param bytesReserved Space that was reserved during block creation. Now that the block is being
   *     finalized we can free up this space.
   * @return
   * @throws IOException
   */
  ReplicaInfo addFinalizedBlock(String bpid, Block b, ReplicaInfo replicaInfo, long bytesReserved)
      throws IOException {
  ...
  @Override
  public VolumeCheckResult check(VolumeCheckContext ignored) throws DiskErrorException {
  ...
  void getVolumeMap(
      String bpid, ReplicaMap volumeMap, final RamDiskReplicaTracker ramDiskReplicaMap)
      throws IOException {
  ...
  boolean isBPDirEmpty(String bpid) throws IOException {
    ...
    if (fileIoProvider.exists(this, rbwDir) && fileIoProvider.list(this, rbwDir).length != 0) {
      return false;
    }
    return true;
  }
  ...
  void deleteBPDirectories(String bpid, boolean force) throws IOException {
    ...
    if (force) {
      ...
      if (!DatanodeUtil.dirNoFilesRecursive(this, finalizedDir, fileIoProvider)
          || !fileIoProvider.fullyDelete(this, finalizedDir)) {
        throw new IOException("Failed to delete " + finalizedDir);
      }
      if (lazypersistDir.exists()
          && ((!DatanodeUtil.dirNoFilesRecursive(this, lazypersistDir, fileIoProvider)
  ...
  public ReplicaInPipeline append(
  ...
  public ReplicaInPipeline convertTemporaryToRbw(ExtendedBlock b, ReplicaInfo temp)
      throws IOException {
    ...
    // create RBW
    final LocalReplicaInPipeline rbw =
        new ReplicaBuilder(ReplicaState.RBW)
            .setBlockId(blockId)
            .setLength(numBytes)
  ...
  public ReplicaInPipeline updateRURCopyOnTruncate(
      ReplicaInfo rur, String bpid, long newBlockId, long recoveryId, long newlength)
      throws IOException {
    ...
    LocalReplica.truncateBlock(
        rur.getVolume(), blockFile, metaFile, rur.getNumBytes(), newlength, fileIoProvider);
  ...
  private File[] copyReplicaWithNewBlockIdAndGS(
      ReplicaInfo replicaInfo, String bpid, long newBlkId, long newGS) throws IOException {
    ...
    return FsDatasetImpl.copyBlockFiles(
        replicaInfo, dstMetaFile, dstBlockFile, true, DFSUtilClient.getSmallBufferSize(conf), conf);
  }

  @Override
  public void compileReport(String bpid, Collection<ScanInfo> report, ReportCompiler reportCompiler)
      throws InterruptedException, IOException {
    compileReport(getFinalizedDir(bpid), getFinalizedDir(bpid), report, reportCompiler);
  }
  ...
  private void compileReport(
      File bpFinalizedDir, File dir, Collection<ScanInfo> report, ReportCompiler reportCompiler)
    ...
    try {
      fileNames = fileIoProvider.listDirectory(this, dir, BlockDirFilter.INSTANCE);
    ...
    for (int i = 0; i < fileNames.size(); i++) {
      ...
      if (!Block.isBlockFilename(file)) {
        if (isBlockMetaFile(Block.BLOCK_FILE_PREFIX, file.getName())) {
          ...
          verifyFileLocation(file, bpFinalizedDir, blockId);
  ...
  /**
   * Helper method to determine if a file name is consistent with a block. meta-data file
   *
  ...
  /**
   * Verify whether the actual directory location of block file has the expected directory path
   * computed using its block ID.
   */
  ...
  public ReplicaInfo moveBlockToTmpLocation(
      ExtendedBlock block, ReplicaInfo replicaInfo, int smallBufferSize, Configuration conf)
      throws IOException {
  ...
  public File[] copyBlockToLazyPersistLocation(
      String bpId,
      long blockId,
      long genStamp,
    ...
    File[] targetFiles =
        FsDatasetImpl.copyBlockFiles(
            blockId, genStamp, replicaInfo, lazyPersistDir, true, smallBufferSize, conf);
    return targetFiles;
  }
  ...
  public void resolveDuplicateReplicas(
      String bpid, ReplicaInfo memBlockInfo, ReplicaInfo diskBlockInfo, ReplicaMap volumeMap)
      throws IOException {
    getBlockPoolSlice(bpid).resolveDuplicateReplicas(memBlockInfo, diskBlockInfo, volumeMap);
  }

  public ReplicaInfo activateSavedReplica(
      String bpid, ReplicaInfo replicaInfo, RamDiskReplica replicaState) throws IOException {
    return getBlockPoolSlice(bpid).activateSavedReplica(replicaInfo, replicaState);
  }
}
