...
/**
 * This class represents replicas that are under block recovery It has a recovery id that is equal
 * to the generation stamp that the replica will be bumped to after recovery The recovery id is used
 * to handle multiple concurrent block recoveries. A recovery with higher recovery id preempts
 * recoveries with a lower id.
 */
public class ReplicaUnderRecovery extends LocalReplica {
  ...
  public void setRecoveryID(long recoveryId) {
    if (recoveryId > this.recoveryId) {
      ...
      throw new IllegalArgumentException(
          "The new recovery id: "
              + recoveryId
              + " must be greater than the current one: "
              + this.recoveryId);
    }
  }
  ...
  public ReplicaRecoveryInfo createInfo() {
    return new ReplicaRecoveryInfo(
        original.getBlockId(),
        original.getBytesOnDisk(),
        original.getGenerationStamp(),
        original.getState());
  }
}
