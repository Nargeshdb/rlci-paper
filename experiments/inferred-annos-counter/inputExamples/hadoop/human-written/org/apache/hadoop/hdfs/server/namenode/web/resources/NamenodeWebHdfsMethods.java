...
public class NamenodeWebHdfsMethods {
  public static final Logger LOG = LoggerFactory.getLogger(NamenodeWebHdfsMethods.class);
  ...
  public NamenodeWebHdfsMethods(@Context HttpServletRequest request) {
    ...
    supportEZ = Boolean.valueOf(request.getHeader(WebHdfsFileSystem.EZ_HEADER));
  }
  ...
  private <T> T doAs(final UserGroupInformation ugi, final PrivilegedExceptionAction<T> action)
      throws IOException, InterruptedException {
    return useIpcCallq ? doAsExternalCall(ugi, action) : ugi.doAs(action);
  }

  private <T> T doAsExternalCall(
      final UserGroupInformation ugi, final PrivilegedExceptionAction<T> action)
      throws IOException, InterruptedException {
    // set the remote address, if coming in via a trust proxy server then
    // the address with be that of the proxied client
    ExternalCall<T> call =
        new ExternalCall<T>(action) {
          @Override
    ...
    if (excludeDatanodes != null) {
      for (String host : StringUtils.getTrimmedStringCollection(excludeDatanodes)) {
        ...
        if (excludeNode != null) {
          ...
          LOG.debug("DataNode {} was requested to be excluded, " + "but it was not found.", host);
        }
      }
    }
    ...
    if (op == PutOpParam.Op.CREATE) {
      ...
      if (clientNode != null) {
        final DatanodeStorageInfo[] storages =
            bm.chooseTarget4WebHDFS(path, clientNode, excludes, blocksize);
  ...
  /**
   * Choose the datanode to redirect the request. Note that the nodes have been sorted based on
   * availability and network distances, thus it is sufficient to return the first element of the
   * node here.
   */
  protected static DatanodeInfo bestNode(DatanodeInfo[] nodes, HashSet<Node> excludes)
      throws IOException {
  ...
  public Credentials createCredentials(final UserGroupInformation ugi, final String renewer)
      throws IOException {
    final NameNode namenode = (NameNode) context.getAttribute("name.node");
    final Credentials c =
  ...
  private URI redirectURI(
    ...
    dn =
        chooseDatanode(
            namenode, path, op, openOffset, blocksize, excludeDatanodes, remoteAddr, status);
    if (dn == null) {
      throw new IOException(
          "Failed to find datanode, suggest to check cluster"
              + " health. excludeDatanodes="
              + excludeDatanodes);
    }
    ...
    if (!UserGroupInformation.isSecurityEnabled()) {
      ...
      // generate a token
      final Token<? extends TokenIdentifier> t = generateDelegationToken(ugi, null);
    ...
    if (op == GetOpParam.Op.OPEN && supportEZ && status.getFileEncryptionInfo() != null) {
      prependReservedRawPath = true;
      rb.header(WebHdfsFileSystem.FEFINFO_HEADER, encodeFeInfo(status.getFileEncryptionInfo()));
    }
    final String uripath =
        WebHdfsFileSystem.PATH_PREFIX + (prependReservedRawPath ? "/.reserved/raw" + path : path);

    int port = "http".equals(scheme) ? dn.getInfoPort() : dn.getInfoSecurePort();
    final URI uri =
        new URI(scheme, null, dn.getHostName(), port, uripath, queryBuilder.toString(), null);
  ...
  @Consumes({"*/*"})
  @Produces({
    MediaType.APPLICATION_OCTET_STREAM + "; " + JettyUtils.UTF_8,
  ...
  public Response putRoot(
      ...
      @QueryParam(UnmaskedPermissionParam.NAME) @DefaultValue(UnmaskedPermissionParam.DEFAULT)
      ...
      @QueryParam(StoragePolicyParam.NAME) @DefaultValue(StoragePolicyParam.DEFAULT)
  ...
  protected void validateOpParams(HttpOpParam<?> op, Param... params) {
    for (Param param : params) {
      if (param.getValue() == null
          || param.getValueString() == null
          || param.getValueString().isEmpty()) {
  ...
  public Response put(
    ...
    return doAs(
        ugi,
        new PrivilegedExceptionAction<Response>() {
          @Override
  ...
  protected Response put(
    ...
    switch (op.getValue()) {
      ...
      case MKDIRS:
        {
          FsPermission masked =
              unmaskedPermission.getValue() == null
                  ? permission.getDirFsPermission()
                  : FsCreateModes.create(
                      permission.getDirFsPermission(), unmaskedPermission.getDirFsPermission());
      ...
      case RENAME:
        {
          ...
          if (s.isEmpty()) {
            ...
            cp.rename2(fullpath, destination.getValue(), s.toArray(new Options.Rename[s.size()]));
            return Response.ok().type(MediaType.APPLICATION_OCTET_STREAM).build();
          }
        }
      case SETREPLICATION:
      ...
      case SETXATTR:
        {
          ...
          cp.setXAttr(
              fullpath,
              XAttrHelper.buildXAttr(xattrName.getXAttrName(), xattrValue.getXAttrValue()),
      ...
      case CREATESNAPSHOT:
        {
          String snapshotPath = cp.createSnapshot(fullpath, snapshotName.getValue());
          final String js =
              JsonUtil.toJsonString(org.apache.hadoop.fs.Path.class.getSimpleName(), snapshotPath);
          return Response.ok(js).type(MediaType.APPLICATION_JSON).build();
        }
      case RENAMESNAPSHOT:
        {
          validateOpParams(op, oldSnapshotName, snapshotName);
          cp.renameSnapshot(fullpath, oldSnapshotName.getValue(), snapshotName.getValue());
          return Response.ok().type(MediaType.APPLICATION_OCTET_STREAM).build();
        }
      ...
      case SETQUOTA:
        ...
        cp.setQuota(fullpath, namespaceQuota.getValue(), storagespaceQuota.getValue(), null);
        return Response.ok().type(MediaType.APPLICATION_OCTET_STREAM).build();
  ...
  private static String encodeFeInfo(FileEncryptionInfo feInfo) {
    ...
    String encodedValue = encoder.encodeToString(PBHelperClient.convert(feInfo).toByteArray());
    return encodedValue;
  }
      ...
      case OPEN:
        {
          ...
          final URI uri =
              redirectURI(
                  ...
                  bufferSize);
          if (!noredirectParam.getValue()) {
            return rb.status(Status.TEMPORARY_REDIRECT)
                .location(uri)
      ...
      case GETDELEGATIONTOKEN:
        {
          ...
          final Token<? extends TokenIdentifier> token =
              generateDelegationToken(ugi, renewer.getValue());
      ...
      case GETXATTRS:
        {
          ...
          List<XAttr> xAttrs =
              cp.getXAttrs(
                  fullpath,
                  (names != null && !names.isEmpty()) ? XAttrHelper.buildXAttrs(names) : null);
      ...
      case GETSERVERDEFAULTS:
        {
          ...
          String serverDefaultsResponse = (String) context.getAttribute("serverDefaults");
          ...
          return Response.ok(serverDefaultsResponse).type(MediaType.APPLICATION_JSON).build();
        }
      case GETSNAPSHOTDIFF:
        {
          SnapshotDiffReport diffReport =
              cp.getSnapshotDiffReport(
  ...
  private String getTrashRoot(Configuration conf, String fullPath) throws IOException {
    UserGroupInformation ugi = UserGroupInformation.getCurrentUser();
    String parentSrc = getParent(fullPath);
    EncryptionZone ez =
        getRpcClientProtocol().getEZForPath(parentSrc != null ? parentSrc : fullPath);
    String trashRoot;
    if (ez != null) {
  ...
  private static DirectoryListing getDirectoryListing(
      final ClientProtocol cp, final String p, byte[] startAfter) throws IOException {
  ...
  private static StreamingOutput getListingStream(final ClientProtocol cp, final String p)
      throws IOException {
