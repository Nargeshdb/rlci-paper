...
abstract class AbstractINodeDiffList<
        N extends INode, A extends INodeAttributes, D extends AbstractINodeDiff<N, A, D>>
    implements Iterable<D> {

  /**
   * Diff list sorted by snapshot IDs, i.e. in chronological order. Created lazily to avoid wasting
   * memory by empty lists.
   */
  ...
  public final DiffList<D> asList() {
    return diffs != null ? DiffList.unmodifiableList(diffs) : DiffList.emptyList();
  }
  ...
  /**
   * Delete a snapshot. The synchronization of the diff list will be done outside. If the diff to
   * remove is not the first one in the diff list, we need to combine the diff with its previous
   * one.
   *
   ...
   */
  public final void deleteSnapshotDiff(
  ...
  DiffList<D> newDiffs() {
    return new DiffListByArrayList<>(INodeDirectory.DEFAULT_FILES_PER_DIRECTORY);
  }
   ...
   *
   * @param anchorId The returned snapshot's id must be &lt;= or &lt; this given snapshot id.
   * @param exclusive True means the returned snapshot's id must be &lt; the given id, otherwise
   *     &lt;=.
  ...
  /**
   * Search for the snapshot whose id is 1) no less than the given id, and 2) most close to the
   * given id.
   */
  ...
  final int[] changedBetweenSnapshots(Snapshot from, Snapshot to) {
    ...
    int laterDiffIndex = later == null ? size : getDiffIndexById(later.getId());
  ...
  /**
   * @return the inode corresponding to the given snapshot. Note that the current inode is returned
   *     if there is no change between the given snapshot and the current state.
   */
  ...
  final D checkAndAddLatestSnapshotDiff(int latestSnapshotId, N currentINode) {
    final D last = getLast();
    return (last != null
            && Snapshot.ID_INTEGER_COMPARATOR.compare(last.getSnapshotId(), latestSnapshotId) >= 0)
        ? last
