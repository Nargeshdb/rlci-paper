...
/**
 * Create proxy objects to communicate with a remote NN. All remote access to an NN should be
 * funneled through this class. Most of the time you'll want to use {@link
 * NameNodeProxies#createProxy(Configuration, URI, Class)}, which will create either an HA- or
 * non-HA-enabled client proxy as appropriate.
 */
@InterfaceAudience.Private
public class NameNodeProxies {

  private static final Logger LOG = LoggerFactory.getLogger(NameNodeProxies.class);
  ...
  public static <T> ProxyAndInfo<T> createProxy(
    ...
    AbstractNNFailoverProxyProvider<T> failoverProxyProvider =
        NameNodeProxiesClient.createFailoverProxyProvider(
            conf, nameNodeUri, xface, true, fallbackToSimpleAuth, new NameNodeHAProxyFactory<T>());
  ...
  /**
   * Creates an explicitly non-HA-enabled proxy object. Most of the time you don't want to use this,
   * and should instead use {@link NameNodeProxies#createProxy}.
   *
   ...
   * @param ugi the user who is making the calls on the proxy object
   * @param withRetries certain interfaces have a non-standard retry policy
   * @param fallbackToSimpleAuth - set to true or false during this method to indicate if a secure
   *     client falls back to simple auth
   * @return an object containing both the proxy and the associated delegation token service it
   *     corresponds to
   * @throws IOException
   */
  ...
  public static <T> ProxyAndInfo<T> createNonHAProxy(
    ...
    if (xface == ClientProtocol.class) {
      proxy =
          (T)
              NameNodeProxiesClient.createProxyWithAlignmentContext(
                  nnAddr, conf, ugi, withRetries, fallbackToSimpleAuth, alignmentContext);
    } else if (xface == JournalProtocol.class) {
      proxy = (T) createNNProxyWithJournalProtocol(nnAddr, conf, ugi, alignmentContext);
    } else if (xface == NamenodeProtocol.class) {
      proxy =
          (T) createNNProxyWithNamenodeProtocol(nnAddr, conf, ugi, withRetries, alignmentContext);
    } else if (xface == GetUserMappingsProtocol.class) {
      proxy = (T) createNNProxyWithGetUserMappingsProtocol(nnAddr, conf, ugi, alignmentContext);
    } else if (xface == RefreshUserMappingsProtocol.class) {
      proxy = (T) createNNProxyWithRefreshUserMappingsProtocol(nnAddr, conf, ugi, alignmentContext);
    } else if (xface == RefreshAuthorizationPolicyProtocol.class) {
      proxy =
          (T)
              createNNProxyWithRefreshAuthorizationPolicyProtocol(
                  nnAddr, conf, ugi, alignmentContext);
    } else if (xface == RefreshCallQueueProtocol.class) {
      proxy = (T) createNNProxyWithRefreshCallQueueProtocol(nnAddr, conf, ugi, alignmentContext);
    } else if (xface == InMemoryAliasMapProtocol.class) {
      proxy = (T) createNNProxyWithInMemoryAliasMapProtocol(nnAddr, conf, ugi, alignmentContext);
    } else if (xface == BalancerProtocols.class) {
      proxy =
          (T)
              createNNProxyWithBalancerProtocol(
                  nnAddr, conf, ugi, withRetries, fallbackToSimpleAuth, alignmentContext);
    } else {
      String message =
          "Unsupported protocol found when creating the proxy "
  ...
  private static InMemoryAliasMapProtocol createNNProxyWithInMemoryAliasMapProtocol(
      ...
      AlignmentContext alignmentContext)
      throws IOException {
    AliasMapProtocolPB proxy =
        createNameNodeProxy(address, conf, ugi, AliasMapProtocolPB.class, 30000, alignmentContext);
  ...
  private static @MustCall("close") JournalProtocol createNNProxyWithJournalProtocol(
      ...
      AlignmentContext alignmentContext)
      throws IOException {
    JournalProtocolPB proxy =
        createNameNodeProxy(address, conf, ugi, JournalProtocolPB.class, 30000, alignmentContext);
  ...
  private static @MustCall("close") RefreshAuthorizationPolicyProtocol
    ...
    RefreshAuthorizationPolicyProtocolPB proxy =
        createNameNodeProxy(
            address, conf, ugi, RefreshAuthorizationPolicyProtocolPB.class, 0, alignmentContext);
  ...
  private static @MustCall("close") RefreshUserMappingsProtocol
      createNNProxyWithRefreshUserMappingsProtocol(
          InetSocketAddress address,
          Configuration conf,
          UserGroupInformation ugi,
          AlignmentContext alignmentContext)
    ...
    RefreshUserMappingsProtocolPB proxy =
        createNameNodeProxy(
            address, conf, ugi, RefreshUserMappingsProtocolPB.class, 0, alignmentContext);
    return new RefreshUserMappingsProtocolClientSideTranslatorPB(proxy);
  }

  private static @MustCall("close") RefreshCallQueueProtocol
      createNNProxyWithRefreshCallQueueProtocol(
          InetSocketAddress address,
          Configuration conf,
          UserGroupInformation ugi,
          AlignmentContext alignmentContext)
    ...
    RefreshCallQueueProtocolPB proxy =
        createNameNodeProxy(
            address, conf, ugi, RefreshCallQueueProtocolPB.class, 0, alignmentContext);
    return new RefreshCallQueueProtocolClientSideTranslatorPB(proxy);
  }

  private static @MustCall("close") GetUserMappingsProtocol
    ...
    GetUserMappingsProtocolPB proxy =
        createNameNodeProxy(
            address, conf, ugi, GetUserMappingsProtocolPB.class, 0, alignmentContext);
  ...
  private static @MustCall("close") NamenodeProtocol createNNProxyWithNamenodeProtocol(
    ...
    NamenodeProtocolPB proxy =
        createNameNodeProxy(address, conf, ugi, NamenodeProtocolPB.class, 0, alignmentContext);
  ...
  private static BalancerProtocols createNNProxyWithBalancerProtocol(
    ...
    NamenodeProtocol namenodeProtocol =
        createNNProxyWithNamenodeProtocol(address, conf, ugi, withRetries, alignmentContext);
    ClientProtocol clientProtocol =
        NameNodeProxiesClient.createProxyWithAlignmentContext(
            address, conf, ugi, withRetries, fallbackToSimpleAuth, alignmentContext);

    return ProxyCombiner.combine(BalancerProtocols.class, namenodeProtocol, clientProtocol);
  }

  private static <T> T createNameNodeProxy(
      InetSocketAddress address,
      Configuration conf,
      UserGroupInformation ugi,
      Class<T> xface,
