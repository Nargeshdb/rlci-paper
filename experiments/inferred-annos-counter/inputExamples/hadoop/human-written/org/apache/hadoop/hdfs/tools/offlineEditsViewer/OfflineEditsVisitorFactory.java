...
public class OfflineEditsVisitorFactory {
   ...
   */
  @SuppressWarnings(
      "builder:required.method.not.called") // TP: IOUtils.closeStream(fout) this will not close
                                            // fout in the regular path because of the null
                                            // assignment (validated)
  public static OfflineEditsVisitor getEditsVisitor(
      String filename, String processor, boolean printToScreen) throws IOException {
    ...
    try {
      ...
      if (StringUtils.equalsIgnoreCase("xml", processor)) {
        ...
        throw new IOException(
            "Unknown processor " + processor + " (valid processors: xml, binary, stats)");
      }
      out = fout = null;
      return vis;
    } finally {
