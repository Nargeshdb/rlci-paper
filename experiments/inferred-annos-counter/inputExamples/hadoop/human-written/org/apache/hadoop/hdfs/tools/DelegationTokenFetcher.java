...
public class DelegationTokenFetcher {
   ...
   * @throws org.apache.hadoop.util.ExitUtil.ExitException if the command failed and exiting was
   *     disabled.
   */
  ...
  /**
   * Command line interface with a specific configuration. Errors in this operation will call {@link
   * ExitUtil#terminate(int)} to exit the process.
   *
   ...
   */
  @VisibleForTesting
  public static void main(Configuration conf, final String[] args) throws Exception {
  ...
  @VisibleForTesting
  static void saveDelegationToken(
      Configuration conf, FileSystem fs, final String renewer, final Path tokenFile)
      throws IOException {
    ...
    if (null != token) {
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            "Fetched token " + fs.getUri() + " for " + token.getService() + " into " + tokenFile);
      }
    } else {
  ...
  public static String printTokensToString(
      final Configuration conf, final Path tokenFile, final boolean verbose) throws IOException {
  ...
  static void printTokens(final Configuration conf, final Path tokenFile, final boolean verbose)
      throws IOException {
    System.out.print(printTokensToString(conf, tokenFile, verbose));
  }

  /**
   * Print usage to the error stream, then call {@link ExitUtil#terminate(int)} with status code 1.
  ...
  private static void printUsage(PrintStream err) {
    ...
    err.println("Options:");
    err.println(
        "  --webservice <url>  URL to contact NN on (starts with "
    ...
    err.println(
        "  --renew             Renew the delegation token.  "
            + "Delegation "
            + "token must have been fetched using the --renewer"
            + " <name> option.");
    err.println(
        "  --print [--verbose] Print the delegation token, when "
