...
/**
 * Thread which runs inside the NN when it's in Standby state, periodically waking up to take a
 * checkpoint of the namespace. When it takes a checkpoint, it saves it to its local storage and
 * then uploads it to the remote NameNode.
 */
...
public class StandbyCheckpointer {
  ...
  public StandbyCheckpointer(Configuration conf, FSNamesystem ns) throws IOException {
    ...
    this.uploadThreadFactory =
        new ThreadFactoryBuilder()
            .setDaemon(true)
            .setNameFormat("TransferFsImageUpload-%d")
            .build();
  ...
  /**
   * Determine the address of the NN we are checkpointing as well as our own HTTP address from the
   * configuration.
   *
   * @throws IOException
   */
  private void setNameNodeAddresses(Configuration conf) throws IOException {
    ...
    for (Configuration activeConf : confForActive) {
      ...
      Preconditions.checkArgument(
          checkAddress(activeNNAddress), "Bad address for active NN: %s", activeNNAddress);
    ...
    // Sanity-check.
    Preconditions.checkArgument(
        checkAddress(myNNAddress), "Bad address for standby NN: %s", myNNAddress);
  }
  ...
  private void doCheckpoint() throws InterruptedException, IOException {
    ...
    try {
      ...
      if (thisCheckpointTxId == prevCheckpointTxId) {
        LOG.info(
            "A checkpoint was triggered but the Standby Node has not "
      ...
      assert txid == thisCheckpointTxId
          : "expected to save checkpoint at txid="
              + thisCheckpointTxId
              + " but instead saved at txid="
              + txid;
      ...
      if (outputDir != null && !outputDir.isEmpty()) {
        try {
          ...
          LOG.warn(
              "Exception encountered while saving legacy OIV image; "
                  + "continuing with other checkpointing steps",
              ioe);
        }
      }
    } finally {
    ...
    HashMap<String, Future<TransferFsImage.TransferResult>> uploads = new HashMap<>();
    for (final URL activeNNAddress : activeNNAddresses) {
      ...
      CheckpointReceiverEntry receiverEntry = checkpointReceivers.get(addressString);
      long secsSinceLastUpload =
          TimeUnit.MILLISECONDS.toSeconds(monotonicNow() - receiverEntry.getLastUploadTime());
      boolean shouldUpload =
          receiverEntry.isPrimary() || secsSinceLastUpload >= checkpointConf.getQuietPeriod();
      if (shouldUpload) {
        Future<TransferFsImage.TransferResult> upload =
            executor.submit(
                new Callable<TransferFsImage.TransferResult>() {
                  @Override
    ...
    for (Map.Entry<String, Future<TransferFsImage.TransferResult>> entry : uploads.entrySet()) {
  ...
  /**
   * Cancel any checkpoint that's currently being made, and prevent any new checkpoints from
   * starting for the next minute or so.
   */
  ...
  private class CheckpointerThread extends Thread {
    ...
    /**
     * Prevent checkpoints from occurring for some time period in the future. This is used when
     * preparing to enter active mode. We need to not only cancel any concurrent checkpoint, but
     * also prevent any checkpoints from racing to start just after the cancel call.
     *
     * @param delayMs the number of MS for which checkpoints will be prevented
     */
      ...
      while (shouldRun) {
        ...
        try {
          ...
          if (needCheckpoint) {
            ...
            LOG.info(
                "Triggering checkpoint because there have been {} txns "
            ...
            LOG.info(
                "Triggering checkpoint because it has been {} seconds "
