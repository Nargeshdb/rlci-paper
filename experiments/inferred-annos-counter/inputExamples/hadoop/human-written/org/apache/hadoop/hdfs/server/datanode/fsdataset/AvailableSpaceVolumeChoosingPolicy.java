...
/**
 * A DN volume choosing policy which takes into account the amount of free space on each of the
 * available volumes when considering where to assign a new replica allocation. By default this
 * policy prefers assigning replicas to those volumes with more available free space, so as to over
 * time balance the available space of all the volumes within a DN. Use fine-grained locks to enable
 * choosing volumes of different storage types concurrently.
 */
public class AvailableSpaceVolumeChoosingPolicy<V extends FsVolumeSpi>
  ...
  public void setConf(Configuration conf) {
    ...
    LOG.info(
        "Available space volume choosing policy initialized: "
            + DFS_DATANODE_AVAILABLE_SPACE_VOLUME_CHOOSING_POLICY_BALANCED_SPACE_THRESHOLD_KEY
            + " = "
            + balancedSpaceThreshold
            + ", "
    ...
    if (balancedPreferencePercent < 0.5) {
      LOG.warn(
          "The value of "
              + DFS_DATANODE_AVAILABLE_SPACE_VOLUME_CHOOSING_POLICY_BALANCED_SPACE_PREFERENCE_FRACTION_KEY
              + " is less than 0.5 so volumes with less available disk space will receive more"
              + " block allocations");
    }
  }
  ...
  private V doChooseVolume(final List<V> volumes, long replicaSize, String storageId)
      throws IOException {
    ...
    if (volumesWithSpaces.areAllVolumesWithinFreeSpaceThreshold()) {
      ...
      long mostAvailableAmongLowVolumes =
          volumesWithSpaces.getMostAvailableSpaceAmongVolumesWithLowAvailableSpace();

      List<V> highAvailableVolumes =
          extractVolumesFromPairs(volumesWithSpaces.getVolumesWithHighAvailableSpace());
      List<V> lowAvailableVolumes =
          extractVolumesFromPairs(volumesWithSpaces.getVolumesWithLowAvailableSpace());

      float preferencePercentScaler =
          (highAvailableVolumes.size() * balancedPreferencePercent)
              + (lowAvailableVolumes.size() * (1 - balancedPreferencePercent));
      float scaledPreferencePercent =
          (highAvailableVolumes.size() * balancedPreferencePercent) / preferencePercentScaler;
      if (mostAvailableAmongLowVolumes < replicaSize
          || random.nextFloat() < scaledPreferencePercent) {
        volume =
            roundRobinPolicyHighAvailable.chooseVolume(
        ...
        volume =
            roundRobinPolicyLowAvailable.chooseVolume(lowAvailableVolumes, replicaSize, storageId);
        if (LOG.isDebugEnabled()) {
          LOG.debug(
              "Volumes are imbalanced. Selecting "
                  + volume
  ...
  private class AvailableSpaceVolumeList {
    ...
    /**
     * @return true if all volumes' free space is within the configured threshold, false otherwise.
     */
    ...
    /**
     * @return the minimum amount of space available on a single volume, across all volumes.
     */
  ...
  /**
   * Used so that we only check the available space on a given volume once, at the beginning of
