...
/**
 * OfflineImageViewerPB to dump the contents of an Hadoop image file to XML or the console. Main
 * entry point into utility, either via the command line or programmatically.
 */
...
public class OfflineImageViewerPB {
  ...
  /**
   * Entry point to command-line-driven operation. User may specify options and start fsimage viewer
   * from the command line. Program will process image file and exit cleanly or, if an error is
  ...
  @SuppressWarnings(
      "builder:required.method.not.called") // FP nullness reasoning: close is called on out in the
                                            // finally block with a null check (validated)
  public static int run(String[] args) throws Exception {
    ...
    try {
      out =
          outputFile.equals("-") || "REVERSEXML".equalsIgnoreCase(processor)
              ? System.out
      ...
      switch (StringUtils.toUpperCase(processor)) {
        ...
        case "DELIMITED":
          ...
          try (PBImageDelimitedTextWriter writer =
                  new PBImageDelimitedTextWriter(out, delimiter, tempPath, printStoragePolicy);
