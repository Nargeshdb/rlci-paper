...
/**
 * Inspects an FSImage storage directory in the "old" (pre-HDFS-1073) format. This format has the
 * following data files: - fsimage - fsimage.ckpt (when checkpoint is being uploaded) - edits -
...
class FSImagePreTransactionalStorageInspector extends FSImageStorageInspector {
  ...
  @SuppressWarnings(
      "builder:required.method.not.called") // FP
                                            ...
                                            // can't verify that cleanupWithLogger() closes in
                                            // (validated)
  ...
  List<FSImageFile> getLatestImages() throws IOException {
    ...
    if (latestNameCheckpointTime > latestEditsCheckpointTime
      ...
      LOG.error(
          "Image checkpoint time "
              + latestNameCheckpointTime
              + " > edits checkpoint time "
              + latestEditsCheckpointTime);
      LOG.error(
          "Name-node will treat the image as the latest state of "
              + "the namespace. Old edits will be discarded.");
    } else if (latestNameCheckpointTime != latestEditsCheckpointTime) {
      throw new IOException(
          "Inconsistent storage detected, "
  ...
  /**
   * @return a list with the paths to EDITS and EDITS_NEW (if it exists) in a given storage
   *     directory.
   */
