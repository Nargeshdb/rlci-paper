...
/**
 * Keep prioritized queues of low redundant blocks. Blocks have redundancy priority, with priority
 * {@link #QUEUE_HIGHEST_PRIORITY} indicating the highest priority. Having a prioritised queue
 * allows the {@link BlockManager} to select which blocks to replicate first -it tries to give
 * priority to data that is most at risk or considered most valuable.
 *
 * <p>The policy for choosing which priority to give added blocks is implemented in {@link
 * #getPriority(BlockInfo, int, int, int, int)}.
 *
 * <p>The queue order is as follows:
 *
 * <ol>
 *   <li>{@link #QUEUE_HIGHEST_PRIORITY}: the blocks that should be redundant first. That is blocks
 *       with only one copy, or blocks with zero live copies but a copy in a node being
 *       decommissioned. These blocks are at risk of loss if the disk or server on which they remain
 *       fails.
 *   <li>{@link #QUEUE_VERY_LOW_REDUNDANCY}: blocks that are very under-replicated compared to their
 *       expected values. Currently that means the ratio of the ratio of actual:expected means that
 *       there is <i>less than</i> 1:3.. These blocks may not be at risk, but they are clearly
 *       considered "important".
 *   <li>{@link #QUEUE_LOW_REDUNDANCY}: blocks that are also under replicated, and the ratio of
 *       actual:expected is good enough that they do not need to go into the {@link
 *       #QUEUE_VERY_LOW_REDUNDANCY} queue.
 *   <li>{@link #QUEUE_REPLICAS_BADLY_DISTRIBUTED}: there are as least as many copies of a block as
 *       required, but the blocks are not adequately distributed. Loss of a rack/switch could take
 *       all copies off-line.
 *   <li>{@link #QUEUE_WITH_CORRUPT_BLOCKS} This is for blocks that are corrupt and for which there
 *       are no-non-corrupt copies (currently) available. The policy here is to keep those corrupt
 *       blocks replicated, but give blocks that are not corrupt higher priority.
 * </ol>
 */
class LowRedundancyBlocks implements Iterable<BlockInfo> {
  ...
  /**
   * The queue for blocks that have the right number of replicas, but which the block manager felt
   * were badly distributed: {@value}
   */
  ...
  /** Return the number of insufficiently redundant blocks excluding corrupt blocks. */
  ...
  /** Return the number of under replicated blocks with the highest priority to recover */
  long getHighestPriorityReplicatedBlockCount() {
    return highestPriorityLowRedundancyReplicatedBlocks.longValue();
  }

  /** Return the number of under replicated EC blocks with the highest priority to recover */
  ...
  long getLowRedundancyECBlockGroups() {
    return lowRedundancyECBlockGroups.longValue() - getCorruptECBlockGroups();
  }
  ...
  private int getPriority(
    ...
    if (block.isStriped()) {
      ...
      return getPriorityStriped(
          curReplicas, outOfServiceReplicas, sblk.getRealDataBlockNum(), sblk.getParityBlockNum());
    } else {
      return getPriorityContiguous(
          curReplicas, readOnlyReplicas, outOfServiceReplicas, expectedReplicas);
    }
  }

  private int getPriorityContiguous(
      int curReplicas, int readOnlyReplicas, int outOfServiceReplicas, int expectedReplicas) {
    if (curReplicas == 0) {
  ...
  private int getPriorityStriped(
      int curReplicas, int outOfServiceReplicas, short dataBlkNum, short parityBlkNum) {
  ...
  synchronized boolean add(
      ...
      int expectedReplicas) {
    final int priLevel =
        getPriority(block, curReplicas, readOnlyReplicas, outOfServiceReplicas, expectedReplicas);
  ...
  synchronized boolean remove(
    ...
    final int priLevel =
        getPriority(
            block, oldReplicas, oldReadOnlyReplicas, outOfServiceReplicas, oldExpectedReplicas);
    ...
    if (priLevel == QUEUE_WITH_CORRUPT_BLOCKS && oldExpectedReplicas == 1 && removedBlock) {
      assert corruptReplicationOneBlocks.longValue() >= 0
          : "Number of corrupt blocks with replication factor 1 " + "should be non-negative";
    }
    return removedBlock;
  }
   ...
   *
   * <p>The priLevel parameter is a hint of which queue to query first: if negative or &gt;= {@link
   * #LEVEL} this shortcutting is not attmpted.
   *
   * <p>If the block is not found in the nominated queue, an attempt is made to remove it from all
   * queues.
   *
   * <p><i>Warning:</i> This is not a synchronized method.
   *
   ...
   *
   * <p>If the block priority has changed from before an attempt is made to remove it from the block
   * queue. Regardless of whether or not the block is in the block queue of (recalculate) priority,
   * an attempt is made to add it to that queue. This ensures that the block will be in its expected
   * priority queue (and only that queue) by the end of the method call.
   *
   * @param block a low redundancy block
   * @param curReplicas current number of replicas of the block
   * @param outOfServiceReplicas the number of out-of-service replicas
  ...
  synchronized void update(
    ...
    int curPri =
        getPriority(
            block, curReplicas, readOnlyReplicas, outOfServiceReplicas, curExpectedReplicas);
    int oldPri =
        getPriority(
            block, oldReplicas, readOnlyReplicas, outOfServiceReplicas, oldExpectedReplicas);
    if (NameNode.stateChangeLog.isDebugEnabled()) {
      NameNode.stateChangeLog.debug(
          "LowRedundancyBlocks.update "
              + block
              + " curReplicas "
              + curReplicas
              + " curExpectedReplicas "
  ...
  /**
   * Get a list of block lists without sufficient redundancy. The index of block lists represents
   * its replication priority. Iterates each block list in priority order beginning with the highest
   * priority list. Iterators use a bookmark to resume where the previous iteration stopped. Returns
   * when the block count is met or iteration reaches the end of the lowest priority list, in which
   * case bookmarks for each block list are reset to the heads of their respective lists.
   *
   * @param blocksToProcess - number of blocks to fetch from low redundancy blocks.
   * @return Return a list of block lists to be replicated. The block list index represents its
   *     redundancy priority.
   */
   ...
   *
   * @param blocksToProcess - number of blocks to fetch from low redundancy blocks.
   * @param resetIterators - After gathering the list of blocks reset the position of all queue
   *     iterators to the head of the queue so subsequent calls will begin at the head of the queue
   * @return Return a list of block lists to be replicated. The block list index represents its
   *     redundancy priority.
   */
