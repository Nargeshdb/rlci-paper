...
/**
 * This is the server side implementation responsible for passing state alignment info to clients.
 */
@InterfaceAudience.Private
@InterfaceStability.Evolving
class GlobalStateIdContext implements AlignmentContext {
  /**
   * Estimated number of journal transactions a typical NameNode can execute per second. The number
   * is used to estimate how long a client's RPC request will wait in the call queue before the
   * Observer catches up with its state id.
   */
  private static final long ESTIMATED_TRANSACTIONS_PER_SECOND = 10000L;

  /**
   * The client wait time on an RPC request is composed of the server execution time plus the
   * communication time. This is an expected fraction of the total wait time spent on server
   * execution.
   */
  ...
  /**
   * Server side implementation only provides state alignment info. It does not receive state
   * alignment info therefore this does nothing.
   */
  @Override
  ...
  /**
   * Server side implementation only receives state alignment info. It does not build RPC requests
  ...
  /**
   * Server-side implementation for processing state alignment info in requests. For Observer it
   * compares the client and the server states and determines if it makes sense to wait until the
   * server catches up with the client state. If not the server throws RetriableException so that
   * the client could retry the call according to the retry policy with another Observer or the
   * Active NameNode.
   *
   * @param header The RPC request header.
   * @param clientWaitTime time in milliseconds indicating how long client waits for the server
   *     response. It is used to verify if the client's state is too far ahead of the server's
  ...
  public long receiveRequestState(RpcRequestHeaderProto header, long clientWaitTime)
      throws IOException {
    if (!header.hasStateId() && HAServiceState.OBSERVER.equals(namesystem.getState())) {
      ...
      throw new StandbyException(
          "Observer Node received request without "
              + "stateId. This mostly likely is because client is not configured "
              + "with "
              + ObserverReadProxyProvider.class.getSimpleName());
    }
    long serverStateId = getLastSeenStateId();
    long clientStateId = header.getStateId();
    FSNamesystem.LOG.trace(
        "Client State ID= {} and Server State ID= {}", clientStateId, serverStateId);

    if (clientStateId > serverStateId && HAServiceState.ACTIVE.equals(namesystem.getState())) {
      FSNamesystem.LOG.warn(
          "The client stateId: {} is greater than "
