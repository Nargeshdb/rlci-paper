...
/**
 * The Checkpointer is responsible for supporting periodic checkpoints of the HDFS metadata.
 *
 * <p>The Checkpointer is a daemon that periodically wakes up up (determined by the schedule
 * specified in the configuration), triggers a periodic checkpoint and then goes back to sleep.
 *
 * <p>The start of a checkpoint is triggered by one of the two factors: (1) time or (2) the size of
 * the edits file.
 */
class Checkpointer extends Daemon {
  public static final Logger LOG = LoggerFactory.getLogger(Checkpointer.class.getName());
  ...
  private void initialize(Configuration conf) throws IOException {
    ...
    String fullInfoAddr =
        conf.get(DFS_NAMENODE_BACKUP_HTTP_ADDRESS_KEY, DFS_NAMENODE_BACKUP_HTTP_ADDRESS_DEFAULT);
    ...
    LOG.info(
        "Checkpoint Period : "
            ...
            + " min)");
    LOG.info(
        "Transactions count is  : " + checkpointConf.getTxnCount() + ", to trigger checkpoint");
  }
  ...
  @Override
  public void run() {
    // How often to check the size of the edit log (min of checkpointCheckPeriod and
    // checkpointPeriod)
    ...
    while (shouldRun) {
      try {
        ...
        if (now >= lastCheckpointTime + checkpointPeriodMSec) {
          ...
          if (txns >= checkpointConf.getTxnCount()) shouldCheckpoint = true;
        }
        if (shouldCheckpoint) {
          doCheckpoint();
  ...
  void doCheckpoint() throws IOException {
    ...
    if (!manifest.getLogs().isEmpty()) {
      ...
      if (firstRemoteLog.getStartTxId() > lastApplied + 1) {
        LOG.info(
            "Unable to roll forward using only logs. Downloading "
                + "image with txid "
                + sig.mostRecentCheckpointTxId);
        MD5Hash downloadedHash =
            TransferFsImage.downloadImageToStorage(
                backupNode.nnHttpAddress, sig.mostRecentCheckpointTxId, bnStorage, true, false);
    ...
    if (cpCmd.needToReturnImage()) {
      TransferFsImage.uploadImageFromStorage(
          backupNode.nnHttpAddress, conf, bnStorage, NameNodeFile.IMAGE, txid);
    }
  ...
  static void rollForwardByApplyingLogs(
      RemoteEditLogManifest manifest, FSImage dstImage, FSNamesystem dstNamesystem)
      throws IOException {
