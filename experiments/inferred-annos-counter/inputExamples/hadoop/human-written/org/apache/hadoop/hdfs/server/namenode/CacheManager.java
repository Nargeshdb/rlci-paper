 ...
 *
 * <p>This class is instantiated by the FSNamesystem. It maintains the mapping of cached blocks to
 * datanodes via processing datanode cache reports. Based on these reports and addition and removal
 * of caching directives, we will schedule caching and uncaching work.
 */
...
public class CacheManager {
   ...
   *
   * <p>listCacheDirectives relies on the ordering of elements in this map to track what has already
   * been listed by the client.
   */
  ...
  /** Cache directives */
  private final Multimap<String, CacheDirective> directivesByPath = HashMultimap.create();
   ...
   *
   * <p>If caching is disabled, we will not process cache reports or store information about what is
   * cached where. We also do not start the CacheReplicationMonitor thread. This will save
   * resources, but provide less functionality.
   *
   * <p>Even when caching is disabled, we still store path-based cache information. This information
   * is stored in the edit log and fsimage. We don't want to lose it just because a configuration
   * setting was turned off. However, we will not act on this information if caching is disabled.
   */
  ...
  public static final class PersistState {
    ...
    public PersistState(
        CacheManagerSection section,
        List<CachePoolInfoProto> pools,
        List<CacheDirectiveInfoProto> directives) {
  ...
  CacheManager(FSNamesystem namesystem, Configuration conf, BlockManager blockManager) {
    ...
    this.enabled =
        conf.getBoolean(DFS_NAMENODE_CACHING_ENABLED_KEY, DFS_NAMENODE_CACHING_ENABLED_DEFAULT);
    this.maxListCachePoolsResponses =
        conf.getInt(
            DFS_NAMENODE_LIST_CACHE_POOLS_NUM_RESPONSES,
            DFS_NAMENODE_LIST_CACHE_POOLS_NUM_RESPONSES_DEFAULT);
    this.maxListCacheDirectivesNumResponses =
        conf.getInt(
    ...
    this.cachedBlocks =
        enabled
            ? new LightWeightGSet<CachedBlock, CachedBlock>(
                LightWeightGSet.computeCapacity(cachedBlocksPercent, "cachedBlocks"))
            : new LightWeightGSet<>(0);
  }
  ...
  /**
   * Resets all tracked directives and pools. Called during 2NN checkpointing to reset FSNamesystem
   * state. See {@link FSNamesystem#clear()}.
   */
  void clear() {
  ...
  public void startMonitorThread() {
    if (!isEnabled()) {
      LOG.info("Not starting CacheReplicationMonitor as name-node caching" + " is disabled.");
      return;
    }
  ...
  /**
   * Calculates the absolute expiry time of the directive from the {@link
   * CacheDirectiveInfo.Expiration}. This converts a relative Expiration into an absolute time based
   * on the local clock.
   *
   * @param info to validate.
   * @param maxRelativeExpiryTime of the info's pool.
   * @return the expiration time, or the pool's max absolute expiration if the info's expiration was
   *     not set.
  ...
  private static long validateExpiryTime(CacheDirectiveInfo info, long maxRelativeExpiryTime)
    ...
    if (relExpiryTime > Expiration.MAX_RELATIVE_EXPIRY_MS) {
      throw new InvalidRequestException(
          "Expiration " + expiry.toString() + " is too far in the future!");
    }
    ...
    if (pool.getBytesNeeded() + stats.getBytesNeeded() > pool.getLimit()) {
      throw new InvalidRequestException(
          "Caching path "
              + path
              + " of size "
              + stats.getBytesNeeded() / replication
              + " bytes at replication "
              + replication
              + " would exceed pool "
              + pool.getPoolName()
              + "'s remaining capacity of "
              + (pool.getLimit() - pool.getBytesNeeded())
              + " bytes.");
    }
  }
  ...
  /** Get a CacheDirective by ID, validating the ID and that the directive exists. */
  ...
  /**
   * Adds a directive, skipping most error checking. This should only be called internally in
   * special scenarios like edit log replay.
   */
  ...
  /**
   * Factory method that makes a new CacheDirectiveInfo by applying fields in a CacheDirectiveInfo
   * to an existing CacheDirective.
   *
   * @param info with some or all fields set.
   * @param defaults directive providing default values for unset fields in info.
  ...
  /**
   * Modifies a directive, skipping most error checking. This is for careful internal use only.
   * modifyDirective can be non-deterministic since its error checking depends on current system
   * time, which poses a problem for edit log replay.
   */
    ...
    try {
      ...
      if (!srcPool.getPoolName().equals(destPool.getPoolName())) {
        ...
        if (!flags.contains(CacheFlag.FORCE)) {
          checkLimit(
              destPool,
              infoWithDefaults.getPath().toUri().getPath(),
  ...
  public BatchedListEntries<CacheDirectiveEntry> listCacheDirectives(
      long prevId, CacheDirectiveInfo filter, FSPermissionChecker pc) throws IOException {
    assert namesystem.hasReadLock();
   ...
   * Create a cache pool.
   *
   * <p>Only the superuser should be able to call this function.
   *
  ...
  public void modifyCachePool(CachePoolInfo info) throws IOException {
    ...
    try {
      ...
      if (info.getDefaultReplication() != null) {
        ...
        bld.append(prefix).append("set default replication to " + defaultReplication);
        prefix = "; ";
      }
  ...
  private void setCachedLocations(LocatedBlock block) {
    ...
    for (DatanodeDescriptor datanode : cachedDNs) {
      ...
      if (!found) {
        LOG.warn(
            "Datanode {} is not a valid cache location for block {} "
  ...
  public final void processCacheReport(final DatanodeID datanodeID, final List<Long> blockIds)
      throws IOException {
    if (!enabled) {
      LOG.debug(
          "Ignoring cache report from {} because {} = false. " + "number of blocks: {}",
          datanodeID,
          DFS_NAMENODE_CACHING_ENABLED_KEY,
          blockIds.size());
      return;
    }
  ...
  private void processCacheReportImpl(
      final DatanodeDescriptor datanode, final List<Long> blockIds) {
  ...
  /**
   * Saves the current state of the CacheManager to the DataOutput. Used to persist CacheManager
   * state in the FSImage.
   *
  ...
  public PersistState saveState() throws IOException {
    ...
    ArrayList<CacheDirectiveInfoProto> directives =
        Lists.newArrayListWithCapacity(directivesById.size());
    ...
    CacheManagerSection s =
        CacheManagerSection.newBuilder()
            .setNextDirectiveId(nextDirectiveId)
            .setNumPools(pools.size())
            .setNumDirectives(directives.size())
            .build();
  ...
  /**
   * Reloads CacheManager state from the passed DataInput. Used during namenode startup to restore
   * CacheManager state from an FSImage.
   *
  ...
  public void loadState(PersistState s) throws IOException {
    ...
    for (CacheDirectiveInfoProto p : s.directives) {
      ...
      CacheDirective directive =
          new CacheDirective(
              p.getId(),
              new Path(p.getPath()).toUri().getPath(),
              (short) p.getReplication(),
              p.getExpiration().getMillis());
      addCacheDirective(poolName, directive);
    }
  }

  private void addCacheDirective(final String poolName, final CacheDirective directive)
      throws IOException {
    CachePool pool = cachePools.get(poolName);
    if (pool == null) {
