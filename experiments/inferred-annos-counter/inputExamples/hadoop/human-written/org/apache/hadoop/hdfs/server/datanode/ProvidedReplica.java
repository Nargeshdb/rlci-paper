...
public abstract class ProvidedReplica extends ReplicaInfo {
   ...
   *
   * @param blockId block id
   * @param pathPrefix A prefix of the {@link Path} associated with this replica on the remote
   *     {@link FileSystem}.
   * @param pathSuffix A suffix of the {@link Path} associated with this replica on the remote
   *     {@link FileSystem}. Resolving the {@code pathSuffix} against the {@code pathPrefix} should
   *     provide the exact {@link Path} of the data associated with this replica on the remote
   *     {@link FileSystem}.
   * @param fileOffset the offset in the remote URI
   * @param blockLen the length of the block
   * @param genStamp the generation stamp of the block
    ...
    if (remoteFS != null) {
      ...
      try {
        if (pathHandle != null) {
          ins =
              remoteFS.open(
                  pathHandle, conf.getInt(IO_FILE_BUFFER_SIZE_KEY, IO_FILE_BUFFER_SIZE_DEFAULT));
        } else {
      ...
      return new BoundedInputStream(new FSDataInputStream(ins), getBlockDataLength());
    } else {
  ...
  public LengthInputStream getMetadataInputStream(long offset) throws IOException {
    return new LengthInputStream(
        new ByteArrayInputStream(NULL_CHECKSUM_ARRAY), NULL_CHECKSUM_ARRAY.length);
  }
  ...
  public int compareWith(ScanInfo info) {
    if (info.getFileRegion()
        .equals(
            new FileRegion(
                this.getBlockId(),
                new Path(getRemoteURI()),
                fileOffset,
                this.getNumBytes(),
