...
public abstract class MD5FileUtils {
  ...
  public static void verifySavedMD5(File dataFile, MD5Hash expectedMD5) throws IOException {
    ...
    if (!expectedMD5.equals(storedHash)) {
      throw new IOException(
          "File "
              + dataFile
              + " did not match stored MD5 checksum "
              + " (stored: "
              + storedHash
              + ", computed: "
              + expectedMD5);
  ...
  /**
   * Read the md5 file stored alongside the given data file and match the md5 file content.
   *
   * @param dataFile the file containing data
   * @return a matcher with two matched groups where group(1) is the md5 string and group(2) is the
   *     data file path.
   */
  private static Matcher readStoredMd5(File md5File) throws IOException {
    BufferedReader reader =
        new BufferedReader(
            new InputStreamReader(Files.newInputStream(md5File.toPath()), Charsets.UTF_8));
    String md5Line;
    try {
  ...
  public static MD5Hash readStoredMd5ForFile(File dataFile) throws IOException {
    ...
    if (!referencedFile.getName().equals(dataFile.getName())) {
      throw new IOException(
          "MD5 file at "
              + md5File
              + " references file named "
              + referencedFile.getName()
              + " but we expected it to reference "
              + dataFile);
    }
  ...
  /**
   * @return a reference to the file with .md5 suffix that will contain the md5 checksum for the
   *     given data file.
   */
