...
public interface HdfsServerConstants {
  ...
  /**
   * Current layout version for NameNode. Please see {@link NameNodeLayoutVersion.Feature} on adding
   * new layout version.
   */
  int NAMENODE_LAYOUT_VERSION = NameNodeLayoutVersion.CURRENT_LAYOUT_VERSION;
  /**
   * Current layout version for DataNode. Please see {@link DataNodeLayoutVersion.Feature} on adding
   * new layout version.
   */
   ...
   *
   * <p>.reserved is only reserved under root ("/").
   */
  String[] RESERVED_PATH_COMPONENTS =
      new String[] {HdfsConstants.DOT_SNAPSHOT_DIR, FSDirectory.DOT_RESERVED_STRING};

  byte[] DOT_SNAPSHOT_DIR_BYTES = DFSUtil.string2Bytes(HdfsConstants.DOT_SNAPSHOT_DIR);
    ...
    static RollingUpgradeStartupOption fromString(String s) {
      ...
      throw new IllegalArgumentException(
          "Failed to convert \""
              + s
              + "\" to "
              + RollingUpgradeStartupOption.class.getSimpleName());
    }
  ...
  enum StartupOption {
    ...
    public MetaRecoveryContext createRecoveryContext() {
      if (!name.equals(RECOVER.name)) return null;
    ...
    public String toString() {
      if (this == ROLLINGUPGRADE) {
        return new StringBuilder(super.toString())
            .append("(")
            .append(getRollingUpgradeStartupOption())
            .append(")")
            .toString();
      }
  ...
  enum BlockUCState {
     ...
     * The block has at least the configured minimal replication number of {@link
     * ReplicaState#FINALIZED} replica(s), and is not going to be modified. NOTE, in some special
     * cases, a block may be forced to COMPLETE state, even if it doesn't have required minimal
     * replications.
     */
    COMPLETE,
    /**
     ...
     * When a file lease expires its last block may not be {@link #COMPLETE} and needs to go through
     * a recovery procedure, which synchronizes the existing replicas contents.
     */
    UNDER_RECOVERY,
    /**
     * The block is committed.<br>
     * The client reported that all bytes are written to data-nodes with the given generation stamp
     * and block length, but no {@link ReplicaState#FINALIZED} replicas has yet been reported by
  ...
  String CRYPTO_XATTR_ENCRYPTION_ZONE = "raw.hdfs.crypto.encryption.zone";
  String CRYPTO_XATTR_FILE_ENCRYPTION_INFO = "raw.hdfs.crypto.file.encryption.info";
  String SECURITY_XATTR_UNREADABLE_BY_SUPERUSER = "security.hdfs.unreadable.by.superuser";
  String XATTR_ERASURECODING_POLICY = "system.hdfs.erasurecoding.policy";
