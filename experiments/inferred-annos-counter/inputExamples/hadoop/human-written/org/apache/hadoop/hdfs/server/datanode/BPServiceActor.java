 ...
 *
 * <ul>
 *   <li>Pre-registration handshake with namenode
 *   <li>Registration with namenode
 *   <li>Send periodic heartbeats to the namenode
 *   <li>Handle commands received from the namenode
 * </ul>
 */
...
class BPServiceActor implements Runnable {
  ...
  BPServiceActor(
    ...
    this.ibrManager = new IncrementalBlockReportManager(dnConf.ibrInterval, dn.getMetrics());
    prevBlockReportId = ThreadLocalRandom.current().nextLong();
    fullBlockReportLeaseId = 0;
    scheduler =
        new Scheduler(
            dnConf.heartBeatInterval,
            dnConf.getLifelineIntervalMs(),
  ...
  /**
   * Perform the first part of the handshake with the NameNode. This calls <code>versionRequest
   * </code> to determine the NN's namespace and version info. It automatically retries until the NN
   * responds or the DN is shutting down.
   *
  ...
  private void checkNNVersion(NamespaceInfo nsInfo) throws IncorrectVersionException {
    ...
    if (VersionUtil.compareVersions(nnVersion, minimumNameNodeVersion) < 0) {
      IncorrectVersionException ive =
          new IncorrectVersionException(minimumNameNodeVersion, nnVersion, "NameNode", "DataNode");
    ...
    if (!nnVersion.equals(dnVersion)) {
      LOG.info(
          "Reported NameNode version '"
              + nnVersion
              + "' does not match "
              + "DataNode version '"
              + dnVersion
              + "' but is within acceptable "
  ...
  List<DatanodeCommand> blockReport(long fullBrLeaseId) throws IOException {
    ...
    boolean useBlocksBuffer =
        bpRegistration
            .getNamespaceInfo()
            .isCapabilitySupported(NamespaceInfo.Capability.STORAGE_BLOCK_REPORT_BUFFERS);
    ...
    try {
      ...
      LOG.info(
          (success ? "S" : "Uns")
              ...
              + Long.toHexString(reportId)
              + ",  containing "
              + reports.length
              + " storage report(s), of which we sent "
              + numReportsSent
              + "."
              + " The reports had "
              + totalBlockCount
              + " total blocks and used "
              + numRPCs
              + " RPC(s). This took "
              + brCreateCost
              + " msecs to generate and "
              + brSendCost
  ...
  HeartbeatResponse sendHeartBeat(boolean requestBlockReportLease) throws IOException {
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          "Sending heartbeat with "
              + reports.length
              + " storage reports from service actor: "
              + this);
    }
    ...
    int numFailedVolumes =
        volumeFailureSummary != null ? volumeFailureSummary.getFailedStorageLocations().length : 0;
  ...
  private String formatThreadName(final String action, final InetSocketAddress addr) {
  ...
  private void offerService() throws Exception {
    LOG.info(
        "For namenode "
            + nnAddr
            + " using"
            + " BLOCKREPORT_INTERVAL of "
            + dnConf.blockReportInterval
            + "msecs"
            + " CACHEREPORT_INTERVAL of "
      ...
      try {
        ...
        if (sendHeartbeat) {
          ...
          boolean requestBlockReportLease =
              (fullBlockReportLeaseId == 0) && scheduler.isBlockReportDue(startTime);
          if (!dn.areHeartbeatsDisabledForTests()) {
            ...
            if (resp.getFullBlockReportLeaseId() != 0) {
              if (fullBlockReportLeaseId != 0) {
                LOG.warn(
                    nnAddr
                        + " sent back a full block report lease "
                        + "ID of 0x"
            ...
            dn.getMetrics()
                .addHeartbeat(scheduler.monotonicNow() - startTime, getRpcMetricSuffix());
        ...
        if (!dn.areIBRDisabledForTests() && (ibrManager.sendImmediately() || sendHeartbeat)) {
          ibrManager.sendIBRs(
              bpNamenode, bpRegistration, bpos.getBlockPoolId(), getRpcMetricSuffix());
        }
        ...
        if (sendHeartbeat) {
          dn.getMetrics()
              .addHeartbeatTotal(scheduler.monotonicNow() - startTime, getRpcMetricSuffix());
        }
   ...
   *
   * <p>The bpDatanode needs to register with the namenode on startup in order 1) to report which
   * storage it is serving now and 2) to receive a registrationID
   *
   * <p>issued by the namenode to recognize registered datanodes.
   *
  ...
  /**
   * No matter what kind of exception we get, keep retrying to offerService(). That's the loop that
   * connects to the NameNode and provides basic DataNode functionality.
   *
   * <p>Only stop when "shouldRun" or "shouldServiceRun" is turned off, which can happen either at
   * shutdown or due to refreshNamenodes.
   */
  @Override
  public void run() {
  ...
  private final class LifelineSender implements Runnable, Closeable {
    ...
    public void start() {
      lifelineThread = new Thread(this, formatThreadName("lifeline", lifelineNnAddr));
    ...
    private void sendLifelineIfDue() throws IOException {
      ...
      if (!scheduler.isLifelineDue(startTime)) {
        if (LOG.isDebugEnabled()) {
          LOG.debug(
              "Skipping sending lifeline for " + BPServiceActor.this + ", because it is not due.");
        }
        return;
      }
      if (dn.areHeartbeatsDisabledForTests()) {
      ...
      sendLifeline();
      dn.getMetrics().addLifeline(scheduler.monotonicNow() - startTime, getRpcMetricSuffix());
    ...
    private void sendLifeline() throws IOException {
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            "Sending lifeline with "
                + reports.length
                + " storage "
                + " reports from service actor: "
                + BPServiceActor.this);
      }
      VolumeFailureSummary volumeFailureSummary = dn.getFSDataset().getVolumeFailureSummary();
      int numFailedVolumes =
          volumeFailureSummary != null
              ? volumeFailureSummary.getFailedStorageLocations().length
              : 0;
      lifelineNamenode.sendLifeline(
          bpRegistration,
          reports,
          dn.getFSDataset().getCacheCapacity(),
          dn.getFSDataset().getCacheUsed(),
  ...
  /**
   * Utility class that wraps the timestamp computations for scheduling heartbeats and block
   * reports.
   */
  static class Scheduler {
    ...
    @VisibleForTesting volatile long nextLifelineTime;

    @VisibleForTesting volatile long lastBlockReportTime = monotonicNow();

    @VisibleForTesting volatile long lastHeartbeatTime = monotonicNow();

    @VisibleForTesting boolean resetBlockReportTime = true;

    @VisibleForTesting volatile long nextOutliersReportTime = monotonicNow();
    ...
    Scheduler(
        ...
        long lifelineIntervalMs,
        long blockReportIntervalMs,
        long outliersReportIntervalMs) {
     ...
     *
     * @param delay specifies the maximum amount of random delay(in milliseconds) in sending the
     *     block report. A value of 0 or less makes the BR to go right away without any delay.
     * @param isRegistration if true, resets the future BRs for randomness, post first BR to avoid
     *     regular BRs from all DN's coming at one time.
     */
    long scheduleBlockReport(long delay, boolean isRegistration) {
      if (delay > 0) { // send BR after random delay
        // Numerical overflow is possible here and is okay.
        nextBlockReportTime = monotonicNow() + ThreadLocalRandom.current().nextInt((int) (delay));
    ...
    /**
     * Schedule the next block report after the block report interval. If the current block report
     * was delayed then the next block report is sent per the original schedule. Numerical overflow
     * is possible here.
     */
    void scheduleNextBlockReport() {
      ...
      if (resetBlockReportTime) {
        nextBlockReportTime =
            monotonicNow() + ThreadLocalRandom.current().nextInt((int) (blockReportIntervalMs));
         ...
         */
        long factor =
            (monotonicNow() - nextBlockReportTime + blockReportIntervalMs) / blockReportIntervalMs;
        if (factor != 0) {
    ...
    private boolean processCommand(DatanodeCommand[] cmds) {
      if (cmds != null) {
        ...
        if (processCommandsMs > dnConf.getProcessCommandsThresholdMs()) {
          LOG.info("Took {} ms to process {} commands from NN", processCommandsMs, cmds.length);
        }
      }
      return true;
    }
