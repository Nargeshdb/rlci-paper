...
/**
 * AclTransformation defines the operations that can modify an ACL. All ACL modifications take as
 * input an existing ACL and apply logic to add new entries, modify existing entries or remove old
 * entries. Some operations also accept an ACL spec: a list of entries that further describes the
 * requested change. Different operations interpret the ACL spec differently. In the case of adding
 * an ACL to an inode that previously did not have one, the existing ACL can be a "minimal ACL"
 * containing exactly 3 entries for owner, group and other, all derived from the {@link
 * FsPermission} bits.
 *
 * <p>The algorithms implemented here require sorted lists of ACL entries. For any existing ACL, it
 * is assumed that the entries are sorted. This is because all ACL creation and modification is
 * intended to go through these methods, and they all guarantee correct sort order in their outputs.
 * However, an ACL spec is considered untrusted user input, so all operations pre-sort the ACL spec
 * as the first step.
 */
...
final class AclTransformation {
  ...
  /**
   * Filters (discards) any existing ACL entries that have the same scope, type and name of any
   * entry in the ACL spec. If necessary, recalculates the mask entries. If necessary, default
   * entries may be inferred by copying the permissions of the corresponding access entries. It is
   * invalid to request removal of the mask entry from an ACL that would otherwise require a mask
  ...
  /**
   * Filters (discards) any existing default ACL entries. The new ACL retains only the access ACL
   * entries.
   *
   * @param existingAcl List<AclEntry> existing ACL
   * @return List<AclEntry> new ACL
   * @throws AclException if validation fails
   */
  public static List<AclEntry> filterDefaultAclEntries(List<AclEntry> existingAcl)
      throws AclException {
    ArrayList<AclEntry> aclBuilder = Lists.newArrayListWithCapacity(MAX_ENTRIES);
    for (AclEntry existingEntry : existingAcl) {
  ...
  /**
   * Merges the entries of the ACL spec into the existing ACL. If necessary, recalculates the mask
   * entries. If necessary, default entries may be inferred by copying the permissions of the
  ...
  /**
   * Completely replaces the ACL with the entries of the ACL spec. If necessary, recalculates the
   * mask entries. If necessary, default entries are inferred by copying the permissions of the
   * corresponding access entries. Replacement occurs separately for each of the access ACL and the
   * default ACL. If the ACL spec contains only access entries, then the existing default entries
   * are retained. If the ACL spec contains only default entries, then the existing access entries
   * are retained. If the ACL spec contains both access and default entries, then both are replaced.
   *
   * @param existingAcl List<AclEntry> existing ACL
   ...
   * @throws AclException if validation fails
   */
  public static List<AclEntry> replaceAclEntries(
      List<AclEntry> existingAcl, List<AclEntry> inAclSpec) throws AclException {
    ValidatedAclSpec aclSpec = new ValidatedAclSpec(inAclSpec);
    ArrayList<AclEntry> aclBuilder = Lists.newArrayListWithCapacity(MAX_ENTRIES);
  ...
  /**
   * Comparator that enforces required ordering for entries within an ACL: -owner entry (unnamed
   * user) -all named user entries (internal ordering undefined) -owning group entry (unnamed group)
   * -all named group entries (internal ordering undefined) -mask entry -other entry All access ACL
   * entries sort ahead of all default ACL entries.
   */
      ...
      new Comparator<AclEntry>() {
        ...
        public int compare(AclEntry entry1, AclEntry entry2) {
          return ComparisonChain.start()
              ...
              .compare(
                  entry1.getType(), entry2.getType(), Ordering.explicit(USER, GROUP, MASK, OTHER))
              .compare(entry1.getName(), entry2.getName(), Ordering.natural().nullsFirst())
              .result();
        }
      };

  /**
   * Builds the final list of ACL entries to return by trimming, sorting and validating the ACL
   ...
   * @throws AclException if validation fails
   */
  private static List<AclEntry> buildAndValidateAcl(ArrayList<AclEntry> aclBuilder)
      throws AclException {
    ...
    for (AclEntryType type : EnumSet.of(USER, GROUP, OTHER)) {
      ...
      if (Collections.binarySearch(
              scopedEntries.getAccessEntries(), accessEntryKey, ACL_ENTRY_COMPARATOR)
          < 0) {
      ...
      if (!scopedEntries.getDefaultEntries().isEmpty()) {
        ...
        if (Collections.binarySearch(
                scopedEntries.getDefaultEntries(), defaultEntryKey, ACL_ENTRY_COMPARATOR)
            < 0) {
  ...
  private static void checkMaxEntries(ScopedAclEntries scopedEntries) throws AclException {
    ...
    if (accessEntries.size() > MAX_ENTRIES) {
      throw new AclException(
          "Invalid ACL: ACL has "
              + accessEntries.size()
              + " access entries, which exceeds maximum of "
              + MAX_ENTRIES
              + ".");
    }
    if (defaultEntries.size() > MAX_ENTRIES) {
      throw new AclException(
          "Invalid ACL: ACL has "
              + defaultEntries.size()
              + " default entries, which exceeds maximum of "
              + MAX_ENTRIES
              + ".");
    }
  }
   ...
   * scope: access and default. This method is responsible for handling the following cases of mask
   * calculation: 1. Throws an exception if the caller attempts to remove the mask entry of an
   * existing ACL that requires it. If the ACL has any named entries, then a mask entry is required.
   * 2. If the caller supplied a mask in the ACL spec, use it. 3. If the caller did not supply a
   * mask, but there are ACL entry changes in this scope, then automatically calculate a new mask.
   * The permissions of the new mask are the union of the permissions on the group entry and all
   * named entries.
   *
   * @param aclBuilder ArrayList<AclEntry> containing entries to build
   * @param providedMask EnumMap<AclEntryScope, AclEntry> mapping each scope to the mask entry that
   *     was provided for that scope (if provided)
   * @param maskDirty EnumSet<AclEntryScope> which contains a scope if the mask entry is dirty
   *     (added or deleted) in that scope
   * @param scopeDirty EnumSet<AclEntryScope> which contains a scope if any entry is dirty (added or
   *     deleted) in that scope
   * @throws AclException if validation fails
   */
  private static void calculateMasks(
      List<AclEntry> aclBuilder,
      EnumMap<AclEntryScope, AclEntry> providedMask,
      EnumSet<AclEntryScope> maskDirty,
      EnumSet<AclEntryScope> scopeDirty)
      throws AclException {
    ...
    for (AclEntryScope scope : scopeFound) {
      if (!providedMask.containsKey(scope)
          && maskNeeded.contains(scope)
  ...
  /**
   * Adds unspecified default entries by copying permissions from the corresponding access entries.
   *
   * @param aclBuilder ArrayList<AclEntry> containing entries to build
   */
    ...
    if (!scopedEntries.getDefaultEntries().isEmpty()) {
      ...
      for (AclEntryType type : EnumSet.of(USER, GROUP, OTHER)) {
        ...
        int defaultEntryIndex =
            Collections.binarySearch(defaultEntries, defaultEntryKey, ACL_ENTRY_COMPARATOR);
        if (defaultEntryIndex < 0) {
          AclEntry accessEntryKey = new AclEntry.Builder().setScope(ACCESS).setType(type).build();
          int accessEntryIndex =
              Collections.binarySearch(accessEntries, accessEntryKey, ACL_ENTRY_COMPARATOR);
  ...
  private static final class ValidatedAclSpec implements Iterable<AclEntry> {
    ...
    /**
     * Creates a ValidatedAclSpec by pre-validating and sorting the given ACL entries.
     * Pre-validation checks that it does not exceed the maximum entries. This check is performed
     * before modifying the ACL, and it's actually insufficient for enforcing the maximum number of
     * entries. Transformation logic can create additional entries automatically,such as the mask
     * and some of the default entries, so we also need additional checks during transformation. The
     * up-front check is still valuable here so that we don't run a lot of expensive transformation
     * logic while holding the namesystem lock for an attacker who intentionally sent a huge ACL
     * spec.
     *
    ...
    /**
     * Returns true if this contains an entry matching the given key. An ACL entry's key consists of
     * scope, type and name (but not permission).
     *
    ...
    /**
     * Returns the entry matching the given key or null if not found. An ACL entry's key consists of
     * scope, type and name (but not permission).
     *
     * @param key AclEntry search key
