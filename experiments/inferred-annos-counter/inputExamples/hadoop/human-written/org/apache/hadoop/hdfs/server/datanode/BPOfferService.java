...
/**
 * One instance per block-pool/namespace on the DN, which handles the heartbeats to the active and
 * standby NNs for that namespace. This class manages an instance of {@link BPServiceActor} for each
 * NN, and delegates calls to both NNs. It also maintains the state about which of the NNs is
 * considered active.
 */
...
class BPOfferService {
  ...
  /**
   * Information about the namespace that this service is registering with. This is assigned after
   * the first phase of the handshake.
   */
  NamespaceInfo bpNSInfo;

  /**
   * The registration information for this block pool. This is assigned after the second phase of
   * the handshake.
   */
  ...
  /**
   * A reference to the BPServiceActor associated with the currently ACTIVE NN. In the case that all
   * NameNodes are in STANDBY mode, this can be null. If non-null, this must always refer to a
   * member of the {@link #bpServices} list.
   */
  private BPServiceActor bpServiceToActive = null;

  /**
   * The list of all actors for namenodes in this nameservice, regardless of their active or standby
   * states.
   */
  private final List<BPServiceActor> bpServices = new CopyOnWriteArrayList<BPServiceActor>();

  /**
   * Each time we receive a heartbeat from a NN claiming to be ACTIVE, we record that NN's most
   * recent transaction ID here, so long as it is more recent than the previous value. This allows
   * us to detect split-brain scenarios in which a prior NN is still asserting its ACTIVE state but
   * with a too-low transaction ID. See HDFS-2627 for details.
   */
  ...
  BPOfferService(
    ...
    Preconditions.checkArgument(
        nnAddrs.size() == lifelineNnAddrs.size(),
    ...
    for (int i = 0; i < nnAddrs.size(); ++i) {
      this.bpServices.add(
          new BPServiceActor(
              nameserviceId, nnIds.get(i), nnAddrs.get(i), lifelineNnAddrs.get(i), this));
    }
  }
  ...
  void refreshNNList(
    ...
    for (InetSocketAddress addedNN : addedNNs) {
      BPServiceActor actor =
          new BPServiceActor(
              serviceId,
              nnIds.get(addrs.indexOf(addedNN)),
              addedNN,
              lifelineAddrs.get(addrs.indexOf(addedNN)),
              this);
      actor.start();
  ...
  void notifyNamenodeReceivedBlock(
    ...
    notifyNamenodeBlock(
        block, BlockStatus.RECEIVED_BLOCK, delHint, storageUuid, isOnTransientStorage);
  }

  void notifyNamenodeReceivingBlock(ExtendedBlock block, String storageUuid) {
    notifyNamenodeBlock(block, BlockStatus.RECEIVING_BLOCK, null, storageUuid, false);
  }

  void notifyNamenodeDeletedBlock(ExtendedBlock block, String storageUuid) {
    notifyNamenodeBlock(block, BlockStatus.DELETED_BLOCK, null, storageUuid, false);
  }

  private void notifyNamenodeBlock(
      ExtendedBlock block,
      BlockStatus status,
      String delHint,
      String storageUuid,
    ...
    checkBlock(block);
    final ReceivedDeletedBlockInfo info =
        new ReceivedDeletedBlockInfo(block.getLocalBlock(), status, delHint);
  ...
  private void checkBlock(ExtendedBlock block) {
    ...
    Preconditions.checkArgument(
        block.getBlockPoolId().equals(bpId),
  ...
  /**
   * Called by the BPServiceActors when they handshake to a NN. If this is the first NN connection,
   * this sets the namespace info for this BPOfferService. If it's a connection to a new NN, it
   * verifies that this namespace matches (eg to prevent a misconfiguration where a StandbyNode from
   * a different cluster is specified)
   */
  void verifyAndSetNamespaceInfo(BPServiceActor actor, NamespaceInfo nsInfo) throws IOException {
    writeLock();

    if (nsInfo.getState() == HAServiceState.ACTIVE && bpServiceToActive == null) {
  ...
  /**
   * After one of the BPServiceActors registers successfully with the NN, it calls this function to
   * verify that the NN it connected to is consistent with other NNs serving the block-pool.
   */
  void registrationSucceeded(BPServiceActor bpServiceActor, DatanodeRegistration reg)
      throws IOException {
    writeLock();
    try {
      if (bpRegistration != null) {
        checkNSEquality(
            bpRegistration.getStorageInfo().getNamespaceID(),
            reg.getStorageInfo().getNamespaceID(),
            "namespace ID");
        checkNSEquality(
            bpRegistration.getStorageInfo().getClusterID(),
            reg.getStorageInfo().getClusterID(),
            "cluster ID");
      }
      bpRegistration = reg;
  ...
  DatanodeRegistration createRegistration() {
    ...
    try {
      Preconditions.checkState(
          bpNSInfo != null, "getRegistration() can only be called after initial handshake");
  ...
  /**
   * Called when an actor shuts down. If this is the last actor to shut down, shuts down the whole
   * blockpool in the DN.
   */
  ...
  /** Ask each of the actors to schedule a block report after the specified delay. */
  ...
  /**
   * @return a proxy to the active NN, or null if the BPOS has not acknowledged any NN as active
   *     yet.
   */
  ...
  /**
   * Update the BPOS's view of which NN is active, based on a heartbeat response from one of the
   * actors.
   *
      ...
      if (nnClaimsActive && !bposThinksActive) {
        ...
        if (!isMoreRecentClaim) {
          ...
          LOG.warn(
              "NN "
                  + actor
                  + " tried to claim ACTIVE state at txid="
                  + txid
                  + " but there was already a more recent claim at txid="
                  + lastActiveClaimTxId);
          return;
        } else {
          if (bpServiceToActive == null) {
  ...
  boolean processCommandFromActor(DatanodeCommand cmd, BPServiceActor actor) throws IOException {
    ...
    if (DatanodeProtocol.DNA_REGISTER == cmd.getAction()) {
      ...
      LOG.info(
          "DatanodeCommand action : DNA_REGISTER from "
              + actor.nnAddr
              + " with "
              + actor.state
              + " state");
  ...
  /**
   * This method should handle all commands from Active namenode except DNA_REGISTER which should be
   * handled earlier itself.
   *
   * @param cmd
  ...
  private boolean processCommandFromActive(DatanodeCommand cmd, BPServiceActor actor)
    ...
    switch (cmd.getAction()) {
      ...
      case DatanodeProtocol.DNA_CACHE:
        LOG.info(
            "DatanodeCommand action: DNA_CACHE for "
                + blockIdCmd.getBlockPoolId()
                + " of ["
      ...
      case DatanodeProtocol.DNA_UNCACHE:
        LOG.info(
            "DatanodeCommand action: DNA_UNCACHE for "
                + blockIdCmd.getBlockPoolId()
                + " of ["
                + blockIdArrayToString(blockIdCmd.getBlockIds())
                + "]");
      ...
      case DatanodeProtocol.DNA_FINALIZE:
        ...
        assert getBlockPoolId().equals(bp)
            : "BP " + getBlockPoolId() + " received DNA_FINALIZE " + "for other block pool " + bp;

        dn.finalizeUpgradeForPool(bp);
        break;
      case DatanodeProtocol.DNA_RECOVERBLOCK:
        String who = "NameNode at " + actor.getNNSocketAddress();
        dn.getBlockRecoveryWorker()
            .recoverBlocks(who, ((BlockRecoveryCommand) cmd).getRecoveringBlocks());
        break;
      case DatanodeProtocol.DNA_ACCESSKEYUPDATE:
        LOG.info("DatanodeCommand action: DNA_ACCESSKEYUPDATE");
        if (dn.isBlockTokenEnabled) {
          dn.blockPoolTokenSecretManager.addKeys(
              getBlockPoolId(), ((KeyUpdateCommand) cmd).getExportedKeys());
        }
        break;
      case DatanodeProtocol.DNA_BALANCERBANDWIDTHUPDATE:
        ...
        if (bandwidth > 0) {
          ...
          LOG.info(
              "Updating balance throttler bandwidth from "
                  + dxcs.balanceThrottler.getBandwidth()
                  + " bytes/s "
                  + "to: "
                  + bandwidth
                  + " bytes/s.");
  ...
  /**
   * This method should handle commands from Standby namenode except DNA_REGISTER which should be
   * handled earlier itself.
   */
  private boolean processCommandFromStandby(DatanodeCommand cmd, BPServiceActor actor)
      throws IOException {
    switch (cmd.getAction()) {
      case DatanodeProtocol.DNA_ACCESSKEYUPDATE:
