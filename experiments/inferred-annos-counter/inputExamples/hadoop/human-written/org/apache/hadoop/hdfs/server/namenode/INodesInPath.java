...
public class INodesInPath {
  ...
  /**
   * Construct {@link INodesInPath} from {@link INode} and its root {@link INodeDirectory}.
   * INodesInPath constructed this way will each have its snapshot and latest snapshot id filled in.
   *
   * <p>This routine is specifically for {@link LeaseManager#getINodeWithLeases(INodeDirectory)} to
   * get open files along with their snapshot details which is used during new snapshot creation to
   * capture their meta data.
   *
   * @param rootDir the root {@link INodeDirectory} under which inode needs to be resolved
  ...
  /**
   * Retrieve existing INodes from a path. The number of INodes is equal to the number of path
   * components. For a snapshot path (e.g. /foo/.snapshot/s1/bar), the ".snapshot/s1" will be
   * represented in one path component corresponding to its Snapshot.Root inode. This 1-1 mapping
   * ensures the path can always be properly reconstructed.
   *
   * <p>Example: <br>
   * Given the path /c1/c2/c3 where only /c1/c2 exists, resulting in the following path components:
   * ["","c1","c2","c3"]
   *
   * <p><code>getExistingPathINodes(["","c1","c2"])</code> should fill the array with
   * [rootINode,c1,c2], <br>
   * <code>getExistingPathINodes(["","c1","c2","c3"])</code> should fill the array with
   * [rootINode,c1,c2,null]
   *
  ...
  static INodesInPath resolve(
      final INodeDirectory startingDir, byte[][] components, final boolean isRaw) {
    ...
    while (count < components.length && curNode != null) {
      ...
      if (!isRef && isDir && dir.isWithSnapshot()) {
        ...
        if (!isSnapshot
            && shouldUpdateLatestId(
                dir.getDirectoryWithSnapshotFeature().getLastSnapshotId(), snapshotId)) {
      ...
      if (isDotSnapshotDir(childName) && dir.isSnapshottable()) {
        ...
        componentsCopy[count] =
            DFSUtil.string2Bytes(DFSUtil.byteArray2PathString(components, count, 2));
  ...
  private static boolean shouldUpdateLatestId(int sid, int snapshotId) {
    return snapshotId == CURRENT_STATE_ID
        || (sid != CURRENT_STATE_ID && ID_INTEGER_COMPARATOR.compare(snapshotId, sid) < 0);
  }

  /**
   * Replace an inode of the given INodesInPath in the given position. We do a deep copy of the
   * INode array.
   *
  ...
  /**
   * Extend a given INodesInPath with a child INode. The child INode will be appended to the end of
   * the new INodesInPath.
   */
  ...
  /**
   * true if this is a /.reserved/raw path. path component resolution strips it from the path so
   * need to track it separately.
   */
  private final boolean isRaw;

  /**
   * For snapshot paths, it is the id of the snapshot; or {@link Snapshot#CURRENT_STATE_ID} if the
   * snapshot does not exist. For non-snapshot paths, it is the id of the latest snapshot found in
   * the path; or {@link Snapshot#CURRENT_STATE_ID} if no snapshot is found.
   */
  private final int snapshotId;

  private INodesInPath(
      INode[] inodes, byte[][] path, boolean isRaw, boolean isSnapshot, int snapshotId) {
  ...
  /**
   * For snapshot paths, return the id of the snapshot specified in the path. For non-snapshot
   * paths, return {@link Snapshot#CURRENT_STATE_ID}.
   */
  ...
  /**
   * @return the i-th inode if i {@literal >=} 0; otherwise, i {@literal <} 0, return the (length +
   *     i)-th inode.
   */
  ...
  /**
   * @param length number of ancestral INodes in the returned INodesInPath instance
   * @return the INodesInPath instance containing ancestral INodes. Note that this method only
   *     handles non-snapshot paths.
   */
  ...
  /**
   * @return an INodesInPath instance containing all the INodes in the parent path. We do a deep
   *     copy here.
   */
  ...
  /**
   * Verify if this {@link INodesInPath} is a descendant of the requested {@link INodeDirectory}.
   *
  ...
  /**
   * @return a new INodesInPath instance that only contains existing INodes. Note that this method
   *     only handles non-snapshot paths.
   */
    ...
    if (inodes[i] != null) {
      for (i++; i < inodes.length && inodes[i] != null; i++) {
        ...
        if (parent_i != inodes[i - 1]
            && (parent_i_1 == null || !parent_i_1.isSnapshottable() || parent_i != parent_i_1)) {
          throw new AssertionError(
              "inodes["
                  ...
                  + i
                  + "]="
                  + inodes[i].toDetailString()
                  + "\n  inodes["
                  + (i - 1)
                  + "]="
                  + inodes[i - 1].toDetailString()
                  + "\n this="
                  + toString(false));
        }
      }
