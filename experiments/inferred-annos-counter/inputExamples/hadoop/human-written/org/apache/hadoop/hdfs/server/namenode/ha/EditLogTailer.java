 ...
 * transactions contained within to a given FSNamesystem.
 */
...
public class EditLogTailer {
  ...
  /**
   * StandbyNode will hold namesystem lock to apply at most this many journal transactions. It will
   * then release the lock and re-acquire it to load more transactions. By default the write lock is
   * held for the entire journal segment. Fine-grained locking allows read requests to get through.
   */
  ...
  /** The last time we successfully loaded a non-zero number of edits from the shared directory. */
  ...
  /**
   * How often the Standby should roll edit logs. Since the Standby only reads from finalized log
   * segments, the Standby will only be as up-to-date as how often the logs are rolled.
   */
  ...
  /**
   * How often the tailer should check if there are new edit log entries ready to be consumed. This
   * is the initial delay before any backoff.
   */
  private final long sleepTimeMs;
  /**
   * The maximum time the tailer should wait between checking for new edit log entries. Exponential
   * backoff will be applied when an edit log tail is performed but no edits are available to be
   * read. If this is less than or equal to 0, backoff is disabled.
   */
  ...
  /**
   * Whether the tailer should tail the in-progress edit log segments. If true, this will also
   * attempt to optimize for latency when tailing the edit logs (if using the {@link
   * org.apache.hadoop.hdfs.qjournal.client.QuorumJournalManager}, this implies using the RPC-based
   * mechanism to tail edits).
   */
  private final boolean inProgressOk;

  /**
   * Release the namesystem lock after loading this many transactions. Then re-acquire the lock to
   * load more edits.
   */
  ...
  public EditLogTailer(FSNamesystem namesystem, Configuration conf) {
    ...
    if (logRollPeriodMs >= 0) {
      ...
      for (RemoteNameNodeInfo info : nns) {
        ...
        Preconditions.checkArgument(
            ipc.getPort() > 0,
            "Active NameNode must have an IPC port configured. " + "Got address '%s'",
            ipc);
      ...
      LOG.info(
          "Not going to trigger log rolls on active node because "
              + DFSConfigKeys.DFS_HA_LOGROLL_PERIOD_KEY
              + " is negative.");
    }
    ...
    if (maxRetries <= 0) {
      LOG.error(
          "Specified a non-positive number of retries for the number of retries for the "
    ...
    this.maxTxnsPerLock =
        conf.getLong(
            DFS_HA_TAILEDITS_MAX_TXNS_PER_LOCK_KEY, DFS_HA_TAILEDITS_MAX_TXNS_PER_LOCK_DEFAULT);
  ...
  public void catchupDuringFailover() throws IOException {
    Preconditions.checkState(
        tailerThread == null || !tailerThread.isAlive(),
  ...
  public long doTailEdits() throws IOException, InterruptedException {
    ...
    try {
      ...
      long editsLoaded = 0;
      try {
        editsLoaded = image.loadEdits(streams, namesystem, maxTxnsPerLock, null, null);
        ...
        if (editsLoaded > 0 || LOG.isDebugEnabled()) {
          LOG.debug(
              String.format("Loaded %d edits starting from txid %d ", editsLoaded, lastTxnId));
        }
  ...
  private boolean tooLongSinceLastLoad() {
    return logRollPeriodMs >= 0 && (monotonicNow() - lastRollTimeMs) > logRollPeriodMs;
  }
  ...
  /**
   * The thread which does the actual work of tailing edits journals and applying the transactions
   * to the FSNS.
   */
    ...
    private void doWork() {
      ...
      while (shouldRun) {
        ...
        try {
          ...
          /**
           * Check again in case someone calls {@link EditLogTailer#stop} while we're triggering an
           * edit log roll, since ipc.Client catches and ignores {@link InterruptedException} in a
           * few places. This fixes the bug described in HDFS-2823.
           */
          if (!shouldRun) {
            break;
          }
          ...
          LOG.error(
              "Unknown error encountered while tailing edits. " + "Shutting down standby NN.", t);
          terminate(1, t);
        }
        ...
        try {
          if (editsTailed == 0 && maxSleepTimeMs > 0) {
            ...
            currentSleepTimeMs =
                Math.min(maxSleepTimeMs, (currentSleepTimeMs == 0 ? 1 : currentSleepTimeMs) * 2);
          } else {
   ...
   *
   * <p>We - lazily - get a proxy to one of the configured namenodes and attempt to make the request
   ...
   *
   * <p>This mechanism is <b>very bad</b> for cases where we care about being <i>fast</i>; it just
    ...
    public T call() throws IOException {
      ...
      while ((cachedActiveProxy = getActiveNodeProxy()) != null) {
        try {
          ...
          LOG.warn("Exception from remote name node " + currentNN + ", try next.", e);
