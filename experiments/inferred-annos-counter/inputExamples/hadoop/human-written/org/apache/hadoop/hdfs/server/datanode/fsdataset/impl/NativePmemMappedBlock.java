...
public class NativePmemMappedBlock implements MappableBlock {
  ...
  public void close() {
    if (pmemMappedAddress != -1L) {
      try {
        String cacheFilePath = PmemVolumeManager.getInstance().getCachePath(key);
        ...
        // by pmem_map_file.
        boolean success = NativeIO.POSIX.Pmem.unmapBlock(pmemMappedAddress, length);
        if (!success) {
        ...
        LOG.info(
            "Successfully uncached one replica:{} from persistent memory"
                + ", [cached path={}, length={}]",
            key,
            cacheFilePath,
            length);
