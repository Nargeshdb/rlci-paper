...
/**
 * Indicates that SASL protocol negotiation expected to read a pre-defined magic number, but the
 * expected value was not seen.
 */
...
public class InvalidMagicNumberException extends IOException {
  ...
  public InvalidMagicNumberException(final int magicNumber, final boolean handshake4Encryption) {
    super(
        String.format(
            "Received %x instead of %x from client.", magicNumber, SASL_TRANSFER_MAGIC_NUMBER));
