...
/**
 * A double-buffer for edits. New edits are written into the first buffer while the second is
 * available to be flushed. Each time the double-buffer is flushed, the two internal buffers are
 * swapped. This allows edits to progress concurrently to flushes without allocating new buffers
 * each time.
 */
...
public class EditsDoubleBuffer {
  ...
  public void close() throws IOException {
    ...
    if (bufSize != 0) {
      ...
      throw new IOException(
          "FSEditStream has " + bufSize + " bytes still to be flushed and cannot be closed.");
    }
  ...
  /**
   * Writes the content of the "ready" buffer to the given output stream, and resets it. Does not
   * swap any buffers.
   */
  ...
  private static class TxnBuffer extends DataOutputBuffer {
    ...
    private void dumpRemainingEditLogs() {
      ...
      FSEditLogOp.Reader reader =
          FSEditLogOp.Reader.create(dis, tracker, NameNodeLayoutVersion.CURRENT_LAYOUT_VERSION);
      FSEditLogOp op;
      LOG.warn(
          "The edits buffer is "
              + size()
      ...
      try {
        ...
        LOG.warn(
            "Unable to dump remaining ops. Remaining raw bytes: "
                + Hex.encodeHexString(remainingRawEdits),
            ioe);
      }
    }
  }
