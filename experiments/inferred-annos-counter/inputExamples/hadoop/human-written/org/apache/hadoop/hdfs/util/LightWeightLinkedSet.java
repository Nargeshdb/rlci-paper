 ...
 * linked lists for collision resolution. In addition it stores elements in a linked list to ensure
 * ordered traversal. This class does not support null element.
 *
...
public class LightWeightLinkedSet<T> extends LightWeightHashSet<T> {
   ...
   * corresponds to the order in which they were inserted.
   *
   * @return first element
   */
  @Override
  ...
  /**
   * Remove all elements from the set and return them in order. Traverse the link list, don't worry
   * about hashtable - faster version of the parent method.
   */
  @Override
    ...
    public T next() {
      if (modification != startModification) {
        throw new ConcurrentModificationException(
            "modification=" + modification + " != startModification = " + startModification);
      }
      if (next == null) {
