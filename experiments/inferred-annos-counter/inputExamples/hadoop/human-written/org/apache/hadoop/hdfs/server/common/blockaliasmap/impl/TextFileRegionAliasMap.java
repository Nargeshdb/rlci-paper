...
public class TextFileRegionAliasMap extends BlockAliasMap<FileRegion> implements Configurable {
  ...
  @VisibleForTesting
  TextReader createReader(Path file, String delim, Configuration cfg, String blockPoolID)
      throws IOException {
  ...
  @VisibleForTesting
  @SuppressWarnings(
      "builder:required.method.not.called") // TP: tmp remains open if codec.createOutputStream(tmp)
                                            // throws an exception
  TextWriter createWriter(Path file, CompressionCodec codec, String delim, Configuration cfg)
      throws IOException {
    FileSystem fs = file.getFileSystem(cfg);
    if (fs instanceof LocalFileSystem) {
      fs = ((LocalFileSystem) fs).getRaw();
    ...
    java.io.Writer out =
        new BufferedWriter(
            new OutputStreamWriter((null == codec) ? tmp : codec.createOutputStream(tmp), "UTF-8"));
  ...
  public static class ReaderOptions implements TextReader.Options, Configurable {
    ...
    public void setConf(Configuration conf) {
      ...
      String tmpfile =
          conf.get(
              DFSConfigKeys.DFS_PROVIDED_ALIASMAP_TEXT_READ_FILE,
              DFSConfigKeys.DFS_PROVIDED_ALIASMAP_TEXT_READ_FILE_DEFAULT);
      file = new Path(tmpfile);
      delim =
          conf.get(
              DFSConfigKeys.DFS_PROVIDED_ALIASMAP_TEXT_DELIMITER,
  ...
  public static class WriterOptions implements TextWriter.Options, Configurable {
    ...
    public void setConf(Configuration conf) {
      this.conf = conf;
      String tmpDir = conf.get(DFSConfigKeys.DFS_PROVIDED_ALIASMAP_TEXT_WRITE_DIR, dir.toString());
      dir = new Path(tmpDir);
  ...
  public static class TextReader extends Reader<FileRegion> {
    ...
    @SuppressWarnings({
      "builder:required.method.not.called", // FP container of owners: can't handle Map.get()
                                            // (validated)
      "builder:required.method.not.called" // FP container of owners: can't handle Map.remove()
                                           // (validated)
    })
    ...
    @SuppressWarnings(
        "builder:required.method.not.called") // TP: i remains open in possible exceptional exit due
                                              // to codec.createInputStream(i) (validated)
    ...
    @Override
    @SuppressWarnings({
      "builder:required.method.not.called", // FP container of owners: ownership of r is transferred
                                            // to map (validated)
      "builder:required.method.not.called" // FP container of owners: Can't handle Map.put()
                                           // (validated)
    })
    public Iterator<FileRegion> iterator() {
      FRIterator i = new FRIterator();
      try {
        BufferedReader r = new BufferedReader(new InputStreamReader(createStream(), "UTF-8"));
        iterators.put(i, r);
    ...
    public void close() throws IOException {
      ...
      synchronized (iterators) {
        for (Iterator<BufferedReader> i = iterators.values().iterator(); i.hasNext(); ) {
          try {
  ...
  public static class TextWriter extends Writer<FileRegion> {
    ...
    public void store(FileRegion token) throws IOException {
      ...
      if (psl.getNonce().length > 0) {
        out.append(delim).append(Base64.getEncoder().encodeToString(psl.getNonce()));
      }
      out.append("\n");
    }
