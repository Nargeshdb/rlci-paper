...
/**
 * Negotiates SASL for DataTransferProtocol on behalf of a server. There are two possible supported
 * variants of SASL negotiation: either a general-purpose negotiation supporting any quality of
 * protection, or a specialized negotiation that enforces privacy as the quality of protection using
 * a cryptographically strong encryption key.
 *
 * <p>This class is used in the DataNode for handling inbound connections.
 */
...
public class SaslDataTransferServer {
   ...
   * @param blockPoolTokenSecretManager used for checking block access tokens and encryption keys
   */
  public SaslDataTransferServer(
      DNConf dnConf, BlockPoolTokenSecretManager blockPoolTokenSecretManager) {
  ...
  public IOStreamPair receive(
    ...
    if (dnConf.getEncryptDataTransfer()) {
      LOG.debug(
          "SASL server doing encrypted handshake for peer = {}, datanodeId = {}", peer, datanodeId);
      ...
      LOG.debug(
          "SASL server doing general handshake for peer = {}, datanodeId = {}", peer, datanodeId);
   ...
   * @throws IOException for any error
   */
  private IOStreamPair getEncryptedStreams(
      Peer peer, OutputStream underlyingOut, InputStream underlyingIn) throws IOException {
    ...
    Map<String, String> saslProps =
        createSaslPropertiesForEncryption(dnConf.getEncryptionAlgorithm());
  ...
  /**
   * The SASL handshake for encrypted vs. general-purpose uses different logic for determining the
   * password. This interface is used to parameterize that logic. It's similar to a Guava Function,
   * but we need to let it throw exceptions.
   */
      ...
      for (Callback callback : callbacks) {
        if (callback instanceof AuthorizeCallback) {
          ...
          throw new UnsupportedCallbackException(
              callback, "Unrecognized SASL DIGEST-MD5 Callback: " + callback);
        }
      }
  ...
  /**
   * Given a secret manager and a username encoded for the encrypted handshake, determine the
   * encryption key.
   *
   ...
   * @throws IOException for any error
   */
  private IOStreamPair getSaslStreams(
      Peer peer, OutputStream underlyingOut, InputStream underlyingIn) throws IOException {
    if (peer.hasSecureChannel()
        || dnConf.getTrustedChannelResolver().isTrusted(getPeerAddress(peer))) {
      return new IOStreamPair(underlyingIn, underlyingOut);
  ...
  /**
   * Calculates the expected correct password on the server side for the general-purpose handshake.
   * The password consists of the block access token's password (known to the DataNode via its
   * secret manager). This expects that the client has supplied a user name consisting of its
   * serialized block access token identifier.
   *
   * @param userName SASL user name containing serialized block access token identifier
  ...
  /**
   * Deserializes a base64-encoded binary representation of a block access token.
   *
   ...
   * @throws IOException for any error
   */
  private IOStreamPair doSaslHandshake(
      Peer peer,
      OutputStream underlyingOut,
      InputStream underlyingIn,
      Map<String, String> saslProps,
    ...
    try {
      ...
      SaslParticipant sasl =
          SaslParticipant.createServerSaslParticipant(saslProps, callbackHandler);
   ...
   * @throws IOException for any error
   */
  private static void sendInvalidKeySaslErrorMessage(DataOutputStream out, String message)
      throws IOException {
