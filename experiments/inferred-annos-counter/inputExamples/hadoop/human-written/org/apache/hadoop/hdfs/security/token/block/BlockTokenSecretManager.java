...
/**
 * BlockTokenSecretManager can be instantiated in 2 modes, master mode and worker mode. Master can
 * generate new block keys and export block keys to workers, while workers can only import and use
 * block keys received from master. Both master and worker can generate and verify block tokens.
 * Typically, master mode is used by NN and worker mode is used by DN.
 */
@InterfaceAudience.Private
public class BlockTokenSecretManager extends SecretManager<BlockTokenIdentifier> {
  public static final Logger LOG = LoggerFactory.getLogger(BlockTokenSecretManager.class);
  ...
  /**
   * keyUpdateInterval is the interval that NN updates its block keys. It should be set long enough
   * so that all live DN's and Balancer should have sync'ed their block keys with NN at least once
  ...
  /** Timer object for querying the current time. Separated out for unit testing. */
  private Timer timer;
  /**
   ...
   */
  public BlockTokenSecretManager(
   ...
   */
  private BlockTokenSecretManager(
      boolean isMaster,
      long keyUpdateInterval,
      long tokenLifetime,
      String blockPoolId,
    ...
    LOG.info("Block token key range: [{}, {})", nnRangeStart, nnRangeStart + intRange);
    generateKeys();
  }
  ...
  public synchronized void setSerialNo(int nextNo) {
    ...
    assert serialNo >= nnRangeStart && serialNo < (nnRangeStart + intRange)
        : "serialNo "
            + serialNo
            + " is not in the designated range: ["
            + nnRangeStart
            + ", "
            + (nnRangeStart + intRange)
            + ")";
  }
  ...
  private synchronized void generateKeys() {
    ...
    currentKey =
        new BlockKey(
            serialNo, timer.now() + 2 * keyUpdateInterval + tokenLifetime, generateSecret());
    setSerialNo(serialNo + 1);
    nextKey =
        new BlockKey(
            serialNo, timer.now() + 3 * keyUpdateInterval + tokenLifetime, generateSecret());
  ...
  public synchronized ExportedBlockKeys exportKeys() {
    ...
    return new ExportedBlockKeys(
        ...
        tokenLifetime,
        currentKey,
        allKeys.values().toArray(new BlockKey[0]));
  }

  private synchronized void removeExpiredKeys() {
    long now = timer.now();
    for (Iterator<Map.Entry<Integer, BlockKey>> it = allKeys.entrySet().iterator();
        it.hasNext(); ) {
  ...
  synchronized boolean updateKeys() throws IOException {
    ...
    currentKey =
        new BlockKey(
            nextKey.getKeyId(),
            timer.now() + 2 * keyUpdateInterval + tokenLifetime,
            nextKey.getKey());
    allKeys.put(currentKey.getKeyId(), currentKey);
  ...
  /**
   * Check if access should be allowed. userID is not checked if null. This method doesn't check if
   * token password is correct. It should be used only when token password has already been verified
   * (e.g., in the RPC layer).
   *
   * <p>Some places need to check the access using StorageTypes and for other places the
   * StorageTypes is not relevant.
   */
  ...
  public void checkAccess(
    ...
    if (userId != null && !userId.equals(id.getUserId())) {
      throw new InvalidToken("Block token with " + id + " doesn't belong to user " + userId);
    }
    if (!id.getBlockPoolId().equals(block.getBlockPoolId())) {
      throw new InvalidToken("Block token with " + id + " doesn't apply to block " + block);
    }
    if (id.getBlockId() != block.getBlockId()) {
      throw new InvalidToken("Block token with " + id + " doesn't apply to block " + block);
    }
    if (isExpired(id.getExpiryDate())) {
      throw new InvalidToken("Block token with " + id + " is expired.");
    }
    if (!id.getAccessModes().contains(mode)) {
      throw new InvalidToken("Block token with " + id + " doesn't have " + mode + " permission");
    }
  }

  /**
   * Check if the requested values can be satisfied with the values in the BlockToken. This is
   * intended for use with StorageTypes and StorageIDs.
   *
   * <p>The current node can only verify that one of the storage [Type|ID] is available. The rest
  ...
  public static <T> void checkAccess(T[] candidates, T[] requested, String msg)
    ...
    if (ArrayUtils.isEmpty(requested)) {
      throw new InvalidToken(
          "The request has no " + msg + ". " + "This is probably a configuration error.");
    }
  ...
  public void checkAccess(
    ...
    try {
      ...
      throw new InvalidToken(
          "Unable to de-serialize block token identifier for user="
              + userId
              + ", block="
              + block
              + ", access mode="
    ...
    if (!Arrays.equals(retrievePassword(id), token.getPassword())) {
      throw new InvalidToken("Block token with " + id + " doesn't have the correct token password");
    }
  }
  ...
  /**
   * check if a token is expired. for unit test only. return true when token is expired, false
   * otherwise
   */
  ...
  public byte[] retrievePassword(BlockTokenIdentifier identifier) throws InvalidToken {
    if (isExpired(identifier.getExpiryDate())) {
      throw new InvalidToken("Block token with " + identifier + " is expired.");
    }
    BlockKey key = null;
    synchronized (this) {
    ...
    if (key == null) {
      throw new InvalidToken(
          "Can't re-compute password for "
              + identifier
              + ", since the required block key (keyID="
              + identifier.getKeyId()
              + ") doesn't exist.");
    }
    return createPassword(identifier.getBytes(), key.getKey());
   ...
   *
   * @return a data encryption key which may be used to encrypt traffic over the
      ...
      if (key == null) {
        throw new InvalidEncryptionKeyException(
            "Can't re-compute encryption key"
                + " for nonce, since the required block key (keyID="
                + keyId
                + ") doesn't exist. Current key: "
                + currentKey.getKeyId());
      }
    }
