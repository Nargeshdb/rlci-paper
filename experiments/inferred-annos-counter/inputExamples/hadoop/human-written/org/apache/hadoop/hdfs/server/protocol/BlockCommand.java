...
public class BlockCommand extends DatanodeCommand {
  ...
  /**
   * This constant is used to indicate that the block deletion does not need explicit ACK from the
   * datanode. When a block is put into the list of blocks to be deleted, it's size is set to this
   * constant. We assume that no block would actually have this size. Otherwise, we would miss ACKs
   * for blocks with such size. Positive number is used for compatibility reasons.
   */
  ...
  public BlockCommand(int action, String poolId, Block blocks[]) {
    this(
        ...
        poolId,
        blocks,
        EMPTY_TARGET_DATANODES,
        EMPTY_TARGET_STORAGE_TYPES,
