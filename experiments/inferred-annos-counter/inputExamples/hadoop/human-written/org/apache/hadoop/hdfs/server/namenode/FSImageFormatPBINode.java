...
public final class FSImageFormatPBINode {
  ...
  public static final class Loader {
    public static PermissionStatus loadPermission(long id, final StringTable stringTable) {
    ...
    public static List<XAttr> loadXAttrs(XAttrFeatureProto proto, final StringTable stringTable) {
    ...
    public static INodeDirectory loadINodeDirectory(INodeSection.INode n, LoaderContext state) {
      ...
      final PermissionStatus permissions =
          loadPermission(d.getPermission(), state.getStringTable());
      final INodeDirectory dir =
          new INodeDirectory(
              n.getId(), n.getName().toByteArray(), permissions, d.getModificationTime());
      ...
      if (d.hasTypeQuotas()) {
        ...
        if (typeQuotas.anyGreaterOrEqual(0)) {
          ...
          if (q == null) {
            dir.addDirectoryWithQuotaFeature(
                new DirectoryWithQuotaFeature.Builder().typeQuotas(typeQuotas).build());
          } else {
      ...
      if (d.hasAcl()) {
        int[] entries =
            AclEntryStatusFormat.toInt(loadAclEntries(d.getAcl(), state.getStringTable()));
    ...
    void loadINodeDirectorySectionInParallel(
        ExecutorService service, ArrayList<FileSummary.Section> sections, String compressionCodec)
        throws IOException {
      LOG.info(
          "Loading the INodeDirectory section in parallel with {} sub-" + "sections",
          sections.size());
      ...
      for (FileSummary.Section s : sections) {
        service.submit(
            () -> {
              ...
              try {
                ins = parent.getInputStreamForSection(s, compressionCodec);
      ...
      if (exceptions.size() != 0) {
        LOG.error("{} exceptions occurred loading INodeDirectories", exceptions.size());
      ...
      while (true) {
        ...
        for (long id : e.getChildrenList()) {
          ...
          if (addToParent(p, child)) {
            ...
            LOG.warn("Failed to add the inode {} to the directory {}", child.getId(), p.getId());
          }
        }
        ...
        for (int refId : e.getRefChildrenList()) {
          ...
          if (addToParent(p, ref)) {
            ...
            LOG.warn(
                "Failed to add the inode reference {} to the directory {}", ref.getId(), p.getId());
          }
        }
      }
      addToCacheAndBlockMap(inodeList);
    ...
    void loadINodeSection(InputStream in, StartupProgress prog, Step currentStep)
        throws IOException {
    ...
    private int loadINodesInSection(InputStream in, Counter counter) throws IOException {
    ...
    private long loadINodeSectionHeader(InputStream in, StartupProgress prog, Step currentStep)
        throws IOException {
    ...
    @SuppressWarnings(
        "builder:required.method.not.called") // TP: loadINodeSectionHeader is called outside any
                                              // try block and if it throws, ins won't be closed)
                                              // (validated)
    void loadINodeSectionInParallel(
        ...
        Step currentStep)
        throws IOException {
      LOG.info("Loading the INode section in parallel with {} sub-sections", sections.size());
      ...
      LOG.info("Completed loading all INode sections. Loaded {} inodes.", totalLoaded.get());
    }
    ...
    private INodeFile loadINodeFile(INodeSection.INode n) {
      ...
      Byte ecPolicyID = (isStriped ? (byte) f.getErasureCodingPolicyID() : null);
      ErasureCodingPolicy ecPolicy =
          isStriped ? fsn.getErasureCodingPolicyManager().getByID(ecPolicyID) : null;
      ...
      for (int i = 0; i < bp.size(); ++i) {
        ...
        if (isStriped) {
          Preconditions.checkState(
              ...
              "File with ID "
                  + n.getId()
                  + " has an invalid erasure coding policy ID "
                  + ecPolicy.getId());
          blocks[i] = new BlockInfoStriped(PBHelperClient.convert(b), ecPolicy);
        } else {
          blocks[i] = new BlockInfoContiguous(PBHelperClient.convert(b), replication);
        }
      }

      final PermissionStatus permissions =
          loadPermission(f.getPermission(), parent.getLoaderContext().getStringTable());

      final INodeFile file =
          new INodeFile(
              n.getId(),
              n.getName().toByteArray(),
              permissions,
              ...
              replication,
              ecPolicyID,
              f.getPreferredBlockSize(),
              (byte) f.getStoragePolicyID(),
              blockType);

      if (f.hasAcl()) {
        int[] entries =
            AclEntryStatusFormat.toInt(loadAclEntries(f.getAcl(), state.getStringTable()));
    ...
    private INodeSymlink loadINodeSymlink(INodeSection.INode n) {
      ...
      final PermissionStatus permissions =
          loadPermission(s.getPermission(), parent.getLoaderContext().getStringTable());
  ...
  public static final class Saver {
    ...
    private static QuotaByStorageTypeFeatureProto.Builder buildQuotaByStorageTypeEntries(
        QuotaCounts q) {
    ...
    public static INodeSection.INodeFile.Builder buildINodeFile(
      ...
      INodeSection.INodeFile.Builder b =
          INodeSection.INodeFile.newBuilder()
    ...
    public static INodeSection.INodeDirectory.Builder buildINodeDirectory(
      ...
      INodeSection.INodeDirectory.Builder b =
    ...
    void serializeINodeDirectorySection(OutputStream out) throws IOException {
      ...
      while (iter.hasNext()) {
        ...
        if (children.size() > 0) {
          INodeDirectorySection.DirEntry.Builder b =
              INodeDirectorySection.DirEntry.newBuilder().setParent(n.getId());
            ...
            if (dir.getInode(inode.getId()) == null) {
              FSImage.LOG.error(
                  "FSImageFormatPBINode#serializeINodeDirectorySection: "
                      ...
                      + "; parent="
                      + (inode.getParent() == null ? "null" : inode.getParent().getFullPathName()));
              ++numImageErrors;
            }
        ...
        if (outputInodes >= parent.getInodesPerSubSection()) {
          outputInodes = 0;
          parent.commitSubSection(summary, FSImageFormatProtobuf.SectionName.INODE_DIR_SUB);
        }
      }
      parent.commitSectionAndSubSection(
          summary,
    ...
    void serializeINodeSection(OutputStream out) throws IOException {
      ...
      INodeSection.Builder b =
          INodeSection.newBuilder()
              .setLastInodeId(fsn.dir.getLastInodeId())
      ...
      while (iter.hasNext()) {
        ...
        if (i % parent.getInodesPerSubSection() == 0) {
          parent.commitSubSection(summary, FSImageFormatProtobuf.SectionName.INODE_SUB);
        }
      }
      parent.commitSectionAndSubSection(
          summary,
    ...
    void serializeFilesUCSection(OutputStream out) throws IOException {
      ...
      for (Long id : filesWithUC) {
        ...
        if (!file.isUnderConstruction()) {
          LOG.warn(
              "Fail to save the lease for inode id "
                  + id
                  + " as the file is not under construction");
          continue;
        }
        String path = file.getFullPathName();
        FileUnderConstructionEntry.Builder b =
            FileUnderConstructionEntry.newBuilder().setInodeId(file.getId()).setFullPath(path);
    ...
    private void save(OutputStream out, INodeDirectory n) throws IOException {
      ...
      INodeSection.INode r =
          buildINodeCommon(n).setType(INodeSection.INode.Type.DIRECTORY).setDirectory(b).build();
    ...
    private void save(OutputStream out, INodeFile n) throws IOException {
      ...
      INodeSection.INode r =
          buildINodeCommon(n).setType(INodeSection.INode.Type.FILE).setFile(b).build();
      r.writeDelimitedTo(out);
    }

    private void save(OutputStream out, INodeSymlink n) throws IOException {
      INodeSection.INodeSymlink.Builder b =
      ...
      INodeSection.INode r =
          buildINodeCommon(n).setType(INodeSection.INode.Type.SYMLINK).setSymlink(b).build();
      r.writeDelimitedTo(out);
    }
    ...
    /**
     * Number of non-fatal errors detected while writing the INodeSection and INodeDirectorySection
     * sections.
     *
