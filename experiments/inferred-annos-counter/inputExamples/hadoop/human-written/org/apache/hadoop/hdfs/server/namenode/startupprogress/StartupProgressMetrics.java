...
/** Links {@link StartupProgress} to a {@link MetricsSource} to expose its information via JMX. */
...
public class StartupProgressMetrics implements MetricsSource {
  ...
  public StartupProgressMetrics(StartupProgress startupProgress) {
    ...
    DefaultMetricsSystem.instance()
        .register(
            ...
            STARTUP_PROGRESS_METRICS_INFO.description(),
            this);
  }
  ...
  public void getMetrics(MetricsCollector collector, boolean all) {
    ...
    builder.addGauge(
        info("PercentComplete", "overall percent complete"), prog.getPercentComplete());
    ...
    for (Phase phase : prog.getPhases()) {
      ...
      addGauge(
          builder, phase, "PercentComplete", " percent complete", prog.getPercentComplete(phase));
    }
  }

  /**
   * Adds a counter with a name built by using the specified phase's name as prefix and then
  ...
  private static void addCounter(
      MetricsRecordBuilder builder, Phase phase, String nameSuffix, String descSuffix, long value) {
    MetricsInfo metricsInfo =
        info(phase.getName() + nameSuffix, phase.getDescription() + descSuffix);
    builder.addCounter(metricsInfo, value);
  }

  /**
   * Adds a gauge with a name built by using the specified phase's name as prefix and then appending
