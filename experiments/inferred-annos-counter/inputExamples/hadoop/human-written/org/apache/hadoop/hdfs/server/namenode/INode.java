...
/**
 * We keep an in-memory representation of the file/block hierarchy. This is a base INode class
 * containing common fields for file and directory inodes.
 */
...
public abstract class INode implements INodeAttributes, Diff.Element<byte[]> {
  ...
  /**
   * @param snapshotId if it is not {@link Snapshot#CURRENT_STATE_ID}, get the result from the given
   *     snapshot; otherwise, get the result from the current inode.
   * @return user name
   */
  ...
  /**
   * @return if the given snapshot id is {@link Snapshot#CURRENT_STATE_ID}, return this; otherwise
   *     return the corresponding snapshot inode.
   */
  ...
  /**
   * When {@link #recordModification} is called on a referred node, this method tells which snapshot
   * the modification should be associated with: the snapshot that belongs to the SRC tree of the
   * rename operation, or the snapshot belonging to the DST tree.
   *
   * @param latestInDst id of the latest snapshot in the DST tree above the reference node
   * @return True: the modification should be recorded in the snapshot that belongs to the SRC tree.
   *     False: the modification should be recorded in the snapshot that belongs to the DST tree.
   */
  ...
  /**
   * This inode is being modified. The previous version of the inode needs to be recorded in the
   * latest snapshot.
   *
   * @param latestSnapshotId The id of the latest snapshot that has been taken. Note that it is
   *     {@link Snapshot#CURRENT_STATE_ID} if no snapshots have been taken.
   */
  ...
  /**
   * Clean the subtree under this inode and collect the blocks from the descents for further block
   * deletion/update. The current inode can either resides in the current tree or be stored as a
   * snapshot copy.
   *
   * <pre>
   ...
   * @param snapshotId The id of the snapshot to delete. {@link Snapshot#CURRENT_STATE_ID} means to
   *     delete the current file/directory.
   * @param priorSnapshotId The id of the latest snapshot before the to-be-deleted snapshot. When
   *     deleting a current inode, this parameter captures the latest snapshot.
   */
  public abstract void cleanSubtree(
      ReclaimContext reclaimContext, final int snapshotId, int priorSnapshotId);

  /**
   * Destroy self and clear everything! If the INode is a file, this method collects its blocks for
   * further block deletion. If the INode is a directory, the method goes down the subtree and
   * collects blocks from the descents, and clears its parent/children references as well. The
   * method also clears the diff list if the INode contains snapshot diff list.
   *
   * @param reclaimContext Record blocks and inodes that need to be reclaimed.
   */
  ...
  public final ContentSummary computeContentSummary(BlockStoragePolicySuite bsps)
    ...
    return computeAndConvertContentSummary(
        Snapshot.CURRENT_STATE_ID, new ContentSummaryComputationContext(bsps));
  }

  /** Compute {@link ContentSummary}. */
  public final ContentSummary computeAndConvertContentSummary(
      int snapshotId, ContentSummaryComputationContext summary) throws AccessControlException {
   ...
   *
   * @param snapshotId Specify the time range for the calculation. If this parameter equals to
   *     {@link Snapshot#CURRENT_STATE_ID}, the result covers both the current states and all the
   *     snapshots. Otherwise the result only covers all the files/directories contained in the
   *     specific snapshot.
  ...
  /**
   * Count subtree {@link Quota#NAMESPACE} and {@link Quota#STORAGESPACE} usages. Entry point for
   * FSDirectory where blockStoragePolicyId is given its initial value.
   */
  public final QuotaCounts computeQuotaUsage(BlockStoragePolicySuite bsps) {
    final byte storagePolicyId =
        isSymlink() ? HdfsConstants.BLOCK_STORAGE_POLICY_ID_UNSPECIFIED : getStoragePolicyID();
   ...
   *
   * <p>With the existence of {@link INodeReference}, the same inode and its subtree may be referred
   * by multiple {@link WithName} nodes and a {@link DstReference} node. To avoid circles while
   * quota usage computation, we have the following rules:
   *
   * <pre>
   ...
   * @param useCache Whether to use cached quota usage. Note that {@link WithName} node never uses
   *     cache for its subtree.
   * @param lastSnapshotId {@link Snapshot#CURRENT_STATE_ID} indicates the computation is in the
   *     current tree. Otherwise the id indicates the computation range for a {@link WithName} node.
  ...
  /**
   * @return the parent as a reference if this is a referred inode; otherwise, return null.
   */
  ...
  /**
   * @return true if this is a reference and the reference count is 1; otherwise, return false.
   */
  ...
  /** Set last access time of inode. */
  public final INode setAccessTime(
      long accessTime, int latestSnapshotId, boolean skipCaptureAccessTimeOnlyChangeInSnapshot) {
  ...
  /**
   * @return the latest block storage policy id of the INode. Specifically, if a storage policy is
   *     directly specified on the INode then return the ID of that policy. Otherwise follow the
   *     latest parental path and return the ID of the first specified storage policy.
   */
  public abstract byte getStoragePolicyID();

  /**
   * @return the storage policy directly specified on the INode. Return {@link
   *     HdfsConstants#BLOCK_STORAGE_POLICY_ID_UNSPECIFIED} if no policy has been specified.
   */
   ...
   * @return the storage policy ID of this INode. Note that for an {@link INodeSymlink} we return
   *     {@link HdfsConstants#BLOCK_STORAGE_POLICY_ID_UNSPECIFIED} instead of throwing Exception
   */
  public byte getStoragePolicyIDForQuota(byte parentStoragePolicyId) {
    byte localId =
        isSymlink() ? HdfsConstants.BLOCK_STORAGE_POLICY_ID_UNSPECIFIED : getLocalStoragePolicyID();
    return localId != HdfsConstants.BLOCK_STORAGE_POLICY_ID_UNSPECIFIED
        ? localId
        : parentStoragePolicyId;
  }

  /**
   * Breaks {@code path} into components.
   *
   * @return array of byte arrays each of which represents a single path component.
   */
  @VisibleForTesting
  ...
  /**
   * Information used to record quota usage delta. This data structure is usually passed along with
   * an operation like {@link #cleanSubtree}. Note that after the operation the delta counts should
   * be decremented from the ancestral directories' quota usage.
   */
  public static class QuotaDelta {
    private final QuotaCounts counts;
    /**
     * The main usage of this map is to track the quota delta that should be applied to another
     * path. This usually happens when we reclaim INodes and blocks while deleting snapshots, and
     * hit an INodeReference. Because the quota usage for a renamed+snapshotted file/directory is
     * counted in both the current and historical parents, any change of its quota usage may need to
     * be propagated along its parent paths both before and after the rename.
     */
    private final Map<INode, QuotaCounts> updateMap;

    /**
     * When deleting a snapshot we may need to update the quota for directories with quota feature.
     * This map is used to capture these directories and their quota usage updates.
     */
  ...
  public static class ReclaimContext {
    ...
    /**
     * @param bsps block storage policy suite to calculate intended storage type usage
     * @param collectedBlocks blocks collected from the descents for further block deletion/update
     *     will be added to the given map.
    ...
    /**
     * make a copy with the same collectedBlocks, removedINodes, and removedUCFiles but a new
     * quotaDelta.
     */
  ...
  public static class BlocksMapUpdateInfo {
    ...
    /**
     * Add a to-be-deleted block into the {@link BlocksMapUpdateInfo#toDeleteList}
     *
    ...
    public void addUpdateReplicationFactor(BlockInfo block, short targetRepl) {
      toUpdateReplicationInfo.add(new UpdatedReplicationInfo(targetRepl, block));
    }
  ...
  /**
   * INode feature such as {@link FileUnderConstructionFeature} and {@link
