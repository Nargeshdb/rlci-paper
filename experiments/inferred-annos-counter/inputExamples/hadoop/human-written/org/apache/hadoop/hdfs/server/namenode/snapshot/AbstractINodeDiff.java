...
/**
 * The difference of an inode between in two snapshots. {@link AbstractINodeDiffList} maintains a
 * list of snapshot diffs,
 *
 * <pre>
 *   d_1 -> d_2 -> ... -> d_n -> null,
 * </pre>
 *
 * where -> denotes the {@link AbstractINodeDiff#posteriorDiff} reference. The current directory
 * state is stored in the field of {@link INode}. The snapshot state can be obtained by applying the
 * diffs one-by-one in reversed chronological order. Let s_1, s_2, ..., s_n be the corresponding
 * snapshots. Then,
 *
 * <pre>
 ...
 * </pre>
 */
abstract class AbstractINodeDiff<
  ...
  A snapshotINode;
  /**
   * Posterior diff is the diff happened after this diff. The posterior diff should be first applied
   * to obtain the posterior snapshot and then apply this diff in order to obtain this snapshot. If
   * the posterior diff is null, the posterior state is the current state.
   */
  ...
  abstract void combinePosteriorAndCollectBlocks(
      INode.ReclaimContext reclaimContext, final N currentINode, final D posterior);
   ...
   */
  abstract void destroyDiffAndCollectBlocks(
