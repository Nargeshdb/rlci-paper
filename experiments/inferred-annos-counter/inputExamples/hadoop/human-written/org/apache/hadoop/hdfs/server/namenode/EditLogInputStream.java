 ...
 *
 * <p>It should stream bytes from the storage exactly as they were written into the #{@link
 * EditLogOutputStream}.
 */
...
public abstract class EditLogInputStream implements Closeable {
  ...
  /**
   * Returns the name of the currently active underlying stream. The default implementation returns
   * the same value as getName unless overridden by the subclass.
   *
   ...
   *
   * <p>This method can be used to skip over corrupted sections of edit logs.
   */
  public void resync() {
    if (cachedOp != null) {
      return;
   ...
   *
   * <p>This is exactly like nextOp, except that we attempt to skip over damaged parts of the edit
   * log
   *
   * @return an operation from the stream or null if at end of stream
   */
  ...
  /**
   * Skip edit log operations up to a given transaction ID, or until the end of the edit log is
   * reached.
   *
   * <p>After this function returns, the next call to readOp will return either end-of-file (null)
   * or a transaction with a txid equal to or higher than the one we asked for.
   *
   * @param txid The transaction ID to read up until.
   * @return Returns true if we found a transaction ID greater than or equal to 'txid' in the log.
   */
  ...
  /**
   * Get the "position" of in the stream. This is useful for debugging and operational purposes.
   *
   * <p>Different stream types can have a different meaning for what the position is. For file
   * streams it means the byte offset from the start of the file.
   *
  ...
  /**
   * Returns true if we are currently reading the log from a local disk or an even faster data
   * source (e.g. a byte buffer).
   */
