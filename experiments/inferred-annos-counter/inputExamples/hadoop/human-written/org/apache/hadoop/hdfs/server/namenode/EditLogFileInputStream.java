...
/**
 * An implementation of the abstract class {@link EditLogInputStream}, which reads edits from a
 * file. That file may be either on the local disk or accessible via a URL.
 */
...
public class EditLogFileInputStream extends EditLogInputStream {
  ...
  /**
   * Open an EditLogInputStream for the given file. The file is pretransactional, so has no txids
   *
   * @param name filename to open
   * @throws LogHeaderCorruptException if the header is either missing or appears to be
   *     corrupt/truncated
   * @throws IOException if an actual IO error occurs while reading the header
   */
  ...
  public static EditLogInputStream fromUrl(
      ...
      long endTxId,
      boolean inProgress) {
    return new EditLogFileInputStream(
        new URLLog(connectionFactory, url), startTxId, endTxId, inProgress);
  }

  /**
   * Create an EditLogInputStream from a {@link ByteString}, i.e. an in-memory collection of bytes.
   *
   ...
   */
  public static EditLogInputStream fromByteString(
      ByteString bytes, long startTxId, long endTxId, boolean inProgress) {
    return new EditLogFileInputStream(
        new ByteStringLog(bytes, String.format("ByteStringEditLog[%d, %d]", startTxId, endTxId)),
        startTxId,
        endTxId,
        inProgress);
  }

  private EditLogFileInputStream(
      LogSource log, long firstTxId, long lastTxId, boolean isInProgress) {
    ...
    try {
      ...
      if (logVersion == -1) {
        ...
        throw new LogHeaderCorruptException(
            "No header present in log (value "
    ...
    switch (state) {
      ...
      case OPEN:
        ...
        if ((op != null) && (op.hasTransactionId())) {
          ...
          if ((txId >= lastTxId) && (lastTxId != HdfsServerConstants.INVALID_TXID)) {
            //
   ...
   * @throws IOException
   */
  static FSEditLogLoader.EditLogValidation scanEditLog(
      File file, long maxTxIdToScan, boolean verifyVersion) throws IOException {
  ...
  /**
   * Exception indicating that the header of an edits log file is corrupted. This can be because the
   * header is not present, or because the header data is invalid (eg claims to be over a newer
      ...
      return SecurityUtil.doAsCurrentUser(
          new PrivilegedExceptionAction<InputStream>() {
            ...
            public InputStream run() throws IOException {
              ...
              try {
                connection =
                    (HttpURLConnection) connectionFactory.openConnection(url, isSpnegoEnabled);
              ...
              if (contentLength != null) {
                ...
                throw new IOException(
                    CONTENT_LENGTH
                        ...
                        + "by the server when trying to fetch "
                        + url);
              }
