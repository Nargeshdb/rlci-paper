...
/**
 * This class is used in Namesystem's jetty to retrieve/upload a file Typically used by the
 * Secondary NameNode to retrieve image and edit file for periodic checkpointing in Non-HA
 * deployments. Standby NameNode uses to upload checkpoints in HA deployments.
 */
...
public class ImageServlet extends HttpServlet {
  ...
  private SortedSet<ImageUploadRequest> currentlyDownloadingCheckpoints =
      Collections.<ImageUploadRequest>synchronizedSortedSet(new TreeSet<ImageUploadRequest>());
  ...
  public void doGet(final HttpServletRequest request, final HttpServletResponse response)
    ...
    try {
      ...
      validateRequest(
          context, conf, request, response, nnImage, parsedParams.getStorageInfoString());
                ...
                private void serveFile(File file) throws IOException {
                  ...
                  try {
                    ...
                    DataTransferThrottler throttler =
                        parsedParams.isBootstrapStandby
                            ? getThrottlerForBootstrapStandby(conf)
                            : getThrottler(conf);
                    TransferFsImage.copyFileToStream(
                        response.getOutputStream(), file, fis, throttler);
                  } finally {
  ...
  private void validateRequest(
    ...
    if (UserGroupInformation.isSecurityEnabled()
      ...
      LOG.warn(
          "Received non-NN/SNN/administrator request for image or edits from "
    ...
    if (theirStorageInfoString != null && !myStorageInfoString.equals(theirStorageInfoString)) {
      String errorMsg =
          "This namenode has storage info "
              + myStorageInfoString
              + " but the secondary expected "
              + theirStorageInfoString;
      response.sendError(HttpServletResponse.SC_FORBIDDEN, errorMsg);
      LOG.warn(
          "Received an invalid request file transfer request "
              + "from a secondary with storage info "
              + theirStorageInfoString);
      throw new IOException(errorMsg);
    }
  }
  ...
  @VisibleForTesting
  static boolean isValidRequestor(ServletContext context, String remoteUser, Configuration conf)
      throws IOException {
    ...
    if (HAUtil.isHAEnabled(conf, DFSUtil.getNamenodeNameServiceId(conf))) {
      ...
      for (Configuration otherNnConf : otherNnConfs) {
        validRequestors.add(
            SecurityUtil.getServerPrincipal(
                otherNnConf.get(DFSConfigKeys.DFS_NAMENODE_KERBEROS_PRINCIPAL_KEY),
   ...
   * @throws IOException
   */
  public static void setVerificationHeadersForGet(HttpServletResponse response, File file)
      throws IOException {
  ...
  static String getParamStringForImage(
      NameNodeFile nnf, long txid, StorageInfo remoteStorageInfo, boolean isBootstrapStandby) {
      ...
      for (Map.Entry<String, String[]> entry : pmap.entrySet()) {
        ...
        if (key.equals("getimage")) {
          ...
          try {
            ...
            String bootstrapStandby = ServletUtil.getParameter(request, IS_BOOTSTRAP_STANDBY);
            isBootstrapStandby = bootstrapStandby != null && Boolean.parseBoolean(bootstrapStandby);
   ...
   * @throws IOException
   */
  static void setVerificationHeadersForPut(HttpURLConnection connection, File file)
      throws IOException {
   ...
   */
  static Map<String, String> getParamsForPutImage(
      Storage storage, long txid, long imageFileSize, NameNodeFile nnf) {
    ...
    try {
      ...
      if (checkRecentImageEnableObj != null) {
        if (checkRecentImageEnableObj instanceof Boolean) {
          ...
          LOG.error(
              "Expecting boolean obj for setting checking recent image, "
                  + "but got "
                  + checkRecentImageEnableObj.getClass()
                  + ". This is "
              ...
              new PrivilegedExceptionAction<Void>() {
                ...
                public Void run() throws Exception {
                  ...
                  HAServiceProtocol.HAServiceState state =
                      NameNodeHttpServer.getNameNodeStateFromContext(getServletContext());
                  if (state != HAServiceProtocol.HAServiceState.ACTIVE
                    ...
                    response.sendError(
                        ...
                        "Nameode "
                            ...
                            + " is currently not in a state which can "
                            + "accept uploads of new fsimages. State: "
                            + state);
                    return null;
                  }
                  ...
                  if (checkRecentImageEnable
                    ...
                    String message =
                        "Rejecting a fsimage due to small time delta "
                  ...
                  try {
                    ...
                    try {
                      ...
                      MD5Hash downloadImageDigest =
                          TransferFsImage.handleUploadImageRequest(
                              request,
                              txid,
                              nnImage.getStorage(),
                              stream,
                              parsedParams.getFileSize(),
                              getThrottler(conf));
  ...
  static class PutImageParams {
    ...
    public PutImageParams(
      ...
      fileSize = ServletUtil.parseLongParam(request, Util.FILE_LENGTH);
      String imageType = ServletUtil.getParameter(request, IMAGE_FILE_TYPE);
      nnf = imageType == null ? NameNodeFile.IMAGE : NameNodeFile.valueOf(imageType);
  ...
  private static class ImageUploadRequest implements Comparable<ImageUploadRequest> {
    ...
    public String toString() {
      return "ImageRequest{" + "txId=" + txId + ", address='" + address + '\'' + '}';
    }
  }
}
