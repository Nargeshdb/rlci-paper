...
public class SlowPeerTracker {
  ...
  /**
   * Time duration after which a report is considered stale. This is set to
   * DFS_DATANODE_OUTLIERS_REPORT_INTERVAL_KEY * 3 i.e. maintained for at least two successive
   * reports.
   */
  ...
  /**
   * Number of nodes to include in JSON report. We will return nodes with the highest number of
   * votes from peers.
   */
  private static final int MAX_NODES_TO_REPORT = 5;

  /**
   * Information about peers that have reported a node as being slow. Each outer map entry is a map
   * of (DatanodeId) {@literal ->} (timestamp), mapping reporting nodes to the timestamp of the last
   * report from that node.
   *
   * <p>DatanodeId could be the DataNodeId or its address. We don't care as long as the caller uses
   * it consistently.
   *
   * <p>Stale reports are not evicted proactively and can potentially hang around forever.
   */
  ...
  /**
   * Add a new report. DatanodeIds can be the DataNodeIds or addresses We don't care as long as the
  ...
  /**
   * Retrieve the non-expired reports that mark a given DataNode as slow. Stale reports are
   * excluded.
   *
  ...
  public Set<String> getReportsForNode(String slowNode) {
    final ConcurrentMap<String, Long> nodeEntries = allReports.get(slowNode);
  ...
  public Map<String, SortedSet<String>> getReportsForAllDataNodes() {
    ...
    for (Map.Entry<String, ConcurrentMap<String, Long>> entry : allReports.entrySet()) {
   ...
   *
   * @return serialized representation of valid reports. null if serialization failed.
   */
  ...
  /**
   * Retrieve reports in a structure for generating JSON, limiting the output to the top numNodes
   * nodes i.e nodes with the most reports.
   *
   * @param numNodes number of nodes to return. This is to limit the size of the generated JSON.
   */
  private Collection<ReportForJson> getJsonReports(int numNodes) {
        ...
        new PriorityQueue<>(
            ...
            new Comparator<ReportForJson>() {
              ...
              public int compare(ReportForJson o1, ReportForJson o2) {
                return Ints.compare(o1.reportingNodes.size(), o2.reportingNodes.size());
              }
            });
    ...
    for (Map.Entry<String, ConcurrentMap<String, Long>> entry : allReports.entrySet()) {
      ...
      if (!validReports.isEmpty()) {
        ...
        } else if (topNReports.peek().getReportingNodes().size() < validReports.size()) {
