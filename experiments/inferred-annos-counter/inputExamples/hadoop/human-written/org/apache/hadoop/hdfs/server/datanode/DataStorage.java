...
public class DataStorage extends Storage {
  ...
  /**
   * Set of bpids for which 'trash' is currently enabled. When trash is enabled block files are
   * moved under a separate 'trash' folder instead of being deleted right away. This can be useful
   * during rolling upgrades, for example. The set is backed by a concurrent HashMap.
   *
   * <p>Even if trash is enabled, it is not used if a layout upgrade is in progress for a storage
   * directory i.e. if the previous directory exists.
   */
  private Set<String> trashEnabledBpids;

  /**
   * Datanode UUID that this storage is currently attached to. This is the same as the legacy
   * StorageID for datanodes that were upgraded from a pre-UUID version. For compatibility with
   * prior versions of Datanodes we cannot make this field a UUID.
   */
  ...
  private static boolean createStorageID(StorageDirectory sd, int lv, Configuration conf) {
    ...
    final boolean haveValidStorageId =
        DataNodeLayoutVersion.supports(LayoutVersion.Feature.ADD_DATANODE_AND_STORAGE_UUIDS, lv)
  ...
  public static boolean createStorageID(
    ...
    if (sd.getStorageLocation() != null
      ...
      sd.setStorageUuid(
          conf.get(
              DFSConfigKeys.DFS_PROVIDER_STORAGEUUID,
  ...
  /**
   * Enable trash for the specified block pool storage. Even if trash is enabled by the caller, it
   * is superseded by the 'previous' directory if a layout upgrade is in progress.
   */
  ...
  /**
   * If rolling upgrades are in progress then do not delete block files immediately. Instead we move
   * the block files to an intermediate 'trash' directory. If there is a subsequent rollback, then
   * the block files will be restored from trash.
   *
   * @return trash directory if rolling upgrade is in progress, null otherwise.
   */
  ...
  /**
   * VolumeBuilder holds the metadata (e.g., the storage directories) of the prepared volume
   * returned from {@link #prepareVolume(DataNode, StorageLocation, List)}. Calling {@link
   * VolumeBuilder#build()} to add the metadata to {@link DataStorage} so that this prepared volume
   * can be active.
   */
  @InterfaceAudience.Private
  ...
  private StorageDirectory loadStorageDirectory(
      ...
      List<Callable<StorageDirectory>> callables)
      throws IOException {
    ...
    try {
      ...
      switch (curState) {
        ...
        case NON_EXISTENT:
          ...
          throw new IOException("Storage directory with location " + location + " does not exist");
        case NOT_FORMATTED: // format
          LOG.info(
              "Storage directory with location {} is not formatted for "
  ...
  /**
   * Prepare a storage directory. It creates a builder which can be used to add to the volume. If
   * the volume cannot be added, it is OK to discard the builder later.
   *
   ...
   * @return a VolumeBuilder that holds the metadata of this storage directory and can be added to
   *     DataStorage later.
   * @throws IOException if encounters I/O errors.
   *     <p>Note that if there is IOException, the state of DataStorage is not modified.
   */
  public VolumeBuilder prepareVolume(
      DataNode datanode, StorageLocation location, List<NamespaceInfo> nsInfos) throws IOException {
    ...
    StorageDirectory sd =
        loadStorageDirectory(datanode, nsInfos.get(0), location, StartupOption.HOTSWAP, null);
    ...
    for (NamespaceInfo nsInfo : nsInfos) {
      ...
      final List<StorageDirectory> dirs =
          bpStorage.loadBpStorageDirectories(
  ...
  static int getParallelVolumeLoadThreadsNum(int dataDirs, Configuration conf) {
    ...
    LOG.info(
        "Using {} threads to upgrade data directories ({}={}, " + "dataDirs={})",
        min,
        key,
        n,
        dataDirs);
  ...
  /**
   * Add a list of volumes to be managed by DataStorage. If the volume is empty, format it,
   * otherwise recover it from previous transitions if required.
   *
   ...
   */
  @VisibleForTesting
  synchronized List<StorageDirectory> addStorageLocations(
      DataNode datanode,
      NamespaceInfo nsInfo,
    ...
    try {
      final List<StorageLocation> successLocations =
          loadDataStorage(datanode, nsInfo, dataDirs, startOpt, executor);
  ...
  private List<StorageLocation> loadDataStorage(
      DataNode datanode,
      NamespaceInfo nsInfo,
      Collection<StorageLocation> dataDirs,
      StartupOption startOpt,
      ...
      if (!containsStorageDir(dataDir)) {
        try {
          ...
          final StorageDirectory sd =
              loadStorageDirectory(datanode, nsInfo, dataDir, startOpt, callables);
  ...
  private List<StorageDirectory> loadBlockPoolSliceStorage(
    ...
    for (StorageLocation dataDir : dataDirs) {
      ...
      try {
        ...
        LOG.warn("Failed to add storage directory {} for block pool {}", dataDir, bpid, e);
      }
    }
    ...
    if (!tasks.isEmpty()) {
      ...
      for (UpgradeTask t : tasks) {
        try {
          ...
          LOG.warn("Failed to upgrade storage directory {} for block pool {}", t.dataDir, bpid, e);
        } catch (InterruptedException e) {
          throw DFSUtilClient.toInterruptedIOException("Task interrupted", e);
        }
      }
  ...
  /**
   * Remove storage dirs from DataStorage. All storage dirs are removed even when the IOException is
   * thrown.
   *
   ...
   */
  synchronized void removeVolumes(final Collection<StorageLocation> storageLocations)
      throws IOException {
    ...
    for (Iterator<StorageDirectory> it = getStorageDirs().iterator(); it.hasNext(); ) {
      ...
      if (storageLocations.contains(sdLocation)) {
        ...
        try {
          ...
          LOG.warn("I/O error attempting to unlock storage directory {}.", sd.getRoot(), e);
          errorMsgBuilder.append(
              String.format("Failed to remove %s: %s%n", sd.getRoot(), e.getMessage()));
        }
      }
    }
  ...
  /**
   * Analyze storage directories for a specific block pool. Recover from previous transitions if
   * required. Perform fs state transition if necessary depending on the namespace info. Read
   * storage info. <br>
  ...
  void format(StorageDirectory sd, NamespaceInfo nsInfo, String newDatanodeUuid, Configuration conf)
      throws IOException {
   ...
   *
   * <p>Rollback if the rollback startup option was specified. Upgrade if this.LV > LAYOUT_VERSION
  ...
  /**
   * Upgrade from a pre-federation layout. Move current storage into a backup directory, and
   * hardlink all its blocks into the new current directory.
   *
  ...
  void upgradeProperties(StorageDirectory sd, Configuration conf) throws IOException {
    createStorageID(sd, layoutVersion, conf);
    LOG.info(
        "Updating layout version from {} to {} for storage {}",
        layoutVersion,
        HdfsServerConstants.DATANODE_LAYOUT_VERSION,
        sd.getRoot());
   ...
   * </ol>
   *
   * If previous directory does not exist and the current version supports federation, perform a
   * simple rollback of layout version. This does not involve saving/restoration of actual data.
   */
  void doRollback(StorageDirectory sd, NamespaceInfo nsInfo) throws IOException {
    ...
    if (!prevDir.exists()) {
      if (DataNodeLayoutVersion.supports(
          LayoutVersion.Feature.FEDERATION, HdfsServerConstants.DATANODE_LAYOUT_VERSION)) {
        readProperties(sd, HdfsServerConstants.DATANODE_LAYOUT_VERSION);
        writeProperties(sd);
        LOG.info(
            "Layout version rolled back to {} for storage {}",
            HdfsServerConstants.DATANODE_LAYOUT_VERSION,
            sd.getRoot());
      }
      return;
   ...
   */
  private static void linkAllBlocks(
      File fromDir, File fromBbwDir, File toDir, int diskLayoutVersion, Configuration conf)
      throws IOException {
    ...
    if (DataNodeLayoutVersion.supports(LayoutVersion.Feature.APPEND_RBW_DIR, diskLayoutVersion)) {
      ...
      linkBlocks(
          fromDir, new File(toDir, STORAGE_DIR_FINALIZED), diskLayoutVersion, hardLink, conf);
    ...
    LOG.info("Linked blocks from {} to {}. {}", fromDir, toDir, hardLink.linkStats.report());
  }
  ...
  static void linkBlocks(
      File fromDir, File toDir, String subdir, int oldLV, HardLink hl, Configuration conf)
      throws IOException {
    linkBlocks(new File(fromDir, subdir), new File(toDir, subdir), oldLV, hl, conf);
  }

  private static void linkBlocks(File from, File to, int oldLV, HardLink hl, Configuration conf)
      throws IOException {
    ...
    if (oldLV
            > DataNodeLayoutVersion.Feature.BLOCKID_BASED_LAYOUT_32_by_32.getInfo()
                .getLayoutVersion()
    ...
    if (!duplicates.isEmpty()) {
      LOG.error(
          "There are {} duplicate block " + "entries within the same volume.", duplicates.size());
      removeDuplicateEntries(idBasedLayoutSingleLinks, duplicates);
    }

    final int numLinkWorkers =
        conf.getInt(
  ...
  /**
   * Find duplicate entries with an array of LinkArgs. Duplicate entries are entries with the same
   * last path component.
   */
  static ArrayList<LinkArgs> findDuplicateEntries(ArrayList<LinkArgs> all) {
    ...
    Collections.sort(
        all,
        new Comparator<LinkArgs>() {
          /**
           * Compare two LinkArgs objects, such that objects with the same terminal source path
  ...
  /**
   * Remove duplicate entries from the list. We do this by choosing: 1. the entries with the highest
   * genstamp (this takes priority), 2. the entries with the longest block files, 3. arbitrarily, if
   * neither #1 nor #2 gives a clear winner.
   *
   * <p>Block and metadata files form a pair-- if you take a metadata file from one subdirectory,
   * you must also take the block file from that subdirectory.
   */
  private static void removeDuplicateEntries(
      ArrayList<LinkArgs> all, ArrayList<LinkArgs> duplicates) {
  ...
  static void linkBlocksHelper(
    ...
    if (blockNames.length > 0) {
      if (upgradeToIdBasedLayout) {
        for (String blockName : blockNames) {
          ...
          idBasedLayoutSingleLinks.add(
              new LinkArgs(new File(from, blockName), new File(blockLocation, blockName)));
          hl.linkStats.countSingleLinks++;
        }
      } else {
    ...
    if (otherNames != null) {
      for (int i = 0; i < otherNames.length; i++) {
        linkBlocksHelper(
            new File(from, otherNames[i]),
            new File(to, otherNames[i]),
            oldLV,
            hl,
            upgradeToIdBasedLayout,
            blockRoot,
  ...
  /**
   * Get the BlockPoolSliceStorage from {@link bpStorageMap}. If the object is not found, create a
   * new object and put it to the map.
   */
  synchronized BlockPoolSliceStorage getBlockPoolSliceStorage(final NamespaceInfo nsInfo) {
    final String bpid = nsInfo.getBlockPoolID();
    BlockPoolSliceStorage bpStorage = bpStorageMap.get(bpid);
    if (bpStorage == null) {
      bpStorage =
