...
/** A striped block writer that writes reconstructed data to the remote target datanode. */
...
class StripedBlockWriter {
   ...
   */
  @SuppressWarnings(
      "builder:required.method.not.called") // FP depends on boolean variable: can't handle boolean
  ...
  private void init() throws IOException {
    ...
    try {
      ...
      IOStreamPair saslStreams =
          datanode
              .getSaslClient()
              .socketSend(socket, unbufOut, unbufIn, keyFactory, blockToken, target);
      ...
      out =
          new DataOutputStream(
              new BufferedOutputStream(unbufOut, DFSUtilClient.getSmallBufferSize(conf)));
      ...
      new Sender(out)
          .writeBlock(
              ...
              0,
              0,
              stripedWriter.getChecksum(),
              stripedWriter.getCachingStrategy(),
              false,
              false,
              null,
              storageId,
    ...
    if (targetBuffer.isDirect()) {
      ...
      stripedWriter
          ...
          .calculateChunkedSums(
              targetBuffer.array(), 0, targetBuffer.remaining(), stripedWriter.getChecksumBuf(), 0);
    }
    ...
    while (targetBuffer.remaining() > 0) {
      DFSPacket packet =
          new DFSPacket(
              ...
              false);
      int maxBytesToPacket =
          stripedWriter.getMaxChunksPerPacket() * stripedWriter.getBytesPerChecksum();
      int toWrite =
          targetBuffer.remaining() > maxBytesToPacket ? maxBytesToPacket : targetBuffer.remaining();
      int ckLen =
          ((toWrite - 1) / stripedWriter.getBytesPerChecksum() + 1)
              * stripedWriter.getChecksumSize();
