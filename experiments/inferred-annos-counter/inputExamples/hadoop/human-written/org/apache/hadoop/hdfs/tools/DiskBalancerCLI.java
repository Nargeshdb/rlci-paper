...
/**
 * DiskBalancer is a tool that can be used to ensure that data is spread evenly across volumes of
 * same storage type.
 *
 * <p>For example, if you have 3 disks, with 100 GB , 600 GB and 200 GB on each disk, this tool will
 * ensure that each disk will have 300 GB.
 *
 * <p>This tool can be run while data nodes are fully functional.
 *
 * <p>At very high level diskbalancer computes a set of moves that will make disk utilization equal
 * and then those moves are executed by the datanode.
 */
public class DiskBalancerCLI extends Configured implements Tool {
  /** Computes a plan for a given set of nodes. */
  public static final String PLAN = "plan";
  /**
   * Output file name, for commands like report, plan etc. This is an optional argument, by default
   * diskbalancer will write all its output to /system/reports/diskbalancer of the current cluster
  ...
  /**
   * Percentage of data unevenness that we are willing to live with. For example - a value like 10
   * indicates that we are okay with 10 % +/- from idealStorage Target.
   */
  ...
  /**
   * The report command prints out a disk fragmentation report about the data cluster. By default it
   * prints the DEFAULT_TOP machines names with high nodeDataDensity
   * {DiskBalancerDataNode#getNodeDataDensity} values. This means that these are the nodes that
   * deviates from the ideal data distribution.
   */
  ...
  public static void main(String[] argv) throws Exception {
    ...
    try {
      ...
      String msg =
          String.format(
              "Exception thrown while running %s.", DiskBalancerCLI.class.getSimpleName());
      LOG.error(msg, ex);
      res = 1;
    }
    System.exit(res);
    ...
    if (cmdArgs.length > 2) {
      throw new HadoopIllegalArgumentException(
          "Invalid or extra Arguments: "
              + Arrays.toString(Arrays.copyOfRange(cmdArgs, 2, cmdArgs.length)));
    }
  ...
  private void addPlanCommands(Options opt) {
    ...
    Option plan =
        OptionBuilder.withLongOpt(PLAN)
            .withDescription(
                "Hostname, IP address or UUID of datanode " + "for which a plan is created.")
            .hasArg()
            .create();
    ...
    Option threshold =
        OptionBuilder.withLongOpt(THRESHOLD)
            .hasArg()
            .withDescription(
                "Percentage of data skew that is tolerated before"
  ...
  private void addHelpCommands(Options opt) {
    Option help =
        OptionBuilder.withLongOpt(HELP)
            .hasOptionalArg()
            .withDescription("valid commands are plan | execute | query | cancel" + " | report")
            .create();
  ...
  private void addExecuteCommands(Options opt) {
    Option execute =
        OptionBuilder.withLongOpt(EXECUTE)
            .hasArg()
            .withDescription("Takes a plan file and " + "submits it for execution by the datanode.")
            .create();
  ...
  private void addQueryCommands(Options opt) {
    Option query =
        OptionBuilder.withLongOpt(QUERY)
            .hasArg()
            .withDescription("Queries the disk balancer " + "status of a given datanode.")
            .create();
    ...
    Option verbose =
        OptionBuilder.withLongOpt(VERBOSE)
            .withDescription("Prints details of the plan that is being executed " + "on the node.")
            .create();
  ...
  private void addReportCommands(Options opt) {
    Option report =
        OptionBuilder.withLongOpt(REPORT)
            .withDescription("List nodes that will benefit from running " + "DiskBalancer.")
            .create();
    ...
    Option node =
        OptionBuilder.withLongOpt(NODE)
            .hasArg()
            .withDescription("Datanode address, " + "it can be DataNodeID, IP or hostname.")
            .create();
  ...
  /**
   * This function parses all command line arguments and returns the appropriate values.
   *
