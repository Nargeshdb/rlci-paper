...
/**
 * A HDFS specific delegation token secret manager. The secret manager is responsible for generating
 * and accepting the password for each token.
 */
...
public class DelegationTokenSecretManager
   ...
   *
   * @param delegationKeyUpdateInterval the number of milliseconds for rolling new secret keys.
   * @param delegationTokenMaxLifetime the maximum lifetime of the delegation tokens in milliseconds
   * @param delegationTokenRenewInterval how often the tokens must be renewed in milliseconds
  ...
  public synchronized void loadSecretManagerState(SecretManagerState state) throws IOException {
    ...
    for (SecretManagerSection.DelegationKey k : state.keys) {
      addKey(
          new DelegationKey(
              k.getId(), k.getExpiryDate(), k.hasKey() ? k.getKey().toByteArray() : null));
    }

    for (SecretManagerSection.PersistToken t : state.tokens) {
      DelegationTokenIdentifier id =
          new DelegationTokenIdentifier(
              new Text(t.getOwner()), new Text(t.getRenewer()), new Text(t.getRealUser()));
   ...
   * @throws IOException
   */
  public synchronized void saveSecretManagerStateCompat(DataOutputStream out, String sdPath)
      throws IOException {
  ...
  public synchronized SecretManagerState saveSecretManagerState() {
    SecretManagerSection s =
        SecretManagerSection.newBuilder()
            .setCurrentId(currentId)
            .setTokenSequenceNumber(delegationTokenSequenceNumber)
            .setNumKeys(allKeys.size())
            .setNumTokens(currentTokens.size())
            .build();
    ArrayList<SecretManagerSection.DelegationKey> keys =
        Lists.newArrayListWithCapacity(allKeys.size());
    ArrayList<SecretManagerSection.PersistToken> tokens =
        Lists.newArrayListWithCapacity(currentTokens.size());
    ...
    for (Entry<DelegationTokenIdentifier, DelegationTokenInformation> e :
      ...
      SecretManagerSection.PersistToken.Builder b =
          SecretManagerSection.PersistToken.newBuilder()
              .setOwner(id.getOwner().toString())
              .setRenewer(id.getRenewer().toString())
              .setRealUser(id.getRealUser().toString())
              .setIssueDate(id.getIssueDate())
              .setMaxDate(id.getMaxDate())
   ...
   *
   * @param identifier DelegationTokenIdentifier read from the edit logs or fsimage
  ...
  private final class SerializerCompat {
    ...
    private synchronized void saveCurrentTokens(DataOutputStream out, String sdPath)
        throws IOException {
