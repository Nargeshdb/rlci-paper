...
/**
 * The NNStorageRetentionManager is responsible for inspecting the storage directories of the NN and
 * enforcing a retention policy on checkpoints and edit logs.
 *
 * <p>It delegates the actual removal of files to a StoragePurger implementation, which might delete
 * the files or instead copy them to a filer or HDFS for later analysis.
 */
public class NNStorageRetentionManager {
  ...
  public NNStorageRetentionManager(
      Configuration conf, NNStorage storage, LogsPurgeable purgeableLogs, StoragePurger purger) {
    this.numCheckpointsToRetain =
        conf.getInt(
    ...
    this.maxExtraEditsSegmentsToRetain =
        conf.getInt(
    ...
    Preconditions.checkArgument(
        numExtraEditsToRetain >= 0,
        DFSConfigKeys.DFS_NAMENODE_NUM_EXTRA_EDITS_RETAINED_KEY + " must not be negative");
  ...
  public NNStorageRetentionManager(
      Configuration conf, NNStorage storage, LogsPurgeable purgeableLogs) {
  ...
  void purgeOldStorage(NameNodeFile nnf) throws IOException {
    ...
    if (purgeLogsFrom > minimumRequiredTxId) {
      throw new AssertionError(
          "Should not purge more edits than required to "
              + "restore: "
              + purgeLogsFrom
              + " should be <= "
              + minimumRequiredTxId);
  ...
  private void purgeCheckpointsOlderThan(
      FSImageTransactionalStorageInspector inspector, long minTxId) {
    for (FSImageFile image : inspector.getFoundImages()) {
   ...
   * @return the transaction ID corresponding to the oldest checkpoint that should be retained.
   */
  ...
  /**
   * Delete old OIV fsimages. Since the target dir is not a full blown storage directory, we simply
   * list and keep the latest ones. For the same reason, no storage inspector is used.
   */
  void purgeOldLegacyOIVImages(String dir, long txid) {
    ...
    if (filesInStorage != null && filesInStorage.length <= numCheckpointsToRetain) {
      return;
    }
