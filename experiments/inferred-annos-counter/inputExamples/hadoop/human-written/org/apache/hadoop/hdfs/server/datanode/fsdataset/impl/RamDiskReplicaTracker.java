...
public abstract class RamDiskReplicaTracker {
  ...
  /**
   * Get an instance of the configured RamDiskReplicaTracker based on the the configuration property
   ...
   */
  static RamDiskReplicaTracker getInstance(
      final Configuration conf, final FsDatasetImpl fsDataset) {
    final Class<? extends RamDiskReplicaTracker> trackerClass =
        conf.getClass(
  ...
  /**
   * Invoked when a replica is opened by a client. This may be used as a heuristic by the eviction
   * scheme.
   */
  ...
  /**
   * Invoked if a replica that was previously dequeued for persistence could not be successfully
   * persisted. Add it back so it can be retried later.
   */
  ...
  /**
   * Return a candidate replica to remove from RAM Disk. The exact replica to be returned may depend
   * on the eviction scheme utilized.
   *
   * @return
   */
  ...
  /**
   * Return RamDiskReplica info given block pool id and block id Return null if it does not exist in
   * RamDisk
   */
