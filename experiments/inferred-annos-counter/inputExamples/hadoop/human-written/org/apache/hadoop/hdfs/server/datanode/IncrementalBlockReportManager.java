...
class IncrementalBlockReportManager {
  ...
  private static class PerStorageIBR {
    ...
    ReceivedDeletedBlockInfo[] removeAll() {
      ...
      final ReceivedDeletedBlockInfo[] rdbis =
          blocks.values().toArray(new ReceivedDeletedBlockInfo[size]);
      blocks.clear();
      return rdbis;
    }
  ...
  /**
   * Between block reports (which happen on the order of once an hour) the DN reports smaller
   * incremental changes to its block list for each storage. This map contains the pending changes
   * not yet to be reported to the NN.
   */
  private final Map<DatanodeStorage, PerStorageIBR> pendingIBRs = Maps.newHashMap();

  /**
   * If this flag is set then an IBR will be sent immediately by the actor thread without waiting
   * for the IBR timer to elapse.
   */
  ...
  void sendIBRs(
    ...
    try {
      ...
      if (success) {
        ...
        putMissing(reports);
        LOG.warn(
            "Failed to call blockReceivedAndDeleted: {}, nnId: {}" + ", duration(ms): {}",
  ...
  /**
   * Add a block for notification to NameNode. If another entry exists for the same block it is
   * removed.
   */
  @VisibleForTesting
  ...
  synchronized void notifyNamenodeBlock(
      ReceivedDeletedBlockInfo rdbi, DatanodeStorage storage, boolean isOnTransientStorage) {
