 ...
 *
 * <p>Each implementation defines its own way to persist the configuration. For example, it can use
 * one JSON file to store the configs for all datanodes; or it can use one file to store in-service
 * datanodes and another file to store decommission-requested datanodes.
 *
 * <p>These files control which DataNodes the NameNode expects to see in the cluster.
 */
