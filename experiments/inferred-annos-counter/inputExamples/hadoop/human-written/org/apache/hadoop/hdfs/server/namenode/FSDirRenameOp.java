...
class FSDirRenameOp {
   ...
   */
  private static void verifyQuotaForRename(FSDirectory fsd, INodesInPath src, INodesInPath dst)
    ...
    final QuotaCounts delta =
        src.getLastINode()
            .computeQuotaUsage(
                bsps, dstParent.getStoragePolicyID(), false, Snapshot.CURRENT_STATE_ID);
  ...
  /**
   * Checks file system limits (max component length and max directory items) during a rename
   * operation.
   */
  static void verifyFsLimitsForRename(FSDirectory fsd, INodesInPath srcIIP, INodesInPath dstIIP)
  ...
  /**
   * <br>
   * Note: This is to be used by {@link FSEditLogLoader} only. <br>
   */
  @Deprecated
  static INodesInPath renameForEditLog(FSDirectory fsd, String src, String dst, long timestamp)
      throws IOException {
   ...
   * @deprecated See {@link #renameToInt(FSDirectory, String, String, boolean, Options.Rename...)}
   */
  @Deprecated
  static INodesInPath unprotectedRenameTo(
      FSDirectory fsd, final INodesInPath srcIIP, final INodesInPath dstIIP, long timestamp)
      throws IOException {
  ...
  /**
   * @see {@link #unprotectedRenameTo(FSDirectory, String, String, INodesInPath, INodesInPath, long,
   *     BlocksMapUpdateInfo, Options.Rename...)}
   */
  static RenameResult renameTo(
      FSDirectory fsd,
      FSPermissionChecker pc,
      String src,
      String dst,
  ...
  /**
   * Rename src to dst. <br>
   * Note: This is to be used by {@link org.apache.hadoop.hdfs.server .namenode.FSEditLogLoader}
   * only. <br>
   *
   * @param fsd FSDirectory
  ...
  static void renameForEditLog(
      FSDirectory fsd, String src, String dst, long timestamp, Options.Rename... options)
      throws IOException {
    BlocksMapUpdateInfo collectedBlocks = new BlocksMapUpdateInfo();
    final INodesInPath srcIIP = fsd.getINodesInPath(src, DirOp.WRITE_LINK);
  ...
  /**
   * Rename src to dst. See {@link DistributedFileSystem#rename(Path, Path, Options.Rename...)} for
  ...
  static RenameResult unprotectedRenameTo(
    ...
    assert fsd.hasWriteLock();
    boolean overwrite =
        options != null && Arrays.asList(options).contains(Options.Rename.OVERWRITE);
   ...
   */
  @Deprecated
  private static RenameResult renameTo(
      FSDirectory fsd,
      FSPermissionChecker pc,
      INodesInPath srcIIP,
      INodesInPath dstIIP,
    ...
    if (NameNode.stateChangeLog.isDebugEnabled()) {
      NameNode.stateChangeLog.debug(
          "DIR* FSDirectory.renameTo: " + srcIIP.getPath() + " to " + dstIIP.getPath());
    }
    final long mtime = Time.now();
  ...
  private static void validateOverwrite(
    ...
    if (dstInode.isDirectory()) {
      final ReadOnlyList<INode> children =
          dstInode.asDirectory().getChildrenList(Snapshot.CURRENT_STATE_ID);
      if (!children.isEmpty()) {
  ...
  private static void validateRenameSource(
      FSDirectory fsd, INodesInPath srcIIP, List<INodeDirectory> snapshottableDirs)
      throws IOException {
    String error;
    final INode srcInode = srcIIP.getLastINode();
    // validate source
  ...
  private static void validateNestSnapshot(
      FSDirectory fsd,
      String srcPath,
      INodeDirectory dstParent,
      List<INodeDirectory> snapshottableDirs)
    ...
    if (snapshottableDirs != null && snapshottableDirs.size() > 0) {
      if (fsd.getFSNamesystem().getSnapshotManager().isDescendantOfSnapshotRoot(dstParent)) {
        ...
        throw new SnapshotException(
            "Unable to rename because "
                + srcPath
                + " has snapshottable descendant directories and "
                + dstPath
  ...
  private static class RenameOperation {
    ...
    RenameOperation(FSDirectory fsd, INodesInPath srcIIP, INodesInPath dstIIP) {
      ...
      srcRefDstSnapshot =
          srcChildIsReference
              ? srcChild.asReference().getDstSnapshotId()
              : Snapshot.CURRENT_STATE_ID;
      oldSrcCounts = new QuotaCounts.Builder().build();
      if (isSrcInSnapshot) {
        final INodeReference.WithName withName =
            srcParent.replaceChild4ReferenceWithName(srcChild, srcLatestSnapshotId);
    ...
    boolean removeSrc4OldRename() {
      ...
      if (removedSrc == -1) {
        NameNode.stateChangeLog.warn(
            "DIR* FSDirRenameOp.unprotectedRenameTo: "
                + "failed to rename "
                + srcIIP.getPath()
                + " to "
                + dstIIP.getPath()
    ...
    INodesInPath addSourceToDestination() {
      ...
      if (withCount == null) {
        ...
        toDst =
            new INodeReference.DstReference(
                dstParent.asDirectory(), withCount, dstIIP.getLatestSnapshotId());
      }
  ...
  private static RenameResult createRenameResult(
      FSDirectory fsd, INodesInPath dst, boolean filesDeleted, BlocksMapUpdateInfo collectedBlocks)
      throws IOException {
