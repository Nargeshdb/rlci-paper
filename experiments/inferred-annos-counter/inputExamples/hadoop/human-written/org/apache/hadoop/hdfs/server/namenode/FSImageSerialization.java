...
/**
 * Static utility functions for serializing various pieces of data in the correct format for the
 * FSImage file.
 *
 * <p>Some members are currently public for the benefit of the Offline Image Viewer which is located
 * outside of this package. These members should be made package-protected when the OIV is
 * refactored.
 */
...
public class FSImageSerialization {
  ...
  /**
   * In order to reduce allocation, we reuse some static objects. However, the methods in this class
   * should be thread-safe since image-saving is multithreaded, so we need to keep the static
   * objects in a thread-local.
   */
  ...
  //
  static INodeFile readINodeUnderConstruction(DataInput in, FSNamesystem fsNamesys, int imgVersion)
      throws IOException {
  ...
  //
  static void writeINodeUnderConstruction(DataOutputStream out, INodeFile cons, String path)
      throws IOException {
  ...
  public static void writeINodeFileAttributes(INodeFileAttributes file, DataOutput out)
      throws IOException {
    writeLocalName(file, out);
    writePermissionStatus(file, out);
    out.writeLong(file.getModificationTime());
  ...
  private static void writeINodeReference(
      INodeReference ref, DataOutput out, boolean writeUnderConstruction, ReferenceMap referenceMap)
      throws IOException {
  ...
  public static void saveINode2Image(
      INode node, DataOutput out, boolean writeUnderConstruction, ReferenceMap referenceMap)
      throws IOException {
    if (node.isReference()) {
  ...
  /**
   * Reading the path from the image and converting it to byte[][] directly this saves us an array
   * copy and conversions to and from String
   *
   * @param in input to read from
   * @return the array each element of which is a byte[] representation of a path component
   * @throws IOException
   */
  @SuppressWarnings("deprecation")
  ...
  /**
   * Write an array of blocks as compactly as possible. This uses delta-encoding for the generation
   * stamp and size, following the principle that genstamp increases relatively slowly, and size is
   * equal for all but the last block of a file.
   */
  ...
  public static void writeCacheDirectiveInfo(
      ContentHandler contentHandler, CacheDirectiveInfo directive) throws SAXException {
    ...
    if (directive.getReplication() != null) {
      XMLUtils.addSaxString(
          contentHandler, "REPLICATION", Short.toString(directive.getReplication()));
    }
    if (directive.getPool() != null) {
    ...
    if (directive.getExpiration() != null) {
      XMLUtils.addSaxString(
          contentHandler, "EXPIRATION", "" + directive.getExpiration().getAbsoluteMillis());
    }
  }
  ...
  public static void writeCachePoolInfo(DataOutputStream out, CachePoolInfo info)
    ...
    boolean hasOwner, hasGroup, hasMode, hasLimit, hasMaxRelativeExpiry, hasDefaultReplication;
  ...
  public static void writeCachePoolInfo(ContentHandler contentHandler, CachePoolInfo info)
      throws SAXException {
    ...
    if (defaultReplication != null) {
      XMLUtils.addSaxString(
          contentHandler, "DEFAULTREPLICATION", Short.toString(defaultReplication));
    }
  }
  ...
  public static CachePoolInfo readCachePoolInfo(Stanza st) throws InvalidXmlException {
    ...
    if (st.hasChildren("DEFAULTREPLICATION")) {
      info.setDefaultReplication(Short.parseShort(st.getValue("DEFAULTREPLICATION")));
    }
    return info;
  }

  public static void writeErasureCodingPolicy(DataOutputStream out, ErasureCodingPolicy ecPolicy)
      throws IOException {
  ...
  public static ErasureCodingPolicy readErasureCodingPolicy(DataInput in) throws IOException {
    ...
    ECSchema ecSchema = new ECSchema(codecName, numDataUnits, numParityUnits, extraOptions);
