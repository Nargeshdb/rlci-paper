...
/**
 * Manage striped readers that performs reading of block data from remote to serve input data for
...
class StripedReader {
  ...
  StripedReader(
      StripedReconstructor reconstructor,
      DataNode datanode,
      Configuration conf,
      StripedReconstructionInfo stripedReconInfo) {
    stripedReadTimeoutInMills =
        conf.getInt(
    ...
    int cellsNum =
        (int)
            ((stripedReconInfo.getBlockGroup().getNumBytes() - 1)
                    / stripedReconInfo.getEcPolicy().getCellSize()
                + 1);
    ...
    xmits =
        Math.max(
            minRequiredSources,
            stripedReconInfo.getTargets() != null ? stripedReconInfo.getTargets().length : 0);
    ...
    Preconditions.checkArgument(
        liveIndices.length >= minRequiredSources, "No enough live striped blocks.");
    Preconditions.checkArgument(
        liveIndices.length == sources.length, "liveBlockIndices and source datanodes should match");
  }
  ...
  private void initReaders() throws IOException {
    ...
    if (nSuccess < minRequiredSources) {
      String error =
          "Can't find minimum sources required by "
  ...
  private void initBufferSize() {
    ...
    bufferSize =
        readBufferSize < bytesPerChecksum
            ? bytesPerChecksum
  ...
  /**
   * Read from minimum source DNs required for reconstruction in the iteration. First try the
   * success list which we think they are the best DNs If source DN is corrupt or slow, try to read
   * some other source DN, and will update the success list.
   *
   * <p>Remember the updated success list and return it for following operations and next iteration
   * read.
   *
  ...
  int[] doReadMinimumSources(int reconstructLength, CorruptedBlocks corruptedBlocks)
      throws IOException {
    ...
    for (int i = 0; i < minRequiredSources; i++) {
      ...
      if (toRead > 0) {
        Callable<BlockReadStats> readCallable = reader.readFromBlock(toRead, corruptedBlocks);
    ...
    if (nSuccess < minRequiredSources) {
      String error =
          "Can't read data from minimum number of sources "
  ...
  /**
   * Schedule a read from some new source DN if some DN is corrupted or slow, this is called from
   * the read iteration. Initially we may only have <code>minRequiredSources</code> number of
   * StripedBlockReader. If the position is at the end of target block, don't need to do real read,
   * and return the array index of source DN, otherwise -1.
   *
   * @param used the used source DNs in this iteration.
   * @return the array index of source DN if don't need to do real read.
   */
  private int scheduleNewRead(BitSet used, int reconstructLength, CorruptedBlocks corruptedBlocks) {
   ...
   *
   * <p>DN uses it to coordinate with NN to adjust the speed of scheduling the EC reconstruction
   * tasks to this DN.
   *
