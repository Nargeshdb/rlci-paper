...
/**
 * StripedBlockReader is used to read block data from one source DN, it contains a block reader,
 * read buffer and striped block index. Only allocate StripedBlockReader once for one source, and
 * the StripedReader has the same array order with sources. Typically we only need to allocate
 * minimum number (minRequiredSources) of StripedReader, and allocate new for new source DN if some
 * existing DN invalid or slow. If some source DN is corrupt, set the corresponding blockReader to
  ...
  private BlockReader createBlockReader(long offsetInBlock) {
    ...
    try {
      ...
      return BlockReaderRemote.newBlockReader(
          ...
          source,
          null,
          stripedReader.getCachingStrategy(),
          -1,
          conf);
    } catch (IOException e) {
    ...
    try {
      ...
      peer =
          DFSUtilClient.peerFromSocketAndKey(
              datanode.getSaslClient(),
              sock,
              datanode.getDataEncryptionKeyFactoryForBlock(b),
              blockToken,
              datanodeId,
              socketTimeout);
      success = true;
