...
public class INodeDirectory extends INodeWithAdditionalFields implements INodeDirectoryAttributes {
   ...
   * @param adopt Indicate whether or not need to set the parent field of child INodes to the new
   *     node
   * @param featuresToCopy any number of features to copy to the new node. The method will do a
   *     reference copy, not a deep copy.
   */
  ...
  /**
   * If the directory contains a {@link DirectoryWithQuotaFeature}, return it; otherwise, return
   * null.
   */
  ...
  /**
   * If feature list contains a {@link DirectoryWithSnapshotFeature}, return it; otherwise, return
   * null.
   */
  ...
  public Snapshot addSnapshot(
    ...
    return getDirectorySnapshottableFeature()
        .addSnapshot(this, id, name, leaseManager, captureOpenFiles, maxSnapshotLimit, mtime);
  }
   ...
   */
  public Snapshot removeSnapshot(ReclaimContext reclaimContext, String snapshotName, long mtime)
      throws SnapshotException {
    return getDirectorySnapshottableFeature()
        .removeSnapshot(reclaimContext, this, snapshotName, mtime);
  }
  ...
  public void renameSnapshot(String path, String oldName, String newName, long mtime)
      throws SnapshotException {
    getDirectorySnapshottableFeature().renameSnapshot(path, oldName, newName, mtime);
  }
  ...
  public void removeSnapshottableFeature() {
    ...
    Preconditions.checkState(
        s != null, "The dir does not have snapshottable feature: this=%s", this);
    this.removeFeature(s);
  ...
  /**
   * Replace the given child with a new child. Note that we no longer need to replace an normal
   * INodeDirectory or INodeFile into an INodeDirectoryWithSnapshot or INodeFileUnderConstruction.
   * The only cases for child replacement is for reference nodes.
   */
  ...
  INodeReference.WithName replaceChild4ReferenceWithName(INode oldChild, int latestSnapshotId) {
    ...
    final INodeReference.WithName ref =
        new INodeReference.WithName(
            this, withCount, oldChild.getLocalNameBytes(), latestSnapshotId);
   ...
   */
  public INode saveChild2Snapshot(
   ...
   * @param snapshotId if it is not {@link Snapshot#CURRENT_STATE_ID}, get the result from the
   *     corresponding snapshot; otherwise, get the result from the current directory.
   ...
   *
   * @return {@link Snapshot#CURRENT_STATE_ID} if the inode is in the children list; {@link
   *     Snapshot#NO_SNAPSHOT_ID} if the inode is neither in the children list nor in any snapshot;
   *     otherwise the snapshot id of the corresponding snapshot diff list.
   */
   ...
   * @param snapshotId if it is not {@link Snapshot#CURRENT_STATE_ID}, get the result from the
   *     corresponding snapshot; otherwise, get the result from the current directory.
   * @return the current children list if the specified snapshot is null; otherwise, return the
   *     children list corresponding to the snapshot. Note that the returned list is never null.
   */
  ...
  /**
   * Remove the specified child from this directory. The basic remove method which actually calls
   * children.remove(..).
   *
   ...
   * @param setModTime set modification time for the parent node not needed when replaying the
   *     addition and the parent already has the proper mod time
   * @return false if the child with this name already exists; otherwise, return true;
   */
  ...
  /**
   * During image loading, the search is unnecessary since the insert position should always be at
   * the end of the map given the sequence they are serialized on disk.
   */
  ...
  /**
   * Add the node to the children list at the given insertion point. The basic add method which
   * actually calls children.add(..).
   */
  ...
  public QuotaCounts computeQuotaUsage(
    ...
    if (useCache && q != null && q.isQuotaSet()) { // use the cached quota
      ...
      return computeDirectoryQuotaUsage(
          bsps, blockStoragePolicyId, counts, useCache, lastSnapshotId);
    }
  }

  private QuotaCounts computeDirectoryQuotaUsage(
      BlockStoragePolicySuite bsps,
      byte blockStoragePolicyId,
      QuotaCounts counts,
      boolean useCache,
  ...
  protected ContentSummaryComputationContext computeDirectoryContentSummary(
      ContentSummaryComputationContext summary, int snapshotId) throws AccessControlException {
   ...
   *
   * <p>Before calling this function, in the rename operation, we replace the original src node (of
   * the rename operation) with a reference node (WithName instance) in both the children list and a
   * created list, delete the reference node from the children list, and add it to the corresponding
   * deleted list.
   *
   * <p>To undo the above operations, we have the following steps in particular:
   *
   * <pre>
   ...
   */
  public void undoRename4ScrParent(final INodeReference oldChild, final INode newChild) {
    DirectoryWithSnapshotFeature sf = getDirectoryWithSnapshotFeature();
  ...
  /**
   * Undo the rename operation for the dst tree, i.e., if the rename operation (with OVERWRITE
   * option) removes a file/dir from the dst tree, add it back and delete possible record in the
   * deleted list.
   */
  public void undoRename4DstParent(
      final BlockStoragePolicySuite bsps, final INode deletedChild, int latestSnapshotId) {
    DirectoryWithSnapshotFeature sf = getDirectoryWithSnapshotFeature();
    assert sf != null : "Directory does not have snapshot feature";
  ...
  public void cleanSubtreeRecursively(
    ...
    int s =
        snapshot != Snapshot.CURRENT_STATE_ID && prior != Snapshot.NO_SNAPSHOT_ID
            ? prior
            : snapshot;
  ...
  public void dumpTreeRecursively(PrintWriter out, StringBuilder prefix, final int snapshot) {
    ...
    final DirectoryWithSnapshotFeature snapshotFeature = getDirectoryWithSnapshotFeature();
   ...
   */
  @VisibleForTesting
  public static void dumpTreeRecursively(
      PrintWriter out, StringBuilder prefix, Iterable<SnapshotAndINode> subs) {
    if (subs != null) {
