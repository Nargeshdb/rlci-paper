...
public abstract class BlockPlacementPolicy {
  public static final Logger LOG = LoggerFactory.getLogger(BlockPlacementPolicy.class);
  ...
  /**
   * choose <i>numOfReplicas</i> data nodes for <i>writer</i> to re-replicate a block with size
   * <i>blocksize</i> If not, return as many as we can.
   *
  ...
  /**
   * @param favoredNodes datanodes that should be favored as targets. This is only a hint and due to
   *     cluster state, namenode may not be able to place the blocks on these datanodes.
   */
  ...
  /**
   * Verify if the block's placement meets requirement of placement policy, i.e. replicas are placed
   * on no less than minRacks racks in the system.
   *
  ...
  /**
   * Select the excess replica storages for deletion based on either delNodehint/Excess storage
   * types.
   *
   * @param availableReplicas available replicas
   * @param delCandidates Candidates for deletion. For normal replication, this set is the same with
   *     availableReplicas. For striped blocks, this set is a subset of availableReplicas.
  ...
  /**
   * Used to setup a BlockPlacementPolicy object. This should be defined by all implementations of a
  ...
  public abstract boolean isMovable(
      Collection<DatanodeInfo> candidates, DatanodeInfo source, DatanodeInfo target);
  ...
  protected <T> DatanodeInfo getDatanodeInfo(T datanode) {
    Preconditions.checkArgument(
        datanode instanceof DatanodeInfo || datanode instanceof DatanodeStorageInfo,
  ...
  /**
   * Split data nodes into two sets, one set includes nodes on rack with more than one replica, the
   * other set contains the remaining nodes.
   *
