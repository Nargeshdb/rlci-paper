 ...
 *
 * <p>For InetSocketAddress A and B with the same host address, we define a partial order between A
 * and B, A &lt;= B iff A.getPort() == B .getPort() || B.getPort() == 0.
 */
public class HostSet implements Iterable<InetSocketAddress> {
  ...
  /**
   * The function that checks whether there exists an entry foo in the set so that foo &lt;= addr.
   */
