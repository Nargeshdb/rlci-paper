...
/**
 * Class that represents a file on disk which stores a single <code>long</code> value, but does not
 * make any effort to make it truly durable. This is in contrast to {@link PersistentLongFile} which
 * fsync()s the value on every change.
 *
 * <p>This should be used for values which are updated frequently (such that performance is
 * important) and not required to be up-to-date for correctness.
 *
 * <p>This class also differs in that it stores the value as binary data instead of a textual
 * string.
 */
...
public class BestEffortLongFile implements Closeable {
  ...
  }) // FP nullness reasoning: overwrite ch only if it's null (validated)
  @CreatesMustCallFor("this")
