...
/**
 * Provide an cyclic {@link Iterator} for a {@link NavigableMap}. The {@link Iterator} navigates the
 * entries of the map according to the map's ordering. If the {@link Iterator} hits the last entry
 * of the map, it will then continue from the first entry.
 */
...
public class CyclicIteration<K, V> implements Iterable<Map.Entry<K, V>> {
  ...
  /**
   * Construct an {@link Iterable} object, so that an {@link Iterator} can be created for iterating
   * the given {@link NavigableMap}. The iteration begins from the starting key exclusively.
   */
