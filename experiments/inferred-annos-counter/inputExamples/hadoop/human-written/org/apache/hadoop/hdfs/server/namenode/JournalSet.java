...
/**
 * Manages a collection of Journals. None of the methods are synchronized, it is assumed that
 * FSEditLog methods, that use this class, use proper synchronization.
 */
...
public class JournalSet implements JournalManager {
  ...
  private static final Comparator<EditLogInputStream> LOCAL_LOG_PREFERENCE_COMPARATOR =
  ...
  public static final Comparator<EditLogInputStream> EDIT_LOG_INPUT_STREAM_COMPARATOR =
      Comparator.comparing(EditLogInputStream::getFirstTxId)
          .thenComparing(EditLogInputStream::getLastTxId);

  /**
   * Container for a JournalManager paired with its currently active stream.
   *
   * <p>If a Journal gets disabled due to an error writing to its stream, then the stream will be
   * aborted and set to null.
   */
  ...
  @Override
  public EditLogOutputStream startLogSegment(final long txId, final int layoutVersion)
      throws IOException {
  ...
  /**
   * In this function, we get a bunch of streams from all of our JournalManager objects. Then we add
   * these to the collection one by one.
   *
   * @param streams The collection to add the streams to. It may or may not be sorted-- this is up
   *     to the caller.
   ...
   * @param onlyDurableTxns Set to true if streams are bounded by the durable TxId. A durable TxId
   *     is the committed txid in QJM or the largest txid written into file in FJM
   */
  @Override
  public void selectInputStreams(
    ...
    final PriorityQueue<EditLogInputStream> allStreams =
        new PriorityQueue<EditLogInputStream>(64, EDIT_LOG_INPUT_STREAM_COMPARATOR);
      ...
      if (acc.isEmpty()) {
        ...
        if (accFirstTxId == elis.getFirstTxId()) {
          ...
          throw new RuntimeException(
              "sorted set invariants violated!  "
                  + "Got stream with first txid "
                  + elis.getFirstTxId()
                  + ", but the last firstTxId was "
                  + accFirstTxId);
        }
      }
    }
  ...
  /**
   * Returns true if there are no journals, all redundant journals are disabled, or any required
   * journals are disabled.
   *
   * @return True if there no journals, all redundant journals are disabled, or any required
   *     journals are disabled.
   */
  ...
  /**
   * Implementations of this interface encapsulate operations that can be iteratively applied on all
   * the journals. For example see {@link JournalSet#mapJournalsAndReportErrors}.
   */
  ...
  /**
   * Apply the given operation across all of the journal managers, disabling any for which the
   * closure throws an IOException.
   *
  ...
  /**
   * An implementation of EditLogOutputStream that applies a requested method on all the journals
   ...
   * returned starting from the transaction id passed. If 'fromTxId' falls in the middle of a log,
   * that log is returned as well.
   *
