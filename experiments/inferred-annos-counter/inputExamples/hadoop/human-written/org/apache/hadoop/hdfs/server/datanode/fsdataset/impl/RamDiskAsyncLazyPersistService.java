...
/**
 * This class is a container of multiple thread pools, one for each non-RamDisk volume with a
 * maximum thread count of 1 so that we can schedule async lazy persist operations easily with
 * volume arrival and departure handled.
 *
...
class RamDiskAsyncLazyPersistService {
  public static final Logger LOG = LoggerFactory.getLogger(RamDiskAsyncLazyPersistService.class);
  ...
  /**
   * Create a RamDiskAsyncLazyPersistService with a set of volumes (specified by their root
   * directories).
   *
   * <p>The RamDiskAsyncLazyPersistService uses one ThreadPool per volume to do the async disk
   * operations.
   */
  ...
  synchronized void removeVolume(FsVolumeImpl volume) {
    ...
    if (executor == null) {
      throw new RuntimeException(
          "Can not find volume with storage id " + storageId + " to remove.");
    } else {
      executor.shutdown();
   ...
   * @return true if there is one thread pool for the volume false otherwise
   */
  ...
  synchronized void execute(String storageId, Runnable task) {
    ...
    if (executor == null) {
      throw new RuntimeException(
          "Cannot find root storage volume with id "
              + storageId
  ...
  /** Gracefully shut down all ThreadPool. Will wait for all lazy persist tasks to finish. */
  ...
  void submitLazyPersistTask(
    ...
    ReplicaLazyPersistTask lazyPersistTask =
        new ReplicaLazyPersistTask(bpId, blockId, genStamp, creationTime, replica, target);
  ...
  class ReplicaLazyPersistTask implements Runnable {
    ...
    public void run() {
      ...
      try (FsVolumeReference ref = this.targetVolume) {
        ...
        File[] targetFiles =
            volume.copyBlockToLazyPersistLocation(
                bpId, blockId, genStamp, replicaInfo, smallBufferSize, conf);
