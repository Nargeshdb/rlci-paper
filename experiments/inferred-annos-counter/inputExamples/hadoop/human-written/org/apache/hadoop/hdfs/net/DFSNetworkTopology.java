...
/**
 * The HDFS specific network topology class. The main purpose of doing this subclassing is to add
 * storage-type-aware chooseRandom method. All the remaining parts should be the same.
 *
 * <p>Currently a placeholder to test storage type info.
 */
public class DFSNetworkTopology extends NetworkTopology {
   ...
   *
   * <p>If scope starts with ~, choose one from the all nodes except for the ones in <i>scope</i>;
   * otherwise, choose one from <i>scope</i>. If excludedNodes is given, choose a node that's not in
   * excludedNodes.
   *
  ...
  public Node chooseRandomWithStorageType(
    ...
    try {
      if (scope.startsWith("~")) {
        return chooseRandomWithStorageType(NodeBase.ROOT, scope.substring(1), excludedNodes, type);
      } else {
        return chooseRandomWithStorageType(scope, null, excludedNodes, type);
      }
    } finally {
   ...
   * excludedNodes.
   *
   * <p>This call would make up to two calls. It first tries to get a random node (with old method)
   * and check if it satisfies. If yes, simply return it. Otherwise, it make a second call (with the
   * new method) by passing in a storage type.
   *
   * <p>This is for better performance reason. Put in short, the key note is that the old method is
   * faster but may take several runs, while the new method is somewhat slower, and always succeed
   * in one trial. See HDFS-11535 for more detail.
   *
   * @param scope range of nodes from which a node will be chosen
   * @param excludedNodes nodes to be excluded from
   * @param type the storage type we search for
    ...
    try {
      ...
      if (dnDescriptor.hasStorageType(type)) {
        ...
        LOG.debug(
            ...
            type);
        return chooseRandomWithStorageType(searchScope, excludedScope, excludedNodes, type);
      }
    } finally {
      netlock.readLock().unlock();
    }
  ...
  /**
   * Choose a random node based on given scope, excludedScope and excludedNodes set. Although in
   * general the topology has at most three layers, this class will not impose such assumption.
   *
   * <p>At high level, the idea is like this, say:
   *
   * <p>R has two children A and B, and storage type is X, say: A has X = 6 (rooted at A there are 6
   * datanodes with X) and B has X = 8.
   *
   * <p>Then R will generate a random int between 1~14, if it's <= 6, recursively call into A,
   * otherwise B. This will maintain a uniformed randomness of choosing datanodes.
   *
   * <p>The tricky part is how to handle excludes.
   *
   * <p>For excludedNodes, since this set is small: currently the main reason of being an excluded
   * node is because it already has a replica. So randomly picking up this node again should be
   * rare. Thus we only check that, if the chosen node is excluded, we do chooseRandom again.
   *
   * <p>For excludedScope, we locate the root of the excluded scope. Subtracting all it's ancestors'
   * storage counters accordingly, this way the excluded root is out of the picture.
   *
   ...
   */
  @VisibleForTesting
  Node chooseRandomWithStorageType(
      final String scope,
      String excludedScope,
  ...
  private boolean isNodeInScope(Node node, String scope) {
    ...
    String nodeLocation = node.getNetworkLocation() + NodeBase.PATH_SEPARATOR_STR;
    return nodeLocation.startsWith(scope);
  }

  /**
   * Choose a random node that has the required storage type, under the given root, with an excluded
   * subtree root (could also just be a leaf node).
   *
   * <p>Note that excludedNode is checked after a random node, so it is not being handled here.
   *
   ...
   * @return a random datanode, with the storage type, and is not in excluded scope
   */
  ...
  /**
   * Given root, excluded root and storage type. Find all the children of the root, that has the
   * storage type available. One check is that if the excluded root is under a children, this
   * children must subtract the storage count of the excluded root.
   *
   ...
   * @return a list of possible nodes, each of them is eligible as the next level root we search.
   */
