...
public class FileWithSnapshotFeature implements INode.Feature {
  ...
  /**
   * We need to distinguish two scenarios: 1) the file is still in the current file directory, it
   * has been modified before while it is included in some snapshot 2) the file is not in the
   * current file directory (deleted), but it is in some snapshot, thus we still keep this inode For
   * both scenarios the file has snapshot feature. We set {@link #isCurrentFileDeleted} to true for
   * 2).
   */
  ...
  boolean changedBetweenSnapshots(INodeFile file, Snapshot from, Snapshot to) {
    ...
    final long laterLength =
        laterDiffIndex == diffList.size()
            ? file.computeFileSize(true, false)
            : diffList.get(laterDiffIndex).getFileSize();
    ...
    INodeFileAttributes laterAttr =
        diffs.getSnapshotINode(
            Math.max(Snapshot.getSnapshotId(from), Snapshot.getSnapshotId(to)), file);
  ...
  public void cleanFile(
    ...
    if (snapshotId == Snapshot.CURRENT_STATE_ID) {
      ...
      final BlockStoragePolicy policy =
          reclaimContext.storagePolicySuite().getPolicy(storagePolicyId);
  ...
  public void updateQuotaAndCollectBlocks(
   ...
   */
  public void collectBlocksAndClear(INode.ReclaimContext reclaimContext, final INodeFile file) {
    ...
    else file.collectBlocksBeyondSnapshot(snapshotBlocks, reclaimContext.collectedBlocks());
  }
