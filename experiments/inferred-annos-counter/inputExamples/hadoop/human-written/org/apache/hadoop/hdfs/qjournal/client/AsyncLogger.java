...
/**
 * Interface for a remote log which is only communicated with asynchronously. This is essentially a
 * wrapper around {@link QJournalProtocol} with the key differences being:
 *
 * <ul>
 *   <li>All methods return {@link ListenableFuture}s instead of synchronous objects.
 *   <li>The {@link RequestInfo} objects are created by the underlying implementation.
 * </ul>
 */
interface AsyncLogger {

  interface Factory {
    AsyncLogger createLogger(
        Configuration conf,
  ...
  /**
   * Let the logger know the highest committed txid across all loggers in the set. This txid may be
   * higher than the last committed txid for <em>this</em> logger. See HDFS-3863 for details.
   */
  ...
  /**
   * Tear down any resources, connections, etc. The proxy may not be used after this point, and any
   * in-flight RPCs may throw an exception.
   */
  public void close();

  /**
   * Append an HTML-formatted report for this logger's status to the provided StringBuilder. This is
   * displayed on the NN web UI.
   */
  ...
  public ListenableFuture<Boolean> canRollBack(
      StorageInfo storage, StorageInfo prevStorage, int targetLayoutVersion);
