...
public class NNStorage extends Storage implements Closeable, StorageErrorReporter {
  ...
  /**
   * Implementation of StorageDirType specific to namenode storage A Storage directory could be of
   * type IMAGE which stores only fsimage, or of type EDITS which stores edits or of type
   * IMAGE_AND_EDITS which stores both fsimage and edits.
   */
  @VisibleForTesting
  ...
  /**
   * TxId of the last transaction that was included in the most recent fsimage file. This does not
   * include any transactions that have since been written to the edit log.
   */
  ...
  protected final List<StorageDirectory> removedStorageDirs = new CopyOnWriteArrayList<>();

  /** Properties from old layout versions that may be needed during upgrade only. */
  ...
  public NNStorage(Configuration conf, Collection<URI> imageDirs, Collection<URI> editsDirs)
    ...
    setStorageDirectories(
        imageDirs, Lists.newArrayList(editsDirs), FSNamesystem.getSharedEditsDirs(conf));
  ...
  /**
   * Set flag whether an attempt should be made to restore failed storage directories at the next
   * available oppurtuinity.
   *
  ...
  void attemptRestoreRemovedStorage() {
    ...
    synchronized (this.restorationLock) {
      LOG.info(
          "NNStorage.attemptRestoreRemovedStorage: check removed(failed) "
              + "storage. removedStorages size = {}",
          removedStorageDirs.size());
      for (StorageDirectory sd : this.removedStorageDirs) {
        File root = sd.getRoot();
        LOG.info(
            "currently disabled dir {}; type={} ;canwrite={}",
  ...
  @VisibleForTesting
  synchronized void setStorageDirectories(Collection<URI> fsNameDirs, Collection<URI> fsEditsDirs)
      throws IOException {
    setStorageDirectories(fsNameDirs, fsEditsDirs, new ArrayList<>());
  }

  /**
   * Set the storage directories which will be used. This should only ever be called from inside
   * NNStorage. However, it needs to remain package private for testing, as StorageDirectories need
   * to be reinitialised after using Mockito.spy() on this class, as Mockito doesn't work well with
   * inner classes, such as StorageDirectory in this case.
   *
   * <p>Synchronized due to initialization of storageDirs and removedStorageDirs.
   *
   ...
   */
  @VisibleForTesting
  synchronized void setStorageDirectories(
      Collection<URI> fsNameDirs, Collection<URI> fsEditsDirs, Collection<URI> sharedEditsDirs)
      throws IOException {
    getStorageDirs().clear();
  ...
  /**
   * Return the list of locations being used for a specific purpose. i.e. Image or edit log storage.
   *
    ...
    for (; it.hasNext(); ) {
      ...
      try {
        ...
        throw new IOException(
            "Exception while processing " + "StorageDirectory " + sd.getRoot(), e);
      }
    }
    return list;
  }
  ...
  /**
   * Determine the last transaction ID noted in this storage directory. This txid is stored in a
   * special seen_txid file since it might not correspond to the latest image or edit log. For
   * example, an image-only directory will have this txid incremented when edits logs roll, even
  ...
  /**
   * Write a small file in all available storage directories that indicates that the namespace has
   * reached some given transaction ID.
   *
   * <p>This is used when the image is loaded to avoid accidental rollbacks in the case where an
   * edit log is fully deleted but there is no checkpoint. See
  ...
  /**
   * @return The first image file whose txid is the same with the given txid and image type is one
   *     of the given types.
   */
  ...
  public static NamespaceInfo newNamespaceInfo() throws UnknownHostException {
    return new NamespaceInfo(newNamespaceID(), newClusterID(), newBlockPoolID(), Time.now());
  }
   ...
   *
   * <p>namespaceID is a persistent attribute of the namespace. It is generated when the namenode is
   * formatted and remains the same during the life cycle of the namenode. When a datanodes register
   * they receive it as the registrationID, which is checked every time the datanode is
   * communicating with the namenode. Datanodes that do not 'know' the namespaceID are rejected.
   *
  ...
  void readProperties(StorageDirectory sd, StartupOption startupOption) throws IOException {
    ...
    if (props == null) {
      throw new IOException("Properties not found  for storage directory " + sd);
    }
    if (HdfsServerConstants.RollingUpgradeStartupOption.ROLLBACK.matches(startupOption)) {
      ...
      if (lv > getServiceLayoutVersion()) {
        ...
        throw new IncorrectVersionException(
            getServiceLayoutVersion(), lv, "storage directory " + sd.getRoot().getAbsolutePath());
      }
      props.setProperty(
          "layoutVersion", Integer.toString(HdfsServerConstants.NAMENODE_LAYOUT_VERSION));
    }
    setFieldsFromProperties(props, sd);
  }

  /**
   * Pull any properties out of the VERSION file that are from older versions of HDFS and only
   * necessary during upgrade.
   */
   ...
   *
   * <p>This should only be used during upgrades.
   */
   ...
   *
   * <p>The version file should always be written last. Missing or corrupted version file indicates
   * that the checkpoint is not valid.
   *
   * @param sd storage directory
   * @throws IOException
   */
  ...
  /**
   * Get a storage file for one of the files that doesn't need a txid associated (e.g version,
   * seen_txid).
   */
  ...
  public static File getFinalizedEditsFile(StorageDirectory sd, long startTxId, long endTxId) {
    return new File(sd.getCurrentDir(), getFinalizedEditsFileName(startTxId, endTxId));
  }

  public static File getTemporaryEditsFile(
      StorageDirectory sd, long startTxId, long endTxId, long timestamp) {
  ...
  public static String getTemporaryEditsFileName(long startTxId, long endTxId, long timestamp) {
    return String.format(
        "%s_%019d-%019d_%019d", NameNodeFile.EDITS_TMP.getName(), startTxId, endTxId, timestamp);
  }
  ...
  /**
   * Return the first readable image file for the given txid and image type, or null if no such
   * image can be found.
   */
  ...
  /**
   * Return the first readable storage file of the given name across any of the 'current'
   * directories in SDs of the given type, or null if no such file exists.
   */
  ...
  /**
   * Reports that a directory has experienced an error. Notifies listeners that the directory is no
   * longer available.
   *
    ...
    if (startOpt == StartupOption.UPGRADE || startOpt == StartupOption.UPGRADEONLY) {
      ...
      if (!NameNodeLayoutVersion.supports(LayoutVersion.Feature.FEDERATION, layoutVersion)) {
        ...
        if (startOpt.getClusterId() != null && !startOpt.getClusterId().equals(getClusterID())) {
          LOG.warn(
              "Clusterid mismatch - current clusterid: {}, Ignoring " + "given clusterid: {}",
              getClusterID(),
  ...
  /**
   * Report that an IOE has occurred on some file which may or may not be within one of the NN image
   * storage directories.
   */
  @Override
   ...
   *
   * <p>clusterID is a persistent attribute of the cluster. It is generated when the cluster is
   * created and remains the same during the life cycle of the cluster. When a new name node is
   * formated, if this is a new cluster, a new clusterID is geneated and stored. Subsequent name
   * node must be given the same ClusterID during its format to be in the same cluster. When a
   * datanode register it receive the clusterID and stick with it. If at any point, name node or
   * data node tries to join another cluster, it will be rejected.
   *
  ...
  /**
   * Try to find current cluster id in the VERSION files. returns first cluster id found in any
   * VERSION file null in case none found
   *
  ...
  public String determineClusterId() {
    ...
    while (sdit.hasNext()) {
      ...
      try {
        ...
        LOG.info(
            "current cluster id for sd={};lv={};" + "cid={}",
            sd.getCurrentDir(),
            layoutVersion,
            cid);
  ...
  private void setBlockPoolID(File storage, String bpid) throws InconsistentFSStateException {
    if (bpid == null || bpid.equals("")) {
      throw new InconsistentFSStateException(
          storage, "file " + Storage.STORAGE_FILE_VERSION + " has no block pool Id.");
    }

    if (!blockpoolID.equals("") && !blockpoolID.equals(bpid)) {
      throw new InconsistentFSStateException(
          storage, "Unexepcted blockpoolID " + bpid + " . Expected " + blockpoolID);
    }
    setBlockPoolID(bpid);
  }
  ...
  /**
   * Iterate over all of the storage dirs, reading their contents to determine their layout
   * versions. Returns an FSImageStorageInspector which has inspected each directory.
   *
   * <p><b>Note:</b> this can mutate the storage info fields (ctime, version, etc).
   *
  ...
  FSImageStorageInspector readAndInspectDirs(
      EnumSet<NameNodeFile> fileTypes, StartupOption startupOption) throws IOException {
   ...
   *
   * @throws IOException When all the storage directory fails to write VERSION file
   */
  @Override
