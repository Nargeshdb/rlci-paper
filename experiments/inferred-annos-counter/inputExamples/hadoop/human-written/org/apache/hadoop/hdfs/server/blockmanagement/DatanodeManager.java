...
public class DatanodeManager {
   ...
   *
   * <p>Done by storing a set of {@link DatanodeDescriptor} objects, sorted by storage id. In order
   * to keep the storage map consistent it tracks all storages ever registered with the namenode. A
   * descriptor corresponding to a specific storage id can be
   *
   * <ul>
   *   <li>added to the map if it is a new storage id;
   *   <li>updated with a new datanode started as a replacement for the old one with the same
   *       storage id; and
   *   <li>removed if and only if an existing datanode is restarted to serve a different storage id.
   * </ul>
   *
   * <br>
   *
  ...
  /**
   * Whether or not to avoid using stale DataNodes for writing. Note that, even if this is
   * configured, the policy may be temporarily disabled when a high percentage of the nodes are
   * marked as stale.
   */
  private final boolean avoidStaleDataNodesForWrite;

  /**
   * When the ratio of stale datanodes reaches this number, stop avoiding writing to stale
   * datanodes, i.e., continue using stale nodes for writing.
   */
  ...
  /**
   * Whether or not this cluster has ever consisted of more than 1 rack, according to the
   * NetworkTopology.
   */
  private boolean hasClusterEverBeenMultiRack = false;

  private final boolean checkIpHostnameInRegistration;
  /** Whether we should tell datanodes what to cache in replies to heartbeat messages. */
  private boolean shouldSendCachingCommands = false;

  /**
   * The number of datanodes for each software version. This list should change during rolling
   * upgrades. Software version -> Number of datanodes with this version
   */
  ...
  @Nullable private final SlowPeerTracker slowPeerTracker;
  @Nullable private final SlowDiskTracker slowDiskTracker;

  /**
   * The minimum time between resending caching directives to Datanodes, in milliseconds.
   *
   * <p>Note that when a rescan happens, we will send the new directives as soon as possible. This
   * timeout only applies to resending directives that we've already sent.
   */
  private final long timeBetweenResendingCachingDirectivesMs;

  DatanodeManager(
      final BlockManager blockManager, final Namesystem namesystem, final Configuration conf)
      throws IOException {
    this.namesystem = namesystem;
    this.blockManager = blockManager;

    this.useDfsNetworkTopology =
        conf.getBoolean(
    ...
    this.datanodeAdminManager =
        new DatanodeAdminManager(namesystem, blockManager, heartbeatManager);
    this.fsClusterStats = newFSClusterStats();
    this.dataNodePeerStatsEnabled =
        conf.getBoolean(
            DFSConfigKeys.DFS_DATANODE_PEER_STATS_ENABLED_KEY,
            DFSConfigKeys.DFS_DATANODE_PEER_STATS_ENABLED_DEFAULT);
    this.dataNodeDiskStatsEnabled =
        Util.isDiskStatsEnabled(
            conf.getInt(
    ...
    this.rejectUnresolvedTopologyDN =
        conf.getBoolean(
    ...
    final int configuredBlockInvalidateLimit =
        conf.getInt(
    ...
    this.blockInvalidateLimit =
        Math.max(countedBlockInvalidateLimit, configuredBlockInvalidateLimit);
    LOG.info(
        DFSConfigKeys.DFS_BLOCK_INVALIDATE_LIMIT_KEY
            + ": configured="
            + configuredBlockInvalidateLimit
            + ", counted="
            + countedBlockInvalidateLimit
            + ", effected="
            + blockInvalidateLimit);

    this.checkIpHostnameInRegistration =
        conf.getBoolean(
    ...
    Preconditions.checkArgument(
        (ratioUseStaleDataNodesForWrite > 0 && ratioUseStaleDataNodesForWrite <= 1.0f),
        DFSConfigKeys.DFS_NAMENODE_USE_STALE_DATANODE_FOR_WRITE_RATIO_KEY
            + " = '"
            + ratioUseStaleDataNodesForWrite
            + "' is invalid. "
            + "It should be a positive non-zero float value, not greater than 1.0f.");
    this.timeBetweenResendingCachingDirectivesMs =
        conf.getLong(
    ...
    this.blocksPerPostponedMisreplicatedBlocksRescan =
        conf.getLong(
  ...
  private static long getStaleIntervalFromConf(Configuration conf, long heartbeatExpireInterval) {
    ...
    if (staleInterval < minStaleInterval) {
      LOG.warn(
          "The given interval for marking stale datanode = "
              + staleInterval
              + ", which is less than "
              + DFSConfigKeys.DFS_NAMENODE_STALE_DATANODE_MINIMUM_INTERVAL_DEFAULT
   ...
   *
   * <p>For striped blocks, it will only move decommissioned/stale nodes to the bottom. For example,
   * assume we have storage list: d0, d1, d2, d3, d4, d5, d6, d7, d8, d9 mapping to block indices:
   * 0, 1, 2, 3, 4, 5, 6, 7, 8, 2
   *
   * <p>Here the internal block b2 is duplicated, locating in d2 and d9. If d2 is a decommissioning
   * node then should switch d2 and d9 in the storage list. After sorting locations, will update
   * corresponding block indices and block tokens.
   */
  ...
  /**
   * Move decommissioned/stale datanodes to the bottom. After sorting it will update block indices
   * and block tokens respectively.
   *
   * @param lb located striped block
   * @param comparator dn comparator
   */
  private void sortLocatedStripedBlock(final LocatedBlock lb, Comparator<DatanodeInfo> comparator) {
  ...
  /**
   * Move decommissioned/stale datanodes to the bottom. Also, sort nodes by network distance.
   *
   * @param lb located block
   * @param targetHost target host
   * @param comparator dn comparator
   */
  private void sortLocatedBlock(
      final LocatedBlock lb, String targetHost, Comparator<DatanodeInfo> comparator) {
    ...
    if (client == null) {
      ...
      if (resolvedHosts != null && !resolvedHosts.isEmpty()) {
        ...
        LOG.error(
            "Node Resolution failed. Please make sure that rack "
    ...
    if (nonDatanodeReader) {
      networktopology.sortByDistanceUsingNetworkLocation(
          client, lb.getLocations(), activeLen, createSecondaryNodeSorter());
    } else {
  ...
  private Consumer<List<DatanodeInfo>> createSecondaryNodeSorter() {
    Consumer<List<DatanodeInfo>> secondarySort = list -> Collections.shuffle(list);
    if (readConsiderLoad) {
      Comparator<DatanodeInfo> comp = Comparator.comparingInt(DatanodeInfo::getXceiverCount);
  ...
  /**
   * Given datanode address or host name, returns the DatanodeDescriptor for the same, or if it
   * doesn't find the datanode, it looks for a machine local and then rack local datanode, if a rack
   * local datanode is not possible either, it returns the DatanodeDescriptor of any random node in
   * the cluster.
   *
    ...
    if (node == null) {
      ...
      if (rackNodes != null) {
        ...
        if (node == null && !rackNodes.isEmpty()) {
          node =
              (DatanodeDescriptor)
                  (rackNodes.get(ThreadLocalRandom.current().nextInt(rackNodes.size())));
        }
      }
  ...
  public DatanodeStorageInfo[] getDatanodeStorageInfos(
      DatanodeID[] datanodeID, String[] storageIDs, String format, Object... args)
      throws UnregisteredNodeException {
  ...
  void removeDeadDatanode(final DatanodeID nodeID, boolean removeBlocksFromBlockMap) {
    ...
    if (d != null && isDatanodeDead(d)) {
      NameNode.stateChangeLog.info(
          "BLOCK* removeDeadDatanode: lost heartbeat from "
              + d
  ...
  /**
   * Will return true for all Datanodes which have a non-null software version and are considered
   * alive (by {@link DatanodeDescriptor#isAlive()}), indicating the node has not yet been removed.
   * Use {@code isAlive} rather than {@link DatanodeManager#isDatanodeDead(DatanodeDescriptor)} to
   * ensure that the version is decremented even if the datanode hasn't issued a heartbeat recently.
   *
  ...
  /**
   * Resolve a node's network location. If the DNS to switch mapping fails then this method
   * guarantees default rack location.
   *
  ...
  private String resolveNetworkLocationWithFallBackToDefaultLocation(DatanodeID node) {
    ...
    try {
      ...
      LOG.error(
          "Unresolved topology mapping. Using "
  ...
  /**
   * Resolve a node's network location. If the DNS to switch mapping fails, then this method throws
   * UnresolvedTopologyException.
   *
   * @param node to resolve to network location
   * @return network location path.
   * @throws UnresolvedTopologyException if the DNS to switch mapping fails to resolve network
   *     location.
   */
  ...
  /**
   * Resolve a node's dependencies in the network. If the DNS to switch mapping fails then this
   * method returns empty list of dependencies
   *
  ...
  private List<String> getNetworkDependenciesWithDefault(DatanodeInfo node) {
    ...
    try {
      ...
      LOG.error(
          "Unresolved dependency mapping for host "
              + node.getHostName()
              + ". Continuing with an empty dependency list");
  ...
  /**
   * Resolves a node's dependencies in the network. If the DNS to switch mapping fails to get
   * dependencies, then this method throws UnresolvedTopologyException.
   *
   * @param node to get dependencies for
   * @return List of dependent host names
  ...
  private List<String> getNetworkDependencies(DatanodeInfo node)
    ...
    if (dnsToSwitchMapping instanceof DNSToSwitchMappingWithDependency) {
      // Get dependencies
      dependencies =
          ((DNSToSwitchMappingWithDependency) dnsToSwitchMapping).getDependency(node.getHostName());
      if (dependencies == null) {
        LOG.error("The dependency call returned null for host " + node.getHostName());
        throw new UnresolvedTopologyException(
            "The dependency call returned " + "null for host " + node.getHostName());
      }
    }
  ...
  /**
   * Remove decommissioned datanode from the the list of live or dead nodes. This is used to not to
   * display a decommissioned datanode to the operators.
   *
   * @param nodeList , array list of live or dead nodes.
   */
  private static void removeDecomNodeFromList(final List<DatanodeDescriptor> nodeList) {
  ...
  void startAdminOperationIfNecessary(DatanodeDescriptor nodeReg) {
    long maintenanceExpireTimeInMS = hostConfigManager.getMaintenanceExpirationTimeInMS(nodeReg);
  ...
  /**
   * Register the given datanode with the namenode. NB: the given registration is mutated and given
   * back to the datanode.
   *
   * @param nodeReg the datanode registration
   * @throws DisallowedDatanodeException if the registration request is denied because the datanode
   *     does not match includes/excludes
   * @throws UnresolvedTopologyException if the registration request is denied because resolving
   *     datanode network location fails.
   */
  public void registerDatanode(DatanodeRegistration nodeReg)
    ...
    if (dnAddress != null) {
      ...
      if (checkIpHostnameInRegistration && !isNameResolved(dnAddress)) {
        ...
        final String message =
            "hostname cannot be resolved (ip=" + ip + ", hostname=" + hostname + ")";
    ...
    try {
      ...
      NameNode.stateChangeLog.info(
          "BLOCK* registerDatanode: from " + nodeReg + " storage " + nodeReg.getDatanodeUuid());

      DatanodeDescriptor nodeS = getDatanode(nodeReg.getDatanodeUuid());
      DatanodeDescriptor nodeN =
          host2DatanodeMap.getDatanodeByXferAddr(nodeReg.getIpAddr(), nodeReg.getXferPort());
  ...
  /**
   * Rereads conf to get hosts and exclude list file names. Rereads the files to update the hosts
   * and exclude lists. It checks if any of the hosts have changed states:
   */
  ...
  /**
   * Reload datanode membership and the desired admin operations from host files. If a node isn't
   * allowed, hostConfigManager.isIncluded returns false and the node can't be used. If a node is
   * allowed and the desired admin operation is defined, it will transition to the desired admin
   * state. If a node is allowed and upgrade domain is defined, the upgrade domain will be set on
   * the node. To use maintenance mode or upgrade domain, set
   * DFS_NAMENODE_HOSTS_PROVIDER_CLASSNAME_KEY to CombinedHostFileManager.class.
   */
  ...
  /**
   * Whether stale datanodes should be avoided as targets on the write path. The result of this
   * function may change if the number of stale datanodes eclipses a configurable threshold.
   *
  ...
  /**
   * Set the number of current stale DataNodes. The HeartbeatManager got this number based on
  ...
  /**
   * @return Return the current number of stale DataNodes (detected by HeartbeatManager).
   */
  ...
  public void fetchDatanodes(
      final List<DatanodeDescriptor> live,
  ...
  /**
   * Check if the cluster now consists of multiple racks. If it does, and this is the first time
   * it's consisted of multiple racks, then process blocks that may now be misreplicated.
   *
  ...
  void checkIfClusterIsNowMultiRack(DatanodeDescriptor node) {
    if (!hasClusterEverBeenMultiRack && networktopology.getNumOfRacks() > 1) {
      String message =
          "DN "
              + node
              + " joining cluster has expanded a formerly "
  ...
  public List<DatanodeDescriptor> getDatanodeListForReport(final DatanodeReportType type) {
    ...
    final boolean listDecommissioningNodes =
        type == DatanodeReportType.ALL || type == DatanodeReportType.DECOMMISSIONING;
    final boolean listEnteringMaintenanceNodes =
        type == DatanodeReportType.ALL || type == DatanodeReportType.ENTERING_MAINTENANCE;
    final boolean listInMaintenanceNodes =
        type == DatanodeReportType.ALL || type == DatanodeReportType.IN_MAINTENANCE;
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          "getDatanodeListForReport with "
              + "includedNodes = "
              + hostConfigManager.getIncludes()
              + ", excludedNodes = "
              + hostConfigManager.getExcludes()
              + ", foundNodes = "
              + foundNodes
              + ", nodes = "
              + nodes);
  ...
  /**
   * Checks if name resolution was successful for the given address. If IP address and host name are
   * the same, then it means name resolution has failed. As a special case, local addresses are also
   * considered acceptable. This is particularly important on Windows, where 127.0.0.1 does not
  ...
  private BlockRecoveryCommand getBlockRecoveryCommand(
      String blockPoolId, DatanodeDescriptor nodeinfo) throws IOException {
    ...
    for (BlockInfo b : blocks) {
      ...
      boolean copyOnTruncateRecovery =
          truncateRecovery && uc.getTruncateBlock().getBlockId() != b.getBlockId();
      ExtendedBlock primaryBlock =
          (copyOnTruncateRecovery)
      ...
      if (recoveryLocations.size() > 1) {
        if (recoveryLocations.size() != storages.length) {
          LOG.info(
              "Skipped stale nodes for recovery : " + (storages.length - recoveryLocations.size()));
        }
      ...
      if (truncateRecovery) {
        ...
        if (b.isStriped()) {
          rBlock =
              new RecoveringStripedBlock(
                  rBlock, uc.getBlockIndices(), ((BlockInfoStriped) b).getErasureCodingPolicy());
        }
      }
  ...
  private void addCacheCommands(
      String blockPoolId, DatanodeDescriptor nodeinfo, List<DatanodeCommand> cmds) {
    ...
    if (shouldSendCachingCommands
      ...
      DatanodeCommand pendingCacheCommand =
          getCacheCommand(nodeinfo.getPendingCached(), DatanodeProtocol.DNA_CACHE, blockPoolId);
      if (pendingCacheCommand != null) {
        cmds.add(pendingCacheCommand);
        sendingCachingCommands = true;
      }
      DatanodeCommand pendingUncacheCommand =
          getCacheCommand(nodeinfo.getPendingUncached(), DatanodeProtocol.DNA_UNCACHE, blockPoolId);
  ...
  public DatanodeCommand[] handleHeartbeat(
    ...
    if (totalBlocks > 0) {
      int numReplicationTasks =
          (int) Math.ceil((double) (totalReplicateBlocks * maxTransfers) / totalBlocks);
      int numECTasks = (int) Math.ceil((double) (totalECBlocks * maxTransfers) / totalBlocks);
      if (LOG.isDebugEnabled()) {
        LOG.debug(
      ...
      List<BlockTargetPair> pendingList = nodeinfo.getReplicationCommand(numReplicationTasks);
      ...
      List<BlockECReconstructionInfo> pendingECList = nodeinfo.getErasureCodeCommand(numECTasks);
      if (pendingECList != null && !pendingECList.isEmpty()) {
        cmds.add(
            new BlockECReconstructionCommand(DNA_ERASURE_CODING_RECONSTRUCTION, pendingECList));
      }
    }
   ...
   *
   * @param list The {@link CachedBlocksList}. This function clears the list.
   * @param action The action to perform in the command.
   * @param poolId The block pool id.
   * @return A DatanodeCommand to be sent back to the DN, or null if there is nothing to be done.
   */
   ...
   *
   * <p>A system administrator can tune the balancer bandwidth parameter
   * (dfs.datanode.balance.bandwidthPerSec) dynamically by calling "dfsadmin -setBalanacerBandwidth
   * newbandwidth", at which point the following 'bandwidth' variable gets updated with the new
   * value for each node. Once the heartbeat command is issued to update the value on the specified
   * datanode, this value will be set back to 0.
   *
  ...
  /**
   * Clear any actions that are queued up to be sent to the DNs on their next heartbeats. This
   * includes block invalidations, recoveries, and replication requests.
   */
  ...
  public void setHeartbeatInterval(long intervalSeconds) {
    setHeartbeatInterval(intervalSeconds, this.heartbeatRecheckInterval);
  }

  public void setHeartbeatRecheckInterval(int recheckInterval) {
    setHeartbeatInterval(this.heartbeatIntervalSeconds, recheckInterval);
  }
  ...
  private void setHeartbeatInterval(long intervalSeconds, int recheckInterval) {
    ...
    this.blockInvalidateLimit = Math.max(20 * (int) (intervalSeconds), blockInvalidateLimit);
  }

  /**
   * Retrieve information about slow peers as a JSON. Returns null if we are not tracking slow
   * peers.
   *
   * @return
   */
  ...
  /**
   * Retrieve information about slow disks as a JSON. Returns null if we are not tracking slow
   * disks.
   *
   * @return
   */
  public String getSlowDisksReport() {
    return slowDiskTracker != null ? slowDiskTracker.getSlowDiskReportAsJsonString() : null;
  }
