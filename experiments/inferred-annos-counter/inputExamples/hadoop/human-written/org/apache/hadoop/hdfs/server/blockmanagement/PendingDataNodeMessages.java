...
/**
 * In the Standby Node, we can receive messages about blocks before they are actually available in
 * the namespace, or while they have an outdated state in the namespace. In those cases, we queue
 * those block-related messages in this structure.
 */
class PendingDataNodeMessages {
  ...
  static class ReportedBlockInfo {
    ...
    public String toString() {
      return "ReportedBlockInfo [block="
          + block
          + ", dn="
  ...
  void enqueueReportedBlock(
      DatanodeStorageInfo storageInfo, Block block, ReplicaState reportedState) {
    if (BlockIdManager.isStripedBlockID(block.getBlockId())) {
      Block blkId = new Block(BlockIdManager.convertToStripedID(block.getBlockId()));
      getBlockQueue(blkId).add(new ReportedBlockInfo(storageInfo, new Block(block), reportedState));
    } else {
  ...
  /**
   * @return any messages that were previously queued for the given block, or null if no messages
   *     were queued.
   */
