...
/**
 * Feature used to store and process the snapshot diff information for a directory. In particular,
 * it contains a directory diff list recording changes made to the directory and its children for
 * each snapshot.
 */
@InterfaceAudience.Private
public class DirectoryWithSnapshotFeature implements INode.Feature {
  /**
   * The difference between the current state and a previous snapshot of the children list of an
   * INodeDirectory.
   */
  ...
  public static class DirectoryDiff
    ...
    DirectoryDiff(
        int snapshotId,
        INodeDirectoryAttributes snapshotINode,
        DirectoryDiff posteriorDiff,
        int childrenSize,
    ...
    /**
     * @return The children list of a directory in a snapshot. Since the snapshot is read-only, the
     *     logical view of the list is never changed although the internal data structure may
     *     mutate.
     */
      ...
      return new ReadOnlyList<INode>() {
        ...
        private List<INode> initChildren() {
          if (children == null) {
            ...
            final int diffIndex = directoryDiffList.getDiffIndexById(getSnapshotId());
            List<DirectoryDiff> diffList =
                directoryDiffList.getDiffListBetweenSnapshots(
                    diffIndex, directoryDiffList.asList().size(), currentDir);
            for (DirectoryDiff d : diffList) {
              combined.combinePosterior(d.diff, null);
            }
            children =
                combined.apply2Current(
                    ReadOnlyList.Util.asList(
  ...
  public static class DirectoryDiffList
    ...
    DiffList<DirectoryDiff> newDiffs() {
      return DirectoryDiffListFactory.createDiffList(INodeDirectory.DEFAULT_FILES_PER_DIRECTORY);
    }

    /** Replace the given child in the created/deleted list, if there is any. */
    public boolean replaceCreatedChild(final INode oldChild, final INode newChild) {
     ...
     *
     * @return the id of the snapshot. {@link Snapshot#NO_SNAPSHOT_ID} if the given inode is not in
     *     any of the snapshot.
     */
    ...
    /**
     * Returns the list of diffs between two indexes corresponding to two snapshots.
     *
     ...
     */
    List<DirectoryDiff> getDiffListBetweenSnapshots(
  ...
  public static void destroyDstSubtree(
      INode.ReclaimContext reclaimContext, INode inode, final int snapshot, final int prior) {
  ...
  /**
   * Clean an inode while we move it from the deleted list of post to the deleted list of prior.
   *
   ...
   */
  private static void cleanDeletedINode(
      INode.ReclaimContext reclaimContext, INode inode, final int post, final int prior) {
      ...
      if (topNode instanceof INodeReference.WithName) {
        ...
        for (INode child : dir.getChildrenList(prior)) {
          if (priorChildrenDiff != null
              && priorChildrenDiff.getDeleted(child.getLocalNameBytes()) != null) {
            continue;
          }
          queue.addLast(child);
        }
   ...
   * These directories are equivalent to the directories stored in the deletes lists.
   */
  ...
  /**
   * Add an inode into parent's children list. The caller of this method needs to make sure that
   * parent is in the given snapshot "latest".
   */
  public boolean addChild(
      INodeDirectory parent, INode inode, boolean setModTime, int latestSnapshotId) {
  ...
  /**
   * Remove an inode from parent's children list. The caller of this method needs to make sure that
   * parent is in the given snapshot "latest".
   */
  ...
  /**
   * @return If there is no corresponding directory diff for the given snapshot, this means that the
   *     current children list should be returned for the snapshot. Otherwise we calculate the
   *     children list for the snapshot and return it.
   */
  public ReadOnlyList<INode> getChildrenList(INodeDirectory currentINode, final int snapshotId) {
    ...
    return diff != null
        ? diff.getChildrenList(currentINode)
  ...
  public INode getChild(INodeDirectory currentINode, byte[] name, int snapshotId) {
    final DirectoryDiff diff = diffs.getDiffById(snapshotId);
    return diff != null
        ? diff.getChild(name, true, currentINode)
  ...
  public QuotaCounts computeQuotaUsage4CurrentDirectory(
    ...
    for (DirectoryDiff d : diffs) {
      for (INode deleted : d.getChildrenDiff().getDeletedUnmodifiable()) {
        ...
        counts.add(
            deleted.computeQuotaUsage(bsps, childPolicyId, false, Snapshot.CURRENT_STATE_ID));
      }
    }
    return counts;
  }
   ...
   *
   * @param fromSnapshot Start point of the diff computation. Null indicates current tree.
   * @param toSnapshot End point of the diff computation. Null indicates current tree.
   * @param diff Used to capture the changes happening to the children. Note that the diff still
   *     represents (later_snapshot - earlier_snapshot) although toSnapshot can be before
   *     fromSnapshot.
   ...
   */
  boolean computeDiffBetweenSnapshots(
      Snapshot fromSnapshot, Snapshot toSnapshot, ChildrenDiff diff, INodeDirectory currentINode) {
    ...
    List<DirectoryDiff> difflist =
        diffs.getDiffListBetweenSnapshots(earlierDiffIndex, laterDiffIndex, currentINode);
        ...
        if (priorDiff != null && priorDiff.getSnapshotId() == prior) {
          ...
          if (priorCreated != null) {
            if (currentINode.isLastReference()
              ...
              // destroyed.
              priorDiff.getChildrenDiff().destroyCreatedList(reclaimContext, currentINode);
            } else {
