...
/**
 * ReplicaUnderConstruction contains information about replicas (or blocks belonging to a block
 * group) while they are under construction.
 *
 * <p>The GS, the length and the state of the replica is as reported by the datanode.
 *
 * <p>It is not guaranteed, but expected, that datanodes actually have corresponding replicas.
 */
class ReplicaUnderConstruction extends Block {
  ...
  ReplicaUnderConstruction(
      Block block, DatanodeStorageInfo target, HdfsServerConstants.ReplicaState state) {
    super(block);
  ...
  /**
   * Expected block replica location as assigned when the block was allocated. This defines the
   * pipeline order. It is not guaranteed, but expected, that the data-node actually has the
   * replica.
   */
