...
public final class NameNodeUtils {
  ...
  /**
   * Return the namenode address that will be used by clients to access this namenode or name
   * service. This needs to be called before the config is overriden.
   *
   * <p>This method behaves as follows:
   *
   * <p>1. fs.defaultFS is undefined: - return null. 2. fs.defaultFS is defined but has no hostname
   * (logical or physical): - return null. 3. Single NN (no HA, no federation): - return URI
   * authority from fs.defaultFS 4. Current NN is in an HA nameservice (with or without federation):
   * - return nameservice for current NN. 5. Current NN is in non-HA namespace, federated cluster: -
   * return value of dfs.namenode.rpc-address.[nsId].[nnId] - If the above key is not defined, then
   * return authority from fs.defaultFS if the port number is > 0. 6. If port number in the
   * authority is missing or zero in step 6: - return null
   */
  @VisibleForTesting
  @Nullable
  static String getClientNamenodeAddress(Configuration conf, @Nullable String nsId) {
    final Collection<String> nameservices = DFSUtilClient.getNameServiceIds(conf);
    ...
    if (nsId != null && nameservices.contains(nsId)) {
      final Collection<String> namenodes =
          conf.getTrimmedStringCollection(DFS_HA_NAMENODES_KEY_PREFIX + "." + nsId);
      if (namenodes.size() > 1) {
        return nsId;
      }
    }
