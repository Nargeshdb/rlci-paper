...
/**
 * This class is to scan the paths recursively. If file is directory, then it will scan for files
 * recursively. If the file is non directory, then it will just submit the same file to process.
 * This will use file string path representation.
 */
...
public class ExternalSPSFilePathCollector implements FileCollector {
  ...
  public ExternalSPSFilePathCollector(SPSService service) {
    ...
    try {
      ...
      LOG.error(
          "Unable to get the filesystem. Make sure Namenode running and "
              + "configured namenode address is correct.",
          e);
    }
  }
  ...
  /** Recursively scan the given path and add the file info to SPS service for processing. */
  private long processPath(Long startID, String childPath) {
    ...
    for (byte[] lastReturnedName = HdfsFileStatus.EMPTY_NAME; ; ) {
      ...
      try {
        ...
        LOG.warn(
            "Failed to list directory " + childPath + ". Ignore the directory and continue.", e);
  ...
  public int remainingCapacity() {
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          "SPS processing Q -> maximum capacity:{}, current size:{}," + " remaining size:{}",
          maxQueueLimitToScan,
          size,
          remainingSize);
    }
  ...
  public void scanAndCollectFiles(long pathId) throws IOException {
    ...
    if (pendingSatisfyItemsCount <= 0) {
      LOG.debug("There is no pending items to satisfy the given path " + "inodeId:{}", pathId);
