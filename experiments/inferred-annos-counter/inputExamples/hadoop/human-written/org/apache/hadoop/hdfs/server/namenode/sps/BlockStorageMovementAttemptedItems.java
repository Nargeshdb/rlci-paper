...
/**
 * A monitor class for checking whether block storage movements attempt completed or not. If this
 * receives block storage movement attempt status(either success or failure) from DN then it will
 * just remove the entries from tracking. If there is no DN reports about movement attempt finished
 * for a longer time period, then such items will retries automatically after timeout. The default
 * timeout would be 5 minutes.
 */
public class BlockStorageMovementAttemptedItems {
  ...
  /**
   * A map holds the items which are already taken for blocks movements processing and sent to DNs.
   */
  ...
  /**
   * Add item to block storage movement attempted items map which holds the tracking/blockCollection
   * id versus time stamp.
   *
   * @param startPathId - start satisfier path identifier
   * @param fileId - file identifier
   * @param monotonicNow - time now
   * @param assignedBlocks - assigned blocks for block movement
  ...
  public void add(
      ...
      int retryCount) {
    AttemptedItemInfo itemInfo =
        new AttemptedItemInfo(
            startPathId, fileId, monotonicNow, assignedBlocks.keySet(), retryCount);
   ...
   *
   * @param reportedDn reported datanode
   * @param type storage type
   * @param reportedBlock reported block
   */
  ...
  private void matchesReportedBlock(
      DatanodeInfo reportedDn, StorageType type, Block reportedBlock) {
    ...
    for (StorageTypeNodePair dn : blkLocs) {
      boolean foundDn = dn.getDatanodeInfo().compareTo(reportedDn) == 0 ? true : false;
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          "Reported block:{} not found in attempted blocks. Datanode:{}" + ", StorageType:{}",
          reportedBlock,
          reportedDn,
          type);
    }
  ...
  /**
   * Sets running flag to false. Also, this will interrupt monitor thread and clear all the queued
   * up tasks.
   */
  ...
  /**
   * A monitor class for checking block storage movement attempt status and long waiting items
   * periodically.
   */
    ...
    public void run() {
      while (monitorRunning) {
        try {
          ...
          LOG.info("BlocksStorageMovementAttemptMonitor thread " + "is interrupted.", ie);
        } catch (IOException ie) {
          LOG.warn(
              "BlocksStorageMovementAttemptMonitor thread " + "received exception and exiting.",
              ie);
        }
      }
    }
  ...
  void blocksStorageMovementUnReportedItemsCheck() {
    synchronized (storageMovementAttemptedItems) {
      Iterator<AttemptedItemInfo> iter = storageMovementAttemptedItems.iterator();
      long now = monotonicNow();
      while (iter.hasNext()) {
        AttemptedItemInfo itemInfo = iter.next();
        if (now > itemInfo.getLastAttemptedOrReportedTime() + selfRetryTimeout) {
          long file = itemInfo.getFile();
          ItemInfo candidate =
              new ItemInfo(itemInfo.getStartPath(), file, itemInfo.getRetryCount() + 1);
          blockStorageMovementNeeded.add(candidate);
          iter.remove();
          LOG.info(
              "TrackID: {} becomes timed out and moved to needed "
  ...
  void blockStorageMovementReportedItemsCheck() throws IOException {
    ...
    for (Block blk : finishedBlks) {
      synchronized (storageMovementAttemptedItems) {
        Iterator<AttemptedItemInfo> iterator = storageMovementAttemptedItems.iterator();
        while (iterator.hasNext()) {
          ...
          if (attemptedItemInfo.getBlocks().isEmpty()) {
            blockStorageMovementNeeded.add(
                new ItemInfo(
                    attemptedItemInfo.getStartPath(),
                    attemptedItemInfo.getFile(),
