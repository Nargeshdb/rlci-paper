...
public final class CachedBlock implements Element, LightWeightGSet.LinkedElement {
  ...
  /** Bit 15: Mark Bit 0-14: cache replication factor. */
  private short replicationAndMark;

  /**
   * Used to implement the CachedBlocksList.
   *
   * <p>Since this CachedBlock can be in multiple CachedBlocksList objects, we need to be able to
   * store multiple 'prev' and 'next' pointers. The triplets array does this.
   *
   * <p>Each triplet contains a CachedBlockList object followed by a prev pointer, followed by a
   * next pointer.
   */
  ...
  /**
   * Get a list of the datanodes which this block is cached, planned to be cached, or planned to be
   * uncached on.
   *
   * @param type If null, this parameter is ignored. If it is non-null, we match only datanodes
   *     which have it on this list. See {@link DatanodeDescriptor.CachedBlocksList.Type} for a
   *     description of all the lists.
   * @return The list of datanodes. Modifying this list does not alter the state of the CachedBlock.
   */
  ...
  public void insertInternal(
      IntrusiveCollection<? extends Element> list, Element prev, Element next) {
    for (int i = 0; i < triplets.length; i += 3) {
      if (triplets[i] == list) {
        throw new RuntimeException(
            "Trying to re-insert an element that " + "is already in the list.");
      }
    }
