...
final class FSDirTruncateOp {
  ...
  /** Private constructor for preventing FSDirTruncateOp object creation. Static-only class. */
   ...
   * @throws IOException
   */
  static TruncateResult truncate(
      ...
      final long mtime,
      final BlocksMapUpdateInfo toRemoveBlocks,
    ...
    try {
      ...
      if (last != null && last.getBlockUCState() == BlockUCState.UNDER_RECOVERY) {
        final BlockInfo truncatedBlock = last.getUnderConstructionFeature().getTruncateBlock();
        if (truncatedBlock != null) {
          final long truncateLength =
              file.computeFileSize(false, false) + truncatedBlock.getNumBytes();
      ...
      fsn.recoverLeaseInternal(
          RecoverLeaseOp.TRUNCATE_FILE, iip, src, clientName, clientMachine, false);
      ...
      if (!onBlockBoundary) {
        ...
        truncateBlock =
            prepareFileForTruncate(fsn, iip, clientName, clientMachine, lastBlockDelta, null);
      }
  ...
  /**
   * Unprotected truncate implementation. Unlike {@link FSDirTruncateOp#truncate}, this will not
   * schedule block recovery.
   *
   * @param fsn namespace
   * @param iip path name
   * @param clientName client name
   ...
   * @throws IOException
   */
  static void unprotectedTruncate(
      final FSNamesystem fsn,
      final INodesInPath iip,
      final String clientName,
      final String clientMachine,
      ...
      throws UnresolvedLinkException, QuotaExceededException, SnapshotAccessControlException,
          IOException {
    assert fsn.hasWriteLock();
    ...
    boolean onBlockBoundary =
        unprotectedTruncate(fsn, iip, newLength, collectedBlocks, mtime, null);

    if (!onBlockBoundary) {
      BlockInfo oldBlock = file.getLastBlock();
      Block tBlk =
          prepareFileForTruncate(
              fsn,
              iip,
              clientName,
              clientMachine,
              file.computeFileSize() - newLength,
              truncateBlock);
  ...
  /**
   * Convert current INode to UnderConstruction. Recreate lease. Create new block for the truncated
   * copy. Schedule truncation of the replicas.
   *
   * @param fsn namespace
   ...
   * @return the returned block will be written to editLog and passed back into this method upon
   *     loading.
   * @throws IOException
   */
  @VisibleForTesting
  static Block prepareFileForTruncate(
      FSNamesystem fsn,
      INodesInPath iip,
      String leaseHolder,
    ...
    fsn.getLeaseManager()
        .addLease(file.getFileUnderConstructionFeature().getClientName(), file.getId());
    ...
    if (shouldCopyOnTruncate) {
      ...
      uc.setTruncateBlock(new BlockInfoContiguous(oldBlock, oldBlock.getReplication()));
      ...
      blockManager.getBlockIdManager().setGenerationStampIfGreater(newGenerationStamp);
      truncatedBlockUC = oldBlock;

      NameNode.stateChangeLog.debug(
          "BLOCK* prepareFileForTruncate: "
              + "{} Scheduling in-place block truncate to new size {}",
          uc,
          uc.getTruncateBlock().getNumBytes());
    }
    if (shouldRecoverNow) {
      truncatedBlockUC
          .getUnderConstructionFeature()
          .initializeBlockRecovery(truncatedBlockUC, newBlock.getGenerationStamp(), true);
    }
  ...
  /**
   * Truncate has the following properties: 1.) Any block deletions occur now. 2.) INode length is
   * truncated now - new clients can only read up to the truncated length. 3.) INode will be set to
   * UC and lastBlock set to UNDER_RECOVERY. 4.) NN will trigger DN truncation recovery and waits
   * for DNs to report. 5.) File is considered UNDER_RECOVERY until truncation recovery completes.
   ...
   */
  private static boolean unprotectedTruncate(
      FSNamesystem fsn,
      INodesInPath iip,
      long newLength,
    ...
    long remainingLength = file.collectBlocksBeyondMax(newLength, collectedBlocks, toRetain);
    file.setModificationTime(mtime);
    // return whether on a block boundary
    return (remainingLength - newLength) == 0;
  }

  private static void verifyQuotaForTruncate(
      FSNamesystem fsn, INodesInPath iip, INodeFile file, long newLength, QuotaCounts delta)
    ...
    final BlockStoragePolicy policy =
        fsd.getBlockStoragePolicySuite().getPolicy(file.getStoragePolicyID());
  ...
  /** Defines if a replica needs to be copied on truncate or can be truncated in place. */
  ...
  static class TruncateResult {
    ...
    /**
     * @return true if client does not need to wait for block recovery, false if client needs to
