...
public abstract class HAState {
  ...
  /**
   * Method to be overridden by subclasses to prepare to enter a state. This method is called
   * <em>without</em> the context being locked, and after {@link #prepareToExitState(HAContext)} has
   * been called for the previous state, but before {@link #exitState(HAContext)} has been called
  ...
  /**
   * Method to be overridden by subclasses to prepare to exit a state. This method is called
   * <em>without</em> the context being locked. This is used by the standby state to cancel any
   * checkpoints that are going on. It can also be used to check any preconditions for the state
   * transition.
   *
   * <p>This method should not make any destructive changes to the state (eg stopping threads) since
   * {@link #prepareToEnterState(HAContext)} may subsequently cancel the state transition.
   *
   * @param context HA context
   * @throws ServiceFailedException on precondition failure
   */
  ...
  public void setState(HAContext context, HAState s) throws ServiceFailedException {
    ...
    throw new ServiceFailedException(
        "Transition from state " + this + " to " + s + " is not allowed.");
  }
   ...
   * @throws StandbyException if a given type of operation is not supported in standby state
   */
