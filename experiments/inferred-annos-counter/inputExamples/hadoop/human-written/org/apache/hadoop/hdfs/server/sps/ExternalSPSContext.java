...
/**
 * This class used to connect to Namenode and gets the required information to SPS from Namenode
 * state.
 */
...
public class ExternalSPSContext implements Context {
  ...
  public boolean isInSafeMode() {
    try {
      return nnc != null ? nnc.getDistributedFileSystem().isInSafeMode() : false;
    } catch (IOException e) {
  ...
  public boolean isFileExist(long path) {
    ...
    try {
      ...
      LOG.warn("Exception while getting file is for the given path:{}", filePath, e);
    }
    return false;
  }
  ...
  public void removeSPSHint(long inodeId) throws IOException {
    ...
    try {
      nnc.getDistributedFileSystem()
          .removeXAttr(filePath, HdfsServerConstants.XATTR_SATISFY_STORAGE_POLICY);
    } catch (IOException e) {
      List<String> listXAttrs = nnc.getDistributedFileSystem().listXAttrs(filePath);
      if (!listXAttrs.contains(HdfsServerConstants.XATTR_SATISFY_STORAGE_POLICY)) {
        LOG.info(
            "SPS hint already removed for the inodeId:{}." + " Ignoring exception:{}",
            inodeId,
            e.getMessage());
      }
    }
  ...
  public int getNumLiveDataNodes() {
    try {
      return nnc.getDistributedFileSystem().getDataNodeStats(DatanodeReportType.LIVE).length;
    } catch (IOException e) {
  ...
  public HdfsFileStatus getFileInfo(long path) throws IOException {
    ...
    try {
      Path filePath = DFSUtilClient.makePathFromFileId(path);
      fileInfo =
          nnc.getDistributedFileSystem().getClient().getLocatedFileInfo(filePath.toString(), false);
