...
public abstract class BlockListAsLongs implements Iterable<BlockReportReplica> {
   ...
   */
  public static BlockListAsLongs decodeBuffer(
   ...
   */
  @VisibleForTesting
  public static BlockListAsLongs decodeBuffers(
      final int numBlocks, final List<ByteString> blocksBufs) {
    return decodeBuffers(numBlocks, blocksBufs, IPC_MAXIMUM_DATA_LENGTH_DEFAULT);
  }
   ...
   */
  public static BlockListAsLongs decodeBuffers(
      final int numBlocks, final List<ByteString> blocksBufs, final int maxDataLength) {
    // this doesn't actually copy the data
    return decodeBuffer(numBlocks, ByteString.copyFrom(blocksBufs), maxDataLength);
  }

  /**
   * Prepare an instance to in-place decode the given list of Longs. Note it's much more efficient
   * to decode ByteString buffers and only exists for compatibility.
   *
  ...
  /**
   * Very efficient encoding of the block report into a ByteString to avoid the overhead of protobuf
   * repeating fields. Primitive repeating fields require re-allocs of an ArrayList&lt;Long&gt; and
   * the associated (un)boxing overhead which puts pressure on GC.
   *
   * <p>The structure of the buffer is as follows: - each replica is represented by 4 longs:
  ...
  /**
   * Convert block report to old-style list of longs. Only used to re-encode the block report when
   * the DN detects an older NN. This is inefficient, but in practice a DN is unlikely to be
   * upgraded first
   *
   * <p>The structure of the array is as follows: 0: the length of the finalized replica list; 1:
   * the length of the under-construction replica list; - followed by finalized replica list where
   * each replica is represented by 3 longs: one for the blockId, one for the block length, and one
   * for the generation stamp; - followed by the invalid replica represented with three -1s; -
   * followed by the under-construction replica list where each replica is represented by 4 longs:
  ...
  /**
   * Returns a singleton iterator over blocks in the block report. Do not add the returned blocks to
   * a collection.
   *
   * @return Iterator
   */
