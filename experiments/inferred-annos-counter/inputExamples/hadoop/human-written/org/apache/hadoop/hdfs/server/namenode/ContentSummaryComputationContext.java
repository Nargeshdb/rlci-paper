...
public class ContentSummaryComputationContext {
  ...
  public static final Logger LOG = LoggerFactory.getLogger(ContentSummaryComputationContext.class);
   ...
   * @param limitPerRun allowed number of operations in one locking period. 0 or a negative number
   *     means no limit (i.e. no yielding)
   */
  public ContentSummaryComputationContext(
      FSDirectory dir, FSNamesystem fsn, long limitPerRun, long sleepMicroSec) {
  ...
  /**
   * Relinquish locks held during computation for a short while and reacquire them. This will give
   * other threads a chance to acquire the contended locks and run.
   *
  ...
  public BlockStoragePolicySuite getBlockStoragePolicySuite() {
    Preconditions.checkState(
        (bsps != null || fsn != null),
        "BlockStoragePolicySuite must be either initialized or available via" + " FSNameSystem");
  ...
  void checkPermission(INodeDirectory inode, int snapshotId, FsAction access)
    ...
    if (dir != null && dir.isPermissionEnabled() && pc != null && !pc.isSuperUser()) {
