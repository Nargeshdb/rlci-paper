...
public class DatanodeProtocolServerSideTranslatorPB implements DatanodeProtocolPB {
  ...
  private static final ErrorReportResponseProto VOID_ERROR_REPORT_RESPONSE_PROTO =
  ...
  private static final ReportBadBlocksResponseProto VOID_REPORT_BAD_BLOCK_RESPONSE =
  ...
  public RegisterDatanodeResponseProto registerDatanode(
      RpcController controller, RegisterDatanodeRequestProto request) throws ServiceException {
    ...
    return RegisterDatanodeResponseProto.newBuilder()
        .setRegistration(PBHelper.convert(registrationResp))
        .build();
  }

  @Override
  public HeartbeatResponseProto sendHeartbeat(
      RpcController controller, HeartbeatRequestProto request) throws ServiceException {
    ...
    try {
      ...
      VolumeFailureSummary volumeFailureSummary =
          request.hasVolumeFailureSummary()
              ? PBHelper.convertVolumeFailureSummary(request.getVolumeFailureSummary())
              : null;
      response =
          impl.sendHeartbeat(
    ...
    if (rollingUpdateStatus != null) {
      ...
      RollingUpgradeStatusProto rus =
          PBHelperClient.convertRollingUpgradeStatus(rollingUpdateStatus);
  ...
  @Override
  public BlockReportResponseProto blockReport(
      RpcController controller, BlockReportRequestProto request) throws ServiceException {
    ...
    for (StorageBlockReportProto s : request.getReportsList()) {
      ...
      if (s.hasNumberOfBlocks()) { // new style buffer based reports
        ...
        Preconditions.checkState(
            s.getBlocksCount() == 0, "cannot send both blocks list and buffers");
    ...
    try {
      cmd =
          impl.blockReport(
              ...
              report,
              request.hasContext() ? PBHelper.convert(request.getContext()) : null);
    } catch (IOException e) {
      throw new ServiceException(e);
    }
  ...
  @Override
  public CacheReportResponseProto cacheReport(
      RpcController controller, CacheReportRequestProto request) throws ServiceException {
    DatanodeCommand cmd = null;
    try {
      cmd =
          impl.cacheReport(
  ...
  public BlockReceivedAndDeletedResponseProto blockReceivedAndDeleted(
    ...
    for (int i = 0; i < sBlocks.size(); i++) {
      ...
      if (sBlock.hasStorage()) {
        info[i] =
            new StorageReceivedDeletedBlocks(PBHelperClient.convert(sBlock.getStorage()), rdBlocks);
      } else {
        info[i] =
    ...
    try {
      impl.blockReceivedAndDeleted(
          PBHelper.convert(request.getRegistration()), request.getBlockPoolId(), info);
    } catch (IOException e) {
      throw new ServiceException(e);
    }
  ...
  public ErrorReportResponseProto errorReport(
      RpcController controller, ErrorReportRequestProto request) throws ServiceException {
    try {
      impl.errorReport(
          PBHelper.convert(request.getRegistartion()), request.getErrorCode(), request.getMsg());
    } catch (IOException e) {
      throw new ServiceException(e);
    }
  ...
  public ReportBadBlocksResponseProto reportBadBlocks(
      RpcController controller, ReportBadBlocksRequestProto request) throws ServiceException {
  ...
  public CommitBlockSynchronizationResponseProto commitBlockSynchronization(
    ...
    try {
      impl.commitBlockSynchronization(
          PBHelperClient.convert(request.getBlock()),
          request.getNewGenStamp(),
          request.getNewLength(),
          request.getCloseFile(),
          request.getDeleteBlock(),
          dns,
          storageIDs);
    } catch (IOException e) {
      throw new ServiceException(e);
