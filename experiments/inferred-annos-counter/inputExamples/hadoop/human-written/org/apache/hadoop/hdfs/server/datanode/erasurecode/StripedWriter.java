...
class StripedWriter {
  ...
  private static final int WRITE_PACKET_SIZE = 64 * 1024;
  ...
  StripedWriter(
    ...
    Preconditions.checkArgument(
        targetIndices.length <= parityBlkNum, "Too much missed striped blocks.");
    initTargetIndices();
  ...
  void init() throws IOException {
    ...
    maxChunksPerPacket =
        Math.max((WRITE_PACKET_SIZE - PacketHeader.PKT_MAX_HEADER_LEN) / chunkSize, 1);
  ...
  private StripedBlockWriter createWriter(short index) throws IOException {
    return new StripedBlockWriter(
        this,
        datanode,
        conf,
        reconstructor.getBlock(targetIndices[index]),
        targets[index],
        targetStorageTypes[index],
        targetStorageIds[index]);
  }
