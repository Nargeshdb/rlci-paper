...
class FSDirMkdirOp {
  ...
  /**
   * For a given absolute path, create all ancestors as directories along the path. All ancestors
   * inherit their parent's permission plus an implicit u+wx permission. This is used by create()
   * and addSymlink() for implicitly creating all directories along the path.
   *
   * <p>For example, path="/foo/bar/spam", "/foo" is an existing directory, "/foo/bar" is not
   * existing yet, the function will create directory bar.
   *
   * @return a INodesInPath with all the existing and newly created ancestor directories created. Or
   *     return null if there are errors.
   */
  static INodesInPath createAncestorDirectories(
      FSDirectory fsd, INodesInPath iip, PermissionStatus permission) throws IOException {
   ...
   * @param perm the permission of the directory. Note that all ancestors created along the path has
   *     implicit {@code u+wx} permissions.
   * @param inheritPerms if the ancestor directories should inherit permissions or use the specified
   *     permissions.
   * @return {@link INodesInPath} which contains all inodes to the target directory, After the
   *     execution parentPath points to the path of the returned INodesInPath. The function return
   *     null if the operation has failed.
   */
  private static INodesInPath createParentDirectories(
      FSDirectory fsd, INodesInPath iip, PermissionStatus perm, boolean inheritPerms)
      throws IOException {
    ...
    if (missing == 0) { // full path exists, return parents.
      ...
      // Ensure that the user can traversal the path by adding implicit
      // u+wx permission to all ancestor directories.
      PermissionStatus basePerm =
          inheritPerms ? existing.getLastINode().getPermissionStatus() : perm;
  ...
  static void mkdirForEditLog(
      ...
      throws QuotaExceededException, UnresolvedLinkException, AclException,
          FileAlreadyExistsException, ParentNotDirectoryException, AccessControlException {
    assert fsd.hasWriteLock();
  ...
  private static INodesInPath createSingleDirectory(
      FSDirectory fsd, INodesInPath existing, byte[] localName, PermissionStatus perm)
      throws IOException {
    assert fsd.hasWriteLock();
    existing =
        unprotectedMkdir(fsd, fsd.allocateNewInodeId(), existing, localName, perm, null, now());
    if (existing == null) {
      return null;
    }
  ...
  private static PermissionStatus addImplicitUwx(
      PermissionStatus parentPerm, PermissionStatus perm) {
    FsPermission p = parentPerm.getPermission();
    FsPermission ancestorPerm;
    if (p.getUnmasked() == null) {
      ancestorPerm =
          new FsPermission(
              p.getUserAction().or(FsAction.WRITE_EXECUTE), p.getGroupAction(), p.getOtherAction());
    } else {
      ancestorPerm =
