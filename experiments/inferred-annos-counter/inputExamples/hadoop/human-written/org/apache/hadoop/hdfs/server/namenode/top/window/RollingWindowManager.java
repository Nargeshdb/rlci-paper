...
/**
 * A class to manage the set of {@link RollingWindow}s. This class is the interface of metrics
 * system to the {@link RollingWindow}s to retrieve the current top metrics.
 *
 * <p>Thread-safety is provided by each {@link RollingWindow} being thread-safe as well as {@link
 * ConcurrentHashMap} for the collection of them.
 */
...
public class RollingWindowManager {
  ...
  private static class RollingWindowMap extends ConcurrentHashMap<String, RollingWindow> {
    private static final long serialVersionUID = -6785807073237052051L;
  }

  /**
   * Represents a snapshot of the rolling window. It contains one Op per operation in the window,
   * with ranked users for each Op.
   */
  ...
  /**
   * Represents an operation within a TopWindow. It contains a ranked set of the top users for the
   * operation.
   */
  ...
  /**
   * Represents a user who called an Op within a TopWindow. Specifies the user and the number of
   * times the user called the operation.
   */
  ...
  /**
   * A mapping from each reported metric to its {@link RollingWindowMap} that maintains the set of
   * {@link RollingWindow}s for the users that have operated on that metric.
   */
  ...
  public RollingWindowManager(Configuration conf, int reportingPeriodMs) {
    ...
    Preconditions.checkArgument(
        bucketsPerWindow <= windowLenMs, "the minimum size of a bucket is 1 ms");
    // same-size buckets
    Preconditions.checkArgument(
        windowLenMs % bucketsPerWindow == 0,
        "window size must be a multiplication of number of buckets");
    topUsersCnt =
        conf.getInt(DFSConfigKeys.NNTOP_NUM_USERS_KEY, DFSConfigKeys.NNTOP_NUM_USERS_DEFAULT);
    Preconditions.checkArgument(
        topUsersCnt > 0, "the number of requested top users must be at least 1");
  }
   ...
   * @return a TopWindow describing the top users for each metric in the window.
   */
