...
public interface FsVolumeSpi extends Checkable<FsVolumeSpi.VolumeCheckContext, VolumeCheckResult> {
   ...
   *
   * <p>It is caller's responsibility to close {@link FsVolumeReference} to decrease the reference
   * count on the volume.
   */
  ...
  /**
   * Reserve disk space for a block (RBW or Re-replicating) so a writer does not run out of space
  ...
  /**
   * Release reserved memory for an RBW block written to transient storage i.e. RAM. bytesToRelease
   * will be rounded down to the OS page size since locked memory reservation must always be a
   * multiple of the page size.
   */
  void releaseLockedMemory(long bytesToRelease);

  /**
   * BlockIterator will return ExtendedBlock entries from a block pool in this volume. The entries
   * will be returned in sorted order.
   *
   * <p>BlockIterator objects themselves do not always have internal synchronization, so they can
   * only safely be used by a single thread at a time.
   *
   * <p>Closing the iterator does not save it. You must call save to save it.
   */
  interface BlockIterator extends Closeable {
    /**
     * Get the next block.
     *
     * <p>Note that this block may be removed in between the time we list it, and the time the
     * caller tries to use it, or it may represent a stale entry. Callers should handle the case
     * where the returned block no longer exists.
     *
     * @return The next block, or null if there are no more blocks. Null if there was an error
     *     determining the next block.
     * @throws IOException If there was an error getting the next block in this volume. In this
     *     case, EOF will be set on the iterator.
     */
    ...
    /**
     * Save this block iterator to the underlying volume. Any existing saved block iterator with
     * this name will be overwritten. maxStalenessMs will not be saved.
     *
     * @throws IOException If there was an error when saving the block iterator.
     */
     ...
     *
     * <p>A maximum staleness of 0 means we will never return stale entries; a larger value will
     * allow us to reduce resource consumption in exchange for returning more potentially stale
     * entries. Even with staleness set to 0, consumers of this API must handle race conditions
     * where block disappear before they can be processed.
     */
    void setMaxStalenessMs(long maxStalenessMs);

    /**
     * Get the wall-clock time, measured in milliseconds since the Epoch, when this iterator was
     * created.
     */
    long getIterStartMs();
    ...
    /**
     * Get the wall-clock time, measured in milliseconds since the Epoch, when this iterator was
     * last saved. Returns iterStartMs if the iterator was never saved.
     */
    long getLastSavedMs();
  ...
  /**
   * Tracks the files and other information related to a block on the disk Missing file is indicated
   * by setting the corresponding member to null.
   *
   * <p>Because millions of these structures may be created, we try to save memory here. So instead
   * of storing full paths, we store path suffixes. The block file, if it exists, will have a path
   * like this: {@literal <volume_base_path>/<block_path>} So we don't need to store the volume
   * path, since we already know what the volume is.
   *
   * <p>The metadata file, if it exists, will have a path like this: {@literal
   * <volume_base_path>/<block_path>_<genstamp>.meta} So if we have a block file, there isn't any
   * need to store the block path again.
   *
   * <p>The accessor functions take care of these manipulations.
   */
  public static class ScanInfo implements Comparable<ScanInfo> {
    private final long blockId;

    /**
     * The block file path, relative to the volume's base directory. If there was no block file
     * found, this may be null. If 'vol' is null, then this is the full path of the block file.
     */
    private final String blockSuffix;

    /**
     * The suffix of the meta file path relative to the block file. If blockSuffix is null, then
     * this will be the entire path relative to the volume base directory, or an absolute path if
     * vol is also null.
     */
     ...
     *
     * <p>For example, the condensed version of /foo//bar is /foo/bar Unlike {@link
     * File#getCanonicalPath()}, this will never perform I/O on the filesystem.
     *
    ...
    /**
     * Create a ScanInfo object for a block. This constructor will examine the block data and
     * meta-data files.
     *
    ...
    public File getBlockFile() {
      return (blockSuffix == null)
          ? null
          : new File(new File(volume.getBaseURI()).getAbsolutePath(), blockSuffix);
    }

    /**
     * Return the length of the data block. The length returned is the length cached when this
     * object was created.
     *
    ...
    public File getMetaFile() {
      ...
      return new File(new File(volume.getBaseURI()).getAbsolutePath(), fileSuffix);
    }
    ...
    public long getGenStamp() {
      return metaSuffix != null
          ? Block.getGenerationStamp(getMetaFile().getName())
  ...
  /**
   * Load last partial chunk checksum from checksum file. Need to be called with FsDataset lock
   * acquired.
   *
   * @param blockFile
   * @param metaFile
  ...
  /**
   * Compile a list of {@link ScanInfo} for the blocks in the block pool with id {@code bpid}.
   *
   ...
   * @throws IOException
   */
  void compileReport(String bpid, Collection<ScanInfo> report, ReportCompiler reportCompiler)
