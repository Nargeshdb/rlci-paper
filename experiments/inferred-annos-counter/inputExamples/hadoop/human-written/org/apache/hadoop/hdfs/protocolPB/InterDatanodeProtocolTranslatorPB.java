 ...
 * InterDatanodeProtocol} interfaces to the RPC server implementing {@link InterDatanodeProtocolPB}.
 */
...
public class InterDatanodeProtocolTranslatorPB
  ...
  public InterDatanodeProtocolTranslatorPB(
  ...
  public ReplicaRecoveryInfo initReplicaRecovery(RecoveringBlock rBlock) throws IOException {
    InitReplicaRecoveryRequestProto req =
        InitReplicaRecoveryRequestProto.newBuilder().setBlock(PBHelper.convert(rBlock)).build();
    ...
    if (!resp.getReplicaFound()) {
      ...
      if (!resp.hasBlock() || !resp.hasState()) {
        throw new IOException(
            "Replica was found but missing fields. " + "Req: " + req + "\n" + "Resp: " + resp);
      }
    }

    BlockProto b = resp.getBlock();
    return new ReplicaRecoveryInfo(
        b.getBlockId(), b.getNumBytes(), b.getGenStamp(), PBHelper.convert(resp.getState()));
  }
