...
/**
 * A utility class to help detect resources (nodes/ disks) whose aggregate latency is an outlier
 * within a given set.
 *
 * <p>We use the median absolute deviation for outlier detection as described in the following
 * publication:
 *
 * <p>Leys, C., et al., Detecting outliers: Do not use standard deviation around the mean, use
 * absolute deviation around the median. http://dx.doi.org/10.1016/j.jesp.2013.03.013
 *
 * <p>We augment the above scheme with the following heuristics to be even more conservative:
 *
 * <p>1. Skip outlier detection if the sample size is too small. 2. Never flag resources whose
 * aggregate latency is below a low threshold. 3. Never flag resources whose aggregate latency is
 * less than a small multiple of the median.
 */
...
public class OutlierDetector {
  ...
  /**
   * Deviation multiplier. A sample is considered to be an outlier if it exceeds the median by
   * (multiplier * median abs. deviation). 3 is a conservative choice.
   */
  private static final int DEVIATION_MULTIPLIER = 3;

  /**
   * If most of the samples are clustered together, the MAD can be low. The median multiplier
   * introduces another safeguard to avoid overaggressive outlier detection.
   */
  @VisibleForTesting static final int MEDIAN_MULTIPLIER = 3;
  ...
  /**
   * Return a set of nodes/ disks whose latency is much higher than their counterparts. The input is
   * a map of (resource {@literal ->} aggregate latency) entries.
   *
   * <p>The aggregate may be an arithmetic mean or a percentile e.g. 90th percentile. Percentiles
   * are a better choice than median since latency is usually not a normal distribution.
   *
   * <p>This method allocates temporary memory O(n) and has run time O(n.log(n)), where n =
   * stats.size().
   *
   * @return
   */
  public Map<String, Double> getOutliers(Map<String, Double> stats) {
    if (stats.size() < minNumResources) {
      LOG.debug(
          "Skipping statistical outlier detection as we don't have "
    ...
    Double upperLimitLatency = Math.max(lowThresholdMs, median * MEDIAN_MULTIPLIER);
    upperLimitLatency = Math.max(upperLimitLatency, median + (DEVIATION_MULTIPLIER * mad));

    final Map<String, Double> slowResources = new HashMap<>();

    LOG.trace(
        "getOutliers: List={}, MedianLatency={}, "
  ...
  public static Double computeMad(List<Double> sortedValues) {
    if (sortedValues.size() == 0) {
      throw new IllegalArgumentException(
          "Cannot compute the Median Absolute Deviation " + "of an empty list.");
    }
