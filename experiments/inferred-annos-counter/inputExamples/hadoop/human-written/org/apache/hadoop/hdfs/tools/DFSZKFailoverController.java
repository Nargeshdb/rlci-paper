...
public class DFSZKFailoverController extends ZKFailoverController {
  ...
  protected HAServiceTarget dataToTarget(byte[] data) {
    ...
    NNHAServiceTarget ret =
        new NNHAServiceTarget(conf, proto.getNameserviceId(), proto.getNamenodeId());
    InetSocketAddress addressFromProtobuf =
        new InetSocketAddress(proto.getHostname(), proto.getPort());

    if (!addressFromProtobuf.equals(ret.getAddress())) {
      throw new RuntimeException(
          "Mismatched address stored in ZK for "
              + ret
              + ": Stored protobuf was "
              + proto
              + ", address from our own "
              + "configuration for this NameNode was "
              + ret.getAddress());
    }
  ...
  @SuppressWarnings(
      "builder:required.method.not.called") // FP @EnsuresCalledMethodsIf limitation: can’t handle
                                            // with EnsuresCalledMethodsIf because the boolean is
                                            // passed as a parameter (validated)
  private void getLocalNNThreadDump() {
    ...
    try {
      String stacksUrl =
          DFSUtil.getInfoServer(localNNTarget.getAddress(), conf, DFSUtil.getHttpClientScheme(conf))
              + "/stacks";
