...
/**
 * This is the JMX management interface for namenode information. End users shouldn't be
 * implementing these interfaces, and instead access this information through the JMX APIs.
 */
...
public interface NameNodeMXBean {
   ...
   *
   * @return Rolling upgrade information if an upgrade is in progress. Else (e.g. if there is no
   *     upgrade or the upgrade is finalized), returns null.
   */
  RollingUpgradeInfo.Bean getRollingUpgradeStatus();

  /**
   * Gets total used space by data nodes for non DFS purposes such as storing temporary files on the
   * local file system.
   *
   ...
   *
   * @return the total space used by the block pool as percentage of total capacity.
   */
   ...
   *
   * @return the total number of missing blocks on the cluster with replication factor 1.
   */
  long getNumberOfMissingBlocksWithReplicationFactorOne();

  /**
   * Gets the total number of replicated low redundancy blocks on the cluster with the highest risk
   * of loss.
   *
   * @return the total number of low redundancy blocks on the cluster with the highest risk of loss.
   */
  long getHighestPriorityLowRedundancyReplicatedBlocks();

  /**
   * Gets the total number of erasure coded low redundancy blocks on the cluster with the highest
   * risk of loss.
   *
   * @return the total number of low redundancy blocks on the cluster with the highest risk of loss.
   */
  ...
  /**
   * Get information about the transaction ID, including the last applied transaction ID and the
