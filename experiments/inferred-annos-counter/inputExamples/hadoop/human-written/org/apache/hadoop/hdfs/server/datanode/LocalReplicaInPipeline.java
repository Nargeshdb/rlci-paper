...
/**
 * This class defines a replica in a pipeline, which includes a persistent replica being written to
 * by a dfs client or a temporary replica being replicated by a source datanode or being copied for
 * the balancing purpose.
 *
 * <p>The base class implements a temporary replica
 */
public class LocalReplicaInPipeline extends LocalReplica implements ReplicaInPipeline {
  ...
  /**
   * Bytes reserved for this replica on the containing volume. Based off difference between the
   * estimated maximum block length and the bytes already written to this block.
   */
   ...
   * @param bytesToReserve disk space to reserve for this replica, based on the estimated maximum
   *     block length.
   */
  public LocalReplicaInPipeline(
      ...
      while (bytesOnDisk < minLength) {
        if (nanos <= 0L) {
          throw new IOException(
              String.format("Need %d bytes, but only %d bytes available", minLength, bytesOnDisk));
        }
  ...
  @Override // ReplicaInPipeline
  @SuppressWarnings(
      "builder:required.method.not.called") // TP: the result of new RandomAccessFile(blockFile,
                                            // "rw") remains open in possible exceptional exit due
                                            // to new RandomAccessFile(blockFile, "rw").getFD()
                                            // (validated)
  public ReplicaOutputStreams createStreams(boolean isCreate, DataChecksum requestedChecksum)
      throws IOException {
    ...
    try {
      blockOut =
          fileIoProvider.getFileOutputStream(
  ...
  public OutputStream createRestartMetaStream() throws IOException {
    ...
    File restartMeta =
        new File(
            blockFile.getParent() + File.pathSeparator + "." + blockFile.getName() + ".restart");
  ...
  public void moveReplicaFrom(ReplicaInfo oldReplicaInfo, File newBlkFile) throws IOException {
    ...
    if (!(oldReplicaInfo instanceof LocalReplica)) {
      throw new IOException(
          "The source replica with blk id "
    ...
    try {
      ...
      try {
        ...
        LOG.warn(
            "Cannot move meta file " + newmeta + "back to the finalized directory " + oldmeta, ex);
      }
      throw new IOException(
          "Block "
              + oldReplicaInfo
