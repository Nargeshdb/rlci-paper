 ...
 * NamenodeProtocol} interfaces to the RPC server implementing {@link NamenodeProtocolPB}.
 */
...
public class NamenodeProtocolTranslatorPB
    ...
    try {
      return rpcProxy
          .getMostRecentCheckpointTxId(
              NULL_CONTROLLER, GetMostRecentCheckpointTxIdRequestProto.getDefaultInstance())
          .getTxId();
    } catch (ServiceException e) {
      throw ProtobufHelper.getRemoteException(e);
    }
  ...
  public CheckpointSignature rollEditLog() throws IOException {
    try {
      return PBHelper.convert(
          rpcProxy.rollEditLog(NULL_CONTROLLER, VOID_ROLL_EDITLOG_REQUEST).getSignature());
    } catch (ServiceException e) {
      throw ProtobufHelper.getRemoteException(e);
    }
  }
  ...
  public NamespaceInfo versionRequest() throws IOException {
    try {
      return PBHelper.convert(
          rpcProxy.versionRequest(NULL_CONTROLLER, VOID_VERSION_REQUEST).getInfo());
    } catch (ServiceException e) {
      throw ProtobufHelper.getRemoteException(e);
    }
  }
  ...
  public NamenodeRegistration registerSubordinateNamenode(NamenodeRegistration registration)
    ...
    RegisterRequestProto req =
        RegisterRequestProto.newBuilder().setRegistration(PBHelper.convert(registration)).build();
    try {
      return PBHelper.convert(
  ...
  public void endCheckpoint(NamenodeRegistration registration, CheckpointSignature sig)
    ...
    EndCheckpointRequestProto req =
        EndCheckpointRequestProto.newBuilder()
            .setRegistration(PBHelper.convert(registration))
            .setSignature(PBHelper.convert(sig))
            .build();
    try {
  ...
  public RemoteEditLogManifest getEditLogManifest(long sinceTxId) throws IOException {
    GetEditLogManifestRequestProto req =
        GetEditLogManifestRequestProto.newBuilder().setSinceTxId(sinceTxId).build();
    try {
  ...
  public boolean isRollingUpgrade() throws IOException {
    IsRollingUpgradeRequestProto req = IsRollingUpgradeRequestProto.newBuilder().build();
    try {
  ...
  public Long getNextSPSPath() throws IOException {
    GetNextSPSPathRequestProto req = GetNextSPSPathRequestProto.newBuilder().build();
    try {
      GetNextSPSPathResponseProto nextSPSPath = rpcProxy.getNextSPSPath(NULL_CONTROLLER, req);
