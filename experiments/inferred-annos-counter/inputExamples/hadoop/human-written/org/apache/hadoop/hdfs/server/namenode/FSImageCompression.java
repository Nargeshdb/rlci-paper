...
public class FSImageCompression {
  ...
  /**
   * Create a compression instance based on the user's configuration in the given Configuration
   * object.
   *
  ...
  /** Create a compression instance using the codec specified by <code>codecClassName</code> */
  static FSImageCompression createCompression(Configuration conf, String codecClassName)
      throws IOException {
   ...
   *
   * @throws IOException if the specified codec is not available or the underlying IO fails.
   */
  static FSImageCompression readCompressionHeader(Configuration conf, DataInput in)
      throws IOException {
  ...
  /**
   * Unwrap a compressed input stream by wrapping it with a decompressor based on this codec. If
   * this instance represents no compression, simply adds buffering to the input stream.
   *
   * @return a buffered stream that provides uncompressed data
   * @throws IOException If the decompressor cannot be instantiated or an IO error occurs.
   */
   ...
   * the same stream wrapped with that codec. If no codec is specified, simply adds buffering to the
   * stream, so that the returned stream is always buffered.
   *
   * @param os The stream to write header to and wrap. This stream should be unbuffered.
   * @return A stream wrapped with the specified compressor, or buffering if compression is not
   *     enabled.
   * @throws IOException if an IO error occurs or the compressor cannot be instantiated
   */
