...
public class AvailableSpaceBlockPlacementPolicy extends BlockPlacementPolicyDefault {
  private static final Logger LOG =
      LoggerFactory.getLogger(AvailableSpaceBlockPlacementPolicy.class);
  ...
  public void initialize(
    ...
    LOG.info(
        "Available space block placement policy initialized: "
            + DFSConfigKeys
                .DFS_NAMENODE_AVAILABLE_SPACE_BLOCK_PLACEMENT_POLICY_BALANCED_SPACE_PREFERENCE_FRACTION_KEY
            + " = "
  ...
  protected DatanodeDescriptor chooseDataNode(
      final String scope, final Collection<Node> excludedNode, StorageType type) {
    ...
    DatanodeDescriptor a =
        (DatanodeDescriptor) dfsClusterMap.chooseRandomWithStorageType(scope, excludedNode, type);
  ...
  protected DatanodeStorageInfo chooseLocalStorage(
    ...
    if (!optimizeLocal) {
      return super.chooseLocalStorage(
          localMachine,
          excludedNodes,
          blocksize,
          maxNodesPerRack,
    ...
    final EnumMap<StorageType, Integer> initialStorageTypesLocal = storageTypes.clone();
    final EnumMap<StorageType, Integer> initialStorageTypesLocalRack = storageTypes.clone();
    ...
    DatanodeStorageInfo localRack =
        chooseLocalRack(
            ...
            results,
            avoidStaleNodes,
            initialStorageTypesLocalRack);
    if (local != null && localRack != null) {
      if (select(local.getDatanodeDescriptor(), localRack.getDatanodeDescriptor(), true)
          == local.getDatanodeDescriptor()) {
  ...
  private void swapStorageTypes(
      EnumMap<StorageType, Integer> fromStorageTypes,
  ...
  @Override
  protected DatanodeDescriptor chooseDataNode(
      final String scope, final Collection<Node> excludedNode) {
  ...
  private DatanodeDescriptor select(
      DatanodeDescriptor a, DatanodeDescriptor b, boolean isBalanceLocal) {
    if (a != null && b != null) {
  ...
  protected int compareDataNode(
      final DatanodeDescriptor a, final DatanodeDescriptor b, boolean isBalanceLocal) {
    if (a.equals(b)
