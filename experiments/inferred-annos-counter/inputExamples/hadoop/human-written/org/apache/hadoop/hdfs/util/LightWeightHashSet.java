...
/**
 * A low memory linked hash set implementation, which uses an array for storing the elements and
 * linked lists for collision resolution. This class does not support null element.
 *
...
public class LightWeightHashSet<T> implements Collection<T> {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(LightWeightHashSet.class);

  /**
   * An internal array of entries, which are the rows of the hash table. The size must be a power of
   * two.
   */
  ...
  /**
   * Remove and return n elements from the hashtable. The order in which entries are removed is
   * unspecified, and and may not correspond to the order in which they were inserted.
   *
   ...
   *
   * @return final capacity, either MIN_CAPACITY, MAX_CAPACITY, or power of 2 closest to the
   *     requested capacity.
   */
    ...
    public T next() {
      if (modification != expectedModification) {
        throw new ConcurrentModificationException(
            "modification=" + modification + " != expectedModification = " + expectedModification);
      }
      if (next == null) {
