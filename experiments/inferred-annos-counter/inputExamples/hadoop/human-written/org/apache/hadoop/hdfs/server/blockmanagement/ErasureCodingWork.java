...
class ErasureCodingWork extends BlockReconstructionWork {
  ...
  public ErasureCodingWork(
      ...
      int priority,
      byte[] liveBlockIndicies,
      byte[] liveBusyBlockIndicies) {
    super(
        block,
        bc,
        srcNodes,
    ...
    LOG.debug("Creating an ErasureCodingWork to {} reconstruct ", block);
  }
  ...
  void chooseTargets(
    ...
    DatanodeStorageInfo[] chosenTargets =
        blockplacement.chooseTarget(
            getSrcPath(),
            getAdditionalReplRequired(),
            getSrcNodes()[0],
            getLiveReplicaStorages(),
            false,
            excludedNodes,
            getBlockSize(),
  ...
  /**
   * @return true if the current source nodes cover all the internal blocks. I.e., we only need to
   *     have more racks.
   */
  private boolean hasAllInternalBlocks() {
    final BlockInfoStriped block = (BlockInfoStriped) getBlock();
    if (liveBlockIndicies.length + liveBusyBlockIndicies.length < block.getRealTotalBlockNum()) {
      return false;
    }
  ...
  /**
   * We have all the internal blocks but not enough racks. Thus we do not need to do decoding but
   * only simply make an extra copy of an internal block. In this scenario, use this method to
   * choose the source datanode for simple replication.
   *
  ...
  void addTaskToDatanode(NumberReplicas numberReplicas) {
    ...
    if (hasNotEnoughRack()) {
      ...
      targets[0]
          ...
          .addBlockToBeErasureCoded(
              new ExtendedBlock(blockPoolId, stripedBlk),
              getSrcNodes(),
              targets,
              getLiveBlockIndicies(),
  ...
  private void createReplicationWork(int sourceIndex, DatanodeStorageInfo target) {
    ...
    final long internBlkLen =
        StripedBlockUtil.getInternalBlockLength(
            stripedBlk.getNumBytes(),
            stripedBlk.getCellSize(),
            stripedBlk.getDataBlockNum(),
            blockIndex);
    final Block targetBlk =
        new Block(
            stripedBlk.getBlockId() + blockIndex, internBlkLen, stripedBlk.getGenerationStamp());
    source.addBlockToBeReplicated(targetBlk, new DatanodeStorageInfo[] {target});
    LOG.debug(
        "Add replication task from source {} to " + "target {} for EC block {}",
        source,
        target,
        targetBlk);
  }
  ...
  private List<Integer> findLeavingServiceSources() {
    ...
    for (int i = 0; i < getSrcNodes().length; i++) {
      if ((getSrcNodes()[i].isDecommissionInProgress()
              || (getSrcNodes()[i].isEnteringMaintenance() && getSrcNodes()[i].isAlive()))
