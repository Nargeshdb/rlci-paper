 ...
 * that honors upgrade domain policy. Here is the replica placement strategy. If the writer is on a
 * datanode, the 1st replica is placed on the local machine, otherwise a random datanode. The 2nd
 * replica is placed on a datanode that is on a different rack. The 3rd replica is placed on a
 * datanode which is on a different node of the rack as the second replica. All 3 replicas have
 * unique upgrade domains.
 */
...
public class BlockPlacementPolicyWithUpgradeDomain extends BlockPlacementPolicyDefault {
  ...
  @Override
  protected boolean isGoodDatanode(
    ...
    boolean isGoodTarget =
        super.isGoodDatanode(node, maxTargetPerRack, considerLoad, results, avoidStaleNodes);
    if (isGoodTarget) {
  ...
  public BlockPlacementStatus verifyBlockPlacement(DatanodeInfo[] locs, int numberOfReplicas) {
    ...
    BlockPlacementStatusWithUpgradeDomain upgradeDomainStatus =
        new BlockPlacementStatusWithUpgradeDomain(
            defaultStatus, getUpgradeDomainsFromNodes(locs), numberOfReplicas, upgradeDomainFactor);
  ...
  boolean useDelHint(
    ...
    return isMovableBasedOnUpgradeDomain(combine(moreThanOne, exactlyOne), delHint, added);
  }
