...
public class FileDiffList extends AbstractINodeDiffList<INodeFile, INodeFileAttributes, FileDiff> {
  ...
  /**
   * Copy blocks from the removed snapshot into the previous snapshot up to the file length of the
   * latter. Collect unused blocks of the removed snapshot.
   */
  void combineAndCollectSnapshotBlocks(
    ...
    if (removedBlocks == null) {
      ...
      if (sf.isCurrentFileDeleted()) sf.collectBlocksAndClear(reclaimContext, file);
      return;
    }
    ...
    for (; i < removedBlocks.length; i++) {
      if (i < earlierBlocks.length && removedBlocks[i] == earlierBlocks[i]) continue;
      if (i < laterBlocks.length && removedBlocks[i] == laterBlocks[i]) continue;
      break;
    }
    ...
    if (lastBlock != null
        && lastBlock.getBlockUCState().equals(HdfsServerConstants.BlockUCState.UNDER_RECOVERY)) {
