...
/**
 * This class maintains the map from a block to its metadata. block's metadata currently includes
 * blockCollection it belongs to and the datanodes that store the block.
 */
class BlocksMap {
  ...
  BlocksMap(int capacity) {
    ...
    this.blocks =
        new LightWeightGSet<Block, BlockInfo>(capacity) {
          @Override
  ...
  /**
   * Searches for the block in the BlocksMap and returns {@link Iterable} of the storages the block
   * belongs to.
   */
  ...
  /**
   * For a block that has already been retrieved from the BlocksMap returns {@link Iterable} of the
   * storages the block belongs to.
   */
  ...
  /**
   * Remove block from the set of blocks belonging to the data-node. Remove data-node from the
   * block.
   */
