 ...
 *
 * <p>Since this is done while holding the namesystem lock, the amount of work per monitor tick is
 * limited.
 */
public class DatanodeAdminDefaultMonitor extends DatanodeAdminMonitorBase
  ...
  /** The number of nodes that have been checked on this tick. Used for statistics. */
  private int numNodesChecked = 0;
  /** The last datanode in outOfServiceNodeBlocks that we've processed. */
  private DatanodeDescriptor iterkey =
      new DatanodeDescriptor(new DatanodeID("", "", "", 0, 0, 0, 0));
  ...
  protected void processConf() {
    ...
    if (numBlocksPerCheck <= 0) {
      LOG.error(
          ...
          DFSConfigKeys.DFS_NAMENODE_DECOMMISSION_BLOCKS_PER_INTERVAL_KEY,
          DFSConfigKeys.DFS_NAMENODE_DECOMMISSION_BLOCKS_PER_INTERVAL_DEFAULT);
      numBlocksPerCheck = DFSConfigKeys.DFS_NAMENODE_DECOMMISSION_BLOCKS_PER_INTERVAL_DEFAULT;
    }
  ...
  public void run() {
    ...
    if (numBlocksChecked + numNodesChecked > 0) {
      LOG.info(
          "Checked {} blocks and {} nodes this tick. {} nodes are now "
  ...
  /**
   * Pop datanodes off the pending list and into decomNodeBlocks, subject to the
   * maxConcurrentTrackedNodes limit.
   */
  ...
  private void check() {
    final Iterator<Map.Entry<DatanodeDescriptor, AbstractList<BlockInfo>>> it =
        new CyclicIteration<>(outOfServiceNodeBlocks, iterkey).iterator();
    ...
    while (it.hasNext() && !exceededNumBlocksPerCheck() && namesystem.isRunning()) {
      ...
      try {
        ...
        // an invalid state.
        LOG.warn("DatanodeAdminMonitor caught exception when processing node " + "{}.", dn, e);
        pendingNodes.add(dn);
        toRemove.add(dn);
      } finally {
        iterkey = dn;
    ...
    for (DatanodeDescriptor dn : toRemove) {
      Preconditions.checkState(
          dn.isDecommissioned() || dn.isInService(),
  ...
  private void pruneReliableBlocks(
      final DatanodeDescriptor datanode, AbstractList<BlockInfo> blocks) {
    processBlocksInternal(datanode, blocks.iterator(), null, true);
  }

  /**
   * Returns a list of blocks on a datanode that are insufficiently replicated or require recovery,
   * i.e. requiring recovery and should prevent decommission or maintenance.
   *
   * <p>As part of this, it also schedules replication/recovery work.
   *
   * @return List of blocks requiring recovery
   */
  private AbstractList<BlockInfo> handleInsufficientlyStored(final DatanodeDescriptor datanode) {
    AbstractList<BlockInfo> insufficient = new ChunkedArrayList<>();
    processBlocksInternal(datanode, datanode.getBlockIterator(), insufficient, false);
    return insufficient;
  }

  /**
   * Used while checking if DECOMMISSION_INPROGRESS datanodes can be marked as DECOMMISSIONED or
   * ENTERING_MAINTENANCE datanodes can be marked as IN_MAINTENANCE. Combines shared logic of
   * pruneReliableBlocks and handleInsufficientlyStored.
   *
   * @param datanode Datanode
   * @param it Iterator over the blocks on the datanode
   * @param insufficientList Return parameter. If it's not null, will contain the insufficiently
   *     replicated-blocks from the list.
   * @param pruneReliableBlocks whether to remove blocks reliable enough from the iterator
   */
  private void processBlocksInternal(
    ...
    LightWeightHashSet<Long> lowRedundancyOpenFiles = new LightWeightLinkedSet<>();
    ...
    while (it.hasNext()) {
      if (insufficientList == null && numBlocksCheckedPerLock >= numBlocksPerCheck) {
      ...
      if (firstReplicationLog) {
        dnAdmin.logBlockReplicationInfo(
