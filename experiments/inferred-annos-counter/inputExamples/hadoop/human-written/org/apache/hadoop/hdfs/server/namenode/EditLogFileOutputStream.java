...
/**
 * An implementation of the abstract class {@link EditLogOutputStream}, which stores edits in a
 * local file.
 */
...
public class EditLogFileOutputStream extends EditLogOutputStream {
   ...
   */
  @SuppressWarnings({
    "builder:required.method.not.called", // TP: rp remains open in possible exceptional exit due to
                                          // rp.getFD() (validated)
    "builder:required.method.not.called" // TP: same reason (validated)
  })
   ...
   * <ul>
   *   <li>the opcode (byte)
   *   <li>the transaction id (long)
   *   <li>the actual Writables for the transaction
   * </ul>
   */
  @Override
  ...
  /**
   * Flush ready buffer to persistent store. currentBuffer is not flushed as it accumulates new log
   * records while readyBuffer will be flushed and synced.
   */
  @Override
  ...
  /**
   * For the purposes of unit tests, we don't need to actually write durably to disk. So, we can
   * skip the fsync() calls for a speed improvement.
   *
