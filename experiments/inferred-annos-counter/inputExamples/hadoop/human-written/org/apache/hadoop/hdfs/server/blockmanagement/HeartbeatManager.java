...
/**
 * Manage the heartbeats received from datanodes. The datanode list and statistics are synchronized
...
class HeartbeatManager implements DatanodeStatistics {
  ...
  /**
   * Stores a subset of the datanodeMap in DatanodeManager, containing nodes that are considered
   * alive. The HeartbeatMonitor periodically checks for out-dated entries, and removes them from
   * the list. It is synchronized by the heartbeat manager lock.
   */
  ...
  HeartbeatManager(
      final Namesystem namesystem, final BlockManager blockManager, final Configuration conf) {
    this.namesystem = namesystem;
    this.blockManager = blockManager;
    boolean avoidStaleDataNodesForWrite =
        conf.getBoolean(
    ...
    if (avoidStaleDataNodesForWrite && staleInterval < recheckInterval) {
      ...
      LOG.info(
          "Setting heartbeat recheck interval to "
              + staleInterval
              + " since "
              + DFSConfigKeys.DFS_NAMENODE_STALE_DATANODE_INTERVAL_KEY
              + " is less than "
  ...
  public synchronized long[] getStats() {
    return new long[] {
      getCapacityTotal(), getCapacityUsed(), getCapacityRemaining(), -1L, -1L, -1L, -1L, -1L, -1L
    };
  }
  ...
  synchronized void updateHeartbeat(
    ...
    blockManager.updateHeartbeat(
        node, reports, cacheCapacity, cacheUsed, xceiverCount, failedVolumes, volumeFailureSummary);
    stats.add(node);
  }

  synchronized void updateLifeline(
      final DatanodeDescriptor node,
      StorageReport[] reports,
      long cacheCapacity,
      long cacheUsed,
    ...
    blockManager.updateHeartbeatState(
        node, reports, cacheCapacity, cacheUsed, xceiverCount, failedVolumes, volumeFailureSummary);
    stats.add(node);
  }
  ...
  synchronized void startMaintenance(final DatanodeDescriptor node) {
    if (!node.isAlive()) {
      ...
      if (node.isDecommissioned()) {
        ...
        LOG.info(
            "MinReplicationToBeInMaintenance is set to zero. "
                + node
                + " is put in maintenance state"
                + " immediately.");
        node.setInMaintenance();
      } else {
  ...
  synchronized void stopDecommission(final DatanodeDescriptor node) {
    LOG.info("Stopping decommissioning of {} node {}", node.isAlive() ? "live" : "dead", node);
    if (!node.isAlive()) {
  ...
  /**
   * Remove deadNode from StaleNodeList if it exists. This method assumes that it is called inside a
   * synchronized block.
   *
  ...
  /**
   * Remove node from StaleNodeList if it exists. If enabled, the log will show whether the node is
   * removed from list because it is dead or not. This method assumes that it is called inside a
   * synchronized block.
   *
   * @param d node descriptor to be marked as dead.
   * @param isDead
   ...
   */
  private boolean removeNodeFromStaleList(DatanodeDescriptor d, boolean isDead) {
    boolean result = false;
    result = staleDataNodes.remove(d);
    if (enableLogStaleNodes && result) {
      LOG.info(
          String.format(
              isDead ? REPORT_REMOVE_DEAD_NODE_ENTRY : REPORT_REMOVE_STALE_NODE_ENTRY, d));
    }
    return result;
  }
  ...
  private void dumpStaleNodes(List<DatanodeDescriptor> staleNodes) {
    ...
    if (enableLogStaleNodes && (!staleNodes.isEmpty())) {
      StringBuilder staleLogMSG =
          new StringBuilder(String.format(REPORT_DELTA_STALE_DN_HEADER, staleNodes.size()));
      for (int ind = 0; ind < staleNodes.size(); ind++) {
        String logFormat =
            (ind % REPORT_STALE_NODE_NODES_PER_LINE == 0)
                ? REPORT_STALE_DN_LINE_ENTRY
                : REPORT_STALE_DN_LINE_TAIL;
   ...
   * re-replicated. While removing dead datanodes, make sure that only one datanode is marked dead
   * at a time within the synchronized section. Otherwise, a cascading effect causes more datanodes
   * to be declared dead. Check if there are any failed storage and if so, Remove all the blocks on
   * the storage. It also covers the following less common scenarios. After DatanodeStorage is
   * marked FAILED, it is still possible to receive IBR for this storage. 1) DN could deliver IBR
   * for failed storage due to its implementation. a) DN queues a pending IBR request. b) The
   * storage of the block fails. c) DN first sends HB, NN will mark the storage FAILED. d) DN then
   * sends the pending IBR request. 2) SBN processes block request from pendingDNMessages. It is
   * possible to have messages in pendingDNMessages that refer to some failed storage. a) SBN
   * receives a IBR and put it in pendingDNMessages. b) The storage of the block fails. c) Edit log
   * replay get the IBR from pendingDNMessages. Alternatively, we can resolve these scenarios with
   * the following approaches. A. Make sure DN don't deliver IBR for failed storage. B. Remove all
   * blocks in PendingDataNodeMessages for the failed storage when we remove all blocks from
   * BlocksMap for that storage.
   */
  @VisibleForTesting
  void heartbeatCheck() {
