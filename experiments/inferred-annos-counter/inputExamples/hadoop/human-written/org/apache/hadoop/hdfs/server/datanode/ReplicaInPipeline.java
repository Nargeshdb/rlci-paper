...
public interface ReplicaInPipeline extends Replica {
  ...
  /**
   * Create an output stream to write restart metadata in case of datanode shutting down for quick
   * restart.
   *
  ...
  /**
   * Causes the current thread to wait until a minimum length is reached, the thread is interrupted,
   * or the specified waiting time elapses.
   *
   * @param minLength The minimum length to achieve
   * @param time the maximum time to wait
   * @param unit the time unit of the time argument
   * @throws IOException if the current thread is interrupted or the minimum length is not achieved
