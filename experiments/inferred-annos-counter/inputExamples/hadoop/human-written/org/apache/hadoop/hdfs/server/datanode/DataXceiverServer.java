...
/**
 * Server used for receiving/sending a block of data. This is created to listen for requests from
 * clients or other DataNodes. This small server does not use the Hadoop IPC mechanism.
 */
class DataXceiverServer implements Runnable {
  public static final Logger LOG = DataNode.LOG;

  /**
   * Default time to wait (in seconds) for the number of running threads to drop below the newly
   * requested maximum before giving up.
   */
  ...
  /**
   * A manager to make sure that cluster balancing does not take too much resources.
   *
   * <p>It limits the number of block moves for balancing and the total amount of bandwidth they can
   * use.
   */
  static class BlockBalanceThrottler extends DataTransferThrottler {
    ...
    /**
     * Update the number of threads which may be used concurrently for moving blocks. The number of
     * threads available can be scaled up or down. If increasing the number of threads, the request
     * will be serviced immediately. However, if decreasing the number of threads, this method will
     * block any new request for moves, wait for any existing backlog of move requests to clear, and
     * wait for enough threads to have finished such that the total number of threads actively
     * running is less than or equal to the new cap. If this method has been unable to successfully
     * set the new, lower, cap within 'duration' seconds, the attempt will be aborted and the
     * original cap will remain.
     *
     ...
     */
    private boolean setMaxConcurrentMovers(final int newMaxThreads, final int duration) {
      Preconditions.checkArgument(newMaxThreads > 0);
      final int delta = newMaxThreads - this.maxThreads;
      LOG.debug("Change concurrent thread count to {} from {}", newMaxThreads, this.maxThreads);
      if (delta == 0) {
        return true;
      }
      if (delta > 0) {
      ...
      try {
        LOG.debug("Removing thread capacity: {}. Max wait: {}", delta, duration);
        boolean acquired = this.semaphore.tryAcquire(Math.abs(delta), duration, TimeUnit.SECONDS);
        if (acquired) {
          this.maxThreads = newMaxThreads;
        } else {
          LOG.warn(
              "Could not lower thread count to {} from {}. Too busy.",
              newMaxThreads,
              this.maxThreads);
        }
        return acquired;
     ...
     *
     * <p>Return true if the thread quota is not exceeded and the counter is incremented; False
     * otherwise.
     */
    boolean acquire() {
  ...
  /**
   * Stores an estimate for block size to check if the disk partition has enough space. Newer
   * clients pass the expected block size to the DataNode. For older clients, just use the
   * server-side default block size.
   */
  ...
  DataXceiverServer(PeerServer peerServer, Configuration conf, DataNode datanode) {
    ...
    this.balanceThrottler =
        new BlockBalanceThrottler(
            conf.getLongBytes(
  ...
  public void run() {
    ...
    while (datanode.shouldRun && !datanode.shutdownForUpgrade) {
      try {
        ...
        LOG.error("{}:DataXceiverServer: Exiting.", datanode.getDisplayName(), te);
  ...
  /**
   * Notify all peers of the shutdown and restart. 'datanode.shouldRun' should still be true and
   * 'datanode.restarting' should be set true before calling this method.
   */
  ...
  /**
   * Causes a thread to block until all peers are removed, a certain amount of time has passed, or
   * the thread is interrupted.
   *
   * @param timeout the maximum time to wait, in nanoseconds
   * @param unit the unit of time to wait
   * @return true if thread returned because all peers were removed; false otherwise
   */
  ...
  /**
   * Update the number of threads which may be used concurrently for moving blocks.
   *
  ...
  public boolean updateBalancerMaxConcurrentMovers(final int movers) {
    return balanceThrottler.setMaxConcurrentMovers(movers, this.maxReconfigureWaitTime);
  }

  /**
   * Update the maximum amount of time to wait for reconfiguration of the maximum number of block
   * mover threads to complete.
   *
