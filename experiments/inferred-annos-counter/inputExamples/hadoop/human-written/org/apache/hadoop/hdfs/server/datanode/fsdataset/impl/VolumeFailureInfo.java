...
final class VolumeFailureInfo {
  ...
  /**
   * Creates a new VolumeFailureInfo, when the capacity lost from this volume failure is unknown.
   * Typically, this means the volume failed immediately at startup, so there was never a chance to
   * query its capacity.
   *
   ...
   */
  public VolumeFailureInfo(StorageLocation failedStorageLocation, long failureDate) {
   ...
   */
  public VolumeFailureInfo(
      StorageLocation failedStorageLocation, long failureDate, long estimatedCapacityLost) {
  ...
  /**
   * Returns estimate of capacity lost. This is said to be an estimate, because in some cases it's
   * impossible to know the capacity of the volume, such as if we never had a chance to query its
   * capacity before the failure occurred.
   *
