 ...
 *
 * <p>Metrics are collected by a custom audit logger, {@link org.apache.hadoop
 * .hdfs.server.namenode.top.TopAuditLogger}, which calls TopMetrics to increment per-operation,
 * per-user counts on every audit log call. These counts are used to show the top users by NameNode
 * operation as well as across all operations.
 *
 * <p>TopMetrics maintains these counts for a configurable number of time intervals, e.g. 1min,
 * 5min, 25min. Each interval is tracked by a RollingWindowManager.
 *
 * <p>These metrics are published as a JSON string via {@link org.apache.hadoop .hdfs.server
 * .namenode.metrics.FSNamesystemMBean#getTopWindows}. This is done by calling {@link
 * org.apache.hadoop.hdfs.server.namenode.top.window .RollingWindowManager#snapshot} on each
...
public class TopMetrics implements MetricsSource {
  ...
  private static void logConf(Configuration conf) {
    LOG.info(
        "NNTop conf: "
            + DFSConfigKeys.NNTOP_BUCKETS_PER_WINDOW_KEY
            + " = "
            + conf.get(DFSConfigKeys.NNTOP_BUCKETS_PER_WINDOW_KEY));
    LOG.info(
        "NNTop conf: "
            + DFSConfigKeys.NNTOP_NUM_USERS_KEY
            + " = "
  ...
  /**
   * A map from reporting periods to WindowManager. Thread-safety is provided by the fact that the
   * mapping is not changed after construction.
   */
  ...
  public TopMetrics(Configuration conf, int[] reportingPeriods) {
    ...
    for (int i = 0; i < reportingPeriods.length; i++) {
      rollingWindowManagers.put(
          reportingPeriods[i], new RollingWindowManager(conf, reportingPeriods[i]));
    }
    isMetricsSourceEnabled =
        conf.getBoolean(DFSConfigKeys.NNTOP_ENABLED_KEY, DFSConfigKeys.NNTOP_ENABLED_DEFAULT);
  }
  ...
  /**
   * Pick the same information that DefaultAuditLogger does before writing to a log file. This is to
   * be consistent when {@link TopMetrics} is charged with data read back from log files instead of
   * being invoked directly by the FsNamesystem
   *
   * @param succeeded
   * @param userName
   * @param addr
  ...
  /**
   * Flatten out the top window metrics into {@link org.apache.hadoop.metrics2.MetricsRecord}s for
   * consumption by external metrics systems. Each metrics record added corresponds to the reporting
   * period a.k.a window length of the configured rolling windows.
   *
   * @param collector
   * @param all
   */
  ...
  private MetricsInfo buildOpTotalCountMetricsInfo(Op op) {
    return Interns.info(
        "op=" + StringUtils.deleteWhitespace(op.getOpType()) + ".TotalCount",
        "Total operation count");
  }

  private MetricsInfo buildOpRecordMetricsInfo(Op op, User user) {
    return Interns.info(
        "op=" + StringUtils.deleteWhitespace(op.getOpType()) + ".user=" + user.getUser() + ".count",
        "Total operations performed by user");
  }
}
