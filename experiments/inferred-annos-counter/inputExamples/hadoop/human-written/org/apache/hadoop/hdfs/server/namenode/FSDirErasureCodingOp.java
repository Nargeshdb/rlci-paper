...
final class FSDirErasureCodingOp {
  ...
  /** Private constructor for preventing FSDirErasureCodingOp object creation. Static-only class. */
  private FSDirErasureCodingOp() {}

  /**
   * Check if the ecPolicyName is valid and enabled, return the corresponding EC policy if is,
   * including the REPLICATION EC policy.
   *
   * @param fsn namespace
  ...
  static ErasureCodingPolicy getEnabledErasureCodingPolicyByName(
    ...
    ErasureCodingPolicy ecPolicy =
        fsn.getErasureCodingPolicyManager().getEnabledPolicyByName(ecPolicyName);
    if (ecPolicy == null) {
      final String sysPolicies =
          Arrays.asList(fsn.getErasureCodingPolicyManager().getEnabledPolicies()).stream()
              .map(ErasureCodingPolicy::getName)
              .collect(Collectors.joining(", "));
      final String message =
          String.format(
              "Policy '%s' does not match any "
                  + "enabled erasure"
                  + " coding policies: [%s]. An erasure coding policy can be"
                  + " enabled by enableErasureCodingPolicy API.",
              ecPolicyName, sysPolicies);
  ...
  /**
   * Check if the ecPolicyName is valid, return the corresponding EC policy if is, including the
  ...
  static ErasureCodingPolicy getErasureCodingPolicyByName(
    ...
    assert fsn.hasReadLock();
    ErasureCodingPolicy ecPolicy =
        fsn.getErasureCodingPolicyManager().getErasureCodingPolicyByName(ecPolicyName);
    if (ecPolicy == null) {
      throw new HadoopIllegalArgumentException(
          "The given erasure coding " + "policy " + ecPolicyName + " does not exist.");
    }
    return ecPolicy;
  }
   ...
   * @param ecPolicyName The erasure coding policy name to set on the target directory.
   ...
   */
  static FileStatus setErasureCodingPolicy(
    ...
    fsd.writeLock();
    try {
      ErasureCodingPolicy ecPolicy = getEnabledErasureCodingPolicyByName(fsn, ecPolicyName);
  ...
  private static List<XAttr> setErasureCodingPolicyXAttr(
      final FSNamesystem fsn, final INodesInPath srcIIP, ErasureCodingPolicy ecPolicy)
      throws IOException {
    FSDirectory fsd = fsn.getFSDirectory();
    ...
    final EnumSet<XAttrSetFlag> flag =
        hasEcXAttr ? EnumSet.of(XAttrSetFlag.REPLACE) : EnumSet.of(XAttrSetFlag.CREATE);
   ...
   * @throws AccessControlException if the user does not have write access
   */
  static FileStatus unsetErasureCodingPolicy(
      final FSNamesystem fsn,
      final String srcArg,
      final FSPermissionChecker pc,
      final boolean logRetryCache)
    ...
    if (xAttrs != null) {
      ...
      throw new NoECPolicySetException("No erasure coding policy explicitly set on " + src);
    }
    return fsd.getAuditFileInfo(iip);
  }
   ...
   * @throws IOException
   */
  static ErasureCodingPolicy addErasureCodingPolicy(
      final FSNamesystem fsn, ErasureCodingPolicy policy, final boolean logRetryCache) {
   ...
   * @throws IOException
   */
  static void removeErasureCodingPolicy(
      final FSNamesystem fsn, String ecPolicyName, final boolean logRetryCache) throws IOException {
   ...
   * @throws IOException
   */
  static boolean enableErasureCodingPolicy(
      final FSNamesystem fsn, String ecPolicyName, final boolean logRetryCache) throws IOException {
    Preconditions.checkNotNull(ecPolicyName);
   ...
   * @throws IOException
   */
  static boolean disableErasureCodingPolicy(
      final FSNamesystem fsn, String ecPolicyName, final boolean logRetryCache) throws IOException {
    Preconditions.checkNotNull(ecPolicyName);
   ...
   * @param fsn namespace
   * @param src path
   * @return {@link ErasureCodingPolicy}, or null if no policy has been set or the policy is
   *     REPLICATION
   * @throws IOException
   * @throws FileNotFoundException if the path does not exist.
   * @throws AccessControlException if no read access
   */
  static ErasureCodingPolicy getErasureCodingPolicy(
      final FSNamesystem fsn, final String src, FSPermissionChecker pc)
      throws IOException, AccessControlException {
    assert fsn.hasReadLock();
  ...
  /**
   * Get the erasure coding policy information for specified path and policy name. If ec policy name
   * is given, it will be parsed and the corresponding policy will be returned. Otherwise, get the
   * policy from the parents of the iip.
   *
   * @param fsn namespace
   ...
   */
  static ErasureCodingPolicy getErasureCodingPolicy(
      FSNamesystem fsn, String ecPolicyName, INodesInPath iip) throws IOException {
    ErasureCodingPolicy ecPolicy;
    if (!StringUtils.isEmpty(ecPolicyName)) {
      ecPolicy = FSDirErasureCodingOp.getEnabledErasureCodingPolicyByName(fsn, ecPolicyName);
    } else {
  ...
  /**
   * Get the erasure coding policy, including the REPLICATION policy. This does not do any
   * permission checking.
   *
   * @param fsn namespace
   * @param iip inodes in the path containing the file
   ...
   */
  static ErasureCodingPolicyInfo[] getErasureCodingPolicies(final FSNamesystem fsn)
      throws IOException {
    assert fsn.hasReadLock();
   ...
   */
  static Map<String, String> getErasureCodingCodecs(final FSNamesystem fsn) throws IOException {
    assert fsn.hasReadLock();
    ...
    try {
      for (int i = iip.length() - 1; i >= 0; i--) {
        ...
        if (inode.isFile()) {
          ...
          return id < 0 ? null : fsd.getFSNamesystem().getErasureCodingPolicyManager().getByID(id);
        }
        ...
        if (xaf != null) {
          ...
          if (xattr != null) {
            ...
            return fsd.getFSNamesystem().getErasureCodingPolicyManager().getByName(ecPolicyName);
          }
        }
      }
    } finally {
