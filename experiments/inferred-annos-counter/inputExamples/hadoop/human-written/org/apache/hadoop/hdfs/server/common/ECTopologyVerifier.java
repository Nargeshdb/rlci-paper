 ...
 *
 * <p>Scenarios when the verification fails: 1. not enough data nodes compared to EC policy's
 * highest data+parity number 2. not enough racks to satisfy BlockPlacementPolicyRackFaultTolerant
 */
...
public final class ECTopologyVerifier {
  ...
  public static ECTopologyVerifierResult getECTopologyVerifierResult(
      final DatanodeInfo[] report, final Collection<ErasureCodingPolicy> policies) {
  ...
  public static ECTopologyVerifierResult getECTopologyVerifierResult(
    ...
    for (ErasureCodingPolicy policy : policies) {
      final int policyDN = policy.getNumDataUnits() + policy.getNumParityUnits();
      minDN = Math.max(minDN, policyDN);
      final int policyRack = (int) Math.ceil(policyDN / (double) policy.getNumParityUnits());
    ...
    return verifyECWithTopology(
        minDN, minRack, numOfRacks, numOfDataNodes, getReadablePolicies(policies));
  }
  ...
  private static ECTopologyVerifierResult verifyECWithTopology(
      ...
      final int numOfRacks,
      final int numOfDataNodes,
      String readablePolicies) {
    String resultMessage;
    if (numOfDataNodes < minDN) {
      resultMessage =
          String.format(
              "%d DataNodes are required for "
    ...
    return new ECTopologyVerifierResult(
        true, String.format("The cluster setup can support EC policies: %s", readablePolicies));
  }
  ...
  private static String getReadablePolicies(final Collection<ErasureCodingPolicy> policies) {
