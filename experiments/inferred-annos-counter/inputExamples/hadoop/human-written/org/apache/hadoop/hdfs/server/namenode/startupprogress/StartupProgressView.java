 ...
 * Callers obtain an instance by calling {@link StartupProgress#createView()} to clone current
 * startup progress state. Subsequent updates to startup progress will not alter the view. This
 * isolates the reader from ongoing updates and establishes a guarantee that the values returned by
 * the view are consistent and unchanging across multiple related read operations. Calculations that
 * require aggregation, such as overall percent complete, will not be impacted by mutations
 * performed in other threads mid-way through the calculation.
 *
 * <p>Methods that return primitive long may return {@link Long#MIN_VALUE} as a sentinel value to
 * indicate that the property is undefined.
 */
...
public class StartupProgressView {
  ...
  /**
   * Returns overall elapsed time, calculated as time between start of loading fsimage and end of
   * safemode.
   *
  ...
  /**
   * Returns elapsed time for the specified phase, calculated as (end - begin) if phase is complete
   * or (now - begin) if phase is running or 0 if the phase is still pending.
   *
   * @param phase Phase to get
   * @return long elapsed time
   */
  ...
  /**
   * Returns elapsed time for the specified phase and step, calculated as (end - begin) if step is
   * complete or (now - begin) if step is running or 0 if the step is still pending.
   *
   * @param phase Phase to get
   * @param step Step to get
   * @return long elapsed time
   ...
   * This is an approximation that assumes all phases have equal running time. In practice, this
   * isn't true, but there isn't sufficient information available to predict proportional weights
   * for each phase.
   *
  ...
  /**
   * Returns percent complete for the specified phase, calculated by aggregating the counter values
   * and totals for all steps within the phase.
   *
   * @param phase Phase to get
   * @return float percent complete
   */
  ...
  /**
   * Returns percent complete for the specified phase and step, calculated as counter value divided
   * by total.
   *
   * @param phase Phase to get
   * @param step Step to get
  ...
  /**
   * Returns the optional size in bytes associated with the specified phase, possibly Long.MIN_VALUE
   * if undefined.
   *
   * @param phase Phase to get
  ...
  /**
   * Creates a new StartupProgressView by cloning data from the specified StartupProgress.
   *
  ...
  /**
   * Returns elapsed time, calculated as (end - begin) if both are defined or (now - begin) if end
   * is undefined or 0 if both are undefined. Begin and end time come from the same AbstractTracking
   * instance.
   *
  ...
  /**
   * Returns elapsed time, calculated as (end - begin) if both are defined or (now - begin) if end
   * is undefined or 0 if both are undefined. Begin and end time may come from different
   * AbstractTracking instances.
   *
  ...
  private long getElapsedTime(AbstractTracking beginTracking, AbstractTracking endTracking) {
    final long elapsed;
    if (beginTracking != null
        && beginTracking.beginTime != Long.MIN_VALUE
        && endTracking != null
        && endTracking.endTime != Long.MIN_VALUE) {
  ...
  /**
   * Returns the StepTracking internal data structure for the specified phase and step, possibly
   * null if not found.
   *
   * @param phase Phase to get
   * @param step Step to get
