...
/**
 * Extension of FSImage for the backup node. This class handles the setup of the journaling spool on
 * the backup namenode.
 */
...
public class BackupImage extends FSImage {
  ...
  /**
   * Current state of the BackupNode. The BackupNode's state transitions are as follows:
   *
   * <p>Initial: DROP_UNTIL_NEXT_ROLL - Transitions to JOURNAL_ONLY the next time the log rolls -
   * Transitions to IN_SYNC in convergeJournalSpool - Transitions back to JOURNAL_ONLY if the log
   * rolls while stopApplyingOnNextRoll is true.
   */
  volatile BNState bnState;

  enum BNState {
    /**
     * Edits from the NN should be dropped. On the next log roll, transition to JOURNAL_ONLY state
     */
    DROP_UNTIL_NEXT_ROLL,
    /**
     * Edits from the NN should be written to the local edits log but not applied to the namespace.
     */
    JOURNAL_ONLY,
    /** Edits should be written to the local edits log and applied to the local namespace. */
    IN_SYNC;
  }

  /**
   * Flag to indicate that the next time the NN rolls, the BN should transition from to JOURNAL_ONLY
   * state. {@see #freezeNamespaceAtNextRoll()}
   */
   ...
   *
   * <p>Depending on bnState, different actions are taken. See {@link BackupImage.BNState}
   *
  ...
  private synchronized void applyEdits(long firstTxId, int numTxns, byte[] data)
    ...
    try {
      ...
      long numTxnsAdvanced =
          logLoader.loadEditRecords(backupInputStream, true, lastAppliedTxId + 1, null, null);
      if (numTxnsAdvanced != numTxns) {
        throw new IOException(
            "Batch of txns starting at txnid "
                + firstTxId
  ...
  /**
   * Transition the BackupNode from JOURNAL_ONLY state to IN_SYNC state. This is done by repeated
   * invocations of tryConvergeJournalSpool until we are caught up to the latest in-progress edits
   * file.
   */
  ...
  private boolean tryConvergeJournalSpool() throws IOException {
    ...
    while (lastAppliedTxId < editLog.getCurSegmentTxId() - 1) {
      ...
      LOG.info(
          "Loading edits into backupnode to try to catch up from txid "
              + lastAppliedTxId
              + " to "
              + target);
    ...
    synchronized (this) {
      ...
      Collection<EditLogInputStream> editStreams =
          getEditLog()
              .selectInputStreams(
                  getEditLog().getCurSegmentTxId(), getEditLog().getCurSegmentTxId());
      ...
      if (stream == null) {
        LOG.warn(
            "Unable to find stream starting with "
                + editLog.getCurSegmentTxId()
      ...
      try {
        ...
        LOG.info(
            "Going to finish converging with remaining "
                + remainingTxns
                + " txns from in-progress stream "
                + stream);
  ...
  /**
   * Receive a notification that the NameNode has begun a new edit log. This causes the BN to also
   * start the new edit log in its local directories.
   */
  ...
  /**
   * Request that the next time the BN receives a log roll, it should stop applying the edits log to
   * the local namespace. This is typically followed on by a call to {@link
  ...
  /**
   * After {@link #freezeNamespaceAtNextRoll()} has been called, wait until the BN receives
   * notification of the next log roll.
   */
  synchronized void waitUntilNamespaceFrozen() throws IOException {
    if (bnState != BNState.IN_SYNC) return;

    LOG.info(
        "Waiting until the NameNode rolls its edit logs in order "
            + "to freeze the BackupNode namespace.");
    while (bnState == BNState.IN_SYNC) {
      Preconditions.checkState(
          stopApplyingEditsOnNextRoll,
          "If still in sync, we should still have the flag set to " + "freeze at next roll");
      try {
        wait();
