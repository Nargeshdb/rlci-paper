...
class ProvidedVolumeImpl extends FsVolumeImpl {
   ...
   * @return the suffix of the path, which when resolved against {@code prefix} gets back the {@code
   *     fullPath}.
   */
  @VisibleForTesting
  ...
  static class ProvidedBlockPoolSlice {
    ...
    void fetchVolumeMap(
        ReplicaMap volumeMap, RamDiskReplicaTracker ramDiskReplicaMap, FileSystem remoteFS)
        throws IOException {
      ...
      if (reader == null) {
        LOG.error(
            "Got null reader from BlockAliasMap " + aliasMap + "; no blocks will be populated");
        return;
      }
      Path blockPrefixPath = new Path(providedVolume.getBaseURI());
      for (FileRegion region : reader) {
        if (containsBlock(
            providedVolume.baseURI, region.getProvidedStorageLocation().getPath().toUri())) {
          String blockSuffix =
              getSuffix(
                  blockPrefixPath, new Path(region.getProvidedStorageLocation().getPath().toUri()));
          PathHandle pathHandle = null;
          if (region.getProvidedStorageLocation().getNonce().length > 0) {
            pathHandle =
                new RawPathHandle(ByteBuffer.wrap(region.getProvidedStorageLocation().getNonce()));
          }
    ...
    public void compileReport(Collection<ScanInfo> report, ReportCompiler reportCompiler)
  ...
  public long getCapacity() {
    try {
      ...
      LOG.warn("Exception when trying to get capacity of ProvidedVolume: {}", e);
    }
    return 0L;
  }
  ...
  private static class ProvidedBlockIteratorState {
    ...
    @JsonProperty private long lastBlockId;
  }
  ...
  public BlockIterator newBlockIterator(String bpid, String name) {
    return new ProviderBlockIteratorImpl(bpid, name, bpSlices.get(bpid).getBlockAliasMap());
  }

  @Override
  public BlockIterator loadBlockIterator(String bpid, String name) throws IOException {
    ProviderBlockIteratorImpl iter =
        new ProviderBlockIteratorImpl(bpid, name, bpSlices.get(bpid).getBlockAliasMap());
    iter.load();
    return iter;
  }
  ...
  void getVolumeMap(
      ...
      throws IOException {
    getProvidedBlockPoolSlice(bpid).fetchVolumeMap(volumeMap, ramDiskReplicaMap, remoteFS);
  }
  ...
  void addBlockPool(String bpid, Configuration conf, Timer timer) throws IOException {
    LOG.info("Adding block pool " + bpid + " to volume with id " + getStorageID());
   ...
   * @return true if the {@code blockURI} can belong to the volume or both URIs are null.
   */
  @VisibleForTesting
  ...
  @VisibleForTesting
  void setFileRegionProvider(String bpid, BlockAliasMap<FileRegion> blockAliasMap)
      throws IOException {
    ProvidedBlockPoolSlice bp = bpSlices.get(bpid);
    if (bp == null) {
      throw new IOException("block pool " + bpid + " is not found");
