...
public final class DirectoryWithQuotaFeature implements INode.Feature {
    ...
    public Builder() {
      this.quota =
          new QuotaCounts.Builder()
              ...
              .typeSpaces(DEFAULT_STORAGE_SPACE_QUOTA)
              .build();
   ...
   * @param type Storage type of the storage space quota to be set. To set storagespace/namespace
   *     quota, type must be null.
   */
  ...
  ContentSummaryComputationContext computeContentSummary(
      final INodeDirectory dir, final ContentSummaryComputationContext summary)
  ...
  /**
   * Sets namespace and storagespace take by the directory rooted at this INode. This should be used
   * carefully. It does not check for quota violations.
   *
  ...
  /**
   * @throws QuotaExceededException if namespace, storagespace or storage type space quota is
  ...
  private String namespaceString() {
    return "namespace: "
        + (quota.getNameSpace() < 0 ? "-" : usage.getNameSpace() + "/" + quota.getNameSpace());
  }

  private String storagespaceString() {
    return "storagespace: "
        + (quota.getStorageSpace() < 0
            ? "-"
