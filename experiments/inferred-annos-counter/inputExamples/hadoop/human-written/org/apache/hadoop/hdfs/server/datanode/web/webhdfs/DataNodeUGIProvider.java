...
/**
 * Create UGI from the request for the WebHDFS requests for the DNs. Note that the DN does not
 * authenticate the UGI -- the NN will authenticate them in subsequent operations.
 */
public class DataNodeUGIProvider {
  private final ParameterParser params;
  @VisibleForTesting static Cache<String, UserGroupInformation> ugiCache;
      ...
      if (UserGroupInformation.isSecurityEnabled()) {
        ...
        final String remoteUser =
            usernameFromQuery == null
                ? JspHelper.getDefaultWebUserName(params.conf()) // not specified in request
                : usernameFromQuery;
  ...
  private String buildNonTokenCacheKey(String doAsUserFromQuery, String remoteUser)
      throws IOException {
    String key =
        doAsUserFromQuery == null
            ? String.format("{%s}", remoteUser)
            : String.format("{%s}:{%s}", remoteUser, doAsUserFromQuery);
    return key;
  }

  private UserGroupInformation nonTokenUGI(
      String usernameFromQuery, String doAsUserFromQuery, String remoteUser) throws IOException {
