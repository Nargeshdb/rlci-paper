...
public class FileUnderConstructionFeature implements INode.Feature {
  ...
  void updateLengthOfLastBlock(INodeFile f, long lastBlockLength) throws IOException {
    ...
    assert (lastBlock != null)
        : "The last block for path " + f.getFullPathName() + " is null when updating its length";
    assert !lastBlock.isComplete()
        : "The last block for path "
            + f.getFullPathName()
  ...
  /**
   * When deleting a file in the current fs directory, and the file is contained in a snapshot, we
   * should delete the last block if it's under construction and its size is 0.
   */
