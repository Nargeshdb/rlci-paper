...
/**
 * An in-memory cache of edits in their serialized form. This is used to serve the {@link
 * Journal#getJournaledEdits(long, int)} call, used by the QJM when {@value
 * DFSConfigKeys#DFS_HA_TAILEDITS_INPROGRESS_KEY} is enabled.
 *
 * <p>When a batch of edits is received by the JournalNode, it is put into this cache via {@link
 * #storeEdits(byte[], long, long, int)}. Edits must be stored contiguously; if a batch of edits is
 * stored that does not align with the previously stored edits, the cache will be cleared before
 * storing new edits to avoid gaps. This decision is made because gaps are only handled when in
 * recovery mode, which the cache is not intended to be used for.
 *
 * <p>Batches of edits are stored in a {@link TreeMap} mapping the starting transaction ID of the
 * batch to the data buffer. Upon retrieval, the relevant data buffers are concatenated together and
 * a header is added to construct a fully-formed edit data stream.
 *
 * <p>The cache is of a limited size capacity determined by {@value
 * DFSConfigKeys#DFS_JOURNALNODE_EDIT_CACHE_SIZE_KEY}. If the capacity is exceeded after adding a
 * new batch of edits, batches of edits are removed until the total size is less than the capacity,
 * starting from the ones containing the oldest transactions. Transactions range in size, but a
 * decent rule of thumb is that 200 bytes are needed per transaction. Monitoring the {@link
 * JournalMetrics#rpcRequestCacheMissAmount} metric is recommended to determine if the cache is too
 * small; it will indicate both how many cache misses occurred, and how many more transactions would
 * have been needed in the cache to serve the request.
 */
...
class JournaledEditsCache {
  ...
  /** Read/write lock pair wrapped in AutoCloseable; these refer to the same underlying lock. */
  ...
  /**
   * Stores the actual data as a mapping of the StartTxnId of a batch of edits to the serialized
   * batch of edits. Stores only contiguous ranges; that is, the last transaction ID in one batch is
   * always one less than the first transaction ID in the next batch. Though the map is protected by
   * the lock, individual data buffers are immutable and can be accessed without locking.
   */
  ...
  /**
   * The lowest/highest transaction IDs present in the cache. {@value INVALID_TXN_ID} if there are
   * no transactions in the cache.
   */
  private long lowestTxnId;

  private long highestTxnId;
  /**
   * The lowest transaction ID that was ever present in the cache since last being reset (i.e. since
   * initialization or since reset due to being out of sync with the Journal). Until the cache size
   * goes above capacity, this is equal to lowestTxnId.
   */
  ...
  JournaledEditsCache(Configuration conf) {
    ...
    if (capacity > 0.9 * Runtime.getRuntime().maxMemory()) {
      Journal.LOG.warn(
          String.format(
              "Cache capacity is set at %d bytes but "
    ...
    Journal.LOG.info(
        "Enabling the journaled edits cache with a capacity " + "of bytes: " + capacity);
  ...
  /**
   * Fetch the data for edits starting at the specific transaction ID, fetching up to {@code
   * maxTxns} transactions. Populates a list of output buffers which contains a serialized version
   * of the edits, and returns the count of edits contained within the serialized buffers. The
   * serialized edits are prefixed with a standard edit log header containing information about the
   * layout version. The transactions returned are guaranteed to have contiguous transaction IDs.
   *
   * <p>If {@code requestedStartTxn} is higher than the highest transaction which has been added to
   * this cache, a response with an empty buffer and a transaction count of 0 will be returned. If
   * {@code requestedStartTxn} is lower than the lowest transaction currently contained in this
   * cache, or no transactions have yet been added to the cache, an exception will be thrown.
   *
   * @param requestedStartTxn The ID of the first transaction to return. If any transactions are
   *     returned, it is guaranteed that the first one will have this ID.
   * @param maxTxns The maximum number of transactions to return.
   * @param outputBuffers A list to populate with output buffers. When concatenated, these form a
   *     full response.
   * @return The number of transactions contained within the set of output buffers.
   * @throws IOException If transactions are requested which cannot be served by this cache.
   */
  int retrieveEdits(long requestedStartTxn, int maxTxns, List<ByteBuffer> outputBuffers)
      throws IOException {
    int txnCount = 0;
    ...
    try (AutoCloseableLock l = readLock.acquire()) {
      ...
      Iterator<Map.Entry<Long, byte[]>> incrBuffIter =
          dataMap.tailMap(dataMap.floorKey(requestedStartTxn), true).entrySet().iterator();
    ...
    firstBuf.position(findTransactionPosition(firstBuf.array(), requestedStartTxn));
    // Remove trailing transactions in the last buffer if necessary
    if (txnCount > maxTxns) {
      ByteBuffer lastBuf = outputBuffers.get(outputBuffers.size() - 1);
      int limit = findTransactionPosition(lastBuf.array(), requestedStartTxn + maxTxns);
      lastBuf.limit(limit);
      txnCount = maxTxns;
    }
  ...
  /**
   * Store a batch of serialized edits into this cache. Removes old batches as necessary to keep the
   * total size of the cache below the capacity. See the class Javadoc for more info.
   *
   * <p>This attempts to always handle malformed inputs gracefully rather than throwing an
   * exception, to allow the rest of the Journal's operations to proceed normally.
   *
   ...
   * @param newLayoutVersion The version of the layout used to serialize the edits
   */
  void storeEdits(byte[] inputData, long newStartTxn, long newEndTxn, int newLayoutVersion) {
    if (newStartTxn < 0 || newEndTxn < newStartTxn) {
      Journal.LOG.error(
          String.format(
              "Attempted to cache data of length %d " + "with newStartTxn %d and newEndTxn %d",
    ...
    try (AutoCloseableLock l = writeLock.acquire()) {
      if (newLayoutVersion != layoutVersion) {
        try {
          ...
          Journal.LOG.error(
              String.format(
                  "Unable to save new edits [%d, %d] "
                      + "due to exception when updating to new layout version %d",
                  newStartTxn, newEndTxn, newLayoutVersion),
              ioe);
          return;
        }
      } else if (lowestTxnId == INVALID_TXN_ID) {
        Journal.LOG.info("Initializing edits cache starting from txn ID " + newStartTxn);
      ...
      if (inputData.length > capacity) {
        ...
        Journal.LOG.warn(
            String.format(
                "A single batch of edits was too "
  ...
  /**
   * Skip through a given stream of edits until the given transaction ID is found. Return the number
   * of bytes that appear prior to the given transaction.
   *
   * @param buf A buffer containing a stream of serialized edits
   * @param txnId The transaction ID to search for
   * @return The number of bytes appearing in {@code buf} <i>before</i> the start of the transaction
   *     with ID {@code txnId}.
   */
  private int findTransactionPosition(byte[] buf, long txnId) throws IOException {
    ...
    FSEditLogOp.Reader reader =
        FSEditLogOp.Reader.create(new DataInputStream(tracker), tracker, layoutVersion);
    long previousPos = 0;
  ...
  /**
   * Update the layout version of the cache. This clears out all existing entries, and populates the
   * new layout version and header for that version.
   *
  ...
  private void updateLayoutVersion(int newLayoutVersion, long newStartTxn) throws IOException {
    ...
    if (layoutVersion != INVALID_LAYOUT_VERSION) {
      logMsg
          .append("; previous version was ")
          .append(layoutVersion)
   ...
   *
   * @param newInitialTxnId The new lowest transaction ID stored in the cache. This should be
   *     {@value INVALID_TXN_ID} if the cache is to remain empty at this time.
   */
  ...
  /**
   * Return the underlying data buffer used to store information about the given transaction ID.
   *
  ...
  private CacheMissException getCacheMissException(long requestedTxnId) {
    if (lowestTxnId == INVALID_TXN_ID) {
      return new CacheMissException(
          0,
          "Cache is empty; either it was never "
