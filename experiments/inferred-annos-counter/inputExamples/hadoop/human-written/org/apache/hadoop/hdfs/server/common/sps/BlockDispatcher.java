...
public class BlockDispatcher {
   ...
   *
   * @param sockTimeout soTimeout
   * @param ioFileBuffSize file io buffer size
   * @param connectToDatanodeViaHostname true represents connect via hostname, false otw
   */
  public BlockDispatcher(
      int sockTimeout, int ioFileBuffSize, boolean connectToDatanodeViaHostname) {
   ...
   *
   * @param blkMovingInfo block to storage info
   * @param saslClient SASL for DataTransferProtocol on behalf of a client
   * @param eb extended block info
   * @param sock target node's socket
   * @param km for creation of an encryption key
   * @param accessToken connection block access token
   * @return status of the block movement
   */
  public BlockMovementStatus moveBlock(
      BlockMovingInfo blkMovingInfo,
      SaslDataTransferClient saslClient,
      ExtendedBlock eb,
      @Owning Socket sock,
      DataEncryptionKeyFactory km,
      Token<BlockTokenIdentifier> accessToken) {
    LOG.info(
        "Start moving block:{} from src:{} to destin:{} to satisfy "
            + "storageType, sourceStoragetype:{} and destinStoragetype:{}",
        blkMovingInfo.getBlock(),
        blkMovingInfo.getSource(),
        blkMovingInfo.getTarget(),
        blkMovingInfo.getSourceStorageType(),
    ...
    try {
      NetUtils.connect(
          sock,
          NetUtils.createSocketAddr(blkMovingInfo.getTarget().getXferAddr(connectToDnViaHostname)),
          socketTimeout);
      ...
      IOStreamPair saslStreams =
          saslClient.socketSend(
              sock, unbufOut, unbufIn, km, accessToken, blkMovingInfo.getTarget());
      ...
      sendRequest(
          out, eb, accessToken, blkMovingInfo.getSource(), blkMovingInfo.getTargetStorageType());
      receiveResponse(in);

      LOG.info(
          "Successfully moved block:{} from src:{} to destin:{} for" + " satisfying storageType:{}",
          blkMovingInfo.getBlock(),
          blkMovingInfo.getSource(),
          blkMovingInfo.getTarget(),
          blkMovingInfo.getTargetStorageType());
      ...
      LOG.debug("Pinned block can't be moved, so skipping block:{}", blkMovingInfo.getBlock(), e);
      return BlockMovementStatus.DN_BLK_STORAGE_MOVEMENT_SUCCESS;
    } catch (IOException e) {
      // TODO: handle failure retries
      LOG.warn(
          "Failed to move block:{} from src:{} to destin:{} to satisfy " + "storageType:{}",
          blkMovingInfo.getBlock(),
          blkMovingInfo.getSource(),
          blkMovingInfo.getTarget(),
          blkMovingInfo.getTargetStorageType(),
  ...
  private static void sendRequest(
      ...
      throws IOException {
    new Sender(out)
        .replaceBlock(eb, targetStorageType, accessToken, source.getDatanodeUuid(), source, null);
  }
