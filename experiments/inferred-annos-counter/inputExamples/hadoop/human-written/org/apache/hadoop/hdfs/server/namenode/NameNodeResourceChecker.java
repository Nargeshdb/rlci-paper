...
public class NameNodeResourceChecker {
  ...
  class CheckedVolume implements CheckableNameNodeResource {
    ...
    public boolean isResourceAvailable() {
      ...
      if (availableSpace < duReserved) {
        LOG.warn(
            "Space available on volume '"
                + volume
                + "' is "
                + availableSpace
                + ", which is below the configured reserved amount "
                + duReserved);
        return false;
      } else {
        return true;
  ...
  public NameNodeResourceChecker(Configuration conf) throws IOException {
    ...
    for (URI editsDirToCheck : localEditDirs) {
      addDirToCheck(
          editsDirToCheck,
          FSNamesystem.getRequiredNamespaceEditsDirs(conf).contains(editsDirToCheck));
    }
  ...
  /**
   * Add the volume of the passed-in directory to the list of volumes to check. If <code>required
   * </code> is true, and this volume is already present, but is marked redundant, it will be marked
   * required. If the volume is already present but marked required then this method is a no-op.
   *
