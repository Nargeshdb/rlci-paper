...
/**
 * Netty handler that integrates with the {@link RestCsrfPreventionFilter}. If the filter determines
 * that the request is allowed, then this handler forwards the request to the next handler in the
 * Netty pipeline. Otherwise, this handler drops the request and immediately sends an HTTP 400
 * response.
 */
...
final class RestCsrfPreventionFilterHandler extends SimpleChannelInboundHandler<HttpRequest> {
  ...
  /**
   * Creates a new RestCsrfPreventionFilterHandler. There will be a new instance created for each
   * new Netty channel/pipeline serving a new request. To prevent the cost of repeated
   * initialization of the filter, this constructor requires the caller to pass in a pre-built,
   * fully initialized filter instance. The filter is stateless after initialization, so it can be
   * shared across multiple Netty channels/pipelines.
   *
  ...
  @Override
  protected void channelRead0(final ChannelHandlerContext ctx, final HttpRequest req)
      throws Exception {
    if (restCsrfPreventionFilter != null) {
      restCsrfPreventionFilter.handleHttpInteraction(new NettyHttpInteraction(ctx, req));
    } else {
  ...
  /**
   * Finish handling this pipeline by writing a response with the "Connection: close" header,
   * flushing, and scheduling a close of the connection.
   *
  ...
  /**
   * Creates a {@link RestCsrfPreventionFilter} for the {@DatanodeHttpServer}. This method takes
   * care of configuration and implementing just enough of the servlet API and related interfaces so
   * that the DataNode can get a fully initialized instance of the filter.
   *
  ...
  public static RestCsrfPreventionFilter initializeState(Configuration conf) {
    if (!conf.getBoolean(
        DFS_WEBHDFS_REST_CSRF_ENABLED_KEY, DFS_WEBHDFS_REST_CSRF_ENABLED_DEFAULT)) {
      return null;
    }
    String restCsrfClassName = RestCsrfPreventionFilter.class.getName();
    Map<String, String> restCsrfParams =
        RestCsrfPreventionFilter.getFilterParams(conf, "dfs.webhdfs.rest-csrf.");
