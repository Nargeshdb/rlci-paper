...
/**
 * This class is used to represent provided blocks that are file regions, i.e., can be described
 * using (path, offset, length).
 */
...
public class FileRegion implements BlockAlias {
  ...
  public FileRegion(
      long blockId, Path path, long offset, long length, long genStamp, byte[] nonce) {
    this(
        new Block(blockId, length, genStamp),
        new ProvidedStorageLocation(path, offset, length, nonce));
  }

  public FileRegion(long blockId, Path path, long offset, long length) {
    this(blockId, path, offset, length, HdfsConstants.GRANDFATHER_GENERATION_STAMP);
  }
