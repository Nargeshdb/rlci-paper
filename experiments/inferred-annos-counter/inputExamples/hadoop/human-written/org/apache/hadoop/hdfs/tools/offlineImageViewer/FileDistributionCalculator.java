...
final class FileDistributionCalculator {
  ...
  private static final int MAX_INTERVALS = 0x8000000; // 128 M = 2^27
  ...
  FileDistributionCalculator(
      Configuration conf, long maxSize, int steps, boolean formatOutput, PrintStream out) {
    this.conf = conf;
          ...
          out.print(
              (i == 0 ? "[" : "(")
                  ...
                  + StringUtils.byteDesc(
                      (long) (i == distribution.length - 1 ? maxFileSize : (long) i * steps))
                  + "]\t"
                  + distribution[i]);
        } else {
