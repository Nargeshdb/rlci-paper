...
public class FsDatasetUtil {
  ...
  public static InputStream getDirectInputStream(long addr, long length) throws IOException {
    try {
      ...
      ByteBuffer byteBuffer = (ByteBuffer) constructor.newInstance(addr, (int) length);
  ...
  /**
   * Find the meta-file for the specified block file and then return the generation stamp from the
   * name of the meta-file. Generally meta file will be the next file in sorted array of file's.
   *
   * @param listdir sorted list of file based on name.
   * @param blockFile block file for which generation stamp is needed.
   * @param index index of block file in array.
  ...
  /**
   * Compute the checksum for a block file that does not already have its checksum computed, and
   * save it to dstMeta file.
   */
  public static void computeChecksum(
      File srcMeta, File dstMeta, File blockFile, int smallBufferSize, Configuration conf)
      throws IOException {
    ...
    ReplicaInfo wrapper =
        new FinalizedReplica(0, 0, 0, null, null) {
          @Override
