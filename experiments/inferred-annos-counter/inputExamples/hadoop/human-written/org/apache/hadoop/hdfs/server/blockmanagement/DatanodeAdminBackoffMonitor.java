...
/**
 * This class implements the logic to track decommissioning and entering maintenance nodes, ensure
 * all their blocks are adequately replicated before they are moved to the decommissioned or
 * maintenance state.
 *
 * <p>This monitor avoids flooding the replication queue with all pending blocks and instead feeds
 * them to the queue as the prior set complete replication.
 *
 * <p>HDFS-14854 contains details about the overall design of this class.
 */
public class DatanodeAdminBackoffMonitor extends DatanodeAdminMonitorBase
    implements DatanodeAdminMonitorInterface {
  /**
   * Map containing the DECOMMISSION_INPROGRESS or ENTERING_MAINTENANCE datanodes that are being
   * tracked so they can be be marked as DECOMMISSIONED or IN_MAINTENANCE. Even after the node is
   * marked as IN_MAINTENANCE, the node remains in the map until maintenance expires checked during
   * a monitor tick.
   *
   * <p>This holds a set of references to the under-replicated blocks on the DN at the time the DN
   * is added to the map, i.e. the blocks that are preventing the node from being marked as
   * decommissioned. During a monitor tick, this list is pruned as blocks becomes replicated.
   *
   * <p>Note also that the reference to the list of under-replicated blocks will be null on initial
   * add
   *
   * <p>However, this map can become out-of-date since it is not updated by block reports or other
   * events. Before being finally marking as decommissioned, another check is done with the actual
   * block map.
   */
  private HashMap<DatanodeDescriptor, HashMap<BlockInfo, Integer>> outOfServiceNodeBlocks =
      new HashMap<>();

  /**
   * Any nodes where decommission or maintenance has been cancelled are added to this queue for
   * later processing.
   */
  private final Queue<DatanodeDescriptor> cancelledNodes = new ArrayDeque<>();

  /**
   * The numbe of blocks to process when moving blocks to pendingReplication before releasing and
   * reclaiming the namenode lock.
   */
  ...
  /**
   * The list of blocks which have been placed onto the replication queue and are waiting to be
  ...
  protected void processConf() {
    this.pendingRepLimit =
        conf.getInt(
            DFSConfigKeys.DFS_NAMENODE_DECOMMISSION_BACKOFF_MONITOR_PENDING_LIMIT,
            DFSConfigKeys.DFS_NAMENODE_DECOMMISSION_BACKOFF_MONITOR_PENDING_LIMIT_DEFAULT);
    if (this.pendingRepLimit < 1) {
      LOG.error(
          "{} is set to an invalid value, it must be greater than " + "zero. Defaulting to {}",
          DFSConfigKeys.DFS_NAMENODE_DECOMMISSION_BACKOFF_MONITOR_PENDING_LIMIT,
          DFSConfigKeys.DFS_NAMENODE_DECOMMISSION_BACKOFF_MONITOR_PENDING_LIMIT_DEFAULT);
      this.pendingRepLimit =
          DFSConfigKeys.DFS_NAMENODE_DECOMMISSION_BACKOFF_MONITOR_PENDING_LIMIT_DEFAULT;
    ...
    this.blocksPerLock =
        conf.getInt(
            DFSConfigKeys.DFS_NAMENODE_DECOMMISSION_BACKOFF_MONITOR_PENDING_BLOCKS_PER_LOCK,
            DFSConfigKeys
    ...
    if (blocksPerLock <= 0) {
      LOG.error(
          "{} is set to an invalid value, it must be greater than " + "zero. Defaulting to {}",
          DFSConfigKeys.DFS_NAMENODE_DECOMMISSION_BACKOFF_MONITOR_PENDING_BLOCKS_PER_LOCK,
          DFSConfigKeys.DFS_NAMENODE_DECOMMISSION_BACKOFF_MONITOR_PENDING_BLOCKS_PER_LOCK_DEFAULT);
      blocksPerLock =
          DFSConfigKeys.DFS_NAMENODE_DECOMMISSION_BACKOFF_MONITOR_PENDING_BLOCKS_PER_LOCK_DEFAULT;
    }
  ...
  /**
   * Queue a node to be removed from tracking. This method must be called under the namenode write
   * lock.
   *
  ...
  public void run() {
    ...
    try {
      ...
      try {
        /**
         * Other threads can modify the pendingNode list and the cancelled node list, so we must
         * process them under the NN write lock to prevent any concurrent modifications.
         *
         * <p>Always process the cancelled list before the pending list, as it is possible for a
         * node to be cancelled, and then quickly added back again. If we process these the other
         * way around, the added node will be removed from tracking by the pending cancel.
         */
      ...
      LOG.warn("DatanodeAdminMonitor caught exception when processing node.", e);
    }
    if (numBlocksChecked + outOfServiceNodeBlocks.size() > 0) {
      LOG.info(
          "Checked {} blocks this tick. {} nodes are now "
              + "in maintenance or transitioning state. {} nodes pending. {} "
              + "nodes waiting to be cancelled.",
          numBlocksChecked,
          outOfServiceNodeBlocks.size(),
          pendingNodes.size(),
          cancelledNodes.size());
    }
  }
  ...
  /**
   * Move any pending nodes into outOfServiceNodeBlocks to initiate the decommission or maintenance
   * mode process.
   *
   * <p>This method must be executed under the namenode write lock to prevent the pendingNodes list
  ...
  /**
   * Process any nodes which have had their decommission or maintenance mode cancelled by an
   * administrator.
   *
   * <p>This method must be executed under the write lock to prevent the cancelledNodes list being
   * modified externally.
   */
  ...
  /**
   * This method performs each of the steps to track a node from decommissioning or entering
   * maintenance to the end state.
   *
   * <p>First, any newly added nodes are scanned.
   *
   * <p>Then any expired maintenance nodes are handled.
   *
   * <p>Next the pendingRep map is scanned and all blocks which are now sufficiently replicated are
   * removed
   *
   * <p>Then new blocks are moved to pendingRep
   *
   * <p>Finally we check if any nodes have completed the replication process and if so move them to
   * their final states.
   *
   * <p>This methods which this method calls will take and release the namenode read and write lock
   * several times.
   */
  private void check() {
  ...
  /**
   * Checks for any nodes which are in maintenance and if maintenance has expired, the node will be
   * moved back to in_service (or dead) as required.
   */
  ...
  /**
   * Loop over all nodes in the passed toRemove list and move the node to the required end state.
   * This will also remove any entries from outOfServiceNodeBlocks and pendingRep for the node if
   * required.
   *
      ...
      for (DatanodeDescriptor dn : toRemove) {
        ...
        if (isHealthy) {
          if (dn.isDecommissionInProgress()) {
            ...
            // from cancelledNodes
            LOG.info(
                "Node {} completed decommission and maintenance "
                    + "but has been moved back to in service",
                dn);
            pendingRep.remove(dn);
            outOfServiceNodeBlocks.remove(dn);
            continue;
            ...
            // Should not happen
            LOG.error(
                "Node {} is in an unexpected state {} and has been "
  ...
  /**
   * Loop over all nodes and check for any which have zero unprocessed or pending blocks. If the
   * node has zero blocks pending, the storage is rescanned to ensure no transient blocks were
   * missed on the first pass.
   *
   * <p>If, after rescan the number of blocks pending replication is zero, the node is added to the
   * passed removeList which will later be processed to complete the decommission or entering
   * maintenance process.
   *
   * @param removeList Nodes which have zero pending blocks are added to this list.
   */
  ...
  /**
   * Returns the number of block pending for the given node by adding those blocks in pendingRep and
  ...
  /**
   * Iterate across all nodes in outOfServiceNodeBlocks which have blocks yet to be processed.
   *
   * <p>The block is removed from outOfServiceNodeBlocks and if it needs replication it is added to
   * the pendingRep map and also to the BlockManager replication queue.
   *
   * <p>Any block that does not need replication is discarded.
   *
   * <p>The method will return when there are the pendingRep map has pendingRepLimit blocks or there
   * are no further blocks to process.
   */
  private void moveBlocksToPending() {
    ...
    try {
      ...
      LOG.info(
          "There are {} blocks pending replication and the limit is "
      ...
      HashMap<DatanodeDescriptor, Iterator<BlockInfo>> iterators = new HashMap<>();
      ...
      Iterator<DatanodeDescriptor> nodeIter = Iterables.cycle(iterators.keySet()).iterator();
  ...
  /**
   * Takes and removes the next block from the given iterator and checks if it needs additional
   * replicas. If it does, it will be scheduled for reconstruction and added to the pendingRep map.
   *
  ...
  private int getPendingCount() {
    ...
    return pendingRep.values().stream().map(a -> a.size()).reduce(0, (a, b) -> a + b);
  }
  ...
  private int getYetToBeProcessedCount() {
    ...
    return outOfServiceNodeBlocks.values().stream().map(a -> a.size()).reduce(0, (a, b) -> a + b);
  }

  /**
   * Scan all the blocks held on a datanodes. For a node being decommissioned we assume that the
   * majority of blocks on the node will need to have new replicas made, and therefore we do not
   * check if they are under replicated here and instead add them to the list of blocks to track.
   *
   * <p>For a node being moved into maintenance, we assume most blocks will be replicated OK and
   * hence we do check their under-replicated status here, hopefully reducing the number of blocks
   * to track.
   *
   * <p>On a re-scan (initalScan = false) we assume the node has been processed already, and hence
   * there should be few under-replicated blocks, so we check the under-replicated status before
   * adding the blocks to the tracking list.
   *
   * <p>This means that for a node being decomission there should be a large number of blocks to
   * process later but for maintenance, a smaller number.
   *
   * <p>As this method does not schedule any blocks for reconstuction, this scan can be performed
   * under the namenode readlock, and the lock is dropped and reaquired for each storage on the DN.
   *
   * @param dn - The datanode to process
   * @param initialScan - True is this is the first time scanning the node or false if it is a
   *     rescan.
   */
  ...
  /**
   * Process the list of pendingReplication Blocks. These are the blocks which have been moved from
   * outOfServiceNodeBlocks, confirmed to be under-replicated and were added to the blockManager
   * replication queue.
   *
   * <p>Any blocks which have been confirmed to be replicated sufficiently are removed from the
   * list.
   *
   * <p>The datanode stats are also updated in this method, updating the total pending block count,
   * the number of blocks in PendingRep which are in open files and the number of blocks in
   * PendingRep which are only on out of service nodes.
   *
   * <p>As this method makes changes to the replication queue, it acquires the namenode write lock
   * while it runs.
   */
  private void processPendingReplication() {
    namesystem.writeLock();
    try {
      for (Iterator<Map.Entry<DatanodeDescriptor, List<BlockInfo>>> entIt =
              pendingRep.entrySet().iterator();
          entIt.hasNext(); ) {
  ...
  /**
   * Checks if a block is sufficiently replicated and optionally schedules it for reconstruction if
   * it is not.
   *
   * <p>If a BlockStats object is passed, this method will also update it if the block is part of an
   * open file or only on outOfService nodes.
   *
   * @param datanode The datanode the block belongs to
   * @param block The block to check
   * @param scheduleReconStruction Whether to add the block to the replication queue if it is not
   *     sufficiently replicated. Passing true will add it to the replication queue, and false will
   *     not.
   * @param suspectBlocks If non-null check if the block is part of an open file or only on out of
   *     service nodes and update the passed object accordingly.
   * @return
   */
  private boolean isBlockReplicatedOk(
      DatanodeDescriptor datanode,
      BlockInfo block,
      Boolean scheduleReconStruction,
    ...
    if (neededReconstruction && scheduleReconStruction) {
      if (!blockManager.neededReconstruction.contains(block)
        ...
        blockManager.neededReconstruction.add(
            block,
            liveReplicas,
    ...
    if (suspectBlocks != null) {
      ...
      if (bc.isUnderConstruction()) {
        ...
        if (!(ucFile instanceof INodeFile) || !ucFile.asFile().isUnderConstruction()) {
          LOG.warn(
              "File {} is not under construction. Skipping add to " + "low redundancy open files!",
              ucFile.getLocalName());
        } else {
