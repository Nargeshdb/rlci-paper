...
/**
 * A CachePool describes a set of cache resources being managed by the NameNode. User caching
 * requests are billed to the cache pool specified in the request.
 *
 * <p>This is an internal class, only used on the NameNode. For identifying or describing a cache
 * pool to clients, please use CachePoolInfo.
 *
 * <p>CachePools must be accessed under the FSNamesystem lock.
 */
@InterfaceAudience.Private
public final class CachePool {
  @Nonnull private final String poolName;
   ...
   *
   * <p>READ permission means that you can list the cache directives in this pool. WRITE permission
   * means that you can add, remove, or modify cache directives in this pool. EXECUTE permission is
   * unused.
   */
  @Nonnull private FsPermission mode;
  ...
  /** Maximum duration that a CacheDirective in this pool remains valid, in milliseconds. */
  ...
  @Nonnull private final DirectiveList directiveList = new DirectiveList(this);

  /**
   * Create a new cache pool based on a CachePoolInfo object and the defaults. We will fill in
   * information that was not supplied according to the defaults.
   */
  ...
  /**
   * Create a new cache pool based on a CachePoolInfo object. No fields in the CachePoolInfo can be
   * blank.
   */
   ...
   *
   * @param fullInfo If true, only the name will be returned (i.e., what you would get if you didn't
   *     have read permission for this pool.)
  ...
  /**
   * Returns a CachePoolInfo describing this CachePool based on the permissions of the calling user.
   * Unprivileged users will see only minimal descriptive information about the pool.
   *
   * @param pc Permission checker to be used to validate the user's permissions, or null
  ...
  public CachePoolEntry getEntry(FSPermissionChecker pc) {
    ...
    return new CachePoolEntry(
        getInfo(hasPermission), hasPermission ? getStats() : new CachePoolStats.Builder().build());
  }
