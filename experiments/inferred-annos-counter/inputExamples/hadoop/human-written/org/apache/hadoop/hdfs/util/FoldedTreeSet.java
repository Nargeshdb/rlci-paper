...
/**
 * A memory efficient implementation of RBTree. Instead of having a Node for each entry each node
 * contains an array holding 64 entries.
 *
 * <p>Based on the Apache Harmony folded TreeMap.
 *
 * @param <E> Entry type
 */
public class FoldedTreeSet<E> implements SortedSet<E> {
  ...
  private static class Node<E> {
    ...
    public void addEntryAt(E entry, int index) {
      ...
      if (leftIndex == 0
          || ((rightIndex != Node.NODE_SIZE - 1) && (rightIndex - index <= index - leftIndex))) {
        rightIndex++;
        System.arraycopy(entries, index, entries, index + 1, rightIndex - index);
        entries[index] = entry;
      } else {
        int newLeftIndex = leftIndex - 1;
        System.arraycopy(entries, leftIndex, entries, newLeftIndex, index - leftIndex);
    ...
    public void addEntriesLeft(Node<E> from) {
      leftIndex -= from.size;
      size += from.size;
      System.arraycopy(from.entries, from.leftIndex, entries, leftIndex, from.size);
    }

    public void addEntriesRight(Node<E> from) {
      System.arraycopy(from.entries, from.leftIndex, entries, rightIndex + 1, from.size);
      size += from.size;
    ...
    private void checkForModification() {
      if (iteratorModCount != tree.modCount) {
        throw new ConcurrentModificationException(
            "Tree has been modified " + "outside of iterator");
      }
    }
  }

  /**
   * Create a new TreeSet that uses the natural ordering of objects. The element type must implement
   * Comparable.
   */
   ...
   * @param cmp User provided Comparator. The comparator should expect that the proved obj will
   *     always be the first method parameter and any stored object will be the second parameter.
  ...
  public Object[] toArray() {
    ...
    if (!isEmpty()) {
      int pos = 0;
      for (Node<E> node = root.getLeftMostNode();
          node != null;
   ...
   * @return the previous entry, or null if this set did not already contain the specified entry
   */
   ...
   * @param replace Should the entry replace an old entry which is equal to the new entry
   * @return null if entry added and didn't exist or the previous value (which might not have been
   *     overwritten depending on the replace parameter)
   */
  ...
  /**
   * Insert an entry last in the sorted tree. The entry must be the considered larger than the
   * currently largest entry in the set when doing current.compareTo(entry), if entry is not the
   * largest entry the method will fall back on the regular add method.
   *
   * @param entry entry to add
  ...
  private E removeElementRight(Node<E> node) {
    ...
    if (node.isEmpty()) {
      ...
      node.next.addEntriesLeft(node);
      deleteNode(node);
    } else if (node.next != null && node.next.size < (Node.NODE_SIZE - 1 - node.rightIndex)) {
  ...
  /**
   * Returns the current size divided by the capacity of the tree. A value between 0.0 and 1.0,
   * where 1.0 means that every allocated node in the tree is completely full.
   *
   * <p>An empty set will return 1.0
   *
   ...
   *
   * <p>Having a compact tree minimize memory usage, but can cause inserts to get slower due to new
   * nodes needs to be allocated as there is no space in any of the existing nodes anymore for
   * entries added in the middle of the set.
   *
   * <p>Useful to do to reduce memory consumption and if the tree is know to not change after
   * compaction or mainly added to at either extreme.
   *
   * @param timeout Maximum time to spend compacting the tree set in milliseconds.
