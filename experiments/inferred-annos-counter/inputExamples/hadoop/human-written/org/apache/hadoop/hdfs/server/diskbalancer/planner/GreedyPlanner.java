...
/**
 * Greedy Planner is a simple planner that computes the largest possible move at any point of time
 * given a volumeSet.
 *
 * <p>This is done by choosing the disks with largest amount of data above and below the
 * idealStorage and then a move is scheduled between them.
 */
public class GreedyPlanner implements Planner {
   ...
   * @param plan - NodePlan
   */
  public void balanceVolumeSet(DiskBalancerDataNode node, DiskBalancerVolumeSet vSet, NodePlan plan)
      throws Exception {
    ...
    while (currentSet.isBalancingNeeded(this.threshold)) {
      ...
      if (!lowVolume.isSkip() && !highVolume.isSkip()) {
        ...
        LOG.debug(
            "Skipping compute move. lowVolume: {} highVolume: {}",
            lowVolume.getPath(),
            highVolume.getPath());
      }
    ...
    LOG.info(
        "Disk Volume set {} - Type : {} plan completed.",
        currentSet.getSetID(),
  ...
  /**
   * Apply steps applies the current step on to a volumeSet so that we can compute next steps until
  ...
  private void applyStep(
      Step nextStep,
      DiskBalancerVolumeSet currentSet,
  ...
  private Step computeMove(
    ...
    if (maxLowVolumeCanReceive <= 0) {
      LOG.debug(
          "{} Skipping disk from computation. Maximum data size " + "achieved.",
          lowVolume.getPath());
    ...
    long maxHighVolumeCanGive =
        highVolume.getUsed()
            - (long) (currentSet.getIdealUsed() * highVolume.computeEffectiveCapacity());
    // This volume cannot give any more data, remove it from the
    // computation matrix
    if (maxHighVolumeCanGive <= 0) {
      LOG.debug(
          " {} Skipping disk from computation. Minimum data size " + "achieved.",
          highVolume.getPath());
    ...
    if (bytesToMove > 0) {
      ...
      nextStep =
          new MoveStep(
              highVolume, currentSet.getIdealUsed(), lowVolume, bytesToMove, currentSet.getSetID());
      ...
      String message =
          String.format(
              "Skipping volume. Volume : %s "
                  ...
                  + "volume from all future balancing calls.",
              volume.getPath(),
  ...
  /**
   * This function is used only for debugging purposes to ensure queue looks correct.
   *
   * @param queue - Queue
   */
  private void printQueue(TreeSet<DiskBalancerVolume> queue) {
    if (LOG.isDebugEnabled()) {
      String format =
          String.format(
              "First Volume : %s, DataDensity : %f, " + "Last Volume : %s, DataDensity : %f",
              queue.first().getPath(),
