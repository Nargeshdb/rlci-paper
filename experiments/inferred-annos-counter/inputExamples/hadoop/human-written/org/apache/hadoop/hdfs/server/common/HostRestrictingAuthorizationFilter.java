...
public class HostRestrictingAuthorizationFilter implements Filter {
  ...
  public static final Predicate<String> RESTRICTED_OPERATIONS =
      qStr ->
          (qStr.trim().equalsIgnoreCase("op=OPEN")
  ...
  private boolean matchRule(String user, String remoteIp, String path) {
    ...
    List<Rule> userRules =
        ((userRules = rulemap.get(user)) != null) ? userRules : new ArrayList<Rule>();
    List<Rule> anyRules =
        ((anyRules = rulemap.get("*")) != null) ? anyRules : new ArrayList<Rule>();

    List<Rule> rules =
        Stream.of(userRules, anyRules).flatMap(l -> l.stream()).collect(Collectors.toList());

    for (Rule rule : rules) {
      SubnetUtils.SubnetInfo subnet = rule.getSubnet();
      String rulePath = rule.getPath();
      LOG.trace(
          "Evaluating rule, subnet: {}, path: {}",
      ...
      try {
        if ((subnet == null || subnet.isInRange(remoteIp))
          ...
          LOG.debug(
              "Found matching rule, subnet: {}, path: {}; returned true",
  ...
  private void loadRuleMap(String ruleString) throws IllegalArgumentException {
    if (ruleString == null || ruleString.equals("")) {
      ...
      Map<Integer, List<String[]>> splits =
          rule_split
      ...
      int cidr = 1;
      int path = 2;
      BiFunction<CopyOnWriteArrayList<Rule>, CopyOnWriteArrayList<Rule>, CopyOnWriteArrayList<Rule>>
          arrayListMerge =
              (v1, v2) -> {
                v1.addAll(v2);
                return v1;
      ...
      for (String[] split : splits.get(3)) {
        LOG.debug(
            "Loaded rule: user: {}, network/bits: {} path: {}",
            split[user],
            split[cidr],
            split[path]);
        Rule rule =
            (split[cidr].trim().equals("*")
                ? new Rule(null, split[path])
                : new Rule(new SubnetUtils(split[cidr]).getInfo(), split[path]));
  ...
  public void handleInteraction(HttpInteraction interaction) throws IOException, ServletException {
    ...
    final String path =
        interaction.getRequestURI().substring(WebHdfsFileSystem.PATH_PREFIX.length());
    String user = interaction.getRemoteUser();

    LOG.trace(
        "Got request user: {}, remoteIp: {}, query: {}, path: {}", user, address, query, path);
    boolean authenticatedQuery =
        Arrays.stream(Optional.ofNullable(query).orElse("").trim().split("&"))
    ...
    if (!interaction.isCommitted() && authenticatedQuery) {
      ...
      if (user == null) {
        ...
        for (String part : queryParts) {
          if (part.trim().startsWith("delegation=")) {
            ...
            LOG.trace(
                "Updated request user: {}, remoteIp: {}, query: {}, " + "path: {}",
                user,
                address,
                query,
                path);
      ...
      if (authenticatedQuery
          && !(matchRule("*", address, path) || matchRule(user, address, path))) {
        LOG.trace("Rejecting interaction; no rule found");
        interaction.sendError(
