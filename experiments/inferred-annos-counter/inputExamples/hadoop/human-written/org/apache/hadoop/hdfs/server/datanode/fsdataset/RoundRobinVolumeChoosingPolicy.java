...
/**
 * Choose volumes with the same storage type in round-robin order. Use fine-grained locks to
 * synchronize volume choosing.
 */
public class RoundRobinVolumeChoosingPolicy<V extends FsVolumeSpi>
  ...
  private V chooseVolume(final int curVolumeIndex, final List<V> volumes, long blockSize)
      throws IOException {
    ...
    while (true) {
      ...
      if (curVolume == startVolume) {
        throw new DiskOutOfSpaceException(
            "Out of space: "
                + "The volume with the most available space (="
                + maxAvailable
                + " B) is less than the block size (="
                + blockSize
                + " B).");
      } else {
        LOG.warn(
            "The volume[{}] with the available space (={} B) is "
                + "less than the block size (={} B).",
            volume.getBaseURI(),
            availableVolumeSize,
            blockSize);
      }
