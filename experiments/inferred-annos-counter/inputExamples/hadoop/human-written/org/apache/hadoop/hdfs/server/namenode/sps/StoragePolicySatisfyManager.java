...
/**
 * This manages satisfy storage policy invoked path ids and expose methods to process these path
 * ids. It maintains sps mode(EXTERNAL/NONE) configured by the administrator.
 *
 * <p>If the configured mode is {@link StoragePolicySatisfierMode#EXTERNAL}, then it won't do
 * anything, just maintains the sps invoked path ids. Administrator requires to start external sps
 * service explicitly, to fetch the sps invoked path ids from namenode, then do necessary
 * computations and block movement in order to satisfy the storage policy. Please refer {@link
 * ExternalStoragePolicySatisfier} class to understand more about the external sps service
 * functionality.
 *
 * <p>If the configured mode is {@link StoragePolicySatisfierMode#NONE}, then it will disable the
 * sps feature completely by clearing all queued up sps path's hint.
 *
 * <p>This class is instantiated by the BlockManager.
 */
public class StoragePolicySatisfyManager {
   ...
   *
   * <p>If the configured mode is {@link StoragePolicySatisfierMode#EXTERNAL}, then it won't do
   * anything. Administrator requires to start external sps service explicitly.
   *
   * <p>If the configured mode is {@link StoragePolicySatisfierMode#NONE}, then the service is
   * disabled and won't do any action.
   */
  public void start() {
    if (!storagePolicyEnabled) {
      LOG.info(
          "Disabling StoragePolicySatisfier service as {} set to {}.",
          DFSConfigKeys.DFS_STORAGE_POLICY_ENABLED_KEY,
          storagePolicyEnabled);
      return;
    }
    ...
    switch (mode) {
      case EXTERNAL:
        LOG.info(
            "Storage policy satisfier is configured as external, "
   ...
   *
   * <p>If the configured mode is {@link StoragePolicySatisfierMode#EXTERNAL}, then it won't do
   * anything. Administrator requires to stop external sps service explicitly, if needed.
   *
   * <p>If the configured mode is {@link StoragePolicySatisfierMode#NONE}, then the service is
   * disabled and won't do any action.
   */
  ...
  /**
   * Sets new sps mode. If the new mode is none, then it will disable the sps feature completely by
   * clearing all queued up sps path's hint.
   */
  public void changeModeEvent(StoragePolicySatisfierMode newMode) {
    if (!storagePolicyEnabled) {
      LOG.info(
          "Failed to change storage policy satisfier as {} set to {}.",
          DFSConfigKeys.DFS_STORAGE_POLICY_ENABLED_KEY,
          storagePolicyEnabled);
      return;
    }
    ...
    switch (newMode) {
      case EXTERNAL:
        if (mode == newMode) {
          LOG.info(
              "Storage policy satisfier is already in mode:{}," + " so ignoring change mode event.",
              newMode);
          return;
        }
      ...
      case NONE:
        if (mode == newMode) {
          LOG.info(
              "Storage policy satisfier is already disabled, mode:{}"
                  + " so ignoring change mode event.",
              newMode);
          return;
        }
  ...
  /**
   * @return true if the internal storage policy satisfier daemon is running, false otherwise.
   */
  @VisibleForTesting
  ...
  /**
   * @return the next SPS path id, on which path users has invoked to satisfy storages.
   */
  ...
  public void verifyOutstandingPathQLimit() throws IOException {
    ...
    if (outstandingPathsLimit - size <= 0) {
      ...
      throw new IOException(
          "Outstanding satisfier queue limit: " + outstandingPathsLimit + " exceeded, try later!");
    }
  }
  ...
  /**
   * @return true if sps is configured as an external service, false otherwise.
   */
