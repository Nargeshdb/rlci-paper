...
public class BlockStorageMovementTracker implements Runnable {
   ...
   *
   * @param moverCompletionService completion service.
   * @param handler blocks movements status handler
   */
  ...
  public void run() {
    while (running) {
      try {
        Future<BlockMovementAttemptFinished> future = moverCompletionService.take();
        if (future != null) {
