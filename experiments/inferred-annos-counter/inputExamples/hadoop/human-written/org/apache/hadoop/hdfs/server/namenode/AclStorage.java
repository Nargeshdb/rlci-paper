 ...
 *
 * <p>If an inode has an ACL, then the ACL bit is set in the inode's {@link FsPermission} and the
 * inode also contains an {@link AclFeature}. For the access ACL, the owner and other entries are
 * identical to the owner and other bits stored in FsPermission, so we reuse those. The access mask
 * entry is stored into the group permission bits of FsPermission. This is consistent with other
 * file systems' implementations of ACLs and eliminates the need for special handling in various
 * parts of the codebase. For example, if a user calls chmod to change group permission bits on a
 * file with an ACL, then the expected behavior is to change the ACL's mask entry. By saving the
 * mask entry into the group permission bits, chmod continues to work correctly without special
 * handling. All remaining access entries (named users and named groups) are stored as explicit
 * {@link AclEntry} instances in a list inside the AclFeature. Additionally, all default entries are
 * stored in the AclFeature.
 *
 * <p>The methods in this class encapsulate these rules for reading or writing the ACL entries to
 * the appropriate location.
 *
 * <p>The methods in this class assume that input ACL entry lists have already been validated and
 * sorted according to the rules enforced by {@link AclTransformation}.
 */
@InterfaceAudience.Private
public final class AclStorage {

  private static final ReferenceCountMap<AclFeature> UNIQUE_ACL_FEATURES =
      new ReferenceCountMap<AclFeature>();

  /**
   * If a default ACL is defined on a parent directory, then copies that default ACL to a newly
   * created child file or directory.
   *
  ...
  public static boolean copyINodeDefaultAcl(INode child) {
    ...
    for (AclEntry entry : parentDefaultEntries) {
      ...
      AclEntry.Builder builder =
          new AclEntry.Builder().setScope(AclEntryScope.ACCESS).setType(type).setName(name);
    ...
    List<AclEntry> defaultEntries =
        child.isDirectory() ? parentDefaultEntries : Collections.<AclEntry>emptyList();
  ...
  /**
   * Reads the existing extended ACL entries of an inode. This method returns only the extended ACL
   * entries stored in the AclFeature. If the inode does not have an ACL, then this method returns
   * an empty list. This method supports querying by snapshot ID.
   *
  ...
  /**
   * Reads the existing ACL of an inode. This method always returns the full logical ACL of the
   * inode after reading relevant data from the inode's {@link FsPermission} and {@link AclFeature}.
   * Note that every inode logically has an ACL, even if no ACL has been set explicitly. If the
   * inode does not have an extended ACL, then the result is a minimal ACL consising of exactly 3
   * entries that correspond to the owner, group and other permissions. This method always reads the
   * inode's current state and does not support querying by snapshot ID. This is because the method
   * is intended to support ACL modification APIs, which always apply a delta on top of current
   * state.
   *
   * @param inode INode to read
  ...
  /**
   * Updates an inode with a new ACL. This method takes a full logical ACL and stores the entries to
   * the inode's {@link FsPermission} and {@link AclFeature}.
   *
   ...
   */
  public static void updateINodeAcl(INode inode, List<AclEntry> newAcl, int snapshotId)
   ...
   */
  private static AclFeature createAclFeature(
      List<AclEntry> accessEntries, List<AclEntry> defaultEntries) {
    // Pre-allocate list size for the explicit entries stored in the feature,
    // which is all entries minus the 3 entries implicitly stored in the
    // permission bits.
    List<AclEntry> featureEntries =
        Lists.newArrayListWithCapacity((accessEntries.size() - 3) + defaultEntries.size());
  ...
  /**
   * Creates the new FsPermission for an inode that is receiving an extended ACL, based on its
   * access ACL entries. For a correctly sorted ACL, the first entry is the owner and the last 2
   * entries are the mask and other entries respectively. Also preserve sticky bit and toggle ACL
   * bit on. Note that this method intentionally copies the permissions of the mask entry into the
   * FsPermission group permissions. This is consistent with the POSIX ACLs model, which presents
   * the mask as the permissions of the group class.
   *
   * @param accessEntries {@code List<AclEntry>} access ACL entries
  ...
  private static FsPermission createFsPermissionForExtendedAcl(
    ...
    return new FsPermission(
        accessEntries.get(0).getPermission(),
  ...
  /**
   * Creates the new FsPermission for an inode that is receiving a minimal ACL, based on its access
   * ACL entries. For a correctly sorted ACL, the owner, group and other permissions are in order.
   * Also preserve sticky bit and toggle ACL bit off.
   *
   * @param accessEntries {@code List<AclEntry>} access ACL entries
   * @param existingPerm FsPermission existing permissions
   * @return FsPermission new permissions
