...
public class VolumeScanner extends Thread {
  ...
  /**
   * The number of scanned bytes in each minute of the last hour.
   *
   * <p>This array is managed as a circular buffer. We take the monotonic time and divide it up into
   * one-minute periods. Each entry in the array represents how many bytes were scanned during that
   * period.
   */
  ...
  /**
   * The block iterators associated with this VolumeScanner.
   *
   * <p>Each block pool has its own BlockIterator.
   */
  private final List<BlockIterator> blockIters = new ArrayList<BlockIterator>();

  /** Blocks which are suspect. The scanner prioritizes scanning these blocks. */
  private final LinkedHashSet<ExtendedBlock> suspectBlocks = new LinkedHashSet<ExtendedBlock>();

  /**
   * Blocks which were suspect which we have scanned. This is used to avoid scanning the same
   * suspect block over and over.
   */
   ...
   *
   * <p>Protected by this object's lock.
   */
  ...
  static class Statistics {
    ...
    public String toString() {
      return new StringBuilder()
          ...
          .append(blocksScannedSinceRestart)
          .append(", scansSinceRestart=")
          .append(scansSinceRestart)
          .append(", scanErrorsSinceRestart=")
          .append(scanErrorsSinceRestart)
          .append(", nextBlockPoolScanStartMs=")
          .append(nextBlockPoolScanStartMs)
          .append(", blockPoolPeriodEndsMs=")
          .append(blockPoolPeriodEndsMs)
  ...
  public void printStats(StringBuilder p) {
    p.append(
        String.format(
            "Block scanner information for volume %s with base" + " path %s%n",
            volume.getStorageID(), volume));
    synchronized (stats) {
      p.append(
              String.format(
                  "Bytes verified in last hour       : %57d%n", stats.bytesScannedInPastHour))
          .append(
              String.format(
                  "Blocks scanned in current period  : %57d%n", stats.blocksScannedInCurrentPeriod))
          .append(
              String.format(
                  "Blocks scanned since restart      : %57d%n", stats.blocksScannedSinceRestart))
          .append(
              String.format("Block pool scans since restart    : %57d%n", stats.scansSinceRestart))
          .append(
              String.format(
                  "Block scan errors since restart   : %57d%n", stats.scanErrorsSinceRestart));
      if (stats.nextBlockPoolScanStartMs > 0) {
        p.append(
            String.format(
                "Hours until next block pool scan  : %57.3f%n",
                positiveMsToHours(stats.nextBlockPoolScanStartMs - Time.monotonicNow())));
      }
      if (stats.blockPoolPeriodEndsMs > 0) {
        p.append(
            String.format(
                "Hours until possible pool rescan  : %57.3f%n",
                positiveMsToHours(stats.blockPoolPeriodEndsMs - Time.now())));
      }
      p.append(
          String.format(
              "Last block scanned                : %57s%n",
              ((stats.lastBlockScanned == null) ? "none" : stats.lastBlockScanned.toString())));
  ...
  static class ScanResultHandler {
    ...
    public void setup(VolumeScanner scanner) {
      LOG.trace("Starting VolumeScanner {}", scanner.volume);
    ...
    public void handle(ExtendedBlock block, IOException e) {
      ...
      if (!volume.getDataset().contains(block)) {
        LOG.debug("Volume {}: block {} is no longer in the dataset.", volume, block);
        return;
      }
      ...
      if (e instanceof FileNotFoundException) {
        LOG.info(
            "Volume {}: verification failed for {} because of "
  ...
  private void expireOldScannedBytesRecords(long monotonicMs) {
    ...
    for (long m = curMinute + 1; m <= newMinute; m++) {
      ...
      LOG.trace(
          "{}: updateScannedBytes is zeroing out slotIdx {}.  " + "curMinute = {}; newMinute = {}",
          this,
          slotIdx,
          curMinute,
          newMinute);
   ...
   *
   * <p>We will consider available block iterators in order. This property is important so that we
   * don't keep rescanning the same block pool id over and over, while other block pools stay
   * unscanned.
   *
   * <p>A block pool is always ready to scan if the iterator is not at EOF. If the iterator is at
   * EOF, the block pool will be ready to scan when conf.scanPeriodMs milliseconds have elapsed
   * since the iterator was last rewound.
   *
   * <p>
   *
   * @return 0 if we found a usable block iterator; the length of time we should delay before
   ...
   * @return The length of the block that was scanned, or -1 if the block could not be scanned.
   */
  private long scanBlock(ExtendedBlock cblock, long bytesPerSec) {
    ...
    try {
      blockSender =
          new BlockSender(
              block, 0, -1, false, true, true, datanode, null, CachingStrategy.newDropBehind());
  ...
  @VisibleForTesting
  static boolean calculateShouldScan(
      String storageId,
   ...
   *
   * @param suspectBlock A suspect block which we should scan, or null to scan the next regularly
   *     scheduled block.
   * @return The number of milliseconds to delay before running the loop again, or 0 to re-run the
   *     loop immediately.
   */
  private long runLoop(ExtendedBlock suspectBlock) {
    ...
    try {
      ...
      if (!calculateShouldScan(
          volume.getStorageID(), conf.targetBytesPerSec, scannedBytesSum, startMinute, curMinute)) {
      ...
      if (suspectBlock != null) {
        ...
        if ((curBlockIter == null) || curBlockIter.atEnd()) {
          ...
          if (timeout > 0) {
            LOG.trace(
                "{}: no block pools are ready to scan yet.  Waiting " + "{} ms.", this, timeout);
            synchronized (stats) {
  ...
  /**
   * If there are elements in the suspectBlocks list, removes and returns the first one. Otherwise,
   * returns null.
   */
  ...
  @Override
  public String toString() {
    return "VolumeScanner(" + volume + ", " + volume.getStorageID() + ")";
  }
    ...
    if (stopping) {
      LOG.debug(
          "{}: Not scheduling suspect block {} for "
              + "rescanning, because this volume scanner is stopping.",
          this,
          block);
      return;
    }
