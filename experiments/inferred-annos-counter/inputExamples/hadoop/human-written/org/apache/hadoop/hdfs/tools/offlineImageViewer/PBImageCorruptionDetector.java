...
/**
 * The PBImageCorruptionDetector detects corruptions in the image. It produces a file with the found
 * issues similar to the Delimited processor. The default delimiter is tab, as this is an unlikely
 * value to be included in an inode path. The delimiter value can be changed via the constructor.
 *
 * <p>It looks for the following kinds of corruptions: - an INode id is mentioned in the
 * INodeDirectorySection, but not present in the INodeSection (corrupt INode case) - an INode has
 * children, but at least one of them is corrupted (missing children case) If multiple layers of
 * directory structure are damaged then it is possible that an INode is corrupted and also having
 * corrupted children.
 *
 * <p>Note that the OIV DetectCorruption processor check is not exhaustive, and only catches the
 * corruptions like above. This processor may be up to extension in the future when new aspects of
...
public class PBImageCorruptionDetector extends PBImageTextWriter {
  ...
  /**
   * Builder object for producing entries (lines) for PBImageCorruptionDetector. The isSnapshot
   * field is mandatory.
   */
  ...
  public String getEntry(String parentPath, FsImageProto.INodeSection.INode inode) {
    ...
    if (corruptionsMap.containsKey(id)) {
      ...
      entryBuilder
          .setCorruption(corruptionsMap.get(id))
  ...
  @Override
  protected void checkNode(FsImageProto.INodeSection.INode p, AtomicInteger numDirs)
      throws IOException {
  ...
  protected void buildNamespace(InputStream in, List<Long> refIdList) throws IOException {
    ...
    while (true) {
      ...
      if (!corrChecker.isNodeIdExist(parentId)) {
        LOG.debug(
            "Corruption detected! Parent node is not contained " + "in the list of known ids!");
      ...
      if (numOfCorruption > 0) {
        if (LOG.isDebugEnabled()) {
          LOG.debug("{} corruption detected! Child nodes are missing.", numOfCorruption);
        }
