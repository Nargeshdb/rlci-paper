 ...
 *
 * <p>This class includes operations that create, access, modify snapshots and/or snapshot-related
 * data. In general, the locking structure of snapshot operations is: <br>
 * 1. Lock the {@link FSNamesystem} lock in {@link FSNamesystem} before calling into {@link
 * SnapshotManager} methods.<br>
...
public class SnapshotManager implements SnapshotStatsMXBean {
  ...
  /**
   * If skipCaptureAccessTimeOnlyChange is set to true, if accessTime of a file changed but there is
   * no other modification made to the file, it will not be captured in next snapshot. However, if
   * there is other modification made to the file, the last access time will be captured together
   * with the modification in next snapshot.
   */
  private boolean skipCaptureAccessTimeOnlyChange = false;
  /**
   * If snapshotDiffAllowSnapRootDescendant is set to true, snapshot diff operation can be run for
   * any descendant directory under a snapshot root directory and the diff calculation will be
   * scoped to the descendant directory.
   */
  ...
  public SnapshotManager(final Configuration conf, final FSDirectory fsdir) {
    ...
    this.skipCaptureAccessTimeOnlyChange =
        conf.getBoolean(
    ...
    this.snapshotDiffAllowSnapRootDescendant =
        conf.getBoolean(
            ...
            DFSConfigKeys.DFS_NAMENODE_SNAPSHOT_DIFF_ALLOW_SNAP_ROOT_DESCENDANT_DEFAULT);
    this.maxSnapshotLimit =
        conf.getInt(
            DFSConfigKeys.DFS_NAMENODE_SNAPSHOT_MAX_LIMIT,
            DFSConfigKeys.DFS_NAMENODE_SNAPSHOT_MAX_LIMIT_DEFAULT);
    LOG.info(
        "Loaded config captureOpenFiles: "
            + captureOpenFiles
  ...
  /**
   * Set the given directory as a snapshottable directory. If the path is already a snapshottable
   * directory, update the quota.
   */
   ...
   * @throws IOException Throw IOException when the given path does not lead to an existing
  ...
  /**
   * Get the snapshot root directory for the given directory. The given directory must either be a
   * snapshot root or a descendant of any snapshot root directories.
   *
   * @param iip INodesInPath for the directory to get snapshot root.
   * @return the snapshot root INodeDirectory
   */
  public INodeDirectory getSnapshottableAncestorDir(final INodesInPath iip) throws IOException {
    final String path = iip.getPath();
    final INodeDirectory dir = INodeDirectory.valueOf(iip.getLastINode(), path);
    if (dir.isSnapshottable()) {
      return dir;
  ...
  /**
   * Create a snapshot of the given path. It is assumed that the caller will perform
   * synchronization.
   *
   ...
   * @throws IOException Throw IOException when 1) the given path does not lead to an existing
   *     snapshottable directory, and/or 2) there exists a snapshot with the given name for the
   *     directory, and/or 3) snapshot number exceeds quota
   */
   ...
   * @param reclaimContext Used to collect information to reclaim blocks and inodes
   */
   ...
   * @throws IOException Throw IOException when 1) the given path does not lead to an existing
   *     snapshottable directory, and/or 2) the snapshot with the old name does not exist for the
   *     directory, and/or 3) there exists a snapshot with the new name for the directory
   */
  public void renameSnapshot(
      final INodesInPath iip,
      final String snapshotRoot,
      final String oldSnapshotName,
      final String newSnapshotName,
      long now)
      throws IOException {
  ...
  /**
   * Read values of {@link #snapshotCounter}, {@link #numSnapshots}, and all snapshots from the
   * DataInput
   */
   ...
   * @return Snapshottable directories that are owned by the current user, represented as an array
   *     of {@link SnapshottableDirectoryStatus}. If {@code userName} is null, return all the
   *     snapshottable dirs.
   */
    ...
    for (INodeDirectory dir : snapshottables.values()) {
      if (userName == null || userName.equals(dir.getUserName())) {
        SnapshottableDirectoryStatus status =
            new SnapshottableDirectoryStatus(
  ...
  /**
   * Compute the difference between two snapshots of a directory, or between a snapshot of the
   * directory and its current tree.
   */
  public SnapshotDiffReport diff(
      final INodesInPath iip, final String snapshotPath, final String from, final String to)
      throws IOException {
    ...
    final SnapshotDiffInfo diffs =
        snapshotRootDir
            .getDirectorySnapshottableFeature()
            .computeDiff(snapshotRootDir, snapshotDescendantDir, from, to);
    return diffs != null
        ? diffs.generateReport()
        : new SnapshotDiffReport(snapshotPath, from, to, Collections.<DiffReportEntry>emptyList());
  }

  /**
   * Compute the partial difference between two snapshots of a directory, or between a snapshot of
   * the directory and its current tree.
   */
  public SnapshotDiffReportListing diff(
      final INodesInPath iip,
  ...
  public static SnapshottableDirectoryStatus.Bean toBean(INodeDirectory d) {
    return new SnapshottableDirectoryStatus.Bean(
        ...
        Short.valueOf(Integer.toOctalString(d.getFsPermissionShort())),
        d.getUserName(),
        d.getGroupName());
  }
  ...
  public static SnapshotInfo.Bean toBean(Snapshot s) {
    return new SnapshotInfo.Bean(
        s.getRoot().getLocalName(),
        s.getRoot().getFullPathName(),
