...
/** DBNameNodeConnector connects to Namenode and extracts information from a given cluster. */
class DBNameNodeConnector implements ClusterConnector {
   ...
   */
  public DBNameNodeConnector(URI clusterURI, Configuration conf)
    ...
    try {
      connector =
          new NameNodeConnector("DiskBalancer", clusterURI, DISKBALANCER_ID_PATH, null, conf, 1);
  ...
  /**
   * This function maps the required fields from DataNodeInfo to disk BalancerDataNode.
   *
   * @param nodeInfo
  ...
  /**
   * Reads the relevant fields from each storage volume and populate the DiskBalancer Node.
   *
   * @param node - Disk Balancer Node
   * @param reports - Array of StorageReport
   */
  private void getVolumeInfoFromStorageReports(DiskBalancerDataNode node, StorageReport[] reports)
      throws Exception {
    ...
    for (StorageReport report : reports) {
      ...
      volume.setSkip(
          (storage.getState() == DatanodeStorage.State.READ_ONLY_SHARED) || report.isFailed());
