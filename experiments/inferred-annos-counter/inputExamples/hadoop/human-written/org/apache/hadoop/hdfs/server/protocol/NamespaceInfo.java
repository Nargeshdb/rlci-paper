...
public class NamespaceInfo extends StorageInfo {
  ...
  public NamespaceInfo(
      ...
      long capabilities,
      HAServiceState st) {
    this(nsID, clusterID, bpID, cT, buildVersion, softwareVersion, capabilities);
    this.state = st;
  }
  ...
  public NamespaceInfo(
      ...
      String softwareVersion,
      long capabilities) {
    super(HdfsServerConstants.NAMENODE_LAYOUT_VERSION, nsID, clusterID, cT, NodeType.NAME_NODE);
    blockPoolID = bpID;
  ...
  public NamespaceInfo(int nsID, String clusterID, String bpID, long cT, HAServiceState st) {
    this(nsID, clusterID, bpID, cT, Storage.getBuildVersion(), VersionInfo.getVersion());
    this.state = st;
  }
  ...
  public boolean isCapabilitySupported(Capability capability) {
    Preconditions.checkArgument(
        capability != Capability.UNKNOWN, "cannot test for unknown capability");
  ...
  public void validateStorage(NNStorage storage) throws IOException {
    if (layoutVersion != storage.getLayoutVersion()
      ...
      throw new IOException(
          "Inconsistent namespace information:\n"
              + "NamespaceInfo has:\n"
              + "LV="
              + layoutVersion
              + ";"
