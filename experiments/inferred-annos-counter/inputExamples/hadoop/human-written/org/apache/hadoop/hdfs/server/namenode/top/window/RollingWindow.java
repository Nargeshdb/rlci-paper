...
/**
 * A class for exposing a rolling window view on the event that occur over time. Events are reported
 * based on occurrence time. The total number of events in the last period covered by the rolling
 * window can be retrieved by the {@link #getSum(long)} method.
 *
 * <p>Assumptions:
 *
 * <p>(1) Concurrent invocation of {@link #incAt} method are possible
 *
 * <p>(2) The time parameter of two consecutive invocation of {@link #incAt} could be in any given
 * order
 *
 * <p>(3) The buffering delays are not more than the window length, i.e., after two consecutive
 * invocation {@link #incAt(long time1, long)} and {@link #incAt(long time2, long)}, time1 &lt;
 * time2 || time1 - time2 &lt; windowLenMs. This assumption helps avoiding unnecessary
 * synchronizations.
 *
 * <p>Thread-safety is built in the {@link RollingWindow.Bucket}
 */
...
public class RollingWindow {
  ...
  /**
   * Each window is composed of buckets, which offer a trade-off between accuracy and space
   * complexity: the lower the number of buckets, the less memory is required by the rolling window
   * but more inaccuracy is possible in reading window total values.
   */
  Bucket[] buckets;
  ...
  /**
   * @param windowLenMs The period that is covered by the window. This period must be more than the
   *     buffering delays.
  ...
  /**
   * Thread-safety is provided by synchronization when resetting the update time as well as atomic
   * fields.
   */
  private class Bucket {
    ...
    /**
     * Check whether the last time that the bucket was updated is no longer covered by rolling
     * window.
     *
    ...
    /**
     * Safely reset the bucket state considering concurrent updates (inc) and resets.
     *
     * @param time the current time
     */
    ...
    /**
     * Increment the bucket. It assumes that staleness check is already performed. We do not need to
     * update the {@link #updateTime} because as long as the {@link #updateTime} belongs to the
     * current view of the rolling window, the algorithm works fine.
     *
     * @param delta
     */
    void inc(long delta) {
   ...
   *
   * <p>If time lags behind the latest update time, the new updates are still included in the sum
   *
   * @param time
