...
/**
 * SkipList is an implementation of a data structure for storing a sorted list of Directory Diff
 * elements, using a hierarchy of linked lists that connect increasingly sparse subsequences(defined
 * by skip interval here) of the diffs. The elements contained in the tree must be mutually
 * comparable.
 *
 * <p>Consider a case where we have 10 snapshots for a directory starting from s0 to s9 each
 * associated with certain change records in terms of inodes deleted and created after a particular
 * snapshot and before the next snapshot. The sequence will look like this:
 *
 * <p>{@literal s0->s1->s2->s3->s4->s5->s6->s7->s8->s9}.
 *
 * <p>Assuming a skip interval of 3, which means a new diff will be added at a level higher than the
 * current level after we have ore than 3 snapshots. Next level promotion happens after 9 snapshots
 * and so on.
 *
 * <p>level 2: {@literal s08------------------------------->s9} level 1: {@literal
 * S02------->s35-------->s68-------->s9} level 0: {@literal s0->s1->s2->s3->s4->s5->s6->s7->s8->s9}
 *
 * <p>s02 will be created by combining diffs for s0, s1, s2 once s3 gets created. Similarly, s08
 * will be created by combining s02, s35 and s68 once s9 gets created.So, for constructing the
 * children list fot s0, we have to combine s08, s9 and reverse apply to the live fs.
 *
 * <p>Similarly, for constructing the children list for s2, s2, s35, s68 and s9 need to get
 * combined(or added) and reverse applied to current fs.
 *
 * <p>This approach will improve the snapshot deletion and snapshot diff calculation.
 *
 * <p>Once a snapshot gets deleted, the list needs to be balanced.
 */
public class DiffListBySkipList implements DiffList<DirectoryDiff> {
  ...
  /**
   * SkipListNode is an implementation of a DirectoryDiff List node, which stores a Directory Diff
   * and references to subsequent nodes.
   */
  static final class SkipListNode implements Comparable<Integer> {
    ...
    /**
     * Constructs a new instance of SkipListNode with the specified data element and level.
     *
    ...
    void setSkipDiff4Target(SkipListNode target, int startLevel, ChildrenDiff childrenDiff) {
    ...
    SkipListNode getSkipNode(int level) {
      return level == 0 ? next : level <= skips.length ? skips[level - 1].getSkipTo() : null;
    }
  ...
  /**
   * The reference to the first node of the list. The list will grow linearly once a new Directory
   * diff gets added. All the list inteface defined methods provide a linear view of the list.
   */
  ...
  /**
   * Adds the specified data element to the beginning of the SkipList, if the element is not already
   * present.
   *
  ...
  private SkipListNode[] findPreviousNodes(SkipListNode node, int nodeLevel) {
    ...
    for (int level = headLevel < nodeLevel ? headLevel : nodeLevel; level >= 0; level--) {
  ...
  /**
   * Adds the specified data element to the end of the SkipList, if the element is not already
   * present.
   *
   * @param diff the element to be inserted
   */
  ...
  private static ChildrenDiff combineDiff(SkipListNode from, SkipListNode to, int level) {
  ...
  /**
   * Returns true if this SkipList contains no data elements. In other words, returns true if the
   * size of this SkipList is zero.
   *
  ...
  /**
   * Iterator is an iterator over the SkipList. This should always provide a linear view of the
   * list.
   */
  @Override
  ...
  /**
   * This function returns the minimal set of diffs required to combine in order to generate all the
   * changes occurred between fromIndex and toIndex.
   *
   ...
   */
  @Override
  public List<DirectoryDiff> getMinListForRange(int fromIndex, int toIndex, INodeDirectory dir) {
