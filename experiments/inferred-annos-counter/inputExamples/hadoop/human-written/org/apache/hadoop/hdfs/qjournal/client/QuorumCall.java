...
class QuorumCall<KEY, RESULT> {
  ...
  /**
   * Interval, in milliseconds, at which a log message will be made while waiting for a quorum call.
   */
  private static final int WAIT_PROGRESS_INTERVAL_MILLIS = 1000;

  /**
   * Start logging messages at INFO level periodically after waiting for this fraction of the
   * configured timeout for any call.
   */
  private static final float WAIT_PROGRESS_INFO_THRESHOLD = 0.3f;
  /**
   * Start logging messages at WARN level after waiting for this fraction of the configured timeout
   * for any call.
   */
  ...
  /**
   * Used in conjunction with {@link #getQuorumTimeoutIncreaseMillis(long, int)} to check for
   * pauses.
   */
  ...
  /**
   * Check for a pause (e.g. GC) since the last time {@link #restartQuorumStopWatch()} was called.
   * If detected, return the length of the pause; else, -1.
   *
   * @param offset Offset the elapsed time by this amount; use if some amount of pause was expected
  ...
  private long getQuorumTimeoutIncreaseMillis(long offset, int millis) {
    ...
    if (pauseTime > (millis * WAIT_PROGRESS_INFO_THRESHOLD)) {
      QuorumJournalManager.LOG.info(
          "Pause detected while waiting for "
   ...
   *
   * <p>Note that, even after this returns, more responses may arrive, causing the return value of
   * other methods in this class to change.
   *
   * @param minResponses return as soon as this many responses have been received, regardless of
   *     whether they are successes or exceptions
   * @param minSuccesses return as soon as this many successful (non-exception) responses have been
   *     received
   * @param maxExceptions return as soon as this many exception responses have been received. Pass 0
   *     to return immediately if any exception is received.
   * @param millis the number of milliseconds to wait for
   * @throws InterruptedException if the thread is interrupted while waiting
   * @throws TimeoutException if the specified timeout elapses before achieving the desired
   *     conditions
   */
  public synchronized void waitFor(
      int minResponses, int minSuccesses, int maxExceptions, int millis, String operationName)
  ...
  /**
   * Check if any of the responses came back with an AssertionError. If so, it re-throws it, even if
   * there was a quorum of responses. This code only runs if assertions are enabled for this class,
   * otherwise it should JIT itself away.
   *
   * <p>This is done since AssertionError indicates programmer confusion rather than some kind of
   * expected issue, and thus in the context of test cases we'd like to actually fail the test case
  ...
  /**
   * @return the total number of calls for which a response has been received, regardless of whether
   *     it threw an exception or returned a successful result.
   */
  ...
  /**
   * @return the number of calls for which a non-exception response has been received.
   */
  ...
  /**
   * @return the number of calls for which an exception response has been received.
   */
  ...
  /**
   * @return the map of successful responses. A copy is made such that this map will not be further
   *     mutated, even if further results arrive for the quorum.
   */
  ...
  /**
   * Return a string suitable for displaying to the user, containing any exceptions that have been
   * received so far.
   */
