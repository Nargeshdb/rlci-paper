...
public class TransferFsImage {
  ...
  @VisibleForTesting static int timeout = 0;
  private static final Logger LOG = LoggerFactory.getLogger(TransferFsImage.class);

  public static void downloadMostRecentImageToDirectory(URL infoServer, File dir)
      throws IOException {
  ...
  public static MD5Hash downloadImageToStorage(
      ...
      throws IOException {
    String fileid =
        ImageServlet.getParamStringForImage(null, imageTxId, dstStorage, isBootstrapStandby);
    ...
    LOG.info(
        "Downloaded file "
            + dstFiles.get(0).getName()
            + " size "
            + dstFiles.get(0).length()
            + " bytes.");
    return hash;
  }
  ...
  static void downloadEditsToStorage(URL fsName, RemoteEditLog log, NNStorage dstStorage)
      throws IOException {
    assert log.getStartTxId() > 0 && log.getEndTxId() > 0 : "bad log: " + log;
    String fileid = ImageServlet.getParamStringForLog(log, dstStorage);
    String finalFileName =
        NNStorage.getFinalizedEditsFileName(log.getStartTxId(), log.getEndTxId());
      ...
      if (f.exists() && FileUtil.canRead(f)) {
        LOG.info(
            "Skipping download of remote edit log "
                + log
                + " since it already is stored locally at "
                + f);
        return;
    ...
    String tmpFileName =
        NNStorage.getTemporaryEditsFileName(log.getStartTxId(), log.getEndTxId(), milliTime);
    ...
    LOG.info(
        "Downloaded file "
            + tmpFiles.get(0).getName()
            + " size "
            + finalFiles.get(0).length()
            + " bytes.");
    ...
    for (StorageDirectory sd : dstStorage.dirIterable(NameNodeDirType.EDITS)) {
      File tmpFile =
          NNStorage.getTemporaryEditsFile(sd, log.getStartTxId(), log.getEndTxId(), milliTime);
      File finalizedFile =
          NNStorage.getFinalizedEditsFile(sd, log.getStartTxId(), log.getEndTxId());
   ...
   * @throws IOException
   */
  public static void downloadAliasMap(URL fsName, File aliasMap, boolean isBootstrapStandby)
      throws IOException {
   ...
   */
  static TransferResult uploadImageFromStorage(
      URL fsName, Configuration conf, NNStorage storage, NameNodeFile nnf, long txid)
      throws IOException {
    return uploadImageFromStorage(fsName, conf, storage, nnf, txid, null);
  }

  /**
   * Requests that the NameNode download an image from this node. Allows for optional external
   * cancelation.
   *
   * @param fsName the http address for the remote NN
   * @param conf Configuration
   ...
   */
  public static TransferResult uploadImageFromStorage(
      URL fsName,
      Configuration conf,
      NNStorage storage,
      NameNodeFile nnf,
  ...
  @SuppressWarnings(
      "builder:required.method.not.called") // TP: possible exceptional exit due to new
                                            // FileInputStream(imageFile) (validated)
  private static void writeFileToPutRequest(
      Configuration conf, HttpURLConnection connection, File imageFile, Canceler canceler)
      throws IOException {
  ...
  /**
   * A server-side method to respond to a getfile http request Copies the contents of the local file
   * into the output stream.
   */
  public static void copyFileToStream(
      @Owning OutputStream out,
      File localfile,
  ...
  @SuppressWarnings(
      "builder:required.method.not.called") // FP nullness reasoning: out is closed in the finally
                                            // block with a null check (validated)
  private static void copyFileToStream(
    ...
    try {
      ...
      reportStr +=
          " Sent total: "
              + total
              + " bytes. Size of last segment intended to send: "
              + num
              + " bytes.";
      if (ioe != null) {
  ...
  /**
   * Client-side Method to fetch file from a server Copies the response from the URL to a list of
   * local files.
   *
   * @param dstStorage if an error occurs writing to one of the files, this storage object will be
   *     notified.
  ...
  public static MD5Hash doGetUrl(
      URL url, List<File> localPaths, Storage dstStorage, boolean getChecksum) throws IOException {
