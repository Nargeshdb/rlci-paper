...
public class SecondaryNameNode implements Runnable, SecondaryNameNodeInfoMXBean {
  ...
  public static final Logger LOG = LoggerFactory.getLogger(SecondaryNameNode.class.getName());
  ...
  public String toString() {
    return getClass().getSimpleName()
        ...
        + new Date(starttime)
        + "\nLast Checkpoint        : "
        + (lastCheckpointTime == 0 ? "--" : new Date(lastCheckpointWallclockTime))
        + " ("
  ...
  private void initialize(final Configuration conf, CommandLineOpts commandLineOpts)
      throws IOException {
    ...
    LOG.info(
        "Checkpoint Period   :"
            ...
            + " secs "
            + "("
            + checkpointConf.getPeriod() / 60
            + " min)");
    LOG.info("Log Size Trigger    :" + checkpointConf.getTxnCount() + " txns");
  }

  /** Wait for the service to finish. (Normally, it runs forever.) */
  private void join() {
    try {
      infoServer.join();
    ...
    while (shouldRun) {
      ...
      try {
        ...
        if (checkpointImage.getMergeErrorCount() > maxRetries) {
          LOG.error("Merging failed " + checkpointImage.getMergeErrorCount() + " times.");
          terminate(1);
        }
      } catch (Throwable e) {
  ...
  static boolean downloadCheckpointFiles(
    ...
    if (manifest.getLogs().isEmpty()) {
      throw new IOException(
          "Found no edit logs to download on NN since txid " + sig.mostRecentCheckpointTxId);
    }

    long expectedTxId = sig.mostRecentCheckpointTxId + 1;
    if (manifest.getLogs().get(0).getStartTxId() != expectedTxId) {
      throw new IOException(
          "Bad edit log manifest (expected txid = " + expectedTxId + ": " + manifest);
    }
                    ...
                    public Boolean run() throws Exception {
                      ...
                      if (sig.mostRecentCheckpointTxId
                        ...
                        dstImage.saveDigestAndRenameCheckpointImage(
                            NameNodeFile.IMAGE, sig.mostRecentCheckpointTxId, downloadedHash);
                      }
  ...
  public void startInfoServer() throws IOException {
    ...
    final boolean xFrameEnabled =
        conf.getBoolean(
            DFSConfigKeys.DFS_XFRAME_OPTION_ENABLED,
            DFSConfigKeys.DFS_XFRAME_OPTION_ENABLED_DEFAULT);

    final String xFrameOptionValue =
        conf.getTrimmed(
            DFSConfigKeys.DFS_XFRAME_OPTION_VALUE, DFSConfigKeys.DFS_XFRAME_OPTION_VALUE_DEFAULT);
    ...
    infoServer.addInternalServlet(
        "imagetransfer", ImageServlet.PATH_SPEC, ImageServlet.class, true);
    infoServer.start();
  ...
  public boolean doCheckpoint() throws IOException {
    ...
    if (isFreshCheckpointer
        || (isSameCluster && !sig.storageVersionMatches(checkpointImage.getStorage()))) {
    ...
    loadImage |=
        downloadCheckpointFiles(fsName, checkpointImage, sig, manifest)
  ...
  static class CommandLineOpts {
    ...
    CommandLineOpts() {
      geteditsizeOpt =
          new Option(
              "geteditsize", "return the number of uncheckpointed transactions on the NameNode");
      checkpointOpt =
          OptionBuilder.withArgName("force")
              .hasOptionalArg()
              .withDescription("checkpoint on startup")
              .create("checkpoint");
      ;
    ...
    public void parse(String... argv) throws ParseException {
      ...
      if (hasGetEdit && hasCheckpoint) {
        throw new ParseException(
            "May not pass both " + geteditsizeOpt.getOpt() + " and " + checkpointOpt.getOpt());
      }
    ...
    void usage() {
      String header =
          "The Secondary NameNode is a helper "
  ...
  static class CheckpointStorage extends FSImage {
    ...
    private static class CheckpointLogPurger implements LogsPurgeable {
      ...
      public void selectInputStreams(
        ...
        while (iter.hasNext()) {
          ...
          FileJournalManager.addStreamsToCollectionFromFiles(
     ...
     */
    CheckpointStorage(Configuration conf, Collection<URI> imageDirs, List<URI> editsDirs)
        throws IOException {
      ...
      // 2NN's edits storage.
      this.archivalManager =
          new NNStorageRetentionManager(conf, storage, new CheckpointLogPurger(storage));
    }

    /**
     * Analyze checkpoint directories. Create directories if they do not exist. Recover from an
     * unsuccessful checkpoint if necessary.
     *
     * @throws IOException
     */
    ...
    /** Ensure that the current/ directory exists in all storage directories */
  ...
  void doMerge(
    ...
    if (loadImage) {
      ...
      if (file == null) {
        throw new IOException(
            "Couldn't find image file at txid "
                + sig.mostRecentCheckpointTxId
