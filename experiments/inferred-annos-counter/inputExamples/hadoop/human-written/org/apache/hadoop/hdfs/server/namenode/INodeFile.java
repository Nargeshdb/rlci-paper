...
public class INodeFile extends INodeWithAdditionalFields
  ...
  /**
   * Bit format: [4-bit storagePolicyID][12-bit BLOCK_LAYOUT_AND_REDUNDANCY] [48-bit
   * preferredBlockSize]
   *
   * <p>BLOCK_LAYOUT_AND_REDUNDANCY contains 12 bits and describes the layout and redundancy of a
   * block. We use the highest 1 bit to determine whether the block is replica or erasure coded. For
   * replica blocks, the tail 11 bits stores the replication factor. For erasure coded blocks, the
   * tail 11 bits stores the EC policy ID, and in the future, we may further divide these 11 bits to
   * store both the EC policy ID and replication factor for erasure coded blocks. The layout of this
   * section is demonstrated as below.
   *
   * <p>Another possible future extension is for future block types, in which case the 'Replica or
   * EC' bit may be extended into the 11 bit field.
   *
   * <p>+---------------+-------------------------------+ | 1 bit | 11 bit |
   * +---------------+-------------------------------+ | Replica or EC |Replica factor or EC policy
   * ID | +---------------+-------------------------------+
   *
   * <p>BLOCK_LAYOUT_AND_REDUNDANCY format for replicated block: 0 [11-bit replication]
   *
   * <p>BLOCK_LAYOUT_AND_REDUNDANCY format for striped block: 1 [11-bit ErasureCodingPolicy ID]
   */
  enum HeaderFormat {
    PREFERRED_BLOCK_SIZE(null, 48, 1),
    BLOCK_LAYOUT_AND_REDUNDANCY(PREFERRED_BLOCK_SIZE.BITS, HeaderFormat.LAYOUT_BIT_WIDTH + 11, 0),
    STORAGE_POLICY_ID(BLOCK_LAYOUT_AND_REDUNDANCY.BITS, BlockStoragePolicySuite.ID_BIT_LENGTH, 0);

    private final LongBitFormat BITS;

    /** Number of bits used to encode block layout type. Different types can be replica or EC */
    ...
    /**
     * Construct block layout redundancy based on the given BlockType, replication factor and EC
     * PolicyID.
     */
    static long getBlockLayoutRedundancy(
        BlockType blockType, Short replication, Byte erasureCodingPolicyID) {
      ...
      switch (blockType) {
        case STRIPED:
          ...
          if (null == ErasureCodingPolicyManager.getInstance().getByID(erasureCodingPolicyID)) {
            throw new IllegalArgumentException(
                String.format("Could not find EC policy with ID 0x%02x", erasureCodingPolicyID));
          }
        ...
        case CONTIGUOUS:
          if (erasureCodingPolicyID != REPLICATION_POLICY_ID) {
            throw new IllegalArgumentException(
                String.format(
                    "Illegal EC policy 0x%02x for CONTIGUOUS block type", erasureCodingPolicyID));
          }
          if (null == replication || replication < 0 || replication > MAX_REDUNDANCY) {
            throw new IllegalArgumentException("Invalid replication value " + replication);
          }
  ...
  INodeFile(
    ...
    final long layoutRedundancy =
        HeaderFormat.getBlockLayoutRedundancy(blockType, replication, ecPolicyID);
  ...
  /**
   * If the inode contains a {@link FileUnderConstructionFeature}, return it; otherwise, return
   * null.
   */
  ...
  /** Convert the file to a complete file, i.e., to remove the Under-Construction feature. */
  ...
  private void assertAllBlocksComplete(int numCommittedAllowed, short minReplication) {
    for (int i = 0; i < blocks.length; i++) {
      final String err = checkBlockComplete(blocks, i, numCommittedAllowed, minReplication);
      if (err != null) {
  ...
  /**
   * Check if the i-th block is COMPLETE; when the i-th block is the last block, it may be allowed
   * to be COMMITTED.
   *
   * @return null if the block passes the check; otherwise, return an error message.
   */
  static String checkBlockComplete(
      BlockInfo[] blocks, int i, int numCommittedAllowed, short minReplication) {
    ...
    if (numExpectedLocations <= minReplication) {
      return b
          + " is "
          + state
          + " but numExpectedLocations = "
          + numExpectedLocations
          + " <= minReplication = "
          + minReplication;
    }
    return null;
  ...
  public void convertLastBlockToUC(BlockInfo lastBlock, DatanodeStorageInfo[] locations)
      throws IOException {
  ...
  /** Remove a block from the block list. This block should be the last one on the list. */
  ...
  /**
   * If feature list contains a {@link FileWithSnapshotFeature}, return it; otherwise, return null.
   */
  ...
  public short getPreferredBlockReplication() {
    ...
    Preconditions.checkNotNull(
        ecPolicy,
        "Could not find EC policy with ID 0x"
  ...
  /** Set the replication factor of this file. */
  public final INodeFile setFileReplication(short replication, int latestSnapshotId)
  ...
  public byte getStoragePolicyID() {
    ...
    if (isStriped()
        && id != BLOCK_STORAGE_POLICY_ID_UNSPECIFIED
        && !ErasureCodingPolicyManager.checkStoragePolicySuitableForECStripedMode(id)) {
      id = HdfsConstants.BLOCK_STORAGE_POLICY_ID_UNSPECIFIED;
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            "The current effective storage policy id : "
                + id
                + " is not suitable for striped mode EC file : "
                + getName()
  ...
  public final void setStoragePolicyID(byte storagePolicyId, int latestSnapshotId)
      throws QuotaExceededException {
    recordModification(latestSnapshotId);
  ...
  private void updateRemovedUnderConstructionFiles(ReclaimContext reclaimContext) {
  ...
  public final QuotaCounts computeQuotaUsage(
    ...
    final BlockStoragePolicy bsp =
        (blockStoragePolicyId == BLOCK_STORAGE_POLICY_ID_UNSPECIFIED)
            ? null
  ...
  /**
   * Compute quota of striped file. Note that currently EC files do not support append/hflush/hsync,
   * thus the file length recorded in snapshots should be the same with the current file length.
   */
  ...
  private long getDiskSpaceQuota(
      ContentCounts counts, FileWithSnapshotFeature sf, int lastSnapshotId) {
    FileDiffList fileDiffList = sf.getDiffs();
    int last = fileDiffList.getLastSnapshotId();
  ...
  /**
   * Compute file size of the current file if the given snapshot is null; otherwise, get the file
   * size from the given snapshot.
   */
  ...
  /**
   * Compute file size of the current file size but not including the last block if it is under
   * construction.
   */
   ...
   * @param usePreferredBlockSize4LastUcBlock If the last block is under construction, should we use
   *     actual block size or preferred block size? Note that usePreferredBlockSize4LastUcBlock is
   *     ignored if includesLastUcBlock == false.
   * @return file size
   */
  public final long computeFileSize(
    ...
    if (!lastBlk.isComplete()) {
      if (!includesLastUcBlock) {
        ...
        size =
            isStriped()
                ? getPreferredBlockSize() * ((BlockInfoStriped) lastBlk).getDataBlockNum()
  ...
  /**
   * Compute size consumed by all blocks of the current file, including blocks in its snapshots. Use
   * preferred block size for the last block if it is under construction.
   */
  ...
  public final QuotaCounts storagespaceConsumedStriped() {
    ...
    for (BlockInfo b : blocks) {
      ...
      long blockSize =
          b.isComplete()
              ? ((BlockInfoStriped) b).spaceConsumed()
              : getPreferredBlockSize() * ((BlockInfoStriped) b).getTotalBlockNum();
  ...
  public void dumpTreeRecursively(PrintWriter out, StringBuilder prefix, final int snapshotId) {
    ...
    final FileWithSnapshotFeature snapshotFeature = getFileWithSnapshotFeature();
   ...
   */
  public long collectBlocksBeyondMax(
      final long max, final BlocksMapUpdateInfo collectedBlocks, Set<BlockInfo> toRetain) {
   ...
   *
   * @param newLength the length for truncation TODO: properly handle striped blocks (HDFS-7622)
   */
  ...
  /**
   * This function is only called when block list is stored in snapshot diffs. Note that this can
   * only happen when truncation happens with snapshots. Since we do not support truncation with
   * striped blocks, we only need to handle contiguous blocks here.
   */
  public void collectBlocksBeyondSnapshot(
      BlockInfo[] snapshotBlocks, BlocksMapUpdateInfo collectedBlocks) {
