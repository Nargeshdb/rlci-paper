...
public abstract class Receiver implements DataTransferProtocol {
  ...
  private TraceScope continueTraceSpan(ClientOperationHeaderProto header, String description) {
    return continueTraceSpan(header.getBaseHeader(), description);
  }

  private TraceScope continueTraceSpan(BaseHeaderProto header, String description) {
  ...
  private static CachingStrategy getCachingStrategy(CachingStrategyProto strategy) {
  ...
  private void opReleaseShortCircuitFds(DataInputStream in) throws IOException {
    ...
    TraceScope traceScope =
        continueTraceSpan(proto.getTraceInfo(), proto.getClass().getSimpleName());
    try {
