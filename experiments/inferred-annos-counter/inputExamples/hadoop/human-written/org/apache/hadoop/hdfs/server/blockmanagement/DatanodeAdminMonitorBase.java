...
/**
 * This abstract class provides some base methods which are inherited by the DatanodeAdmin BackOff
 * and Default Monitors, which control decommission and maintenance mode.
 */
public abstract class DatanodeAdminMonitorBase
  ...
  /**
   * Used by the Configurable interface, which is used by ReflectionUtils to create an instance of
   * the monitor class. This method will be called to pass the Configuration to the new object.
   *
  ...
  public void setConf(Configuration conf) {
    ...
    if (this.maxConcurrentTrackedNodes < 0) {
      LOG.error(
          "{} is set to an invalid value, it must be zero or greater. " + "Defaulting to {}",
          DFSConfigKeys.DFS_NAMENODE_DECOMMISSION_MAX_CONCURRENT_TRACKED_NODES,
          DFSConfigKeys.DFS_NAMENODE_DECOMMISSION_MAX_CONCURRENT_TRACKED_NODES_DEFAULT);
      this.maxConcurrentTrackedNodes =
          DFSConfigKeys.DFS_NAMENODE_DECOMMISSION_MAX_CONCURRENT_TRACKED_NODES_DEFAULT;
  ...
  /**
   * Abstract method which must be implemented by the sub-classes to process set various instance
   * variables from the Configuration passed at object creation time.
   */
  protected abstract void processConf();

  /**
   * Start tracking a node for decommission or maintenance. The given Datanode will be queued for
   * later processing in pendingNodes. This method must be called under the namenode write lock.
   *
  ...
  /**
   * Get the number of datanodes nodes in the pending queue. Ie the count of nodes waiting to
   * decommission but have not yet started the process.
   *
