...
public class XMLUtils {
  ...
  /**
   * Given a code point, determine if it should be mangled before being represented in an XML
   * document.
   *
   * <p>Any code point that isn't valid in XML must be mangled. See
   * http://en.wikipedia.org/wiki/Valid_characters_in_XML for a quick reference, or the w3 standard
   ...
   *
   * <p>There are three kinds of code points in XML: - Those that can be represented normally, -
   * Those that have to be escaped (for example, &amp; must be represented as {@literal &amp;}) -
   * Those that cannot be represented at all in XML.
   *
   * <p>The built-in SAX functions will handle the first two types for us just fine. However,
   * sometimes we come across a code point of the third type. In this case, we have to mangle the
   * string in order to represent it at all. We also mangle backslash to avoid confusing a backslash
   * in the string with part our escape sequence.
   *
   * <p>The encoding used here is as follows: an illegal code point is represented as '\ABCD;',
   * where ABCD is the hexadecimal value of the code point.
   *
  ...
  /**
   * Demangle a string from an XML document. See {@link #mangleXmlString(String, boolean)} for a
   * description of the mangling format.
   *
   ...
   */
  public static void addSaxString(ContentHandler contentHandler, String tag, String val)
      throws SAXException {
