 ...
 *
 * <p>TODO namespaceID should be long and computed as hash(address + port)
 */
...
public class StorageInfo {
   ...
   *
   * <p>Assigned to the file system at formatting and never changes after that. Shared by all file
   * system components.
   */
  ...
  protected void checkStorageType(Properties props, StorageDirectory sd)
    ...
    if (!storageType.equals(type)) {
      throw new InconsistentFSStateException(
          sd.root,
          "Incompatible node types: storageType="
              + storageType
              + " but StorageDirectory type="
              + type);
    }
  }
  ...
  protected void setClusterId(Properties props, int layoutVersion, StorageDirectory sd)
      throws InconsistentFSStateException {
    ...
    if (LayoutVersion.supports(getServiceLayoutFeatureMap(), Feature.FEDERATION, layoutVersion)) {
      ...
      if (!(clusterID.equals("") || cid.equals("") || clusterID.equals(cid))) {
        throw new InconsistentFSStateException(
            sd.getRoot(), "cluster Id is incompatible with others.");
      }
      clusterID = cid;
    }
  }
  ...
  protected void setLayoutVersion(Properties props, StorageDirectory sd)
    ...
    if (lv < getServiceLayoutVersion()) { // future version
      throw new IncorrectVersionException(
          getServiceLayoutVersion(), lv, "storage directory " + sd.root.getAbsolutePath());
    }
    layoutVersion = lv;
  }
  ...
  protected static String getProperty(Properties props, StorageDirectory sd, String name)
      throws InconsistentFSStateException {
    String property = props.getProperty(name);
    if (property == null) {
      throw new InconsistentFSStateException(
          sd.root, "file " + STORAGE_FILE_VERSION + " has " + name + " missing.");
    }
    return property;
  }
