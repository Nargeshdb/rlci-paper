...
/**
 * BlockIdManager allocates the generation stamps and the block ID. The {@link FSNamesystem} is
 * responsible for persisting the allocations in the {@link FSEditLog}.
 */
public class BlockIdManager {
  /** The global generation stamp for legacy blocks with randomly generated block IDs. */
  private final GenerationStamp legacyGenerationStamp = new GenerationStamp();
  /** The global generation stamp for this file system. */
  private final GenerationStamp generationStamp = new GenerationStamp();
  /**
   * Most recent global generation stamp as seen on Active NameNode. Used by StandbyNode only.
   *
   * <p>StandbyNode does not update its global {@link #generationStamp} during edits tailing. The
   * global generation stamp on StandbyNode is updated
   *
   * <ol>
   *   <li>when the block with the next generation stamp is actually received
   *   <li>during fail-over it is bumped to the last value received from the Active NN through edits
   *       and stored as {@link #impendingGenerationStamp}
   * </ol>
   *
   * The former helps to avoid a race condition with IBRs during edits tailing. The latter
   * guarantees that generation stamps are never reused by new Active after fail-over.
   *
   * <p>See HDFS-14941 for more details.
   */
  private final GenerationStamp impendingGenerationStamp = new GenerationStamp();
  /**
   * The value of the generation stamp when the first switch to sequential block IDs was made.
   * Blocks with generation stamps below this value have randomly allocated block IDs. Blocks with
   * generation stamps above this value had sequentially allocated block IDs. Read from the fsImage
   * (or initialized as an offset from the V1 (legacy) generation stamp on upgrade).
   */
  ...
  /**
   * Upgrades the generation stamp for the filesystem by reserving a sufficient range for all
   * existing blocks. Should be invoked only during the first upgrade to sequential block IDs.
   */
  public long upgradeLegacyGenerationStamp() {
    Preconditions.checkState(
        generationStamp.getCurrentValue() == GenerationStamp.LAST_RESERVED_STAMP);
    generationStamp.skipTo(
        legacyGenerationStamp.getCurrentValue()
  ...
  /**
   * Sets the maximum allocated contiguous block ID for this filesystem. This is the basis for
   * allocating new block IDs.
   */
  ...
  /**
   * Sets the maximum allocated striped block ID for this filesystem. This is the basis for
   * allocating new block IDs.
   */
  ...
  /**
   * Determine whether the block ID was randomly generated (legacy) or sequentially generated. The
   * generation stamp value is used to make the distinction.
   *
   ...
   *
   * <p>Before HDFS-4645, block ID was randomly generated (legacy), so it is possible that legacy
   * block ID to be negative, which should not be considered as striped block ID.
   *
  ...
  /**
   * See {@link #isStripedBlock(Block)}, we should not use this function alone to determine a block
   * is striped block.
   */
  ...
  /**
   * The last 4 bits of HdfsConstants.BLOCK_GROUP_INDEX_MASK(15) is 1111, so the last 4 bits of
   * (~HdfsConstants.BLOCK_GROUP_INDEX_MASK) is 0000 and the other 60 bits are 1. Group ID is the
   * first 60 bits of any data/parity block id in the same striped block group.
   */
  ...
  public static byte getBlockIndex(Block reportedBlock) {
    return (byte) (reportedBlock.getBlockId() & HdfsServerConstants.BLOCK_GROUP_INDEX_MASK);
  }
