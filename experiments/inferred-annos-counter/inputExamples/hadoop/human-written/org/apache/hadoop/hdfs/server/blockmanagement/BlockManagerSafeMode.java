 ...
 *
 * <p>During name node startup, counts the number of <em>safe blocks</em>, those that have at least
 * the minimal number of replicas, and calculates the ratio of safe blocks to the total number of
 * blocks in the system, which is the size of blocks. When the ratio reaches the {@link #threshold}
 * and enough live data nodes have registered, it needs to wait for the safe mode {@link #extension}
 * interval. After the extension period has passed, it will not leave safe mode until the safe
 * blocks ratio reaches the {@link #threshold} and enough live data node registered.
 */
...
class BlockManagerSafeMode {
  ...
  @VisibleForTesting final long extension;
  ...
  BlockManagerSafeMode(
      BlockManager blockManager, Namesystem namesystem, boolean haEnabled, Configuration conf) {
    this.blockManager = blockManager;
    this.namesystem = namesystem;
    this.haEnabled = haEnabled;
    this.threshold =
        conf.getFloat(
            DFS_NAMENODE_SAFEMODE_THRESHOLD_PCT_KEY, DFS_NAMENODE_SAFEMODE_THRESHOLD_PCT_DEFAULT);
    if (this.threshold > 1.0) {
      LOG.warn("The threshold value shouldn't be greater than 1, " + "threshold: {}", threshold);
    }
    this.datanodeThreshold =
        conf.getInt(
            DFS_NAMENODE_SAFEMODE_MIN_DATANODES_KEY, DFS_NAMENODE_SAFEMODE_MIN_DATANODES_DEFAULT);
    int minReplication =
        conf.getInt(
    ...
    this.safeReplication =
        conf.getInt(DFSConfigKeys.DFS_NAMENODE_SAFEMODE_REPLICATION_MIN_KEY, minReplication);
    ...
    this.extension =
        conf.getTimeDuration(
            DFS_NAMENODE_SAFEMODE_EXTENSION_KEY,
  ...
  /**
   * The transition of the safe mode state machine. If safe mode is not currently on, this is a
   * no-op.
   */
  void checkSafeMode() {
    assert namesystem.hasWriteLock();
  ...
  /**
   * Adjust the total number of blocks safe and expected during safe mode. If safe mode is not
   * currently on, this is a no-op.
   *
  ...
  void adjustBlockTotals(int deltaSafe, int deltaTotal) {
    ...
    synchronized (this) {
      LOG.debug(
          "Adjusting block totals from {}/{} to {}/{}",
          blockSafe,
          blockTotal,
          blockSafe + deltaSafe,
          blockTotal + deltaTotal);
      assert blockSafe + deltaSafe >= 0
          : "Can't reduce blockSafe " + blockSafe + " by " + deltaSafe + ": would be negative";
      assert blockTotal + deltaTotal >= 0
          : "Can't reduce blockTotal " + blockTotal + " by " + deltaTotal + ": would be negative";
   ...
   *
   * <p>Never track blocks incrementally in non-HA code.
   *
   * <p>In the HA case, the StandbyNode can be in safemode while the namespace is modified by the
   * edit log tailer. In this case, the number of total blocks changes as edits are processed (eg
   * blocks are added and deleted). However, we don't want to do the incremental tracking during the
   * startup-time loading process -- only once the initial total has been set after the image has
   * been loaded.
   */
  ...
  String getSafeModeTip() {
    ...
    synchronized (this) {
      if (blockSafe < blockThreshold) {
        ...
        msg +=
            String.format(
                "The reported blocks %d has reached the threshold" + " %.4f of total blocks %d. ",
    ...
    if (datanodeThreshold > 0) {
      ...
      if (numLive < datanodeThreshold) {
        ...
        msg +=
            String.format(
                "The number of live datanodes %d has reached " + "the minimum number %d. ",
    ...
    if (getBytesInFuture() > 0) {
      msg +=
          "Name node detected blocks with generation stamps "
              ...
              + getBytesInFuture()
              + " byte(s). Please restart name node with "
  ...
  boolean leaveSafeMode(boolean force) {
    ...
    if (bytesInFuture > 0) {
      if (force) {
        LOG.warn(
            "Leaving safe mode due to forceExit. This will cause a data " + "loss of {} byte(s).",
            bytesInFuture);
        bytesInFutureBlocks.reset();
        bytesInFutureECBlockGroups.reset();
      } else {
        LOG.error(
            "Refusing to leave safe mode without a force flag. "
      ...
      LOG.warn(
          "forceExit used when normal exist would suffice. Treating "
    ...
    NameNode.stateChangeLog.info(
        "STATE* Network topology has {} racks and {}" + " datanodes",
        nt.getNumOfRacks(),
        nt.getNumOfLeaves());
    NameNode.stateChangeLog.info(
        "STATE* UnderReplicatedBlocks has {} blocks", blockManager.numOfUnderReplicatedBlocks());
  ...
  /**
   * Increment number of safe blocks if the current block is contiguous and it has reached minimal
   * replication or if the current block is striped and the number of its actual data blocks reaches
   * the number of data units specified by the erasure coding policy. If safe mode is not currently
   * on, this is a no-op.
   *
   * @param storageNum current number of replicas or number of internal blocks of a striped block
   *     group
   * @param storedBlock current storedBlock which is either a BlockInfoContiguous or a
   *     BlockInfoStriped
   */
  synchronized void incrementSafeBlockCount(int storageNum, BlockInfo storedBlock) {
    ...
    final int safeNumberOfNodes =
        storedBlock.isStriped()
    ...
    if (storageNum == safeNumberOfNodes) {
      ...
      if (prog.getStatus(Phase.SAFEMODE) != Status.COMPLETE) {
        if (this.awaitingReportedBlocksCounter == null) {
          this.awaitingReportedBlocksCounter =
              prog.getCounter(Phase.SAFEMODE, STEP_AWAITING_REPORTED_BLOCKS);
        }
  ...
  /**
   * Decrement number of safe blocks if the current block is contiguous and it has just fallen below
   * minimal replication or if the current block is striped and its actual data blocks has just
   * fallen below the number of data units specified by erasure coding policy. If safe mode is not
   * currently on, this is a no-op.
   */
  synchronized void decrementSafeBlockCount(BlockInfo b) {
    ...
    final int safeNumberOfNodes =
        b.isStriped() ? ((BlockInfoStriped) b).getRealDataBlockNum() : safeReplication;
  ...
  /**
   * Check if the block report replica has a generation stamp (GS) in future. If safe mode is not
   * currently on, this is a no-op.
   *
  ...
  /**
   * Returns the number of bytes that reside in blocks with Generation Stamps greater than
   * generation stamp known to Namenode.
   *
  ...
  /**
   * Get time (counting in milliseconds) left to leave extension period. It should leave safemode at
   * once if blockTotal = 0 rather than wait extension time (30s by default).
   *
   * <p>Negative value indicates the extension period has passed.
   */
  ...
  private void initializeReplQueuesIfNecessary() {
    ...
    boolean canInitializeReplQueues =
        blockManager.shouldPopulateReplQueues() && blockSafe >= blockReplQueueThreshold;
  ...
  /** Checks consistency of the class state. This is costly so only runs if asserts are enabled. */
  private void doConsistencyCheck() {
    ...
    synchronized (this) {
      if (blockTotal != activeBlocks && !(blockSafe >= 0 && blockSafe <= blockTotal)) {
        LOG.warn(
            "SafeMode is in inconsistent filesystem state. "
  ...
  /**
   * Periodically check whether it is time to leave safe mode. This thread starts when the threshold
   * level is reached.
   */
  private class SafeModeMonitor implements Runnable {
     ...
     *
     * <p>Safe mode can be turned off iff the threshold is reached, and the extension time has
     * passed.
     */
