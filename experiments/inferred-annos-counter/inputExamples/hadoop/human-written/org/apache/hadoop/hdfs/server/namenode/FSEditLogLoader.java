...
public class FSEditLogLoader {
  static final Logger LOG = LoggerFactory.getLogger(FSEditLogLoader.class.getName());
  static final long REPLAY_TRANSACTION_LOG_INTERVAL = 1000; // 1sec

  /** Limit logging about edit loading to every 5 seconds max. */
  @VisibleForTesting static final long LOAD_EDIT_LOG_INTERVAL_MS = 5000;
  ...
  long loadFSEdits(
    ...
    try {
      ...
      long numEdits =
          loadEditRecords(edits, false, expectedStartingTxId, maxTxnsToRead, startOpt, recovery);
      long endTime = timer.monotonicNow();
      LogAction postLogAction =
          loadEditsLogHelper.record("post", endTime, numEdits, edits.length(), endTime - startTime);
      if (postLogAction.shouldLog()) {
        FSImage.LOG.info(
            "Loaded {} edits file(s) (the last named {}) of "
                + "total size {}, total edits {}, total load time {} ms",
            postLogAction.getCount(),
            edits.getName(),
  ...
  long loadEditRecords(
    ...
    return loadEditRecords(
        in, closeOnExit, expectedStartingTxId, Long.MAX_VALUE, startOpt, recovery);
  }
  ...
  long loadEditRecords(
    ...
    try {
      while (true) {
        try {
          ...
          try {
            ...
            MetaRecoveryContext.editLogLoaderPrompt(
                "Failed to " + "apply edit log operation " + op + ": error " + e.getMessage(),
                recovery,
                "applying edits");
          }
          ...
          MetaRecoveryContext.LOG.warn(
              "Stopped reading edit log at " + in.getPosition() + "/" + in.length());
          break;
        }
      }
    } finally {
      ...
      if (FSImage.LOG.isDebugEnabled()) {
        ...
        FSImage.LOG.debug("maxTxnsToRead = " + maxTxnsToRead + " actual edits read = " + numEdits);
      }
    ...
    if (inodeId == HdfsConstants.GRANDFATHER_INODE_ID) {
      if (NameNodeLayoutVersion.supports(LayoutVersion.Feature.ADD_INODE_ID, logVersion)) {
        throw new IOException(
            "The layout version " + logVersion + " supports inodeId but gave bogus inodeId");
      }
  ...
  private long applyEditLogOp(
      FSEditLogOp op, FSDirectory fsDir, StartupOption startOpt, int logVersion, long lastInodeId)
      throws IOException {
    ...
    switch (op.opCode) {
      case OP_ADD:
        {
          ...
          if (oldFile == null) { // this is OP_ADD on a new file (case 1)
            ...
            final short replication =
                fsNamesys.getBlockManager().adjustReplication(addCloseOp.replication);
            assert addCloseOp.blocks.length == 0;

            // add to the file tree
            inodeId = getAndUpdateLastInodeId(addCloseOp.inodeId, logVersion, lastInodeId);
            newFile =
                FSDirWriteFileOp.addFileForEditLog(
                    fsDir,
                    inodeId,
                    ...
                    addCloseOp.mtime,
                    addCloseOp.atime,
                    addCloseOp.blockSize,
                    true,
                    addCloseOp.clientName,
                    addCloseOp.clientMachine,
                    addCloseOp.storagePolicyId,
                    addCloseOp.erasureCodingPolicyId);
            assert newFile != null;
            ...
            if (toAddRetryCache) {
              ...
              fsNamesys.addCacheEntryWithPayload(
                  addCloseOp.rpcClientId, addCloseOp.rpcCallId, stat);
            }
            ...
            if (!oldFile.isUnderConstruction()) {
              ...
              LocatedBlock lb =
                  FSDirAppendOp.prepareFileForAppend(
                      fsNamesys,
                      iip,
                      addCloseOp.clientName,
                      addCloseOp.clientMachine,
              ...
              if (toAddRetryCache) {
                HdfsFileStatus stat = FSDirStatAndListingOp.createFileStatusForEditLog(fsDir, iip);
                fsNamesys.addCacheEntryWithPayload(
                    addCloseOp.rpcClientId,
                    addCloseOp.rpcCallId,
          ...
          ErasureCodingPolicy ecPolicy =
              FSDirErasureCodingOp.unprotectedGetErasureCodingPolicy(fsDir.getFSNamesystem(), iip);
      ...
      case OP_CLOSE:
        {
          ...
          if (file.isUnderConstruction()) {
            ...
            file.toCompleteFile(
                file.getModificationTime(), 0, fsNamesys.getBlockManager().getMinReplication());
          }
          break;
        }
      case OP_APPEND:
        {
          ...
          if (!file.isUnderConstruction()) {
            ...
            if (toAddRetryCache) {
              HdfsFileStatus stat = FSDirStatAndListingOp.createFileStatusForEditLog(fsDir, iip);
              fsNamesys.addCacheEntryWithPayload(
                  appendOp.rpcClientId, appendOp.rpcCallId, new LastBlockWithStatus(lb, stat));
            }
          }
          break;
        }
      case OP_UPDATE_BLOCKS:
        {
          ...
          if (FSNamesystem.LOG.isDebugEnabled()) {
            FSNamesystem.LOG.debug(
                op.opCode + ": " + path + " numblocks : " + updateOp.blocks.length);
          }
          ...
          if (FSNamesystem.LOG.isDebugEnabled()) {
            FSNamesystem.LOG.debug(
                op.opCode
                    ...
                    + path
                    + " new block id : "
                    + addBlockOp.getLastBlock().getBlockId());
          }
          INodesInPath iip = fsDir.getINodesInPath(path, DirOp.READ);
          INodeFile oldFile = INodeFile.valueOf(iip.getLastINode(), path);
      ...
      case OP_SET_REPLICATION:
        {
          ...
          short replication =
              fsNamesys.getBlockManager().adjustReplication(setReplicationOp.replication);
      ...
      case OP_SET_NS_QUOTA:
        {
          ...
          FSDirAttrOp.unprotectedSetQuota(
              fsDir, iip, setNSQuotaOp.nsQuota, HdfsConstants.QUOTA_DONT_SET, null);
          break;
        }
      case OP_CLEAR_NS_QUOTA:
        {
          ...
          final INodesInPath iip = fsDir.getINodesInPath(src, DirOp.WRITE);
          FSDirAttrOp.unprotectedSetQuota(
              fsDir, iip, HdfsConstants.QUOTA_RESET, HdfsConstants.QUOTA_DONT_SET, null);
          break;
        }
      case OP_SET_QUOTA:
        {
      ...
      case OP_SET_QUOTA_BY_STORAGETYPE:
        {
          ...
          FSDirAttrOp.unprotectedSetQuota(
              ...
              setQuotaByStorageTypeOp.dsQuota,
      ...
      case OP_SYMLINK:
        {
          ...
          FSDirSymlinkOp.unprotectedAddSymlink(
              fsDir,
              iip.getExistingINodes(),
              iip.getLastLocalName(),
              inodeId,
      ...
      case OP_CREATE_SNAPSHOT:
        {
          ...
          final String snapshotRoot =
              renameReservedPathsOnUpgrade(createSnapshotOp.snapshotRoot, logVersion);
          ...
          String path =
              fsNamesys
                  ...
                  .createSnapshot(
                      ...
                      iip,
                      snapshotRoot,
                      createSnapshotOp.snapshotName,
                      createSnapshotOp.mtime);
          if (toAddRetryCache) {
            fsNamesys.addCacheEntryWithPayload(
                createSnapshotOp.rpcClientId, createSnapshotOp.rpcCallId, path);
          }
          break;
        }
      case OP_DELETE_SNAPSHOT:
        {
          ...
          final String snapshotRoot =
              renameReservedPathsOnUpgrade(deleteSnapshotOp.snapshotRoot, logVersion);
          INodesInPath iip = fsDir.getINodesInPath(snapshotRoot, DirOp.WRITE);
          fsNamesys
              .getSnapshotManager()
              .deleteSnapshot(
                  ...
                  new INode.ReclaimContext(
                      fsNamesys.dir.getBlockStoragePolicySuite(),
                      collectedBlocks,
                      removedINodes,
                      null),
                  deleteSnapshotOp.mtime);
      ...
      case OP_RENAME_SNAPSHOT:
        {
          ...
          final String snapshotRoot =
              renameReservedPathsOnUpgrade(renameSnapshotOp.snapshotRoot, logVersion);
          INodesInPath iip = fsDir.getINodesInPath(snapshotRoot, DirOp.WRITE);
          fsNamesys
              .getSnapshotManager()
              .renameSnapshot(
                  iip,
                  snapshotRoot,
                  renameSnapshotOp.snapshotOldName,
      ...
      case OP_DISALLOW_SNAPSHOT:
        {
          ...
          final String snapshotRoot =
              renameReservedPathsOnUpgrade(disallowSnapshotOp.snapshotRoot, logVersion);
      ...
      case OP_SET_GENSTAMP_V2:
        {
          ...
          // see HDFS-14941
          blockManager.getBlockIdManager().setImpendingGenerationStamp(setGenstampV2Op.genStampV2);
          break;
        }
      case OP_ALLOCATE_BLOCK_ID:
        {
          ...
          if (BlockIdManager.isStripedBlockID(allocateBlockIdOp.blockId)) {
            ...
            blockManager
                .getBlockIdManager()
                .setLastAllocatedStripedBlockId(allocateBlockIdOp.blockId);
          } else {
            blockManager
                .getBlockIdManager()
                .setLastAllocatedContiguousBlockId(allocateBlockIdOp.blockId);
          }
          break;
        }
      ...
      case OP_ADD_CACHE_DIRECTIVE:
        {
          ...
          CacheDirectiveInfo result =
              fsNamesys.getCacheManager().addDirectiveFromEditLog(addOp.directive);
          if (toAddRetryCache) {
      ...
      case OP_SET_XATTR:
        {
          ...
          FSDirXAttrOp.unprotectedSetXAttrs(
              fsDir, iip, setXAttrOp.xAttrs, EnumSet.of(XAttrSetFlag.CREATE, XAttrSetFlag.REPLACE));
          if (toAddRetryCache) {
      ...
      case OP_REMOVE_XATTR:
        {
          ...
          FSDirXAttrOp.unprotectedRemoveXAttrs(fsDir, iip, removeXAttrOp.xAttrs);
          if (toAddRetryCache) {
      ...
      case OP_TRUNCATE:
        {
          ...
          FSDirTruncateOp.unprotectedTruncate(
              fsNamesys,
              iip,
              truncateOp.clientName,
              truncateOp.clientMachine,
              truncateOp.newLength,
              truncateOp.timestamp,
      ...
      case OP_SET_STORAGE_POLICY:
        {
          ...
          FSDirAttrOp.unprotectedSetStoragePolicy(
              fsDir, fsNamesys.getBlockManager(), iip, setStoragePolicyOp.policyId);
          break;
        }
      case OP_ADD_ERASURE_CODING_POLICY:
        AddErasureCodingPolicyOp addOp = (AddErasureCodingPolicyOp) op;
        fsNamesys.getErasureCodingPolicyManager().addPolicy(addOp.getEcPolicy());

        if (toAddRetryCache) {
          fsNamesys.addCacheEntryWithPayload(op.rpcClientId, op.rpcCallId, addOp.getEcPolicy());
        }
        break;
      case OP_ENABLE_ERASURE_CODING_POLICY:
        EnableErasureCodingPolicyOp enableOp = (EnableErasureCodingPolicyOp) op;
        fsNamesys.getErasureCodingPolicyManager().enablePolicy(enableOp.getEcPolicy());
        if (toAddRetryCache) {
          fsNamesys.addCacheEntry(op.rpcClientId, op.rpcCallId);
        }
        break;
      case OP_DISABLE_ERASURE_CODING_POLICY:
        DisableErasureCodingPolicyOp disableOp = (DisableErasureCodingPolicyOp) op;
        fsNamesys.getErasureCodingPolicyManager().disablePolicy(disableOp.getEcPolicy());
        if (toAddRetryCache) {
          fsNamesys.addCacheEntry(op.rpcClientId, op.rpcCallId);
        }
        break;
      case OP_REMOVE_ERASURE_CODING_POLICY:
        ...
        fsNamesys.getErasureCodingPolicyManager().removePolicy(removeOp.getEcPolicy());
        if (toAddRetryCache) {
          fsNamesys.addCacheEntry(op.rpcClientId, op.rpcCallId);
        }
        break;
  ...
  private static String formatEditLogReplayError(
      EditLogInputStream in, long recentOpcodeOffsets[], long txid) {
    StringBuilder sb = new StringBuilder();
    sb.append("Error replaying edit log at offset " + in.getPosition())
        .append(".  Expected transaction ID was ")
        .append(txid);
  ...
  private void addNewBlock(AddBlockOp op, INodeFile file, ErasureCodingPolicy ecPolicy)
      throws IOException {
      ...
      if (oldLastBlock.getBlockId() != pBlock.getBlockId()
        ...
        throw new IOException(
            "Mismatched block IDs or generation stamps for the old last block of file "
                ...
                + oldLastBlock
                + ", and the block read from editlog is "
                + pBlock);
      }
  ...
  private void updateBlocks(
    ...
    for (int i = 0; i < oldBlocks.length && i < newBlocks.length; i++) {
      ...
      if (oldBlock.getBlockId() != newBlock.getBlockId()
        ...
        throw new IOException(
            "Mismatched block IDs or generation stamps, "
                + "attempting to replace block "
                + oldBlock
                + " with "
                + newBlock
                + " as block # "
    ...
    if (newBlocks.length < oldBlocks.length) {
      ...
      if (!file.isUnderConstruction()) {
        throw new IOException(
            "Trying to remove a block from file " + path + " which is not under construction.");
      }
  ...
  /**
   * Throw appropriate exception during upgrade from 203, when editlog loading could fail due to
   * opcode conflicts.
   */
  private void check203UpgradeFailure(int logVersion, Throwable e) throws IOException {
    ...
    if (Storage.is203LayoutVersion(logVersion)
      ...
      String msg =
          "During upgrade failed to load the editlog version "
              + logVersion
              + " from release 0.20.203. Please go back to the old "
  ...
  /**
   * Find the last valid transaction ID in the stream. If there are invalid or corrupt transactions
   * in the middle of the stream, scanEditLog will skip over them. This reads through the stream but
   * does not close it.
   *
   * @param maxTxIdToScan Maximum Tx ID to try to scan. The scan returns after reading this or a
   *     higher ID. The file portion beyond this ID is potentially being updated.
   */
  static EditLogValidation scanEditLog(EditLogInputStream in, long maxTxIdToScan) {
    long lastPos;
    ...
    while (true) {
      ...
      try {
        ...
        FSImage.LOG.warn(
            "Caught exception after scanning through "
                + numValid
                + " ops from "
                + in
        ...
        if (in.getPosition() <= lastPos) {
          FSImage.LOG.warn(
              "After resync, the position, {} is not greater "
   ...
   *
   * <p>This stream also allows us to set a limit on how many bytes we can read without getting an
   * exception.
   */
  public static class PositionTrackingInputStream extends FilterInputStream
    ...
    private void checkLimit(long amt) throws IOException {
      ...
      if (extra > 0) {
        throw new IOException(
            "Tried to read " + amt + " byte(s) past " + "the limit at offset " + limitPos);
      }
    }
    ...
    public long skip(long amt) throws IOException {
      ...
      if (extra > 0) {
        throw new IOException(
            "Tried to skip " + extra + " bytes past " + "the limit at offset " + limitPos);
      }
  ...
  /**
   * Creates a Step used for updating startup progress, populated with information from the given
   * edits. The step always includes the log's name. If the log has a known length, then the length
   * is included in the step too.
   *
