 ...
 *
 * <p>DFSClients request shared memory segments from the DataNode. The ShortCircuitRegistry
 * generates and manages these segments. Each segment has a randomly generated 128-bit ID which
 * uniquely identifies it. The segments each contain several "slots."
 *
 * <p>Before performing a short-circuit read, DFSClients must request a pair of file descriptors
 * from the DataNode via the REQUEST_SHORT_CIRCUIT_FDS operation. As part of this operation,
 * DFSClients pass the ID of the shared memory segment they would like to use to communicate
 * information about this replica, as well as the slot number within that segment they would like to
 * use. Slot allocation is always done by the client.
 *
 * <p>Slots are used to track the state of the block on the both the client and datanode. When this
 * DataNode mlocks a block, the corresponding slots for the replicas are marked as "anchorable".
 * Anchorable blocks can be safely read without verifying the checksum. This means that
 * BlockReaderLocal objects using these replicas can skip checksumming. It also means that we can do
 * zero-copy reads on these replicas (the ZCR interface has no way of verifying checksums.)
 *
 * <p>When a DN needs to munlock a block, it needs to first wait for the block to be unanchored by
 * clients doing a no-checksum read or a zero-copy read. The DN also marks the block's slots as
 * "unanchorable" to prevent additional clients from initiating these operations in the future.
 *
 * <p>The counterpart of this class on the client is {@link DfsClientShmManager}.
 */
public class ShortCircuitRegistry {
  public static final Logger LOG = LoggerFactory.getLogger(ShortCircuitRegistry.class);
  ...
  public static class RegisteredShm extends ShortCircuitShm implements DomainSocketWatcher.Handler {
    ...
    RegisteredShm(
        String clientName, ShmId shmId, FileInputStream stream, ShortCircuitRegistry registry)
        throws IOException {
      super(shmId, stream);
  ...
  /**
   * A watcher which sends out callbacks when the UNIX domain socket associated with a shared memory
   * segment closes.
   */
  ...
  public ShortCircuitRegistry(Configuration conf) throws IOException {
    ...
    try {
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            "created new ShortCircuitRegistry with interruptCheck="
                + interruptCheck
                + ", shmPath="
                + shmFactory.getPath());
      }
  ...
  /**
   * Invalidate any slot associated with a blockId that we are invalidating (deleting) from this
   * DataNode. When a slot is invalid, the DFSClient will not use the corresponding replica for new
   * read or mmap operations (although existing, ongoing read or mmap operations will complete.)
   *
   * @param blockId The block ID.
   */
  ...
  public static class NewShmInfo implements Closeable {
    ...
    @MustCallAlias
    NewShmInfo(ShmId shmId, @MustCallAlias FileInputStream stream) {
      this.shmId = shmId;
      this.stream = stream;
    }
   ...
   * @param sock The DomainSocket to associate with this memory segment. When this socket is closed,
   *     or the other side writes anything to the socket, the segment will be closed. This can
   *     happen at any time, including right after this function returns.
   * @return A NewShmInfo object. The caller must close the NewShmInfo object once they are done
   *     with it.
   * @throws IOException If the new memory segment could not be created.
   */
  public NewShmInfo createNewMemorySegment(String clientName, DomainSocket sock)
      throws IOException {
  ...
  public synchronized void registerSlot(ExtendedBlockId blockId, SlotId slotId, boolean isCached)
      throws InvalidRequestException {
    if (!enabled) {
      if (LOG.isTraceEnabled()) {
        LOG.trace(
            this + " can't register a slot because the " + "ShortCircuitRegistry is not enabled.");
      }
      throw new UnsupportedOperationException();
    }
    ...
    if (shm == null) {
      throw new InvalidRequestException(
          "there is no shared memory segment " + "registered with shmId " + shmId);
    }
  ...
  public static interface Visitor {
    boolean accept(
        HashMap<ShmId, RegisteredShm> segments, HashMultimap<ExtendedBlockId, Slot> slots);
  }
