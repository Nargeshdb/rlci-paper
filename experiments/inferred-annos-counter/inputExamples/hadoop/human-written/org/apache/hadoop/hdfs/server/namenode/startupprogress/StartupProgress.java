 ...
 * Its methods provide ways to indicate begin and end of a {@link Phase} or {@link Step} within a
 * phase. Additional methods provide ways to associate a step or phase with optional information,
 * such as a file name or file size. It also provides counters, which can be incremented by the
 * caller to indicate progress through a long-running task.
 *
 * <p>This class is thread-safe. Any number of threads may call any methods, even for the same phase
 * or step, without risk of corrupting internal state. For all begin/end methods and set methods,
 * the last one in wins, overwriting any prior writes. Instances of {@link Counter} provide an
 * atomic increment operation to prevent lost updates.
 *
 * <p>After startup completes, the tracked data is frozen. Any subsequent updates or counter
 * increments are no-ops.
 *
 * <p>For read access, call {@link #createView()} to create a consistent view with a clone of the
 * data.
 */
...
public class StartupProgress {
  ...
  /**
   * Creates a new StartupProgress by initializing internal data structure for tracking progress of
   * all defined phases.
   */
  ...
  /**
   * Begins execution of the specified step within the specified phase. This is a no-op if the phase
  ...
  /**
   * Ends execution of the specified step within the specified phase. This is a no-op if the phase
   * is already completed.
   *
  ...
  /**
   * Returns a counter associated with the specified phase and step. Typical usage is to increment a
   * counter within a tight loop. Callers may use this method to obtain a counter once and then
   * increment that instance repeatedly within a loop. This prevents redundant lookup operations and
   * object creation within the tight loop. Incrementing the counter is an atomic operation, so
   * there is no risk of lost updates even if multiple threads increment the same counter.
   *
   * @param phase Phase to get
  ...
  /**
   * Sets the optional file name associated with the specified phase. For example, this can be used
   * while loading fsimage to indicate the full path to the fsimage file.
   *
   * @param phase Phase to set
  ...
  /**
   * Sets the optional size in bytes associated with the specified phase. For example, this can be
   * used while loading fsimage to indicate the size of the fsimage file.
   *
   * @param phase Phase to set
  ...
  /**
   * Sets the total associated with the specified phase and step. For example, this can be used
   * while loading edits to indicate the number of operations to be applied.
   *
   * @param phase Phase to set
   * @param step Step to set
  ...
  /**
   * Creates a {@link StartupProgressView} containing data cloned from this StartupProgress.
   * Subsequent updates to this StartupProgress will not be shown in the view. This gives a
   * consistent, unchanging view for callers that need to perform multiple related read operations.
   * Calculations that require aggregation, such as overall percent complete, will not be impacted
  ...
  /**
   * Returns true if the entire startup process has completed, determined by checking if each phase
   * is complete.
   *
  ...
  /**
   * Lazily initializes the internal data structure for tracking the specified phase and step.
   * Returns either the newly initialized data structure or the existing one. Initialization is
   * atomic, so there is no risk of lost updates even if multiple threads attempt to initialize the
