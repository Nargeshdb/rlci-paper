 ...
 * Checkpoint command.
 *
 * <p>Returned to the backup node by the name-node as a reply to the {@link
 * NamenodeProtocol#startCheckpoint(NamenodeRegistration)} request.<br>
 * Contains:
 *
 * <ul>
 *   <li>{@link CheckpointSignature} identifying the particular checkpoint
 *   <li>indicator whether the backup image should be discarded before starting the checkpoint
 *   <li>indicator whether the image should be transfered back to the name-node upon completion of
 *       the checkpoint.
 * </ul>
 */
...
public class CheckpointCommand extends NamenodeCommand {
  ...
  /**
   * Indicates whether the new checkpoint image needs to be transfered back to the name-node after
