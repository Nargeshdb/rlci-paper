...
/** This class aggregates information from {@link SlowDiskReports} received via heartbeats. */
...
public class SlowDiskTracker {
  ...
  /**
   * Number of disks to include in JSON report per operation. We will return disks with the highest
   * latency.
   */
  ...
  /**
   * Information about disks that have been reported as being slow. It is map of (Slow Disk ID) ->
   * (DiskLatency). The DiskLatency contains the disk ID, the latencies reported and the timestamp
  ...
  public SlowDiskTracker(Configuration conf, Timer timer) {
    ...
    this.reportGenerationIntervalMs =
  ...
  public void addSlowDiskReport(String dataNodeID, SlowDiskReports dnSlowDiskReport) {
    Map<String, Map<DiskOp, Double>> slowDisks = dnSlowDiskReport.getSlowDisks();

    long now = timer.monotonicNow();

    for (Map.Entry<String, Map<DiskOp, Double>> slowDiskEntry : slowDisks.entrySet()) {

      String diskID = getSlowDiskIDForReport(dataNodeID, slowDiskEntry.getKey());
    ...
    if (isUpdateInProgress.compareAndSet(false, true)) {
      ...
      new Thread(
              new Runnable() {
                @Override
                public void run() {
                  slowDisksReport = getSlowDisks(diskIDLatencyMap, MAX_DISKS_TO_REPORT, now);
  ...
  public static class DiskLatency {
    ...
    private final Map<DiskOp, Double> latencyMap;
    ...
    public DiskLatency(String slowDiskID, Map<DiskOp, Double> latencyMap, long timestamp) {
      this.slowDiskID = slowDiskID;
      this.latencyMap = latencyMap;
   ...
   *
   * @param numDisks number of disks to return. This is to limit the size of the generated JSON.
   */
  private ArrayList<DiskLatency> getSlowDisks(
    ...
    final PriorityQueue<DiskLatency> topNReports =
        new PriorityQueue<>(
            reports.size(),
    ...
    for (Map.Entry<String, DiskLatency> entry : reports.entrySet()) {
      ...
      if (now - diskLatency.timestamp < reportValidityMs) {
        ...
        } else if (topNReports.peek().getMaxLatency() < diskLatency.getMaxLatency()) {
          topNReports.poll();
          topNReports.add(diskLatency);
        }
      } else {
