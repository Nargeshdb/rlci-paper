 ...
 *
 * <p>The data-node that receives this command treats itself as a primary data-node in the recover
 * process.
 *
 * <p>Block recovery is identified by a recoveryId, which is also the new generation stamp, which
 * the block will have after the recovery succeeds.
 */
...
public class BlockRecoveryCommand extends DatanodeCommand {
  ...
  /**
   * This is a block with locations from which it should be recovered and the new generation stamp,
   * which the block will have after successful recovery.
   *
   * <p>The new generation stamp of the block, also plays role of the recovery id.
   */
  @InterfaceAudience.Private
  @InterfaceStability.Evolving
  ...
  public static class RecoveringStripedBlock extends RecoveringBlock {
    ...
    public RecoveringStripedBlock(
        RecoveringBlock rBlock, byte[] blockIndices, ErasureCodingPolicy ecPolicy) {
      super(rBlock);
