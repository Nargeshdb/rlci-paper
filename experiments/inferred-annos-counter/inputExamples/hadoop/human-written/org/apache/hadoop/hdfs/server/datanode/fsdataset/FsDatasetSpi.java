...
/**
 * This is a service provider interface for the underlying storage that stores replicas for a data
 * node. The default implementation stores replicas on local drives.
 */
...
public interface FsDatasetSpi<V extends FsVolumeSpi> extends FSDatasetMBean {
  ...
  abstract class Factory<D extends FsDatasetSpi<?>> {
    ...
    /** Create a new object. */
    public abstract D newInstance(DataNode datanode, DataStorage storage, Configuration conf)
        throws IOException;
  ...
  /**
   * It behaviors as an unmodifiable list of FsVolume. Individual FsVolume can be obtained by using
   * {@link #get(int)}.
   *
   * <p>This also holds the reference counts for these volumes. It releases all the reference counts
   * in {@link #close()}.
   */
  class FsVolumeReferences implements Iterable<FsVolumeSpi>, Closeable {
    ...
    private static class FsVolumeSpiIterator implements Iterator<FsVolumeSpi> {
      private final List<FsVolumeReference> references;
      private int idx = 0;
   ...
   *
   * <p>The caller must release the reference of each volume by calling {@link
   ...
   *
   * <p>If the FSDataset supports block scanning, this function registers the new volume with the
   * block scanner.
   *
   * @param location The storage location for the new volume.
   * @param nsInfos Namespace information for the new volume.
   */
  void addVolume(final StorageLocation location, final List<NamespaceInfo> nsInfos)
      throws IOException;

  /**
   * Removes a collection of volumes from FsDataset.
   *
   * <p>If the FSDataset supports block scanning, this function removes the volumes from the block
   * scanner.
   *
   * @param volumes The paths of the volumes to be removed.
   * @param clearFailure set true to clear the failure information about the volumes.
   */
  ...
  /**
   * Check whether the in-memory block record matches the block on the disk, and, in case that they
   * are not matched, update the record or mark it as corrupted.
   */
  void checkAndUpdate(String bpid, ScanInfo info) throws IOException;

  /**
   * @param b - the block
   * @return a stream if the meta-data of the block exists; otherwise, return null.
   * @throws IOException
   */
  ...
  /**
   * Get reference to the replica meta info in the replicasMap. To be called from methods that are
   * synchronized on implementations of {@link FsDatasetSpi}
   *
   ...
   * @return an input stream to read the contents of the specified block, starting at the offset
   * @throws IOException
   */
  InputStream getBlockInputStream(ExtendedBlock b, long seekOffset) throws IOException;

  /**
   * Returns an input stream at specified offset of the specified block. The block is still in the
   * tmp directory and is not finalized
   *
   * @return an input stream to read the contents of the specified block, starting at the offset
   * @throws IOException
   */
  ReplicaInputStreams getTmpInputStreams(ExtendedBlock b, long blkoff, long ckoff)
      throws IOException;
   ...
   */
  ReplicaHandler createTemporary(
   ...
   */
  ReplicaInPipeline convertTemporaryToRbw(ExtendedBlock temporary) throws IOException;
  ...
  /**
   * Recover a failed pipeline close. It bumps the replica's generation stamp and finalize it if RBW
   * replica
   *
   * @param b block
   * @param newGS the new generation stamp for the replica
  ...
  /**
   * Finalizes the block previously opened for writing using writeToBlock. The block size is what is
   * in the parameter b and it must match the amount of data written
   *
   * @param b Block to be finalized
   * @param fsyncDir whether to sync the directory changes to durable device.
   * @throws IOException
   * @throws ReplicaNotFoundException if the replica can not be found when the block is been
   *     finalized. For instance, the block resides on an HDFS volume that has been removed.
   */
  void finalizeBlock(ExtendedBlock b, boolean fsyncDir) throws IOException;

  /**
   * Unfinalizes the block previously opened for writing using writeToBlock. The temporary file
   * associated with this block is deleted.
   *
   * @throws IOException
   */
   ...
   * @throws IOException
   */
  void adjustCrcChannelPosition(ExtendedBlock b, ReplicaOutputStreams outs, int checksumSize)
      throws IOException;

  /**
   * Checks how many valid storage volumes there are in the DataNode.
   *
   * @return true if more than the minimum number of valid volumes are left in the FSDataSet.
   */
   ...
   *
   * @return actual state of the replica on this data-node or null if data-node does not have the
   *     replica.
   */
   ...
   */
  Replica updateReplicaUnderRecovery(
      ExtendedBlock oldBlock, long recoveryId, long newBlockId, long newLength) throws IOException;
  ...
  /**
   * Deletes the block pool directories. If force is false, directories are deleted only if no block
   * files exist for the block pool. If force is true entire directory for the blockpool is deleted
   * along with its contents.
   *
   * @param bpid BlockPool Id to be deleted.
   * @param force If force is false, directories are deleted only if no block files exist for the
   *     block pool, otherwise entire directory for the blockpool is deleted along with its
   *     contents.
   * @throws IOException
   */
  ...
  /**
   * Enable 'trash' for the given dataset. When trash is enabled, files are moved to a separate
   * trash directory instead of being deleted immediately. This can be useful for example during
   * rolling upgrades.
   */
  ...
  void submitBackgroundSyncFileRangeRequest(
      final ExtendedBlock block,
      final ReplicaOutputStreams outs,
      final long offset,
      final long nbytes,
      final int flags);

  /** Callback from RamDiskAsyncLazyPersistService upon async lazy persist task end */
  void onCompleteLazyPersist(
      String bpId, long blockId, long creationTime, File[] savedFiles, V targetVolume);
  ...
  ReplicaInfo moveBlockAcrossStorage(
      final ExtendedBlock block, StorageType targetStorageType, String storageId)
      throws IOException;

  /**
   * Set a block to be pinned on this datanode so that it cannot be moved by Balancer/Mover.
   *
   * <p>It is a no-op when dfs.datanode.block-pinning.enabled is set to false.
   */
   ...
   */
  ReplicaInfo moveBlockAcrossVolumes(final ExtendedBlock block, FsVolumeSpi destination)
      throws IOException;
