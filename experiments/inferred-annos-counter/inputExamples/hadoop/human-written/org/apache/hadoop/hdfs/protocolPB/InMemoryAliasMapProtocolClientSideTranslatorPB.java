...
/**
 * This class is the client side translator to translate requests made to the {@link
 * InMemoryAliasMapProtocol} interface to the RPC server implementing {@link AliasMapProtocolPB}.
 */
...
public class InMemoryAliasMapProtocolClientSideTranslatorPB
  ...
  private static final Logger LOG =
      LoggerFactory.getLogger(InMemoryAliasMapProtocolClientSideTranslatorPB.class);
  ...
  public static Collection<InMemoryAliasMapProtocol> init(Configuration conf) {
    ...
    for (String nsId : getNameServiceIds(conf)) {
      try {
        ...
        if (namenodeURI != null) {
          aliasMaps.add(
              NameNodeProxies.createProxy(newConf, namenodeURI, InMemoryAliasMapProtocol.class)
                  .getProxy());
          LOG.info("Connected to InMemoryAliasMap at {}", namenodeURI);
        }
      } catch (IOException | URISyntaxException e) {
        LOG.warn(
            "Exception in connecting to InMemoryAliasMap for nameservice " + "{}: {}", nsId, e);
      }
    }
    ...
    if (conf.get(DFS_PROVIDED_ALIASMAP_INMEMORY_RPC_ADDRESS) != null) {
      URI uri =
          createUri(
              "hdfs",
              NetUtils.createSocketAddr(conf.get(DFS_PROVIDED_ALIASMAP_INMEMORY_RPC_ADDRESS)));
      try {
        aliasMaps.add(
            NameNodeProxies.createProxy(conf, uri, InMemoryAliasMapProtocol.class).getProxy());
  ...
  public InMemoryAliasMap.IterationResult list(Optional<Block> marker) throws IOException {
    ...
    try {
      ...
      if (nextMarker.isInitialized()) {
        return new InMemoryAliasMap.IterationResult(
            fileRegions, Optional.of(PBHelperClient.convert(nextMarker)));
      } else {
  ...
  public Optional<ProvidedStorageLocation> read(@Nonnull Block block) throws IOException {
    ...
    ReadRequestProto request =
        ReadRequestProto.newBuilder().setKey(PBHelperClient.convert(block)).build();
    try {
  ...
  public String getBlockPoolId() throws IOException {
    try {
      BlockPoolResponseProto response =
          rpcProxy.getBlockPoolId(null, BlockPoolRequestProto.newBuilder().build());
