...
/**
 * This class handles the external SPS block movements. This will move the given block to a target
 * datanode by directly establishing socket connection to it and invokes function {@link
...
public class ExternalSPSBlockMoveTaskHandler implements BlockMoveTaskHandler {
  ...
  public ExternalSPSBlockMoveTaskHandler(
      Configuration conf, NameNodeConnector nnc, SPSService spsService) {
    int moverThreads =
        conf.getInt(
    ...
    blkDispatcher =
        new BlockDispatcher(HdfsConstants.READ_TIMEOUT, ioFileBufferSize, connectToDnViaHostname);
  ...
  private ThreadPoolExecutor initializeBlockMoverThreadPool(int num) {
    ...
    ThreadPoolExecutor moverThreadPool =
        new ThreadPoolExecutor(
            ...
            new ThreadPoolExecutor.CallerRunsPolicy() {
              ...
              public void rejectedExecution(Runnable runnable, ThreadPoolExecutor e) {
                LOG.info(
                    "Execution for block movement to satisfy storage policy"
  ...
  private class ExternalBlocksMovementsStatusHandler implements BlocksMovementsStatusHandler {
    ...
    public void handle(BlockMovementAttemptFinished attemptedMove) {
      service.notifyStorageMovementAttemptFinishedBlk(
          attemptedMove.getTargetDatanode(),
          attemptedMove.getTargetType(),
          attemptedMove.getBlock());
    }
  }

  /** This class encapsulates the process of moving the block replica to the given target. */
  private class BlockMovingTask implements Callable<BlockMovementAttemptFinished> {
    ...
    public BlockMovementAttemptFinished call() {
      ...
      return new BlockMovementAttemptFinished(
    ...
    private BlockMovementStatus moveBlock() {
      ...
      try {
        accessToken =
            km.getAccessToken(
                eb, new StorageType[] {blkMovingInfo.getTargetStorageType()}, new String[0]);
      } catch (IOException e) {
