...
class OfflineImageReconstructor {
  public static final Logger LOG = LoggerFactory.getLogger(OfflineImageReconstructor.class);
  ...
  /** The FileSummary builder, where we gather information about each section we wrote. */
  ...
  private OfflineImageReconstructor(CountingOutputStream out, InputStreamReader reader)
    ...
    this.sections.put(ErasureCodingSectionProcessor.NAME, new ErasureCodingSectionProcessor());
    ...
    this.sections.put(
        FilesUnderConstructionSectionProcessor.NAME, new FilesUnderConstructionSectionProcessor());
   ...
   *
   * @param expected The name of the next tag we expect. We will validate that the tag has this
   *     name, unless this string is enclosed in braces.
   * @param allowEnd If true, we will also end events. If false, end events cause an exception.
        ...
        case XMLEvent.CHARACTERS:
          if (!ev.asCharacters().isWhiteSpace()) {
            throw new IOException(
                "Got unxpected characters while "
                    + "looking for "
                    + expected
                    + ": "
            ...
            if (!ev.asStartElement().getName().getLocalPart().equals(expected)) {
              throw new IOException(
                  "Failed to find <"
                      ...
                      + ">; "
                      + "got "
                      + ev.asStartElement().getName().getLocalPart()
                      + " instead.");
            }
          }
          return ev;
  ...
  private void loadNodeChildrenHelper(Node parent, String expected, String terminators[])
      throws IOException {
    XMLEvent ev = null;
    while (true) {
      try {
        ...
        switch (ev.getEventType()) {
          ...
          case XMLEvent.START_ELEMENT:
            ...
            break;
          case XMLEvent.CHARACTERS:
            String val = XMLUtils.unmangleXmlString(ev.asCharacters().getData(), false);
  ...
  /**
   * Load a subtree of the XML into a Node structure. We will keep consuming XML events until we
   * exit the current subtree. If there are any terminators specified, we will always leave the
   * terminating end tag event in the stream.
   *
   * @param parent The node to fill in.
   * @param expected A string to display in exceptions.
   * @param terminators Entering any one of these XML tags terminates our traversal.
   * @throws IOException
   */
  private void loadNodeChildren(Node parent, String expected, String... terminators)
      throws IOException {
    loadNodeChildrenHelper(parent, expected, terminators);
    if (LOG.isTraceEnabled()) {
      LOG.trace(
          "loadNodeChildren(expected="
              + expected
              + ", terminators=["
              + StringUtils.join(",", terminators)
              + "]):"
              + parent.dump());
    }
  }
  ...
  private class NameSectionProcessor implements SectionProcessor {
    ...
    public void process() throws IOException {
      ...
      if (LOG.isDebugEnabled()) {
        LOG.debug(
            SectionName.NS_INFO.name() + " writing header: {" + TextFormat.printToString(s) + "}");
      }
  ...
  private class ErasureCodingSectionProcessor implements SectionProcessor {
    ...
    public void process() throws IOException {
      ...
      while (true) {
        ErasureCodingPolicyProto.Builder policyBuilder = ErasureCodingPolicyProto.newBuilder();
        ...
        Integer cellSize = ec.removeChildInt(ERASURE_CODING_SECTION_POLICY_CELL_SIZE);
        policyBuilder.setCellSize(cellSize);
        String policyState = ec.removeChildStr(ERASURE_CODING_SECTION_POLICY_STATE);
        if (policyState != null) {
          policyBuilder.setState(HdfsProtos.ErasureCodingPolicyState.valueOf(policyState));
        }
        ...
        String codecName = schema.removeChildStr(ERASURE_CODING_SECTION_SCHEMA_CODEC_NAME);
        schemaBuilder.setCodecName(codecName);
        Integer dataUnits = schema.removeChildInt(ERASURE_CODING_SECTION_SCHEMA_DATA_UNITS);
        schemaBuilder.setDataUnits(dataUnits);
        Integer parityUnits = schema.removeChildInt(ERASURE_CODING_SECTION_SCHEMA_PARITY_UNITS);
        schemaBuilder.setParityUnits(parityUnits);
        Node options = schema.removeChild(ERASURE_CODING_SECTION_SCHEMA_OPTIONS);
        if (options != null) {
          while (true) {
            Node option = options.removeChild(ERASURE_CODING_SECTION_SCHEMA_OPTION);
            if (option == null) {
              break;
            }
            String key = option.removeChildStr(ERASURE_CODING_SECTION_SCHEMA_OPTION_KEY);
            String value = option.removeChildStr(ERASURE_CODING_SECTION_SCHEMA_OPTION_VALUE);
            schemaBuilder.addOptions(
  ...
  private INodeSection.INodeFile.Builder createINodeFileBuilder(Node node) throws IOException {
    ...
    if (blockType != null) {
      switch (blockType) {
        ...
        case "STRIPED":
          ...
          break;
        default:
          throw new IOException("INode XML found with unknown <blocktype> " + blockType);
      }
    }
    return bld;
  }
  ...
  private void processDirectoryXml(Node node, INodeSection.INode.Builder inodeBld)
      throws IOException {
    inodeBld.setType(INodeSection.INode.Type.DIRECTORY);
    INodeSection.INodeDirectory.Builder bld = createINodeDirectoryBuilder(node);
    inodeBld.setDirectory(bld);
    // Will check remaining keys and serialize in processINodeXml
  }

  private INodeSection.INodeDirectory.Builder createINodeDirectoryBuilder(Node node)
      throws IOException {
  ...
  private void processSymlinkXml(Node node, INodeSection.INode.Builder inodeBld)
      throws IOException {
  ...
  private INodeSection.AclFeatureProto.Builder aclXmlToProto(Node acls) throws IOException {
    ...
    while (true) {
      ...
      int v =
          ((nameId & ACL_ENTRY_NAME_MASK) << ACL_ENTRY_NAME_OFFSET)
  ...
  private class SecretManagerSectionProcessor implements SectionProcessor {
    ...
    public void process() throws IOException {
      ...
      Integer expectedNumKeys =
          secretHeader.removeChildInt(SECRET_MANAGER_SECTION_NUM_DELEGATION_KEYS);
      ...
      for (int actualNumKeys = 0; actualNumKeys < expectedNumKeys; actualNumKeys++) {
        ...
        if (expiry == null) {
          throw new IOException(
              "Delegation key stanza <delegationKey> " + "lacked an <expiry> field.");
        }
      ...
      for (int actualNumTokens = 0; actualNumTokens < expectedNumTokens; actualNumTokens++) {
        try {
          ...
          throw new IOException(
              "Only read " + actualNumTokens + " tokens out of " + expectedNumTokens, e);
        }
    ...
    public void process() throws IOException {
      ...
      while (actualNumPools < expectedNumPools) {
        try {
          ...
          throw new IOException(
              "Only read " + actualNumPools + " cache pools out of " + expectedNumPools, e);
        }
        actualNumPools++;
        Node pool = new Node();
  ...
  private class SnapshotDiffSectionProcessor implements SectionProcessor {
    ...
    private void processDirDiffEntry() throws IOException {
      ...
      for (int actualDiffs = 0; actualDiffs < expectedDiffs; actualDiffs++) {
        try {
          ...
          throw new IOException(
              "Only read " + (actualDiffs + 1) + " diffs out of " + expectedDiffs, e);
        }
        ...
        Node snapshotCopy = dirDiff.removeChild(SNAPSHOT_DIFF_SECTION_SNAPSHOT_COPY);
        if (snapshotCopy != null) {
          bld.setSnapshotCopy(createINodeDirectoryBuilder(snapshotCopy));
        }
        Integer expectedCreatedListSize =
            dirDiff.removeChildInt(SNAPSHOT_DIFF_SECTION_CREATED_LIST_SIZE);
    ...
    private void processFileDiffEntry() throws IOException {
      ...
      for (int actualDiffs = 0; actualDiffs < expectedDiffs; actualDiffs++) {
        ...
        Node snapshotCopy = fileDiff.removeChild(SNAPSHOT_DIFF_SECTION_SNAPSHOT_COPY);
        if (snapshotCopy != null) {
  ...
  /**
   * Permission is serialized as a 64-bit long. [0:24):[25:48):[48:64) (in Big Endian). The first
   * and the second parts are the string ids of the user and group name, and the last 16 bits are
   * the permission bits.
   *
    ...
    if (components.length != 3) {
      throw new IOException(
          "Unable to parse permission string "
              + perm
              + ": expected 3 components, but only had "
              + components.length);
    }
  ...
  /**
   * The FSImage contains a string table which maps strings to IDs. This is a simple form of
   * compression which takes advantage of the fact that the same strings tend to occur over and over
   * again. This function will return an ID which we can use to represent the given string. If the
   * string already exists in the string table, we will use that ID; otherwise, we will allocate a
   * new one.
   *
   * @param str The string.
   * @return The ID in the string table.
   * @throws IOException If we run out of bits in the string table. We only have 25 bits.
   */
  ...
  /**
   * Record the length of a section of the FSImage in our FileSummary object. The FileSummary
   * appears at the end of the FSImage and acts as a table of contents for the file.
   *
   * @param sectionNamePb The name of the section as it should appear in the fsimage. (This is
   *     different than the XML name.)
   * @throws IOException
   */
  void recordSectionLength(String sectionNamePb) throws IOException {
    ...
    fileSummaryBld.addSections(
        FileSummary.Section.newBuilder()
  ...
  private void readVersion() throws IOException {
    try {
      ...
      // fsimage.
      throw new IOException(
          "No <version> section found at the top of "
    ...
    if (layoutVersion.intValue() != NameNodeLayoutVersion.CURRENT_LAYOUT_VERSION) {
      throw new IOException(
          "Layout version mismatch.  This oiv tool "
              + "handles layout version "
              + NameNodeLayoutVersion.CURRENT_LAYOUT_VERSION
              + ", but the "
              + "XML file has <layoutVersion> "
              + layoutVersion
              + ".  Please "
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          "Loaded <version> with onDiskVersion="
              + onDiskVersion
              + ", layoutVersion="
              + layoutVersion
              + ".");
  ...
  private void writeStringTableSection() throws IOException {
    FsImageProto.StringTableSection sectionHeader =
        FsImageProto.StringTableSection.newBuilder().setNumEntry(stringTable.size()).build();
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          SectionName.STRING_TABLE.name()
              + " writing header: {"
    ...
    while (!unprocessedSections.isEmpty()) {
      ...
      if (ev.getEventType() == XMLStreamConstants.END_ELEMENT) {
        if (ev.asEndElement().getName().getLocalPart().equals("fsimage")) {
          throw new IOException(
              "FSImage XML ended prematurely, without "
                  + "including section(s) "
                  + StringUtils.join(", ", unprocessedSections));
        }
        throw new IOException(
            "Got unexpected tag end event for "
