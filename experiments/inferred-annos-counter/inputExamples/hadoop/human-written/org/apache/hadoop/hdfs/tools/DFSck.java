 ...
 *       <ul>
 *         <li>move corrupted files to /lost+found directory on DFS ({@link
 *             org.apache.hadoop.hdfs.server.namenode.NamenodeFsck#doMove}). Remaining data blocks
 *             are saved as a block chains, representing longest consecutive series of valid blocks.
 *         <li>delete corrupted files ({@link
 *             org.apache.hadoop.hdfs.server.namenode.NamenodeFsck#doDelete})
 *       </ul>
 ...
 * detailed statistics on block locations and replication factors of each file. The tool also
 * provides and option to filter open files during the scan.
 */
...
public class DFSck extends Configured implements Tool {
      ...
      try {
        ...
        while ((line = input.readLine()) != null) {
          ...
          if ((line.isEmpty())
              ...
              || (line.startsWith("The filesystem under path"))) continue;
          numCorrupt++;
          if (numCorrupt == 1) {
  ...
  /**
   * Derive the namenode http address from the current file system, either default or as set by
   * "-fs" in the generic options.
   *
  ...
  private URI getCurrentNamenodeAddress(Path target) throws IOException {
    ...
    return DFSUtil.getInfoServer(
        HAUtil.getAddressOfActive(fs), conf, DFSUtil.getHttpClientScheme(conf));
  }
  ...
  private int doWork(final String[] args) throws IOException {
    ...
    url.append("&path=")
        .append(
            URLEncoder.encode(Path.getPathWithoutSchemeAndAuthority(dirpath).toString(), "UTF-8"));
