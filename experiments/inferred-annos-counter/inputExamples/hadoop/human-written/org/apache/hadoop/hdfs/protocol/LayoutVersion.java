 ...
 *
 * <p>Layout version is changed for following reasons:
 *
 * <ol>
 *   <li>The layout of how namenode or datanode stores information on disk changes.
 *   <li>A new operation code is added to the editlog.
 *   <li>Modification such as format of a record, content of a record in editlog or fsimage.
 * </ol>
 *
 * <br>
 * <b>How to update layout version:<br>
 * </b> When a change requires new layout version, please add an entry into {@link Feature} with a
 * short enum name, new layout version and description of the change. Please see {@link Feature} for
 * further details. <br>
 */
@InterfaceAudience.Private
public class LayoutVersion {
  /**
   * Version in which HDFS-2991 was fixed. This bug caused OP_ADD to sometimes be skipped for
   * append() calls. If we see such a case when loading the edits, but the version is known to have
   * that bug, we workaround the issue. Otherwise we should consider it a corruption and bail.
   */
  ...
  /**
   * Enums for features that change the layout version before rolling upgrade is supported. <br>
   * <br>
   * To add a new layout version:
   *
   * <ul>
   *   <li>Define a new enum constant with a short enum name, the new layout version and description
   *       of the added feature.
   *   <li>When adding a layout version with an ancestor that is not same as its immediate
   *       predecessor, use the constructor where a specific ancestor can be passed.
   * </ul>
   */
  public enum Feature implements LayoutFeature {
    ...
    OPTIMIZE_PERSIST_BLOCKS(
        -40,
        "Serialize block lists with delta-encoded variable length ints, " + "add OP_UPDATE_BLOCKS"),
    ...
    OPTIMIZE_SNAPSHOT_INODES(-45, -43, "Reduce snapshot inode memory footprint", false),
    SEQUENTIAL_BLOCK_ID(
        -46,
     ...
     * @param description description of the feature
     * @param reserved true when this is a layout version reserved for previous version
  ...
  public static class FeatureInfo {
    ...
    /**
     * Accessor method for feature minimum compatible layout version. If the feature does not define
     * a minimum compatible layout version, then this method returns the feature's own layout
     * version. This would indicate that the feature cannot provide compatibility with any prior
     * layout version.
     *
  ...
  public static void updateMap(
      Map<Integer, SortedSet<LayoutFeature>> map, LayoutFeature[] features) {
    // Go through all the enum constants and build a map of
    // LayoutVersion <-> Set of all supported features in that LayoutVersion
    SortedSet<LayoutFeature> existingFeatures =
        new TreeSet<LayoutFeature>(new LayoutFeatureComparator());
        ...
        throw new AssertionError(
            String.format(
                "Features must be listed in order of minimum compatible layout "
                    + "version.  Check features %s and %s.",
                prevF, f));
      }
      prevF = f;
  ...
  public String getString(Map<Integer, SortedSet<LayoutFeature>> map, LayoutFeature[] values) {
    ...
    for (LayoutFeature f : values) {
      final FeatureInfo info = f.getInfo();
      buf.append(f)
          .append(" introduced in layout version ")
          .append(info.getLayoutVersion())
          .append(" (")
    ...
    for (LayoutFeature f : values) {
      final FeatureInfo info = f.getInfo();
      buf.append(info.getLayoutVersion())
          .append(": ")
          .append(map.get(info.getLayoutVersion()))
          .append("\n");
    }
    return buf.toString();
  }
  ...
  public static boolean supports(
      Map<Integer, SortedSet<LayoutFeature>> map, final LayoutFeature f, final int lv) {
