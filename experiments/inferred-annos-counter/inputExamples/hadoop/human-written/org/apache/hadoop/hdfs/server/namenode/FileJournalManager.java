...
public class FileJournalManager implements JournalManager {
  ...
  private static final Pattern EDITS_REGEX =
      Pattern.compile(NameNodeFile.EDITS.getName() + "_(\\d+)-(\\d+)");
  private static final Pattern EDITS_INPROGRESS_REGEX =
      Pattern.compile(NameNodeFile.EDITS_INPROGRESS.getName() + "_(\\d+)");
  private static final Pattern EDITS_INPROGRESS_STALE_REGEX =
      Pattern.compile(NameNodeFile.EDITS_INPROGRESS.getName() + "_(\\d+).*(\\S+)");

  @VisibleForTesting File currentInProgress = null;

  /**
   * A FileJournalManager should maintain the largest Tx ID that has been safely written to its edit
   * log files. It should limit readers to read beyond this ID to avoid potential race with ongoing
   * writers. Initial value indicates that all transactions can be read.
   */
  private long lastReadableTxId = Long.MAX_VALUE;

  @VisibleForTesting StoragePurger purger = new NNStorageRetentionManager.DeletionStoragePurger();

  public FileJournalManager(
      Configuration conf, StorageDirectory sd, StorageErrorReporter errorReporter) {
    this.conf = conf;
    this.sd = sd;
  ...
  @Override
  public synchronized EditLogOutputStream startLogSegment(long txid, int layoutVersion)
      throws IOException {
    try {
   ...
   *
   * <p>Edits files that are ending before the minTxIdToKeep are purged. Edits in progress files
   * that are starting before minTxIdToKeep are purged. Edits in progress files that are marked as
   * empty, trash, corrupted or stale by file extension and starting before minTxIdToKeep are
   * purged. Edits in progress files that are after minTxIdToKeep, but before the current edits in
   * progress files are marked as stale for clarity.
   *
   * <p>In case file removal or rename is failing a warning is logged, but that does not fail the
   * operation.
   *
   ...
   * @param inProgressOk whether or not to include the in-progress edit log segment
   * @return a list of remote edit logs
   * @throws IOException if edit logs cannot be listed.
   */
  public List<RemoteEditLog> getRemoteEditLogs(long firstTxId, boolean inProgressOk)
      throws IOException {
    ...
    for (EditLogFile elf : allLogFiles) {
      ...
      if (elf.isInProgress()) {
        try {
          ...
          LOG.error(
              "got IOException while trying to validate header of " + elf + ".  Skipping.", e);
          continue;
        }
      }
  ...
  /**
   * Discard all editlog segments whose first txid is greater than or equal to the given txid, by
   * renaming them with suffix ".trash".
   */
  ...
  /**
   * returns matching edit logs via the log directory. Simple helper function that lists the files
   * in the logDir and calls matchEditLogs(File[])
   *
    ...
    for (File f : filesInStorage) {
      ...
      if (inProgressEditsMatch.matches()) {
        try {
          ...
          LOG.error(
              "In-progress edits file " + f + " has improperly " + "formatted transaction ID");
          // skip
        }
      }
      if (forPurging) {
  ...
  public synchronized void selectInputStreams(
  ...
  public synchronized void selectInputStreams(
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          this
              + ": selecting input streams starting at "
              + fromTxId
  ...
  static void addStreamsToCollectionFromFiles(
      Collection<EditLogFile> elfs,
      Collection<EditLogInputStream> streams,
      long fromTxId,
      long maxTxIdToScan,
      boolean inProgressOk) {
        ...
        if (LOG.isDebugEnabled()) {
          LOG.debug(
              "passing over "
                  ...
                  + " because it ends at "
                  + elf.lastTxId
                  + ", but we only care about transactions "
                  + "as new as "
                  + fromTxId);
        }
        continue;
      }
      EditLogFileInputStream elfis =
          new EditLogFileInputStream(
              elf.getFile(), elf.getFirstTxId(), elf.getLastTxId(), elf.isInProgress());
  ...
  public static EditLogFile getLogFile(File dir, long startTxId, boolean inProgressOk)
      throws IOException {
  ...
  public static class EditLogFile {
    ...
    EditLogFile(File file, long firstTxId, long lastTxId) {
      ...
      assert (lastTxId != HdfsServerConstants.INVALID_TXID) && (lastTxId >= firstTxId);
    }
    ...
    EditLogFile(File file, long firstTxId, long lastTxId, boolean isInProgress) {
      ...
      Preconditions.checkArgument(!isInProgress || lastTxId == HdfsServerConstants.INVALID_TXID);
    ...
    /**
     * Find out where the edit log ends. This will update the lastTxId of the EditLogFile or mark it
     * as corrupt if it is.
     *
    ...
    public void scanLog(long maxTxIdToScan, boolean verifyVersion) throws IOException {
      EditLogValidation val =
          EditLogFileInputStream.scanEditLog(file, maxTxIdToScan, verifyVersion);
    ...
    public String toString() {
      return String.format(
          "EditLogFile(file=%s,first=%019d,last=%019d," + "inProgress=%b,hasCorruptHeader=%b)",
  ...
  public void doPreUpgrade() throws IOException {
    ...
    try {
      ...
      LOG.error(
          "Failed to move aside pre-upgrade storage " + "in image directory " + sd.getRoot(), ioe);
      throw ioe;
    }
  }

  /**
   * This method assumes that the fields of the {@link Storage} object have already been updated to
   * the appropriate new values for the upgrade.
   */
  @Override
