...
public class DatanodeStorageInfo {
  ...
  /** Set to false on any NN failover, and reset to true whenever a block report is received. */
  private boolean heartbeatedSinceFailover = false;

  /**
   * At startup or at failover, the storages in the cluster may have pending block deletions from a
   * previous incarnation of the NameNode. The block contents are considered as stale until a block
   * report is received. When a storage is considered as stale, the replicas on it are also
   * considered as stale. If any block has at least one stale replica, then no invalidations will be
   * processed for this block. See HDFS-1972.
   */
  ...
  DatanodeStorageInfo(
      DatanodeDescriptor dn, String storageID, StorageType storageType, State state) {
    this.dn = dn;
  ...
  @VisibleForTesting
  public void setUtilizationForTesting(
  ...
  /**
   * For use during startup. Expects block to be added in sorted order to enable fast insert in to
   * the DatanodeStorageInfo
   *
  ...
  /**
   * Decrement the number of blocks scheduled for each given storage. This will be called during
   * abandon block or delete of UC block.
   */
  ...
  /**
   * @return the first {@link DatanodeStorageInfo} corresponding to the given datanode
   */
  static DatanodeStorageInfo getDatanodeStorageInfo(
      final Iterable<DatanodeStorageInfo> infos, final DatanodeDescriptor datanode) {
    if (datanode == null) {
      return null;
    }
