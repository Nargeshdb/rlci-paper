...
class FSDirDeleteOp {
    ...
    try {
      if (deleteAllowed(iip)) {
        ...
        ReclaimContext context =
            new ReclaimContext(
                fsd.getBlockStoragePolicySuite(), collectedBlocks, removedINodes, removedUCFiles);
   ...
   *
   * <p>For large directories, deletion is incremental. The blocks under the directory are collected
   * and deleted a small number at a time holding the {@link FSNamesystem} lock.
   *
   * <p>For small directory or file the deletion is done in one shot.
   *
   * @param fsn namespace
  ...
  /**
   * Delete a path from the name space Update the count at each ancestor directory with quota <br>
   * Note: This is to be used by {@link org.apache.hadoop.hdfs.server.namenode.FSEditLog} only. <br>
   *
   * @param fsd the FSDirectory instance
   ...
   *
   * <p>For large directories, deletion is incremental. The blocks under the directory are collected
   * and deleted a small number at a time holding the {@link
   * org.apache.hadoop.hdfs.server.namenode.FSNamesystem} lock.
   *
   * <p>For small directory or file the deletion is done in one shot.
   *
   * @param fsn namespace
  ...
  static BlocksMapUpdateInfo deleteInternal(
      FSNamesystem fsn, INodesInPath iip, boolean logRetryCache) throws IOException {
   ...
   */
  private static boolean unprotectedDelete(
      FSDirectory fsd, INodesInPath iip, ReclaimContext reclaimContext, long mtime) {
    assert fsd.hasWriteLock();
    ...
    if (NameNode.stateChangeLog.isDebugEnabled()) {
      NameNode.stateChangeLog.debug(
          "DIR* FSDirectory.unprotectedDelete: " + iip.getPath() + " is removed");
    }
    return true;
  }
}
