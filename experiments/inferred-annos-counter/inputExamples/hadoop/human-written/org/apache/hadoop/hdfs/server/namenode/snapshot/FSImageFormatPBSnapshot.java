...
public class FSImageFormatPBSnapshot {
  ...
  public static final class Loader {
    ...
    /**
     * Load the snapshots section from fsimage. Also add snapshottable feature to snapshottable
     * directories.
     */
    public void loadSnapshotSection(InputStream in) throws IOException {
      ...
      for (long sdirId : section.getSnapshottableDirList()) {
        ...
        if (!dir.isSnapshottable()) {
          ...
          dir.setSnapshotQuota(DirectorySnapshottableFeature.SNAPSHOT_QUOTA_DEFAULT);
        }
    ...
    private void loadFileDiffList(InputStream in, INodeFile file, int size) throws IOException {
      ...
      for (int i = 0; i < size; i++) {
        ...
        if (pbf.hasSnapshotCopy()) {
          ...
          boolean isStriped = (fileInPb.getBlockType() == BlockTypeProto.STRIPED);
          Short replication = (!isStriped ? (short) fileInPb.getReplication() : null);
          Byte ecPolicyID = (isStriped ? (byte) fileInPb.getErasureCodingPolicyID() : null);
          copy =
    ...
    private List<INode> loadCreatedList(InputStream in, INodeDirectory dir, int size)
        throws IOException {
    ...
    private List<INode> loadDeletedList(
        final List<INodeReference> refList,
        InputStream in,
        INodeDirectory dir,
        List<Long> deletedNodes,
    ...
    private void loadDirectoryDiffList(
      ...
      for (int i = 0; i < size; i++) {
        ...
        if (useRoot) {
          ...
          boolean noQuota =
              dirCopyInPb.getNsQuota() == -1
                  && dirCopyInPb.getDsQuota() == -1
                  && (!dirCopyInPb.hasTypeQuotas());

          if (noQuota) {
            copy =
                new INodeDirectoryAttributes.SnapshotCopy(name, permission, acl, modTime, xAttrs);
          } else {
            ...
            if (dirCopyInPb.hasTypeQuotas()) {
              ...
              typeQuotas =
                  new EnumCounters<StorageType>(StorageType.class, HdfsConstants.QUOTA_RESET);
  ...
  public static final class Saver {
    ...
    public Saver(
        FSImageFormatProtobuf.Saver parent,
    ...
    public void serializeINodeReferenceSection(OutputStream out) throws IOException {
      ...
      for (INodeReference ref : refList) {
        INodeReferenceSection.INodeReference.Builder rb = buildINodeReference(ref, i++);
      ...
      if (fsn.getFSDirectory().getInode(ref.getId()) == null) {
        FSImage.LOG.error(
            "FSImageFormatPBSnapshot: Missing referred INodeId "
                + ref.getId()
                + " for INodeReference index "
                + refIndex
                + "; path="
                + ref.getFullPathName()
                + "; parent="
                + (ref.getParent() == null ? "null" : ref.getParent().getFullPathName()));
        ++numImageErrors;
      }
      return rb;
    }
    ...
    private void saveCreatedList(List<INode> created, OutputStream out) throws IOException {
      ...
      for (INode c : created) {
        SnapshotDiffSection.CreatedListEntry.newBuilder()
            .setName(ByteString.copyFrom(c.getLocalNameBytes()))
            .build()
      ...
      if (sf != null) {
        ...
        for (int i = diffList.size() - 1; i >= 0; i--) { // reverse order!
          ...
          for (INode d : deleted) {
            ...
            final int result =
                previousNode == null ? -1 : previousNode.compareTo(d.getLocalNameBytes());
            if (result == 0) {
              FSImage.LOG.error(
                  "Name '"
                      + d.getLocalName()
    ...
    /**
     * Number of non-fatal errors detected while writing the SnapshotDiff and INodeReference
     * sections.
     *
