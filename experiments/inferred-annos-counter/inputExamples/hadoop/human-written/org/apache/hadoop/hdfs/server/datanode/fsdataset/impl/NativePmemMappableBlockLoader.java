...
public class NativePmemMappableBlockLoader extends PmemMappableBlockLoader {
   ...
   * Load the block.
   *
   * <p>Map the block and verify its checksum.
   *
   * <p>The block will be mapped to PmemDir/BlockPoolId/subdir#/subdir#/BlockId, in which PmemDir is
   * a persistent memory volume chosen by PmemVolumeManager.
   *
   * @param length The current length of the block.
   * @param blockIn The block input stream. Should be positioned at the start. The caller must close
   *     this.
   * @param metaIn The meta file input stream. Should be positioned at the start. The caller must
   *     close this.
    ...
    try {
      ...
      if (region == null) {
        throw new IOException(
            "Failed to map the block " + blockFileName + " to persistent storage.");
      }
      ...
      LOG.info(
          "Successfully cached one replica:{} into persistent memory"
              + ", [cached path={}, address={}, length={}]",
          key,
          filePath,
          region.getAddress(),
          length);
      ...
      if (mappableBlock == null) {
        if (region != null) {
          ...
          POSIX.Pmem.unmapBlock(region.getAddress(), region.getLength());
  ...
  /**
   * Verifies the block's checksum meanwhile map block to persistent memory. This is an I/O
   * intensive operation.
   */
  private void verifyChecksumAndMapBlock(
      POSIX.PmemMappedRegion region,
      long length,
      FileInputStream metaIn,
    ...
    BlockMetadataHeader header =
        BlockMetadataHeader.readHeader(
            new DataInputStream(
                new BufferedInputStream(metaIn, BlockMetadataHeader.getHeaderSize())));
    ...
    try {
      ...
      while (bytesVerified < length) {
        Preconditions.checkState(
            bytesVerified % bytesPerChecksum == 0, "Unexpected partial chunk before EOF.");
        ...
        POSIX.Pmem.memCopy(blockBuf.array(), mappedAddress, region.isPmem(), bytesRead);
  ...
  @Override
  public MappableBlock getRecoveredMappableBlock(File cacheFile, String bpid, byte volumeIndex)
      throws IOException {
    NativeIO.POSIX.PmemMappedRegion region =
        NativeIO.POSIX.Pmem.mapBlock(cacheFile.getAbsolutePath(), cacheFile.length(), true);
    if (region == null) {
      throw new IOException(
          "Failed to recover the block " + cacheFile.getName() + " in persistent storage.");
    }
    ...
    LOG.info(
        "Recovering persistent memory cache for block {}, "
            + "path = {}, address = {}, length = {}",
        key,
        path,
        addr,
