...
class ReplicaMap {
  ...
  /** Add a replica's meta information into the map, if already exist return the old replicaInfo. */
  ...
  void mergeAll(ReplicaMap other) {
    other.map.forEach(
        (bp, replicaInfos) -> {
          replicaInfos.forEach(replicaInfo -> add(bp, replicaInfo));
        });
  }

  /**
   * Remove the replica's meta information from the map that matches the input block's id and
   * generation stamp
   *
   * @param bpid block pool id
   * @param block block with its id as the key
  ...
  /**
   * Get a collection of the replicas for given block pool This method is <b>not synchronized</b>.
   * It needs to be synchronized externally using the lock, both for getting the replicas values
   * from the map and iterating over it. Mutex can be accessed using {@link #getLock()} method.
   *
   * @param bpid block pool id
