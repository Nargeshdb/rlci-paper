...
public abstract class FSTreeTraverser {
  ...
  /**
   * Iterate through all files directly inside parent, and recurse down directories. The listing is
   * done in batch, and can optionally start after a position. The iteration of the inode tree is
   * done in a depth-first fashion. But instead of holding all {@link INodeDirectory}'s in memory on
   * the fly, only the path components to the current inode is held. This is to reduce memory
   * consumption.
   *
   * @param parent The inode id of parent directory
   * @param startId Id of the start inode.
   * @param startAfter Full path of a file the traverse should start after.
   * @param traverseInfo info which may required for processing the child's.
   * @throws IOException
  ...
  protected void traverseDir(
      ...
      final long startId,
      byte[] startAfter,
      final TraverseInfo traverseInfo)
  ...
  /**
   * Iterates the parent directory, and add direct children files to current batch. If batch size
   * meets configured threshold, current batch will be submitted for the processing.
   *
   * <p>Locks could be released and reacquired when a batch submission is finished.
   *
   * @param startId Id of the start inode.
   * @return The inode which was just processed, if lock is held in the entire process. Null if lock
   *     is released.
   * @throws IOException
   * @throws InterruptedException
   */
  protected INode traverseDirInt(
      final long startId, INode curr, List<byte[]> startAfters, final TraverseInfo traverseInfo)
      throws IOException, InterruptedException {
    ...
    final INodeDirectory parent = curr.isDirectory() ? curr.asDirectory() : curr.getParent();
    ReadOnlyList<INode> children = parent.getChildrenList(Snapshot.CURRENT_STATE_ID);
    ...
    for (int i = INodeDirectory.nextChild(children, startAfter); i < children.size(); ++i) {
      ...
      if (shouldSubmitCurrentBatch()) {
        ...
        INode newParent = dir.resolvePath(pc, parentPath, FSDirectory.DirOp.READ).getLastINode();
      ...
      if ((timer.monotonicNow() - lockStartTime) > readLockReportingThresholdMs) {
        readUnlock();
        try {
          throttle();
        } finally {
   ...
   *
   * <p>The parent of the lowest level startAfter is returned. If somewhere in the middle of
   * startAfters changed, the parent of the lowest unchanged level is returned.
   *
   * @param startId Id of the start inode.
   * @param startAfters the cursor, represented by a list of path bytes.
   * @return the parent inode corresponding to the startAfters, or null if the furthest parent is
   *     deleted.
   */
  private INode resolvePaths(final long startId, List<byte[]> startAfters) throws IOException {
    ...
    for (int i = 0; i < startAfters.size(); ++i) {
      ...
      INode curr = parent.getChild(startAfters.get(i), Snapshot.CURRENT_STATE_ID);
      if (curr == null) {
   ...
   * @return true if inode is added to currentBatch and should be process for next operation. false
   *     otherwise: could be inode is not a file.
   * @throws IOException
   * @throws InterruptedException
   */
  protected abstract boolean processFileInode(INode inode, TraverseInfo traverseInfo)
      throws IOException, InterruptedException;
