...
/**
 * StripedBlockChecksumReconstructor reconstruct one or more missed striped block in the striped
 * block group, the minimum number of live striped blocks should be no less than data block number.
 * Then checksum will be recalculated using the newly reconstructed block.
 */
...
public abstract class StripedBlockChecksumReconstructor extends StripedReconstructor {
  ...
  protected StripedBlockChecksumReconstructor(
  ...
  public void reconstruct() throws IOException {
    ...
    try {
      while (requestedLen > 0 && getPositionInBlock() < maxTargetLength) {
        ...
        checksumDataLen += checksumWithTargetOutput(targetBuffer.array(), toReconstructLen);
  ...
  /**
   * Should return a representation of a completed/reconstructed digest which is suitable for debug
   * printing.
   */
  ...
  /**
   * This will be called repeatedly with chunked checksums computed in-flight over reconstructed
   * data.
   *
   * @param dataBytesPerChecksum the number of underlying data bytes corresponding to each checksum
   *     inside {@code checksumBytes}.
   */
  abstract void updateDigester(byte[] checksumBytes, int dataBytesPerChecksum) throws IOException;

  /**
   * This will be called when reconstruction of entire requested length is complete and any final
   * digests should be committed to implementation-specific output fields.
   */
  ...
  private long checksumWithTargetOutput(byte[] outputData, int toReconstructLen)
    ...
    if (requestedLen <= toReconstructLen) {
      ...
      int checksumRemaining =
          (remainingLen / getChecksum().getBytesPerChecksum()) * getChecksum().getChecksumSize();
