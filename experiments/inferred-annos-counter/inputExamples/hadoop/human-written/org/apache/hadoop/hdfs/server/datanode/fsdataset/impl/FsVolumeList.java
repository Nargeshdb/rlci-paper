...
class FsVolumeList {
  ...
  private final Map<StorageLocation, VolumeFailureInfo> volumeFailureInfos =
      Collections.synchronizedMap(new TreeMap<StorageLocation, VolumeFailureInfo>());
  ...
  FsVolumeList(
      List<VolumeFailureInfo> initialVolumeFailureInfos,
  ...
  private FsVolumeReference chooseVolume(List<FsVolumeImpl> list, long blockSize, String storageId)
      throws IOException {
    while (true) {
      FsVolumeImpl volume = blockChooser.chooseVolume(list, blockSize, storageId);
      try {
   ...
   * @param storageId the storage id which may or may not be used by the VolumeChoosingPolicy.
   * @return next volume to store the block in.
   */
  FsVolumeReference getNextVolume(StorageType storageType, String storageId, long blockSize)
      throws IOException {
  ...
  void getAllVolumesMap(
      final String bpid, final ReplicaMap volumeMap, final RamDiskReplicaTracker ramDiskReplicaMap)
      throws IOException {
            ...
            public void run() {
              try (FsVolumeReference ref = v.obtainReference()) {
                FsDatasetImpl.LOG.info(
                    "Adding replicas to map for block pool " + bpid + " on volume " + v + "...");
                ...
                FsDatasetImpl.LOG.info(
                    "Caught exception while adding replicas " + "from " + v + ". Will throw later.",
                    ioe);
    ...
    FsDatasetImpl.LOG.info(
        "Total time to add all replicas to map for block pool "
            + bpid
            + ": "
            + totalTimeTaken
            + "ms");
  ...
  /**
   * Updates the failed volume info in the volumeFailureInfos Map and calls {@link
   * #removeVolume(FsVolumeImpl)} to remove the volume from the volume list for each of the failed
   * volumes.
   *
      ...
      for (FsVolumeSpi vol : failedVolumes) {
        ...
        try (FsVolumeReference ref = fsv.obtainReference()) {
          ...
          FsDatasetImpl.LOG.debug(
              "Caught exception when obtaining " + "reference count on closed volume", e);
        } catch (IOException e) {
  ...
  /**
   * Wait for the reference of the volume removed from a previous {@link
   * #removeVolume(FsVolumeImpl)} call to be released.
   *
  ...
  void waitVolumeRemoved(int sleepMillis, Condition condition) {
    while (!checkVolumesRemoved()) {
      ...
      try {
        ...
        FsDatasetImpl.LOG.info(
            "Thread interrupted when waiting for " + "volume reference to be released.");
  ...
  /**
   * Check whether the reference of the volume from a previous {@link #removeVolume(FsVolumeImpl)}
   * call is released.
   *
  ...
  void addVolumeFailureInfo(VolumeFailureInfo volumeFailureInfo) {
    ...
    if (!volumeFailureInfos.containsKey(volumeFailureInfo.getFailedStorageLocation())) {
      volumeFailureInfos.put(volumeFailureInfo.getFailedStorageLocation(), volumeFailureInfo);
    }
  }

  private void addVolumeFailureInfo(FsVolumeImpl vol) {
    addVolumeFailureInfo(
        new VolumeFailureInfo(vol.getStorageLocation(), Time.now(), vol.getCapacity()));
  }
  ...
  void addBlockPool(final String bpid, final Configuration conf) throws IOException {
          ...
          new Thread() {
            public void run() {
              try (FsVolumeReference ref = v.obtainReference()) {
                ...
                long timeTaken = Time.monotonicNow() - startTime;
                FsDatasetImpl.LOG.info(
                    "Time taken to scan block pool " + bpid + " on " + v + ": " + timeTaken + "ms");
              } catch (IOException ioe) {
                FsDatasetImpl.LOG.info(
                    "Caught exception while scanning " + v + ". Will throw later.", ioe);
                unhealthyDataDirs.put(v, ioe);
              }
            }
          };
    ...
    long totalTimeTaken = Time.monotonicNow() - totalStartTime;
    FsDatasetImpl.LOG.info(
        "Total time to scan all replicas for block pool " + bpid + ": " + totalTimeTaken + "ms");
    if (!unhealthyDataDirs.isEmpty()) {
      throw new AddBlockPoolException(unhealthyDataDirs);
    }
  }

  void removeBlockPool(String bpid, Map<DatanodeStorage, BlockListAsLongs> blocksPerVolume) {
    for (FsVolumeImpl v : volumes) {
