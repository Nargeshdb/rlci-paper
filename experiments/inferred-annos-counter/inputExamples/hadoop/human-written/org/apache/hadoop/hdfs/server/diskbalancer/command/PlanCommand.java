 ...
 *
 * <p>Plan command reads the Cluster Info and creates a plan for specified data node or a set of
 * Data nodes.
 *
 * <p>It writes the output to a default location unless changed by the user.
 */
public class PlanCommand extends Command {
  ...
  public PlanCommand(Configuration conf, final PrintStream ps) {
    ...
    addValidCommandParameters(
        DiskBalancerCLI.OUTFILE,
        "Output directory in "
            + "HDFS. The generated plan will be written to a file in this "
            + "directory.");
    addValidCommandParameters(
        DiskBalancerCLI.BANDWIDTH, "Maximum Bandwidth to be used while copying.");
    addValidCommandParameters(
        DiskBalancerCLI.THRESHOLD,
    ...
    addValidCommandParameters(DiskBalancerCLI.VERBOSE, "Run plan command in " + "verbose mode.");
   ...
   *
   * <p>-plan -node IP -plan -node hostName -plan -node DatanodeUUID
   *
  ...
  public void execute(CommandLine cmd) throws Exception {
    ...
    if (node == null) {
      throw new IllegalArgumentException(
          "Unable to find the specified node. " + cmd.getOptionValue(DiskBalancerCLI.PLAN));
    }

    try (FSDataOutputStream beforeStream =
        create(
            String.format(
                DiskBalancerCLI.BEFORE_TEMPLATE, cmd.getOptionValue(DiskBalancerCLI.PLAN)))) {
    ...
    try {
      if (plan != null && plan.getVolumeSetPlans().size() > 0) {
        ...
        final String planFileName =
            String.format(DiskBalancerCLI.PLAN_TEMPLATE, cmd.getOptionValue(DiskBalancerCLI.PLAN));
        ...
        outputLine =
            String.format(
                "No plan generated. DiskBalancing not needed for node: %s" + " threshold used: %s",
