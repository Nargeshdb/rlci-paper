 ...
 *
 * <p>Adding more options:
 *
 * <ul>
 *   <li>If adding a simple option to get a value corresponding to a key in the configuration, use
 *       regular {@link GetConf.CommandHandler}. See {@link GetConf.Command#EXCLUDE_FILE} example.
 *   <li>If adding an option that is does not return a value for a key, add a subclass of {@link
 *       GetConf.CommandHandler} and set it up in {@link GetConf.Command}.
 *       <p>See {@link GetConf.Command#NAMENODE} for example.
 *       <p>Add for the new option added, a map entry with the corresponding {@link
...
public class GetConf extends Configured implements Tool {
  ...
  enum Command {
    ...
    static {
      ...
      map.put(StringUtils.toLowerCase(JOURNALNODE.getName()), new JournalNodeCommandHandler());
      map.put(
          StringUtils.toLowerCase(INCLUDE_FILE.getName()),
          new CommandHandler(DFSConfigKeys.DFS_HOSTS));
      map.put(
          StringUtils.toLowerCase(EXCLUDE_FILE.getName()),
          new CommandHandler(DFSConfigKeys.DFS_HOSTS_EXCLUDE));
      map.put(
          StringUtils.toLowerCase(NNRPCADDRESSES.getName()), new NNRpcAddressesCommandHandler());
  ...
  static class JournalNodeCommandHandler extends CommandHandler {
    @Override
    public int doWorkInternal(GetConf tool, String[] args) throws URISyntaxException, IOException {
  ...
  /**
   * Handler for {@link Command#NNRPCADDRESSES} If rpc addresses are defined in configuration, we
   * return them. Otherwise, return empty string.
   */
  static class NNRpcAddressesCommandHandler extends CommandHandler {
    @Override
    public int doWorkInternal(GetConf tool, String[] args) throws IOException {
      Configuration config = tool.getConf();
      List<ConfiguredNNAddress> cnnlist =
