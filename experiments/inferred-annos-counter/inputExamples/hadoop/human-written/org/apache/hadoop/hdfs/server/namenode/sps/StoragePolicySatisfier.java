...
/**
 * Setting storagePolicy on a file after the file write will only update the new storage policy type
 * in Namespace, but physical block storage movement will not happen until user runs "Mover Tool"
 * explicitly for such files. The StoragePolicySatisfier Daemon thread implemented for addressing
 * the case where users may want to physically move the blocks by a dedicated daemon (can run inside
 * Namenode or stand alone) instead of running mover tool explicitly. Just calling client API to
 * satisfyStoragePolicy on a file/dir will automatically trigger to move its physical storage
 * locations as expected in asynchronous manner. Here SPS will pick the file blocks which are
 * expecting to change its storages, then it will build the mapping of source block location and
 * expected storage type and location to move. After that this class will also prepare requests to
 * send to Datanode for processing the physical block movements.
 */
...
public class StoragePolicySatisfier implements SPSService, Runnable {
  ...
  public void init(final Context context) {
    ...
    this.storageMovementsMonitor =
        new BlockStorageMovementAttemptedItems(this, storageMovementNeeded, context);
    this.spsWorkMultiplier = getSPSWorkMultiplier(getConf());
    this.blockMovementMaxRetry =
        getConf()
            .getInt(
  ...
  /**
   * Start storage policy satisfier demon thread. Also start block storage movements monitor for
   * retry the attempts if needed.
   */
  @Override
  public synchronized void start(StoragePolicySatisfierMode serviceMode) {
    if (serviceMode == StoragePolicySatisfierMode.NONE) {
      LOG.error("Can't start StoragePolicySatisfier for the given mode:{}", serviceMode);
      return;
    }
    LOG.info(
        "Starting {} StoragePolicySatisfier.", StringUtils.toLowerCase(serviceMode.toString()));
    isRunning = true;
    ...
    if (storagePolicySatisfierThread != null) {
      try {
        ...
        if (LOG.isDebugEnabled()) {
          LOG.debug("Interrupted Exception while waiting to join sps thread," + " ignoring it", ie);
        }
      }
    }
  }
  ...
  public void run() {
    while (isRunning) {
      ...
      try {
        ...
        if (!ctxt.isInSafeMode()) {
          ...
          if (itemInfo != null) {
            ...
            if (fileStatus == null || fileStatus.isDir()) {
              ...
              existingStoragePolicy = ctxt.getStoragePolicy(existingStoragePolicyID);
        ...
        if (storageMovementNeeded.size() == 0 || blockCount > (numLiveDn * spsWorkMultiplier)) {
          Thread.sleep(3000);
          blockCount = 0L;
        }
        if (retryItem) {
        ...
        LOG.error(
            "Exception during StoragePolicySatisfier execution - " + "will continue next cycle", e);
  ...
  private BlocksMovingAnalysis analyseBlocksStorageMovementsAndAssignToDN(
      HdfsLocatedFileStatus fileInfo, BlockStoragePolicy existingStoragePolicy) throws IOException {
    ...
    if (!lastBlkComplete) {
      ...
      LOG.info(
          "File: {} is under construction. So, postpone" + " this to the next retry iteration",
          fileInfo.getFileId());
      return new BlocksMovingAnalysis(
          BlocksMovingAnalysis.Status.ANALYSIS_SKIPPED_FOR_RETRY, new HashMap<>());
    }

    List<LocatedBlock> blocks = locatedBlocks.getLocatedBlocks();
    if (blocks.size() == 0) {
      LOG.info(
          "File: {} is not having any blocks." + " So, skipping the analysis.",
          fileInfo.getFileId());
      return new BlocksMovingAnalysis(
          BlocksMovingAnalysis.Status.BLOCKS_TARGET_PAIRING_SKIPPED, new HashMap<>());
    }
    ...
    for (int i = 0; i < blocks.size(); i++) {
      ...
      hasLowRedundancyBlocks |= isLowRedundancyBlock(blockInfo, replication, ecPolicy);

      List<StorageType> expectedStorageTypes;
      if (blockInfo.isStriped()) {
        if (ErasureCodingPolicyManager.checkStoragePolicySuitableForECStripedMode(
            existingStoragePolicy.getId())) {
          expectedStorageTypes =
              existingStoragePolicy.chooseStorageTypes((short) blockInfo.getLocations().length);
        } else {
      ...
      List<StorageType> existing =
          new LinkedList<StorageType>(Arrays.asList(blockInfo.getStorageTypes()));
      if (!removeOverlapBetweenStorageTypes(expectedStorageTypes, existing, true)) {
        boolean blocksPaired =
            computeBlockMovingInfos(
                blockMovingInfos,
                blockInfo,
                expectedStorageTypes,
                existing,
    ...
    for (BlockMovingInfo blkMovingInfo : blockMovingInfos) {
      ...
      try {
        ...
        Set<StorageTypeNodePair> nodesWithStorage = assignedBlocks.get(blkMovingInfo.getBlock());
  ...
  /**
   * The given block is considered as low redundancy when the block locations length is less than
   * expected replication factor. For EC blocks, redundancy is the summation of data + parity
   * blocks.
   *
   * @param blockInfo block
   * @param replication replication factor of the given file block
   * @param ecPolicy erasure coding policy of the given file block
   * @return true if the given block is low redundant.
   */
  private boolean isLowRedundancyBlock(
      LocatedBlock blockInfo, int replication, ErasureCodingPolicy ecPolicy) {
    ...
    hasLowRedundancyBlock = blockInfo.getLocations().length < replication ? true : false;
    return hasLowRedundancyBlock;
  }

  /**
   * Compute the list of block moving information corresponding to the given blockId. This will
   * check that each block location of the given block is satisfying the expected storage policy. If
   * block location is not satisfied the policy then find out the target node with the expected
   * storage type to satisfy the storage policy.
   *
   * @param blockMovingInfos - list of block source and target node pair
   * @param blockInfo - block details
   * @param expectedStorageTypes - list of expected storage type to satisfy the storage policy
   * @param existing - list to get existing storage types
   * @param storages - available storages
   * @param liveDns - live datanodes which can be used as targets
   * @param ecPolicy - ec policy of sps invoked file
   * @return false if some of the block locations failed to find target node to satisfy the storage
   *     policy, true otherwise
   */
  private boolean computeBlockMovingInfos(
      ...
      List<StorageType> expectedStorageTypes,
      List<StorageType> existing,
      DatanodeInfo[] storages,
      DatanodeMap liveDns,
      ErasureCodingPolicy ecPolicy) {
    boolean foundMatchingTargetNodesForBlock = true;
    if (!removeOverlapBetweenStorageTypes(expectedStorageTypes, existing, true)) {
      List<StorageTypeNodePair> sourceWithStorageMap = new ArrayList<StorageTypeNodePair>();
      List<DatanodeInfo> existingBlockStorages =
          new ArrayList<DatanodeInfo>(Arrays.asList(storages));
      ...
      while (iterator.hasNext()) {
        ...
        if (checkSourceAndTargetTypeExists(dnInfo, existing, expectedStorageTypes, liveDns)) {
          sourceWithStorageMap.add(new StorageTypeNodePair(dnInfo.getStorageType(), dnInfo));
          iterator.remove();
      ...
      for (StorageType existingType : existing) {
        ...
        while (iterator.hasNext()) {
          ...
          if (storageType == existingType) {
            iterator.remove();
            sourceWithStorageMap.add(new StorageTypeNodePair(storageType, dnStorageInfo));
            break;
          }
        }
      }
  ...
  /**
   * Find the good target node for each source node for which block storages was misplaced.
   *
   * @param blockMovingInfos - list of block source and target node pair
   * @param blockInfo - Block
   * @param sourceWithStorageList - Source Datanode with storages list
   * @param expectedTypes - Expecting storages to move
   * @param targetDns - Available DNs for expected storage types
   * @param ecPolicy - erasure coding policy of sps invoked file
   * @param excludeNodes - existing source nodes, which has replica copy
   * @return false if some of the block locations failed to find target node to satisfy the storage
   *     policy
   */
    ...
    for (int i = 0; i < sourceWithStorageList.size(); i++) {
      ...
      if (!expectedTypes.contains(existingTypeNodePair.storageType)) {
        StorageTypeNodePair chosenTarget =
            chooseTargetTypeInSameNode(
                blockInfo, existingTypeNodePair.dn, targetDns, expectedTypes);
        if (chosenTarget != null) {
          if (blockInfo.isStriped()) {
            buildStripedBlockMovingInfos(
                blockInfo,
                existingTypeNodePair.dn,
                existingTypeNodePair.storageType,
                chosenTarget.dn,
                chosenTarget.storageType,
                blockMovingInfos,
                ecPolicy);
          } else {
            buildContinuousBlockMovingInfos(
                blockInfo,
                existingTypeNodePair.dn,
                existingTypeNodePair.storageType,
                chosenTarget.dn,
  ...
  private boolean checkIfAlreadyChosen(List<BlockMovingInfo> blockMovingInfos, DatanodeInfo dn) {
  ...
  private void buildContinuousBlockMovingInfos(
      LocatedBlock blockInfo,
      DatanodeInfo sourceNode,
      StorageType sourceStorageType,
      DatanodeInfo targetNode,
    ...
    BlockMovingInfo blkMovingInfo =
        new BlockMovingInfo(blk, sourceNode, targetNode, sourceStorageType, targetStorageType);
    blkMovingInfos.add(blkMovingInfo);
  }

  private void buildStripedBlockMovingInfos(
      LocatedBlock blockInfo,
      DatanodeInfo sourceNode,
      StorageType sourceStorageType,
      DatanodeInfo targetNode,
    ...
    for (int i = 0; i < indices.length; i++) {
      ...
      if (blkIndex >= 0) {
        ...
        if (sourceNode.equals(locations[i])) {
          ...
          long numBytes =
              StripedBlockUtil.getInternalBlockLength(extBlock.getNumBytes(), ecPolicy, blkIndex);
          ...
          BlockMovingInfo blkMovingInfo =
              new BlockMovingInfo(
                  blk, sourceNode, targetNode, sourceStorageType, targetStorageType);
          blkMovingInfos.add(blkMovingInfo);
        }
      }
    }
   ...
   * @param targetDns - set of target datanodes with its respective storage type
    ...
    for (StorageType t : targetTypes) {
      ...
      for (DatanodeWithStorage.StorageDetails targetNode : targetNodeStorages) {
        if (targetNode.getDatanodeInfo().equals(source)) {
          ...
          if (LOG.isDebugEnabled()) {
            LOG.debug(
                "Datanode:{} storage type:{} doesn't have sufficient "
                    + "space:{} to move the target block size:{}",
                source,
                t,
                targetNode,
                blockInfo.getBlockSize());
          }
        }
      }
    }
  ...
  private StorageTypeNodePair chooseTarget(
      LocatedBlock block,
      DatanodeInfo source,
      List<StorageType> targetTypes,
      Matcher matcher,
  ...
  private EnumMap<StorageType, List<DatanodeWithStorage.StorageDetails>>
      findTargetsForExpectedStorageTypes(List<StorageType> expected, DatanodeMap liveDns) {
    EnumMap<StorageType, List<DatanodeWithStorage.StorageDetails>> targetsMap =
        new EnumMap<StorageType, List<DatanodeWithStorage.StorageDetails>>(StorageType.class);
    ...
    for (StorageType storageType : expected) {
      ...
      for (DatanodeWithStorage n : nodes) {
        final DatanodeWithStorage.StorageDetails node = getMaxRemaining(n, storageType);
        if (node != null) {
          listNodes.add(node);
        }
      }
  ...
  private static DatanodeWithStorage.StorageDetails getMaxRemaining(
    ...
    List<DatanodeWithStorage.StorageDetails> storages = node.getNodesWithStorages(storageType);
  ...
  private boolean checkSourceAndTargetTypeExists(
      DatanodeInfo dn,
  ...
  public static class DatanodeMap {
     ...
     *
     * @param node datanode
     * @param storageTypes list of available storage types in the given datanode
     * @param maxSize2Move available space which can be used for scheduling block move
     */
  ...
  /**
   * Keeps datanode with its respective set of supported storage types. It holds the available space
   * in each volumes and will be used while pairing the target datanodes.
   */
  public static final class DatanodeWithStorage {
    ...
    public String toString() {
      return new StringBuilder()
          .append("DatanodeWithStorageInfo(\n  ")
          .append("Datanode: ")
          .append(datanode)
          .append(" StorageTypeNodeMap: ")
          .append(storageMap)
          .append(")")
          .toString();
    }
    ...
    final class StorageDetails {
      ...
      /**
       * Checks whether this datanode storage has sufficient space to occupy the given block size.
       */
   ...
   *
   * @param dnInfo reported datanode
   * @param storageType - storage type
   * @param block movement attempt finished block.
   */
  @Override
  public void notifyStorageMovementAttemptFinishedBlk(
      DatanodeInfo dnInfo, StorageType storageType, Block block) {
  ...
  /**
   * Clear the queues from to be storage movement needed lists and items tracked in storage movement
   * monitor.
   */
  public void clearQueues() {
    LOG.warn(
        "Clearing all the queues from StoragePolicySatisfier. So, "
            + "user requests on satisfying block storages would be discarded.");
    storageMovementNeeded.clearAll();
  }

  /**
   * This class contains information of an attempted blocks and its last attempted or reported time
   * stamp. This is used by {@link
   * BlockStorageMovementAttemptedItems#storageMovementAttemptedItems}.
   */
  static final class AttemptedItemInfo extends ItemInfo {
     ...
     *
     * @param rootId rootId for trackId
     * @param trackId trackId for file.
     * @param lastAttemptedOrReportedTime last attempted or reported time
     * @param blocks scheduled blocks
     * @param retryCount file retry count
     */
    AttemptedItemInfo(
        long rootId,
        long trackId,
  ...
  public void addFileToProcess(ItemInfo trackInfo, boolean scanCompleted) {
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          "Added track info for inode {} to block " + "storageMovementNeeded queue",
          trackInfo.getFile());
    }
  }

  @Override
  public void addAllFilesToProcess(
      long startPath, List<ItemInfo> itemInfoList, boolean scanCompleted) {
   ...
   *
   * @param expected - Expected storage types list.
   * @param existing - Existing storage types list.
   * @param ignoreNonMovable ignore non-movable storage types by removing them from both expected
   *     and existing storage type list to prevent non-movable storage from being moved.
   * @returns if the existing types or the expected types is empty after removing the overlap.
   */
  private static boolean removeOverlapBetweenStorageTypes(
      List<StorageType> expected, List<StorageType> existing, boolean ignoreNonMovable) {
