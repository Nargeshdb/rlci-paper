...
public abstract class NNUpgradeUtil {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(NNUpgradeUtil.class);

  /**
   * Return true if this storage dir can roll back to the previous storage state, false otherwise.
   * The NN will refuse to run the rollback operation unless at least one JM or fsimage storage
   * directory can roll back.
   *
   ...
   */
  static boolean canRollBack(
      StorageDirectory sd, StorageInfo storage, StorageInfo prevStorage, int targetLayoutVersion)
      throws IOException {
  ...
  /**
   * Finalize the upgrade. The previous dir, if any, will be renamed and removed. After this is
   * completed, rollback is no longer allowed.
   *
  ...
  /**
   * Perform any steps that must succeed across all storage dirs/JournalManagers involved in an
   * upgrade before proceeding onto the actual upgrade stage. If a call to any JM's or local storage
   * dir's doPreUpgrade method fails, then doUpgrade will not be called for any JM. The existing
   * current dir is renamed to previous.tmp, and then a new, empty current dir is created.
   *
  ...
  public static void renameCurToTmp(StorageDirectory sd) throws IOException {
    ...
    Preconditions.checkState(
        !prevDir.exists(), "Previous directory must not exist for preupgrade.");
    Preconditions.checkState(
        !tmpDir.exists(),
  ...
  /**
   * Perform the upgrade of the storage dir to the given storage info. The new storage info is
   * written into the current directory, and the previous.tmp directory is renamed to previous.
   *
  ...
  /**
   * Perform rollback of the storage dir to the previous state. The existing current dir is removed,
   * and the previous dir is renamed to current.
   *
  ...
  static void doRollBack(StorageDirectory sd) throws IOException {
    ...
    Preconditions.checkState(
        !tmpDir.exists(),
        "removed.tmp directory must not exist for rollback." + "Consider restarting for recovery.");
    // rename current to tmp
    File curDir = sd.getCurrentDir();
