...
class InvalidateBlocks {
  ...
  InvalidateBlocks(
      final int blockInvalidateLimit, long pendingPeriodInMs, final BlockIdManager blockIdManager) {
  ...
  private void printBlockDeletionTime() {
    ...
    BlockManager.LOG.info(
        "The block deletion will start around {}", sdf.format(calendar.getTime()));
  }
  ...
  /**
   * @return The total number of blocks of type {@link
   *     org.apache.hadoop.hdfs.protocol.BlockType#CONTIGUOUS} to be invalidated.
   */
  long getBlocks() {
  ...
  /**
   * @return The total number of blocks of type {@link
   *     org.apache.hadoop.hdfs.protocol.BlockType#STRIPED} to be invalidated.
   */
  long getECBlocks() {
  ...
  private void putBlocksSet(
      final DatanodeInfo dn, final Block block, final LightWeightHashSet set) {
    if (blockIdManager.isStripedBlock(block)) {
    ...
    if (set.add(block)) {
      ...
      if (log) {
        NameNode.blockStateChangeLog.debug(
            "BLOCK* {}: add {} to {}", getClass().getSimpleName(), block, datanode);
      }
    }
  }
  ...
  private void dumpBlockSet(
      final Map<DatanodeInfo, LightWeightHashSet<Block>> nodeToBlocksMap, final PrintWriter out) {
    for (Entry<DatanodeInfo, LightWeightHashSet<Block>> entry : nodeToBlocksMap.entrySet()) {
  ...
  synchronized void dump(final PrintWriter out) {
    final int size = nodeToBlocks.values().size() + nodeToECBlocks.values().size();
    out.println(
  ...
  /**
   * Get blocks to invalidate by limit as blocks that can be sent in one message is limited.
   *
   * @return the remaining limit
   */
  private int getBlocksToInvalidateByLimit(
      LightWeightHashSet<Block> blockSet,
      List<Block> toInvalidate,
      LongAdder statsAdder,
      int limit) {
  ...
  synchronized List<Block> invalidateWork(final DatanodeDescriptor dn) {
    ...
    if (delay > 0) {
      BlockManager.LOG.debug(
          "Block deletion is delayed during NameNode startup. "
              + "The deletion will start after {} ms.",
          delay);
      return null;
    }
    ...
    if (nodeToBlocks.get(dn) != null) {
      remainingLimit =
          getBlocksToInvalidateByLimit(
              nodeToBlocks.get(dn), toInvalidate, numBlocks, remainingLimit);
    }
    if ((remainingLimit > 0) && (nodeToECBlocks.get(dn) != null)) {
      getBlocksToInvalidateByLimit(
          nodeToECBlocks.get(dn), toInvalidate, numECBlocks, remainingLimit);
    }
