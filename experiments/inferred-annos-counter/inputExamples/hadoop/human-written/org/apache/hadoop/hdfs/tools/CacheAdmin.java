...
public class CacheAdmin extends Configured implements Tool {
  ...
  private static class AddCacheDirectiveInfoCommand implements AdminHelper.Command {
    ...
    public String getLongUsage() {
      ...
      listing.addRow(
          "<pool-name>",
          "The pool to which the directive will be "
  ...
  private static class ModifyCacheDirectiveInfoCommand implements AdminHelper.Command {
    ...
    public String getLongUsage() {
      ...
      listing.addRow(
          "<path>", "A path to cache. The path can be " + "a directory or a file. (optional)");
      listing.addRow("-force", "Skips checking of cache pool resource limits.");
  ...
  private static class RemoveCacheDirectiveInfosCommand implements AdminHelper.Command {
    ...
    public String getLongUsage() {
      ...
      listing.addRow(
          "-path <path>",
          "The path of the cache directives to remove.  "
      ...
      try {
        ...
        RemoteIterator<CacheDirectiveEntry> iter =
            dfs.listCacheDirectives(
                new CacheDirectiveInfo.Builder().setPath(new Path(path)).build());
        while (iter.hasNext()) {
  ...
  private static class ListCacheDirectiveInfoCommand implements AdminHelper.Command {
    ...
    public int run(Configuration conf, List<String> args) throws IOException {
      ...
      TableListing.Builder tableBuilder =
      ...
      try {
        ...
        System.out.print(
            String.format("Found %d entr%s%n", numEntries, numEntries == 1 ? "y" : "ies"));
        if (numEntries > 0) {
    ...
    public String getLongUsage() {
      ...
      listing.addRow(
          ...
          "UNIX-style permissions for the pool. "
              ...
              + String.format("0%03o", FsPermission.getCachePoolDefault().toShort())
              + ".");
      listing.addRow(
          "<limit>",
      ...
      listing.addRow(
          "<maxTtl>",
          "The maximum allowed time-to-live for "
  ...
  private static class ModifyCachePoolCommand implements AdminHelper.Command {
    ...
    public String getLongUsage() {
      ...
      listing.addRow(
          "<defaultReplication>", "Default replication num for " + "directives in this pool");
      listing.addRow(
          "<maxTtl>",
    ...
    public int run(Configuration conf, List<String> args) throws IOException {
      ...
      if (!changed) {
        System.err.println(
            "You must specify at least one attribute to " + "change in the cache pool.");
        return 1;
      }
      DistributedFileSystem dfs = AdminHelper.getDFS(conf);
      try {
  ...
  private static class ListCachePoolsCommand implements AdminHelper.Command {
    ...
    public String getLongUsage() {
      ...
      return getShortUsage()
          + "\n"
          + WordUtils.wrap(
              "Display information about one or more cache pools, "
    ...
    public int run(Configuration conf, List<String> args) throws IOException {
      ...
      System.out.print(
          String.format("Found %d result%s.%n", numResults, (numResults == 1 ? "" : "s")));
      if (numResults > 0) {
