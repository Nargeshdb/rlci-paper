...
/**
 * This class extends the DatanodeInfo class with ephemeral information (eg health, capacity, what
 * blocks are associated with the Datanode) that is private to the Namenode, ie this class is not
 * exposed to clients.
 */
...
public class DatanodeDescriptor extends DatanodeInfo {
  ...
  /**
   * The blocks which we know are cached on this datanode. This list is updated by periodic cache
   * reports.
   */
  ...
  /** The time when the last batch of caching directives was sent, in monotonic milliseconds. */
  ...
  /**
   * When set to true, the node is not in include list and is not allowed to communicate with the
   * namenode
   */
  ...
  void updateHeartbeat(
    ...
    updateHeartbeatState(
        reports, cacheCapacity, cacheUsed, xceiverCount, volFailures, volumeFailureSummary);
  ...
  private void updateStorageStats(
    ...
    if (checkFailedStorages) {
      if (this.volumeFailures != volFailures) {
        LOG.info(
            "Number of failed storages changes from {} to {}", this.volumeFailures, volFailures);
      }
      synchronized (storageMap) {
  ...
  /**
   * Remove stale storages from storageMap. We must not remove any storages as long as they have
   * associated block replicas.
   */
  private void pruneStorageMap(final StorageReport[] reports) {
    synchronized (storageMap) {
      LOG.debug(
          "Number of storages reported in heartbeat={};" + " Number of storages in storageMap={}",
          reports.length,
          storageMap.size());
      ...
      for (final DatanodeStorageInfo storageInfo : excessStorages.values()) {
        if (storageInfo.numBlocks() == 0) {
          DatanodeStorageInfo info = storageMap.remove(storageInfo.getStorageID());
          if (!hasStorageType(info.getStorageType())) {
            ...
            if (getParent() instanceof DFSTopologyNodeImpl) {
              ((DFSTopologyNodeImpl) getParent())
                  .childRemoveStorage(getName(), info.getStorageType());
            }
          }
  ...
  private static class BlockIterator implements Iterator<BlockInfo> {
    ...
    private BlockIterator(final int startBlock, final DatanodeStorageInfo... storages) {
      if (startBlock < 0) {
  ...
  void addBlockToBeErasureCoded(
      ExtendedBlock block,
      DatanodeDescriptor[] sources,
      DatanodeStorageInfo[] targets,
      byte[] liveBlockIndices,
      ErasureCodingPolicy ecPolicy) {
    assert (block != null && sources != null && sources.length > 0);
    BlockECReconstructionInfo task =
        new BlockECReconstructionInfo(block, sources, targets, liveBlockIndices, ecPolicy);
    erasurecodeBlocks.offer(task);
    BlockManager.LOG.debug(
        "Adding block reconstruction task "
            + task
            + "to "
            + getName()
  ...
  public List<BlockECReconstructionInfo> getErasureCodeCommand(int maxTransfers) {
  ...
  /**
   * Find whether the datanode contains good storage of given type to place block of size <code>
   * blockSize</code>.
   *
   * <p>Currently datanode only cares about the storage type, in this method, the first storage of
   * given type we see is returned.
   *
  ...
  public class LeavingServiceStatus {
    ...
    synchronized void set(
        int lowRedundancyBlocksInOpenFiles,
  ...
  /**
   * @return The time at which we last sent caching directives to this DataNode, in monotonic
   *     milliseconds.
   */
  ...
  /**
   * @param time The time at which we last sent caching directives to this DataNode, in monotonic
   *     milliseconds.
   */
  ...
  public boolean checkBlockReportReceived() {
    ...
    for (DatanodeStorageInfo storageInfo : this.getStorageInfos()) {
      if (storageInfo.getBlockReportCount() == 0) return false;
    }
    return true;
  }
