...
public class StoragePolicySummary {
  ...
  static List<Entry<StorageTypeAllocation, Long>> sortByComparator(
    ...
    Collections.sort(
        ...
        new Comparator<Entry<StorageTypeAllocation, Long>>() {
          public int compare(
              Entry<StorageTypeAllocation, Long> o1, Entry<StorageTypeAllocation, Long> o2) {
      ...
      if (sta.policyMatches()) {
        ...
        if (nonCompliantBlocksSB.length() == 0) {
          nonCompliantBlocksSB
              ...
              .append("\nStorage Policy                  ")
              .append("Specified Storage Policy      # of blocks       % of blocks\n");
        }
        nonCompliantFormatter.format(
            "%-35s %-20s %10d  %20s%n",
            sta.getStoragePolicyDescriptor(),
  ...
  static class StorageTypeAllocation {
    ...
    private static String getStorageAllocationAsString(
      ...
      for (Map.Entry<StorageType, Integer> storageTypeCountEntry :
          storageType_countmap.entrySet()) {
        sb.append(
            storageTypeCountEntry.getKey().name() + ":" + storageTypeCountEntry.getValue() + ",");
      }
      if (sb.length() > 1) {
    ...
    String getStoragePolicyDescriptor() {
      ...
      if (actualStoragePolicy != null) {
        storagePolicyDescriptorSB
            .append(getStorageAllocationAsString())
            .append("(")
    ...
    public boolean equals(Object another) {
      return (another instanceof StorageTypeAllocation
          && Objects.equals(
              specifiedStoragePolicy, ((StorageTypeAllocation) another).specifiedStoragePolicy)
          && Arrays.equals(storageTypes, ((StorageTypeAllocation) another).storageTypes));
    }
  }
}
