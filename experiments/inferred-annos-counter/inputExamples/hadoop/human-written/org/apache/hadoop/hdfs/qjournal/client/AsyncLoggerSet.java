...
/**
 * Wrapper around a set of Loggers, taking care of fanning out calls to the underlying loggers and
 * constructing corresponding {@link QuorumCall} instances.
 */
class AsyncLoggerSet {
  ...
  /**
   * Set the highest successfully committed txid seen by the writer. This should be called after a
   * successful write to a quorum, and is used for extra sanity checks against the protocol. See
   * HDFS-3863.
   */
  ...
  /**
   * @return the epoch number for this writer. This may only be called after a successful call to
   *     {@link #createNewUniqueEpoch(NamespaceInfo)}.
   */
  long getEpoch() {
  ...
  /**
   * Wait for a quorum of loggers to respond to the given call. If a quorum can't be achieved,
   * throws a QuorumException.
   *
   ...
   */
  <V> Map<AsyncLogger, V> waitForWriteQuorum(
      QuorumCall<AsyncLogger, V> q, int timeoutMs, String operationName) throws IOException {
  ...
  /**
   * @return the (mutable) list of loggers, for use in tests to set up spies
   */
  @VisibleForTesting
  ...
  QuorumCall<AsyncLogger, Void> format(NamespaceInfo nsInfo, boolean force) {
    Map<AsyncLogger, ListenableFuture<Void>> calls = Maps.newHashMap();
    for (AsyncLogger logger : loggers) {
      ListenableFuture<Void> future = logger.format(nsInfo, force);
      calls.put(logger, future);
    }
    return QuorumCall.create(calls);
  }
  ...
  public QuorumCall<AsyncLogger, Boolean> canRollBack(
