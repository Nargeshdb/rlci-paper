...
/**
 * PBImageXmlWriter walks over an fsimage structure and writes out an equivalent XML document that
 * contains the fsimage's components.
 */
...
public final class PBImageXmlWriter {
  ...
  private void dumpCacheManagerSection(InputStream is) throws IOException {
    ...
    for (int i = 0; i < s.getNumDirectives(); ++i) {
      ...
      o(SECTION_ID, p.getId())
          .o(SECTION_PATH, p.getPath())
  ...
  private void dumpXattrs(INodeSection.XAttrFeatureProto xattrs) {
    ...
    for (INodeSection.XAttrCompactProto xattr : xattrs.getXAttrsList()) {
      ...
      int ns =
          (XATTR_NAMESPACE_MASK & (encodedName >> XATTR_NAMESPACE_OFFSET))
              | ((XATTR_NAMESPACE_EXT_MASK & (encodedName >> XATTR_NAMESPACE_EXT_OFFSET)) << 2);
      o(INODE_SECTION_NS, XAttrProtos.XAttrProto.XAttrNamespaceProto.forNumber(ns).toString());
      o(
          SECTION_NAME,
          SerialNumberManager.XATTR.getString(
              XATTR_NAME_MASK & (encodedName >> XATTR_NAME_OFFSET), stringTable));
  ...
  private void dumpAcls(AclFeatureProto aclFeatureProto) {
    ImmutableList<AclEntry> aclEntryList =
        FSImageFormatPBINode.Loader.loadAclEntries(aclFeatureProto, stringTable);
  ...
  private void dumpErasureCodingSection(InputStream in) throws IOException {
    ...
    if (s.getPoliciesCount() > 0) {
      ...
      for (int i = 0; i < s.getPoliciesCount(); ++i) {
        ...
        dumpErasureCodingPolicy(PBHelperClient.convertErasureCodingPolicyInfo(policy));
      }
      out.println("</" + ERASURE_CODING_SECTION_NAME + ">\n");
    }
  }
  ...
  private void dumpErasureCodingPolicy(ErasureCodingPolicyInfo ecPolicyInfo) {
    ...
    o(ERASURE_CODING_SECTION_SCHEMA_PARITY_UNITS, schema.getNumParityUnits());
    if (schema.getExtraOptions().size() > 0) {
      out.println("<" + ERASURE_CODING_SECTION_SCHEMA_OPTIONS + ">");
      for (Map.Entry<String, String> option : schema.getExtraOptions().entrySet()) {
  ...
  private void dumpINodeFields(INodeSection.INode p) {
    o(SECTION_ID, p.getId())
        .o(INODE_SECTION_TYPE, p.getType())
  ...
  private String dumpPermission(long permission) {
    PermissionStatus permStatus =
        FSImageFormatPBINode.Loader.loadPermission(permission, stringTable);
    return String.format(
        "%s:%s:%04o",
        permStatus.getUserName(),
        permStatus.getGroupName(),
        ...
        case DIRECTORYDIFF:
          {
            for (int i = 0; i < e.getNumOfDiff(); ++i) {
              ...
              SnapshotDiffSection.DirectoryDiff d =
              ...
              for (int j = 0; j < d.getCreatedListSize(); ++j) {
                SnapshotDiffSection.CreatedListEntry ce =
                    SnapshotDiffSection.CreatedListEntry.parseDelimitedFrom(in);
