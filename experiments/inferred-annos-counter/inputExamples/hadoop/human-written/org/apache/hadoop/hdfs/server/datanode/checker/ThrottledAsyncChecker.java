...
/**
 * An implementation of {@link AsyncChecker} that skips checking recently checked objects. It will
 * enforce at least {@link minMsBetweenChecks} milliseconds between two successive checks of any one
 * object.
 *
 * <p>It is assumed that the total number of Checkable objects in the system is small, (not more
 * than a few dozen) since the checker uses O(Checkables) storage and also potentially O(Checkables)
 * threads.
 *
 * <p>{@link minMsBetweenChecks} should be configured reasonably by the caller to avoid spinning up
 * too many threads frequently.
 */
...
public class ThrottledAsyncChecker<K, V> implements AsyncChecker<K, V> {
  ...
  /**
   * The minimum gap in milliseconds between two successive checks of the same object. This is the
   * throttle.
   */
  ...
  /** Map of checks that are currently in progress. Protected by the object lock. */
  private final Map<Checkable, ListenableFuture<V>> checksInProgress;

  /**
   * Maps Checkable objects to a future that can be used to retrieve the results of the operation.
   * Protected by the object lock.
   */
  private final Map<Checkable, LastCheckResult<V>> completedChecks;

  public ThrottledAsyncChecker(
      final Timer timer,
   ...
   *
   * <p>If the object has been checked recently then the check will be skipped. Multiple concurrent
   * checks for the same object will receive the same Future.
   */
  @Override
  public Optional<ListenableFuture<V>> schedule(Checkable<K, V> target, K context) {
    ...
    if (diskCheckTimeout > 0) {
      lf =
          TimeoutFuture.create(
              lfWithoutTimeout, diskCheckTimeout, TimeUnit.MILLISECONDS, scheduledExecutorService);
    } else {
      lf = lfWithoutTimeout;
    }
   ...
   * @param lf
   */
  private void addResultCachingCallback(Checkable<K, V> target, ListenableFuture<V> lf) {
    Futures.addCallback(
        lf,
        new FutureCallback<V>() {
          ...
          public void onFailure(@Nonnull Throwable t) {
            synchronized (ThrottledAsyncChecker.this) {
              checksInProgress.remove(target);
              completedChecks.put(target, new LastCheckResult<>(t, timer.monotonicNow()));
            }
          }
        },
   ...
   * {@inheritDoc}.
   *
   * <p>The results of in-progress checks are not useful during shutdown, so we optimize for faster
   * shutdown by interrupt all actively executing checks.
   */
  @Override
  ...
  /**
   * Status of running a check. It can either be a result or an exception, depending on whether the
   * check completed or threw.
   */
