...
public class HFileInfo implements SortedMap<byte[], byte[]> {
   ...
   */
  @SuppressWarnings("builder:required.method.not.called") // FP: for SequenceInputStream sis, we
  // would like to express that since both incoming InputStreams are @MustCall({}) and/or
  // @NotOwning, sis should be @MustCall({}).  The checker cannot currently capture and verify such
  // a property.
